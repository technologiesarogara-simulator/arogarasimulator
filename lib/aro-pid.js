...
(function () {
  ...
  function update3D(r) {
    ...
    r.legs.forEach(function (l) {
      ...
      /* A colour the engineer picked in 2D is a decision about the line, so
         it carries into 3D. Left alone, the leg is coloured by velocity as a
         diagnostic instead. */
      var col = s.userColour ? new THREE.Color(s.colour).getHex()
        : (!isFinite(l.V) ? 0x64748b : l.V > r.tot.Vallow ? 0xef4444 : l.V > 4.5 ? 0xf59e0b : l.V < 0.9 ? 0x38bdf8 : 0x22c55e);
  ...
  function tagText(r) {
    ...
    return r.legs.length + ' legs · ' + f2(r.tot.L) + ' m · ' + DOC.items.length + ' components · '
      + (picked ? (picked === DOC.segs.length ? 'colours as set on the P&ID' : picked + ' leg' + (picked > 1 ? 's' : '') + ' coloured on the P&ID, the rest by velocity')

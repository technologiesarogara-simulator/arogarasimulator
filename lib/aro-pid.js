...
(function () {
  ...
  var LIB = [
    { key: 'gate',    name: 'Gate valve',        k: 0.15, sym: 'valve',  ico: 'wheel',  colour: '#38bdf8',
      note: 'on/off isolation — full bore when open, so it costs almost nothing in ΔP' },
    { key: 'globe',   name: 'Globe valve',       k: 6.00, sym: 'globe',  ico: 'wheel',  colour: '#f59e0b',
      note: 'throttling duty — the seat turns the flow twice, which is why K is 6 and not 0.15' },
    { key: 'ball',    name: 'Ball valve',        k: 0.05, sym: 'ball',   ico: 'lever',  colour: '#22c55e',
      note: 'quarter-turn isolation, the lowest loss in the library — quick shutoff, poor for throttling' },
    { key: 'butterfly', name: 'Butterfly valve', k: 0.86, sym: 'bfly',   ico: 'lever',  colour: '#a78bfa',
      note: 'compact isolation for large bore; the disc sits in the stream even when open' },
    { key: 'check',   name: 'Swing check valve', k: 2.00, sym: 'check',  ico: 'plain',  colour: '#ef4444',
      note: 'prevents reverse flow — needed wherever the line lifts or two pumps share a header' },
    { key: 'lcheck',  name: 'Lift check valve',  k: 10.0, sym: 'check',  ico: 'plain',  colour: '#ef4444',
      note: 'tighter shutoff than a swing check but a heavy K — use only where leak-back matters' },
    { key: 'plug',    name: 'Plug valve',        k: 0.40, sym: 'ball',   ico: 'lever',  colour: '#22c55e',
      note: 'quarter-turn isolation for dirty service' },
    ...
    { key: 'e90',     name: 'Elbow 90° std',     k: 0.75, sym: 'bend',   ico: 'bend',   colour: '#94a3b8',
      note: 'a square change of direction' },
    { key: 'e90lr',   name: 'Elbow 90° long rad', k: 0.45, sym: 'bend',  ico: 'bend',   colour: '#94a3b8',
      note: 'R/D 1.5 — 40 % less loss than a standard elbow and kinder to erosive service' },
    { key: 'e45',     name: 'Elbow 45°',         k: 0.35, sym: 'bend',   ico: 'bend',   colour: '#94a3b8',
      note: 'a shallow offset; two of these often beat one 90°' },
    { key: 'teeR',    name: 'Tee — through run', k: 0.40, sym: 'tee',    ico: 'tee',    colour: '#94a3b8',
      note: 'straight through the header' },
    { key: 'teeB',    name: 'Tee — branch',      k: 1.50, sym: 'tee',    ico: 'tee',    colour: '#94a3b8',
      note: 'the flow turns into the branch, costing nearly four times the through path' },
    ...
    { key: 'strain',  name: 'Strainer',          k: 2.00, sym: 'strain', ico: 'plain',  colour: '#fbbf24',
      note: 'protects the downstream machine; the K rises sharply as it blinds, so allow margin' },
    { key: 'orifice', name: 'Orifice plate',     k: 2.50, sym: 'orif',   ico: 'plain',  colour: '#fbbf24',
      note: 'flow measurement — the permanent loss is real and belongs in the hydraulics' },
    { key: 'flange',  name: 'Flange pair',       k: 0.00, sym: 'flange', ico: 'plain',  colour: '#64748b',
  ...
  function addItem(key, segId, t) {
    var it = { id: UID++, key: key, segId: segId, t: t == null ? 0.5 : t };
  ...
  function legs(overrideNps, overrideSch) {
    ...
    return DOC.segs.map(function (s, i) {
      ...
      var sumK = mine.reduce(function (a, it) { return a + lib(it.key).k; }, 0);
  ...
  function draw() {
    ...
    if (!DOC.segs.length) {
      ...
      CTX.fillText('LINE tool: click the FROM point, then click at each change of direction.', 18, 26);
      CTX.fillText('Pick a component from the library, then click on a line to place it.', 18, 46);
      return;
    }
    ...
    DOC.segs.forEach(function (s, i) {
      ...
      CTX.fillText('L' + (i + 1) + '  ' + f1(segLen(s, scale)) + ' m  ·  ' + s.nps + '" S' + s.sch, mx + 8, my - 9);

      [[s.x1, s.y1], [s.x2, s.y2]].forEach(function (p) {
        CTX.fillStyle = '#0b1220'; CTX.strokeStyle = '#64748b'; CTX.lineWidth = 2;
        CTX.beginPath(); CTX.arc(p[0], p[1], 4.5, 0, Math.PI * 2); CTX.fill(); CTX.stroke();
      });
    });

    DOC.items.forEach(function (it) {
      var s = segById(it.segId); if (!s) return;
      var p = itemXY(it);
      var ang = Math.atan2(s.y2 - s.y1, s.x2 - s.x1);
      var c = lib(it.key);
      symbol(CTX, p.x, p.y, ang, c, SEL && SEL.kind === 'item' && SEL.id === it.id);
      CTX.fillStyle = c.colour; CTX.font = '9px monospace';
      CTX.fillText(c.name, p.x + 12, p.y + 16);
    });
  ...
  function icon3D(c, R) {
    ...
    [-R * 1.4, R * 1.4].forEach(function (dx) {
      var fl = new THREE.Mesh(new THREE.CylinderGeometry(R * 1.8, R * 1.8, R * 0.35, 20), mat);
      fl.rotation.z = Math.PI / 2; fl.position.x = dx; g.add(fl);
    });
    if (c.ico === 'wheel') {
      var stem = new THREE.Mesh(new THREE.CylinderGeometry(R * 0.22, R * 0.22, R * 2.4, 10), mat);
      stem.position.y = R * 2.0; g.add(stem);
      var wh = new THREE.Mesh(new THREE.TorusGeometry(R * 1.1, R * 0.16, 8, 20), mat);
      wh.position.y = R * 3.1; wh.rotation.x = Math.PI / 2; g.add(wh);
    } else if (c.ico === 'lever') {
      var lv = new THREE.Mesh(new THREE.BoxGeometry(R * 0.3, R * 0.3, R * 3.0), mat);
      lv.position.y = R * 2.0; g.add(lv);
      var st2 = new THREE.Mesh(new THREE.CylinderGeometry(R * 0.22, R * 0.22, R * 1.8, 10), mat);
      st2.position.y = R * 1.4; g.add(st2);
    } else if (c.ico === 'actuator') {
      var act = new THREE.Mesh(new THREE.CylinderGeometry(R * 1.5, R * 1.5, R * 1.1, 18), mat);
      act.position.y = R * 3.0; g.add(act);
      var st3 = new THREE.Mesh(new THREE.CylinderGeometry(R * 0.22, R * 0.22, R * 2.2, 10), mat);
  ...
  function advise(r) {
    ...
    Object.keys(placed).forEach(function (k) {
      var c = lib(k);
      out.push({ w: c.name + ' ×' + placed[k], h: 'K ' + c.k + ' each  ·  ' + f4(placed[k] * c.k * 0.5 * num('pid-rho', 998.2) * Math.pow(r.legs[0].V, 2) / 1e5) + ' bar at L1 velocity',
  ...
  function syncSelPanel() {
    ...
    if (SEL.kind === 'seg') {
      ...
      $('pid-sel-title').textContent = lib(it.key).name.toUpperCase() + ' SELECTED  ·  K ' + lib(it.key).k;
  ...
  function paletteHTML() {
    return '<div id="pid-palette" style="display:grid;grid-template-columns:1fr 1fr;gap:4px;">'
      + LIB.map(function (c) {
        return '<button class="pid-lib" data-key="' + c.key + '" title="K = ' + c.k + '" style="text-align:left;background:rgba(2,6,18,0.6);border:1px solid var(--border-muted);color:' + c.colour + ';font-family:var(--font-mono);font-size:9px;font-weight:700;padding:5px 6px;border-radius:3px;cursor:pointer;">'
          + esc(c.name) + '<span style="color:#64748b;"> K' + c.k + '</span></button>';
      }).join('') + '</div>';
  }
  ...
  function panelHTML() {
    ...
    h += '<div id="pid-selbox" style="display:none;margin-top:10px;border:1px solid var(--color-saffron);border-radius:5px;padding:8px;background:rgba(255,117,56,0.06);">'
      ...
      + '<label style="display:block;margin:4px 0;font-family:var(--font-mono);font-size:10px;color:var(--text-muted);">COLOUR'
    ...
    var h = '<div style="font-family:Arial,sans-serif;color:#111827;">'
      ...
      + r.legs.map(function (l) {
          return '<tr>' + ['L' + l.idx, l.nps + '" S' + l.sch, f2(l.L), f2(l.dz), f2(l.V), Math.round(l.Re).toLocaleString(),
            ...
            (l.items.length ? l.items.map(function (i) { return esc(lib(i.key).name); }).join(', ') : '—')]
      ...
      + DOC.items.map(function (it) {
          ...
          return '<tr><td style="padding:4px;border-bottom:1px solid #e5e7eb;">' + esc(c.name) + '</td><td style="padding:4px;border-bottom:1px solid #e5e7eb;">L' + (DOC.segs.indexOf(s) + 1) + '</td><td style="padding:4px;border-bottom:1px solid #e5e7eb;text-align:right;">' + c.k + '</td></tr>';
        }).join('')
  ...
  function onDown(e) {
    ...
    if (TOOL === 'line') {
      var from = lastEnd();
      if (!from) { DOC.pending = p; addSeg(p.x, p.y, p.x, p.y); }
      else {
        var last = DOC.segs[DOC.segs.length - 1];
        if (last.x1 === last.x2 && last.y1 === last.y2) { last.x2 = p.x; last.y2 = p.y; }
        else addSeg(from.x, from.y, p.x, p.y);
      }
      render(); return;
    }
    if (TOOL === 'delete') {
    ...
    if (hs2) { SEL = { kind: 'seg', id: hs2.seg.id }; syncSelPanel(); draw(); return; }
  ...
  function wire() {
    ...
    $('pid-clear').addEventListener('click', function () { DOC.segs = []; DOC.items = []; SEL = null; LASTR = null; render(); });
  ...
  window.AROPID = {
    ...
    doc: DOC, lib: LIB, setTool: setTool,

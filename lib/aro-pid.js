...
(function () {
  ...
  function update3D(r) {
    ...
    var flowMat = new THREE.MeshBasicMaterial({ color: 0xeaffff, transparent: true, opacity: 0.95, blending: THREE.AdditiveBlending, depthWrite: false });
    ...
    r.legs.forEach(function (l) {
      ...
      /* A pipe that only sits there reads as a solid rod, not a line in
         service. A row of small glowing beads riding the OUTSIDE of the
         pipe — not its centreline, which an opaque metal wall simply hides —
         travels from L(idx) towards the next joint at a speed keyed to the
         leg's own velocity. Rotating pipe.rotation.z only ever turns the
         model in the XY plane, so a fixed world-Z lift keeps a bead sitting
         visibly on "top" of the pipe regardless of which way the leg runs.
         They move from x1,y1 to x2,y2 in the drawn segment order, the same
         order the FROM/TO markers use in 2D, so the direction always
         matches the actual flow through the route. */
      if (isFinite(l.V) && l.V > 0) {
        var ux1 = (sbx - sax) / slen, uy1 = (sby - say) / slen;
        var beadR = Math.max(0.045, R * 0.4);
        var liftZ = R * 1.25 + beadR * 0.6;
        var nDrops = Math.max(3, Math.min(10, Math.round(slen / (R * 4))));
        var spd = Math.max(0.6, Math.min(9, l.V)) * (slen / nDrops) * 0.9;   // model-units/sec
        for (var fi = 0; fi < nDrops; fi++) {
          var sp = new THREE.Mesh(new THREE.SphereGeometry(beadR, 10, 8), flowMat);
          var t0 = fi / nDrops;
          sp.position.set(sax + ux1 * slen * t0, say + uy1 * slen * t0, liftZ);
          g.add(sp);
          flowParts.push({ m: sp, ax: sax, ay: say, ux: ux1, uy: uy1, len: slen, t: t0, spd: spd, z: liftZ });
        }
      }
    ...
    three.anim = flowParts.length ? function (dt) {
      for (var i = 0; i < flowParts.length; i++) {
        var p = flowParts[i];
        p.t += (p.spd * dt) / p.len;
        if (p.t > 1) p.t -= 1;
        p.m.position.set(p.ax + p.ux * p.len * p.t, p.ay + p.uy * p.len * p.t, p.z);
      }
    } : null;
  ...
  function setView3D(on) {
    ...
    var b2 = $('pid-v2d'), b3 = $('pid-v3d'), bc = $('pid-colmode');
  ...
  function tagText(r) {
    ...
    return r.legs.length + ' legs · ' + f2(r.tot.L) + ' m · ' + DOC.items.length + ' components · '
      ...
      + ' · flow animated in the direction drawn · drag to rotate, scroll to zoom';
  }

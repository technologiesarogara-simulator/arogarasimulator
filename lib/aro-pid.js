...
(function () {
  ...
  /* Real-world pipe finish by MOC, for the 3D view's MATERIAL colour mode —
     the same key set as the MAT table in aro-linesize.js's "PIPE MATERIAL"
     select, which is what actually feeds S.mat() below. Velocity colouring
     stays the default (it is the design-review signal); this is what the
     line actually looks like when that toggle is switched on. */
  var MATCOLOR = {
    'CS': 0x5b6472, 'MS': 0x5b6472, 'Commercial steel / welded steel': 0x5b6472, 'Alloy Steel': 0x64707e,
    'GI': 0xb8c2cf, 'Wrought iron': 0x4b5563, 'Riveted steel': 0x5b6472,
    'SS304': 0xd8dde3, 'SS304L': 0xd8dde3, 'SS316': 0xd8dde3, 'SS316L': 0xd8dde3, 'SS321': 0xd8dde3,
    'SS310': 0xd8dde3, 'Duplex SS': 0xd6d4cc, 'Super Duplex SS': 0xd6d4cc,
    'Cast iron': 0x33383f, 'Asphalted cast iron': 0x1c1f24, 'Concrete': 0x9c9284,
    'Copper': 0xb87333, 'Brass': 0xd4a13a,
    'PVC': 0xd8d2c4, 'CPVC': 0xcbb994, 'HDPE': 0x1c1c1c, 'FRP': 0x8a8f79,
    'PTFE Lined': 0xf5f5f0, 'Rubber Lined': 0x1a1a1a,
    'Hastelloy C276': 0x8a8d90, 'Monel 400': 0x9c9c94, 'Inconel 600/625': 0x8f8f88,
    'User defined': 0x8a94a3
  };
  var COLOUR_MODE = 'velocity';                              // 'velocity' | 'material' — toggled from the 3D toolbar
  var FLOW_VISIBLE = true;                                   // fluid-flow animation on/off — toggled from the 3D toolbar
  var BG_COLOR = '#eef1f4';                                  // canvas/scene background — user-set from the toolbar swatch
  // MOVE/PAN is one tool (TOOL === 'pan') shared by both the 2D and 3D
  // views — see init3D()'s mousemove handler for what it does in 3D.
  ...
  function kOf(key, nps, it) {
    var c = lib(key);
    if ((key === 'red' || key === 'ered') && it && it.d1 && it.d2) return redK(it);
    return (c.ki != null) ? kBand(nps)[c.ki] : c.k;
  }

  /* A reducer's loss is set by the size change it makes, not by a table
     entry. Contraction uses the standard sudden-contraction form damped for
     a shaped cone; expansion uses the Borda-Carnot term. Both are referred
     to the SMALL bore, which is where the velocity head is highest. */
  function redK(it) {
    var a = bore(it.d1, it.sch || '40').D, b = bore(it.d2, it.sch || '40').D;
    if (!(a > 0) || !(b > 0) || a === b) return 0.04;
    var beta = Math.min(a, b) / Math.max(a, b), b2 = beta * beta;
    var k = (b < a) ? 0.5 * (1 - b2) * 0.5      /* gradual contraction ≈ half a sudden one */
                    : Math.pow(1 - b2, 2);      /* gradual expansion, cone assumed ≤ 20° */
    return Math.max(0.02, Math.round(k * 100) / 100);
  }
  /* The next size down / up in the ASME table, used for the default pair. */
  function stepNps(n, dir) {
    var k = Object.keys(PIPE).map(parseFloat).sort(function (x, y) { return x - y; });
    var i = k.indexOf(parseFloat(n));
    if (i < 0) return n;
    return k[Math.min(k.length - 1, Math.max(0, i + dir))];
  }
  ...
  var TAGP = { gate:'GV', globe:'GLV', ball:'BV', butterfly:'BFV', check:'NRV', lcheck:'NRV', plug:'PV',
              ...
              red:'RED', strain:'ST', orifice:'FE', flange:'FL', flgFF:'FL', flgRTJ:'FL',
              flgWN:'FL', flgSO:'FL', flgSW:'FL', flgTHD:'FL', flgLAP:'FL', flgBLD:'BL', spec:'SB',
              needle:'NV', diaphragm:'DV', tstrain:'ST', ered:'RED', spade:'SP', ringspacer:'RS',
              hpv:'HPV', lpd:'LPD', funnel:'FN' };
  var TAGN = {};
  ...
  var LIB = [
    ...
    { key: 'needle',  name: 'Needle valve',      k: 9.00, sym: 'needle', ico: 'needle', colour: '#0ea5e9',
      note: 'a tapered stem against a fine seat for precise low-flow trim — small bore, so its K is high even wide open' },
    { key: 'diaphragm', name: 'Diaphragm valve', k: 2.30, sym: 'diaphragm', ico: 'diaphragm', colour: '#f59e0b',
      note: 'a flexible liner pinched by the diaphragm — no packing in the flow path, so it is the choice for slurries and corrosives' },
    ...
    { key: 'red',     name: 'Reducer — concentric', k: 0.30, sym: 'red', ico: 'red', colour: '#94a3b8',
      note: 'a size change on the pipe centreline — fine on discharge lines, but leaves a pocket at a pump suction' },
    { key: 'ered',    name: 'Reducer — eccentric', k: 0.30, sym: 'ered', ico: 'ered', colour: '#94a3b8',
      note: 'flat on top (or bottom, flipped) so there is no low or high pocket — the correct reducer right at a pump suction' },
    { key: 'strain',  name: 'Strainer — Y-type', k: 2.00, sym: 'strain', ico: 'strain', colour: '#fbbf24',
      note: 'a Y-pattern bowl angled off the run; protects the downstream machine, and the K rises sharply as it blinds, so allow margin' },
    { key: 'tstrain', name: 'Strainer — T-type / basket', k: 1.20, sym: 'tstrain', ico: 'tstrain', colour: '#fbbf24',
      note: 'a straight-through body with a top-pulled basket — lower loss and a bigger screen area than a Y-type, at the cost of headroom to lift the basket' },
    ...
    { key: 'flange',  name: 'Flange RF',          k: 0.00, sym: 'flgRF',  ico: 'flgRF',  colour: '#64748b',
      note: 'raised-face flange pair, the general-purpose joint on ASME pipework; the gasket sits on the raised ring so bolt load is concentrated where it seals' },
    { key: 'flgFF',   name: 'Flange FF',          k: 0.00, sym: 'flgFF',  ico: 'flgFF',  colour: '#64748b',
      note: 'flat-face flange, used against cast-iron or plastic equipment where a raised face would crack the mating flange' },
    { key: 'flgRTJ',  name: 'Flange RTJ',         k: 0.00, sym: 'flgRTJ', ico: 'flgRTJ', colour: '#64748b',
      note: 'ring-type joint, the high-pressure and high-temperature choice — a metal ring seats in a machined groove' },
    { key: 'flgWN',   name: 'Flange weld neck',   k: 0.00, sym: 'flgWN',  ico: 'flgWN',  colour: '#64748b',
      note: 'weld-neck flange with a tapered hub butt-welded to the pipe; the strongest joint and the one to use in cyclic or high-stress service' },
    { key: 'flgSO',   name: 'Flange slip-on',     k: 0.00, sym: 'flgSO',  ico: 'flgSO',  colour: '#64748b',
      note: 'slip-on flange, slid over the pipe and fillet-welded both sides — cheaper to fit but weaker in fatigue than a weld neck' },
    { key: 'flgSW',   name: 'Flange socket weld', k: 0.05, sym: 'flgSW',  ico: 'flgSW',  colour: '#64748b',
      note: 'socket-weld flange for small-bore lines; the pipe bottoms into a socket, leaving a small step the flow has to cross' },
    { key: 'flgTHD',  name: 'Flange threaded',    k: 0.05, sym: 'flgTHD', ico: 'flgTHD', colour: '#64748b',
      note: 'threaded flange for small-bore or non-weldable material; avoid where the joint sees vibration or thermal cycling' },
    { key: 'flgLAP',  name: 'Flange lap joint',   k: 0.00, sym: 'flgLAP', ico: 'flgLAP', colour: '#64748b',
      note: 'lap-joint flange over a stub end — the flange can rotate for bolt-hole alignment and can be a cheaper material than the pipe' },
    { key: 'flgBLD',  name: 'Blind flange',       k: 0.00, sym: 'flgBLD', ico: 'flgBLD', colour: '#94a3b8',
      note: 'blind flange closing the line — a dead end for the hydraulics, shown so the drawing records the termination' },
    { key: 'spec',    name: 'Spectacle blind',    k: 0.00, sym: 'spec',   ico: 'spec',   colour: '#fbbf24',
      note: 'spectacle blind giving a positive, visible isolation for maintenance; swung to the open side it costs nothing in the flow' },
    { key: 'spade',   name: 'Spade blind (paddle)', k: 0.00, sym: 'spade', ico: 'spade', colour: '#94a3b8',
      note: 'a single solid paddle dropped between two flanges — the maintenance isolation on a line too small or too crowded for a spectacle blind' },
    { key: 'ringspacer', name: 'Ring spacer',    k: 0.00, sym: 'ringspacer', ico: 'ringspacer', colour: '#64748b',
      note: 'an open ring the same thickness as its spade, restoring the gap when the line is back in service' },
    { key: 'hpv',     name: 'High point vent',   k: 0.00, sym: 'hpv',    ico: 'hpv',    colour: '#38bdf8',
      note: 'a small valved nozzle at the crest of a run — bleeds trapped air or vapour so the line fills and gauges read true' },
    { key: 'lpd',     name: 'Low point drain',   k: 0.00, sym: 'lpd',    ico: 'lpd',    colour: '#fbbf24',
      note: 'a small valved nozzle at the sag of a run — empties the line for maintenance and clears settled solids or condensate' },
    { key: 'funnel',  name: 'Drain funnel / tundish', k: 0.00, sym: 'funnel', ico: 'funnel', colour: '#94a3b8',
      note: 'an open hopper under a drain or vent so the discharge is seen and directed, never sprayed loose' },
    /* ──────────────────────────────────────────────────
       ISA S5.1 — Pipeline End Connections & Fittings
       ────────────────────────────────────────────────── */
    { key: 'cthreaded', name: 'Cap — Threaded', k: 0.00, sym: 'cap', ico: 'cap', colour: '#94a3b8',
      note: 'a capped line end, threaded connection for removal/installation' },
    { key: 'plug-cap',  name: 'Cap — Plug', k: 0.00, sym: 'cap', ico: 'cap', colour: '#94a3b8',
      note: 'a solid plug cap, the simplest line closure for temporary or dead ends' },
    { key: 'hose-conn', name: 'Hose Connection', k: 0.00, sym: 'hose', ico: 'hose', colour: '#64748b',
      note: 'a flexible hose coupling with quick-disconnect or threaded connection' },
    { key: 'cbutt',     name: 'Cap — Butt Welded', k: 0.00, sym: 'cap', ico: 'cap', colour: '#94a3b8',
      note: 'a welded cap closure, permanent or semi-permanent line termination' },
    { key: 'blind-end', name: 'Blind Flange End', k: 0.00, sym: 'flgBLD', ico: 'flgBLD', colour: '#94a3b8',
      note: 'a blind flange closing a flanged connection — the dead end for the hydraulic model' },
    { key: 'red-conc',  name: 'Reducer — Concentric', k: 0.30, sym: 'red', ico: 'red', colour: '#94a3b8',
      note: 'a size change on the pipe centreline — common on discharge lines; can trap pockets at suction' },
    { key: 'pspool',    name: 'Pipe Spool with Flange', k: 0.00, sym: 'spool', ico: 'spool', colour: '#94a3b8',
      note: 'a fabricated spooled line connecting two flange pairs — used in modular or replacement work' }
  ];
  function lib(key) { for (var i = 0; i < LIB.length; i++) if (LIB[i].key === key) return LIB[i]; return LIB[0]; }

  /* ─────────────────────────────────────────────────────────────
     ISA/PIP Line Type Legend — piping & instrumentation drawing
     Each line type has a distinct rendering in the P&ID to show
     the physical state or function of that connection.
     ────────────────────────────────────────────────────────────── */
  var LINE_TYPES = {
  ...
  var SNAP = 25, ORTHO = true, GHOST = null, RUNNING = false, GRID_ON = true, AUTO_ROTATE3D = false;
  ...
  /* ─────────── document helpers ─────────── */
  /* The workbench is a drawing surface for whichever line-sizing panel
     mounted it. Everything that is not about the drawing itself - bore,
     schedule, material, fluid, flow, pressure - is read back through SRC,
     so nothing is ever asked for twice and any phase can host it. */
  var SRC = null;
  function defSrc() {
    var g = function (id, d) { var e = document.getElementById(id); if (!e) return d; var v = parseFloat(e.value); return isFinite(v) ? v : d; };
    var t = function (id, d) { var e = document.getElementById(id); return e ? (e.value || d) : d; };
    return {
      nps: function () { return parseFloat(t('tp2-nps', '3')); },
      sch: function () { return t('tp2-sch', '40'); },
      mat: function () { return t('tp2-mat', 'CS'); },
      rho: function () {
        var rl = g('tp2-rhol', 998.2), rg = g('tp2-rhog', 1.204);
        var wl = g('tp2-wl', 20000), wg = g('tp2-wg', 60);
        var x = (wl + wg) > 0 ? wg / (wl + wg) : 0;
        return 1 / ((x / rg) + ((1 - x) / rl));
      },
      mu: function () {
        var ml = g('tp2-mul', 1.002), mg = g('tp2-mug', 0.0181);
        var wl = g('tp2-wl', 20000), wg = g('tp2-wg', 60);
        var x = (wl + wg) > 0 ? wg / (wl + wg) : 0;
        return (x * ml) + ((1 - x) * mg);
      },
      W: function () { return g('tp2-wl', 20000) + g('tp2-wg', 60); },
      pup: function () { return g('tp2-pup', 6); },
      C: function () { return g('tp2-cfactor', 100); },
      from: function () { return t('tp2-from', ''); },
      to: function () { return t('tp2-to', ''); },
      sizeIds: ['tp2-nps', 'tp2-sch']
    };
  }
  function src() { if (!SRC) SRC = defSrc(); return SRC; }
  function defNps() { return src().nps(); }
  function defSch() { return src().sch(); }

  /* ── Keeping the panel's pipe data and the drawing on the same size ─────
     A segment records the bore it was drawn at, and that snapshot was never
     revisited. Changing NPS or schedule in the panel re-rendered the drawing
     but left every leg on its old size, so the panel could read 4" sch 80
     while the whole route was still evaluated at 4" sch 40 — two different
     lines on one screen, with the hydraulics coming off the stale one. The
     traffic ran the other way too: APPLY TO THE WHOLE DRAWING re-sized every
     leg and never told the panel, so the panel kept sizing on the bore the
     engineer had just replaced.

     The two are now tied in both directions. DEFSIZE remembers the panel
     size the drawing was last aligned to, so a panel change moves the legs
     that were still following the panel and leaves alone any leg the
     engineer deliberately set to something else — the workbench supports
     mixed sizes on purpose, with reducers between them, and a blanket
     overwrite would quietly destroy that. SIZESYNC stops the two sides
     bouncing an update back and forth. */
  var DEFSIZE = null, SIZESYNC = false;
  function panelSize() { return { nps: defNps(), sch: String(defSch()) }; }
  function sameSize(a, b) {
    return a && b && String(a.nps) === String(b.nps) && String(a.sch) === String(b.sch);
  }
  /* Write a size the drawing has adopted back into the panel's pipe data. */
  function pushSizeToPanel(nps, sch) {
    var ids = src().sizeIds || [];
    if (ids.length < 2) { DEFSIZE = { nps: nps, sch: String(sch) }; return; }
    SIZESYNC = true;
    try {
      var n = document.getElementById(ids[0]), c = document.getElementById(ids[1]);
      if (n) { n.value = String(nps); n.dispatchEvent(new Event('change', { bubbles: true })); }
      if (c) { c.value = String(sch); c.dispatchEvent(new Event('change', { bubbles: true })); }
    } catch (e) {} 
    SIZESYNC = false;
    DEFSIZE = { nps: nps, sch: String(sch) };
  }
  /* Re-size every leg that is still following the panel. */
  function adoptPanelSize() {
    var now = panelSize();
    if (DEFSIZE && !sameSize(DEFSIZE, now)) {
      DOC.segs.forEach(function (sg) {
        if (sameSize(sg, DEFSIZE)) { sg.nps = now.nps; sg.sch = now.sch; }
      });
      DOC.items.forEach(function (it) {
        if ((it.key === 'red' || it.key === 'ered') && String(it.sch) === String(DEFSIZE.sch)) {
          it.d1 = now.nps; it.d2 = stepNps(now.nps, -1); it.sch = now.sch;
        }
      });
    }
    DEFSIZE = now;
  ...
  function addSeg(x1, y1, x2, y2, nps, sch, ltype) {
    var s = { id: UID++, x1: x1, y1: y1, x2: x2, y2: y2,
              nps: nps || defNps(), sch: sch || defSch(), colour: '#38bdf8', ltype: ltype || 'existing' };
  ...
  function addItem(key, segId, t, tag) {
    ...
    if (key === 'red' || key === 'ered') {
      var sg = segById(segId);
      it.d1 = sg ? sg.nps : defNps();
      it.d2 = stepNps(it.d1, -1);
      if (it.d2 === it.d1) it.d2 = stepNps(it.d1, 1);
      it.sch = sg ? sg.sch : defSch();
    }
    DOC.items.push(it);
    if (key === 'red' || key === 'ered') applyReducers();
    return it;
  }
  /* A reducer is a size change, so the drawing must show it: every leg up to
     and including the one carrying the reducer runs at its FROM size, and
     every leg after it runs at the TO size. With several reducers each one
     governs the stretch that follows it, until the next reducer. */
  function applyReducers() {
    if (!DOC.segs.length) return;
    var reds = DOC.items.filter(function (it) { return (it.key === 'red' || it.key === 'ered') && it.d1 && it.d2; })
      .map(function (it) { return { it: it, i: DOC.segs.map(function (s) { return s.id; }).indexOf(it.segId) }; })
      .filter(function (r) { return r.i >= 0; })
      .sort(function (a, b) { return (a.i - b.i) || (a.it.t - b.it.t); });
    if (!reds.length) return;
    var j;
    for (j = 0; j <= reds[0].i; j++) { DOC.segs[j].nps = reds[0].it.d1; if (reds[0].it.sch) DOC.segs[j].sch = reds[0].it.sch; }
    for (var r = 0; r < reds.length; r++) {
      var end = (r + 1 < reds.length) ? reds[r + 1].i : DOC.segs.length - 1;
      for (j = reds[r].i + 1; j <= end; j++) { DOC.segs[j].nps = reds[r].it.d2; if (reds[r].it.sch) DOC.segs[j].sch = reds[r].it.sch; }
    }
  }
  ...
  function legs(overrideNps, overrideSch) {
    ...
    var S = src();
    var eps = ROUGH[S.mat()] !== undefined ? ROUGH[S.mat()] : 0.045;
    /* The drawing is the same line as the panel, so it sees the same fluid. */
    var rho = S.rho(), mu = S.mu(), W = S.W();
    var Q = W / rho;
    ...
    return DOC.segs.map(function (s, i) {
      ...
      var sumK = mine.reduce(function (a, it) { return a + kOf(it.key, nps, it); }, 0);
  ...
  function totals(ls) {
    ...
    /* The allowable is the STREAM's, not a fixed 75 %.
       This used to apply API RP 14E at a hardcoded 75 % whatever the panel
       beside it was set to, and each stream sets its own erosional basis —
       the set point is an input on the single-phase panels, and slurry
       derives its limit from the lining material rather than from 14E at
       all. So the drawing could report the run over the erosional limit
       while the panel driving it reported the same run inside it, on the
       same duty. The panel publishes the allowable it actually used and the
       drawing uses that; the 75 % default only applies when a source does
       not offer one. */
    var srcVal = (typeof src().vallow === 'function') ? src().vallow() : NaN;
    var Vallow = isFinite(srcVal) && srcVal > 0 ? srcVal : Ve * 0.75;
    ...
    t.dpAllow = Math.max(0.05, Math.min(0.5 * t.Leq / 100, 0.10 * (src().pup() + 1.01325)));
  ...
  function compute() {
    ...
    /* Would one uniform size serve the whole drawing better? Try every bore
       AND every schedule, keeping the smallest bore that passes velocity,
       erosion and ΔP — and within that bore, the lightest wall that does.

       This used to sweep bores at one schedule only: whatever the panel
       happened to be set to. So a drawing that needed a heavier or lighter
       wall was told "no single size satisfies the whole drawing" while the
       line-sizing engine beside it — which sweeps both — had already found
       one and settled on it. The two panels contradicted each other on the
       same duty, and the drawing side was the one that was wrong. */
    var sizes = Object.keys(PIPE).map(Number).sort(function (a, b) { return a - b; });
    r.uniform = null;
    outer:
    for (var i = 0; i < sizes.length; i++) {
      for (var j = 0; j < SCHEDULES.length; j++) {
        if (!PIPE[sizes[i]] || PIPE[sizes[i]].s[SCHEDULES[j]] === undefined) continue;
        var u = totals(legs(sizes[i], SCHEDULES[j]));
        if (u.velOk && u.eroOk && u.dpOk) {
          r.uniform = { nps: sizes[i], sch: SCHEDULES[j], tot: u };
          break outer;
        }
      }
    }
  ...
  function symbol(ctx, x, y, ang, c, selected) {
    ...
    /* Two faces astride the line — the shape every flange shares. */
    function pair(gap) {
      ctx.beginPath();
      ctx.moveTo(-gap, -s); ctx.lineTo(-gap, s);
      ctx.moveTo(gap, -s); ctx.lineTo(gap, s);
      ctx.stroke();
    }
    ...
    switch (c.sym) {
      ...
      case 'needle': bowtie(false);
                    ctx.beginPath(); ctx.moveTo(-3, -3); ctx.lineTo(3, -3); ctx.lineTo(0, 3); ctx.closePath(); ctx.fill();
                    ctx.beginPath(); ctx.moveTo(0, -3); ctx.lineTo(0, -s - 6); ctx.moveTo(-4, -s - 6); ctx.lineTo(4, -s - 6); ctx.stroke(); break;
      case 'diaphragm': bowtie(false);
                    ctx.beginPath(); ctx.arc(0, -s - 3, 5, Math.PI, 0); ctx.stroke();
                    ctx.beginPath(); ctx.moveTo(0, -s + 2); ctx.lineTo(0, -s - 3); ctx.stroke(); break;
      ...
      case 'ered':  ctx.beginPath(); ctx.moveTo(-s, -6); ctx.lineTo(s, -6); ctx.lineTo(s, 3); ctx.lineTo(-s, s + 3); ctx.closePath(); ctx.stroke();
                    ctx.lineWidth = 3; ctx.beginPath(); ctx.moveTo(-s, -6); ctx.lineTo(s, -6); ctx.stroke(); break;
      ...
      case 'tstrain': ctx.beginPath(); ctx.moveTo(-s, -6); ctx.lineTo(s, -6); ctx.lineTo(s, 6); ctx.lineTo(-s, 6); ctx.closePath(); ctx.stroke();
                    ctx.beginPath(); ctx.moveTo(-3, -6); ctx.lineTo(-3, -s - 3); ctx.lineTo(3, -s - 3); ctx.lineTo(3, -6); ctx.stroke(); break;
      case 'orif':  ctx.beginPath(); ctx.moveTo(0, -s); ctx.lineTo(0, -3); ctx.moveTo(0, 3); ctx.lineTo(0, s); ctx.stroke(); break;
      /* Flanges are drawn as the pair of faces they are, with the detail that
         tells the types apart: a raised ring, a flat face, a groove, a hub. */
      case 'flgRF': pair(3); ctx.lineWidth = 3; ctx.beginPath(); ctx.moveTo(-1, -4); ctx.lineTo(-1, 4); ctx.moveTo(1, -4); ctx.lineTo(1, 4); ctx.stroke(); break;
      case 'flgFF': pair(3); break;
      case 'flgRTJ': pair(3); ctx.beginPath(); ctx.arc(0, 0, 2.6, 0, Math.PI * 2); ctx.stroke(); break;
      case 'flgWN': pair(5); ctx.beginPath(); ctx.moveTo(-5, -s); ctx.lineTo(-9, -5); ctx.moveTo(-5, s); ctx.lineTo(-9, 5);
                    ctx.moveTo(5, -s); ctx.lineTo(9, -5); ctx.moveTo(5, s); ctx.lineTo(9, 5); ctx.stroke(); break;
      case 'flgSO': pair(4); ctx.beginPath(); ctx.moveTo(-4, -5); ctx.lineTo(4, -5); ctx.moveTo(-4, 5); ctx.lineTo(4, 5); ctx.stroke(); break;
      case 'flgSW': pair(3); ctx.beginPath(); ctx.moveTo(-3, -4); ctx.lineTo(3, -4); ctx.moveTo(-3, 4); ctx.lineTo(3, 4); ctx.stroke(); break;
      case 'flgTHD': pair(3); ctx.lineWidth = 1;
                    for (var ti = -3; ti <= 3; ti += 2) { ctx.beginPath(); ctx.moveTo(ti, -5); ctx.lineTo(ti + 1, 5); ctx.stroke(); } break;
      case 'flgLAP': pair(4); ctx.beginPath(); ctx.moveTo(-4, -4); ctx.lineTo(4, -4); ctx.lineTo(4, 4); ctx.lineTo(-4, 4); ctx.closePath(); ctx.stroke(); break;
      case 'flgBLD': ctx.beginPath(); ctx.moveTo(-2, -s); ctx.lineTo(-2, s); ctx.stroke();
                    ctx.beginPath(); ctx.moveTo(2, -s); ctx.lineTo(2, s); ctx.lineTo(6, s); ctx.lineTo(6, -s); ctx.closePath(); ctx.fill(); break;
      case 'spec':  ctx.beginPath(); ctx.moveTo(-s, -s); ctx.lineTo(-s, s); ctx.stroke();
                    ctx.beginPath(); ctx.arc(-1, 0, 4.5, 0, Math.PI * 2); ctx.stroke();
                    ctx.beginPath(); ctx.arc(8, 0, 4.5, 0, Math.PI * 2); ctx.fill();
                    ctx.beginPath(); ctx.moveTo(3.5, 0); ctx.lineTo(3.5, 0); ctx.stroke(); break;
      case 'spade': pair(2); ctx.beginPath(); ctx.arc(0, 0, 4.5, 0, Math.PI * 2); ctx.fill();
                    ctx.beginPath(); ctx.moveTo(4.5, 0); ctx.lineTo(s + 4, 0); ctx.stroke(); break;
      case 'ringspacer': pair(2); ctx.beginPath(); ctx.arc(0, 0, 4.5, 0, Math.PI * 2); ctx.stroke(); break;
      case 'hpv':   ctx.beginPath(); ctx.moveTo(0, -2); ctx.lineTo(0, -s - 4); ctx.stroke();
                    ctx.beginPath(); ctx.moveTo(-4, -s - 4); ctx.lineTo(4, -s - 4); ctx.lineTo(0, -s - 10); ctx.closePath(); ctx.stroke(); break;
      case 'lpd':   ctx.beginPath(); ctx.moveTo(0, 2); ctx.lineTo(0, s + 4); ctx.stroke();
                    ctx.beginPath(); ctx.moveTo(-4, s + 4); ctx.lineTo(4, s + 4); ctx.lineTo(0, s + 10); ctx.closePath(); ctx.stroke(); break;
      case 'funnel': ctx.beginPath(); ctx.moveTo(-6, s + 2); ctx.lineTo(6, s + 2); ctx.lineTo(1.5, s + 10); ctx.lineTo(-1.5, s + 10); ctx.closePath(); ctx.stroke(); break;
      case 'cap':    ctx.beginPath(); ctx.moveTo(-s, -6); ctx.lineTo(s, -6); ctx.lineTo(s + 2, 0); ctx.lineTo(s, 6); ctx.lineTo(-s, 6); ctx.lineTo(-s - 2, 0); ctx.closePath(); ctx.stroke(); break;
      case 'hose':   ctx.beginPath(); ctx.arc(0, 0, 6, 0, Math.PI * 2); ctx.stroke();
                    ctx.lineWidth = 1; for (var hi = -4; hi <= 4; hi += 3) { ctx.beginPath(); ctx.moveTo(hi, -6); ctx.lineTo(hi, 6); ctx.stroke(); } break;
      case 'spool':  pair(2); ctx.lineWidth = 1;
                    ctx.beginPath(); ctx.moveTo(-7, -s); ctx.lineTo(-7, s); ctx.moveTo(7, -s); ctx.lineTo(7, s); ctx.stroke(); break;
  ...
  function drawLineDecorations(ctx, s, ltype, colour) {
    var lt = LINE_TYPES[ltype] || LINE_TYPES.existing;
    var dx = s.x2 - s.x1, dy = s.y2 - s.y1, len = Math.hypot(dx, dy);
    if (len === 0) return;
    var ang = Math.atan2(dy, dx);
    if (lt.dash) { ctx.setLineDash(lt.dash); }
    if (lt.tick) {
      var space = lt.tickSpace || 14;
      for (var d = space / 2; d < len; d += space) {
        var pt = pointAtDist([{x: s.x1, y: s.y1}, {x: s.x2, y: s.y2}], d);
        drawTickMark(ctx, pt.x, pt.y, ang, lt.tick, colour);
      }
    }
    if (lt.wavy) {
      ctx.setLineDash([]);
      ...
      for (var i = 0; i <= len; i++) {
        ...
        var x = s.x1 + dx * t;
        var y = s.y1 + dy * t;
        var off = Math.sin(i * freq) * amp;
        var px = x - Math.sin(ang) * off;
        var py = y + Math.cos(ang) * off;
  ...
  function drawTickMark(ctx, x, y, ang, kind, colour) {
    var perp = ang + Math.PI / 2;
    function drawSeg(a, len) {
      ctx.beginPath();
    ...
    if (kind === 'perp') drawSeg(perp, 5);
    else if (kind === 'hatch') drawSeg(ang + Math.PI / 4, 6);
    else if (kind === 'x') { drawSeg(ang + Math.PI / 4, 4); drawSeg(ang - Math.PI / 4, 4); }
    else if (kind === 'L') {
      var bx = x - Math.cos(ang) * 3, by = y - Math.sin(ang) * 3;
      ctx.beginPath();
      ctx.moveTo(bx - Math.cos(perp) * 4, by - Math.sin(perp) * 4);
      ctx.lineTo(bx, by);
      ctx.lineTo(bx + Math.cos(ang) * 5, by + Math.sin(ang) * 5);
      ctx.stroke();
    }
    else if (kind === 'dot') {
      ctx.beginPath();
  ...
  function draw() {
    ...
    if (GRID_ON) {
      CTX.strokeStyle = 'rgba(100,116,139,0.18)'; CTX.lineWidth = 1 / VIEW.k;
    ...
    DOC.segs.forEach(function (s, i) {
      ...
      drawLineDecorations(CTX, s, s.ltype || 'existing', s.colour);
      CTX.setLineDash([]);
      CTX.shadowBlur = 0;
      ...
      [[s.x1, s.y1], [s.x2, s.y2]].forEach(function (pt) {
        CTX.fillStyle = BG_COLOR; CTX.strokeStyle = sel ? '#0f172a' : '#64748b'; CTX.lineWidth = 2;
    ...
    CTX.font = 'bold 11px monospace'; CTX.fillText(src().from() || 'FROM', a.x1 + 10, a.y1 - 12);
    CTX.fillStyle = '#ef4444'; CTX.beginPath(); CTX.arc(b.x2, b.y2, 6, 0, Math.PI * 2); CTX.fill();
    CTX.fillText(src().to() || 'TO', b.x2 + 10, b.y2 - 12);
  }
  ...
  /* Soft radial-glow sprite for the flow particles — a small canvas gradient,
     built once and reused by every leg's THREE.Points, so a dense real-time
     stream still costs one texture and one draw call per leg rather than a
     hard-edged low-poly sphere mesh (and its own draw call) per droplet. */
  var FLOW_TEX = null;
  function flowSprite() {
    if (FLOW_TEX) return FLOW_TEX;
    var c = document.createElement('canvas'); c.width = c.height = 64;
    var g = c.getContext('2d');
    var grad = g.createRadialGradient(32, 32, 0, 32, 32, 32);
    grad.addColorStop(0, 'rgba(255,255,255,1)');
    grad.addColorStop(0.35, 'rgba(200,244,255,0.95)');
  ...
  function icon3D(c, R) {
    ...
    if (ico === 'ered') {                                   // same cone, flagged with a flat top strip — no low/high pocket
      var econe = cyl(R * 0.62, R * 1.25, R * 2.4, 22, paint); econe.rotation.z = Math.PI / 2; add(econe);
      var es1 = cyl(R * 0.62, R * 0.62, R * 0.9, 18, paint); es1.rotation.z = Math.PI / 2; es1.position.x = R * 1.6; add(es1);
      var es2 = cyl(R * 1.25, R * 1.25, R * 0.9, 18, paint); es2.rotation.z = Math.PI / 2; es2.position.x = -R * 1.6; add(es2);
      var efa = cyl(R * 1.0, R * 1.0, R * 0.3, 20, steel); efa.rotation.z = Math.PI / 2; efa.position.x = R * 2.1; add(efa);
      var efb = cyl(R * 1.75, R * 1.75, R * 0.3, 22, steel); efb.rotation.z = Math.PI / 2; efb.position.x = -R * 2.1; add(efb);
      var flat = new THREE.Mesh(new THREE.BoxGeometry(R * 3.0, R * 0.14, R * 1.5), steel);
      flat.position.set(0, R * 1.15, 0); add(flat);
      return g;
    }
    /* The flange family. Every one is a pair of bolted faces on a stub of
       pipe; what changes is the face itself and how it meets the pipe. */
    if (ico && ico.indexOf('flg') === 0 || ico === 'spec') {
      var stub = cyl(R, R, R * 3.0, 18, paint); stub.rotation.z = Math.PI / 2; add(stub);

      if (ico === 'flgWN') {                                   // tapered welding hub
        [-1, 1].forEach(function (sgn) {
          var hub = cyl(R * 1.05, R * 1.55, R * 1.1, 18, steel);
          hub.rotation.z = Math.PI / 2; hub.position.x = sgn * R * 1.0; add(hub);
        });
      } else if (ico === 'flgSO' || ico === 'flgSW') {          // welded collar
        [-1, 1].forEach(function (sgn) {
          var cl = cyl(R * 1.3, R * 1.3, R * 0.5, 16, steel);
          cl.rotation.z = Math.PI / 2; cl.position.x = sgn * R * 1.0; add(cl);
        });
      } else if (ico === 'flgLAP') {                            // stub end behind the flange
        [-1, 1].forEach(function (sgn) {
          var se = cyl(R * 1.5, R * 1.5, R * 0.3, 20, steel);
          se.rotation.z = Math.PI / 2; se.position.x = sgn * R * 0.85; add(se);
        });
      } else if (ico === 'flgTHD') {                            // thread ring
        [-1, 1].forEach(function (sgn) {
          for (var q = 0; q < 3; q++) {
            var th = new THREE.Mesh(new THREE.TorusGeometry(R * 1.05, R * 0.06, 6, 18), steel);
            th.position.x = sgn * (R * 0.9 + q * R * 0.22); th.rotation.y = Math.PI / 2; add(th);
          }
        });
      }

      flangePair(R * 1.9, R * 0.5);

      if (ico === 'flgRF') {                                    // the raised face itself
        [-1, 1].forEach(function (sgn) {
          var rf = cyl(R * 1.35, R * 1.35, R * 0.18, 22, brass);
          rf.rotation.z = Math.PI / 2; rf.position.x = sgn * R * 0.22; add(rf);
        });
      } else if (ico === 'flgRTJ') {                            // seated metal ring
        var ring = new THREE.Mesh(new THREE.TorusGeometry(R * 1.15, R * 0.16, 8, 24), brass);
        ring.rotation.y = Math.PI / 2; add(ring);
      } else if (ico === 'flgBLD') {                            // closed off
        var cap = cyl(R * 1.9, R * 1.9, R * 0.55, 24, steel);
        cap.rotation.z = Math.PI / 2; cap.position.x = R * 0.7; add(cap);
      } else if (ico === 'spec') {                              // spectacle blind
        var open = new THREE.Mesh(new THREE.TorusGeometry(R * 1.0, R * 0.18, 8, 22), paint); open.rotation.y = Math.PI / 2; add(open);
        var solid = cyl(R * 1.5, R * 1.5, R * 0.22, 22, paint); solid.rotation.z = Math.PI / 2; solid.position.set(0, R * 3.0, 0); add(solid);
        var web = new THREE.Mesh(new THREE.BoxGeometry(R * 0.16, R * 1.9, R * 0.5), paint); web.position.y = R * 1.7; add(web);
      }
      return g;
    }
    if (ico === 'orif') {
    ...
    if (ico === 'tstrain') {                                // straight body, top-pulled basket
      var tbody = cyl(R * 1.3, R * 1.3, R * 3.0, 20, paint); tbody.rotation.z = Math.PI / 2; add(tbody);
      flangePair(R * 1.8, R * 1.8);
      var basket = cyl(R * 1.1, R * 1.1, R * 2.0, 18, paint); basket.position.y = R * 2.1; add(basket);
      var lid = cyl(R * 1.25, R * 1.25, R * 0.35, 20, steel); lid.position.y = R * 3.15; add(lid);
      for (var bb = 0; bb < 6; bb++) {
        var ba = bb * Math.PI / 3;
        var blt = cyl(R * 0.09, R * 0.09, R * 0.4, 6, dark);
        blt.position.set(Math.cos(ba) * R * 0.95, R * 3.15, Math.sin(ba) * R * 0.95); add(blt);
      }
      return g;
    }
    if (ico === 'spade') {                                  // single blind paddle with a handle tab
      var spstub = cyl(R, R, R * 1.6, 18, paint); spstub.rotation.z = Math.PI / 2; add(spstub);
      flangePair(R * 1.6, R * 0.9);
      var plate = cyl(R * 1.35, R * 1.35, R * 0.22, 22, steel); plate.rotation.z = Math.PI / 2; add(plate);
      var tab = new THREE.Mesh(new THREE.BoxGeometry(R * 0.16, R * 1.6, R * 0.5), steel); tab.position.y = R * 2.2; add(tab);
      return g;
    }
    if (ico === 'ringspacer') {                             // open ring — the spaded-open companion piece
      var rsstub = cyl(R, R, R * 1.2, 18, paint); rsstub.rotation.z = Math.PI / 2; add(rsstub);
      flangePair(R * 1.5, R * 0.7);
      var openring = new THREE.Mesh(new THREE.TorusGeometry(R * 1.0, R * 0.16, 8, 22), paint); openring.rotation.y = Math.PI / 2; add(openring);
      return g;
    }
    if (ico === 'hpv' || ico === 'lpd') {                   // small branch nozzle with an isolation valve — vent up, drain down
      var dir = (ico === 'hpv') ? 1 : -1;
      var noz = cyl(R * 0.4, R * 0.4, R * 1.6, 14, paint); noz.position.y = dir * R * 1.4; add(noz);
      var nf = cyl(R * 0.65, R * 0.65, R * 0.18, 16, steel); nf.position.y = dir * R * 2.2; add(nf);
      var vbody = new THREE.Mesh(new THREE.SphereGeometry(R * 0.55, 14, 10), paint); vbody.position.y = dir * R * 2.6; add(vbody);
      var vstem = cyl(R * 0.08, R * 0.08, R * 0.7, 8, steel); vstem.position.y = dir * (R * 2.6 + R * 0.55); add(vstem);
      var vwheel = new THREE.Mesh(new THREE.TorusGeometry(R * 0.4, R * 0.08, 6, 16), brass);
      vwheel.position.y = dir * (R * 2.6 + R * 0.55 + R * 0.7); vwheel.rotation.x = Math.PI / 2; add(vwheel);
      return g;
    }
    if (ico === 'funnel') {                                 // open hopper under a drain or vent
      var fbody = new THREE.Mesh(new THREE.CylinderGeometry(R * 1.4, R * 0.35, R * 2.2, 20, 1, true), paint);
      fbody.material.side = THREE.DoubleSide; fbody.position.y = -R * 1.6; add(fbody);
      var lip = new THREE.Mesh(new THREE.TorusGeometry(R * 1.4, R * 0.12, 8, 22), steel); lip.position.y = -R * 0.6; add(lip);
      var spout = cyl(R * 0.28, R * 0.28, R * 1.0, 12, steel); spout.position.y = -R * 3.0; add(spout);
      return g;
    }
    ...
    else if (ico === 'needle') {                            // slender tapered stem, fine-adjustment handle
      var nstem = cyl(R * 0.11, R * 0.11, R * 1.8, 10, steel); nstem.position.y = R * 2.4; add(nstem);
      var ntip = new THREE.Mesh(new THREE.ConeGeometry(R * 0.24, R * 0.6, 12), steel); ntip.position.y = R * 1.4; add(ntip);
      var thandle = cyl(R * 0.5, R * 0.5, R * 1.0, 12, brass); thandle.position.y = R * 3.6; add(thandle);
      var tknob = cyl(R * 0.14, R * 0.14, R * 0.4, 8, dark); tknob.position.y = R * 4.3; add(tknob);
    }
    else if (ico === 'diaphragm') {                         // domed diaphragm cap over a weir body
      var dcap = new THREE.Mesh(new THREE.SphereGeometry(R * 1.25, 18, 10, 0, Math.PI * 2, 0, Math.PI / 2), steel);
      dcap.position.y = R * 2.5; add(dcap);
      var dring = cyl(R * 1.25, R * 1.25, R * 0.2, 18, brass); dring.position.y = R * 2.45; add(dring);
      stem(R * 0.8); wheel(R * 3.55, R * 1.05);
    }
    return g;
  }
  ...
  /* Palette tiles show the real 3D component, rendered once offscreen and
     ...
     ONE RENDERER, NOT FIFTY. Each tile used to build its own WebGLRenderer,
     render, and then throw the GL context away: fifty context creations,
     fifty shader compilations and fifty forceContextLoss calls in a single
     synchronous burst. Measured on the first switch to MANUAL, that was a
     5.7 second block of the main thread — the page frozen at the moment the
     engineer asked for the drawing workbench. The renderer is now made once
     and reused; only the scene changes. Every tile is rendered at the same
     size, with the same lights and the same camera framing as before, so
     the finished palette is identical. */
  var ICO_CACHE = {};
  var ICO_RN = null, ICO_W = 104, ICO_H = 72;
  function icoRenderer() {
    if (ICO_RN !== null) return ICO_RN;
    try {
      var cv = document.createElement('canvas'); cv.width = ICO_W; cv.height = ICO_H;
      var rn = new THREE.WebGLRenderer({ canvas: cv, antialias: true, alpha: true, preserveDrawingBuffer: true });
      rn.setPixelRatio(2); rn.setSize(ICO_W, ICO_H, false);
      ICO_RN = { rn: rn, cv: cv };
    } catch (e) { ICO_RN = false; }
    return ICO_RN;
  }
  /* The meshes are per-tile and must still be released, or fifty tiles leak
     fifty sets of geometry and materials into the shared context. */
  function disposeTree(g) {
    g.traverse(function (o) {
      if (o.geometry) o.geometry.dispose();
      if (o.material) { [].concat(o.material).forEach(function (m) { if (m && m.dispose) m.dispose(); }); }
    });
  }
  function icon3DImage(key) {
    ...
    try {
      var W = ICO_W, H = ICO_H;
      var R2 = icoRenderer(); if (!R2) return null;
      var rn = R2.rn, cv = R2.cv;
      ...
      sc.remove(g); disposeTree(g);          // the renderer is kept, the model is not
      ICO_CACHE[key] = url;
      return url;
    } catch (e) { return null; }
  }
  ...
  function init3D() {
    ...
    var scene = new THREE.Scene(); scene.background = new THREE.Color(BG_COLOR);
    ...
    var sph = { r: 90, theta: 0.7, phi: 1.1, tx: 0, ty: 0, minR: 20, maxR: 400 };
    function place() {
    ...
    place();
    var down = null;
    cv.addEventListener('mousedown', function (e) { down = { x: e.clientX, y: e.clientY, th: sph.theta, ph: sph.phi, tx: sph.tx, ty: sph.ty }; });
    window.addEventListener('mousemove', function (e) {
      if (!down) return;
      if (TOOL === 'pan') {
        /* MOVE/PAN tool active: shift the orbit target instead of rotating around it.
           The model is flat in this scene's X/Y plane (every mesh sits at
           z=0 — see update3D), so panning only ever needs to move tx/ty; the
           camera's own world-space right/up vectors (X/Y components only)
           convert a screen-pixel drag into that plane so the pan tracks the
           mouse regardless of the current orbit angle, and its magnitude
           scales with distance so it feels the same at any zoom level. */
        cam.updateMatrixWorld();
        var m = cam.matrixWorld.elements;
        var rightX = m[0], rightY = m[1], upX = m[4], upY = m[5];
        var worldPerPx = 2 * sph.r * Math.tan(cam.fov * Math.PI / 360) / Math.max(1, cv.clientHeight);
        var dx = -(e.clientX - down.x) * worldPerPx, dy = (e.clientY - down.y) * worldPerPx;
        sph.tx = down.tx + rightX * dx + upX * dy;
        sph.ty = down.ty + rightY * dx + upY * dy;
      } else {
    ...
    /* Plain scroll zooms this preview directly — no modifier needed, same
       as every other 3D canvas in the app. (This is the 3D route preview,
       #pid-3d — separate from the 2D sketch canvas below, which is its own
       dedicated drawing surface and already used plain-scroll zoom.) */
    cv.addEventListener('wheel', function (e) {
      e.preventDefault();
      sph.r = Math.max(sph.minR, Math.min(sph.maxR, sph.r * (e.deltaY < 0 ? 0.9 : 1.1))); place();
    }, { passive: false });
    var last = performance.now();
    /* Skip render() once the P&ID canvas is hidden (offsetParent null),
       whether that's the 2D/3D toggle inside this workbench or the user
       switching to another stream's tab entirely — a page carrying this,
       the four line-sizing streams and PHE/Tank each with their own live
       WebGL context can hit the browser's context limit, and the eviction
       that follows leaves whichever context loses out rendering nothing
       forever with no error anywhere. */
    (function loop(now) {
      requestAnimationFrame(loop);
      var dt = Math.min(0.1, (now - last) / 1000); last = now;
      if (!cv.offsetParent) return;
      if (AUTO_ROTATE3D && !down) { sph.theta += 0.6 * dt; place(); }
      if (three && three.anim) three.anim(dt);
      rn.render(scene, cam);
    })(last);
  }
  function resize3D() {
  ...
  /* An elbow at a joint is not a symbol dropped on a pipe — it is the corner
     itself. The bend is built as a torus arc whose two tangent points sit on
     the incoming and outgoing legs, so it meets both pipes squarely whatever
     angle the route turns through. */
  function bend3D(P, u, v, R, colour) {
    var grp = new THREE.Group();
    var Rb = R * 2.2;                                          // bend centreline radius
    var A = { x: P.x - u.x * Rb, y: P.y - u.y * Rb };           // tangent on the incoming leg
    var B = { x: P.x + v.x * Rb, y: P.y + v.y * Rb };           // tangent on the outgoing leg
    var C = { x: P.x - u.x * Rb + v.x * Rb, y: P.y - u.y * Rb + v.y * Rb };
    var a0 = Math.atan2(A.y - C.y, A.x - C.x);
    var a1 = Math.atan2(B.y - C.y, B.x - C.x);
    var arc = a1 - a0;
    while (arc <= -Math.PI) arc += 2 * Math.PI;
    while (arc > Math.PI) arc -= 2 * Math.PI;
    if (arc < 0) { a0 = a1; arc = -arc; }                      // sweep positive from the other end
    if (arc < 1e-3) return grp;

    var mat = new THREE.MeshStandardMaterial({ color: colour, metalness: 0.55, roughness: 0.35 });
    var el = new THREE.Mesh(new THREE.TorusGeometry(Rb, R, 14, Math.max(8, Math.round(arc / 0.12)), arc), mat);
    el.position.set(C.x, C.y, 0);
    el.rotation.z = a0;
    grp.add(el);

    /* Weld collars where the bend meets each leg, so the joint reads as a
       fitting rather than as two pipes intersecting. */
    var steel = new THREE.MeshStandardMaterial({ color: 0xb8c2cf, metalness: 0.9, roughness: 0.25 });
    [[A, u], [B, v]].forEach(function (pr) {
      var col = new THREE.Mesh(new THREE.CylinderGeometry(R * 1.28, R * 1.28, R * 0.4, 18), steel);
      col.position.set(pr[0].x, pr[0].y, 0);
      col.rotation.z = Math.atan2(pr[1].y, pr[1].x) - Math.PI / 2;
      grp.add(col);
    });
    return grp;
  }

  /* Park a corner tag outside the turn, opposite the bend, clear of the
     corner itself and of both leg labels. */
  /* ── the key under the 3D view ────────────────────────────────────────
     Every figure that used to float beside a pipe lives here instead: one
     row per leg, one per component, each found by the tag drawn on the
     model. A route may have any number of legs, so there is no arrangement
     of floating labels that stays readable — a table simply grows. */
  function build3DKey(r) {
    var el = document.getElementById('pid-3dkey');
    if (!el) return;
    var U = function (v, t, d) {
      return (typeof window.fromSIDisplay === 'function') ? window.fromSIDisplay(t, v, d) : (isFinite(v) ? v.toFixed(d) : '—');
    };
    var th = 'padding:4px 7px;text-align:left;font-weight:800;color:#94a3b8;border-bottom:1px solid rgba(148,163,184,0.22);white-space:nowrap;';
    var td = 'padding:3px 7px;border-bottom:1px solid rgba(148,163,184,0.10);white-space:nowrap;';

    var legs = '';
    (r.legs || []).forEach(function (l) {
      legs += '<tr>'
        + '<td style="' + td + 'color:var(--text-header);font-weight:700;">L' + l.idx + '</td>'
        + '<td style="' + td + '">' + l.nps + '&Prime; sch ' + esc(String(l.sch)) + '</td>'
        + '<td style="' + td + '">' + U(l.L, 'length-m', 2) + '</td>'
        + '<td style="' + td + '">' + U(l.dz, 'length-m', 2) + '</td>'
        + '<td style="' + td + 'color:' + (l.V > 3 ? '#f59e0b' : '#4f9d78') + ';">' + U(l.V, 'velocity', 2) + '</td>'
        + '<td style="' + td + '">' + U(l.dp, 'press-drop', 4) + '</td>'
        + '</tr>';
    });
    ...
    (r.legs || []).forEach(function (l) {
      (l.items || []).forEach(function (it) {
        var c = lib(it.key) || { name: it.key };
        comps += '<tr>'
          + '<td style="' + td + 'color:#7dd3fc;font-weight:700;">' + esc(it.tag) + '</td>'
          + '<td style="' + td + '">' + esc(c.name || '') + '</td>'
          + '<td style="' + td + '">L' + l.idx + '</td>'
          + '<td style="' + td + 'color:#94a3b8;">' + (it.auto ? 'assumed at the corner' : 'placed') + '</td>'
          + '</tr>';
      });
    });

    if (!legs && !comps) { el.style.display = 'none'; el.innerHTML = ''; return; }
    el.style.display = 'block';
    el.innerHTML =
      '<div style="display:flex;gap:12px;flex-wrap:wrap;font-family:var(--font-mono);font-size:9.5px;color:#cbd5e1;">'
      + (legs ? '<div style="flex:1 1 340px;min-width:0;">'
          + '<div style="font-size:9.5px;font-weight:800;color:var(--color-saffron);letter-spacing:.05em;margin-bottom:4px;">LINE KEY</div>'
          + '<div style="overflow-x:auto;"><table style="width:100%;border-collapse:collapse;">'
          + '<tr><th style="' + th + '">TAG</th><th style="' + th + '">SIZE</th><th style="' + th + '">LENGTH</th>'
          + '<th style="' + th + '">&Delta;z</th><th style="' + th + '">VELOCITY</th><th style="' + th + '">&Delta;P</th></tr>'
          + legs + '</table></div></div>' : '')
      + (comps ? '<div style="flex:1 1 300px;min-width:0;">'
          + '<div style="font-size:9.5px;font-weight:800;color:var(--color-saffron);letter-spacing:.05em;margin-bottom:4px;">COMPONENT KEY</div>'
          + '<div style="overflow-x:auto;"><table style="width:100%;border-collapse:collapse;">'
          + '<tr><th style="' + th + '">TAG</th><th style="' + th + '">COMPONENT</th><th style="' + th + '">ON</th><th style="' + th + '">NOTE</th></tr>'
          + comps + '</table></div></div>' : '')
      + '</div>';
  }

  function placeBendTag(g, x, y, ux, uy, vx, vy, R, it, c) {
    var nm = sprite(it.tag + (it.auto ? ' \u231d' : ''), it.auto ? '#0369a1' : c.colour);
    var ox = -(ux + vx), oy = -(uy + vy);
    var m = Math.hypot(ox, oy) || 1;
    nm.position.set(x + (ox / m) * R * 7.0, y + (oy / m) * R * 7.0, 0);
    g.add(nm);
  }
  ...
  function update3D(r) {
    ...
    var matHex = MATCOLOR[src().mat()]; if (matHex == null) matHex = 0x8a94a3;
    /* Flow particles: one THREE.Points stream per leg (one draw call each)
       instead of a handful of individual sphere meshes — cheap enough to run
       a dense, continuously-moving stream that reads as fluid in real time
       rather than a few beads ticking along the pipe. FLOW_VISIBLE toggles
       the whole group's visibility with no rebuild — the toolbar button. */
    var flowPtsMat = new THREE.PointsMaterial({
      size: 0.16, map: flowSprite(), color: 0xeaffff, transparent: true, opacity: 0.95,
      blending: THREE.AdditiveBlending, depthWrite: false, sizeAttenuation: true
    });
    var flowGroup = new THREE.Group(); flowGroup.visible = FLOW_VISIBLE; g.add(flowGroup);
    var flowLegs = [];
    var boreMat = new THREE.MeshStandardMaterial({ color: 0x05070c, metalness: 0.1, roughness: 0.9 });

    /* Which leg to flag as the extreme, so the view answers "where" as well
       as "how much" — the same figures already in the key table, called out
       on the model itself instead of left to be found by scanning rows. */
    var hiVIdx = -1, loVIdx = -1, hiDpIdx = -1, loDpIdx = -1;
    if (r.legs.length > 1) {
      var vLegs = r.legs.filter(function (l) { return isFinite(l.V) && l.V > 0; });
      var dpLegs = r.legs.filter(function (l) { return isFinite(l.dpPa); });
      if (vLegs.length > 1) {
        hiVIdx = vLegs.reduce(function (a, b) { return b.V > a.V ? b : a; }).idx;
        loVIdx = vLegs.reduce(function (a, b) { return b.V < a.V ? b : a; }).idx;
        if (hiVIdx === loVIdx) { hiVIdx = -1; loVIdx = -1; }
      }
      if (dpLegs.length > 1) {
        hiDpIdx = dpLegs.reduce(function (a, b) { return b.dpPa > a.dpPa ? b : a; }).idx;
        loDpIdx = dpLegs.reduce(function (a, b) { return b.dpPa < a.dpPa ? b : a; }).idx;
        if (hiDpIdx === loDpIdx) { hiDpIdx = -1; loDpIdx = -1; }
      }
    }
    ...
    r.legs.forEach(function (l) {
      ...
      if (len < 1e-6) return;
      /* Real bore, exaggerated ×3 to stay visible on screen — the floor used
         to sit at 0.35, well above every common NPS below about 4" once
         exaggerated, so a 1" and a 2" leg drew at the identical radius. The
         floor now only catches genuinely hairline bores, so NPS actually
         reads as a size difference on the model, not just in the key table. */
      var R = Math.max(0.10, l.Dmm / 1000 * 3.0);
      /* A colour the engineer picked in 2D is a decision about the line, so
         it carries into 3D and wins over either colour mode. Otherwise the
         leg is coloured by velocity (the design-review diagnostic) or by
         its actual MOC finish, whichever the 3D toolbar toggle has set. */
      var velCol = !isFinite(l.V) ? 0x64748b : l.V > r.tot.Vallow ? 0xef4444 : l.V > 4.5 ? 0xf59e0b : l.V < 0.9 ? 0x38bdf8 : 0x22c55e;
      var col = s.userColour ? new THREE.Color(s.colour).getHex()
        : (COLOUR_MODE === 'material' ? matHex : velCol);

      /* Where a bend takes the corner, the straight stops at its tangent
         point instead of running into it — otherwise the pipe and the elbow
         overlap and the joint looks wrong. */
      var Rb = R * 2.2;
      var isBendKey = function (k) { return /^(e90|e90lr|e45|m0|m30|m60|m90|teeB)$/.test(k); };
      /* A bend takes the joint whether it was dropped on the leg arriving at
         it or the leg leaving it, so both are looked for. */
      var bendAtJoint = function (jointIdx) {
        var into = r.legs[jointIdx - 1], outOf = r.legs[jointIdx];
        return allItems().some(function (it) {
          if (!isBendKey(it.key)) return false;
          if (into && it.segId === into.seg.id && it.t >= 0.5) return true;
          if (outOf && it.segId === outOf.seg.id && it.t < 0.5) return true;
          return false;
        });
      };
      var trimA = (l.idx > 1 && bendAtJoint(l.idx - 1)) ? Rb : 0;
      var trimB = (l.idx < r.legs.length && bendAtJoint(l.idx)) ? Rb : 0;
      var ux0 = (bx - ax) / len, uy0 = (by - ay) / len;
      var sax = ax + ux0 * trimA, say = ay + uy0 * trimA;
      var sbx = bx - ux0 * trimB, sby = by - uy0 * trimB;
      var slen = Math.max(0.05, Math.hypot(sbx - sax, sby - say));

      /* Open-ended so a free end (nothing else capping it) shows the bore as
         a dark ring behind the outer wall, the way a real cut pipe end
         does, instead of reading as a solid painted rod. A thin dark disc
         sits just inside each free end to stand for that hollow bore — a
         bend or fitting at that end draws its own hardware over the top. */
      /* Glass/acrylic wall instead of opaque painted metal — the flow stream
         (added below) rides just outside the true bore for visibility
         reasons, but a solid metal wall still hid it from any angle where
         the near wall sat between the camera and the stream. A translucent
         wall keeps the pipe readable as a real object (colour, highlight,
         the joint hardware) while letting the flow show through from any
         side, DoubleSide so the far wall doesn't vanish and break the
         glass-tube look. */
      var pipe = new THREE.Mesh(new THREE.CylinderGeometry(R, R, slen, 24, 1, true),
        new THREE.MeshStandardMaterial({
          color: col, metalness: 0.15, roughness: 0.15, transparent: true, opacity: 0.42,
          side: THREE.DoubleSide, depthWrite: false
        }));
      pipe.renderOrder = 1;
      pipe.position.set((sax + sbx) / 2, (say + sby) / 2, 0);
      pipe.rotation.z = Math.atan2(by - ay, bx - ax) - Math.PI / 2;
      g.add(pipe);
      var wallR = R * 0.85;
      /* Sign points back into the tube from each end — +ux0 from A (towards
         B), -ux0 from B (towards A) — so the disc sits just inside the open
         end rather than floating past its tip. */
      [[trimA, sax, say, 1], [trimB, sbx, sby, -1]].forEach(function (end) {
        if (end[0] > 0) return;                                // a bend caps this end instead
        var cap = new THREE.Mesh(new THREE.CylinderGeometry(wallR, wallR, R * 0.12, 20), boreMat);
        cap.position.set(end[1] + ux0 * end[3] * R * 0.04, end[2] + uy0 * end[3] * R * 0.04, 0);
        cap.rotation.z = Math.atan2(by - ay, bx - ax) - Math.PI / 2;
        g.add(cap);
      });

      /* A pipe that only sits there reads as a solid rod, not a line in
         service. A dense stream of small glowing particles riding the
         OUTSIDE of the pipe — not its centreline, which an opaque metal
         wall simply hides — travels from L(idx) towards the next joint at a
         speed keyed to the leg's own velocity, close enough together that
         the eye reads a continuous flowing stream rather than a few beads
         ticking along. All the particles on a leg are one THREE.Points
         object (one draw call), which is what makes that density affordable.
         Rotating pipe.rotation.z only ever turns the model in the XY plane,
         so a fixed world-Z lift keeps the stream sitting visibly on "top" of
         the pipe regardless of which way the leg runs. Particles move from
         x1,y1 to x2,y2 in the drawn segment order, the same order the
         FROM/TO markers use in 2D, so the direction always matches the
         actual flow through the route. */
      if (isFinite(l.V) && l.V > 0) {
        var ux1 = (sbx - sax) / slen, uy1 = (sby - say) / slen;
        var ptSize = Math.max(0.05, R * 0.5);
        var liftZ = R * 1.25 + ptSize * 0.5;
        var nDrops = Math.max(8, Math.min(40, Math.round(slen / (R * 1.3))));
        var spd = Math.max(0.6, Math.min(9, l.V)) * (slen / nDrops) * 0.9;   // model-units/sec
        var fpos = new Float32Array(nDrops * 3), fts = new Float32Array(nDrops);
        for (var fi = 0; fi < nDrops; fi++) {
          var t0 = fi / nDrops;
          fts[fi] = t0;
          fpos[fi * 3] = sax + ux1 * slen * t0;
          fpos[fi * 3 + 1] = say + uy1 * slen * t0;
          fpos[fi * 3 + 2] = liftZ;
        }
        var fgeo = new THREE.BufferGeometry();
        fgeo.setAttribute('position', new THREE.BufferAttribute(fpos, 3));
        var fmat = flowPtsMat.clone(); fmat.size = ptSize;
        var fpts = new THREE.Points(fgeo, fmat);
        flowGroup.add(fpts);
        flowLegs.push({ pts: fpts, pos: fpos, ts: fts, n: nDrops, ax: sax, ay: say, ux: ux1, uy: uy1, len: slen, spd: spd, z: liftZ });
      }

      /* Tag only — bore, length and velocity are in the key beneath the view. */
      var lab = sprite('L' + l.idx, '#1e293b');
      var labOff = l.idx % 2 ? R + 2.6 : -(R + 2.6);
      lab.position.set((ax + bx) / 2, (ay + by) / 2 + labOff, 0); g.add(lab);

      /* Call out the worst and best legs directly on the model — the key
         table already has the numbers, but "which leg" still meant scanning
         rows. Only shown once more than one leg exists to compare. */
      var flag = l.idx === hiVIdx ? '↑ HIGH VELOCITY' : l.idx === loVIdx ? '↓ LOW VELOCITY'
        : l.idx === hiDpIdx ? '↑ HIGH ΔP' : l.idx === loDpIdx ? '↓ LOW ΔP' : null;
      if (flag) {
        var flagCol = (l.idx === hiVIdx || l.idx === hiDpIdx) ? '#ef4444' : '#38bdf8';
        var fl = sprite(flag, flagCol);
        fl.position.set((ax + bx) / 2, (ay + by) / 2 + labOff * 1.9, 0); g.add(fl);
      }

      l.items.forEach(function (it, ii) {
        var c = lib(it.key);
        var px = ax + (bx - ax) * it.t, py = ay + (by - ay) * it.t;
        var ux = (bx - ax) / len, uy = (by - ay) / len;

        /* An elbow or mitre near a joint becomes the corner itself. It can be
           dropped towards either end of its leg, so both the joint ahead and
           the joint behind are considered before falling back to an inline
           symbol. */
        var isBend = /^(e90|e90lr|e45|m0|m30|m60|m90|teeB)$/.test(it.key);
        if (isBend) {
          var legXY = function (sg) {
            var ax2 = (sg.x1 - mid.x) * K, ay2 = -(sg.y1 - mid.y) * K;
            var bx2 = (sg.x2 - mid.x) * K, by2 = -(sg.y2 - mid.y) * K;
            var d = Math.hypot(bx2 - ax2, by2 - ay2);
            return d > 1e-6 ? { ax: ax2, ay: ay2, bx: bx2, by: by2, ux: (bx2 - ax2) / d, uy: (by2 - ay2) / d } : null;
          };
          var made = false;
          if (it.t >= 0.5 && r.legs[l.idx]) {                 // joint at the far end
            var nx = legXY(r.legs[l.idx].seg);
            if (nx && (Math.abs(nx.ux - ux) > 1e-6 || Math.abs(nx.uy - uy) > 1e-6)) {
              g.add(bend3D({ x: bx, y: by }, { x: ux, y: uy }, { x: nx.ux, y: nx.uy }, R, col));
              placeBendTag(g, bx, by, ux, uy, nx.ux, nx.uy, R, it, c);
              made = true;
            }
          }
          if (!made && it.t < 0.5 && l.idx > 1 && r.legs[l.idx - 2]) {   // joint behind
            var pv = legXY(r.legs[l.idx - 2].seg);
            if (pv && (Math.abs(pv.ux - ux) > 1e-6 || Math.abs(pv.uy - uy) > 1e-6)) {
              g.add(bend3D({ x: ax, y: ay }, { x: pv.ux, y: pv.uy }, { x: ux, y: uy }, R, col));
              placeBendTag(g, ax, ay, pv.ux, pv.uy, ux, uy, R, it, c);
              made = true;
            }
          }
          if (made) return;
        ...
        var ic = icon3D(c, R);
        ic.position.set(px, py, 0);
        ic.rotation.z = Math.atan2(by - ay, bx - ax);
        g.add(ic);
        var nm = sprite(it.tag + (it.auto ? ' \u231d' : ''), it.auto ? '#0369a1' : c.colour);
        nm.position.set(px, py - R * 4.2 - (ii % 2) * R * 3.0, 0); g.add(nm);
      });
    });

    build3DKey(r);

    three.flowGroup = flowGroup;
    three.anim = flowLegs.length ? function (dt) {
      if (!flowGroup.visible) return;                        // hidden — skip the buffer rewrite entirely
      for (var i = 0; i < flowLegs.length; i++) {
        var leg = flowLegs[i], ts = leg.ts, pos = leg.pos, dtn = (leg.spd * dt) / leg.len;
        for (var fi = 0; fi < leg.n; fi++) {
          var t = ts[fi] + dtn; if (t > 1) t -= 1; ts[fi] = t;
          pos[fi * 3] = leg.ax + leg.ux * leg.len * t;
          pos[fi * 3 + 1] = leg.ay + leg.uy * leg.len * t;
        }
        leg.pts.geometry.attributes.position.needsUpdate = true;
      }
    } : null;
  ...
  function advise(r) {
    ...
    r.legs.forEach(function (l) {
      if (l.V > t.Vallow) out.push({ sev: 'fail', w: 'Leg L' + l.idx, h: 'Above the erosional limit',
      ...
      else if (l.V > 4.5) out.push({ sev: 'action', w: 'Leg L' + l.idx, h: 'Velocity high at ' + f2(l.V) + ' m/s',
        why: 'Above roughly 4.5 m/s the line becomes noisy and the fittings wear. One size up on this leg brings it back.' });
      else if (l.V < 0.9 && l.V > 0) out.push({ sev: 'action', w: 'Leg L' + l.idx, h: 'Velocity low at ' + f2(l.V) + ' m/s',
        why: 'Below about 0.9 m/s solids and any second phase settle out in this leg. Drop one size.' });
      if (l.dz > 0.5) out.push({ sev: 'info', w: 'Leg L' + l.idx, h: 'Riser of ' + f2(l.dz) + ' m',
      ...
      if (l.sumK > 0 && l.dpFitPa > l.dpFricPa) out.push({ sev: 'info', w: 'Leg L' + l.idx, h: 'Fittings dominate this leg',
    ...
    Object.keys(placed).forEach(function (k) {
      ...
      out.push({ sev: 'info', w: c.name + ' ×' + placed[k], h: 'K ' + kk + ' at ' + r.legs[0].nps + '" each  ·  ' + f4(placed[k] * kk * 0.5 * r.legs[0].rho * Math.pow(r.legs[0].V, 2) / 1e5) + ' bar at L1 velocity',
    ...
    if (!DOC.items.some(function (i) { return lib(i.key).key === 'gate' || lib(i.key).key === 'ball' || lib(i.key).key === 'butterfly'; }))
      out.push({ sev: 'action', w: 'Missing', h: 'No isolation valve on the run',
    ...
    if (t.dz > 0.5 && !DOC.items.some(function (i) { return /check/.test(i.key); }))
      out.push({ sev: 'action', w: 'Missing', h: 'No check valve on a rising line',
    ...
    if (!t.dpOk) out.push({ sev: 'fail', w: 'Hydraulics', h: 'Friction + fittings ΔP ' + f3(t.dpDyn) + ' bar over the ' + f3(t.dpAllow) + ' bar allowance',
    ...
    if (r.uniform) {
      ...
      out.push({ sev: same ? 'ok' : 'action', w: 'Line sizing', h: same ? 'Uniform ' + r.uniform.nps + '" throughout is correct'
      ...
      out.push({ sev: 'fail', w: 'Line sizing', h: 'No single size satisfies the whole drawing',
    ...
    if (t.velOk && t.eroOk && t.dpOk) out.unshift({ sev: 'ok', w: 'Overall', h: 'The drawn P&ID is hydraulically suitable',
  ...
  function render() {
    ...
    r.legs.forEach(function (l) {
      ...
      h += '<tr style="border-bottom:1px dashed var(--border-muted);color:var(--text-header);">'
    ...
    /* ── Review, split by what it asks of the engineer ──────────────────
       Every entry used to be painted amber unless its heading happened to
       contain "suitable" or "correct". A note explaining the K factor of a
       globe valve the engineer had placed on purpose therefore looked
       exactly like a failed check, and a route that passed everything still
       came back as a wall of warning boxes — which is why a finished
       auto-design read as though it were still broken.

       Entries now carry a severity and are grouped by it: what must change,
       then what merely needs looking at, then reference notes on what is on
       the drawing. The counts are on the headings, so "0 to fix" is
       visible without reading a single box. */
    var adv = advise(r);
    var SEV = {
      fail:   { rank: 0, col: '#f87171', bg: 'rgba(248,113,113,0.07)', head: 'MUST BE FIXED' },
      action: { rank: 1, col: '#f59e0b', bg: 'rgba(245,158,11,0.06)', head: 'WORTH CHANGING' },
      info:   { rank: 2, col: '#7ea2d8', bg: 'rgba(126,162,216,0.05)', head: 'ON THE DRAWING — FOR REFERENCE' },
      ok:     { rank: 3, col: '#22c55e', bg: 'rgba(34,197,94,0.07)', head: 'PASSING' }
    };
    var nFail = 0, nAct = 0;
    adv.forEach(function (a) {
      if (!a.sev) a.sev = /suitable|correct/.test(a.h) ? 'ok' : 'action';
      if (a.sev === 'fail') nFail++; else if (a.sev === 'action') nAct++;
    });
    h += '<div class="pid-cardh">REVIEW — WHAT, WHERE AND WHY'
      + '<span style="float:right;font-weight:700;color:' + (nFail ? '#f87171' : nAct ? '#f59e0b' : '#22c55e') + ';">'
      + (nFail || nAct
          ? nFail + ' to fix · ' + nAct + ' to consider'
          : '\u2713 nothing outstanding')
      + '</span></div>';
    ['fail', 'action', 'ok', 'info'].forEach(function (sv) {
      var group = adv.filter(function (a) { return a.sev === sv; });
      if (!group.length) return;
      var S = SEV[sv];
      h += '<div style="font-family:var(--font-mono);font-size:9px;font-weight:800;letter-spacing:0.07em;'
        + 'color:' + S.col + ';margin:10px 0 2px;opacity:0.85;">' + S.head + ' · ' + group.length + '</div>';
      group.forEach(function (a) {
        h += '<div style="border:1px solid var(--border-muted);border-left:3px solid ' + S.col
          + ';border-radius:4px;padding:7px 9px;margin:6px 0;background:' + S.bg + ';">'
          + '<div style="font-family:var(--font-mono);font-size:10px;font-weight:800;color:' + S.col + ';">'
          + esc(a.w) + ' — ' + esc(a.h) + '</div>'
          + '<div style="font-family:var(--font-mono);font-size:9.5px;color:#cbd5e1;line-height:1.55;margin-top:3px;">'
          + esc(a.why) + '</div></div>';
      });
    });
    el.innerHTML = h;
    ...
    if (VIEW3D) {
      update3D(r);
      var tg = $('pid-3dtag');
      if (tg) tg.textContent = tagText(r);
    }
    ...
    if (ub) ub.addEventListener('click', function () {
      ...
      DOC.items.forEach(function (it) { if (it.key === 'red' || it.key === 'ered') { it.d1 = r.uniform.nps; it.d2 = stepNps(r.uniform.nps, -1); it.sch = r.uniform.sch; } });
      applyReducers();
      // the panel sizes its hydraulics on its own pipe data, so it has to
      // learn the size the drawing just adopted or the two disagree
      pushSizeToPanel(r.uniform.nps, r.uniform.sch);
      render();
    });
  ...
  function syncSelPanel() {
    ...
    if (SEL.kind === 'seg') {
      ...
      $('pid-selred').style.display = 'none';
      $('pid-sel-nps').value = s.nps; $('pid-sel-sch').value = s.sch; $('pid-sel-col').value = s.colour; $('pid-sel-ltype').value = s.ltype || 'existing';
      ...
      $('pid-selred').style.display = (it.key === 'red' || it.key === 'ered') ? 'block' : 'none';
      if (it.key === 'red' || it.key === 'ered') {
        $('pid-red-d1').value = it.d1; $('pid-red-d2').value = it.d2; $('pid-red-sch').value = it.sch || defSch();
        var ba = bore(it.d1, it.sch || '40').Dmm, bb = bore(it.d2, it.sch || '40').Dmm;
        $('pid-rednote').innerHTML = esc(it.d1 + '\u2033 (' + f1(ba) + ' mm ID) \u2192 ' + it.d2 + '\u2033 (' + f1(bb) + ' mm ID)  \u00b7  \u03b2 '
          + f2(Math.min(ba, bb) / Math.max(ba, bb)) + '  \u00b7  K ' + redK(it))
          + '<br/>Every leg up to and including this one takes the FROM size; everything downstream takes the TO size.';
        $('pid-sel-title').textContent = 'REDUCER ' + it.d1 + '\u2033 \u00d7 ' + it.d2 + '\u2033  \u00b7  K ' + redK(it);
      }
    }
  }
  ...
  function paintPalette() {
    /* The flat ISA glyph is drawn first and immediately — it is the designed
       fallback, it is cheap 2D, and it means the palette is usable the
       instant the workbench opens.

       The 3D thumbnails then replace it a few at a time, each slice kept
       inside a single frame. Rendering all fifty in one synchronous loop is
       what froze the page when an engineer switched to MANUAL; the finished
       palette is the same either way, so the only thing that changed is that
       the workbench is not held hostage while it is built. */
    ...
    var imgs = [].slice.call(document.querySelectorAll('.pid-ico3'));
    var now = function () { return (window.performance && performance.now) ? performance.now() : Date.now(); };
    var queue = [], pumping = false;
    var raf = function (f) {
      return (typeof requestAnimationFrame === 'function') ? requestAnimationFrame(f) : setTimeout(f, 16);
    };
    function paintOne(img) {
    ...
    function pump() {
      if (pumping) return;
      pumping = true;
      PAINTQ = raf(function step() {
        var t0 = now();
        while (queue.length) {
          paintOne(queue.shift());
          if (now() - t0 > 10) break;            // one frame's worth, then yield
        }
        if (queue.length) PAINTQ = raf(step);
        else { pumping = false; PAINTQ = null; }
      });
    }
    function enqueue(img) {
      if (img.__pidQueued) return;
      img.__pidQueued = true;
      queue.push(img);
      pump();
    }
    /* Only the tiles the engineer can actually see are rendered. The palette
       is a long two-group list and most of it is below the fold when the
       workbench opens; rendering all of it up front spent seconds on
       thumbnails nobody had scrolled to yet. The rest arrive as they come
       into view, and each is still cached the first time it is made. */
  ...
  function fld(label, id, unit, v, step) {
    return '<label style="display:block;margin:4px 0;font-family:var(--font-mono);font-size:10px;color:var(--text-muted);">' + label
      ...
      + '<input id="' + id + '" type="number" step="' + (step || 'any') + '" value="' + v + '" style="flex:1;background:#fff;border:1px solid var(--border-muted);color:#0f172a;font-family:var(--font-mono);font-size:11px;padding:5px 7px;border-radius:3px;"/>'
      + (unit ? '<span style="font-size:9px;color:#64748b;min-width:52px;">' + unit + '</span>' : '') + '</span></label>';
  }
  function txtf(label, id, v) {
    return '<label style="display:block;margin:4px 0;font-family:var(--font-mono);font-size:10px;color:var(--text-muted);">' + label
      + '<input id="' + id + '" type="text" value="' + esc(v || '') + '" style="width:100%;margin-top:2px;background:#fff;border:1px solid var(--border-muted);color:#0f172a;font-family:var(--font-mono);font-size:11px;padding:5px 7px;border-radius:3px;box-sizing:border-box;"/></label>';
  }
  function sel(label, id, opts, cur) {
    return '<label style="display:block;margin:4px 0;font-family:var(--font-mono);font-size:10px;color:var(--text-muted);">' + label
      + '<select id="' + id + '" style="width:100%;margin-top:2px;background:#fff;border:1px solid var(--border-muted);color:#0f172a;font-family:var(--font-mono);font-size:11px;padding:5px 7px;border-radius:3px;">'
  ...
  function panelHTML() {
    ...
    var TBDIV = '<span class="pid-tbdiv"></span>';
    h += '<div class="pid-toolbar">'
      + '<span class="pid-tbgrp">'
      + '<button class="pid-tool" data-tool="line">✏ LINE</button>'
      + '<button class="pid-tool" data-tool="select">✥ SELECT / DRAG</button>'
      + '<button class="pid-tool" data-tool="pan">✋ MOVE / PAN</button>'
      ...
      + '<button id="pid-grid" class="pid-act">▦ GRID: ON</button>'
      + '</span>' + TBDIV
      + '<span class="pid-tbgrp">'
      ...
      + '<button id="pid-colmode" class="pid-act" style="display:none;">🎨 COLOUR: ' + COLOUR_MODE.toUpperCase() + '</button>'
      + '<button id="pid-flowvis" class="pid-act" style="display:none;">💧 FLUID: ' + (FLOW_VISIBLE ? 'VISIBLE' : 'HIDDEN') + '</button>'
      + '<button id="pid-autorot" class="pid-act" style="display:none;" title="Spin the model a continuous 360° — drag to take the camera back">⟳ 360°</button>'
      + '</span>' + TBDIV
      + '<span class="pid-tbgrp">'
      + '<label class="pid-act" title="Canvas / 3D background colour" style="padding:0 6px;overflow:hidden;position:relative;display:inline-flex;align-items:center;gap:5px;">🎨 BACKGROUND'
      + '<input id="pid-bg" type="color" value="' + BG_COLOR + '" style="width:22px;height:20px;padding:0;border:none;background:none;cursor:pointer;"/></label>'
      + '</span>' + TBDIV
      + '<span class="pid-tbgrp">'
      ...
      + '<input id="pid-scale" type="number" step="0.01" value="0.05" style="width:62px;background:#fff;border:1px solid var(--border-muted);color:#0f172a;font-family:var(--font-mono);font-size:10px;padding:4px 5px;border-radius:3px;"/>'
      + '<span style="font-size:9px;color:#64748b;">m/px</span></label></div>';

    /* The drawing row is canvas plus library, and nothing else. The library
       column is exactly as tall as the canvas and scrolls inside itself, so
       the tools stay beside the drawing while it is being made. Everything
       below - selection bar, verdict and the evaluation - then runs the full
       width instead of being squeezed into a column beside empty space. */
    var CANVAS_H = 470;
    h += '<div style="display:flex;gap:10px;align-items:flex-start;">';

    h += '<div style="flex:1;min-width:0;position:relative;">'
      + '<canvas id="pid-canvas" width="1180" height="' + CANVAS_H + '" style="width:100%;height:' + CANVAS_H + 'px;background:' + BG_COLOR + ';border:1px solid var(--border-muted);border-radius:6px;cursor:crosshair;display:block;"></canvas>'
      + '<canvas id="pid-3d" style="width:100%;height:' + CANVAS_H + 'px;background:' + BG_COLOR + ';border:1px solid var(--border-muted);border-radius:6px;display:none;cursor:grab;"></canvas>'
      + '<div id="pid-3dtag" style="display:none;position:absolute;left:10px;top:10px;max-width:calc(100% - 20px);font-family:var(--font-mono);font-size:10px;font-weight:700;color:#0f172a;background:rgba(255,255,255,0.9);border:1px solid var(--border-muted);padding:5px 9px;border-radius:4px;"></div>'
      /* The route can be any number of legs, so the figures cannot live in the
         scene — they collide as soon as two legs run close together. The model
         carries the tag only, and the tag is looked up here. */
      + '<div id="pid-3dkey" style="display:none;margin-top:8px;"></div>'
      + '</div>';

    h += '<div style="width:228px;flex:none;height:' + CANVAS_H + 'px;display:flex;flex-direction:column;border:1px solid var(--border-muted);border-radius:6px;background:#f8fafc;overflow:hidden;">'
      + '<div style="flex:none;padding:7px 8px 5px;border-bottom:1px solid var(--border-muted);">'
      + '<div style="font-family:var(--font-mono);font-size:10px;font-weight:800;color:var(--color-saffron);letter-spacing:0.05em;">COMPONENT LIBRARY</div>'
      + '<div style="font-family:var(--font-mono);font-size:8.5px;color:#94a3b8;line-height:1.4;margin-top:2px;">Click a component, then the line it goes on. Corner elbows are assumed.</div></div>'
      + '<div style="flex:1;min-height:0;overflow-y:auto;padding:6px 8px 8px;">' + paletteHTML() + '</div>'
      + '</div>';
    ...
    h += '<div id="pid-selbox" style="display:none;margin-top:8px;border:1px solid var(--color-saffron);border-radius:5px;padding:8px;background:rgba(255,117,56,0.06);">'
      ...
      + '<div id="pid-selred" style="display:none;margin-top:6px;">'
      + '<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(120px,1fr));gap:8px;align-items:end;">'
      + sel('FROM SIZE (upstream)', 'pid-red-d1', Object.keys(PIPE), '3')
      + sel('TO SIZE (downstream)', 'pid-red-d2', Object.keys(PIPE), '2')
      + sel('SCHEDULE', 'pid-red-sch', SCHEDULES, '40') + '</div>'
      + '<div id="pid-rednote" style="font-family:var(--font-mono);font-size:8.5px;color:#94a3b8;line-height:1.45;margin-top:4px;"></div></div>'
      ...
      + '<label style="display:block;margin:4px 0;font-family:var(--font-mono);font-size:10px;color:var(--text-muted);">LINE TYPE (ISA)'
      + '<select id="pid-sel-ltype" style="width:100%;margin-top:2px;background:#fff;border:1px solid var(--border-muted);color:#0f172a;font-family:var(--font-mono);font-size:11px;padding:5px 7px;border-radius:3px;"><optgroup label="Process Piping">'
      + Object.keys(LINE_TYPES).filter(function(k) { return LINE_TYPES[k].group === 'Process Piping'; }).map(function(k) { return '<option value="' + k + '">' + LINE_TYPES[k].label + '</option>'; }).join('')
      + '</optgroup><optgroup label="Pipeline Modification">'
      + Object.keys(LINE_TYPES).filter(function(k) { return LINE_TYPES[k].group === 'Pipeline Modification'; }).map(function(k) { return '<option value="' + k + '">' + LINE_TYPES[k].label + '</option>'; }).join('')
      + '</optgroup><optgroup label="Tracing / Jacket">'
      + Object.keys(LINE_TYPES).filter(function(k) { return LINE_TYPES[k].group === 'Tracing / Jacket'; }).map(function(k) { return '<option value="' + k + '">' + LINE_TYPES[k].label + '</option>'; }).join('')
      + '</optgroup><optgroup label="Instrument Connection">'
      + Object.keys(LINE_TYPES).filter(function(k) { return LINE_TYPES[k].group === 'Instrument Connection'; }).map(function(k) { return '<option value="' + k + '">' + LINE_TYPES[k].label + '</option>'; }).join('')
      + '</optgroup><optgroup label="Communication Signal">'
      + Object.keys(LINE_TYPES).filter(function(k) { return LINE_TYPES[k].group === 'Communication Signal'; }).map(function(k) { return '<option value="' + k + '">' + LINE_TYPES[k].label + '</option>'; }).join('')
  ...
  function css() {
    return '<style>'
      + '.pid-toolbar{display:flex;flex-wrap:wrap;gap:4px;align-items:center;margin-bottom:6px;background:#f1f5f9;border:1px solid var(--border-muted);border-radius:7px;padding:6px 8px;box-shadow:inset 0 1px 0 rgba(255,255,255,0.6);}'
      + '.pid-tbgrp{display:flex;flex-wrap:wrap;gap:4px;align-items:center;}'
      + '.pid-tbdiv{width:1px;align-self:stretch;min-height:22px;background:var(--border-muted);margin:0 3px;}'
      ...
      + '.pid-act-off{border-color:#94a3b8;color:#94a3b8;}'
      + '.pid-act.on{background:rgba(34,197,94,0.14);border-color:#22c55e;color:#22c55e;}'
      + '.pid-tool{background:#fff;border:1px solid var(--border-muted);color:#475569;font-family:var(--font-mono);font-size:10px;font-weight:700;padding:7px 4px;border-radius:4px;cursor:pointer;}'
      + '.pid-tool.on{border-color:#22c55e;color:#22c55e;background:rgba(34,197,94,0.10);}'
      + '.pid-lib{background:#fff;border:1px solid var(--border-muted);border-radius:4px;padding:4px 2px;cursor:pointer;font-family:var(--font-mono);font-size:8.5px;font-weight:700;line-height:1.25;text-align:center;}'
      ...
      + '.pid-rr span{color:var(--text-muted);}.pid-rr b{color:var(--text-header);}.pid-rr.ok b{color:#22c55e;}.pid-rr.warn b{color:#ef4444;}'
  ...
  function report() {
    ...
    var h = '<div style="font-family:Arial,sans-serif;color:#111827;">'
      + '<h2 style="text-align:center;color:#ea580c;margin:0;">AROGARA FLOWSIZE — P&amp;ID LINE HYDRAULICS REPORT</h2>'
      + '<div style="text-align:center;font-size:10px;color:#6b7280;">AROGARA FLOWSIZE · Digital Engineering Design Platform</div>'
      ...
      + '<div style="font-size:9.5px;color:#6b7280;margin-bottom:4px;">Columns are keyed A–L; the key table follows so every column fits the page without wrapping into its neighbour.</div>'
      + '<table style="width:100%;border-collapse:collapse;font-size:9.5px;table-layout:fixed;"><tr style="background:#f3f4f6;">'
      + ['A', 'B', 'C', 'D', 'E', 'F', 'G', 'H', 'I', 'J', 'K', 'L']
        .map(function (x, i) { return '<th style="padding:3px;text-align:left;width:' + (i === 11 ? 20 : i === 5 ? 9 : 6.5) + '%;">' + x + '</th>'; }).join('') + '</tr>'
      + r.legs.map(function (l) {
          return '<tr>' + ['L' + l.idx, l.nps + '" S' + l.sch, f2(l.L), f2(l.dz), f2(l.V), Math.round(l.Re).toLocaleString(),
            ...
            .map(function (c) { return '<td style="padding:3px;border-bottom:1px solid #e5e7eb;word-break:break-word;">' + c + '</td>'; }).join('') + '</tr>';
        }).join('') + '</table>'
      + '<div style="font-size:10px;font-weight:700;color:#374151;margin:8px 0 3px;">COLUMN KEY</div>'
      + '<table style="width:100%;border-collapse:collapse;font-size:10px;table-layout:fixed;">'
      + [['A', 'Leg reference as drawn on the P&ID'], ['B', 'Nominal size and schedule'],
         ['C', 'Developed leg length, m'], ['D', 'Static height change over the leg, m'],
         ['E', 'Mean velocity, m/s'], ['F', 'Reynolds number'],
         ['G', 'Sum of fitting resistance coefficients on the leg'],
         ['H', 'Friction pressure drop, bar'], ['I', 'Fittings pressure drop, bar'],
         ['J', 'Static pressure drop, bar'], ['K', 'Total leg pressure drop, bar'],
         ['L', 'Tagged components carried by the leg']]
        .map(function (x) { return '<tr><td style="padding:3px 6px;border-bottom:1px solid #e5e7eb;width:8%;font-weight:800;color:#ea580c;">' + x[0] + '</td><td style="padding:3px 6px;border-bottom:1px solid #e5e7eb;color:#374151;">' + esc(x[1]) + '</td></tr>'; }).join('')
      + '</table>'
      ...
      + allItems().map(function (it) {
          ...
          return '<tr><td style="padding:4px;border-bottom:1px solid #e5e7eb;">' + esc(it.tag) + ' — ' + esc(c.name) + '</td><td style="padding:4px;border-bottom:1px solid #e5e7eb;">L' + (DOC.segs.indexOf(s) + 1) + ' (' + (s ? s.nps : '—') + '")</td><td style="padding:4px;border-bottom:1px solid #e5e7eb;text-align:right;">' + kOf(it.key, s ? s.nps : 2, it) + '</td></tr>';
        }).join('')
  ...
  function modal(title, inner) {
    ...
    pb.onclick = function () {
      ...
      var p = window.AROPDF($('pid-mbody'), 'PID_Line_Hydraulics_Report.pdf', { landscape: true });
  ...
  function setView3D(on) {
    VIEW3D = on;
    var c2 = $('pid-canvas'), c3 = $('pid-3d'), tag = $('pid-3dtag'), key = $('pid-3dkey');
    if (!c2 || !c3) return;
    c2.style.display = on ? 'none' : 'block';
    c3.style.display = on ? 'block' : 'none';
    if (tag) tag.style.display = on ? 'block' : 'none';
    /* The key belongs to the 3D view — it explains the tags drawn on it. */
    if (key) key.style.display = (on && key.innerHTML) ? 'block' : 'none';
    var b2 = $('pid-v2d'), b3 = $('pid-v3d'), bc = $('pid-colmode'), bf = $('pid-flowvis'), br = $('pid-autorot');
    ...
    if (!on) { AUTO_ROTATE3D = false; if (br) br.classList.remove('on'); }   // leaving 3D always leaves the spin off, not silently running in the background
    if (on) {
      if (!three) init3D();
      var r = LASTR || compute();
      if (r) { update3D(r); if (tag) tag.textContent = tagText(r); }
  ...
  function tagText(r) {
    var picked = DOC.segs.filter(function (s) { return s.userColour; }).length;
    return r.legs.length + ' legs · ' + f2(r.tot.L) + ' m · ' + DOC.items.length + ' components · '
      + (picked ? (picked === DOC.segs.length ? 'colours as set on the P&ID' : picked + ' leg' + (picked > 1 ? 's' : '') + ' coloured on the P&ID, the rest ' + (COLOUR_MODE === 'material' ? 'by material' : 'by velocity'))
                : COLOUR_MODE === 'material' ? 'colour by pipe material — ' + esc(src().mat())
                : 'colour by velocity — green in band, amber outside 0.9–4.5 m/s, red over the erosional limit')
      + (FLOW_VISIBLE ? ' · flow animated in the direction drawn' : ' · fluid flow hidden — toggle FLUID: VISIBLE to show it')
      + (TOOL === 'pan' ? ' · MOVE/PAN tool is on — drag to move the view, scroll to zoom' : ' · drag to rotate, scroll to zoom — pick the MOVE/PAN tool to pan instead');
  }
  ...
  function setTool(t) {
    ...
    if (hint) hint.textContent = t === 'line' ? 'Click to start the run, then click at every change of direction. Double-click, press ESC, or press FINISH RUN to end it.'
      ...
      : t === 'pan' ? 'Drag anywhere to move the whole drawing — nothing on it changes. Middle-click or Shift+drag do the same in any other tool.'
      : 'Click a line or component to remove it.';
    if (CV) CV.style.cursor = t === 'delete' ? 'not-allowed' : t === 'pan' ? 'grab' : t === 'select' ? 'default' : 'crosshair';
    var c3 = $('pid-3d'); if (c3) c3.style.cursor = t === 'pan' ? 'move' : 'grab';
    // Keep the 3D caption's "drag to rotate / MOVE-PAN is on" line in step
    // if the tool changes while already looking at the 3D view.
    if (VIEW3D && LASTR) { var tag3 = $('pid-3dtag'); if (tag3) tag3.textContent = tagText(LASTR); }
  }
  ...
  function onDown(e) {
    if (PAN || e.button === 1 || e.shiftKey || TOOL === 'pan') return;   // panning, not drawing
    var p = xy(e);
  ...
  function onUp() {
    if (PAN) { PAN = null; CV.style.cursor = TOOL === 'pan' ? 'grab' : TOOL === 'select' ? 'default' : 'crosshair'; return; }
  ...
  function wire() {
    ...
    CV.addEventListener('mousedown', function (e) {
      // middle button, Shift+drag (any tool), or the MOVE/PAN tool itself
    ...
    [].slice.call(document.querySelectorAll('.pid-tool')).forEach(function (b) {
      /* .pid-tool is also used by the 2D/3D view switch buttons, which carry
         no data-tool — without this guard, clicking either of them called
         setTool(null), which (a === null matching every tool-less button)
         silently un-highlighted whichever draw tool was selected and reset
         TOOL to null, so a tool chosen before switching views didn't survive
         the switch. */
      var dt = b.getAttribute('data-tool');
      // Clicking the already-active tool again drops back to SELECT/DRAG
      // rather than re-applying the same tool with no visible change —
      // otherwise the only way out of PAN (or DELETE) is to remember to
      // pick a *different* button, which isn't obvious once it's engaged.
      if (dt) b.addEventListener('click', function () { setTool(TOOL === dt ? 'select' : dt); });
    });
    ...
    $('pid-colmode').addEventListener('click', function () {
      COLOUR_MODE = COLOUR_MODE === 'velocity' ? 'material' : 'velocity';
      $('pid-colmode').textContent = '🎨 COLOUR: ' + COLOUR_MODE.toUpperCase();
      var r = LASTR || compute();
      if (r) { update3D(r); var tag = $('pid-3dtag'); if (tag) tag.textContent = tagText(r); }
    });
    $('pid-flowvis').addEventListener('click', function () {
      FLOW_VISIBLE = !FLOW_VISIBLE;
      $('pid-flowvis').textContent = '💧 FLUID: ' + (FLOW_VISIBLE ? 'VISIBLE' : 'HIDDEN');
      /* No rebuild needed — the group already exists, so this is just a
         visibility flip (and the anim loop skips its buffer rewrite while
         hidden, so hiding the fluid also saves the per-frame update cost). */
      if (three && three.flowGroup) three.flowGroup.visible = FLOW_VISIBLE;
      var tag = $('pid-3dtag'); if (tag && LASTR) tag.textContent = tagText(LASTR);
    });
    $('pid-autorot').addEventListener('click', function () {
      AUTO_ROTATE3D = !AUTO_ROTATE3D;
      $('pid-autorot').classList.toggle('on', AUTO_ROTATE3D);
    });
    $('pid-bg').addEventListener('input', function () {
      BG_COLOR = $('pid-bg').value;
      var c2 = $('pid-canvas'), c3 = $('pid-3d');
      if (c2) c2.style.background = BG_COLOR;
      if (c3) c3.style.background = BG_COLOR;
      if (three && three.scene) three.scene.background = new THREE.Color(BG_COLOR);
    });
    ...
    $('pid-grid').addEventListener('click', function () {
      GRID_ON = !GRID_ON;
      $('pid-grid').textContent = '▦ GRID: ' + (GRID_ON ? 'ON' : 'OFF');
      $('pid-grid').classList.toggle('pid-act-off', !GRID_ON);
      render();
    });
    [].slice.call(document.querySelectorAll('.pid-sw')).forEach(function (b) {
      b.addEventListener('click', function () {
        ...
        if (SEL && SEL.kind === 'seg') { var sg = segById(SEL.id); if (sg) { sg.colour = col; sg.userColour = true; } }
        render();
      });
    });
    $('pid-colall').addEventListener('click', function () {
      var col = val('pid-sel-col', '#38bdf8');
      DOC.segs.forEach(function (sg) { sg.colour = col; sg.userColour = true; });
      render();
    });
    ...
    $('pid-applyall').addEventListener('click', function () {
      ...
      DOC.segs.forEach(function (s) { s.nps = n; s.sch = c; });
      DOC.items.forEach(function (it) { if (it.key === 'red' || it.key === 'ered') { it.d1 = n; it.d2 = stepNps(n, -1); it.sch = c; } });
      applyReducers();
      pushSizeToPanel(n, c);
      render();
    });
    (src().sizeIds || []).forEach(function (id) {
      var e = document.getElementById(id);
      if (e) e.addEventListener('change', function () {
        if (SIZESYNC) { refreshPaletteK(); return; }   // this change came FROM the drawing
        adoptPanelSize();
        refreshPaletteK(); applyReducers(); render();
      });
    });
    ...
    /* Changing a reducer's from/to pair re-sizes the run either side of it. */
    ['pid-red-d1', 'pid-red-d2', 'pid-red-sch'].forEach(function (id) {
      var e = $(id); if (!e) return;
      e.addEventListener('change', function () {
        if (!SEL || SEL.kind !== 'item') return;
        var it = DOC.items.filter(function (i) { return i.id === SEL.id; })[0];
        if (!it || (it.key !== 'red' && it.key !== 'ered')) return;
        it.d1 = parseFloat($('pid-red-d1').value);
        it.d2 = parseFloat($('pid-red-d2').value);
        it.sch = $('pid-red-sch').value;
        applyReducers();
        render();
      });
    });
    ['pid-sel-nps', 'pid-sel-sch', 'pid-sel-col', 'pid-sel-ltype'].forEach(function (id) {
      $(id).addEventListener('change', function () {
        if (!SEL || SEL.kind !== 'seg') return;
        var s = segById(SEL.id); if (!s) return;
        s.nps = parseFloat(val('pid-sel-nps', '2')); s.sch = val('pid-sel-sch', '40'); s.ltype = val('pid-sel-ltype', 'existing');
        var newCol = val('pid-sel-col', '#38bdf8');
        if (newCol !== s.colour) { s.colour = newCol; s.userColour = true; }
        render();
      });
    });
    var sc = $('pid-scale');
  ...
  /* One workbench, many hosts. Each panel keeps its own drawing, so moving
     between line types never mixes two routes together. */
  var DOCS = {}, HOST = null;
  function build(hostId, cfg) {
    var id = hostId || 'line-pid-content';
    var host = document.getElementById(id); if (!host) return;
    if (built && HOST === id) { if (cfg) { SRC = cfg; DEFSIZE = panelSize(); } render(); return; }

    if (HOST) {
      DOCS[HOST] = { segs: DOC.segs, items: DOC.items, tags: JSON.parse(JSON.stringify(TAGN)), uid: UID };
      /* Empty the previous host — its markup carries the same element ids, so
         leaving it in place makes getElementById find the hidden panel. */
      var prev = document.getElementById(HOST);
      if (prev && prev !== host) prev.innerHTML = '';
    }
    var keep = DOCS[id];
    DOC.segs = keep ? keep.segs : [];
    DOC.items = keep ? keep.items : [];
    Object.keys(TAGN).forEach(function (k) { delete TAGN[k]; });
    if (keep) { Object.keys(keep.tags).forEach(function (k) { TAGN[k] = keep.tags[k]; }); UID = keep.uid; }
    SEL = null; DRAG = null; ARMED = null; RUNNING = false; GHOST = null; LASTR = null; three = null;
    /* TOOL/PAN are module-level, but this singleton is reused for every line-
       sizing sub-tab — without resetting them here, a MOVE/PAN left engaged
       on one sub-tab silently carries into the next one's freshly-built
       toolbar (which shows nothing "on") and blocks drawing there too. */
    TOOL = 'line'; PAN = null;
    SRC = cfg || defSrc();
    HOST = id;
    ...
    built = true;
    wire();
    // anchor the drawing to the panel's current pipe data, so the first
    // change to NPS or schedule knows which legs were still following it
    DEFSIZE = panelSize();
    setTool(TOOL);
  }
  ...
  function summary() {
    ...
    allItems().forEach(function (it) {
      ...
      else { extraK += kOf(it.key, nps, it); extra.push({ tag: it.tag, name: lib(it.key).name, k: kOf(it.key, nps, it) }); }
    });
    ...
    /* The panel's flow view used to draw one generic pipe from the totals,
       whatever the engineer had actually routed — a four-leg run with a
       riser and two bores came back as a single straight tube with one
       velocity and one ΔP. The per-leg breakdown goes across too, with each
       leg's own geometry, bore, velocity and losses, so the view can be the
       route that was drawn rather than an average of it. */
    var cr = compute();
    var route = cr ? cr.legs.map(function (l) {
      return { idx: l.idx, nps: l.nps, sch: l.sch, Dmm: l.Dmm,
               L: l.L, dz: l.dz, horiz: l.horiz, V: l.V, sumK: l.sumK,
               dpPa: l.dpPa, dpFricPa: l.dpFricPa, dpFitPa: l.dpFitPa, dpStatPa: l.dpStatPa,
               x1: l.seg.x1, y1: l.seg.y1, x2: l.seg.x2, y2: l.seg.y2,
               items: (l.items || []).map(function (it) {
                 var c = lib(it.key) || {};
                 // colour and symbol family travel with the component so the
                 // flow view can draw it without a second component library
                 return { tag: it.tag, name: c.name, key: it.key, t: it.t,
                          colour: c.colour, sym: c.sym, k: kOf(it.key, l.nps, it) };
               }) };
    }) : [];
  ...
  window.AROPID = {
    ...
    addSeg: addSeg, addItem: addItem, applyReducers: applyReducers,
    ...
    reset: function () {
      ...
      DEFSIZE = panelSize();          // a fresh drawing follows the panel again
      Object.keys(TAGN).forEach(function (k) { delete TAGN[k]; });
      VIEW = { k: 1, ox: 0, oy: 0 };

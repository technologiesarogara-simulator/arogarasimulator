...
(function () {
  ...
  function paletteHTML() {
    ...
    function grp(title, list) {
      return '<div style="font-family:var(--font-mono);font-size:9px;font-weight:800;color:var(--color-saffron);letter-spacing:0.06em;margin:8px 0 4px;">' + title + '</div>'
        + '<div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(104px,1fr));gap:4px;">'
        + list.map(function (c) {
  ...
  function panelHTML() {
    ...
    h += '<div style="position:relative;">'
      + '<canvas id="pid-canvas" width="1180" height="430" style="width:100%;background:#0b1220;border:1px solid var(--border-muted);border-radius:6px;cursor:crosshair;display:block;"></canvas>'
      + '<canvas id="pid-3d" style="width:100%;height:430px;background:#0b1220;border:1px solid var(--border-muted);border-radius:6px;display:none;cursor:grab;"></canvas>'
    ...
    h += '<div id="pid-selbox" style="display:none;margin-top:8px;border:1px solid var(--color-saffron);border-radius:5px;padding:8px;background:rgba(255,117,56,0.06);">'
      ...
      + '<div id="pid-selsize" style="display:grid;grid-template-columns:repeat(auto-fit,minmax(130px,1fr));gap:8px;align-items:end;">'
    ...
    h += '<div style="margin-top:10px;font-family:var(--font-mono);font-size:9px;color:#94a3b8;">Pick a component, then click a line to drop it. Counts feed section 6 of the input column automatically.</div>';
    h += paletteHTML();
  ...
  function summary() {
    ...
    return { counts: counts, extra: extra, extraK: extraK, legs: DOC.segs.length,
             L: L, dz: rise - drop, rise: rise, drop: drop, items: DOC.items.length };
  }

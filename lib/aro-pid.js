...
(function () {
  ...
  var BG_COLOR = '#0b1220';                                  // canvas/scene background — user-set from the toolbar swatch
  ...
  function placeBendTag(g, x, y, ux, uy, vx, vy, R, it, c) {
    var nm = sprite(it.tag + (it.auto ? ' \u231d' : ''), it.auto ? '#7dd3fc' : c.colour);
  ...
  function update3D(r) {
    ...
    r.legs.forEach(function (l) {
      ...
      var Rb = R * 2.2;
      var isBendKey = function (k) { return /^(e90|e90lr|e45|m0|m30|m60|m90)$/.test(k); };
      ...
      var pipe = new THREE.Mesh(new THREE.CylinderGeometry(R, R, slen, 24, 1, true),
        new THREE.MeshStandardMaterial({ color: col, metalness: 0.8, roughness: 0.22 }));
      ...
      l.items.forEach(function (it, ii) {
        ...
        /* An elbow or mitre near a joint becomes the corner itself. It can be
           ...
           symbol. */
        var isBend = /^(e90|e90lr|e45|m0|m30|m60|m90)$/.test(it.key);
        if (isBend) {

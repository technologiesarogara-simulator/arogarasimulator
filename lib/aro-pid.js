...
(function () {
  ...
  function syncSelPanel() {
    ...
    if (SEL.kind === 'seg') {
      ...
      $('pid-sel-title').textContent = lib(it.key).name.toUpperCase() + '  ·  K ' + lib(it.key).k;
      ...
      return '<div style="font-family:var(--font-mono);font-size:9px;font-weight:800;color:var(--color-saffron);letter-spacing:0.06em;margin:8px 0 4px;">' + title + '</div>'
        ...
        + list.map(function (c) {
          return '<button class="pid-lib" data-key="' + c.key + '" title="' + esc(c.name) + ' — K ' + c.k + '">'
            ...
            + '<span style="display:block;color:#64748b;font-size:8px;">K ' + c.k + '</span></button>';
        }).join('') + '</div>';
    }
  ...
  function report() {
    ...
    var h = '<div style="font-family:Arial,sans-serif;color:#111827;">'
      ...
      + DOC.items.map(function (it) {
          ...
          return '<tr><td style="padding:4px;border-bottom:1px solid #e5e7eb;">' + esc(it.tag) + ' — ' + esc(c.name) + '</td><td style="padding:4px;border-bottom:1px solid #e5e7eb;">L' + (DOC.segs.indexOf(s) + 1) + '</td><td style="padding:4px;border-bottom:1px solid #e5e7eb;text-align:right;">' + c.k + '</td></tr>';
        }).join('')

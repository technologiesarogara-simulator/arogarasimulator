...
(function () {
  ...
  function init3D() {
    ...
    /* Ctrl/Cmd+scroll to zoom, same opt-in gesture every other 3D canvas in
       the app requires — a plain scroll always passes through to the page
       instead of getting captured by this preview. (This is the 3D route
       preview, #pid-3d — separate from the 2D sketch canvas below, which
       is a dedicated drawing surface and keeps plain-scroll zoom, the same
       CAD-tool convention the ARO Workbench's own 3D canvas uses.) */
  ...
  function tagText(r) {
    ...
    return r.legs.length + ' legs · ' + f2(r.tot.L) + ' m · ' + DOC.items.length + ' components · '
      ...
      + (TOOL === 'pan' ? ' · MOVE/PAN tool is on — drag to move the view, Ctrl/⌘+scroll to zoom' : ' · drag to rotate, Ctrl/⌘+scroll to zoom — pick the MOVE/PAN tool to pan instead');
  }

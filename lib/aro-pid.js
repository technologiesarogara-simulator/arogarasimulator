...
(function () {
  ...
  function render() {
    ...
    var r = LASTR = compute();
    if (!r) {
      el.innerHTML = '<div style="font-family:var(--font-mono);font-size:10px;color:#64748b;padding:8px;">Draw at least one line, then press EVALUATE.</div>';
      var v0 = $('pid-verdict');
  ...
  function panelHTML() {
    ...
    h += '<div style="display:flex;gap:4px;margin-top:5px;">'
      + '<button id="pid-zin" class="pid-act" style="flex:1;">🔍 ZOOM IN</button>'
      + '<button id="pid-zout" class="pid-act" style="flex:1;">🔍 ZOOM OUT</button>'
      + '<button id="pid-fit" class="pid-act" style="flex:1;">⤢ FIT</button></div>';
    h += '<div style="display:flex;gap:4px;margin-top:5px;">'
    ...
    h += '<div class="panel" style="max-height:calc(100vh - 200px);overflow-y:auto;">'
      ...
      + '<div id="pid-verdict" style="display:none;margin-top:10px;font-family:var(--font-mono);font-size:10.5px;font-weight:700;border-radius:5px;padding:8px 10px;line-height:1.45;"></div>'
      + '<div id="pid-results" style="margin-top:12px;"></div>'
      + '<div style="font-family:var(--font-mono);font-size:9.5px;color:#94a3b8;text-align:center;margin:14px 0 3px;">3D MODEL OF THE DRAWN LINE — BORE, LENGTH AND COMPONENTS FOLLOW THE SKETCH · DRAG TO ROTATE · SCROLL TO ZOOM</div>'
  ...
  function wire() {
    ...
    $('pid-fit').addEventListener('click', fitView);
    ...
    $('pid-eval').addEventListener('click', function () {
      render();
      /* The output lives further down a scrolling panel — bring it to the
         user rather than leaving the click looking like it did nothing. */
      var v = $('pid-verdict');
      if (v && v.style.display !== 'none') v.scrollIntoView({ behavior: 'smooth', block: 'center' });
    });

...
(function () {
  ...
  function update3D(r) {
    ...
    var flowMat = new THREE.MeshStandardMaterial({ color: 0xffffff, emissive: 0xbfe4ff, emissiveIntensity: 0.9, metalness: 0.1, roughness: 0.4 });
    var flowParts = [];
    ...
    r.legs.forEach(function (l) {
      ...
      var pipe = new THREE.Mesh(new THREE.CylinderGeometry(R, R, slen, 20),
      ...
      /* A pipe that only sits there reads as a solid rod, not a line in
         service — a handful of small glowing spheres travelling from L(idx)
         towards the next joint, at a speed keyed to the leg's own velocity,
         is enough to read as "fluid moving" without needing a real fluid
         simulation. They run from x1,y1 to x2,y2 in the drawn segment order,
         which is the same order the FROM/TO markers use in 2D, so the
         direction always matches the actual flow through the route. */
      if (isFinite(l.V) && l.V > 0) {
        var ux1 = (sbx - sax) / slen, uy1 = (sby - say) / slen;
        var nDrops = Math.max(2, Math.min(7, Math.round(slen / (R * 5))));
        var spd = Math.max(0.6, Math.min(9, l.V)) * (slen / nDrops) * 0.9;   // model-units/sec
        for (var fi = 0; fi < nDrops; fi++) {
          var sp = new THREE.Mesh(new THREE.SphereGeometry(R * 0.32, 10, 8), flowMat);
          var t0 = fi / nDrops;
          sp.position.set(sax + ux1 * slen * t0, say + uy1 * slen * t0, R * 0.02);
          g.add(sp);
          flowParts.push({ m: sp, ax: sax, ay: say, ux: ux1, uy: uy1, len: slen, t: t0, spd: spd });
        }
      }

      /* Tag only — bore, length and velocity are in the key beneath the view. */
      var lab = sprite('L' + l.idx, '#cbd5e1');
      lab.position.set((ax + bx) / 2, (ay + by) / 2 + (l.idx % 2 ? R + 2.6 : -(R + 2.6)), 0); g.add(lab);
    ...
    three.anim = flowParts.length ? function (dt) {
      for (var i = 0; i < flowParts.length; i++) {
        ...
        p.t += (p.spd * dt) / p.len;
        if (p.t > 1) p.t -= 1;
        p.m.position.set(p.ax + p.ux * p.len * p.t, p.ay + p.uy * p.len * p.t, p.m.position.z);
      }
    } : null;

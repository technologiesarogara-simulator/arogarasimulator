...
(function () {
  ...
  function update3D(r) {
    ...
    r.legs.forEach(function (l) {
      ...
      var trimA = (l.idx > 1 && allItems().some(function (it) {
        return isBendKey(it.key) && it.segId === r.legs[l.idx - 2].seg.id && it.t > 0.9;
      })) ? Rb : 0;
      var trimB = (l.idx < r.legs.length && allItems().some(function (it) {
        return isBendKey(it.key) && it.segId === s.id && it.t > 0.9;
      })) ? Rb : 0;
      ...
      l.items.forEach(function (it, ii) {
        ...
        /* An elbow or mitre sitting on a joint becomes the corner itself. */
        var nxt = (it.t > 0.9) ? r.legs[l.idx] : null;          // legs are 1-indexed by idx
        var isBend = /^(e90|e90lr|e45|m0|m30|m60|m90)$/.test(it.key);
        if (isBend && nxt) {
          var ns = nxt.seg;
          var nax = (ns.x1 - mid.x) * K, nay = -(ns.y1 - mid.y) * K;
          var nbx = (ns.x2 - mid.x) * K, nby = -(ns.y2 - mid.y) * K;
          var nlen = Math.hypot(nbx - nax, nby - nay);
          if (nlen > 1e-6) {
            var vx = (nbx - nax) / nlen, vy = (nby - nay) / nlen;
            if (Math.abs(vx - ux) > 1e-6 || Math.abs(vy - uy) > 1e-6) {
              g.add(bend3D({ x: bx, y: by }, { x: ux, y: uy }, { x: vx, y: vy }, R, col));
              /* Park the tag outside the turn, opposite the bend, so it
                 clears the corner and both leg labels. */
              var bn = sprite(it.tag + (it.auto ? ' ⌐' : '') + ' ' + c.name, it.auto ? '#7dd3fc' : c.colour);
              var outx = -(ux + vx), outy = -(uy + vy);
              var om = Math.hypot(outx, outy) || 1;
              bn.position.set(bx + (outx / om) * R * 7.0, by + (outy / om) * R * 7.0, 0);
              g.add(bn);
              return;
            }
          }

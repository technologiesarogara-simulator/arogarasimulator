/* ═══════════════════════════════════════════════════════════════════════
   AROGARA — 2D ENGINEERING DRAWING LAYER
...
(function () {
  ...
  function sheet(o, body) {
    ...
    s += K.txt(tbX + 8, tbY + rh - 4, 'AROGARA', { size: 10, weight: 'bold' });
  ...
  function svgFor(id) {
    var def = REG[id];
    if (!def) return null;
    /* A drawing that details one PART of a module's design (the impeller,
       the casing) rather than the module's own duty carries def.stateId so
       the title block reflects the calculation that actually produced its
       numbers, not a lookup against a module id that was never registered
       in AROSTATE and would always read back NOT CALCULATED. */
    var stId = def.stateId || id;
    var st = window.AROSTATE;
    if (st && st.modules && st.modules().indexOf(stId) >= 0 && !st.isCalculated(stId)) return null;
    var data;
    ...
    try {
      return sheet({
        ...
        w: def.w, h: def.h, block: blockFor(stId, def.block ? def.block(data) : null),
        superseded: st ? st.state(stId) === 'OUTDATED' : false
  ...
  function open(id) {
    ...
    var svg = svgFor(id);
    if (!svg) {
      modal('2D ENGINEERING DRAWING — ' + name,
  ...
  /* modal() escapes the title. Callers must therefore hand it the raw name —
     escaping it first put "Shell &amp; Tube Exchanger" in the dialog header. */
  ...
  /* ═══════════════════════════════════════════════════════════════════════
     ...
     The general-arrangement elevation above shows the impeller as an
     indicative disc inside the casing. This is the part itself: front view
     (looking down the shaft) and an axisymmetric side-section silhouette,
     dimensioned from the same Phase 4/5 numbers the results panel and the
     3D viewer already use — eye diameter, OD, vane count, blade exit angle.
     Vane curvature is drawn schematically (a quadratic sweep shaped by the
     computed β2, not a conformal hydraulic profile) — this is a published
     mid-range screening, not a vendor hydraulic design, and the sheet says
     so in its own numbers box.
     ═══════════════════════════════════════════════════════════════════════ */
  register('pump-impeller', {
    title: 'CENTRIFUGAL PUMP — IMPELLER DETAIL',
    stateId: 'pump',
    w: 940, h: 760,
    subtitle: function (d) {
      return 'FRONT VIEW + SIDE SECTION  ·  ' + esc(d.shapeFamily || '').toUpperCase()
        + ' IMPELLER  ·  Ns ' + f(d.Ns, 0) + ' (US)  ·  ' + d.vaneCount + ' VANES';
    },
    data: function () {
      var s = window.state && window.state.pump;
      var adv = window.pumpAdvancedState;
      if (!s || !s.calculated || !s.results || !adv) return null;
      var eu = adv.euler, cl = adv.classify;
      if (!eu || !eu.applicable || !cl || !cl.valid) return null;
      var moc = adv.mocImpeller;
      var shaft = adv.shaft;
      return {
        tag: (s.inputs && s.inputs.pumpTag) || 'P-101',
        shapeFamily: eu.shapeFamily, Ns: eu.Ns,
        D1_mm: cl.eyeRatio.mid * eu.D2_m * 1000, D2_mm: eu.D2_m * 1000,
        beta2Deg: eu.beta2Deg, vaneCount: Math.round(cl.vaneCount.mid),
        U2_ms: eu.U2_ms, Cm2_ms: eu.Cm2_ms,
        shaftDia_mm: (shaft && shaft.applicable && shaft.top) ? shaft.top.shaftDiameter_mm : NaN,
        material: (moc && moc.applicable && moc.top) ? moc.top.name : null
      };
    },
    block: function (d) {
      return [['TAG', d.tag], ['DUTY CLASS', String(d.shapeFamily || '—').toUpperCase()]];
    },
    draw: function (d, K, W, H) {
      var s = '';
      var ratio = Math.min(0.95, Math.max(0.15, d.D1_mm / d.D2_mm));

      /* ── front view — looking down the shaft ─────────────────────────── */
      var cx1 = 300, cy = 320, R2 = 148;
      var R1 = R2 * ratio;
      var pxPerMM = R2 / (d.D2_mm / 2);
      var Rhub = isFinite(d.shaftDia_mm) ? Math.max(14, Math.min(R1 - 10, (d.shaftDia_mm / 2) * pxPerMM * 2.2))
        : R1 * 0.34;
      R1 = Math.max(Rhub + 16, Math.min(R2 - 16, R1));

      s += K.centre(cx1 - R2 - 16, cy, cx1 + R2 + 16, cy);
      s += K.centre(cx1, cy - R2 - 16, cx1, cy + R2 + 16);

      var beta2c = Math.min(85, Math.max(5, d.beta2Deg));
      var sweep = 0.3 + (1 - beta2c / 90) * 1.15;
      for (var a = 0; a < d.vaneCount; a++) {
        var t0 = a * 2 * Math.PI / d.vaneCount;
        var t1 = t0 + sweep, tm = t0 + sweep * 0.55;
        var midR = (Rhub + R2) / 2 + (R2 - Rhub) * 0.06;
        var x0 = cx1 + Rhub * Math.cos(t0), y0 = cy + Rhub * Math.sin(t0);
        var xm = cx1 + midR * Math.cos(tm), ym = cy + midR * Math.sin(tm);
        var x1 = cx1 + (R2 - 7) * Math.cos(t1), y1 = cy + (R2 - 7) * Math.sin(t1);
        s += K.path('M' + x0.toFixed(1) + ' ' + y0.toFixed(1) + ' Q' + xm.toFixed(1) + ' ' + ym.toFixed(1)
          + ' ' + x1.toFixed(1) + ' ' + y1.toFixed(1), { stroke: K.THIN, w: 1.1 });
      }
      s += K.circle(cx1, cy, R2, { stroke: K.INK, w: 1.5, fill: 'none' });
      s += K.circle(cx1, cy, R1, { stroke: K.THIN, w: 0.8, dash: '5,3' });
      s += K.circle(cx1, cy, Rhub, { stroke: K.INK, w: 1.1, fill: K.METAL2 });

      s += K.dimH(cx1 - R2, cx1 + R2, cy + R2 + 34, 'D2 (IMPELLER OD)  ' + f(d.D2_mm, 1) + ' mm');
      s += K.leader(cx1 + R1 * Math.cos(-2.6), cy + R1 * Math.sin(-2.6), cx1 - 130, 168,
        'D1 (EYE)  ' + f(d.D1_mm, 1) + ' mm — HIDDEN', { anchor: 'end' });
      s += isFinite(d.shaftDia_mm)
        ? K.leader(cx1 + Rhub * Math.cos(0.9), cy + Rhub * Math.sin(0.9), cx1 + R2 + 20, 400,
            'HUB BORE  ⌀' + f(d.shaftDia_mm, 1) + ' mm  (from shaft sizing)')
        : K.leader(cx1 + Rhub * Math.cos(0.9), cy + Rhub * Math.sin(0.9), cx1 + R2 + 20, 400,
            'HUB BORE — shaft not sized yet');
      s += K.leader(cx1 + (R2 - 7) * Math.cos(sweep * 0.5), cy + (R2 - 7) * Math.sin(sweep * 0.5),
        cx1 + R2 + 20, 200, d.vaneCount + ' VANES  ·  β2 ' + f(d.beta2Deg, 1) + '°');
      s += K.txt(cx1, 128, 'FRONT VIEW — LOOKING DOWN THE SHAFT', { anchor: 'middle', size: 8.5, weight: 'bold', fill: K.THIN });

      /* ── side section — axisymmetric silhouette ──────────────────────── */
      var cx2 = 745, xL = cx2 - 95, xR = cx2 + 130;
      var R2s = 108, R1s = R2s * ratio, Rhubs = Math.max(12, R1s * 0.4);
      s += K.centre(xL - 40, cy, xR + 30, cy);
      s += K.path('M' + xL + ' ' + (cy - R1s) + ' Q' + (xL + (xR - xL) * 0.55) + ' ' + (cy - R1s * 0.55)
        + ' ' + xR + ' ' + (cy - R2s), { stroke: K.INK, w: 1.4 });
      s += K.path('M' + xL + ' ' + (cy + R1s) + ' Q' + (xL + (xR - xL) * 0.55) + ' ' + (cy + R1s * 0.55)
        + ' ' + xR + ' ' + (cy + R2s), { stroke: K.INK, w: 1.4 });
      s += K.line(xR, cy - R2s, xR, cy + R2s, { stroke: K.INK, w: 1.4 });
      s += K.rect(xL - 22, cy - Rhubs, 26, Rhubs * 2, { fill: K.METAL2, stroke: K.INK, w: 1.1 });
      if (isFinite(d.shaftDia_mm)) {
        var boreH = Math.max(4, Math.min(Rhubs * 1.4, (d.shaftDia_mm / 2) * pxPerMM * (R2s / R2) * 2));
        s += K.rect(xL - 34, cy - boreH / 2, 14, boreH, { fill: '#ffffff', stroke: K.INK, w: 0.9 });
      }
      s += K.dimV(cy - R1s, cy + R1s, xL - 46, 'D1  ' + f(d.D1_mm, 1) + ' mm', { from: xL - 70 });
      s += K.dimV(cy - R2s, cy + R2s, xR + 44, 'D2  ' + f(d.D2_mm, 1) + ' mm', { from: xR + 20 });
      s += K.txt(cx2, 128, 'SIDE SECTION — AXISYMMETRIC ABOUT ℄',
        { anchor: 'middle', size: 8.5, weight: 'bold', fill: K.THIN });
      s += K.txt(cx2, 142, '(axial width not screened by this calculation)',
        { anchor: 'middle', size: 7, fill: K.THIN });

      s += K.tag(cx1, 96, d.tag + ' — IMPELLER');
      s += K.txt(cx1, 142, 'rotation sense not specified by this screening',
        { anchor: 'middle', size: 7, fill: K.THIN });

      var nx = 40, ny = 520, nw = 860;
      s += K.rect(nx, ny, nw, 118, { stroke: K.INK, w: 0.9, fill: '#ffffff' });
      s += K.txt(nx + 8, ny + 15, 'IMPELLER DESIGN DATA  —  PRELIMINARY ASSUMPTION', { size: 8.5, weight: 'bold', fill: K.HOT });
      s += K.line(nx, ny + 21, nx + nw, ny + 21, { stroke: K.FAINT, w: 0.5 });
      var rows = [
        ['Shape family', String(d.shapeFamily || '—').toUpperCase()],
        ['Specific speed Ns (US)', f(d.Ns, 0)],
        ['Eye diameter D1', f(d.D1_mm, 1) + ' mm'],
        ['Impeller OD D2', f(d.D2_mm, 1) + ' mm'],
        ['Vane count / exit angle β2', d.vaneCount + '  /  ' + f(d.beta2Deg, 1) + '°'],
        ['Impeller tip speed U2', f(d.U2_ms, 2) + ' m/s'],
        ['Material of construction', d.material || 'Not screened — set fluid/pressure in the MOC panel']
      ];
      var half = Math.ceil(rows.length / 2), colW = nw / 2;
      rows.forEach(function (r, i2) {
        var col = i2 < half ? 0 : 1, row = i2 < half ? i2 : i2 - half;
        var bx = nx + 8 + col * colW, by = ny + 36 + row * 16;
        s += K.txt(bx, by, r[0], { size: 8, fill: K.THIN });
        s += K.txt(bx + colW - 16, by, r[1], { size: 8.5, weight: 'bold', anchor: 'end' });
      });
      return s;
    }
  });
  ...
  /* ═══════════════════════════════════════════════════════════════════════
     PUMP — casing detail
     The volute in plan section: cutwater clearance over the impeller OD,
     the spiral wall opening out to the calculated throat, and the suction
     and discharge connections. The spiral itself is drawn as a schematic
     Archimedean sweep from the cutwater to the throat — the calculation
     screens a throat velocity and a cutwater clearance, not a vendor
     volute profile, so the wall shape here is illustrative while every
     dimension called out on it is the one the calculation produced.
     ═══════════════════════════════════════════════════════════════════════ */
  register('pump-casing', {
    title: 'CENTRIFUGAL PUMP — CASING (VOLUTE) DETAIL',
    stateId: 'pump',
    w: 940, h: 760,
    subtitle: function (d) {
      return 'PLAN SECTION  ·  ' + esc(d.shapeFamily || '').toUpperCase() + ' VOLUTE  ·  THROAT D3(eq) '
        + f(d.D3eq_mm, 1) + ' mm  ·  CLASS ' + esc(d.cls || '—');
    },
    data: function () {
      var s = window.state && window.state.pump;
      var adv = window.pumpAdvancedState;
      if (!s || !s.calculated || !s.results || !adv) return null;
      var cs = adv.casing, eu = adv.euler;
      if (!cs || !cs.applicable || !eu || !eu.applicable) return null;
      var moc = adv.mocCasing, r = s.results;
      return {
        tag: (s.inputs && s.inputs.pumpTag) || 'P-101',
        shapeFamily: cs.shapeFamily, D2_mm: eu.D2_m * 1000,
        D3eq_mm: cs.volute.D3eq_mm, Vth_ms: cs.volute.Vth_ms, kVolute: cs.volute.kVolute,
        gapRadial_mm: cs.cutwater.gapRadial_mm, gapPct: cs.cutwater.gapPct, casingID_mm: cs.cutwater.casingID_mm,
        designPressBarG: cs.pressureClass.designPressBarG, cls: cs.pressureClass.cls,
        classMaxBarG: cs.pressureClass.classMaxBarG,
        sucNps: r.sucNozzle ? r.sucNozzle.nps : '—', disNps: r.disNozzle ? r.disNozzle.nps : '—',
        material: (moc && moc.applicable && moc.top) ? moc.top.name : null
      };
    },
    block: function (d) {
      return [['TAG', d.tag], ['PRESSURE CLASS', 'ASME ' + (d.cls || '—')]];
    },
    draw: function (d, K, W, H) {
      var s = '';
      var cx = 300, cy = 320, Rimp = 96;
      var pxPerMM = Rimp / (d.D2_mm / 2);
      var gapPx = Math.max(5, Math.min(30, d.gapRadial_mm * pxPerMM));
      var Rcut = Rimp + gapPx, Rmax = Rcut + Rimp * 0.62;

      s += K.centre(cx - Rmax - 20, cy, cx + Rmax + 130, cy);
      s += K.centre(cx, cy - Rmax - 20, cx, cy + Rmax + 20);
      s += K.circle(cx, cy, Rimp, { stroke: K.THIN, w: 0.8, dash: '5,3' });

      var N = 72, thetaStart = -Math.PI / 2, wrap = 2 * Math.PI * 0.93;
      var pts = [];
      for (var i = 0; i <= N; i++) {
        var frac = i / N, theta = thetaStart + frac * wrap;
        var R = Rcut + (Rmax - Rcut) * frac;
        pts.push([cx + R * Math.cos(theta), cy + R * Math.sin(theta)]);
      }
      var path = 'M' + pts[0][0].toFixed(1) + ' ' + pts[0][1].toFixed(1);
      for (var j = 1; j < pts.length; j++) path += ' L' + pts[j][0].toFixed(1) + ' ' + pts[j][1].toFixed(1);
      s += K.path(path, { stroke: K.INK, w: 1.5 });

      var cutX = pts[0][0], cutY = pts[0][1];
      s += K.circle(cutX, cutY, 3, { fill: K.INK, stroke: K.INK });
      s += K.leader(cutX, cutY, cutX + 10, cutY - 70,
        'CUTWATER CLEARANCE  ' + f(d.gapRadial_mm, 2) + ' mm  (' + f(d.gapPct, 1) + '% of D2)');

      var endPt = pts[pts.length - 1];
      var ux = (endPt[0] - cx), uy = (endPt[1] - cy);
      var un = Math.sqrt(ux * ux + uy * uy) || 1; ux /= un; uy /= un;
      var px = -uy, py = ux, nozW = 30, nozL = 78;
      var p1 = [endPt[0] + px * nozW / 2, endPt[1] + py * nozW / 2];
      var p2 = [endPt[0] - px * nozW / 2, endPt[1] - py * nozW / 2];
      var p3 = [endPt[0] + ux * nozL + px * nozW / 2, endPt[1] + uy * nozL + py * nozW / 2];
      var p4 = [endPt[0] + ux * nozL - px * nozW / 2, endPt[1] + uy * nozL - py * nozW / 2];
      s += K.path('M' + p1[0].toFixed(1) + ' ' + p1[1].toFixed(1) + ' L' + p3[0].toFixed(1) + ' ' + p3[1].toFixed(1)
        + ' L' + p4[0].toFixed(1) + ' ' + p4[1].toFixed(1) + ' L' + p2[0].toFixed(1) + ' ' + p2[1].toFixed(1) + ' Z',
        { stroke: K.INK, w: 1.2, fill: '#ffffff' });
      s += K.leader(endPt[0] + ux * nozL * 0.6, endPt[1] + uy * nozL * 0.6, cx + Rmax + 40, cy - 60,
        'DISCHARGE NOZZLE  NPS ' + d.disNps + '  (TANGENTIAL)');
      s += K.leader((endPt[0] + pts[Math.round(N * 0.7)][0]) / 2, (endPt[1] + pts[Math.round(N * 0.7)][1]) / 2,
        cx + Rmax + 40, cy + 10,
        'VOLUTE THROAT  D3(eq) ' + f(d.D3eq_mm, 1) + ' mm  ·  Vth ' + f(d.Vth_ms, 2) + ' m/s');

      var sucR = Rimp * 0.3;
      s += K.circle(cx, cy, sucR, { stroke: K.INK, w: 1.2, fill: '#ffffff' });
      s += K.circle(cx, cy, 4, { stroke: K.INK, w: 1, fill: K.INK });
      s += K.leader(cx + sucR * 0.7, cy + sucR * 0.7, cx + 40, cy + 160,
        'SUCTION — AXIAL INLET  NPS ' + d.sucNps + '  (into the page)');

      s += K.txt(cx, 128, 'PLAN SECTION — VOLUTE WALL SHOWN SCHEMATICALLY', { anchor: 'middle', size: 8.5, weight: 'bold', fill: K.THIN });
      s += K.tag(cx, 92, d.tag + ' — CASING');

      var nx = 40, ny = 530, nw = 860;
      s += K.rect(nx, ny, nw, 118, { stroke: K.INK, w: 0.9, fill: '#ffffff' });
      s += K.txt(nx + 8, ny + 15, 'CASING DESIGN DATA  —  PRELIMINARY ASSUMPTION', { size: 8.5, weight: 'bold', fill: K.HOT });
      s += K.line(nx, ny + 21, nx + nw, ny + 21, { stroke: K.FAINT, w: 0.5 });
      var rows = [
        ['Shape family', String(d.shapeFamily || '—').toUpperCase()],
        ['Volute throat velocity', f(d.Vth_ms, 2) + ' m/s  (k = ' + f(d.kVolute, 2) + ' × U2)'],
        ['Volute throat, equivalent D3', f(d.D3eq_mm, 1) + ' mm'],
        ['Cutwater radial clearance', f(d.gapRadial_mm, 2) + ' mm  (' + f(d.gapPct, 1) + '% of D2)'],
        ['Casing bore ID (impeller + clearance)', f(d.casingID_mm, 1) + ' mm'],
        ['Design pressure', f(d.designPressBarG, 1) + ' barg'],
        ['Pressure class (ASME B16.5)', 'CLASS ' + (d.cls || '—') + (isFinite(d.classMaxBarG) ? '  (max ' + f(d.classMaxBarG, 1) + ' barg)' : '')],
        ['Material of construction', d.material || 'Not screened — set fluid/pressure in the MOC panel']
      ];
      var half = Math.ceil(rows.length / 2), colW = nw / 2;
      rows.forEach(function (r, i2) {
  ...
  register('dphe', {
    ...
    raw: function (d) {
      if (typeof window.buildDPHEFabDrawingSVG !== 'function') return null;
      var svg = window.buildDPHEFabDrawingSVG(d);
      if (!svg) return null;
      /* The sheet carried an elevation, a nozzle schedule and general notes,
         and no parts list. The plate and shell-and-tube sheets both carry
         one, and a shop cannot order from a drawing that does not say what to
         order. Built from the same result the drawing above it is built from,
         so the two cannot disagree. */
      return svg + dpheBom(d);
    }
  });
  ...
  function dpheBom(d) {
    ...
    function mm(v) { return isFinite(v) ? (v * 1000).toFixed(1) : '—'; }
    var N = Math.round(g('dphe-hairpins', isFinite(d.nElbows) ? d.nElbows / 2 : NaN));
    if (!isFinite(N) || N < 1) N = isFinite(d.nElbows) ? Math.round(d.nElbows / 2) : 0;
    var L = isFinite(d.stdLength) ? d.stdLength : g('dphe-length', NaN);
    var ip = d.stdInnerPipe || {}, op = d.stdOuterPipe || {};
    var Di = g('dphe-di', NaN), Do = g('dphe-do', NaN), D2 = g('dphe-d2', NaN);
    var legLen = isFinite(L) ? L : NaN;
    var totalIn = isFinite(d.totalPipeLen) ? d.totalPipeLen : (isFinite(legLen) ? N * 2 * legLen : NaN);
    var matH = d.matHot || 'CS', matC = d.matCold || 'CS';
    var tn = d.tubeNozPipe || {}, an = d.annNozPipe || {};
    var rows = [
      ['Inner pipe (tube side)', matC + ' — ASTM A106 Gr.B', String(N * 2),
        (ip.nps ? 'NPS ' + ip.nps + '" ' : '') + 'Ø' + mm(Do) + ' OD × ' + f(legLen, 2) + ' m',
        'ASME B36.10'],
      ['Outer pipe (annulus)', matH + ' — ASTM A106 Gr.B', String(N * 2),
        (op.nps ? 'NPS ' + op.nps + '" ' : '') + 'Ø' + mm(D2) + ' ID × ' + f(legLen, 2) + ' m',
        'ASME B36.10'],
      ['Return bend, 180° LR', matC, String(N), 'Ø' + mm(Do) + ' OD, R = 1.5 D', 'ASME B16.9'],
      ['Return bonnet / annulus cover', matH, String(N), 'bolted, gasketed', 'shop fabricated'],
      ['Shell closure flange', matH, String(N * 4), 'CL150 WN-RF', 'ASME B16.5'],
      ['Gland / packing box', matH, String(N * 2), 'for inner pipe entry', 'shop fabricated'],
      ['Series jumper (U-bend)', matC, String(Math.max(0, N - 1)), 'tube side, hairpin to hairpin', 'ASME B16.9'],
      ['Annulus nozzle + flange', matH, String(N * 2),
        (an.nps ? 'NPS ' + an.nps + '" ' : '') + 'CL150 RF', 'ASME B16.5'],
      ['Tube-side nozzle + flange', matC, '2',
        (tn.nps ? 'NPS ' + tn.nps + '" ' : '') + 'CL150 RF', 'ASME B16.5'],
      ['Gasket, spiral wound', 'SS316 / graphite', String(N * 4), 'to suit CL150 RF', 'ASME B16.20'],
      ['Stud bolt + 2 nuts', 'A193 B7 / A194 2H', String(N * 16), 'M16 (5/8" UNC)', 'ASME B16.5'],
      ['Saddle support', 'ASTM A36', String(Math.max(2, Math.ceil(N / 4))),
        'max span 3000 mm', 'with wear plate'],
      ['Support frame / rack', 'ASTM A36', '1', 'stacked hairpin bank', 'shop fabricated'],
      ['Vent and drain', 'A105', '4', 'NPS 3/4" CPLG', 'ASME B16.11'],
      ['Nameplate + bracket', 'SS304', '1', 'laser etched', 'ASME U-stamp']
    ];
    var h = '<div style="margin-top:14px;font-family:Arial;">'
      + '<div style="font-size:13px;font-weight:800;color:#ea580c;border-bottom:2px solid #ea580c;'
      + 'padding-bottom:3px;margin-bottom:6px;">BILL OF MATERIAL — LIST OF MATERIALS</div>'
      + '<table style="width:100%;border-collapse:collapse;font-size:11px;">'
      + '<tr style="background:#f1f5f9;">'
      + ['#', 'Item / Component', 'Material', 'Qty', 'Size / Description', 'Standard'].map(function (x) {
        return '<th style="padding:5px 6px;border:1px solid #cbd5e1;text-align:left;">' + esc(x) + '</th>';
      }).join('') + '</tr>';
    rows.forEach(function (r, i) {
      h += '<tr>' + [i + 1, r[0], r[1], r[2], r[3], r[4]].map(function (x) {
        return '<td style="padding:4px 6px;border:1px solid #e2e8f0;">' + esc(x) + '</td>';
      }).join('') + '</tr>';
    });
    h += '</table><div style="font-size:10px;color:#64748b;margin-top:6px;">'
      + 'Quantities follow the calculated geometry — ' + N + ' hairpins, '
      + f(totalIn, 1) + ' m of each pipe. Materials are a starting specification, not a '
      + 'selection: confirm against the service and the mechanical design before ordering.'
      + '</div></div>';
    return h;
  }
  ...
  /* ═══════════════════════════════════════════════════════════════════════
     ...
     The plate module has carried a complete fabrication sheet all along —
     frame elevation, plate detail on the calculated chevron, port and gasket
     arrangement, tie-bolt schedule, a bill of material and a title block. It
     built that sheet inside a function that opened its own dialog and
     returned nothing, so the platform's 2D DRAWING control could not reach
     it and told the engineer NO DRAWING FOR THIS MODULE YET while the sheet
     sat one button away. The builder is now separable, and this is the other
     caller: the same sheet, from the same result, through the same control
     as every other module.
     ═══════════════════════════════════════════════════════════════════════ */
  register('phe', {
    title: 'PLATE HEAT EXCHANGER — FABRICATION / GA DRAWING',
    data: function () {
      /* Only when the design has actually been run. The builder would happily
         compute one on demand, and a drawing produced by a calculation the
         engineer never asked for is a drawing of a design that does not
         exist. */
      try {
        if (window.AROSTATE && window.AROSTATE.state('phe') === 'NOT_CALCULATED') return null;
      } catch (e) {}
      return (window.AROPHE && typeof window.AROPHE.buildDrawing === 'function') ? true : null;
    },
    raw: function () {
      try {
        var d = window.AROPHE.buildDrawing();
        if (!d || !d.svg) return null;
        /* The bill of material travels with the sheet — it is part of what
           makes it a manufacturing document rather than a picture. */
        return d.svg + (d.bom || '');
      } catch (e) { return null; }
    }
  });
  ...
  register('sthe', {
    ...
    data: function () {
      ...
      /* Read the declared unit rather than guessing it from the magnitude —
         the length field is data-unit-type="length-mm" and reads in
         millimetres, the same reading the calculation takes. Guessing meant
         the drawing could state a tube length a thousand times the one that
         was sized, and reading the raw box would have been wrong again in
         US units, where the same field is inches. */
      function dom(id) {
        if (window.siOf) { var s = window.siOf(id, NaN); if (isFinite(s)) return s; }
        var e = document.getElementById(id);
        return e ? parseFloat(e.value) : NaN;
      }
      var Lmm = dom('sthe-tube-L');

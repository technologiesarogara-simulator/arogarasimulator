...
(function () {
  ...
  /* Comfortable-sizing-margin check, distinct from plain range
     containment: a duty flow sitting right at the edge of a family's
     rated range leaves no headroom (early wear on the low end,
     runout risk on the high end) even though it technically "fits". */
  function sizingCriterion(Q, range) {
    if (Q == null || !isFinite(Q) || !range || range[0] == null) return 'marginal';
    var lo = range[0], hi = range[1];
    if (Q < lo || Q > hi) return 'fail';
    var frac = (hi > lo) ? (Q - lo) / (hi - lo) : 0.5;
    if (frac < 0.1 || frac > 0.9) return 'marginal';
    return 'pass';
  }
  ...
  /* ── selectFamilies: the ranked comparison across all 23 reference rows
     duty = { Q_m3h, H_m, viscosityCst, npshMarginM (optional),
              ...
              pulsationSensitive (bool), tempC (optional),
              fluidKey (optional, for material screening),
              designPressBarG (optional, for material screening) }
  ...
  function selectFamilies(duty) {
    ...
    var tempC = (duty.tempC == null) ? null : duty.tempC;
    var fluidKey = duty.fluidKey || null, designPressBarG = (duty.designPressBarG == null) ? null : duty.designPressBarG;

    /* Casing material screening is duty-driven (fluid/temp/pressure), not
       family-driven — the same candidate material set is being asked
       "does anything here survive this service" regardless of which
       pump type is being ranked. Computed once, read by every row,
       exactly like AROPUMPMOC.screenMaterials is already called
       elsewhere in this suite. */
    var materialScreen = null;
    if (window.AROPUMPMOC && fluidKey && isFinite(tempC) && isFinite(designPressBarG)) {
      materialScreen = window.AROPUMPMOC.screenMaterials({ component: 'casing', fluidKey: fluidKey, tempC: tempC, designPressBarG: designPressBarG });
    }
    ...
    var ranked = rows.map(function (fam) {
      ...
      // Temperature suitability against the family's typical rated range.
      // Informational only (no score weight) — flow/head/viscosity plus
      // the existing duty-character adjustments already drive the
      // ranking, and every regression test in this file was tuned
      // against that balance; these three new rows answer "is this
      // param OK" without re-tuning "which family wins".
      var tempKnown = tempC != null && isFinite(tempC);
      var tempFit = fitScore(tempC, fam.tempRangeC);
      criteria.temp = tempKnown ? criterionVerdict(tempFit) : 'marginal';
      if (tempKnown) {
        reasons.push(describeFit('temperature', tempC, fam.tempRangeC, '°C'));
        if (criteria.temp === 'fail') {
          warnings.push('Operating temperature ' + fmt(tempC) + ' °C falls outside this family\'s typical ' + fmt(fam.tempRangeC[0]) + '–' + fmt(fam.tempRangeC[1]) + ' °C range.');
        }
      }

      // Recommended material selection suitability — a real material
      // screening verdict (window.AROPUMPMOC), never a guessed pass/fail.
      // The same screen applies to every row (see materialScreen above),
      // so a fluid/temp/pressure combination that has no comfortable
      // material shows up as a shared caution across the whole shortlist
      // rather than silently favouring one family over another.
      var materialTop = null;
      if (materialScreen && materialScreen.applicable) {
        materialTop = materialScreen.top;
        if (materialTop.verdict === 'SUITABLE') { criteria.material = 'pass'; }
        else if (materialTop.verdict === 'CHECK') { criteria.material = 'marginal'; }
        else { criteria.material = 'fail'; warnings.push('Best-screened casing material (' + materialTop.name + ') is only ' + materialTop.verdict + ' for this fluid/temperature/pressure — confirm construction material before proceeding.'); }
      } else {
        criteria.material = 'marginal';
      }

      // Pump sizing/capacity suitability — a comfortable rated-capacity
      // margin check, distinct from the plain flow-range containment
      // the "flow" criterion above already gives: a duty sitting right
      // at a family's rated edge technically "fits" but leaves no
      // sizing headroom (early wear at the low end, runout risk high).
      criteria.sizing = sizingCriterion(Q, fam.flowRangeM3h);
      if (criteria.sizing === 'marginal') {
        warnings.push('Duty flow sits within 10% of this family\'s rated range edge — thin sizing margin; consider the next size up/down in a vendor selection.');
      }
      ...
      return {
        ...
        npshCharacteristic: fam.npshCharacteristic || null,
    ...
    return {
      ...
      duty: { Q_m3h: Q, H_m: H, viscosityCst: visc, npshMarginM: npshMargin,
        ...
        tempC: tempC, fluidKey: fluidKey, designPressBarG: designPressBarG },
      viscosity: visco,
      ranked: ranked,
      top: top,

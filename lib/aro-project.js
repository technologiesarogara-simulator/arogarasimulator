...
(function () {
  ...
  /* ── Cloud sync (Firestore, via window.AROCLOUD) ────────────────────────
     localStorage stays the store render() reads from — this is a
     write-through mirror onto it, not a second source of truth. A signed-in
     engineer's projects live in projects/{id} (module:'project') and follow
     them to any device; a demo session or a signed-out visitor has no uid,
     so every call below is a silent no-op and the app behaves exactly as it
     always did — local-only. */
  function cloudSignedIn() {
    try { return !!(window.AROCLOUD && window.AROCLOUD.uid()); } catch (e) { return false; }
  }
  function pushCloud(rec) {
    if (!cloudSignedIn()) return;
    try { window.AROCLOUD.saveProject('project', rec.projectName || 'Untitled', rec, rec.id); } catch (e) {}
  }
  function deleteCloud(id) {
    if (!cloudSignedIn()) return;
    try { window.AROCLOUD.deleteProject(id); } catch (e) {}
  }
  /* Pull every project Firestore has for this account and merge it into the
     local archive (cloud wins when it is newer — e.g. the same account
     saved from another device). writeRecord() is the same primitive the
     JSON-import path already uses to land a full record + index entry. */
  function syncCloudProjects() {
    if (!window.AROCLOUD || !cloudSignedIn()) return;
    window.AROCLOUD.listProjects('project').then(function (list) {
      var list0 = archive(), changed = false;
      list.forEach(function (rec) {
        var full = rec.payload;
        if (!full || typeof full !== 'object') return;
        full.id = rec.id;
        full.modifiedAt = rec.updatedAt || full.modifiedAt;
        full.createdAt = rec.createdAt || full.createdAt;
        var local = list0[rec.id];
        if (!local || String(full.modifiedAt || '') > String(local.modifiedAt || '')) {
          writeRecord(full);
          changed = true;
        }
      });
  ...
  window.addEventListener('aro-cloud-signin', syncCloudProjects);
  ...
  function projectsBlock() {
    ...
    if (!keys.length) {
      h += '<div class="aro-pj-empty">' + (cloudSignedIn()
        ? 'No projects saved to your account yet. Create one above &mdash; it syncs to your AROGARA account and follows you to any device you sign in on.'
        : 'No projects saved in this browser yet. Sign in with a password, Google or OTP account (not Developer access) to have projects sync to your account across devices &mdash; or use EXPORT PROJECT to take one elsewhere.') + '</div>';
    } else {
  ...
  function reportHtml() {
    ...
    h += sec(1, 'PROJECT INFORMATION', tbl(['ITEM', 'VALUE'], [
      ...
      ['Software', 'AROGARA — engine v' + (window.AROENG ? window.AROENG.version : '—')]
    ...
    h += '<div style="margin-top:20px;border-top:1px solid #d1d5db;padding-top:8px;font-size:9px;color:#6b7280;">'
      + 'AROGARA — Digital Engineering Design Platform. This report is a calculation record. '
  ...
  /* ── Legacy top-bar store import ──────────────────────────────────────
     Before this file's store became the single project record, the top
     bar's own SAVE wrote a completely different shape into aro_projects_v1
     — a flat map of {id: {name, rev, client, engineer, note, fields, ...}},
     with no equipment/lines concept at all. Those saves are real work and
     must not vanish; they are converted into this file's schema (as an
     equipment-less, line-less project carrying the same whole-workspace
     field snapshot) and added to the one archive both UIs now read. The
     source key is left exactly as it was — this only ever adds. */
  var LS_LEGACY_STORE = 'aro_projects_v1';
  function legacySnapToRecord(id, snap) {
    return {
      id: id, projectName: snap.name || 'UNTITLED (imported)', client: snap.client || '',
      projectNumber: '', location: '', engineer: snap.engineer || '', company: '',
      description: snap.note || '', revision: snap.rev || 'A',
      date: String(snap.created || nowISO()).slice(0, 10),
      createdAt: snap.created || nowISO(), modifiedAt: snap.modified || nowISO(),
      equipment: [], lines: [],
      fields: snap.fields || {}, modulesSnapshot: snap.modules || {},
      units: snap.units, engine: snap.engine, importedFromLegacy: true
    };
  }
  /* Writes one project record straight into the unified store and its
     index — the common tail of both the bulk legacy import below and a
     single-file IMPORT JSON, so there is one place that knows the list
     entry's shape has to match the full record's. */
  function writeRecord(rec) {
    var list = archive();
    try {
      localStorage.setItem(LS + ':' + rec.id, JSON.stringify(rec));
      list[rec.id] = { id: rec.id, projectName: rec.projectName, projectNumber: rec.projectNumber,
                       client: rec.client, revision: rec.revision, modifiedAt: rec.modifiedAt };
      localStorage.setItem(LS_LIST, JSON.stringify(list));
      return true;
  ...
  function importLegacyStore() {
    var legacy;
    try { legacy = JSON.parse(localStorage.getItem(LS_LEGACY_STORE) || '{}'); } catch (e) { return 0; }
    var ids = Object.keys(legacy);
    if (!ids.length) return 0;
    var list = archive();
    var n = 0;
    ids.forEach(function (id) {
      if (list[id]) return;                       // already unified, nothing to do
      var snap = legacy[id];
      if (!snap || typeof snap !== 'object') return;
      if (writeRecord(legacySnapToRecord(id, snap))) n++;
    });
    return n;
  }
  /* A single imported file, from either era: a full unified project
     (has its own equipment/lines) is written and opened as-is; an older
     flat workspace snapshot (name/rev/note/fields, no equipment concept)
     is converted the same way the bulk legacy import converts one. */
  function importJSON(obj) {
    if (!obj || typeof obj !== 'object') return null;
    var id = obj.id || uid('prj');
    var rec = (obj.equipment !== undefined || obj.projectName !== undefined)
      ? (function () { var r = JSON.parse(JSON.stringify(obj)); r.id = id;
                        r.equipment = r.equipment || []; r.lines = r.lines || []; return r; })()
      : legacySnapToRecord(id, obj);
    if (!writeRecord(rec)) return null;
    openProject(id);
    return id;
  }
  ...
  function boot() {
    injectCss();
    P = load();
    try { importLegacyStore(); } catch (e) {}
    render();
    // Pull cloud projects as soon as the account is known — either already
    // signed in this tab, or (the common case on a reload) once
    // arogara-auth.js finishes silently restoring the Firebase session.
    if (window.AROAUTH && window.AROAUTH.whenReady) window.AROAUTH.whenReady().then(syncCloudProjects);
    else syncCloudProjects();
  ...
  window.AROPROJECT = {
    ...
    /* ── Storage primitives for other UIs on the same project ──────────
       aro-engineering.js's own Save/Open bar used to keep an entirely
       separate store (aro_projects_v1 / aro_project_current_v1) with a
       different schema, so an engineer could save from the top bar and
       never see that work in the PROJECT tab's own SAVED PROJECTS list,
       or the reverse. These let it read and write through the SAME
       project record this file already owns, so there is exactly one
       store on disk — the top bar keeps its own UI, buttons and
       whole-workspace field snapshot, it simply no longer forks the data. */
    list: function () { return archive(); },
    loadById: function (id) { openProject(id); },
    deleteById: function (id) {
      var s = archive(); delete s[id];
      try { localStorage.setItem(LS_LIST, JSON.stringify(s)); localStorage.removeItem(LS + ':' + id); } catch (e) {}
      deleteCloud(id);
    },
    newProject: function () {
      try { localStorage.removeItem(LS); } catch (e) {}
      P = null; dirty = false; closeContext(); ensureWorkspace();
    },
    /* Merge project-detail fields (name/client/engineer/rev/note, in the
       vocabulary the caller already uses) into whatever project is open,
       creating the untitled workspace first if none is. Never touches
       equipment/lines. */
    setDetails: function (patch) {
      ensureWorkspace();
      if (patch.name != null) P.projectName = patch.name;
      if (patch.client != null) P.client = patch.client;
      if (patch.engineer != null) P.engineer = patch.engineer;
      if (patch.rev != null) P.revision = patch.rev;
      if (patch.note != null) P.description = patch.note;
      touch();
    },
    /* The whole-workspace field snapshot the top bar's SAVE has always
       taken — kept as an optional bag on the same record rather than a
       parallel document, so a project saved from either UI is one project. */
    setFields: function (map) { ensureWorkspace(); P.fields = map; },
    getFields: function () { return (P && P.fields) || {}; },
    setModulesSnapshot: function (map) { ensureWorkspace(); P.modulesSnapshot = map; },
    getModulesSnapshot: function () { return (P && P.modulesSnapshot) || {}; },
    setUnits: function (u) { if (P) P.units = u; },
    getUnits: function () { return P ? P.units : null; },
    save: function () { ensureWorkspace(); save(); },
    id: function () { return P ? P.id : null; },
    /* A copy under a new name and a new id, carrying the same equipment,
       lines and field snapshot forward — "save a copy" should not mean
       "lose every tag that was on it". */
    saveAs: function (newName) {
      ensureWorkspace();
      var c = JSON.parse(JSON.stringify(P));
      c.id = uid('prj');
      c.projectName = newName || (P.projectName + ' COPY');
      c.createdAt = nowISO();
      delete c.transient;
      P = c;
      save();
    ...
    /* One-time absorption of the legacy top-bar store (aro_projects_v1),
       so its saves show up in the unified list instead of staying invisible
       to the PROJECT tab forever. Never deletes the legacy copy — this is
       an import, not a migration, and only fills in ids the unified list
       does not already have. */
    importLegacyStoreIfNeeded: importLegacyStore,
    /* A single imported file — either a full unified export (equipment,
       lines and all) or an older flat workspace snapshot — written into
       the one store and opened. Returns the id, or null if the object
       was not recognisable as either shape. */

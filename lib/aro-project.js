...
(function () {
  ...
  var REG = {
    ...
    'line-gas': {
      ...
      tagPrefix: 'L', tab: 'line-tab', state: 'line-gas', seed: function (o) { seedLine('gs', o); }
    },
    'line-steam': {
      kind: 'line', label: 'Steam Line Sizing', type: 'Process Line — Steam',
      tagPrefix: 'L', tab: 'line-tab', state: 'line-steam', seed: function (o) { seedLine('st', o); }
    },
    'line-slurry': {
      kind: 'line', label: 'Slurry Line Sizing', type: 'Process Line — Slurry',
      tagPrefix: 'L', tab: 'line-tab', state: 'line-slurry', seed: function (o) { seedLine('sl', o); }
    },
    'line-twophase': {
      kind: 'line', label: 'Two-Phase Line Sizing', type: 'Process Line — Two-Phase',
      tagPrefix: 'L', tab: 'line-tab', state: 'line-twophase', seed: function (o) { seedLine('tp2', o); }
    }
  };
  ...
  function render() {
    ...
    if (!host) return;
    host.innerHTML = P ? projectHtml() : welcomeHtml();
    contextStrip();
  }
  ...
  function field(k, label, val, wide) {
    return '<div' + (wide ? ' style="grid-column:1/-1;"' : '') + '><label>' + esc(label) + '</label>'
      + '<input id="aro-pf-' + k + '" value="' + esc(val || '') + '"></div>';
  }
  function formHtml(o) {
  ...
  function projectHtml() {
    ...
    h += '<h2>AROGARA PROJECT &mdash; ' + esc(P.projectName || 'UNTITLED') + (dirty ? ' *' : '') + '</h2>'

...
(function () {
  ...
  WB.report = function () {
    ...
    var tables = '<h4>Equipment Technical Data Sheet</h4><table class="wb-table"><tr><th>#</th><th>Tag</th><th>Name</th><th>Type</th><th>Location</th><th>Fluid</th><th>Flow m³/h</th><th>T °C</th><th>P barg</th></tr>' + (eq || '<tr><td colspan="9">No equipment placed.</td></tr>') + '</table>'
      ...
      + legend;
    setStatus('Building 2D + 3D views for the report…', '#38bdf8');
    capture2D(function (img2d) {
      capture3D(function (img3d) {
        var imgStyle = 'max-width:100%;display:block;margin:0 auto;border:1px solid #cbd5e1;border-radius:4px;';
        var views = '<h4>2D P&amp;ID Schematic View</h4>' + (img2d ? '<div class="wb-rep-view">' + '<img src="' + img2d + '" alt="2D flowsheet" style="' + imgStyle + '"/></div>' : '<div class="wb-rep-note">2D view unavailable.</div>')
          + '<h4>3D Model View</h4>' + (img3d ? '<div class="wb-rep-view" style="background:#0b1220;padding:6px;border-radius:4px;">' + '<img src="' + img3d + '" alt="3D model" style="' + imgStyle + '"/></div>' : '<div class="wb-rep-note">3D view unavailable — the 3D engine may still be loading.</div>');
        var html = '<div class="wb-rep-head">ARO WORKBENCH — P&amp;ID PROCESS SYSTEM REPORT</div>'
          ...
          + titleBlock + verdict + views + tables;
        modal('SYSTEM REPORT — 2D + 3D · TECHNICAL DATA SHEET', html, true);
        setStatus('Report ready — view it inline or use DOWNLOAD PDF.', '#16a34a');
      });
    });
  };
  ...
  function wireUI(root) {
    ...
    function set3D(on) {
      WB.mode3d = on;
      if (on) { WB.pidMode = false; if (btnpid) btnpid.classList.remove('active'); }
    ...
    if (btn2d) btn2d.addEventListener('click', function () { WB.pidMode = false; if (btnpid) btnpid.classList.remove('active'); set3D(false); });
    if (btnpid) btnpid.addEventListener('click', function () {
      WB.pidMode = true;
      btnpid.classList.add('active');
      if (WB.mode3d) set3D(false); else render();
      setStatus('P&ID SCHEMATIC MODE — standard ISA/PIP line-art symbols (bowtie valves, capsule vessels, tube-bank exchangers). Same flowsheet, same connections — only the equipment glyph changed.', '#334155');
    });

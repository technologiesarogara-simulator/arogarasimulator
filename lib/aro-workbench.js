  ...
  function render() {
    ...
    WB.nodes.forEach(function (n) {
      ...
      lib.ports.forEach(function (pt, pi) {
        ...
        s += '<circle cx="' + pt.x + '" cy="' + pt.y + '" r="' + r + '" fill="' + rl.c + '" stroke="' + (connected ? '#0f172a' : '#fff') + '" stroke-width="' + (connected ? 2 : 1.5) + '" data-port="' + n.id + ':' + pi + '" style="cursor:crosshair"/>';
        if (showLbl && (pt.name || rl.lbl)) {
          var lx = pt.x + (pt.dir === 'e' ? 8 : pt.dir === 'w' ? -8 : 0);
          var ly = pt.y + (pt.dir === 's' ? 14 : pt.dir === 'n' ? -7 : 3);
      ...
      s += '<text x="' + (lib.w / 2) + '" y="' + (lib.h + 24) + '" font-size="9" font-weight="700" fill="#0f172a" text-anchor="middle" font-family="monospace" stroke="#fff" stroke-width="3" paint-order="stroke" style="pointer-events:none">' + (n.tag || '') + '</text>';
      s += '</g>';
    });
  ...
  function onClick(e) {
    ...
    if (WB.mode === 'pipe' && portEl) {
      ...
      if (!WB.pendingPort) { WB.pendingPort = ref; render(); }
        ...
        var fromPort = fromN ? LIB_INDEX[fromN.t].ports[WB.pendingPort.pi] : null;
        ...
        var p = { id: 'L' + (++WB.seq), from: WB.pendingPort, to: ref, role: role,

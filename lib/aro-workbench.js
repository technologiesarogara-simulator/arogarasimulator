/* ══════════════════════════════════════════════════════════════════════
   ARO WORKBENCH — CAD-like Process Flow editor (MVP)
...
(function () {
  ...
  /* ───────────── Canonical port registry (Stage 1 of the 2D/3D port-sync fix) ─────────────
     ...
     world units) — this registry never holds a coordinate.
  ...
  /* Tag each of the 5 registry equipment types' own 2D ports with the
     ...
     itself owns — are untouched; this only adds an identity 3D can be
     found by. A type not yet in AROPORTS (everything outside this first
     batch of 5) is left exactly as it was. */
  ...
  /* Read-only for the common component layer, which resolves one component's
     category, 2D icon, 3D casting and take-off metadata from a single key.
  ...
  /* ───────────── Equipment category classifier ─────────────
     Maps each palette item to a coarse equipment category (pump, vessel,
     heat exchanger, valve…) used to decide whether a sizing module can
     size it (window.AROENG.canSize) and which module to hand it to
     (sendToModule below) — a shared taxonomy consumed outside this file
     too (see lib/aro-component.js). */
  ...
  function pidCatOf(t) {
    ...
    var c = lib.cat || '';
    /* A spherical vessel is drawn as a circle in the 2D icon and 3D model
       alike specifically because its shape, not its tag, is what a reader
       needs to recognise at a glance — checked ahead of the generic
       vessel rule below. */
  ...
  /* Load a flowsheet recipe as an editable starting flowsheet. A recipe is
  ...
  function render() {
    ...
    WB.pipes.forEach(function (p) {
      ...
      /* Selecting an EQUIPMENT node used to give no visual sign of what it's
         piped to anywhere in 2D — you had to open the properties panel
  ...
  function renderProps() {
    ...
    if (WB.sel.kind === 'node') {
      ...
      if (window.AROENG && window.AROENG.canSize(pidCatOf(n.t))) {
        h += '<button class="wb-panel-btn wb-size-link" data-size-link="1">'
          + '&#8594; SIZE THIS EQUIPMENT</button>';
      }
      /* The 2D sheet tags equipment the way a real drawing does — this
         jumps straight to that exact item on the 3D model instead of
         making the engineer hunt for it. */
      if (window.ARO3D) {
        h += '<button class="wb-panel-btn wb-locate3d-link" data-locate3d="1">'
          + '&#9678; LOCATE IN 3D</button>';
        // Only offer this in 3D — walking is a 3D-scene concept, and jumping
        // into it from 2D with no visible operator would be confusing.
        if (WB.mode3d) {
          h += '<button class="wb-panel-btn wb-walkto-link" data-walkto="1">'
            + '&#128694; WALK TO EQUIPMENT</button>'
            + '<button class="wb-panel-btn wb-viewtop-link" data-viewtop="1">'
            + '&#128065; VIEW TOP</button>';
          // Only equipment tall enough to need one (the operability check's
          // "platform req." flag) gets a real ladder+deck in the scene —
          // offering the button everywhere would just walk the operator to
          // equipment with nothing to climb.
          if (window.ARO3D.platformRequiredForNid && window.ARO3D.platformRequiredForNid(n.id)) {
            var onThisPlatform = window.ARO3D.operatorOnPlatformNid && window.ARO3D.operatorOnPlatformNid() === n.id;
            h += '<button class="wb-panel-btn wb-climb-link" data-climb="1">'
              + (onThisPlatform ? '&#11015; CLIMB DOWN' : '&#11014; CLIMB TO PLATFORM') + '</button>';
          }
        }
      }
    ...
    var viewTopBtn = propEl.querySelector('[data-viewtop]');
    if (viewTopBtn) viewTopBtn.addEventListener('click', function () {
      var nid = WB.sel && WB.sel.id;
      if (!nid || !window.ARO3D) return;
      window.ARO3D.viewEquipmentTop(nid);
    });
    var climbBtn = propEl.querySelector('[data-climb]');
    if (climbBtn) climbBtn.addEventListener('click', function () {
      var nid = WB.sel && WB.sel.id;
      if (!nid || !window.ARO3D || !window.ARO3D.climbToPlatform) return;
      window.ARO3D.climbToPlatform(nid);
      // The button's own label (CLIMB TO PLATFORM / CLIMB DOWN) depends on
      // whether the operator is already up there — refresh the panel a
      // moment after the climb finishes so it flips to the other state
      // instead of staying stale until something else re-renders it.
      setTimeout(function () { if (WB.sel && WB.sel.id === nid) renderProps(); }, 2600);
    });
  ...
  /* Capture the 2D canvas, regardless of which workbench the engineer is
     currently looking at — re-render, then rasterise. */
  function captureView(cb) {
    try { render(); } catch (e) {}
    capture2D(cb);
  }
  ...
  /* One drawing laid out as a real A4 landscape sheet: ruled border, column
     ...
     bottom-right title block — the layout of a reference process drawing.
     Each view (2D / 3D) gets its own numbered sheet. */
  function a4Sheet(title, img, sheetNo, sheetTotal, alt, dark) {
    ...
    var pno = 'DWG-' + String(WB.seq || 1).padStart(3, '0');
    return ''
      + '<div class="wb-a4">'
      ...
      +       '<td><b>DRAWN BY</b><br>AROGARA</td>'
      +       '<td><b>CHECKED</b><br>QA / QC</td>'
      +     '</tr>'
      +     '<tr>'
      +       '<td class="l"><b>TITLE</b><br>' + title + '</td>'
      +       '<td class="l"><b>DWG NO.</b><br>' + pno + '</td>'
      +       '<td><b>REV.</b><br>0</td>'
  ...
  WB.report = function () {
    ...
    var titleBlock = '<table class="wb-table wb-rep-titleblock"><tr><td><b>PROJECT</b><br>' + (WB.projectName || 'AROGARA Process Flowsheet') + '</td>'
      + '<td><b>DWG NO.</b><br>DWG-' + String(WB.seq || 1).padStart(3, '0') + '</td>'
      + '<td><b>REV.</b><br>0</td>'
      + '<td><b>DATE</b><br>' + new Date().toLocaleDateString() + '</td>'
      + '<td><b>DRAWN BY</b><br>AROGARA</td></tr></table>';
    var tables = '<h4>Equipment Technical Data Sheet</h4><table class="wb-table"><tr><th>#</th><th>Tag</th><th>Name</th><th>Type</th><th>Location</th><th>Fluid</th><th>Flow m³/h</th><th>T °C</th><th>P barg</th></tr>' + (eq || '<tr><td colspan="9">No equipment placed.</td></tr>') + '</table>'
      ...
      + legend;
    setStatus('Building 2D and 3D views for the report…', '#38bdf8');
    // Both workbenches are views of the same model, so the report carries
    // both: detailed 2D and the 3D model. Remember which workbench the
    // engineer is on — capturing walks through 3D, and we must land them
    // back exactly where they were.
    var mode0 = { m3: !!WB.mode3d };
    captureView(function (img2d) {
      capture3D(function (img3d) {
        if (WB.setWorkbench) WB.setWorkbench(mode0.m3);
        var sheets =
            a4Sheet('2D FLOWSHEET — DETAILED EQUIPMENT VIEW', img2d, 1, 2, '2D flowsheet', false)
          + a4Sheet('3D MODEL VIEW', img3d, 2, 2, '3D model', true);
        var html = '<div class="wb-rep-head">ARO WORKBENCH — PROCESS SYSTEM REPORT</div>'
          + '<div class="wb-rep-sub">AROGARA · ' + new Date().toLocaleString() + ' · (NOT FOR CONSTRUCTION)</div>'
          + titleBlock + verdict + sheets + tables;
        modal('SYSTEM REPORT — 2D + 3D · TECHNICAL DATA SHEET', html, true);
        setStatus('Report ready — 2D and 3D sheets. View inline or use DOWNLOAD PDF.', '#16a34a');
      });
    });
  };
  ...
  function showImportReview(o) {
    ...
    var body = ''
      ...
      + 'Everything stays editable in the 2D and 3D workbenches.'
      + (o.ocrDone && canModel
      ...
      btn.addEventListener('click', function () {
        ...
        if (act === 'model') {
          ...
          setStatus('Built ' + res.equipment + ' equipment and ' + res.lines + ' inferred lines from the ' + o.label
            + ' — editable in 2D and 3D. Check the routing, then RUN ANALYSIS.', '#16a34a');
        }
      });
    });
  }
  ...
  /* ───────────── UI construction ─────────────
     The library panel shows the rich detailed equipment icons — the same
     catalogue used by both the 2D and 3D workbenches. */
  ...
  // template (not part of buildPaletteHTML) so palette refreshes can drop
  // it in without re-wiring anything; its own listener is delegated on a
  // container that's never replaced (see buildUI).
  ...
  /* Swap the library thumbnails when the workbench changes. Drag binding is
     ...
     switching between 2D and 3D doesn't quietly reset an active search. */
  ...
  function buildUI(root) {
    ...
    root.innerHTML =
      ...
      + '  <span class="wb-brand"><img src="icon-192.png" alt="" width="15" height="15" style="vertical-align:-3px;margin-right:5px;border-radius:3px;">ARO WORKBENCH</span>'
      ...
      + grp('FIND', '<input type="text" class="wb-tool wb-vtxt" id="wb-3d-find" placeholder="Find equipment…" list="wb-3d-find-list" style="width:230px;" title="Type a tag or name — pick a suggestion or press Enter/click GO to jump the 3D camera exactly to it"><datalist id="wb-3d-find-list"></datalist>'
          + '<button class="wb-tool" id="wb-3d-find-go" title="Jump the camera to the matched equipment">' + icon('search') + '</button>')
      + grp('OPERATOR', '<button class="wb-tool wb-vtxt" id="wb-3d-operator" title="Enter Operator Mode — walk through the plant as a correctly-scaled (1.73m) person. Click the ground to walk there, or WASD to move and drag the mouse to look around. Shift runs, R resets your position, Esc exits. You can\'t walk through equipment or low piping.">OPERATOR</button>'
          ...
          + '</div>'
          + '<button class="wb-tool wb-vtxt" id="wb-3d-reach" style="display:none;" title="Show the configurable ergonomic reach envelope around the operator (project-defined comfortable/max reach, not a validated ergonomic standard) — flags valves/instruments mounted above it">REACH</button>'
          ...
          + '<button class="wb-tool wb-vtxt" id="wb-3d-maint" title="Show maintenance clearance envelopes (motor/coupling pull, tube-bundle removal, manway/ladder access — shape depends on equipment type). Turns red where another item or a pipe run is blocking that clearance.">MAINT</button>'
          + '<button class="wb-tool wb-vtxt" id="wb-3d-safety" title="Show safety exclusion zones around rotating, hot and pressurised equipment. Radii are project-defined placeholders, not a validated hazard study.">SAFETY</button>'
          + '<button class="wb-tool wb-vtxt" id="wb-3d-opcheck" title="Run the operability checker — for every equipment item: can an operator reach it, is its maintenance clearance blocked, does it need a platform">CHECK</button>'
          + '<button class="wb-tool wb-vtxt" id="wb-3d-assembly" title="Send the operator to the plant\'s designated assembly/muster point — the same destination the CALL OPERATOR panel\'s GO TO ASSEMBLY POINT button uses">ASSEMBLY</button>'
          + '<button class="wb-tool wb-vtxt" id="wb-3d-vehicle" title="Board the site vehicle for fast long-distance travel — walks there first if you\'re not already next to it, then boards automatically. Click again to exit.">VEHICLE</button>')
      ...
      + '  </div>'
      + '  <div class="wb-resizer" id="wb-resizer2" title="Drag to resize the properties panel"></div>'
  ...
  function wireUI(root) {
      ...
      if (on && window.ARO3D) {
        window.ARO3D.onSelect = function (props, tris) {
          ...
          /* If this 3D mesh is a piece of equipment from the shared model,
             show the SAME editable properties panel the 2D workbench uses.
             Previously 3D offered only a rename box and a read-only dump,
             so tag/fluid/flow/temperature could not be edited from 3D at
             all — and any change made in 2D could not be made here. Both
             workbenches now edit one model. */
          ...
          // A field crew figure isn't process equipment — "WALK TO
          // EQUIPMENT" pointed at a person made no sense. Selecting one
          // "radios" it instead: the controllable operator answers (radio
          // to the ear, a ring tone) and the panel offers what that call
          // was actually for.
          if (props.Type === 'Field Operator') { renderCallOperatorPanel(); return; }
          // The site vehicle isn't equipment either — it gets its own
          // board/exit control instead of WALK TO / VIEW TOP / CLIMB.
          if (props.Type === 'Site Vehicle') { renderVehiclePanel(); return; }
          propEl.innerHTML = '<div class="wb-prop-title">' + (props.Type || '3D Equipment') + '</div>'
            ...
            + '<button class="wb-panel-btn wb-walkto-link" id="wb-3d-walkto-raw">&#128694; WALK TO EQUIPMENT</button>'
            + '<button class="wb-panel-btn wb-viewtop-link" id="wb-3d-viewtop-raw">&#128065; VIEW TOP</button>'
            + (window.ARO3D.platformRequiredForNid && window.ARO3D.platformRequiredForNid(nid3) ? '<button class="wb-panel-btn wb-climb-link" id="wb-3d-climb-raw">&#11014; CLIMB TO PLATFORM / DOWN</button>' : '')
          ...
          var viewTopRaw = propEl.querySelector('#wb-3d-viewtop-raw');
          if (viewTopRaw) viewTopRaw.addEventListener('click', function () { window.ARO3D.viewEquipmentTop(null); });
          var climbRaw = propEl.querySelector('#wb-3d-climb-raw');
          if (climbRaw) climbRaw.addEventListener('click', function () { window.ARO3D.climbToPlatform(null); });
        };
        ...
        // tube-side port in 2D instead: the two views then disagreed about
    ...
    WB.setMode3D = set3D;
    /* Put the workbench back exactly as the engineer left it. The report
       briefly enters 3D to capture it (capture3D) — this restores whichever
       mode the engineer was actually in before that happened. */
    WB.setWorkbench = function (m3) {
      set3D(!!m3);
      refreshPalette();
      if (!m3) render();
    };
    ...
    if (btn2d) btn2d.addEventListener('click', function () {
      ...
      setStatus('2D FLOWSHEET WORKBENCH — detailed equipment icons. Same model as the 3D workbench; anything you draw here appears in both.', '#0369a1');
    });
    ...
    var reachBtn = root.querySelector('#wb-3d-reach');
    var operatorPanelBuilt = false;
    /* CALL OPERATOR — selecting one of the ambient "Field Operator" crew
       figures near a big unit used to show the generic raw-mesh panel
       (WALK TO EQUIPMENT pointed at a person, an inert prop dump) with no
       actual role. Radioing them now visibly answers on the controllable
       operator (ring tone + radio-to-ear pose in aro-workbench-3d.js) and
       then offers what that call was actually for — climb the nearest
       platform, log a routine monitor report, or head to the assembly
       point — each wired to the matching A3 function, same "one button,
       one real action" rule as every other control in this panel. */
    function renderCallOperatorPanel() {
      if (!propEl || !window.ARO3D) return;
      var picked = window.ARO3D.inspect ? window.ARO3D.inspect().picked : null;
      var px = picked ? picked.position.x : 0, pz = picked ? picked.position.z : 0;
      var nearNid = window.ARO3D.nearestEquipmentNid ? window.ARO3D.nearestEquipmentNid(px, pz, false) : null;
      var platNid = window.ARO3D.nearestEquipmentNid ? window.ARO3D.nearestEquipmentNid(px, pz, true) : null;
      var equipLabel = window.ARO3D.equipmentLabel ? window.ARO3D.equipmentLabel(nearNid) : 'the field';

      /* Answering the call turns Operator Mode on if it wasn't already —
         and setOperatorMode() itself unconditionally pushes the live
         OPERATOR MODE panel as one of its very last steps (plus resets
         any suspension hold on the way in, deliberately, so a stale hold
         never survives an unrelated toggle). That push happens
         SYNCHRONOUSLY inside operatorTakeCall() below, before this
         function can react to it — so rather than trying to race or
         suppress that one push, this renders the "choose an action" panel
         from scratch AFTER the ring delay, once that transition has
         already settled, and only then asks to hold the panel there. */
      function renderActive() {
        if (window.ARO3D.setOperatorPanelSuspended) window.ARO3D.setOperatorPanelSuspended(true);
        propEl.innerHTML = '<div class="wb-prop-title">FIELD OPERATOR</div>'
          + '<div class="wb-prop-note" style="color:#16a34a;">&#128222; &ldquo;This is the field &mdash; calling in from '
          + equipLabel + '.&rdquo;</div>'
          + '<div class="wb-prop-note" style="color:#0369a1;">Dispatch base: choose how the crew responds. Whichever way you '
          + 'send them, they keep to the plant\'s marked walkways/roads — never a shortcut across the process area.</div>'
          + (platNid ? '<button class="wb-panel-btn" id="wb-call-climb">&#11014; CLIMB TO PLATFORM (ON FOOT)</button>' : '')
          + '<button class="wb-panel-btn" id="wb-call-report">&#128203; SEND MONITOR REPORT</button>'
          + '<button class="wb-panel-btn" id="wb-call-walk">&#128694; WALK TO THE OPERATOR</button>'
          + '<button class="wb-panel-btn" id="wb-call-drive">&#128663; DRIVE TO THE OPERATOR</button>'
          + '<button class="wb-panel-btn" id="wb-call-assembly">&#128681; RETURN TO ASSEMBLY POINT</button>'
          + '<div class="wb-prop-note" id="wb-call-dispatch-row" style="display:none;color:#0369a1;font-weight:700;"></div>';
        var dispatchRow = propEl.querySelector('#wb-call-dispatch-row');
        // CLIMB / REPORT are always a foot dispatch (you can't climb a
        // ladder or read a gauge from the driver's seat) — if the operator
        // happens to already be driving, park the vehicle first so a radio
        // call can never read as "drove off-plan to get here".
        function dispatchOnFoot(label, go) {
          if (window.ARO3D.isOperatorInVehicle && window.ARO3D.isOperatorInVehicle()) window.ARO3D.exitVehicle();
          go();
          if (dispatchRow) { dispatchRow.style.display = ''; dispatchRow.textContent = '→ Dispatched on foot to: ' + label; }
        }
        var climbBtn = propEl.querySelector('#wb-call-climb');
        if (climbBtn) climbBtn.addEventListener('click', function () {
          if (window.ARO3D.setOperatorPanelSuspended) window.ARO3D.setOperatorPanelSuspended(false);
          dispatchOnFoot('the platform on the nearest tall vessel', function () { window.ARO3D.climbToPlatform(platNid); });
        });
        var reportBtn = propEl.querySelector('#wb-call-report');
        if (reportBtn) reportBtn.addEventListener('click', function () { window.ARO3D.sendMonitorReport(nearNid); if (dispatchRow) { dispatchRow.style.display = ''; dispatchRow.textContent = '→ Monitor report logged for the nearest equipment.'; } });
        var walkBtn = propEl.querySelector('#wb-call-walk');
        if (walkBtn) walkBtn.addEventListener('click', function () {
          if (window.ARO3D.setOperatorPanelSuspended) window.ARO3D.setOperatorPanelSuspended(false);
          dispatchOnFoot('meet ' + equipLabel + '\'s crew, on foot', function () { window.ARO3D.walkToEquipment(nearNid); });
        });
        var driveBtn = propEl.querySelector('#wb-call-drive');
        if (driveBtn) driveBtn.addEventListener('click', function () {
          if (window.ARO3D.setOperatorPanelSuspended) window.ARO3D.setOperatorPanelSuspended(false);
          if (window.ARO3D.driveToPoint) {
            // Park BESIDE the field operator, not exactly on top of them —
            // px/pz is the crew figure's own coordinate, and a full-size
            // vehicle centred there would visibly overlap them (and often
            // whatever equipment they're standing next to).
            var op = window.ARO3D.debugOperator ? window.ARO3D.debugOperator() : null;
            var dest = window.ARO3D.approachPointNear ? window.ARO3D.approachPointNear(px, pz, op ? op.x : px, op ? op.z : pz) : null;
            window.ARO3D.driveToPoint(dest ? dest.x : px, dest ? dest.z : pz, 'meet ' + equipLabel + '\'s crew');
          }
          if (dispatchRow) { dispatchRow.style.display = ''; dispatchRow.textContent = '→ Dispatched by vehicle to meet ' + equipLabel + '\'s crew — the response vehicle keeps to the plant road.'; }
        });
        var assemblyBtn = propEl.querySelector('#wb-call-assembly');
        // Goes back the same way the crew arrived — still driving if they
        // drove out, still on foot if they walked — rather than forcing a
        // mode switch on the way back.
        if (assemblyBtn) assemblyBtn.addEventListener('click', function () {
          if (window.ARO3D.setOperatorPanelSuspended) window.ARO3D.setOperatorPanelSuspended(false);
          var byVehicle = window.ARO3D.isOperatorInVehicle && window.ARO3D.isOperatorInVehicle();
          window.ARO3D.goToAssemblyPoint();
          if (dispatchRow) { dispatchRow.style.display = ''; dispatchRow.textContent = '→ ' + (byVehicle ? 'Driving' : 'Walking') + ' back to the assembly point.'; }
        });
      }

      propEl.innerHTML = '<div class="wb-prop-title">FIELD OPERATOR</div>'
        + '<div class="wb-prop-note">A crew member stationed in the field. Radio them for an instruction.</div>'
        + '<button class="wb-panel-btn" id="wb-call-op-btn">&#128222; CALL OPERATOR</button>';
      var callBtn = propEl.querySelector('#wb-call-op-btn');
      if (callBtn) callBtn.addEventListener('click', function () {
        window.ARO3D.operatorTakeCall(6);
        callBtn.disabled = true; callBtn.textContent = '… RINGING …';
        setTimeout(renderActive, 900);
      });
    }
    // SITE VEHICLE — board/exit toggle, re-rendered after each click so the
    // button label always matches whether the operator is actually in it.
    function renderVehiclePanel() {
      if (!propEl || !window.ARO3D) return;
      var inVehicle = window.ARO3D.isOperatorInVehicle && window.ARO3D.isOperatorInVehicle();
      propEl.innerHTML = '<div class="wb-prop-title">SITE VEHICLE</div>'
        + '<div class="wb-prop-note">Board it for fast long-distance travel across the plant — much quicker than walking.</div>'
        + '<button class="wb-panel-btn" id="wb-vehicle-toggle">' + (inVehicle ? '&#11015; EXIT VEHICLE' : '&#128663; BOARD VEHICLE') + '</button>';
      var btn = propEl.querySelector('#wb-vehicle-toggle');
      if (btn) btn.addEventListener('click', function () {
        if (window.ARO3D.isOperatorInVehicle && window.ARO3D.isOperatorInVehicle()) window.ARO3D.exitVehicle();
        else window.ARO3D.boardVehicle();
        renderVehiclePanel();
      });
    }
    ...
    function renderOperatorPanel(info) {
      ...
      if (!operatorPanelBuilt || !propEl.querySelector('#wb-op-x')) {
        propEl.innerHTML = '<div class="wb-prop-title">OPERATOR MODE</div>'
          ...
          + '<div class="wb-prop-note" id="wb-op-dest-row" style="display:none;color:#0369a1;font-weight:700;">&#8594; Dispatched to: <span id="wb-op-dest"></span></div>'
          ...
          + '<div class="wb-rrow" id="wb-op-elev-row" style="display:none;"><span>Elevation (m)</span><b id="wb-op-elev">0.0</b></div>'
          // Only shown while actually in the vehicle — this is the "how do I
          // get out once I've arrived" control the CALL OPERATOR / VEHICLE
          // panels don't cover once driving is already under way and this
          // live panel has taken back over the screen.
          + '<button class="wb-xbtn" id="wb-op-exit-vehicle" style="width:100%;margin-top:10px;display:none;background:#f59e0b;border-color:#f59e0b;color:#1a1c22;font-weight:800;">&#11015; GET OUT OF VEHICLE</button>'
        ...
        var exitVehicleBtn = propEl.querySelector('#wb-op-exit-vehicle');
        if (exitVehicleBtn) exitVehicleBtn.addEventListener('click', function () { window.ARO3D && window.ARO3D.exitVehicle && window.ARO3D.exitVehicle(); });
      ...
      // "Where is the operator actually headed" — the direct answer to
      // "the maintenance guy must know where he wants to go", shown live
      // in the panel instead of only as a status-bar line that scrolls
      // away once the walk starts.
      var destRow = propEl.querySelector('#wb-op-dest-row'), destVal = propEl.querySelector('#wb-op-dest');
      if (destRow && destVal) {
        if (info.destination) {
          destRow.style.display = '';
          destVal.textContent = info.destination + (info.inVehicle ? ' (by vehicle)' : '');
        } else {
          destRow.style.display = 'none';
        }
      }
      // On a platform (or mid-climb) — surface it here too, not just as a
      // one-off status-bar message, since the properties panel is what's
      // actually on screen while the operator stays up there.
      var elevRow = propEl.querySelector('#wb-op-elev-row'), elevVal = propEl.querySelector('#wb-op-elev');
      if (elevRow && elevVal) {
        var elev = info.elevation || 0;
        if (elev > 0.05 || info.climbing) {
          elevRow.style.display = '';
          elevVal.textContent = elev.toFixed(1) + (info.climbing ? ' (climbing…)' : info.onPlatformNid ? ' (on platform)' : '');
        } else {
          elevRow.style.display = 'none';
        }
      }
      var exitVehicleRow = propEl.querySelector('#wb-op-exit-vehicle');
      if (exitVehicleRow) exitVehicleRow.style.display = info.inVehicle ? '' : 'none';
    }
    function setOperatorUI(on) {
      ...
      if (reachBtn) { reachBtn.style.display = on ? 'inline-flex' : 'none'; if (!on) reachBtn.classList.remove('on'); }
    ...
    // REACH — Phase 8's ergonomic reach envelope, only meaningful while
    // Operator Mode is on (hidden/reset alongside it in setOperatorUI above).
    if (reachBtn) reachBtn.addEventListener('click', function () {
      reachBtn.classList.toggle('on');
      window.ARO3D && window.ARO3D.setReachCheckVisible(reachBtn.classList.contains('on'));
    });
    ...
    // MAINTENANCE CLEARANCE — Phase 5. Turns the specific envelope red and
    // drops a status-bar count when something is actually blocking it,
    // rather than silently drawing green zones no one double-checks.
    var maintBtn = root.querySelector('#wb-3d-maint');
    if (maintBtn) maintBtn.addEventListener('click', function () {
      maintBtn.classList.toggle('on');
      window.ARO3D && window.ARO3D.setMaintenanceVisible(maintBtn.classList.contains('on'));
    });
    // SAFETY ZONE — Phase 6.
    var safetyBtn = root.querySelector('#wb-3d-safety');
    if (safetyBtn) safetyBtn.addEventListener('click', function () {
      safetyBtn.classList.toggle('on');
      window.ARO3D && window.ARO3D.setSafetyZoneVisible(safetyBtn.classList.contains('on'));
    });
    // RUN OPERABILITY CHECK — Phase 7. Replaces the PROPERTIES panel with
    // a PASS/WARNING/FAIL sweep of every equipment item, same "engine
    // computes, UI just renders" split as the rest of this module.
    var opCheckBtn = root.querySelector('#wb-3d-opcheck');
    function badge(status) {
      var c = status === 'PASS' ? '#16a34a' : status === 'WARNING' ? '#d97706' : status === 'FAIL' ? '#dc2626' : '#94a3b8';
      return '<span style="display:inline-block;min-width:64px;text-align:center;padding:1px 6px;border-radius:3px;font-size:10px;font-weight:800;letter-spacing:.04em;color:#fff;background:' + c + ';">' + status + '</span>';
    }
    /* The three status columns aren't self-explanatory from a badge alone —
       this is what each one actually tested, shown as a one-time legend so
       "WARNING" reads as a specific, checkable claim instead of a colour. */
    var OPCHECK_COLS = [
      { key: 'operatorAccess', h: 'ACCESS', help: 'Can the operator actually walk here? PASS = a clear path exists from the current position; FAIL = no walkable route (blocked by a wall/vessel/other equipment with no way round).' },
      { key: 'maintenanceAccess', h: 'MAINT.', help: 'Is there room to pull this item\'s maintenance clearance envelope (e.g. tube bundle, coupling spacer) without hitting something else? PASS = clearance is free; WARNING = another item sits inside the clearance envelope (see the note below); N/A = this equipment type has no defined maintenance clearance.' },
      { key: 'reach', h: 'REACH', help: 'Can a standing operator reach every port/valve on this item by hand? PASS = every port is within reach height; WARNING = one or more ports sit above the reach limit — this is what "platform required" is based on.' }
    ];
    function opcheckLegend() {
      return '<div class="wb-prop-note" style="margin:2px 0 8px;padding:6px 8px;background:rgba(148,163,184,0.08);border-radius:4px;">'
        + OPCHECK_COLS.map(function (c) { return '<div style="margin-bottom:3px;"><b>' + c.h + '</b> — ' + c.help + '</div>'; }).join('')
        + '<div>Click any row to jump to that item in 3D. A WARNING/FAIL row also gets a button to go straight there and see why.</div></div>';
    }
    function renderOperabilityReport(report) {
      if (!propEl) return;
      var nFail = report.filter(function (r) { return r.operatorAccess === 'FAIL'; }).length;
      var nWarn = report.filter(function (r) { return r.maintenanceAccess === 'WARNING'; }).length;
      var nPlat = report.filter(function (r) { return r.platformRequired; }).length;
      var nReach = report.filter(function (r) { return r.reachWarning; }).length;
      var nRoute = report.filter(function (r) { return r.routeNote; }).length;
      var h = '<div class="wb-prop-title">OPERABILITY CHECK</div>'
        + '<div class="wb-prop-note">' + report.length + ' equipment checked — ' + nFail + ' unreachable, ' + nWarn + ' maintenance conflict' + (nWarn === 1 ? '' : 's') + ', ' + nReach + ' reach warning' + (nReach === 1 ? '' : 's') + ', ' + nRoute + ' route flag' + (nRoute === 1 ? '' : 's') + ', ' + nPlat + ' need' + (nPlat === 1 ? 's' : '') + ' a platform.</div>'
        + '<button class="wb-tool wb-vtxt" id="wb-opcheck-close" style="width:100%;margin:4px 0 8px;">&larr; BACK TO PROPERTIES</button>'
        + '<button class="wb-tool wb-vtxt" id="wb-opcheck-legend-tog" style="width:100%;margin:0 0 6px;">? WHAT DO THESE MEAN</button>'
        + '<div id="wb-opcheck-legend" style="display:none;">' + opcheckLegend() + '</div>';
      report.forEach(function (r, i) {
        var needsAction = r.operatorAccess === 'FAIL' || r.maintenanceAccess === 'WARNING' || r.reachWarning;
        h += '<div class="wb-rrow wb-opcheck-row" data-oc-nid="' + r.nid + '" style="align-items:center;flex-wrap:wrap;gap:4px;cursor:pointer;" title="Click to locate this item in 3D">'
          + '<span style="flex:1 1 auto;min-width:120px;">' + r.type + (r.platformRequired ? ' <i style="color:#d97706;">(platform req.)</i>' : '') + '</span>'
          + '<span title="' + OPCHECK_COLS[0].help.replace(/"/g, '&quot;') + '">' + badge(r.operatorAccess) + '</span>'
          + '<span title="' + OPCHECK_COLS[1].help.replace(/"/g, '&quot;') + '">' + badge(r.maintenanceAccess) + '</span>'
          + '<span title="' + OPCHECK_COLS[2].help.replace(/"/g, '&quot;') + '">' + badge(r.reachWarning ? 'WARNING' : 'PASS') + '</span></div>'
          + (r.maintenanceConflict ? '<div class="wb-prop-note" style="margin:-2px 0 4px;color:#dc2626;">Maintenance: ' + r.maintenanceConflict + '</div>' : '')
          + (r.reachWarning ? '<div class="wb-prop-note" style="margin:-2px 0 4px;color:#d97706;">Reach: ' + r.reachWarning + '</div>' : '')
          + (r.routeNote ? '<div class="wb-prop-note" style="margin:-2px 0 4px;color:#0369a1;">Route: ' + r.routeNote + '</div>' : '')
          + (needsAction ? '<button class="wb-tool wb-vtxt wb-opcheck-goto" data-oc-goto="' + r.nid + '" style="width:100%;margin:0 0 4px;">&#9678; GO SEE WHY</button>' : '')
          + (r.platformRequired ? '<button class="wb-tool wb-vtxt wb-opcheck-climb" data-oc-climb="' + r.nid + '" style="width:100%;margin:0 0 8px;">&#11014; CLIMB TO PLATFORM</button>' : '');
      });
      propEl.innerHTML = h;
      // While Operator Mode is active it pushes a live OPERATOR MODE panel
      // into this same properties column every few frames — left alone,
      // that stomps this report back off screen within a fraction of a
      // second of it opening. Hold it off for as long as the report itself
      // is the thing on screen.
      if (window.ARO3D && window.ARO3D.setOperatorPanelSuspended) window.ARO3D.setOperatorPanelSuspended(true);
      var closeBtn = propEl.querySelector('#wb-opcheck-close');
      if (closeBtn) closeBtn.addEventListener('click', function () {
        if (window.ARO3D && window.ARO3D.setOperatorPanelSuspended) window.ARO3D.setOperatorPanelSuspended(false);
        renderProps();
      });
      var legendTog = propEl.querySelector('#wb-opcheck-legend-tog');
      var legendEl = propEl.querySelector('#wb-opcheck-legend');
      if (legendTog && legendEl) legendTog.addEventListener('click', function () {
        legendEl.style.display = legendEl.style.display === 'none' ? 'block' : 'none';
      });
      function jumpTo(nid, walk) {
        if (!nid || !window.ARO3D) return;
        // Sending the operator off walking somewhere is exactly when the
        // live OPERATOR MODE panel (location, elevation) becomes useful
        // again, so release the hold this report put on it.
        if (walk && window.ARO3D.setOperatorPanelSuspended) window.ARO3D.setOperatorPanelSuspended(false);
        // walkToEquipment turns Operator Mode on by itself if it isn't
        // already active, so "GO SEE WHY" always ends with the operator
        // actually standing at the flagged item, not just the camera there.
        var go = function () { if (walk) window.ARO3D.walkToEquipment(nid); else window.ARO3D.selectByNid(nid); };
        if (!WB.mode3d && typeof WB.setMode3D === 'function') { WB.setMode3D(true); setTimeout(go, 60); }
        else go();
      }
      propEl.querySelectorAll('.wb-opcheck-row').forEach(function (row) {
        row.addEventListener('click', function (ev) {
          if (ev.target && ev.target.hasAttribute && ev.target.hasAttribute('data-oc-goto')) return;
          jumpTo(row.getAttribute('data-oc-nid'), false);
        });
      });
      propEl.querySelectorAll('[data-oc-goto]').forEach(function (b) {
        b.addEventListener('click', function (ev) {
          ev.stopPropagation();
          jumpTo(b.getAttribute('data-oc-goto'), true);
        });
      });
      propEl.querySelectorAll('[data-oc-climb]').forEach(function (b) {
        b.addEventListener('click', function (ev) {
          ev.stopPropagation();
          var nid = b.getAttribute('data-oc-climb');
          if (!nid || !window.ARO3D || !window.ARO3D.climbToPlatform) return;
          if (window.ARO3D.setOperatorPanelSuspended) window.ARO3D.setOperatorPanelSuspended(false);
          var go = function () { window.ARO3D.climbToPlatform(nid); };
          if (!WB.mode3d && typeof WB.setMode3D === 'function') { WB.setMode3D(true); setTimeout(go, 60); }
          else go();
        });
      });
    }
    if (opCheckBtn) opCheckBtn.addEventListener('click', function () {
      if (!window.ARO3D) return;
      var report = window.ARO3D.runOperabilityCheck();
      renderOperabilityReport(report);
      setStatus('Operability check complete — ' + report.length + ' equipment items evaluated.', '#0369a1');
    });
    var assemblyBtn = root.querySelector('#wb-3d-assembly');
    if (assemblyBtn) assemblyBtn.addEventListener('click', function () {
      if (!window.ARO3D || !window.ARO3D.goToAssemblyPoint) return;
      window.ARO3D.goToAssemblyPoint();
    });
    var vehicleToolBtn = root.querySelector('#wb-3d-vehicle');
    if (vehicleToolBtn) vehicleToolBtn.addEventListener('click', function () {
      if (!window.ARO3D || !window.ARO3D.boardVehicle) return;
      if (window.ARO3D.isOperatorInVehicle && window.ARO3D.isOperatorInVehicle()) window.ARO3D.exitVehicle();
      else window.ARO3D.boardVehicle();
    });
    ...
    if (findBox) {
      ...
      // Picking a suggestion from the datalist fires 'change' (not
      // 'keydown') in every browser — previously the only way to actually
      // run the search was to press Enter afterwards, which wasn't
      // obvious, so the box looked like it "did nothing" when you clicked
      // a suggestion. A dedicated GO button covers the same gap by touch/click.
      findBox.addEventListener('change', jumpToFind);
      var findGoBtn = root.querySelector('#wb-3d-find-go');
      if (findGoBtn) findGoBtn.addEventListener('click', jumpToFind);
    }
    ...
    // thumbnails can be re-rendered (refreshPalette) without losing
    // draggability.
    ...
    // innerHTML swap without needing to be re-attached.
    ...
    // Same drag-to-resize for the right-hand PROPERTIES panel — it was
    // previously a fixed 252px with no way to widen it, which is why long
    // panel buttons (WALK TO EQUIPMENT etc.) wrapped and looked cramped
    // even after the button style itself was fixed.
    var rz2 = root.querySelector('#wb-resizer2');
    try { var savedPropW = parseInt(localStorage.getItem('wbPropW')); if (savedPropW >= 220 && savedPropW <= 520) body.style.setProperty('--wb-prop', savedPropW + 'px'); } catch (e) {}
    if (rz2) {
      var rz2Drag = null;
      rz2.addEventListener('mousedown', function (e) {
        rz2Drag = { x: e.clientX, w: root.querySelector('.wb-props').getBoundingClientRect().width };
        rz2.classList.add('dragging'); e.preventDefault();
      });
      window.addEventListener('mousemove', function (e) {
        if (!rz2Drag) return;
        // dragging right widens the palette resizer but should NARROW this
        // panel (it sits on the panel's left edge), so the delta is inverted
        var w = Math.max(220, Math.min(520, rz2Drag.w - (e.clientX - rz2Drag.x)));
        body.style.setProperty('--wb-prop', w + 'px');
      });
      window.addEventListener('mouseup', function () {
        if (!rz2Drag) return;
        rz2Drag = null; rz2.classList.remove('dragging');
        try { localStorage.setItem('wbPropW', parseInt(root.querySelector('.wb-props').getBoundingClientRect().width)); } catch (e) {}
      });
    }
  ...
  var WB_CSS = [
    ...
    '.wb-brand{color:#f8fafc;font-family:monospace;font-weight:800;font-size:13px;letter-spacing:0.06em;margin-right:14px;}',
    ...
    '.wb-body{flex:1;display:grid;grid-template-columns:var(--wb-pal,212px) 7px 1fr 7px var(--wb-prop,272px);min-height:0;}',
    ...
    // Full-width action buttons on the PROPERTIES panel (SIZE / LOCATE /
    // WALK TO / VIEW TOP). These previously reused .wb-tool, a 26px-tall
    // SQUARE icon-button style meant for the toolbar — fine for a single
    // glyph, but multi-word labels wrapped inside that fixed height and
    // read as broken/clipped. This class sizes to its text instead.
    '.wb-panel-btn{display:block;width:100%;box-sizing:border-box;margin:4px 0;padding:7px 10px;border:1px solid #cbd5e1;background:#fff;border-radius:6px;cursor:pointer;color:#334155;font-size:11px;font-weight:800;letter-spacing:.04em;line-height:1.3;white-space:normal;text-align:center;}',
    '.wb-panel-btn:hover{background:#ff7538;color:#fff;border-color:#ea580c;}',
    ...
    '@media (max-width:1100px){.wb-body{grid-template-columns:var(--wb-pal,172px) 7px 1fr 7px var(--wb-prop,220px);}}'
  ].join('');

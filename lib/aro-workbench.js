...
(function () {
  ...
  // Live scale bar + X/Y/Z axis triad (spins with the 360° rotation)
  function updateOverlays() {
    ...
    if (lEl) lEl.textContent = (20 / WB.zoom).toFixed(1) + ' units';   // the 60px bar in world units
    var dark = /^#(0|1)/.test(WB.bgColor);
    ...
    if (tri) {
      var a = (WB.viewRotate || 0) * Math.PI / 180, ca = Math.cos(a), sa = Math.sin(a);
      var iso = (WB.viewMode === 'iso');
      var cx = 35, cy = 38, L = 22;
      function axis(dx, dy, col, lbl, zAxis) {
        var ex, ey;
        if (zAxis) { ex = cx; ey = cy - L; }   // Z stays vertical
        else { ex = cx + (dx * ca - dy * sa) * L; ey = cy + (dx * sa + dy * ca) * (iso ? 0.5 : 1) * L; }
        return '<line x1="' + cx + '" y1="' + cy + '" x2="' + ex + '" y2="' + ey + '" stroke="' + col + '" stroke-width="2.4"/>'
          + '<circle cx="' + ex + '" cy="' + ey + '" r="4" fill="' + col + '"/>'
          + '<text x="' + ex + '" y="' + (ey + 3) + '" font-size="7" font-weight="700" fill="#fff" text-anchor="middle">' + lbl + '</text>';
      }
      tri.innerHTML = '<circle cx="35" cy="38" r="26" fill="' + (dark ? 'rgba(15,23,42,0.6)' : 'rgba(255,255,255,0.75)') + '" stroke="#94a3b8" stroke-width="0.8"/>'
        + axis(1, 0, '#dc2626', 'X', false)
        + axis(0, 1, '#16a34a', 'Y', false)
        + axis(0, 0, '#2563eb', 'Z', true)
        + '<circle cx="35" cy="38" r="2.2" fill="#334155"/>';
    }
  }
  ...
  function buildUI(root) {
    ...
    root.innerHTML =
      ...
      + '<button class="wb-tool" id="wb-bg-btn" title="Change background colour">🎨</button>'
      + '<span class="wb-tsep"></span>'
      ...
      + '    <div class="wb-hint">Drag component ▸ drop · <b>drag port→port to connect</b> · <b>↻ = 360° rotate</b> · ▩ grid · 🎨 background · scroll = zoom · space-drag = pan</div>'
  ...
  function wireUI(root) {
    ...
    var bgBtn = root.querySelector('#wb-bg-btn');
    if (bgBtn) bgBtn.addEventListener('click', function () {
      WB.bgIdx = (WB.bgIdx + 1) % BG_COLORS.length; WB.bgColor = BG_COLORS[WB.bgIdx];
  ...
  var WB_CSS = [
    ...
    '.wb-triad{position:absolute;right:8px;bottom:36px;pointer-events:none;}',

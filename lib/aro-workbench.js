...
(function () {
  ...
  function buildUI(root) {
    ...
    root.innerHTML =
      ...
      + '<span class="wb-tsep"></span>'
      + '<button class="wb-tool" id="wb-3d-btn" title="Open the real Three.js 3D CAD viewport" style="background:linear-gradient(135deg,#0284c7,#38bdf8);color:#fff;border-color:#0369a1;font-weight:700;width:auto;padding:0 12px;">🧊 REAL 3D</button>'
  ...
  function wireUI(root) {
    ...
    // Real Three.js 3D CAD viewport
    var btn3d = root.querySelector('#wb-3d-btn');
    if (btn3d) btn3d.addEventListener('click', function () { if (window.ARO3D) window.ARO3D.open(); else alert('3D engine not loaded.'); });

...
(function () {
  ...
  var TAG_STOP = /^(REV|DWG|DOC|SHT|SH|SHEET|NO|PG|PAGE|ISO|ANSI|ASME|API|DIN|BS|EN|DATE|SCALE|PROJ|JOB|CH|CHK|APP|DR|OF|FIG|NTS|TYP|MM|IN|KG|PSI|BAR)$/i;
  ...
  function showImportReview(o) {
    ...
    var canModel = tags > 0;
    var detail = canModel
      ? '<div class="wb-imp-ok"><b>' + tags + '</b> equipment tag' + (tags > 1 ? 's' : '') + ' recognised — this drawing can be converted into a live, editable model.</div>'
      : '<div class="wb-imp-warn"><b>No equipment tags found.</b> This '
        + o.label + ' has no readable text — typically a scanned or image-only sheet. '
        + 'It can still be placed as a backdrop to draw over with the library.</div>';
    var body = ''
      ...
      + 'Everything stays editable in the 2D, P&amp;ID and 3D workbenches.</div>'
      + '<div class="wb-imp-btns">'
      + (canModel ? '<button class="wb-btn" data-imp="model">Convert to editable model &amp; open</button>' : '')
      + '<button class="wb-btn' + (canModel ? ' wb-btn-mut' : '') + '" data-imp="backdrop">Open as trace-over backdrop</button>'

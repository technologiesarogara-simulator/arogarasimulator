...
(function () {
  ...
  var LIB = {
    ...
    'Instruments': [
      ...
      { t: 'thermowell', n: 'Thermowell', w: 34, h: 20, ports: [P(0, 10, 'w', 'in', 'Process'), P(34, 10, 'e', 'out', '')],
        draw: function () { return '<line x1="4" y1="10" x2="34" y2="10" stroke="url(#wbSteel)" stroke-width="6"/><path d="M4 6 L4 14" stroke="#334155" stroke-width="2"/><path d="M8 7 L8 13 M12 7 L12 13" stroke="#64748b" stroke-width="1"/>'; } },
      ...
      { t: 'temp-switch', n: 'Temperature Switch', w: 30, h: 20, ports: [P(0, 10, 'w', 'in', 'Process')],
        draw: function () { return '<circle cx="22" cy="10" r="8" fill="#1e293b" stroke="#0f172a" stroke-width="1.2"/><line x1="0" y1="10" x2="14" y2="10" stroke="url(#wbSteel)" stroke-width="4"/>'; } },
  ...
  /* The "Fittings" palette section (elbows/tees/reducers/flange pair) was
     removed from the visible palette, but any project saved before that
     change can still reference these type keys on already-placed nodes.
     Kept here — indexed but not shown as a sidebar category — purely so
     old flowsheets keep rendering instead of hitting an undefined
     LIB_INDEX[n.t] lookup. */
  var LIB_LEGACY = {
    'Fittings (legacy)': [
  ...
  function pidSymbol(cat, w, h, node) {
    ...
    if (cat === 'vessel') return (h >= w ? '<rect x="' + (cx - w * 0.28) + '" y="4" width="' + (w * 0.56) + '" height="' + (h - 8) + '" rx="' + (w * 0.26) + '" fill="#fff" stroke="#0f172a" stroke-width="1.6"/>'
      : '<rect x="4" y="' + (cy - h * 0.28) + '" width="' + (w - 8) + '" height="' + (h * 0.56) + '" rx="' + (h * 0.26) + '" fill="#fff" stroke="#0f172a" stroke-width="1.6"/>');
  ...
  function restore(s) { var o = JSON.parse(s); WB.nodes = o.nodes; WB.pipes = o.pipes; WB.seq = o.seq; WB.sel = null; WB.selMulti = []; render(); renderProps(); sync3D(); }
  ...
  WB.save = function () {
    ...
    if (window.ARODATA) {
      ...
      if (nm) {
        ...
        window.ARODATA.saveProject('workbench', nm, payload, WB.cloudId).then(function (rec) {
  ...
  WB.cloudOpen = function () {
    if (!window.ARODATA) { alert('Cloud storage is not available.'); return; }
    setStatus('Loading your saved projects…', '#38bdf8');
    window.ARODATA.listProjects('workbench').then(function (list) {
  ...
  var TAG_RE = /^([A-Za-z]{1,4})[-\s]?(\d{2,4}[A-Za-z]?)$/;
  function typeForTag(tag) {
    for (var i = 0; i < TAG_RULES.length; i++) if (TAG_RULES[i][0].test(tag)) return TAG_RULES[i][1];
    return 'v-vessel';
  }
  ...
  function buildModelFromTexts(texts, box, opts) {
    ...
    texts.forEach(function (t) {
      var raw = String(t.text || '').trim().replace(/\s+/g, ' ');
      var m = raw.match(TAG_RE);
      if (!m) return;
      var tag = (m[1] + '-' + m[2]).toUpperCase();
      if (seen[tag]) return;                 // same tag repeated on the sheet
      seen[tag] = 1;
      found.push({ tag: tag, type: typeForTag(m[1]), x: t.x, y: t.y });
    });
    ...
    for (var i = 0; i + 1 < chain.length; i++) {
      ...
      WB.pipes.push({ id: 'L' + (++WB.seq),
        from: { id: a.id, pi: outPortIndex(a.t) },
        to: { id: b.id, pi: inPortIndex(b.t) },
  ...
  /* Offer the conversion after an import that carried readable tags. */
  function offerModelBuild(texts, box, opts, sourceLabel) {
    var probe = buildModelProbe(texts);
    if (!probe) {
      setStatus('Imported ' + sourceLabel + ' as a backdrop — no equipment tags found, so draw over it with the library.', '#f59e0b');
      return;
    }
    var msg = 'Found ' + probe + ' equipment tag' + (probe > 1 ? 's' : '') + ' in this ' + sourceLabel + '.\n\n'
      + 'Build an editable, runnable model from them?\n\n'
      + 'Equipment is placed where it sits on the drawing and typed from its tag '
      + '(P- pump, E- exchanger, V- vessel, T- column…). Lines are INFERRED in reading '
      + 'order so RUN ANALYSIS has something to solve — check and re-route them to match '
      + 'the real process. The drawing stays behind as a backdrop.\n\n'
      + 'Cancel to keep it as a trace-over backdrop only.';
    if (!window.confirm(msg)) {
      setStatus('Imported ' + sourceLabel + ' as a backdrop (' + probe + ' tags detected — re-import to build a model).', '#0369a1');
      return;
    }
    var res = buildModelFromTexts(texts, box, opts);
    setStatus('Built ' + res.equipment + ' equipment and ' + res.lines + ' inferred lines from the ' + sourceLabel
  ...
  function buildModelProbe(texts) {
    ...
    var seen = {}, n = 0;
    texts.forEach(function (t) {
      var m = String(t.text || '').trim().replace(/\s+/g, ' ').match(TAG_RE);
      if (!m) return;
      var tag = (m[1] + '-' + m[2]).toUpperCase();
      if (!seen[tag]) { seen[tag] = 1; n++; }
    });
    return n;
  }
  WB.import = function () {
    ...
    inp.onchange = function () {
      ...
      if (['png', 'jpg', 'jpeg', 'gif', 'webp', 'bmp'].indexOf(ext) >= 0) {
        ...
        r.onload = function () {
          var img = new Image();
          img.onload = function () { setBackdrop(r.result, img.width, img.height, 'Imported ' + ext.toUpperCase() + ' drawing'); done(); };
      ...
      if (ext === 'svg') {
        ...
        rs.onload = function () {
          var img = new Image();
          img.onload = function () { setBackdrop(img.src, img.naturalWidth || 900, img.naturalHeight || 600, 'Imported SVG drawing'); done(); };
      ...
      if (ext === 'dxf') {
        ...
        rd.onload = function () {
          ...
          setBackdrop('data:image/svg+xml;base64,' + btoa(unescape(encodeURIComponent(res.svg))), res.w, res.h,
            'Imported DXF — ' + res.entities + ' entities');
          done();
          // DXF TEXT entities carry real tags — offer a runnable model
          offerModelBuild(res.texts, { minX: res.minX, minY: res.minY, maxX: res.maxX, maxY: res.maxY },
            { flipY: true }, 'DXF');
        };
            ...
            pdfjsLib.getDocument({ data: new Uint8Array(rp.result) }).promise.then(function (doc) {
              return doc.getPage(1).then(function (page) {
                ...
                return page.render({ canvasContext: cx, viewport: vp }).promise.then(function () {
                  setBackdrop(cv.toDataURL('image/png'), vp.width, vp.height,
                    'Imported PDF page 1 of ' + doc.numPages);
                  done();
                  /* A PDF P&ID keeps its labels as real text, so the tags can
                     be read back with their coordinates and turned into
                     equipment — no image recognition needed. */
                  return page.getTextContent().then(function (tc) {
                    var items = (tc.items || []).map(function (it) {
                      var tr = it.transform || [1, 0, 0, 1, 0, 0];
                      return { text: it.str, x: tr[4], y: tr[5] };
                    }).filter(function (t) { return t.text && t.text.trim(); });
                    var pv = page.getViewport({ scale: 1 });
                    offerModelBuild(items, { minX: 0, minY: 0, maxX: pv.width, maxY: pv.height },
                      { flipY: true }, 'PDF');
                  }).catch(function () {});
                });
              });
  ...
  function buildPaletteHTML() {
    ...
    Object.keys(LIB).forEach(function (cat) {
      ...
      LIB[cat].forEach(function (it) {
        ...
        palette += '<div class="wb-lib' + (pid ? ' wb-lib-pid' : '') + '" draggable="true" data-t="' + it.t + '" title="' + it.n + '">'
  ...
  /* Swap the library thumbnails when the workbench changes. Drag binding is
     ...
     never leaves dead, undraggable tiles behind. */
  function refreshPalette() {
    var el = document.querySelector('.wb-palette');
    if (el) el.innerHTML = buildPaletteHTML();
  }
  function buildUI(root) {
    ...
    root.innerHTML =
      ...
      + toolBtn('pan', '✋', 'Move / Pan — drag anywhere to move the whole canvas (Space or middle-click do this in any tool)')
      + actBtn('selectall', '▦', 'Select all equipment (or drag a box on empty canvas) — drag any one of them to move the whole group')
      ...
      + '<span class="wb-tsep"></span>'
      + '<button class="wb-tool" id="wb-mode2d" title="2D flowsheet mode — detailed equipment icons" style="width:auto;padding:0 10px;font-weight:700;">2D</button>'
      + '<button class="wb-tool" id="wb-modepid" title="P&amp;ID schematic mode — standard ISA/PIP line-art symbols (bowtie valves, capsule vessels, tube-bank exchangers…)" style="width:auto;padding:0 10px;font-weight:700;background:linear-gradient(135deg,#334155,#0f172a);color:#fff;border-color:#0f172a;">📐 P&amp;ID</button>'
      ...
      + '<span class="wb-tsep"></span>'
      + '<label class="wb-tool" title="Line size (NPS, inches) — click a 3D line first, then set the pipe diameter" style="width:auto;padding:0 6px;gap:5px;display:inline-flex;align-items:center;font-weight:700;cursor:pointer;">⌀'
      + '<select id="wb-3dline-nps" style="border:none;background:transparent;font-weight:700;cursor:pointer;">'
      + ['0.5','0.75','1','1.5','2','3','4','6','8','10','12','16','20','24'].map(function (v) { return '<option value="' + v + '"' + (v === '3' ? ' selected' : '') + '>' + v + '&quot;</option>'; }).join('')
      + '</select></label>'
      + '<button class="wb-tool" id="wb-3dline-sizeall" title="Apply this size to ALL pipes" style="width:auto;padding:0 8px;">ALL DIA</button>'
      + '</span>'
  ...
  function onDown(e) {
    ...
    if (g && WB.mode === 'select') {
      ...
      WB.selMulti = [];
      WB.sel = { kind: 'node', id: n.id }; drag = { id: n.id, dx: w0.x - n.x, dy: w0.y - n.y, moved: false }; renderProps(); render();
      return;
    }
    ...
    if (!g && WB.mode === 'select' && !e.target.closest('[data-pipe]')) {

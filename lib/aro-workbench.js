  ...
  function wireUI(root) {
    ...
    function renderCallOperatorPanel() {
      ...
      propEl.innerHTML = '<div class="wb-prop-title">FIELD OPERATOR</div>'
        ...
        + '<div id="wb-call-op-actions" style="display:none;">'
        + '<div class="wb-prop-note" style="color:#16a34a;">&#128222; On the line — choose an action. The operator always goes on foot, along the plant\'s own walkways — never by vehicle.</div>'
        + (platNid ? '<button class="wb-panel-btn" id="wb-call-climb">&#11014; CLIMB TO PLATFORM</button>' : '')
        + '<button class="wb-panel-btn" id="wb-call-report">&#128203; SEND MONITOR REPORT</button>'
        + '<button class="wb-panel-btn" id="wb-call-assembly">&#128681; GO TO ASSEMBLY POINT</button>'
      ...
      var actions = propEl.querySelector('#wb-call-op-actions');
      var dispatchRow = propEl.querySelector('#wb-call-dispatch-row');
      // Every action taken from a call is a foot patrol, not a vehicle run
      // — if the operator happens to already be driving, park the vehicle
      // first so "unauthorized route" (driving off-plan to answer a radio
      // call) can't happen, then dispatch on foot and say so right here in
      // the panel, not just as a status-bar line that scrolls away.
      ...
      if (climbBtn) climbBtn.addEventListener('click', function () { dispatchOnFoot('the platform on the nearest tall vessel', function () { window.ARO3D.climbToPlatform(platNid); }); });
      ...
      if (assemblyBtn) assemblyBtn.addEventListener('click', function () { dispatchOnFoot('the assembly point', function () { window.ARO3D.goToAssemblyPoint(); }); });
    }

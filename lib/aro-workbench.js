  ...
  function wireUI(root) {
    ...
    function renderOperabilityReport(report) {
      ...
      var h = '<div class="wb-prop-title">OPERABILITY CHECK</div>'
        + '<div class="wb-prop-note">' + report.length + ' equipment checked — ' + nFail + ' unreachable, ' + nWarn + ' maintenance conflict' + (nWarn === 1 ? '' : 's') + ', ' + nPlat + ' need' + (nPlat === 1 ? 's' : '') + ' a platform.</div>'
      ...
      report.forEach(function (r) {
        h += '<div class="wb-rrow" style="align-items:center;flex-wrap:wrap;gap:4px;"><span style="flex:1 1 auto;min-width:120px;">' + r.type + (r.platformRequired ? ' <i style="color:#d97706;">(platform req.)</i>' : '') + '</span>'
          + badge(r.operatorAccess) + badge(r.maintenanceAccess) + '</div>'
          + (r.maintenanceConflict ? '<div class="wb-prop-note" style="margin:-2px 0 4px;color:#dc2626;">' + r.maintenanceConflict + '</div>' : '');
      });
      propEl.innerHTML = h;

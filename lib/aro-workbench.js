...
(function () {
  ...
  function wireUI(root) {
    ...
    function renderVizLegend(mode) {
      ...
      var bands = window.ARO3D && window.ARO3D.getVizBands ? window.ARO3D.getVizBands() : null;
      var html = '';
      if (mode === 'velocity' || mode === 'pressure') {
        var rows = bands ? bands[mode] : [];
        html += '<h4>' + (mode === 'velocity' ? 'Velocity' : 'Pressure Drop') + '</h4>';
        rows.forEach(function (r) { html += '<div class="wb-vizrow"><span class="wb-vizswatch" style="background:' + r.c + '"></span>' + r.label + '</div>'; });
        html += '<div class="wb-vizrow"><span class="wb-vizswatch" style="background:' + (bands ? bands.notCalc : '#94a3b8') + '"></span>Not yet calculated</div>';
        ...
        html += '<h4>Flow</h4><div class="wb-viznote" style="margin-top:0;border-top:none;padding-top:0;">Animated fluid inside every pipe. Speed reflects RUN ANALYSIS velocity where calculated — indicative, not a transient CFD solve.</div>'
  ...
  var WB_CSS = [
    ...
    '.wb-vizlegend{position:absolute;left:12px;bottom:44px;background:rgba(15,23,42,0.94);color:#e2e8f0;border-radius:8px;padding:10px 12px;font-family:var(--font-mono,monospace);font-size:11px;z-index:5;min-width:172px;box-shadow:0 4px 14px rgba(0,0,0,0.28);}',
    '.wb-vizlegend h4{margin:0 0 6px;font-size:10px;letter-spacing:0.06em;color:#94a3b8;font-weight:700;text-transform:uppercase;}',
    '.wb-vizlegend .wb-vizrow{display:flex;align-items:center;gap:7px;margin:3px 0;}',
    '.wb-vizlegend .wb-vizswatch{width:13px;height:13px;border-radius:3px;flex:none;}',
    '.wb-vizlegend .wb-viznote{margin-top:7px;padding-top:7px;border-top:1px solid rgba(148,163,184,0.25);color:#94a3b8;font-size:9.5px;line-height:1.45;}',
    '.wb-flowctrl{display:flex;align-items:center;gap:6px;margin-top:8px;padding-top:8px;border-top:1px solid rgba(148,163,184,0.25);}',
    '.wb-flowctrl button{background:#1e293b;color:#e2e8f0;border:1px solid #475569;border-radius:4px;padding:3px 9px;font-size:10px;cursor:pointer;font-family:inherit;}',
    '.wb-flowctrl button:hover{background:#334155;}',
    '.wb-flowctrl input[type=range]{width:64px;accent-color:#22c55e;}',
    '.wb-flowctrl .wb-flowspeed{font-size:9.5px;color:#94a3b8;min-width:26px;}',

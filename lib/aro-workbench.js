...
(function () {
  ...
  var AROPORTS = window.AROPORTS = {
    /* Golden-pump canonical port spec (AROGARA equipment-library Stage 1/2).
       nps/pressureClass here are the equipment's own rated nozzle spec —
       the same field the spec's PortDefinition.nps/pressureClass describe —
       consulted by connectPorts() to block an incompatible-class connection
       (rule 7) and by the 3D factory to size each nozzle's real flange, so
       a pipe of a different NPS gets a real reducer instead of a stretched
       flange (rule 6). */
    'pump': [
      { id: 'suction', role2d: 'in', role3d: 'in', name: 'Suction', nps: 3, pressureClass: '150' },
      { id: 'discharge', role2d: 'out', role3d: 'out', name: 'Discharge', nps: 2, pressureClass: '150' },
      { id: 'vent', role2d: 'vent', role3d: 'vent', name: 'Casing Vent', nps: 0.75, pressureClass: '150' },
      { id: 'drain', role2d: 'drain', role3d: 'drain', name: 'Casing Drain', nps: 0.75, pressureClass: '150' }
    ],
    'v-vessel': [
    ...
    'h-vessel': [
      ...
      { id: 'vent', role2d: 'vent', role3d: null, name: 'Vent' },
      { id: 'drain', role2d: 'drain', role3d: 'out2', name: 'Drain' }
    ],
    'sthe': [
    ...
    'column': [
      ...
      { id: 'reflux', role2d: 'recycle', role3d: 'in2', name: 'Reflux' },
      { id: 'bottoms', role2d: 'liq', role3d: 'out2', name: 'Bottoms' }
    ],
    'cone-tank': [
    ...
    'boiler': [
      { id: 'bfwIn', role2d: 'in', role3d: 'in', name: 'BFW / Feed Inlet' },
      { id: 'steamOut', role2d: 'vap', role3d: 'out', name: 'Steam / Vapor Outlet' }
    ],
    'condenser': [
      { id: 'vaporIn', role2d: 'vap', role3d: 'in', name: 'Vapor In' },
      { id: 'condensate', role2d: 'liq', role3d: 'out', name: 'Condensate Out' },
      { id: 'cwIn', role2d: 'cold-in', role3d: 'in2', name: 'CW In' },
      { id: 'cwOut', role2d: 'cold-out', role3d: 'out2', name: 'CW Out' }
    ]
  };
  ...
  /* ── Flat industrial schematic kit ─────────────────────────────────────
     Thin dark outline, flat (non-gradient) fill, and real engineering
     marks — a short nozzle stub ending in a flange face, with bolt dots on
     the larger process connections, plus supports/skirts/motors — instead
     of the glossy 3D-bevel look above. Deliberately NOT a copy of any
     third-party simulator's icon set: original shapes, but the same
     flat/clean schematic language, with more visible engineering detail
     (flanges, bolts, nozzles, motors, supports) than a plain 2-letter-badge
     schematic bothers to draw. */
  var FI_INK = '#1e293b', FI_LINE = '#334155';
  function fiTickHoriz(x, y, len) { var h = len / 2; return '<line x1="' + (x - h) + '" y1="' + y + '" x2="' + (x + h) + '" y2="' + y + '" stroke="' + FI_INK + '" stroke-width="2.2" stroke-linecap="round"/>'; }
  function fiTickVert(x, y, len) { var h = len / 2; return '<line x1="' + x + '" y1="' + (y - h) + '" x2="' + x + '" y2="' + (y + h) + '" stroke="' + FI_INK + '" stroke-width="2.2" stroke-linecap="round"/>'; }
  function fiBoltsHoriz(x, y, r, n) { var s = ''; for (var i = 0; i < n; i++) { var t = n === 1 ? 0 : (i / (n - 1) - 0.5) * 2; s += '<circle cx="' + (x + t * r) + '" cy="' + y + '" r="0.8" fill="' + FI_INK + '"/>'; } return s; }
  function fiBoltsVert(x, y, r, n) { var s = ''; for (var i = 0; i < n; i++) { var t = n === 1 ? 0 : (i / (n - 1) - 0.5) * 2; s += '<circle cx="' + x + '" cy="' + (y + t * r) + '" r="0.8" fill="' + FI_INK + '"/>'; } return s; }
  function fiSkid(cx, y, w) {                       // flat steel skid with a light hatch texture
    var s = '<rect x="' + (cx - w / 2) + '" y="' + y + '" width="' + w + '" height="4.5" rx="1" fill="#a8b3c2" stroke="' + FI_LINE + '" stroke-width="1"/>';
    var n = Math.max(2, Math.round(w / 9));
    for (var i = 0; i < n; i++) { var x0 = cx - w / 2 + 3 + i * (w - 6) / Math.max(1, n - 1); s += '<line x1="' + x0 + '" y1="' + (y + 0.6) + '" x2="' + (x0 + 3) + '" y2="' + (y + 3.9) + '" stroke="' + FI_LINE + '" stroke-width="0.7" opacity="0.6"/>'; }
    return s;
  }
  function fiSaddle(x, yTop, yBase, w) {             // flat trapezoid saddle support for a horizontal shell
    return '<path d="M' + x + ' ' + yTop + ' L' + (x + w) + ' ' + yTop + ' L' + (x + w + 3) + ' ' + yBase + ' L' + (x - 3) + ' ' + yBase + ' Z" fill="#b8c3d1" stroke="' + FI_LINE + '" stroke-width="1.1"/>';
  }
  ...
  var LIB = {
    'Equipment': [
      { t: 'pump', n: 'Centrifugal Pump', w: 68, h: 54, ports: [P(2, 32, 'w', 'in', 'Suction'), P(20, 4, 'n', 'out', 'Discharge'), P(34, 4, 'n', 'vent', 'Casing Vent'), P(18, 50, 's', 'drain', 'Casing Drain')],
        draw: function () { return fiSkid(37, 48, 56)
          + '<rect x="40" y="19" width="22" height="22" rx="2.5" fill="#c3cee0" stroke="' + FI_LINE + '" stroke-width="1.6"/>'
          + '<line x1="45" y1="21" x2="45" y2="39" stroke="' + FI_LINE + '" stroke-width="1"/><line x1="51" y1="21" x2="51" y2="39" stroke="' + FI_LINE + '" stroke-width="1"/><line x1="57" y1="21" x2="57" y2="39" stroke="' + FI_LINE + '" stroke-width="1"/>'
          + '<rect x="34" y="26" width="7" height="12" rx="1.5" fill="#dbe2ee" stroke="' + FI_LINE + '" stroke-width="1.4"/>'
          + '<circle cx="24" cy="32" r="15" fill="#a9c6ea" stroke="#274a72" stroke-width="1.8"/>'
          + '<circle cx="24" cy="32" r="5.5" fill="#7fa2cf" stroke="#274a72" stroke-width="1.2"/>'
          + '<line x1="9" y1="32" x2="2" y2="32" stroke="' + FI_LINE + '" stroke-width="4" stroke-linecap="square"/>' + fiTickVert(2, 32, 9) + fiBoltsVert(2, 32, 3, 3)
          + '<path d="M24 17 V10 H20 V4" fill="none" stroke="' + FI_LINE + '" stroke-width="4" stroke-linecap="square"/>' + fiTickHoriz(20, 4, 9) + fiBoltsHoriz(20, 4, 3, 3)
          + '<path d="M30 20 V12 H34 V4" fill="none" stroke="' + FI_LINE + '" stroke-width="2.2" stroke-linecap="square"/>' + fiTickHoriz(34, 4, 5.5)
          + '<path d="M19 46 V50" fill="none" stroke="' + FI_LINE + '" stroke-width="2.2" stroke-linecap="square"/>' + fiTickHoriz(18, 50, 5.5); } },
      ...
      { t: 'sthe', n: 'Shell & Tube HX', w: 96, h: 50, ports: [P(8, 25, 'w', 'cold-in', 'Tube In'), P(88, 25, 'e', 'cold-out', 'Tube Out'), P(24, 10, 'n', 'hot-in', 'Shell In'), P(72, 40, 's', 'hot-out', 'Shell Out')],
        draw: function () { return fiSaddle(28, 40, 46, 8) + fiSaddle(64, 40, 46, 8)
          + '<rect x="14" y="10" width="68" height="30" rx="15" fill="#cbd5e1" stroke="' + FI_LINE + '" stroke-width="1.8"/>'
          + '<line x1="22" y1="12" x2="22" y2="38" stroke="' + FI_LINE + '" stroke-width="1.4"/><line x1="74" y1="12" x2="74" y2="38" stroke="' + FI_LINE + '" stroke-width="1.4"/>'
          + '<g stroke="#94a3b8" stroke-width="1"><line x1="24" y1="18" x2="72" y2="18"/><line x1="24" y1="25" x2="72" y2="25"/><line x1="24" y1="32" x2="72" y2="32"/></g>'
          + '<line x1="14" y1="25" x2="8" y2="25" stroke="' + FI_LINE + '" stroke-width="4" stroke-linecap="square"/>' + fiTickVert(8, 25, 9) + fiBoltsVert(8, 25, 3, 3)
          + '<line x1="82" y1="25" x2="88" y2="25" stroke="' + FI_LINE + '" stroke-width="4" stroke-linecap="square"/>' + fiTickVert(88, 25, 9) + fiBoltsVert(88, 25, 3, 3)
          + '<line x1="24" y1="18" x2="24" y2="10" stroke="' + FI_LINE + '" stroke-width="2.6" stroke-linecap="square"/>' + fiTickHoriz(24, 10, 7)
          + '<line x1="72" y1="32" x2="72" y2="40" stroke="' + FI_LINE + '" stroke-width="2.6" stroke-linecap="square"/>' + fiTickHoriz(72, 40, 7); } },
      ...
      { t: 'condenser', n: 'Condenser', w: 88, h: 46, ports: [P(22, 0, 'n', 'vap', 'Vapor In'), P(0, 26, 'w', 'cold-in', 'CW In'), P(88, 26, 'e', 'cold-out', 'CW Out'), P(66, 46, 's', 'liq', 'Condensate')],
        draw: function () { return fiSaddle(26, 38, 44, 8) + fiSaddle(58, 38, 44, 8)
          + '<rect x="10" y="12" width="68" height="26" rx="13" fill="#a7dbe0" stroke="#0f5e63" stroke-width="1.8"/>'
          + '<line x1="18" y1="20" x2="72" y2="20" stroke="#5aa8ad" stroke-width="1"/><line x1="18" y1="30" x2="72" y2="30" stroke="#5aa8ad" stroke-width="1"/>'
          + '<line x1="22" y1="12" x2="22" y2="0" stroke="' + FI_LINE + '" stroke-width="2.4" stroke-linecap="square"/>' + fiTickHoriz(22, 0, 6)
          + '<line x1="10" y1="26" x2="0" y2="26" stroke="' + FI_LINE + '" stroke-width="2.6" stroke-linecap="square"/>' + fiTickVert(0, 26, 7) + fiBoltsVert(0, 26, 2.4, 2)
          + '<line x1="78" y1="26" x2="88" y2="26" stroke="' + FI_LINE + '" stroke-width="2.6" stroke-linecap="square"/>' + fiTickVert(88, 26, 7) + fiBoltsVert(88, 26, 2.4, 2)
          + '<line x1="66" y1="38" x2="66" y2="46" stroke="' + FI_LINE + '" stroke-width="2.4" stroke-linecap="square"/>' + fiTickHoriz(66, 46, 6); } },
    ...
    'Vessels & Tanks': [
      ...
      { t: 'h-vessel', n: 'Horizontal Vessel', w: 90, h: 46, ports: [P(0, 23, 'w', 'in', 'Feed'), P(90, 23, 'e', 'out', 'Product'), P(45, 0, 'n', 'vent', 'Vent'), P(45, 46, 's', 'drain', 'Drain')],
        draw: function () { return fiSaddle(24, 38, 44, 8) + fiSaddle(64, 38, 44, 8)
          + '<rect x="8" y="8" width="74" height="30" rx="15" fill="#a9c6ea" stroke="#274a72" stroke-width="1.8"/>'
          + '<circle cx="70" cy="23" r="2.6" fill="#dbe8f7" stroke="#274a72" stroke-width="1"/>'
          + '<line x1="8" y1="23" x2="0" y2="23" stroke="' + FI_LINE + '" stroke-width="4" stroke-linecap="square"/>' + fiTickVert(0, 23, 9) + fiBoltsVert(0, 23, 3, 3)
          + '<line x1="82" y1="23" x2="90" y2="23" stroke="' + FI_LINE + '" stroke-width="4" stroke-linecap="square"/>' + fiTickVert(90, 23, 9) + fiBoltsVert(90, 23, 3, 3)
          + '<line x1="45" y1="8" x2="45" y2="0" stroke="' + FI_LINE + '" stroke-width="2.4" stroke-linecap="square"/>' + fiTickHoriz(45, 0, 6)
          + '<line x1="45" y1="38" x2="45" y2="46" stroke="' + FI_LINE + '" stroke-width="2.4" stroke-linecap="square"/>' + fiTickHoriz(45, 46, 6); } },
      ...
      { t: 'cone-tank', n: 'Cone Roof Tank', w: 76, h: 70, ports: [P(0, 58, 'w', 'out', 'Outlet'), P(38, 70, 's', 'drain', 'Drain'), P(38, 2, 'n', 'in', 'Fill'), P(66, 24, 'e', 'vent', 'Vent')],
        draw: function () { return '<path d="M12 24 L38 6 L64 24 Z" fill="#dbe2ee" stroke="' + FI_LINE + '" stroke-width="1.6"/>'
          + '<rect x="12" y="24" width="52" height="34" fill="#a9c6ea" stroke="#274a72" stroke-width="1.8"/>'
          + '<ellipse cx="38" cy="58" rx="26" ry="4" fill="#8fb3dc" stroke="#274a72" stroke-width="1.4"/>'
          + '<line x1="38" y1="10" x2="38" y2="2" stroke="' + FI_LINE + '" stroke-width="2.4" stroke-linecap="square"/>' + fiTickHoriz(38, 2, 6)
          + '<line x1="64" y1="24" x2="66" y2="24" stroke="' + FI_LINE + '" stroke-width="2.2" stroke-linecap="square"/>' + fiTickVert(66, 24, 5)
          + '<line x1="12" y1="58" x2="0" y2="58" stroke="' + FI_LINE + '" stroke-width="4" stroke-linecap="square"/>' + fiTickVert(0, 58, 9) + fiBoltsVert(0, 58, 3, 3)
          + '<line x1="38" y1="58" x2="38" y2="70" stroke="' + FI_LINE + '" stroke-width="2.4" stroke-linecap="square"/>' + fiTickHoriz(38, 70, 6); } },
    ...
    'Columns & Reactors': [
      { t: 'column', n: 'Distillation Column', w: 48, h: 88, ports: [P(24, 0, 'n', 'vap', 'Overhead'), P(0, 44, 'w', 'in', 'Feed'), P(48, 30, 'e', 'recycle', 'Reflux'), P(24, 88, 's', 'liq', 'Bottoms')],
        draw: function () { return '<path d="M12 14 Q24 4 36 14 V72 Q36 82 24 82 Q12 82 12 72 Z" fill="#cdbfe8" stroke="#4a2f86" stroke-width="1.8"/>'
          + '<g stroke="#7a5fb0" stroke-width="1"><line x1="14" y1="20" x2="34" y2="20"/><line x1="14" y1="30" x2="34" y2="30"/><line x1="14" y1="40" x2="34" y2="40"/><line x1="14" y1="50" x2="34" y2="50"/><line x1="14" y1="60" x2="34" y2="60"/><line x1="14" y1="70" x2="34" y2="70"/></g>'
          + '<circle cx="33" cy="46" r="2.6" fill="#e6ddf5" stroke="#4a2f86" stroke-width="1"/>'
          + '<path d="M14 82 L34 82 L31 88 L17 88 Z" fill="#b8c3d1" stroke="' + FI_LINE + '" stroke-width="1.2"/>'
          + '<line x1="24" y1="8" x2="24" y2="0" stroke="' + FI_LINE + '" stroke-width="2.6" stroke-linecap="square"/>' + fiTickHoriz(24, 0, 7) + fiBoltsHoriz(24, 0, 2.4, 2)
          + '<line x1="12" y1="44" x2="0" y2="44" stroke="' + FI_LINE + '" stroke-width="2.6" stroke-linecap="square"/>' + fiTickVert(0, 44, 7) + fiBoltsVert(0, 44, 2.4, 2)
          + '<line x1="36" y1="30" x2="48" y2="30" stroke="' + FI_LINE + '" stroke-width="2.2" stroke-linecap="square"/>' + fiTickVert(48, 30, 6)
          + '<line x1="24" y1="82" x2="24" y2="88" stroke="' + FI_LINE + '" stroke-width="2.6" stroke-linecap="square"/>' + fiTickHoriz(24, 88, 7) + fiBoltsHoriz(24, 88, 2.4, 2); } },
    ...
    'Utilities & Mixers': [
      ...
      { t: 'boiler', n: 'Boiler', w: 62, h: 58, ports: [P(0, 40, 'w', 'in', 'BFW In'), P(31, 0, 'n', 'vap', 'Steam Out')],
        ...
        draw: function () { return '<path d="M16 18 Q31 4 46 18 L46 46 Q46 52 40 52 L22 52 Q16 52 16 46 Z" fill="#e7cba3" stroke="#7a4a12" stroke-width="1.8"/>'
          + '<path d="M25 44 Q31 32 37 44 Q38 48 31 48 Q24 48 25 44 Z" fill="#d98a4a" stroke="#7a4a12" stroke-width="1.2"/>'
          + '<line x1="20" y1="24" x2="42" y2="24" stroke="#c99a5f" stroke-width="1.2"/><line x1="20" y1="30" x2="42" y2="30" stroke="#c99a5f" stroke-width="1.2"/>'
          + '<line x1="16" y1="40" x2="0" y2="40" stroke="' + FI_LINE + '" stroke-width="4" stroke-linecap="square"/>' + fiTickVert(0, 40, 9) + fiBoltsVert(0, 40, 3, 2)
          + '<line x1="31" y1="7" x2="31" y2="0" stroke="' + FI_LINE + '" stroke-width="2.6" stroke-linecap="square"/>' + fiTickHoriz(31, 0, 7); } },
      { t: 'ejector', n: 'Steam Ejector', w: 58, h: 36, ports: [P(0, 18, 'w', 'in', 'Suction'), P(58, 18, 'e', 'out', 'Discharge'), P(28, 0, 'n', 'hot-in', 'Motive')],
        draw: function () { return '<path d="M4 11 L22 11 L32 18 L54 12 L54 24 L32 18 L22 25 L4 25 Z" fill="#d7dee6" stroke="' + FI_LINE + '" stroke-width="1.6"/>'
          + '<line x1="4" y1="18" x2="0" y2="18" stroke="' + FI_LINE + '" stroke-width="2.6" stroke-linecap="square"/>' + fiTickVert(0, 18, 6)
          + '<line x1="54" y1="18" x2="58" y2="18" stroke="' + FI_LINE + '" stroke-width="2.6" stroke-linecap="square"/>' + fiTickVert(58, 18, 6)
          + '<line x1="28" y1="9" x2="28" y2="0" stroke="' + FI_LINE + '" stroke-width="2.2" stroke-linecap="square"/>' + fiTickHoriz(28, 0, 5); } },
      { t: 'mixer', n: 'Static Mixer', w: 62, h: 28, ports: [P(0, 14, 'w', 'in', 'In'), P(62, 14, 'e', 'out', 'Out')],
        draw: function () { return '<rect x="6" y="4" width="50" height="20" rx="10" fill="#dbe2ee" stroke="' + FI_LINE + '" stroke-width="1.6"/>'
          + '<path d="M12 6 L24 22 M24 6 L36 22 M36 6 L48 22" stroke="' + FI_LINE + '" stroke-width="1.4"/>'
          + '<line x1="6" y1="14" x2="0" y2="14" stroke="' + FI_LINE + '" stroke-width="2.6" stroke-linecap="square"/>' + fiTickVert(0, 14, 7) + fiBoltsVert(0, 14, 2.4, 2)
          + '<line x1="56" y1="14" x2="62" y2="14" stroke="' + FI_LINE + '" stroke-width="2.6" stroke-linecap="square"/>' + fiTickVert(62, 14, 7) + fiBoltsVert(62, 14, 2.4, 2); } },
  ...
  /* ───────────── Refinery FLOWSHEET library ─────────────
     Industrial process-unit templates based on real refinery flowsheets.
     Each is an object with explicit equipment positions and pipe connections
     so the resulting diagram mirrors actual plant layouts. */
  var FLOWSHEETS = {
    'Crude & Distillation': {
      'Atmospheric Distillation Unit (ADU/CDU)': {
        /* Node x/y here are the 2D SHEET layout — wide enough for the long
           process tags (rule: 2D/P&ID need room for text; that's a
           different constraint than 3D's compact real-plant spacing).
           The two are decoupled by K in aro-workbench-3d.js, which scales
           these SAME positions down much further for the 3D scene, so
           tightening 3D no longer has to mean crushing 2D/P&ID labels
           into each other. */
        fluid: 'Crude Oil', zoom: 0.62, panX: 10, panY: 10,
        nodes: [
          { t: 'cone-tank', tag: 'T-101 CRUDE FEED TANK',              x: 40,   y: 140, nps: 8 },
          { t: 'pump',      tag: 'P-101A/B CRUDE CHARGE PUMPS',        x: 100,  y: 320, nps: 8 },
          { t: 'sthe',      tag: 'E-101A-C PREHEAT EXCHANGER TRAIN',   x: 280,  y: 200, nps: 6 },
          { t: 'h-vessel',  tag: 'V-101 ELECTROSTATIC DESALTER',       x: 480,  y: 200, nps: 6 },
          { t: 'boiler',    tag: 'H-101 CRUDE FIRED HEATER',           x: 680,  y: 160, nps: 6 },
          { t: 'column',    tag: 'C-101 ATM. DISTILLATION COLUMN',     x: 870,  y: 50,  nps: 6 },
          { t: 'condenser', tag: 'E-102 OVERHEAD CONDENSER',           x: 1020, y: 60,  nps: 4 },
          { t: 'h-vessel',  tag: 'V-102 REFLUX DRUM',                  x: 1020, y: 180, nps: 4 },
          { t: 'pump',      tag: 'P-102 REFLUX PUMP',                  x: 1040, y: 300, nps: 4 },
          { t: 'column',    tag: 'C-102 NAPHTHA STABILIZER',           x: 1200, y: 70,  nps: 4 }
        ],
        pipes: [
          { from: 0, fp: 'out',     to: 1, tp: 'in',      tag: 'CRUDE FROM T-101',       nps: 8 },
          { from: 1, fp: 'out',     to: 2, tp: 'cold-in',  tag: 'CRUDE TO PREHEAT',       nps: 8 },
          { from: 2, fp: 'cold-out', to: 3, tp: 'in',      tag: 'HOT CRUDE TO DESALTER',  nps: 6 },
          { from: 3, fp: 'out',     to: 4, tp: 'in',      tag: 'DESALTED CRUDE',         nps: 6 },
          { from: 4, fp: 'vap',     to: 5, tp: 'in',      tag: 'HEATED CRUDE TO COLUMN', nps: 6 },
          { from: 5, fp: 'vap',     to: 6, tp: 'vap',     tag: 'OVERHEAD VAPOUR',        nps: 6 },
          { from: 6, fp: 'liq',     to: 7, tp: 'in',      tag: 'CONDENSATE TO V-102',    nps: 4 },
          { from: 7, fp: 'out',     to: 8, tp: 'in',      tag: 'TO REFLUX PUMP',         nps: 4 },
          /* Jog point pulled in from the automatic midpoint (which sat right
             at the condenser's left edge, putting the line's own tag text
             on top of it) to just clear of the column's own recycle nozzle
             instead. */
          { from: 8, fp: 'out',     to: 5, tp: 'recycle',  tag: 'REFLUX RETURN TO C-101', nps: 4,
            wps: [{ x: 940, y: 282 }, { x: 940, y: 80 }] },
          /* Dropped below the whole E-102/V-102/P-102 reflux train instead of
             the automatic single mid-x jog, which cut close enough past the
             condenser's bottom edge to read as crossing it. */
          { from: 5, fp: 'liq',     to: 9, tp: 'in',      tag: 'NAPHTHA TO STABILIZER',  nps: 4,
            wps: [{ x: 894, y: 260 }, { x: 1160, y: 260 }, { x: 1160, y: 114 }] }
        ]
      },
      'Vacuum Distillation Unit (VDU)': {
        fluid: 'Crude Oil', zoom: 0.65, panX: 10, panY: 10,
        nodes: [
          { t: 'pump',     tag: 'P-201 RESIDUE CHARGE PUMP',          x: 50,   y: 260, nps: 8 },
          { t: 'sthe',     tag: 'E-201 FEED/EFFLUENT EXCHANGER',      x: 240,  y: 180, nps: 6 },
          { t: 'boiler',   tag: 'H-201 VACUUM FURNACE',               x: 450,  y: 160, nps: 6 },
          { t: 'column',   tag: 'C-201 VACUUM DISTILLATION TOWER',    x: 660,  y: 30,  nps: 6 },
          { t: 'ejector',  tag: 'EJ-201A-C STEAM EJECTOR TRAIN',      x: 860,  y: 50,  nps: 4 },
          { t: 'h-vessel', tag: 'V-201 HOTWELL',                      x: 1060, y: 100, nps: 4 },
          { t: 'pump',     tag: 'P-202 VACUUM RESIDUE PUMP',          x: 700,  y: 340, nps: 8 }
        ],
        pipes: [
          { from: 0, fp: 'out',     to: 1, tp: 'cold-in',  tag: 'ATM RESIDUE TO HX',       nps: 8 },
          { from: 1, fp: 'cold-out', to: 2, tp: 'in',      tag: 'HEATED RESIDUE TO FURNACE', nps: 6 },
          { from: 2, fp: 'vap',     to: 3, tp: 'in',      tag: 'HEATED TO FLASH ZONE',     nps: 6 },
          { from: 3, fp: 'vap',     to: 4, tp: 'in',      tag: 'VACUUM OVERHEAD VAPOUR',   nps: 6 },
          { from: 4, fp: 'out',     to: 5, tp: 'in',      tag: 'EJECTOR DISCHARGE',        nps: 4 },
          { from: 3, fp: 'liq',     to: 6, tp: 'in',      tag: 'VACUUM RESIDUE',           nps: 8 }
        ]
      },
      'Two-Stage Crude Desalting Unit': {
        fluid: 'Crude Oil', zoom: 0.62, panX: 10, panY: 10,
        nodes: [
          { t: 'pump',     tag: 'P-301 CRUDE BOOSTER PUMP',                  x: 50,   y: 240, nps: 8 },
          { t: 'sthe',     tag: 'E-301 CRUDE PREHEATER',                     x: 240,  y: 130, nps: 6 },
          { t: 'mixer',    tag: 'MX-301 FIRST-STAGE MIXER',                  x: 440,  y: 200, nps: 6 },
          { t: 'h-vessel', tag: 'V-301 FIRST-STAGE ELECTROSTATIC DESALTER',  x: 620,  y: 90,  nps: 6 },
          { t: 'mixer',    tag: 'MX-302 INTERSTAGE MIXER',                   x: 830,  y: 200, nps: 6 },
          { t: 'h-vessel', tag: 'V-302 SECOND-STAGE ELECTROSTATIC DESALTER', x: 1020, y: 90,  nps: 6 },
          { t: 'h-vessel', tag: 'V-303 BRINE DRUM',                          x: 540,  y: 360, nps: 4 },
          { t: 'pump',     tag: 'P-302 BRINE PUMP',                          x: 740,  y: 340, nps: 4 }
        ],
        pipes: [
          { from: 0, fp: 'out',     to: 1, tp: 'cold-in',  tag: 'RAW CRUDE TO PREHEATER',  nps: 8 },
          { from: 1, fp: 'cold-out', to: 2, tp: 'in',      tag: 'HEATED CRUDE TO MIXER',   nps: 6 },
          { from: 2, fp: 'out',     to: 3, tp: 'in',      tag: 'CRUDE/WATER EMULSION',    nps: 6 },
          { from: 3, fp: 'out',     to: 4, tp: 'in',      tag: 'FIRST-STAGE OIL',         nps: 6 },
          { from: 4, fp: 'out',     to: 5, tp: 'in',      tag: 'TO SECOND DESALTER',      nps: 6 },
          { from: 3, fp: 'drain',   to: 6, tp: 'in',      tag: 'BRINE FROM V-301',        nps: 4 },
          { from: 6, fp: 'out',     to: 7, tp: 'in',      tag: 'BRINE TO PUMP',           nps: 4 }
        ]
      }
    }
  };
  ...
  // The equipment's own rated nozzle spec (nps/pressureClass) for one port,
  // by canonical id — undefined for any type/port not yet carrying that
  // metadata in AROPORTS (an honest gap, not a guessed default).
  function portSpec(t, portId) {
    var reg = AROPORTS[t]; if (!reg || !portId) return null;
    for (var i = 0; i < reg.length; i++) if (reg[i].id === portId) return reg[i];
    return null;
  }
  ...
  WB.loadFlowsheet = function (section, name) {
    ...
    pushUndo();
    WB.nodes = []; WB.pipes = []; WB.seq = 0; WB.sel = null; WB.selMulti = [];
    var made = [];
    if (typeof recipe === 'object' && recipe.nodes) {
      var fl = recipe.fluid || 'Crude Oil';
      recipe.nodes.forEach(function (nd) {
        var t = LIB_INDEX[nd.t] ? nd.t : 'v-vessel';
        var n = { id: 'N' + (++WB.seq), t: t, x: nd.x, y: nd.y,
          tag: nd.tag || defaultTag(t), fluid: fl, flow: 50, temp: 120, press: 5, nps: nd.nps || 6 };
        WB.nodes.push(n); made.push(n);
      });
      (recipe.pipes || []).forEach(function (pd) {
        var a = made[pd.from], b = made[pd.to];
        if (!a || !b) return;
        var fp = firstPortByRole(a.t, [pd.fp]);
        var tp = firstPortByRole(b.t, [pd.tp]);
        var role = LIB_INDEX[a.t].ports[fp].role;
        WB.pipes.push({ id: 'L' + (++WB.seq),
          from: { id: a.id, pi: fp, portId: portIdAt(a.t, fp) },
          to:   { id: b.id, pi: tp, portId: portIdAt(b.t, tp) },
          role: role, tag: pd.tag || 'L-' + WB.seq, fluid: fl, flow: 50, nps: pd.nps || 6, length: 12, dz: 0,
          wps: pd.wps });
      });
      WB.zoom = recipe.zoom || 0.9; WB.panX = recipe.panX || 10; WB.panY = recipe.panY || 10;
    } else {
  ...
  function routePipe(a, b, wps) {
    ...
    if (WB.ortho) {
      ...
      pts.push(la);
      /* Manual bypass waypoints (wps, sheet px, same space as node x/y):
         the automatic single mid-X jog above has no idea other equipment
         sits in between — on a dense flowsheet a straight jog can cut
         right through a piece of equipment it was never meant to touch
         (the "line drawn through the column" defect). A hand-placed
         detour route, like a real drafter would add for a line crossing
         a busy area, replaces the automatic jog only where it's given. */
      if (wps && wps.length) {
        wps.forEach(function (w) { pts.push({ x: w.x, y: w.y }); });
      } else if (Math.abs(la.x - lb.x) > 0.5 && Math.abs(la.y - lb.y) > 0.5) {
  ...
  function render() {
    ...
    WB.pipes.forEach(function (p) {
      ...
      var pts = routePipe(pa, pb, p.wps); p._pts = pts;
  ...
  WB.newProject = function () { if (WB.nodes.length && !confirm('Start a new project? Unsaved work will be lost.')) return; pushUndo(); WB.nodes = []; WB.pipes = []; WB.seq = 0; WB.sel = null; WB.selMulti = []; WB.backdrop = null; render(); renderProps(); sync3D(); setStatus('New project.', '#94a3b8'); };
  WB.save = function () {
  ...
  WB.flowsheetBrowser = function () {
    ...
    var body = '<div class="wb-fs-intro">Pick a standard petroleum / petrochemical process flowsheet to load as an editable P&amp;ID template. Each opens with real equipment and piping — study it, then edit to build your own flowsheet. <b>' + total + ' flowsheets</b> in ' + Object.keys(FLOWSHEETS).length + ' section.</div>';
    ...
    Object.keys(FLOWSHEETS).forEach(function (sec) {
      ...
      Object.keys(FLOWSHEETS[sec]).forEach(function (name) {
        var r = FLOWSHEETS[sec][name];
        var n = (typeof r === 'object' && r.nodes) ? r.nodes.length : r.split(';').length;
  ...
  function buildUI(root) {
    ...
    root.innerHTML =
      ...
      + '  <div class="wb-menu"><button>Flowsheets ▾</button><div class="wb-drop"><a data-a="flowsheets">📋 Refinery Flowsheet Library (3 flowsheets)…</a></div></div>'
      ...
      + grp('LINE COLOUR',
          '<label class="wb-tool wb-vtxt" title="Line colour — click a 3D line first for one line, or use ALL" style="gap:5px;">' + icon('palette') + '<input type="color" id="wb-3dline-col" value="#b8c0cc" style="width:24px;height:18px;padding:0;border:none;background:none;cursor:pointer"/></label>'
  ...
  function connectPorts(from, to) {
    ...
    // Rule 7: an incompatible pressure class is blocked, not silently
    // stretched. Only fires when BOTH ends carry a real rated class (never
    // fabricated for equipment outside the registry) and they differ.
    var fromSpec = fromN ? portSpec(fromN.t, fromPortId) : null;
    var toSpec = toN ? portSpec(toN.t, toPortId) : null;
    if (fromSpec && toSpec && fromSpec.pressureClass && toSpec.pressureClass && fromSpec.pressureClass !== toSpec.pressureClass) {
      setStatus('Cannot connect: ' + fromSpec.name + ' (Class ' + fromSpec.pressureClass + ') is not rated to mate with ' + toSpec.name + ' (Class ' + toSpec.pressureClass + ').', '#dc2626');
      return false;
    }
    pushUndo();
  ...
  var WB_CSS = [
    ...
    '.wb-body{flex:1;display:grid;grid-template-columns:0 0 1fr 252px;min-height:0;}',
    // same reasoning as .wb-palette above — stays a real, zero-size grid item
'.wb-resizer{width:0;min-width:0;overflow:hidden;border:none;}',
    '.wb-resizer:hover,.wb-resizer.dragging{background:#ff7538;}',
    /* display:none would drop this from CSS Grid's item flow entirely, and
   with 4 declared columns but only the remaining, still-visible items to
   auto-place, the canvas and properties panel would land in the two
   columns meant for this one and the resizer (both 0px) instead of their
   own — an invisible, zero-width canvas. Kept as a real, zero-size grid
   item instead so auto-placement still lands everything in the right
   track. */
'.wb-palette{width:0;min-width:0;overflow:hidden;padding:0;border:none;}',
    ...
    '@media (max-width:1100px){.wb-body{grid-template-columns:0 0 1fr 212px;}}'
  ].join('');

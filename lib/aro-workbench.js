...
(function () {
  ...
  function showImportReview(o) {
    ...
    if (canModel) {
      ...
      detail = '<div class="wb-imp-warn"><b>This ' + o.label + ' carries no text layer at all.</b> '
        + 'It is a scanned or image-only sheet — the tags on it are pixels, not characters, so there '
        + 'is nothing to read. Re-export it from the CAD or Visio original as a normal (text) PDF, or '
        + 'export the sheet to DXF, and the tags will be picked up. It can still be placed as a '
        + 'backdrop and traced over with the library.</div>';
    } else {

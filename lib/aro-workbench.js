...
(function () {
  ...
  function buildUI(root) {
    ...
    root.innerHTML =
      ...
      + '<span class="wb-tsep"></span>'
      + '<button class="wb-tool" id="wb-3dpipe" title="Pipe tool — connect equipment ports in 3D" style="width:auto;padding:0 9px;font-weight:700;">🔗 PIPE</button>'
      + '<button class="wb-tool" data-3dview="iso" title="Isometric">ISO</button><button class="wb-tool" data-3dview="top">Top</button><button class="wb-tool" data-3dview="front">Fr</button><button class="wb-tool" data-3dview="left">Lf</button><button class="wb-tool" data-3dview="perspective">Psp</button>'
      + '<button class="wb-tool" data-3dmode="wire" title="Wireframe">▨</button><button class="wb-tool" data-3dmode="xray" title="Transparent">◑</button><button class="wb-tool" data-3dmode="section" title="Section">▤</button>'
      + '<button class="wb-tool" data-3dexp="obj" title="Export OBJ" style="width:auto;padding:0 8px;">OBJ</button>'
      + '</span>'
      ...
      + '    <div class="wb-hint">Drag component ▸ drop · <b>drag port→port to connect</b> · <b>drag the X/Y/Z gizmo to rotate</b> (or ↻ mode) · ▩ grid · 🎨 colour · scroll = zoom</div>'
  ...
  function wireUI(root) {
    ...
    function set3D(on) {
      WB.mode3d = on;
      if (svgEl) svgEl.style.display = on ? 'none' : 'block';
      ...
      if (on && window.ARO3D) {
        ...
        setStatus('REAL 3D MODE — drag any equipment from the library and it becomes a real 3D mesh. Left-drag to orbit, wheel to zoom.', '#38bdf8');
      } else {
    ...
    if (triad) {
      ...
      triad.addEventListener('mousedown', function (e) {
        triDrag = { x: e.clientX, start: WB.viewRotate || 0 }; e.preventDefault(); e.stopPropagation();
      });
      ...
      window.addEventListener('mouseup', function () {
        if (triDrag) { setStatus('View rotated to ' + Math.round(WB.viewRotate) + '° — drag the gizmo or use ↻ to rotate.', '#0369a1'); }
        triDrag = null;
      });
      triad.addEventListener('dblclick', function (e) { e.stopPropagation(); WB.viewRotate = 0; WB.viewMode = 'plan'; root.querySelectorAll('[data-view]').forEach(function (b) { b.classList.toggle('active', b.getAttribute('data-view') === 'plan'); }); render(); });
      triad.addEventListener('wheel', function (e) { e.preventDefault(); e.stopPropagation(); WB.viewRotate = (((WB.viewRotate || 0) + (e.deltaY < 0 ? 15 : -15)) % 360 + 360) % 360; render(); }, { passive: false });
    }
    ...
    wrap.addEventListener('drop', function (e) {
      ...
      if (WB.mode3d && window.ARO3D) {
        var lib = LIB_INDEX[t];
        window.ARO3D.addByType(t, lib ? lib.n : t);   // real 3D mesh in the workbench
        return;
      }
  ...
  var WB_CSS = [
    ...
    '.wb-triad{position:absolute;right:8px;bottom:36px;cursor:grab;}',

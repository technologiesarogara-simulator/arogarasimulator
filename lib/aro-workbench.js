...
(function () {
  ...
  Object.keys(LIB_LEGACY).forEach(function (cat) { LIB_LEGACY[cat].forEach(function (it) { LIB_INDEX[it.t] = it; }); });
  ...
  /* A still image of the 2D flowsheet, framed to its own content — the SVG
     serialises with its inline background colour intact, so the picture
     drops straight into a report or a PDF as a normal <img>. */
  function capture2D() {

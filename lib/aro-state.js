...
(function () {
  ...
  var MODULES = {
    ...
    'line-twophase': { tab: 'line-tab', label: 'Two-Phase Line Sizing',
      /* #tp-results is the older two-phase panel that still sits inside the
         same content block. It was never gated, so it showed 0.00 m/s,
         0.000 kg/m³, 0.00 Xtt and 0.0000 bar before anything had been
         calculated — an engineer reading that sees a result, not a blank. */
      gates: ['#tp2-results', '#tp2-advisor', '#tp-results', '#tp-sim-overlay'],
      inputs: '#line-twophase-content input, #line-twophase-content select' },
    sthe: { tab: 'sthe-tab', label: 'Shell & Tube Exchanger',
      /* This last selector used to read '#sthe-tab .panel-output
         .analysis-section', and the double-pipe exchanger shares that tab.
         So every DPHE analysis block — the U0 verification, all four charts,
         the summary report — was hidden by whether SHELL AND TUBE had been
         calculated. An engineer could run a double-pipe design, watch the
         result cards fill in, and find the graphs and the analysis simply
         absent, with nothing on screen connecting that to a different
         exchanger they had never opened. Scoped to the shell-and-tube
         sub-panel, a module now gates only its own output. */
      gates: ['#sthe-water-warning', '#sthe-tuning-panel', '#sthe-recommendations',
              '#sthe-key-summary', '#sthe-flow-compare', '#sthe-sub .panel-output .analysis-section'],
    ...
    dphe: { tab: 'sthe-tab', label: 'Double Pipe Exchanger',
      gates: ['#dphe-results', '#dphe-summary-report', '#dphe-suggestions', '#dphe-charts',
              '#dphe-auto-suggest', '#dphe-u-verdict', '#dphe-u-chart', '#dphe-envelope-chart',
              '#dphe-u0-band-chart', '#dphe-phase-chart', '#dphe-phase-note'],
      /* The U0 verification section holds an INPUT (assumed U0) so it stays on
         screen; only its verdict and its charts are withheld, and the section
         says why. */
      notes: [['#dphe-u-verdict', 'Run the calculation to verify the assumed U\u2080 against the '
                                + 'calculated coefficient and draw the operating envelope.']],
      inputs: '#dphe-form input, #dphe-form select' },
    /* Was scoped to #sthe-sub — the shell-and-tube sub-panel, not the plate
       exchanger's own #phe-sub. Every phe- input sits under #phe-sub, so the
       fingerprint always found nothing, always matched its own empty string,
       and PHE could never be marked OUTDATED: editing a design after a run
       gave no "SUPERSEDED — INPUTS CHANGED" warning at all, on every other
       module but this one. */
    phe: { tab: 'sthe-tab', label: 'Plate Heat Exchanger',
      gates: ['#phe-results'], inputs: '#phe-sub [id^="phe-"]' },
  ...
  var CSS = [
    ...
    '.aro-wait{margin:6px 0;padding:7px 10px;border-left:2px solid var(--border-muted);',
    '  font-family:var(--font-mono);font-size:10px;color:var(--text-muted);line-height:1.55;}',
    '.aro-wait b{letter-spacing:.08em;color:var(--text-muted);}',
    /* an input error, stated plainly, distinct from an engineering warning */
    '.aro-invalid{position:fixed;right:18px;bottom:46px;z-index:100002;max-width:min(430px,92vw);',
    '  background:var(--bg-panel);border:1px solid rgba(198,61,61,0.6);border-left:3px solid #c63d3d;',
    '  border-radius:5px;padding:12px 15px;font-family:var(--font-mono);font-size:11px;',
    '  color:var(--text-main);box-shadow:0 10px 34px rgba(0,0,0,0.30);line-height:1.55;}',
    '.aro-invalid-h{font-size:11px;font-weight:800;letter-spacing:.07em;color:var(--color-fail,#c63d3d);}',
    'body.theme-day .aro-invalid{background:#fbeff0;color:#18212b;}',
  ...
  function apply(id) {
    ...
    for (var i = 0; i < m.gates.length; i++) {
      ...
      var key = id + '::' + sel;
      var ph = PLACED[key];
      /* Only the module's principal panel carries the full NOT CALCULATED
         notice. Repeating it above every withheld chart is noise. */
      if (i > 0) { if (ph) ph.style.display = 'none'; continue; }
      if (gated) {
        var host = els[0];
    ...
    waitingNotes(id, gated);
    staleStrip(id, s === OLD);
    if (!gated) remeasureCharts(id);
  }

  /* A chart drawn into a hidden container is drawn at zero by zero, and it
     stays that way after the container is revealed: the canvas has a real
     size on screen and a 0x0 backing store, so the panel shows an empty
     rectangle where a graph should be. Chart.js will re-measure on demand,
     so every chart inside a panel that has just been released is asked to.
     Twice — once now, and once after a frame, because the browser has not
     necessarily laid the container out yet at the moment the gate is lifted. */
  function remeasureCharts(id) {
    var m = MODULES[id];
    if (!m || !window.Chart || !window.Chart.getChart) return;
    function pass() {
      for (var i = 0; i < m.gates.length; i++) {
        var els = all(m.gates[i]);
        for (var j = 0; j < els.length; j++) {
          var host = els[j];
          var canvases = host.tagName === 'CANVAS' ? [host]
            : Array.prototype.slice.call(host.querySelectorAll('canvas'));
          for (var k = 0; k < canvases.length; k++) {
            try {
              var c = window.Chart.getChart(canvases[k]);
    ...
    pass();
    if (window.requestAnimationFrame) window.requestAnimationFrame(function () { setTimeout(pass, 0); });
    else setTimeout(pass, 30);
  }

  /* A section that must stay on screen because it holds an input should say
     that its analysis is not active yet, rather than showing an empty verdict
     where a verdict will later be. */
  function waitingNotes(id, gated) {
    var m = MODULES[id];
    if (!m || !m.notes) return;
    for (var i = 0; i < m.notes.length; i++) {
      var anchor = $(m.notes[i][0]);
      var key = 'aro-wait-' + id + '-' + i;
      var el = document.getElementById(key);
      if (!gated) { if (el) el.remove(); continue; }
      if (!anchor || !anchor.parentNode) continue;
      if (!el) {
        el = document.createElement('div');
        el.id = key;
        el.className = 'aro-wait';
        el.innerHTML = '<b>&mdash; WAITING FOR CALCULATION</b> <span>' + esc(m.notes[i][1]) + '</span>';
  ...
  function set(id, state, extra) {
    ...
    var s = get(id), was = s.state;
    s.state = state;
    if (extra) for (var k in extra) s[k] = extra[k];
    apply(id);
    /* Modules scroll their results into view at the end of a run. On the
       FIRST run that happened while the panel was still gated, so the scroll
       measured a display:none element and went nowhere — the results
       appeared, silently, below the fold. Reveal once the gate has actually
       lifted. */
    if (state === OK && was !== OK && was !== OLD) {
      var gate = (MODULES[id].gates[0] || '').replace(/^#/, '');
      if (gate && typeof window.aroRevealResults === 'function') {
        setTimeout(function () { try { window.aroRevealResults(gate); } catch (e) {} }, 60);
      }
    }
    notify(id);
  }

  /* ── A module's own write-back is not an input change ──────────────────
     Several modules solve for something the form also holds and write it
     back: the double pipe exchanger writes the outlet temperature it just
     solved for and the hairpin count it selected, shell & tube writes the
     shell diameter and the baffle spacing it chose. Those writes land after
     the module has published, so the fingerprint taken at publish time was
     one revision behind the form almost immediately.

     Nothing showed it until the next edit. Every input on a tab re-tests
     every module on that tab, so one keystroke in a shell & tube field
     marked a double pipe design OUTDATED that nobody had touched — and an
     outdated design has its bill of material, its isometric and its report
     figures withheld, exactly as it should when the design really has moved.
     The verdict was right about a change that never happened.

     So take the fingerprint again once the module's own write-backs have
     settled. If a real edit arrives in that window it flips the module to
     OUTDATED first and this bails, so a genuine change is never absorbed
     into the baseline. */
  function settle(id) {
    [0, 60, 400].forEach(function (ms) {
      setTimeout(function () {
        var s = get(id);
  ...
  function outdatedCheck(id) {
    ...
    if (now !== s.last) { s.last = now; s.rev = (s.rev || 0) + 1; }
  ...
  function boot() {
    ...
    /* A gesture inside a tab is what turns a later publish into a real
       ...
       This listens on CLICK rather than pointerdown, so a control activated
       by keyboard or by an assistive action still counts.

       Only TRUSTED events count. The browser marks an event isTrusted when a
       human caused it, and false when script dispatched it — which is exactly
       the distinction needed here. Opening a tagged design from a project
       writes the tag, service and duty into the module's fields and fires
       change events to make them take; without this test that seeding read as
       an engineer working, the module recalculated on half-filled inputs, and
       the project object came back marked FAIL before anybody had run
       anything. The human click that started the sequence has already set the
    ...
    /* ── A refused submit must say so ───────────────────────────────────
       Every RUN button is a form submit, so the browser runs HTML5
       constraint validation first. When a field fails one — a value outside
       min/max, or off a step the markup declares — the browser CANCELS the
       submit and the module's handler never runs. The button does nothing,
       no message appears anywhere the engineer will see, and the design
       simply refuses to calculate for no stated reason. A low liquid level
       of 1.2 on a field declared step="1" did exactly that.

       The invalid event fires on each offending control before the submit is
       abandoned, so it is the one place this can be caught and turned into
       an engineering message. This reports; it does not override the
       constraint. */
    var invalidBatch = null;
    document.addEventListener('invalid', function (e) {
      var el = e.target;
      if (!el || !el.validationMessage) return;
      if (!invalidBatch) {
        invalidBatch = [];
        setTimeout(function () {
          var list = invalidBatch; invalidBatch = null;
          if (!list || !list.length) return;
          reportInvalid(list);
        }, 0);
      }
      var lab = null;
      try { lab = document.querySelector('label[for="' + CSS.escape(el.id) + '"]'); } catch (er) {}
      invalidBatch.push({
        el: el,
        name: (lab ? lab.textContent : '').replace(/\s+/g, ' ').trim() || el.id || 'a field',
  ...
  /* An input error is not an engineering warning and not a software fault.
     It says which field, what is wrong with it, and what will fix it. */
  function reportInvalid(list) {
    var first = list[0];
    var lines = list.slice(0, 6).map(function (x) {
      return '<li style="margin-bottom:5px;"><b>' + esc(x.name) + '</b> — ' + esc(x.why) + '</li>';
    }).join('');
    var box = document.getElementById('aro-invalid');
    if (box) box.remove();
    box = document.createElement('div');
    box.id = 'aro-invalid';
    box.className = 'aro-invalid';
    box.innerHTML = '<div class="aro-invalid-h">&#10007; INVALID INPUT — THE CALCULATION DID NOT RUN</div>'
      + '<ul style="margin:7px 0 0;padding-left:18px;">' + lines + '</ul>'
      + '<div style="margin-top:8px;font-size:10px;opacity:.85;">Correct the value above and press RUN again. '
      + 'Nothing has been calculated, so no result on this screen has changed.</div>'
      + '<button type="button" class="aro-nc-b" style="margin-top:9px;" id="aro-invalid-x">DISMISS</button>';
    document.body.appendChild(box);
    var x = document.getElementById('aro-invalid-x');
  ...
  window.AROSTATE = {
    ...
    /* diagnostics: which tabs the engineer has actually engaged with */
    gestures: function () { return JSON.parse(JSON.stringify(GESTURE)); },
    /* The input revision is what makes a calculation version mean something:
       it counts the times the inputs have moved away from the run on screen.
       CALC v3 alone says how often the engine ran; INPUT REV 04 against
       CALC v3 says the two no longer describe the same design. */
    inputRev: function (id) { return get(id).rev || 0; },

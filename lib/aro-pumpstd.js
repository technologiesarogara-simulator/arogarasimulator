/* ══════════════════════════════════════════════════════════════════════
   ...
   below is a clause in a standard rather than a rule of thumb:
     ...
     · API 610 / ISO 13709  cl. 6.1.6  NPSH margin
     · API 610 cl. 6.1.11  minimum continuous stable flow
     · API 610 cl. 6.1.7   suction specific speed
...
(function () {
  ...
  /* ── SUGGESTED PUMP SPEED ─────────────────────────────────────────────
     "Pump Speed" is typed as a machine-data assumption (2900 rpm by
     convention), never derived from the duty. This picks, from the
     standard 50 Hz induction-motor speeds, the one whose specific speed
     lands closest to the 2000-3000 (US) band that aro-pumpcurve.js's own
     attainable-efficiency model already treats as the flat, no-penalty
     plateau — i.e. the cheapest standard speed that doesn't push the
     impeller into an inefficient corner for this duty. A screening
     suggestion only: it never overrides what the engineer enters. */
  var STANDARD_SPEEDS_RPM = [2900, 1450, 960, 720];

  function suggestSpeed(Q_m3h, H_m, stages) {
    if (!(Q_m3h > 0) || !(H_m > 0)) return null;
    var candidates = STANDARD_SPEEDS_RPM.map(function (rpm) {
      var Ns = specificSpeed(rpm, Q_m3h, H_m, stages);
      return { rpm: rpm, Ns: Ns, dist: isFinite(Ns) ? Math.abs(Ns - 2500) : Infinity };
    });
    var best = candidates[0];
    candidates.forEach(function (c) { if (c.dist < best.dist) best = c; });
    return { rpm: best.rpm, Ns: best.Ns, candidates: candidates,
             clause: 'screening suggestion — nearest standard 50 Hz speed to the 2000-3000 (US) specific-speed plateau used for the efficiency prediction' };
  }
  ...
  function nssVerdict(Nss) {
    ...
    if (Nss <= 11000) return { ok: true, text: 'Acceptable — within the 11 000 that API 610 cl. 6.1.7 treats as the normal ceiling.' };
    return { ok: false, text: 'Above 11 000 — API 610 cl. 6.1.7 calls for review. The impeller eye is large for the duty and suction recirculation will start well inside the operating range. Lower the speed, raise NPSHa, or specify a double-suction first stage.' };
  }
  ...
  window.AROPUMPSTD = {
    ...
    suggestSpeed: suggestSpeed,
    STANDARD_SPEEDS_RPM: STANDARD_SPEEDS_RPM,

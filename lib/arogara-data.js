...
(function () {
  ...
  /* Resolve as soon as `p` settles, or after `ms` — whichever comes first.
     Firestore keeps write promises pending forever while offline, so any
     path the UI waits on has to be bounded. */
  ...
  function saveProfile(profile) {
    ...
    var write = db().then(function (d) {
      return d.collection('users').doc(id).set(
        Object.assign({ uid: id, createdAt: p.createdAt || ts() }, p), { merge: true }
      );
    }).catch(function (err) {
      console.warn('[ARODATA] profile save fell back to local:', err && err.message);
    });
    // A Firestore write against an unreachable/not-yet-created database is
    // queued offline and its promise NEVER settles. Callers (the sign-in
    // gate) must not block on it — resolve on the local copy and let the
    // cloud write land whenever it can.
    return settleWithin(write, 2500).then(function () { return p; });
  }
  ...
  function saveProject(module, name, payload, projectId) {
    ...
    // Always keep a local copy first, so a save is never lost to a slow
    // or unreachable database.
    var keepLocal = function (r) {
    ...
    var cloud = db().then(function (d) {
      ...
      console.warn('[ARODATA] project save fell back to local:', err && err.message);
      if (!rec.id) { rec.id = projectId || ('local-' + Date.now()); keepLocal(rec); }
    });
    // Same rule as saveProfile: an offline Firestore write never settles,
    // so the UI must not wait on it indefinitely.
    return settleWithin(cloud, 4000).then(function () {
      if (!rec.id) { rec.id = projectId || ('local-' + Date.now()); keepLocal(rec); }
      return rec;
    });
  }
  ...
  /* Named AROCLOUD, not ARODATA: lib/aro-datalib.js also exports
     window.ARODATA (the fluid/material property registry), and it loads
     after this file, so window.ARODATA silently became the property
     registry — every call below was throwing "not a function" with no
     visible error, since nothing awaits these promises for their result. */
  window.AROCLOUD = {

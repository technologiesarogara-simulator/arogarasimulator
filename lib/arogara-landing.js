...
(function () {
  ...
  var FIGURES = [
    ...
    { n: '22',  label: 'Unit quantities',      source: 'each convertible across SI, US customary and mixed metric' },
    { n: '6',   label: 'Design modules',       source: 'pump, line sizing, shell & tube, double pipe, plate exchanger, tank & workbench' }
  ];

  var MODULES = [
    { ic: '🌀', t: 'Pump Hydraulics', d: 'Duty to datasheet: NPSH, total head, power and motor selection with a live 3D pump loop.',
    ...
    { ic: '📏', t: 'Line Sizing', d: 'Every phase on one engine, with the velocity and erosion limits that govern each.',
      f: ['Liquid', 'Steam', 'Gas', 'Slurry', 'Two-phase', 'Pressure drop', 'Velocity limits', 'Erosion check'] },
    { ic: '🔩', t: 'Shell &amp; Tube (STHE)', d: 'Thermal and hydraulic design with TEMA layouts and fabrication-ready output.',
    ...
    { ic: '🔁', t: 'Double-Pipe (DPHE)', d: 'Hairpin sizing with the counter- and co-current cases compared side by side.',
      f: ['Hairpin sizing', 'Counter vs co-current', 'Annulus hydraulics', 'Fouling allowance', 'Manufacturing drawing', 'Datasheet'] },
    { ic: '🟧', t: 'Plate HEx (PHE)', d: 'Chevron-plate design by ε-NTU with the over-surface tuned automatically.',
    ...
    { ic: '🛢️', t: 'Tank Design', d: 'Vertical and horizontal storage tanks with the shell course build-up worked out.',
      f: ['Vertical & horizontal', 'Shell course thickness', 'Roof design', 'Bottom design', 'Nozzle schedule', 'Wind & seismic load'] },
    { ic: '🧊', t: '3D Workbench', d: 'Build the loop on a canvas and watch the model turn as the design changes.',
      f: ['Drag-and-drop P&ID', 'Live 3D plant view', 'Fittings & valves library', 'Flow animation', 'Export to report'] },
    { ic: '🤖', t: 'ARO AI Copilot', d: 'Ask about the design in plain language and get an engineer’s answer, not a search result.',
      f: ['Explains every result', 'Suggests corrections', 'Reads your current design', 'Cites the governing clause'] }
  ];

  var WORKFLOW = [
    { ic: '📥', t: 'Process data',        d: 'Duty, fluid, temperature, pressure.' },
    { ic: '⚙️', t: 'Calculation engine',  d: 'Transparent, standards-based formulae.' },
    { ic: '📐', t: 'Equipment sizing',    d: 'Bores, areas, heads, power.' },
    { ic: '🎯', t: 'Optimisation',        d: 'Auto-design corrections, one click each.' },
    { ic: '🧊', t: '3D model',            d: 'Live, and updated as you type.' },
    { ic: '📄', t: 'Fabrication drawing', d: 'GA drawings and bills of material.' },
    { ic: '📋', t: 'Datasheet',           d: 'The enquiry document, filled in.' },
    { ic: '📕', t: 'PDF report',          d: 'The whole design package, exported.' }
  ];
  ...
  var INDUSTRIES = [
    ['🛢️', 'Oil &amp; Gas'], ['⚗️', 'Chemical Plants'], ['🏭', 'Petrochemical'],
    ['🔥', 'Refineries'], ['⚡', 'Power Plants'], ['🥛', 'Food Industry'],
    ['💊', 'Pharmaceutical'], ['💧', 'Water Treatment'], ['🌬️', 'Hydrogen Plants'],
    ['🌱', 'Fertilizer']
  ];

  var LIBRARY = [
    ['💨', 'Steam tables',            'Saturated and superheated properties.'],
    ['💧', 'Water properties',        'Density, viscosity and vapour pressure with temperature.'],
    ['🧱', 'Material database',       'Allowable stress, conductivity and roughness.'],
    ['🔄', 'Unit converter',          '22 quantities across three unit systems.'],
    ['📖', 'Formula reference',       'Every correlation shown with its source.'],
    ['📏', 'Pipe schedule database',  'Bores by NPS and schedule to ASME B36.10M.'],
    ['⭕', 'Flange database',         'Ratings and facings to ASME B16.5.'],
    ['🌀', 'Pump database',           'Efficiency bands and suction specific speed limits.'],
    ['🔩', 'Exchanger database',      'TEMA types, layouts and fouling factors.']
  ];
  ...
  var AUDIENCE = [
    ['⚗️', 'Chemical Engineers'], ['🔧', 'Mechanical Engineers'], ['📊', 'Process Consultants'],
    ['🔬', 'Research Institutes'], ['🎓', 'Engineering Students'], ['🏗️', 'Plant Designers']
  ];

  var ROADMAP = [
    'Steam Ejector', 'Pressure Vessel', 'Relief Valve', 'Distillation Column',
    'Cooling Tower', 'Reactor', 'P&amp;ID Generator', 'CFD',
    'BOM Generator', 'Cost Estimation', 'Plant Layout', 'AI Design Assistant'
  ];
  ...
  var FOOTER = [
    ...
    { h: 'Resources', items: [
      ...
      { t: 'Tutorials', soon: 1 }, { t: 'Release Notes', soon: 1 }, { t: 'Blog', soon: 1 } ] },
    { h: 'Developers', items: [
      { t: 'API Reference', soon: 1 }, { t: 'Integrations', soon: 1 } ] },
    { h: 'Company', items: [
      { t: 'Vision &amp; Mission', to: 'al-vision' }, { t: 'Who it’s for', to: 'al-who' },
      { t: 'Contact', mail: 'technologiesarogara@gmail.com' } ] },
    { h: 'Legal', items: [
      { t: 'Privacy Policy', soon: 1 }, { t: 'Terms of Use', soon: 1 },
      { t: 'License Agreement', soon: 1 } ] }
  ];
  ...
  function css() {
    return '<style id="aro-land-css">'
      ...
      + '.al-trust{margin-top:24px;font-size:12.5px;color:#6f86b6;letter-spacing:.03em;}'
      ...
      + '.al-vmc .ic{font-size:28px;margin-bottom:12px;display:block;}'
      ...
      + '.al-mod .mi{width:44px;height:44px;border-radius:12px;display:flex;align-items:center;justify-content:center;font-size:22px;background:linear-gradient(135deg,rgba(255,117,56,.18),rgba(42,82,190,.18));border:1px solid rgba(126,162,255,.18);margin-bottom:14px;}'
      ...
      + '.al-step .si{font-size:22px;display:block;margin-bottom:9px;}'
      ...
      + '.al-ind{display:inline-flex;align-items:center;gap:8px;padding:10px 16px;border-radius:999px;border:1px solid rgba(126,162,255,.16);background:rgba(13,22,47,.55);font-size:13.5px;color:#c3d4f2;transition:border-color .16s,background .16s,transform .16s;}'
      ...
      + '.al-libitem .li{font-size:20px;flex:none;line-height:1.2;}'
      ...
      + '.al-whoitem .wi{font-size:24px;display:block;margin-bottom:8px;}'
      ...
      + '.al-footgrid{max-width:1180px;margin:0 auto;display:grid;grid-template-columns:1.6fr repeat(5,1fr);gap:28px;}'
      ...
      + '@media(max-width:860px){.al-navlinks a{display:none;}.al-mods,.al-flow,.al-std,.al-road,.al-lib,.al-vm,.al-caps,.al-ai{grid-template-columns:repeat(2,1fr);}.al-footgrid{grid-template-columns:repeat(3,1fr);}.al-footbrand{grid-column:1/-1;}}'
  ...
  function moduleCard(m) {
    return '<div class="al-card al-mod"><div class="mi">' + m.ic + '</div>'
      + '<h4>' + m.t + '</h4><p>' + m.d + '</p>'
      + '<ul class="al-feat">' + m.f.map(function (x) { return '<li>' + x + '</li>'; }).join('') + '</ul></div>';
  }
  function step(s) {
    return '<div class="al-card al-step"><span class="si">' + s.ic + '</span><b>' + s.t + '</b><span>' + s.d + '</span></div>';
  }
  function stdItem(s) {
    return '<div class="al-card al-stditem"><span class="ck">✓</span><div><b>' + s[0] + '</b><span>' + s[1] + '</span></div></div>';
  }
  function libItem(l) {
    return '<div class="al-card al-libitem"><span class="li">' + l[0] + '</span><div><b>' + l[1] + '</b><span>' + l[2] + '</span></div></div>';
  }
  function cap(t, d) {
  ...
  function html() {
    return css()
      ...
      +      '<a href="#al-vision">Vision</a><a href="#al-modules">Modules</a><a href="#al-flow">Workflow</a>'
      +      '<a href="#al-standards">Standards</a><a href="#al-library">Library</a><a href="#al-roadmap">Roadmap</a>'
      +      '<button class="al-btn al-btn-primary" data-launch>Launch App →</button>'
      + '  </div>'
      + '</nav></div>'
      ...
      + '  <p>AROGARA brings pump hydraulics, pipe line-sizing, heat-exchanger and tank design together with live 3D visualisation, auto-optimisation to industrial norms, and one-click fabrication drawings &amp; reports — so engineers move from duty to datasheet in minutes, not days.</p>'
      ...
      + '      <div class="al-vmc"><span class="ic">🎯</span><h3>Our Mission</h3><p>To help teams size pumps, pipelines, exchangers and tanks accurately and confidently — coupling transparent, standards-based engineering with live 3D insight, smart optimisation and instant, fabrication-ready documentation.</p></div>'
      + '    </div>'
      + '  </section>'
      ...
      + '    <div class="al-inds">' + INDUSTRIES.map(function (i) {
             return '<span class="al-ind">' + i[0] + ' ' + i[1] + '</span>'; }).join('') + '</div>'
      + '  </section>'
      ...
      + '    <div class="al-who">' + AUDIENCE.map(function (a) {
             return '<div class="al-card al-whoitem"><span class="wi">' + a[0] + '</span><b>' + a[1] + '</b></div>'; }).join('') + '</div>'
      + '  </section>'
      ...
      + '    <div class="al-footbrand"><div class="fb">AROGARA TECHNOLOGIES</div>'
      + '      <p>Bharat FlowSize — process engineering, reimagined. A single cloud workbench for sizing, visualising and documenting plant equipment.</p></div>'

...
(function () {
  ...
  var FIGURES = [
    ...
    { n: '10',  label: 'Modelled modules',     source: 'pump, five line-sizing services, shell & tube, double pipe, plate exchanger and storage tank — each with its own parametric 3D model, isometric and take-off' },
    { n: '118', label: '3D components',        source: 'pipe, fittings, valves, instruments, pumps, vessels, columns, exchangers, filters and structure — every one a real mesh, placeable in the ARO Workbench' },
    { n: '98',  label: 'Registry entries',     source: '49 fluids and 49 materials indexed out of the module tables, each property carrying its unit, its stated condition and the table it came from' }
  ];
  ...
  var LIBRARY = [
    ...
    ['cube',      'Component library',       '118 parametric 3D components, shared by every module.'],
    ['data',      'Engineering data registry', '49 fluids and 49 materials, cross-checked and sourced.'],
    ['target',    'Project design basis',    'One basis, adopted into the modules that need it.']
  ];
  ...
  var FOOTER = [
    ...
    { h: 'Company', items: [
      ...
      { t: 'Website', href: 'https://www.arogara.co.in' },
      { t: 'Contact', mail: 'technologiesarogara@gmail.com' } ] }
  ];

  function css() {
    return '<style id="aro-land-css">'
      + '#aro-landing{position:fixed;inset:0;z-index:100001;overflow-y:auto;overflow-x:hidden;background:#ffffff;color:#1c2540;font-family:"Outfit",system-ui,sans-serif;-webkit-font-smoothing:antialiased;}'
      + '#aro-landing *{box-sizing:border-box;min-width:0;}'
      + '#aro-landing a{color:inherit;text-decoration:none;}'
      + '#aro-landing :focus-visible{outline:2px solid #b44c0f;outline-offset:3px;border-radius:4px;}'
      /* A drafting-table grid, not a glowing gradient hero — the accent
         colour is spent on the brand mark and the CTA, not smeared across
         floating blurred orbs behind the headline. */
      + '.al-bg{position:fixed;inset:0;overflow:hidden;pointer-events:none;z-index:0;}'
      + '.al-grid{position:absolute;inset:0;background-image:linear-gradient(rgba(15,23,42,.05) 1px,transparent 1px),linear-gradient(90deg,rgba(15,23,42,.05) 1px,transparent 1px);background-size:32px 32px;mask-image:linear-gradient(180deg,#000 0%,#000 30%,transparent 62%);}'
      + '.al-corner{position:absolute;width:26px;height:26px;border-color:rgba(180,76,15,.4);border-style:solid;border-width:0;}'
      + '.al-corner.tl{top:14px;left:14px;border-top-width:2px;border-left-width:2px;}'
      + '.al-corner.tr{top:14px;right:14px;border-top-width:2px;border-right-width:2px;}'
      + '.al-wrap{position:relative;z-index:1;max-width:1180px;margin:0 auto;padding:0 24px;}'

      /* The nav is sticky, with its own solid surface so content scrolling
         beneath it never shows through. */
      + '.al-navbar{position:sticky;top:0;z-index:20;background:rgba(255,255,255,.92);backdrop-filter:blur(14px);-webkit-backdrop-filter:blur(14px);border-bottom:1px solid rgba(15,23,42,.08);}'
      ...
      + '.al-brand img{width:36px;height:36px;border-radius:4px;}'
      + '.al-brand .bn{font-weight:700;font-size:15.5px;letter-spacing:.14em;color:#111a30;line-height:1.1;}'
      + '.al-brand .bt{font-size:9.5px;letter-spacing:.24em;color:#5b6b8c;font-weight:600;}'
      + '.al-navlinks{display:flex;align-items:center;gap:22px;font-size:13.5px;color:#47547a;font-weight:500;}'
      ...
      + '.al-navlinks a:hover{color:#111a30;border-bottom-color:#b44c0f;}'

      /* Flat, not glowing — a button reads as clickable from its border and
         weight, not from a soft coloured halo behind it. */
      + '.al-btn{cursor:pointer;font-family:inherit;font-weight:600;border-radius:6px;border:1px solid transparent;transition:transform .1s,background .15s,border-color .15s;white-space:nowrap;}'
      + '.al-btn:active{transform:translateY(1px);}'
      + '.al-btn-primary{background:#b44c0f;color:#fff8f1;padding:11px 22px;font-size:14px;border-color:#b44c0f;}'
      + '.al-btn-primary:hover{background:#9c400c;}'
      + '.al-btn-ghost{background:transparent;border-color:rgba(15,23,42,.22);color:#1c2540;padding:11px 22px;font-size:14px;}'
      + '.al-btn-ghost:hover{border-color:rgba(15,23,42,.4);background:rgba(15,23,42,.03);}'

      + '.al-hero{text-align:center;padding:64px 24px 26px;position:relative;z-index:1;max-width:920px;margin:0 auto;}'
      /* A drawing-register line, not a status pill with a pulsing dot — it
         reads the way the app's own module bar does (PROJECT · MODULE ·
         REV), tying the marketing page to the product it is selling. */
      + '.al-docline{display:inline-flex;align-items:center;gap:10px;padding:5px 0;border-top:1px solid rgba(15,23,42,.16);border-bottom:1px solid rgba(15,23,42,.16);font-family:ui-monospace,"IBM Plex Mono",monospace;font-size:11px;letter-spacing:.06em;color:#5b6b85;margin-bottom:26px;}'
      + '.al-docline b{color:#b44c0f;font-weight:700;}'
      + '.al-docline s{width:1px;height:11px;background:rgba(15,23,42,.18);display:inline-block;}'
      + '.al-hero h1{font-size:clamp(30px,5.2vw,52px);line-height:1.12;font-weight:700;color:#111a30;margin:0 0 20px;letter-spacing:-.015em;}'
      + '.al-hero h1 .g{color:#b44c0f;}'
      + '.al-hero p{font-size:clamp(15px,2.1vw,18.5px);line-height:1.62;color:#5b6b8c;max-width:680px;margin:0 auto 30px;}'
      + '.al-cta{display:flex;gap:14px;justify-content:center;flex-wrap:wrap;}'
      + '.al-trust{margin-top:24px;font-size:12.5px;color:#7381a3;letter-spacing:.03em;display:inline-flex;align-items:center;gap:7px;justify-content:center;}'
      ...
      + '.al-eyebrow{text-align:center;font-size:11.5px;letter-spacing:.22em;text-transform:uppercase;color:#b44c0f;font-weight:700;margin-bottom:12px;}'
      + '.al-h2{text-align:center;font-size:clamp(25px,3.7vw,36px);font-weight:700;color:#111a30;margin:0 0 12px;letter-spacing:-.01em;}'
      + '.al-lead{text-align:center;font-size:15.5px;color:#5b6b8c;max-width:660px;margin:0 auto 40px;line-height:1.6;}'

      /* One card surface for the whole page, styled like an index card in a
         technical binder — a squared-off left edge, not a floating chip
         that lifts and glows on hover. */
      + '.al-card{border:1px solid rgba(15,23,42,.11);border-left:2px solid rgba(15,23,42,.11);border-radius:4px;background:#ffffff;transition:border-color .15s;}'
      + '.al-card:hover{border-left-color:#b44c0f;}'

      + '.al-figs{display:grid;grid-template-columns:repeat(6,1fr);gap:12px;max-width:1080px;margin:40px auto 0;}'
      + '.al-fig{padding:16px 12px;text-align:center;border-left:none!important;border-top:2px solid rgba(180,76,15,.5);}'
      + '.al-fig .n{font-family:ui-monospace,"IBM Plex Mono",monospace;font-size:clamp(19px,2.3vw,26px);font-weight:700;color:#111a30;line-height:1.1;}'
      + '.al-fig .l{font-size:11.5px;color:#5b6b8c;margin-top:5px;line-height:1.35;}'

      + '.al-vm{display:grid;grid-template-columns:1fr 1fr;gap:20px;}'
      + '.al-vmc{border:1px solid rgba(15,23,42,.11);border-top:2px solid #33436b;border-radius:4px;padding:30px;background:#fbfbfa;}'
      + '.al-vmc .ic{display:inline-flex;align-items:center;justify-content:center;width:40px;height:40px;border-radius:4px;color:#b44c0f;background:#fff;border:1px solid rgba(15,23,42,.14);margin-bottom:14px;}'
      + '.al-vmc h3{font-size:19px;color:#111a30;margin:0 0 10px;font-weight:600;}'
      + '.al-vmc p{font-size:14.5px;color:#47547a;line-height:1.66;margin:0;}'
      ...
      + '.al-mod .mi{width:40px;height:40px;border-radius:4px;display:flex;align-items:center;justify-content:center;color:#b44c0f;background:#fbfbfa;border:1px solid rgba(15,23,42,.14);margin-bottom:14px;}'
      + '.al-mod h4{font-size:16px;color:#111a30;margin:0 0 6px;font-weight:600;}'
      + '.al-mod > p{font-size:13px;color:#5b6b8c;line-height:1.55;margin:0 0 13px;}'
      + '.al-feat{list-style:none;margin:auto 0 0;padding:12px 0 0;border-top:1px solid rgba(15,23,42,.07);display:flex;flex-direction:column;gap:5px;}'
      + '.al-feat li{font-size:12.5px;color:#47547a;display:flex;gap:7px;align-items:flex-start;line-height:1.4;}'
      + '.al-feat li::before{content:"✓";color:#16a34a;font-weight:700;flex:none;font-size:11.5px;line-height:1.5;}'
      ...
      + '.al-step::after{content:counter(alstep,decimal-leading-zero);position:absolute;top:12px;right:14px;font-size:11px;font-weight:700;color:rgba(15,23,42,.22);font-family:ui-monospace,monospace;}'
      + '.al-step .si{display:flex;align-items:center;color:#33436b;margin-bottom:9px;}'
      + '.al-step b{display:block;color:#111a30;font-size:14px;font-weight:600;margin-bottom:4px;}'
      + '.al-step span{font-size:12.5px;color:#5b6b8c;line-height:1.5;}'
      ...
      + '.al-stditem .ck{flex:none;width:22px;height:22px;border-radius:7px;background:rgba(34,197,94,.12);border:1px solid rgba(34,197,94,.4);color:#16a34a;display:flex;align-items:center;justify-content:center;font-size:12px;font-weight:700;}'
      + '.al-stditem b{display:block;color:#182238;font-size:13.5px;font-weight:600;}'
      + '.al-stditem span{display:block;color:#5b6b8c;font-size:12px;margin-top:2px;line-height:1.45;}'

      + '.al-inds{display:flex;flex-wrap:wrap;gap:10px;justify-content:center;}'
      + '.al-ind{display:inline-flex;align-items:center;gap:9px;padding:8px 15px;border-radius:4px;border:1px solid rgba(15,23,42,.14);background:#ffffff;font-size:13.5px;color:#33436b;transition:border-color .15s;}'
      + '.al-ind:hover{border-color:#b44c0f;}'
      + '.al-ind .aro-ic{color:#5b6b85;flex:none;}'
      + '.al-ind:hover .aro-ic{color:#b44c0f;}'
      ...
      + '.al-libitem .li{display:flex;align-items:center;flex:none;color:#33436b;margin-top:1px;}'
      + '.al-libitem b{display:block;color:#111a30;font-size:14px;font-weight:600;margin-bottom:3px;}'
      + '.al-libitem span{font-size:12.5px;color:#5b6b8c;line-height:1.5;}'
      ...
      + '.al-ask{display:flex;gap:10px;align-items:center;padding:13px 16px;border-radius:4px;border:1px solid rgba(15,23,42,.11);background:#fbfbfa;font-size:14px;color:#1c2540;font-family:ui-monospace,"IBM Plex Mono",monospace;}'
      + '.al-ask .q{color:#b44c0f;font-weight:700;flex:none;}'
      + '.al-aitext h3{font-size:clamp(22px,3vw,30px);color:#111a30;margin:0 0 12px;font-weight:700;}'
      + '.al-aitext p{font-size:15px;color:#5b6b8c;line-height:1.65;margin:0 0 14px;}'
      ...
      + '.al-cap .ck{flex:none;width:26px;height:26px;border-radius:8px;background:rgba(34,197,94,.12);border:1px solid rgba(34,197,94,.4);color:#16a34a;display:flex;align-items:center;justify-content:center;font-size:14px;font-weight:700;}'
      + '.al-cap b{color:#182238;font-size:14.5px;font-weight:600;}.al-cap span{display:block;color:#5b6b8c;font-size:13px;margin-top:2px;line-height:1.5;}'

      + '.al-who{display:grid;grid-template-columns:repeat(6,1fr);gap:12px;}'
      + '.al-whoitem{padding:18px 12px;text-align:center;}'
      + '.al-whoitem .wi{display:flex;align-items:center;justify-content:center;color:#b44c0f;margin-bottom:8px;}'
      + '.al-whoitem b{font-size:13px;color:#1c2540;font-weight:600;line-height:1.35;display:block;}'

      + '.al-road{display:grid;grid-template-columns:repeat(4,1fr);gap:12px;}'
      + '.al-roaditem{padding:14px 16px;display:flex;align-items:center;gap:10px;border:1px dashed rgba(15,23,42,.22);border-radius:4px;background:#fbfbfa;}'
      + '.al-roaditem .rd{width:6px;height:6px;border-radius:50%;background:#b44c0f;flex:none;}'
      + '.al-roaditem b{font-size:13.5px;color:#33436b;font-weight:600;}'

      /* A structural closing panel — a bordered block with a rule across
         the top, not a soft two-tone gradient wash. */
      + '.al-band{margin:20px auto 0;border-radius:4px;padding:48px 32px;text-align:center;background:#fbfbfa;border:1px solid rgba(15,23,42,.12);border-top:3px solid #b44c0f;}'
      + '.al-band h2{font-size:clamp(24px,4vw,34px);color:#111a30;margin:0 0 12px;font-weight:700;}'
      + '.al-band p{color:#47547a;font-size:15px;margin:0 0 26px;}'

      + '.al-foot{border-top:1px solid rgba(15,23,42,.08);margin-top:40px;padding:44px 24px 26px;}'
      + '.al-footgrid{max-width:1180px;margin:0 auto;display:grid;grid-template-columns:1.8fr repeat(3,1fr);gap:32px;}'
      + '.al-footbrand .fb{color:#111a30;font-weight:700;letter-spacing:.12em;font-size:14px;margin-bottom:8px;}'
      + '.al-footbrand p{color:#6b7a9e;font-size:12.5px;line-height:1.7;margin:0;max-width:280px;}'
      + '.al-footcol h5{font-size:11px;letter-spacing:.16em;text-transform:uppercase;color:#b44c0f;margin:0 0 12px;font-weight:700;}'
      + '.al-footcol ul{list-style:none;margin:0;padding:0;display:flex;flex-direction:column;gap:9px;}'
      + '.al-footcol a,.al-footcol .na{font-size:13px;color:#5b6b8c;transition:color .16s;cursor:pointer;display:inline-flex;align-items:center;gap:6px;}'
      + '.al-footcol a:hover{color:#111a30;}'
      + '.al-footcol .na{color:#94a3bd;cursor:default;}'
      + '.al-soon{font-size:9px;letter-spacing:.08em;text-transform:uppercase;padding:1px 5px;border-radius:5px;border:1px solid rgba(15,23,42,.16);color:#7381a3;font-weight:700;}'
      + '.al-footbot{max-width:1180px;margin:32px auto 0;padding-top:20px;border-top:1px solid rgba(15,23,42,.08);text-align:center;color:#7381a3;font-size:12.5px;line-height:1.8;}'
      ...
      + '@media(prefers-reduced-motion:reduce){.al-rv{opacity:1;transform:none;transition:none;}}'

      /* Breakpoints. Each grid steps down rather than squeezing, so nothing
         ever overlaps or overflows the viewport. */
      + '@media(max-width:1100px){.al-mods,.al-flow,.al-std,.al-road,.al-igrid{grid-template-columns:repeat(3,1fr);}.al-figs,.al-who{grid-template-columns:repeat(3,1fr);}}'
      + '@media(max-width:860px){.al-navlinks a{display:none;}.al-mods,.al-flow,.al-std,.al-road,.al-lib,.al-vm,.al-caps,.al-ai,.al-igrid{grid-template-columns:repeat(2,1fr);}.al-footgrid{grid-template-columns:repeat(2,1fr);}.al-footbrand{grid-column:1/-1;}}'
      /* ── engineering-console sections ─────────────────────────────── */
      + '.al-statement{font-size:clamp(15px,2.3vw,20px)!important;color:#182238!important;font-weight:600;'
      ...
      + '.al-pipeRow{margin:0;}'
      + '.al-pipeArrow{color:#8090b3;font-size:16px;line-height:1.5;margin:5px 0;}'
      + '.al-node{display:inline-block;font-family:ui-monospace,"IBM Plex Mono",monospace;font-size:11px;'
      +   'font-weight:700;letter-spacing:.09em;color:#33436b;background:#fbfbfa;'
      +   'border:1px solid rgba(15,23,42,.16);border-radius:4px;padding:8px 16px;}'
      ...
      +   'letter-spacing:.08em;color:#b44c0f;background:#fbfbfa;'
      +   'border:1px solid rgba(180,76,15,.35);border-radius:4px;padding:7px 12px;}'
      /* al-wfstep, not al-step: al-step (the workflow-chain cards above)
         already carries a counter-generated corner badge — reusing that
         class here duplicated the number this section already prints as
         al-stepn, showing "01" twice on the same card. */
      + '.al-steps{display:grid;grid-template-columns:repeat(5,1fr);gap:12px;margin-top:26px;}'
      + '.al-wfstep{background:#fbfbfa;border:1px solid rgba(15,23,42,.11);'
      +   'border-top:2px solid rgba(180,76,15,.5);border-radius:4px;padding:16px 14px;}'
      + '.al-stepn{font-family:ui-monospace,monospace;font-size:20px;font-weight:800;color:#b44c0f;'
      +   'line-height:1;margin-bottom:8px;}'
      + '.al-wfstep h4{margin:0 0 6px;font-size:12px;letter-spacing:.09em;color:#111a30;font-weight:700;}'
      + '.al-wfstep p{margin:0;font-size:12.5px;line-height:1.55;color:#5b6b8c;}'
      /* Capability grid — replaces a wall of small text chips with fewer,
         larger icon cards: one glance per row instead of reading thirty
         words. See al-igrid below. */
      + '.al-igrid{display:grid;grid-template-columns:repeat(4,1fr);gap:14px;margin-top:24px;}'
      + '.al-igitem{padding:20px 18px;text-align:center;}'
      + '.al-igitem .ii{display:inline-flex;align-items:center;justify-content:center;width:46px;height:46px;border-radius:4px;color:#b44c0f;background:#fbfbfa;border:1px solid rgba(15,23,42,.14);margin-bottom:12px;}'
      + '.al-igitem b{display:block;color:#111a30;font-size:13.5px;font-weight:700;margin-bottom:4px;}'
      + '.al-igitem span{display:block;color:#5b6b8c;font-size:12px;line-height:1.5;}'
      + '.al-tri{display:grid;grid-template-columns:1fr 26px 1fr 26px 1fr;gap:10px;align-items:stretch;margin-top:26px;}'
      + '.al-tric{background:#fbfbfa;border:1px solid rgba(15,23,42,.11);'
      +   'border-radius:4px;padding:16px 15px;}'
      + '.al-tric h4{margin:0 0 10px;font-family:ui-monospace,monospace;font-size:11px;letter-spacing:.08em;'
      +   'color:#b44c0f;font-weight:800;}'
      + '.al-tric ul{margin:0;padding-left:16px;}'
      + '.al-tric li{font-size:12.5px;line-height:1.7;color:#47547a;}'
      + '.al-triA{display:flex;align-items:center;justify-content:center;color:#8090b3;font-size:18px;}'
      + '.al-notecard{margin-top:22px;border:1px solid rgba(15,23,42,.1);border-radius:8px;'
      +   'background:#f8f9fd;padding:18px 20px;}'
      + '.al-notecard h4{margin:0 0 10px;font-size:13.5px;letter-spacing:.02em;color:#182238;}'
      + '.al-notecard ul{margin:0;padding-left:18px;}'
      + '.al-notecard li{margin:6px 0;font-size:13px;line-height:1.6;color:#47547a;}'
      + '.al-notecard li b,.al-fine b{color:#182238;}'
      + '.al-fine{margin:12px 0 0;font-size:12.5px;line-height:1.65;color:#5b6b8c;}'
      /* Flowsheet-style connector: a short process line with a direction
         arrowhead, the way a real P&ID draws flow between tags — not a
         plain "→" character. */
      + '.al-traceline{display:flex;flex-wrap:wrap;align-items:center;gap:0;margin-top:8px;justify-content:center;row-gap:14px;}'
      + '.al-traceseg{position:relative;width:26px;height:2px;background:#33436b;flex:none;margin:0 2px;}'
      + '.al-traceseg::after{content:"";position:absolute;right:-1px;top:50%;transform:translateY(-50%);width:0;height:0;'
      +   'border-top:4px solid transparent;border-bottom:4px solid transparent;border-left:6px solid #33436b;}'
      + '@media(max-width:900px){.al-steps,.al-igrid{grid-template-columns:repeat(2,1fr);}'
      +   '.al-tri{grid-template-columns:1fr;}.al-triA{display:none;}}'
      + '@media(max-width:560px){.al-mods,.al-flow,.al-std,.al-road,.al-lib,.al-vm,.al-caps,.al-ai,.al-figs,.al-igrid{grid-template-columns:1fr;}.al-who{grid-template-columns:repeat(2,1fr);}.al-footgrid{grid-template-columns:repeat(2,1fr);}.al-sec{padding:42px 0;}.al-hero{padding:48px 18px 20px;}}'
      + '</style>';
  }
  ...
  function igItem(x) {
    return '<div class="al-card al-igitem"><span class="ii">' + I(x[0], 24) + '</span><b>' + x[1] + '</b><span>' + x[2] + '</span></div>';
  }
  var CAPGRID = [
    ['piping', 'Piping &amp; Fittings', 'Pipe, elbow, tee and reducer — every run built to the calculated bore.'],
    ['valve', 'Valves &amp; Actuators', 'Gate, globe, ball, check and control valves, with real actuators.'],
    ['flange', 'Flanges &amp; Fasteners', 'Weld-neck flanges, gaskets and stud bolts to ASME B16.5.'],
    ['gauge', 'Instruments &amp; Supports', 'Pressure gauges, nozzles and pipe shoe supports, placed.'],
    ['orbit', 'Navigate Freely', 'Rotate, zoom, pan and fit the model to the view — one gesture each.'],
    ['cube', '7 Standard Views', 'ISO, front, back, left, right, top, bottom — true orthographic, not a tilted camera.'],
    ['chart', 'Live Overlays', 'Colour the model by flow, velocity or pressure, straight from the calculation.'],
    ['report', 'Ready to Export', 'Bill of material, isometric and snapshot — read off the model, not typed.']
  ];
  function footCol(c) {
    return '<div class="al-footcol"><h5>' + c.h + '</h5><ul>' + c.items.map(function (i) {
      ...
      if (i.href) return '<li><a href="' + i.href + '" target="_blank" rel="noopener">' + i.t + '</a></li>';
  ...
  function html() {
    return css()
      ...
      + '  <div class="al-corner tl"></div>'
      + '  <div class="al-corner tr"></div>'
      + '</div>'

      + '<div class="al-navbar"><nav class="al-nav">'
      + '  <div class="al-brand"><img src="icon-512.png" alt="AROGARA"/><div><div class="bn">AROGARA</div><div class="bt">TECHNOLOGIES</div></div></div>'
      + '  <div class="al-navlinks">'
      +      '<a href="#al-architecture">Architecture</a><a href="#al-modules">Modules</a>'
      +      '<a href="#al-steps">Workflow</a><a href="#al-3d">3D</a><a href="#al-data">Data</a><a href="#al-deliver">Deliverables</a>'
      ...
      + '  <div class="al-docline">PLATFORM <b>AROGARA</b><s></s>TYPE <b>Digital Engineering Design</b><s></s>ORIGIN <b>India</b></div>'
      + '  <h1>AROGARA<br><span class="g">Digital Engineering Design Platform</span></h1>'
      ...
      +      [['01', 'DESIGN', 'Define engineering requirements and operating conditions.'],
             ...
             .map(function (t) {
               return '<div class="al-wfstep"><div class="al-stepn">' + t[0] + '</div>'
      ...
      + '    <div class="al-igrid">' + CAPGRID.map(igItem).join('') + '</div>'
      + '    <div class="al-notecard">'
      + '      <h4>Three things follow from building the model out of the calculated geometry</h4>'
      + '      <ul>'
      + '        <li><b>The take-off is counted, not typed.</b> Every pipe length, elbow, flange, gasket '
      +          'and stud in the bill of material is read off the assembled model, so a bill of material '
      +          'cannot list a component the model does not contain.</li>'
      + '        <li><b>The isometric is a projection of the same model.</b> Not a second drawing kept in '
      +          'step by hand — the same geometry, drawn at ±30°, with the same take-off.</li>'
      + '        <li><b>An elevation is taken through an orthographic lens.</b> A front or top view drawn '
      +          'through a perspective camera is not an elevation, so the platform switches lens rather '
      +          'than simply moving the camera.</li>'
      + '      </ul>'
      + '      <p class="al-fine">Flow, velocity and pressure are shown as a <b>one-dimensional '
      +        'engineering visualisation</b>. They colour the model from the result the calculation '
      +        'already produced — the velocity from V = Q/A, and the pressure falling along the route in '
      +        'proportion to the friction, fitting and elevation terms the hydraulics used. Nothing local '
      +        'is invented and no field is solved. <b>This is not CFD and does not claim to be.</b></p>'
      + '      <p class="al-fine">Before a calculation has run, the geometry is drawn and every '
      +        'engineering overlay reads <b>NOT CALCULATED</b>. No verdict, no figure and no status is '
      +        'shown for a design that has not been computed.</p>'
      + '    </div>'
      + '  </section>'

      + '  <section class="al-sec al-rv" id="al-data">'
      + '    <div class="al-eyebrow">Engineering data</div>'
      + '    <h2 class="al-h2">One design basis. Every table checked against the others.</h2>'
      + '    <p class="al-lead">Water was defined independently in five places in this application, and '
      +      'the materials in three. None of those tables carried a source, a condition or a status, and '
      +      'each module read only its own. They are now indexed rather than replaced — the calculation '
      +      'engines are untouched — and normalised into SI so they can be compared.</p>'
      + '    <div class="al-tri">'
      + '      <div class="al-tric"><h4>PROVENANCE</h4><ul>'
      +        '<li>Value, unit and the module table it came from</li>'
      +        '<li>The condition the property was stated at</li>'
      +        '<li>Where a table states no temperature, the registry says so rather than inventing 25 °C</li>'
      +        '<li>Reference value or software default, marked</li></ul></div>'
      + '      <div class="al-triA">→</div>'
      + '      <div class="al-tric"><h4>CROSS-CHECK</h4><ul>'
      +        '<li>Where two modules define the same property, both are normalised and compared</li>'
      +        '<li>Disagreements are listed worst first, with the ratio between them</li>'
      +        '<li>Agreement is worth knowing; disagreement is worth much more</li>'
      +        '<li>Nothing is silently reconciled — the engineer decides</li></ul></div>'
      + '      <div class="al-triA">→</div>'
      + '      <div class="al-tric"><h4>DESIGN BASIS</h4><ul>'
      +        '<li>Project fluid, material, pressures, temperatures, corrosion allowance, standards</li>'
      +        '<li>Adopt into one module, or across the project, with the change listed line by line</li>'
      +        '<li>A module that already carries a value keeps it and is shown as an override</li>'
      +        '<li>Replacing an engineer\u2019s number silently is worse than asking them to look</li></ul></div>'
      + '    </div>'
      + '  </section>'
      ...
      +        '<li>Shell &amp; tube general arrangement with tube-layout section</li>'
      +        '<li>Nozzle schedule and design data</li><li>Title block and revision</li>'
      ...
      +        '<li>Orthographic elevations</li><li>Component inspection</li>'
      +        '<li>The existing analytical view, kept alongside</li></ul></div>'
      + '      <div class="al-triA">→</div>'
      ...
      + '    <p class="al-lead">Transparent assumptions. Traceable calculations. Every value carries '
      +      'where it came from — entered, calculated, selected, predicted or a software default — and '
      +      'no result is shown before it has been computed.</p>'
      + '    <div class="al-traceline">'
      +      ['INPUT', 'ASSUMPTION', 'CALCULATION', 'RESULT', 'VALIDATION', 'DRAWING', 'REPORT']
             .map(function (x, i) {
               return (i ? '<span class="al-traceseg"></span>' : '')
      ...
      + '    <p>AROGARA brings engineering calculations, design validation, visualization and '
      +      'documentation into one digital engineering environment.</p>'
      + '    <button class="al-btn al-btn-primary" data-launch>Launch AROGARA</button>'
      + '  </div></section>'
      ...
      + '    <div class="al-footbrand"><div class="fb">AROGARA</div>'

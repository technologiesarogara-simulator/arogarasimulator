...
(function () {
  ...
  var ARCHETYPES = {
    ...
    'peristaltic': {
      ...
      flowPath: 'Suction hose → rollers/shoes compress the hose against the casing track, pushing a fluid slug forward → discharge hose',
      fabricationParts: [
        { tag: 'casing-track', label: 'Casing track (lubricated or dry-run)', materialRole: null },
        { tag: 'hose', label: 'Pump hose (elastomer, wear item)', materialRole: null },
        { tag: 'rotor-rollers', label: 'Rotor with rollers or shoes', materialRole: null },
        { tag: 'shaft', label: 'Drive shaft', materialRole: 'shaft' },
        { tag: 'hose-clamps', label: 'Hose end clamps / connectors', materialRole: null },
        { tag: 'suction-hose', label: 'Suction hose connection', materialRole: null },
        { tag: 'discharge-hose', label: 'Discharge hose connection', materialRole: null }
      ]
    },
  ...
  function connectionTypeFor(row) {
    if (!row) return null;
    if (row.id === 'peristaltic-hose') return 'Hose connection (barbed/clamped)';

  ...
  function buildLivePumpPanelData(input) {
    ...
    return {
      ...
      dutyReadout: {
        ...
        /* The real standard motor size the hydraulics engine already
           selected — read through here so the 3D viewer can scale the
           motor sub-assembly from an actual computed value instead of
           guessing or inventing one. */
        motorKw: isFinite(duty.motorKw) ? duty.motorKw : null
      },

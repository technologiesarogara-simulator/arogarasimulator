...
(function () {
  ...
  function update3D(r) {
    ...
    // nozzles: outlet (bottom), inlet (top), overflow with improved sizing
    ...
    /* Let the wheel zoom out far enough to frame the whole tank and back in
       ...
       fit the default view. */
    three.sph.minR = Math.max(0.6, Math.max(R, H) * 0.25);
  ...
  function applyDesignCorrections() {
    ...
    for (var iter = 0; iter < 6; iter++) {
      ...
      if (isFailing(/^H\/D|^Capacity check/)) {
        var capForSizing = r.reqCap > 0 ? r.reqCap : r.geoCap;
        if (capForSizing > 0) {
          var cls = sizeClass(capForSizing);
          var midRatio = (cls.ld[0] + cls.ld[1]) / 2;
          var Dsugg = Math.cbrt((4 * capForSizing) / (Math.PI * midRatio));

...
(function () {
  ...
  var MATERIALS = {
    ...
    'User defined':            { rho: 7850, S: 137, rough: 0.045 }
  };
  ...
  function panelHTML() {
    ...
    h += '<div class="panel panel-input" style="max-height:calc(100vh - 120px);overflow-y:auto;overflow-x:hidden;">'
      ...
      + '<button id="tank-reset" class="tk-hbtn tk-hbtn-red" title="Reset to defaults"><span style="font-size:13px;">⟲</span><span>RESET</span></button></div>'
    ...
    h += two(fld('Design pressure', 'tk-pdes', 'barg', 0, '0.1', 'pressure'),
             fld('Corrosion allowance', 'tk-ca', 'mm', 1.5, '0.5'));
    ...
    h += hdr('4 · GEOMETRY');
    h += fld('Required working capacity', 'tk-reqcap', 'm³', 50, '1');
    h += two(fld('Tank height (shell)', 'tk-H', 'mm', 5000, '10', 'length-mm'),
             fld('Tank dia. ID', 'tk-D', 'mm', 3900, '10', 'length-mm'));
    h += two(fld('Shell thickness (0=auto)', 'tk-t', 'mm', 0, '0.5'),
             fld('Bottom plate thk.', 'tk-tb', 'mm', 6, '0.5'));
    h += fld('Joint efficiency E', 'tk-E', '–', 0.85, '0.05');

    h += hdr('5 · LLLL — LOW LOW LIQUID LEVEL');
    h += two(fld('Min. bottom→nozzle CL (API 650)', 'tk-noz-cl', 'mm', 240, '10'),
             fld('Outlet nozzle dia.', 'tk-noz-out', 'inch', 4, '0.5'));
    h += two(fld('Liquid level above nozzle', 'tk-liq-above', 'mm', 150, '10'),
             fld('Specific requirement height', 'tk-lll-spec', 'mm', 500, '10'));

    h += hdr('6 · LLL — LOW LIQUID LEVEL');
    h += two(fld('Outlet / pump suction flow', 'tk-q-out', 'm³/hr', 20, '1'),
             fld('Residence time', 'tk-res-out', 'min', 3, '0.5'));
    h += fld('LLLL→LLL min. (API 650)', 'tk-lll-min', 'mm', 100, '10');

    h += hdr('7 · HHLL — HIGH HIGH LIQUID LEVEL');
    h += two(fld('Overflow nozzle size', 'tk-noz-ovf', 'inch', 3, '0.5'),
             fld('Project min. from curb angle', 'tk-hhll-spec', 'mm', 100, '10'));

    h += hdr('8 · HLL — HIGH LIQUID LEVEL');
    h += two(fld('Inlet / pump discharge flow', 'tk-q-in', 'm³/hr', 20, '1'),
             fld('Residence time', 'tk-res-in', 'min', 3, '0.5'));
    h += fld('HHLL→HLL min. (API 650)', 'tk-hll-min', 'mm', 100, '10');
  ...
  function compute() {
    ...
    var mat = MATERIALS[matName] || MATERIALS['CS (A36 / IS 2062)'];
    ...
    var tCalc = (4.9 * Dm * Math.max(0, Hm - 0.3) * G) / (mat.S * E) + CA;
    var tMinApi = Dm < 15 ? 5 : Dm < 36 ? 6 : Dm < 60 ? 8 : 10;  // API 650 minimum shell
    var tReq = Math.max(tCalc, tMinApi);
  ...
  function calc() {
    ...
    h += row('Total geometric capacity', f2(r.geoCap) + ' m³');
    h += row('Total working height', f1(r.workH) + ' mm');
    h += row('Total working capacity', f2(r.workCap) + ' m³');
    h += row('Required working capacity', f2(r.reqCap) + ' m³');
    ...
    h += row('BTL', '0 mm');
    h += row('LLLL  (low low — trip)', f1(r.elLLLL) + ' mm');
    h += row('LLL  (low — alarm)', f1(r.elLLL) + ' mm');
    h += row('HLL  (high — alarm)', f1(r.elHLL) + ' mm');
    h += row('HHLL  (high high — trip)', f1(r.elHHLL) + ' mm');
    h += row('Overflow nozzle', f1(r.elOverflow) + ' mm  (300 below TTL)');
    h += row('TTL', f1(r.elTTL) + ' mm');
    h += row('Freeboard above HLL', f1(r.freeboard) + ' mm  (min 300)', r.fbOk ? 'ok' : 'warn');

    h += '<div class="tk-cardh">LEVEL BUILD-UP</div>';
    h += row('LLLL: nozzle CL + r + liquid', f1(r.lllCalc) + ' mm → used ' + f1(r.hLLLL) + ' mm');
    h += row('LLLL→LLL (residence)', f1(r.hResOut) + ' mm → used ' + f1(r.hLLL) + ' mm');
    h += row('HHLL below curb (1.5×ovf)', f1(r.hCurbApi) + ' mm → used ' + f1(r.hHHLLcurb) + ' mm');
    h += row('HHLL→HLL (residence)', f1(r.hResIn) + ' mm → used ' + f1(r.hHLL) + ' mm');
    ...
    h += row('t = 4.9·D·(H−0.3)·G/(S·E) + CA', f2(r.tCalc) + ' mm');
    h += row('API 650 minimum shell', r.tMinApi + ' mm');
    h += row('Required thickness', f2(r.tReq) + ' mm');
    h += row('Selected thickness', f1(r.t) + ' mm', r.tOk ? 'ok' : 'warn');
    if (!r.tOk) h += row('⚠ Thickness check', 'BELOW required — increase', 'warn');

    h += '<div class="tk-cardh">WEIGHT TAKE-OFF (' + esc(r.matName) + ', ρ ' + r.mat.rho + ' kg/m³)</div>';
    h += row('Shell plate', f0(r.wShell) + ' kg');
    h += row('Bottom plate', f0(r.wBottom) + ' kg');
    h += row('Roof plate', f0(r.wRoof) + ' kg');
    h += row('Appurtenances (12 %)', f0(r.wAppurt) + ' kg');
    h += row('Empty (erection) weight', f0(r.wEmpty) + ' kg', 'ok');
    h += row('Operating weight', f0(r.wOper) + ' kg');
    h += row('Hydrotest weight (water full)', f0(r.wTest) + ' kg');
    ...
    h += row('Freeboard ≥ 300 mm', r.fbOk ? 'PASS' : 'FAIL', r.fbOk ? 'ok' : 'warn');
    h += row('Diameter ≤ 60 m (API 650)', r.dOk ? 'PASS' : 'REVIEW', r.dOk ? 'ok' : 'warn');
    h += row('Height ≤ 25 m (API 650)', r.hOk ? 'PASS' : 'REVIEW', r.hOk ? 'ok' : 'warn');
    h += row('Roof suited to ' + esc(r.fluid), r.roofSuggest, '');

    $('tk-results').innerHTML = h;
    var tag = $('tk-3dtag');
    if (tag) tag.textContent = f1(r.Dm) + ' m Ø × ' + f1(r.Hm) + ' m · ' + r.roof + ' · ' + f2(r.workCap) + ' m³ working';
    update3D(r);
    updateMatInfo();
    updateRoofHint(r);
  }
  ...
  function updateMatInfo() {
    ...
    var m = MATERIALS[val('tk-mat', 'CS (A36 / IS 2062)')];
    if (m) el.textContent = 'ρ ' + m.rho + ' kg/m³ · allowable stress S ' + m.S + ' MPa · roughness ' + m.rough + ' mm';
  }
  function updateRoofHint(r) {
    ...
    el.innerHTML = '🛢 <b>Recommended roof for ' + esc(val('tk-fluid', 'Water')) + ':</b> ' + esc(sug)
      + ' · API 650 limits: Ø ≤ 60 m, height ≤ 25 m, freeboard ≥ 300 mm (agitated ≥ 750 mm).';
  }
  ...
  var INPUT_IDS = ['tk-tag', 'tk-loc', 'tk-project', 'tk-client', 'tk-engineer', 'tk-rev', 'tk-fluid', 'tk-rho', 'tk-tdes',
    'tk-pdes', 'tk-ca', 'tk-mat', 'tk-orient', 'tk-roof', 'tk-service', 'tk-reqcap', 'tk-H', 'tk-D', 'tk-t', 'tk-tb', 'tk-E',
  ...
  function wire() {
    ...
    var cb = $('tk-calc'); if (cb) cb.addEventListener('click', function () { calc(); runStatus(); });
    var rb = $('tk-report'); if (rb) rb.addEventListener('click', report);
    var db = $('tk-draw'); if (db) db.addEventListener('click', drawing);
    var rs = $('tk-reset'); if (rs) rs.addEventListener('click', function () { if (DEFAULTS) restore(DEFAULTS); });
    function setView(v) {
      tkView = v;
  ...
  function runStatus() {
    ...
    el.innerHTML = (r.capOk && r.tOk ? '✓ DESIGN COMPLETE' : '⚠ REVIEW REQUIRED')
      ...
      + r.t.toFixed(1) + ' mm · ' + Math.round(r.wEmpty).toLocaleString() + ' kg empty';
  }
  ...
  function bomRows(r) {
    ...
    return [
      ['Shell plate', r.matName, 1, 'API 650 · t ' + r.t.toFixed(1) + ' mm', f0(r.wShell), 'Ø' + Math.round(r.D) + ' × ' + Math.round(r.H) + ' mm'],
      ['Bottom plate', r.matName, 1, 'API 650 · t ' + r.tb.toFixed(1) + ' mm', f0(r.wBottom), '≥6 mm + CA'],
      ['Roof plate (' + r.roof + ')', r.matName, 1, 't ' + r.tRoof.toFixed(1) + ' mm', f0(r.wRoof), r.roofRise ? 'rise ' + (r.roofRise * 1000).toFixed(0) + ' mm' : 'flat / floating'],
      ...
      ['Overflow nozzle', r.matName, 1, 'ASME B16.5 · ' + r.dOvf + '"', f0(r.dOvf * 4), '300 mm below TTL'],
  ...
  function drawing() {
    ...
    lvls.forEach(function (L) {
      ...
      s += txt(tx + tw2 + 90, ly + 3, L[0] + '  ' + Math.round(L[1]) + ' mm', 8, L[2], 'start', '700');
    });
    // overflow nozzle
    ...
    // dims
    s += dim(tx - 46, ty, tx - 46, ty + th, Math.round(r.H) + ' mm');
    s += dim(tx, ty + th + 46, tx + tw2, ty + th + 46, 'Ø ' + Math.round(r.D) + ' mm ID');
    s += txt(tx + tw2 / 2, ty + th + 74, r.roof + ' · ' + esc(r.matName) + ' · shell t ' + r.t.toFixed(1) + ' mm', 8.5, '#64748b', 'middle');
    ...
    s += table(dx, dy + 10, [190, 190], [
      ...
      ['Design temperature', r.tdes.toFixed(0) + ' °C'],
      ['Design pressure', r.pdes.toFixed(2) + ' barg'],
      ['Corrosion allowance', r.CA.toFixed(1) + ' mm'],
      ['Tank ID × height', Math.round(r.D) + ' × ' + Math.round(r.H) + ' mm'],
      ['L/D ratio', r.LD.toFixed(2) + '  (' + r.svc + ')'],
      ['Geometric capacity', r.geoCap.toFixed(2) + ' m³'],
      ['Working capacity', r.workCap.toFixed(2) + ' m³'],
      ['Required capacity', r.reqCap.toFixed(2) + ' m³'],
      ['Capacity check', r.capOk ? 'PASS' : 'FAIL'],
      ['Shell thickness (API 650)', r.t.toFixed(1) + ' mm  (req ' + r.tReq.toFixed(2) + ')'],
      ['Bottom plate', r.tb.toFixed(1) + ' mm'],
    ...
    s += table(lx, ly + 10, [120, 120, 140], [
      ...
      ['HLL (alarm)', Math.round(r.elHLL), 'residence ' + r.hHLL.toFixed(0) + ' mm'],
      ['LLL (alarm)', Math.round(r.elLLL), 'residence ' + r.hLLL.toFixed(0) + ' mm'],
  ...
  function report() {
    ...
    b += '<div style="text-align:center;margin-bottom:14px;"><div style="font-size:18px;font-weight:800;color:#ea580c;">BHARAT FLOWSIZE — STORAGE TANK DESIGN REPORT</div><div style="font-size:10px;color:#64748b;">AROGARA TECHNOLOGIES | API 650 DESIGN BASIS</div></div>';
    ...
    b += T([['Service fluid', esc(r.fluid)], ['Density / SG', r.rho.toFixed(0) + ' kg/m³  /  ' + f2(r.G)],
      ['Design temperature', f1(r.tdes) + ' °C'], ['Design pressure', f2(r.pdes) + ' barg'],
      ['Material', esc(r.matName) + '  (ρ ' + r.mat.rho + ' kg/m³, S ' + r.mat.S + ' MPa)'],
      ['Corrosion allowance', f1(r.CA) + ' mm'], ['Orientation / roof', r.orient + ' / ' + r.roof],
      ['Recommended roof for service', r.roofSuggest]]);
    b += sec('3 · GEOMETRY & CAPACITY');
    b += T([['Tank ID × shell height', Math.round(r.D) + ' × ' + Math.round(r.H) + ' mm'],
      ['L/D ratio', f2(r.LD) + '  (' + r.svc + ' band ' + r.band[0] + '–' + r.band[1] + ')'],
      ['Total geometric capacity  π/4·D²·H', f2(r.geoCap) + ' m³'],
      ['Total working height', f1(r.workH) + ' mm'],
      ['Total working capacity', f2(r.workCap) + ' m³'],
      ['Required working capacity', f2(r.reqCap) + ' m³'],
      ['Capacity check', r.capOk ? 'PASS — capacity adequate' : 'FAIL — increase tank size']]);
    b += sec('4 · LIQUID LEVELS');
    b += T([['LLLL — nozzle CL + radius + liquid', f1(r.lllCalc) + ' mm  → adopted ' + f1(r.hLLLL) + ' mm'],
      ['LLLL → LLL (residence ' + f1(num('tk-res-out', 3)) + ' min)', f1(r.hResOut) + ' mm  → adopted ' + f1(r.hLLL) + ' mm'],
      ['HHLL below curb (1.5 × ' + r.dOvf + '")', f1(r.hCurbApi) + ' mm  → adopted ' + f1(r.hHHLLcurb) + ' mm'],
      ['HHLL → HLL (residence ' + f1(num('tk-res-in', 3)) + ' min)', f1(r.hResIn) + ' mm  → adopted ' + f1(r.hHLL) + ' mm'],
      ['Elevations (BTL/LLLL/LLL/HLL/HHLL/TTL)', '0 / ' + f1(r.elLLLL) + ' / ' + f1(r.elLLL) + ' / ' + f1(r.elHLL) + ' / ' + f1(r.elHHLL) + ' / ' + f1(r.elTTL) + ' mm'],
    ...
    b += T([['Formula', 't = 4.9·D·(H−0.3)·G / (S·E) + CA'],
      ...
      ['Calculated thickness', f2(r.tCalc) + ' mm'], ['API 650 minimum', r.tMinApi + ' mm'],
      ['Required thickness', f2(r.tReq) + ' mm'], ['Selected thickness', f1(r.t) + ' mm  (' + (r.tOk ? 'PASS' : 'FAIL') + ')'],
      ['Bottom plate', f1(r.tb) + ' mm  (≥6 mm + CA)'], ['Roof plate', f1(r.tRoof) + ' mm']]);
    b += sec('6 · WEIGHT TAKE-OFF');
    b += T([['Shell plate', f0(r.wShell) + ' kg'], ['Bottom plate', f0(r.wBottom) + ' kg'], ['Roof plate', f0(r.wRoof) + ' kg'],
      ['Appurtenances (12 %)', f0(r.wAppurt) + ' kg'], ['Empty (erection) weight', f0(r.wEmpty) + ' kg'],
      ['Liquid at working level', f0(r.wLiquid) + ' kg'], ['Operating weight', f0(r.wOper) + ' kg'],
      ['Hydrotest weight (water full)', f0(r.wTest) + ' kg']]);

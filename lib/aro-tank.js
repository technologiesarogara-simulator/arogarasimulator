...
(function () {
  ...
  /* Typical density (SI, kg/m³) by service fluid, for the Density field's
     auto-fill — the same "pick a fluid, get its properties" pattern the
     line-sizing modules use. Fluids too variable to guess safely (Food,
     Chemical, Other) are left null: the field is cleared instead of quietly
     filling in a number that could be badly wrong for the actual product. */
  var FLUID_PROPS = {
    'Water': { rho: 1000 }, 'Diesel': { rho: 840 }, 'Crude Oil': { rho: 870 },
    'LPG': { rho: 540 }, 'Food': { rho: null }, 'Chemical': { rho: null }, 'Other': { rho: null }
  };
  ...
  function panelHTML() {
    ...
    h += two(fld('Overflow nozzle size', 'tk-noz-ovf', 'inch', 3, '0.5'),
             fld('Project min. from curb angle', 'tk-hhll-spec', 'mm', 150, '10', 'length-mm'));
  ...
  function calc() {
    ...
    designAssistant(r).forEach(function (c) {

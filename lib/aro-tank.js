...
(function () {
  ...
  var PIPE_NPS = {
    0.5: { od: 0.840, mm: 21.3, label: '1/2"' },
    0.75: { od: 1.050, mm: 26.7, label: '3/4"' },
    1: { od: 1.315, mm: 33.4, label: '1"' },
    1.5: { od: 1.900, mm: 48.3, label: '1.5"' },
    2: { od: 2.375, mm: 60.3, label: '2"' },
    3: { od: 3.500, mm: 88.9, label: '3"' },
    4: { od: 4.500, mm: 114.3, label: '4"' },
    6: { od: 6.625, mm: 168.3, label: '6"' },
    8: { od: 8.625, mm: 219.1, label: '8"' },
    10: { od: 10.750, mm: 273.1, label: '10"' },
    12: { od: 12.750, mm: 323.9, label: '12"' },
    14: { od: 14.000, mm: 355.6, label: '14"' },
    16: { od: 16.000, mm: 406.4, label: '16"' },
    18: { od: 18.000, mm: 457.2, label: '18"' },
    20: { od: 20.000, mm: 508.0, label: '20"' },
    24: { od: 24.000, mm: 609.6, label: '24"' }
  };
  ...
  /* Real-world liquid tint for the 3D view — approximate industry colour so
     the engineer can tell at a glance which fluid is in the tank, the same
     way the pump-hydraulics 3D does. Not a physical property; purely visual. */
  var FLUID_COLOR = {
    'Water': 0x2e86de, 'Diesel': 0x8a5a28, 'Crude Oil': 0x1f150a, 'LPG': 0xd6e9b0,
    'Methanol': 0xcfe8ff, 'Ethanol': 0xe8f3ff, 'Benzene': 0xf2e6a8, 'Toluene': 0xf0e6c0,
    'Glycerol': 0xf5f0d8, 'Acetic Acid': 0xeef0e0, 'Acetone': 0xe4f4ff, 'Butanol': 0xf0ecd8,
    'Chloroform': 0xd8e8e8, 'Ether (Diethyl)': 0xe8f8ff, 'Hexane': 0xf0f4d8,
    'Kerosene': 0xe8c878, 'Linseed Oil': 0xc8952a, 'Olive Oil': 0x8a9a2e, 'Phenol': 0xf0d8c0,
    'Sulfuric Acid': 0xb8b8a8, 'Hydrochloric Acid': 0xd0e0d8, 'Nitric Acid': 0xe8e888,
    'Sodium Hydroxide': 0xd8e8f0, 'Ammonia': 0xcfe8ff, 'Isopropanol': 0xe8f4ff,
    'Food': 0xf0e0b0, 'Chemical': 0x8bb4de, 'Other': 0x2563eb
  };
  function fluidColor(name) { return (FLUID_COLOR[name] !== undefined) ? FLUID_COLOR[name] : 0x2563eb; }
  /* Material of Construction (MOC) Recommendation Engine
     Based on fluid properties and temperature per industry standards */
  function recommendMOC(fluid, tempC) {
    var corr = FLUID_CORROSIVITY[fluid] || 'unknown';
    var props = FLUID_PROPS[fluid] || {};
    tempC = tempC || 20;

    /* Decision tree: (1) corrosivity, (2) temperature, (3) chemical compatibility */
    if (/Ammonia|LPG|Cryogenic/.test(fluid)) {
      return 'Stainless Steel (SS 304/316) — cryogenic service requires low-temp alloy';
    }
    if (corr === 'highly-corrosive' || corr === 'caustic') {
      return tempC > 60 ? 'Stainless Steel (SS 316L) — high-temp corrosive service'
           : 'Stainless Steel (SS 304) or Rubber-lined CS — acid/alkali resistant';
    }
    if (/Sulfuric|Hydrochloric|Nitric|Acetic/.test(fluid)) {
      return 'Stainless Steel (SS 316L) — acid service requires duplex or lined tank';
    }
    if (corr === 'corrosive') {
      return tempC > 70 ? 'Stainless Steel (SS 304) or Epoxy-lined CS'
           : 'Carbon Steel (CS A36/IS 2062) with internal epoxy/rubber lining';
    }
    if (tempC > 70 && corr === 'mild') {
      return 'Stainless Steel (SS 304) — elevated temp + mild corrosivity requires SS';
    }
    if (tempC > 100) {
      return 'Carbon Steel (CS A36 / IS 2062) with heat treatment — high-temp service';
    }
    /* Default for benign fluids at normal temperature */
    return 'Carbon Steel (CS A36 / IS 2062) — standard storage tank material';
  }
  ...
  /* ── ARODATA adapter — stabilization plan §05 ───────────────────────────
     ...
     migration. Only density feeds this module's own calculation (API 650
     shell sizing does not use viscosity/surface tension/conductivity, so
     they are never routed through here even though FLUID_PROPS carries
     them) — everything ARODATA currently holds was migrated wholesale
     from other modules' tables with no condition recorded and is
     correctly marked not usable, so this falls through to FLUID_PROPS for
     every fluid today. */
  window.AROFLUIDLOG = window.AROFLUIDLOG || [];
  /* lastStatus is a side channel, not a return-shape change: the one call
     site reads it immediately after calling, and a function returning a
     bare number (used directly in arithmetic elsewhere in this file) is
     safer left alone than turned into an object every caller would need
     updating for. */
  function resolveTankFluidRho(name, legacyRho) {
    resolveTankFluidRho.lastStatus = null;
    if (legacyRho == null || !window.ARODATA || !window.ARODATA.resolve || !name) return legacyRho;
    var sid = 'fluid:' + String(name).toLowerCase().replace(/[^a-z0-9]+/g, ' ').trim();
    try {
      var r = window.ARODATA.resolve(sid, 'density');
      if (r && r.usableInCalc && r.effective && isFinite(r.effective.si)) { resolveTankFluidRho.lastStatus = r.status; return r.effective.si; }
      if (r && r.masterPick) {
        window.AROFLUIDLOG.push({ module: 'tank', fluid: name, property: 'density',
          legacy: legacyRho, arodata: r.masterPick.si, status: r.status, at: Date.now() });
  ...
  function panelHTML() {
    ...
    h += '<div class="panel panel-input" style="max-height:calc(100vh - 120px);overflow-y:auto;overflow-x:hidden;">'
      ...
      + '<button id="tk-reset" class="tk-hbtn tk-hbtn-red" title="Reset to defaults"><span style="font-size:13px;">↺</span><span>RESET</span></button>'
      + '<button id="tk-selftest" class="tk-hbtn" title="Run the internal design self-test across a spread of tank sizes — your current sheet is untouched" style="border-color:#38bdf8;color:#38bdf8;"><span style="font-size:13px;">🧪</span><span>TEST</span></button></div>'
    ...
    h += '<label style="display:block;margin:4px 0;font-family:var(--font-mono);font-size:10px;color:var(--text-muted);">OUTLET NOZZLE (STANDARDIZED)<span style="display:flex;align-items:center;gap:4px;margin-top:2px;">'
      ...
      + Object.keys(PIPE_NPS).sort(function(a,b){return parseFloat(a)-parseFloat(b);}).map(function(k){var p=PIPE_NPS[k];return '<option value="'+k+'">'+p.label+' ('+p.od.toFixed(2)+'" / '+p.mm.toFixed(0)+' mm)</option>';}).join('')
    ...
    h += '<div style="display:flex;gap:8px;margin-top:14px;">'
      + '<button id="tk-calc" style="flex:1;background:linear-gradient(135deg,#16a34a,#22c55e);border:none;color:#fff;font-family:var(--font-mono);font-size:12px;font-weight:800;letter-spacing:0.06em;padding:11px;border-radius:5px;cursor:pointer;">▶ RUN</button>'
      + '<button id="tk-autodesign" style="flex:1;background:linear-gradient(135deg,#ea580c,#f97316);border:none;color:#fff;font-family:var(--font-mono);font-size:12px;font-weight:800;letter-spacing:0.06em;padding:11px;border-radius:5px;cursor:pointer;">⚡ AUTO-DESIGN</button>'
      + '</div>';
    ...
    h += '<div class="panel" style="max-height:calc(100vh - 120px);overflow-y:auto;">'
      ...
      + '<div style="font-family:var(--font-mono);font-size:9.5px;color:#94a3b8;text-align:center;margin-bottom:4px;">3D TANK — LIVE VIEW · DRAG TO ROTATE · CTRL/&#8984;+SCROLL TO ZOOM</div>'
      + '<div id="tk-3dwrap" style="position:relative;width:100%;height:480px;background:#0b1220;border:1px solid var(--border-muted);border-radius:6px;overflow:hidden;">'
  ...
  function designAssistant(r) {
    ...
    /* Freeboard check — TTL down to HHLL is exactly hHHLLcurb (H − elHHLL) */
    var freeboardOk = r.hHHLLcurb >= 300;
    out.push({
      label: 'Freeboard above HHLL (API 650 min ≥ 0.3 m)',
      ok: freeboardOk, value: U(r.hHHLLcurb, 'length-mm', 0),
      note: freeboardOk ? 'adequate freeboard' : 'below minimum — increase the overflow nozzle size or the curb clearance spec',
      priority: !freeboardOk ? 'critical' : undefined
    });
    /* API 650 atmospheric-shell envelope — dOk/hOk were already computed in
       compute() and printed in the report table, but never actually checked
       here, so a design that solved out to (say) a 69 m tall shell read as
       clean on this panel. A single atmospheric tank outside this envelope
       is not a "review it" item — it does not exist as a buildable design. */
    out.push({
      label: 'Diameter — API 650 atmospheric-tank envelope (≤ 60 m)',
      ok: r.dOk, value: U(r.Dm, 'length-m', 2),
      note: r.dOk ? 'within the standard atmospheric-tank envelope' : 'exceeds the practical single-shell diameter — this capacity needs more than one tank',
      priority: !r.dOk ? 'critical' : undefined
    });
    out.push({
      label: 'Shell height — API 650 atmospheric-tank envelope (≤ 25 m)',
      ok: r.hOk, value: U(r.Hm, 'length-m', 2),
      note: r.hOk ? 'within the standard atmospheric-tank envelope' : 'exceeds the practical single-shell height — go wider (lower L/D) rather than taller',
      priority: !r.hOk ? 'critical' : undefined
    });
    return out;
  }
  ...
  function calc() {
    ...
    /* Mutually exclusive buckets — PASS + REVIEW + CRITICAL must equal the
       total check count, otherwise the summary cards lie about what's on
       screen. CRITICAL is reserved for the checks designAssistant() tagged
       priority:'critical' (capacity, freeboard — the design physically
       won't hold); everything else that fails is advisory REVIEW. */
    var passes = checks.filter(function(c) { return c.ok; });
    var critical = checks.filter(function(c) { return !c.ok && c.priority === 'critical'; });
    var review = checks.filter(function(c) { return !c.ok && c.priority !== 'critical'; });
    var anyFailing = critical.length + review.length;

    /* The same buckets feed the design status bar and the design report, so
       the header count and this panel can never disagree. A check the tank
       tagged 'critical' is a failure; everything else that fails is a
       judgement call and is carried as a warning rather than a red mark. */
    if (window.AROENG) {
      try {
        window.AROENG.publish('tank', {
          checks: checks.map(function (c, i) {
            return { key: c.key || ('tk' + i), label: c.label,
                     clause: c.priority === 'critical' ? 'API 650 / duty requirement' : 'Field practice (advisory)',
                     detail: c.value + (c.note ? ' — ' + c.note : ''),
                     status: c.ok ? 'pass' : (c.priority === 'critical' ? 'fail' : 'warn') };
          }),
          values: r
        });
      } catch (e) {}
    ...
    h += '<div style="background:linear-gradient(135deg,rgba(15,23,42,0.6),rgba(30,41,59,0.6));border:1px solid var(--border-muted);border-radius:6px;padding:12px;margin-bottom:12px;">'
      ...
      + '<div style="background:rgba(251,191,36,0.1);border:1px solid #fbbf24;border-radius:4px;padding:8px;text-align:center;"><div style="font-size:18px;font-weight:800;color:#fbbf24;">' + review.length + '</div><div style="font-size:8px;color:#fcd34d;">REVIEW</div></div>'
    ...
    if (review.length > 0) {
      h += '<div style="background:rgba(251,191,36,0.08);border-left:3px solid #fbbf24;padding:8px;margin-bottom:10px;border-radius:3px;">'
        + '<div style="font-family:var(--font-mono);font-size:9px;font-weight:700;color:#fcd34d;margin-bottom:6px;">⚠ REVIEW — FIELD-PRACTICE ITEMS WORTH A SECOND LOOK</div>';
      review.forEach(function(c) {
        h += '<div style="font-family:var(--font-mono);font-size:8.5px;color:#cbd5e1;margin:4px 0;line-height:1.4;">'
          + '<span style="color:#fbbf24;font-weight:600;">!</span> <b>' + c.label + '</b> [' + c.value + ']<br/>'
          + '<span style="color:#94a3b8;margin-left:16px;">→ ' + c.note + '</span></div>';
      });
      h += '</div>';
    }
    ...
    if (anyFailing > 0) {
      h += '<button id="tk-apply-fixes" style="display:block;width:100%;margin-bottom:10px;background:linear-gradient(135deg,#16a34a,#22c55e);border:none;color:#fff;font-family:var(--font-mono);font-size:10px;font-weight:800;letter-spacing:0.04em;padding:9px;border-radius:4px;cursor:pointer;">⚡ APPLY DESIGN CORRECTIONS — RESIZE / RE-RATE TO CLEAR ALL ' + anyFailing + ' OPEN ITEM' + (anyFailing > 1 ? 'S' : '') + '</button>';
    }
    ...
    var tag = $('tk-3dtag');
    if (tag) tag.textContent = r.fluid + ' · ' + U(r.Dm, 'length-m', 1) + ' Ø × ' + U(r.Hm, 'length-m', 1) + ' · ' + r.roof + ' · ' + U(r.workCap, 'volume', 2) + ' working';
    update3D(r);
    updateMatInfo();
    updateMatHint(r);
    updateRoofHint(r);
  ...
  function init3D() {
    ...
    /* A single directional light leaves the far side of the shell lit by
       the hemisphere light alone — for a near-metallic shell material
       (metalness 0.7-0.85 for most of the material choices below) that
       side reads as flat black, since a metal's diffuse response to soft
       ambient light is small; only the specular highlight from a direct
       light actually shows its colour. The industrial 3D viewport avoids
       this with a second, dimmer fill light from the opposite side —
       added here for the same reason, so the tank is never more than a
       silhouette from any angle. */
    var fillDir = new THREE.DirectionalLight(0xa8c0d8, 0.4); fillDir.position.set(-16, 10, -18); scene.add(fillDir);
    var grid = new THREE.GridHelper(140, 28, 0x224, 0x1a2740); scene.add(grid);
    var group = new THREE.Group(); scene.add(group);
    var sph = { r: 46, theta: 0.9, phi: 1.15, tx: 0, ty: 6, tz: 0, minR: 1, maxR: 400 };
    function place() {
    ...
    three = { scene: scene, cam: cam, rn: rn, group: group, grid: grid, sph: sph, place: place, canvas: canvas };
    ...
    try { if (window.ARO3DI && window.ARO3DI.registerLegacy)
      window.ARO3DI.registerLegacy('tank', function () { return three; }); } catch (e) {}
    place();
    var down = null;
    ...
    /* Zoom range scales with the current tank (set in update3D via sph.minR/
       maxR) so a 1 m tank can be zoomed in close and a 60 m tank can still be
       zoomed out far enough to see the whole thing — a fixed 10–240 range
       either stranded small tanks tiny-in-frame or clipped through big ones. */
    /* Ctrl/Cmd+scroll to zoom, same opt-in gesture every other 3D canvas in
       ...
       instead of getting captured by this preview. */
    canvas.addEventListener('wheel', function (e) {
      if (!(e.ctrlKey || e.metaKey)) return;
      e.preventDefault();
      sph.r = Math.max(sph.minR, Math.min(sph.maxR, sph.r * (e.deltaY < 0 ? 0.9 : 1.1))); place();
    }, { passive: false });
    /* Drawn only when the tank is on screen. This scene is static between
       calculations, so redrawing it from a background tab bought nothing and
       cost a frame's worth of GPU on every other module's animation. */
    (function loop() {
  ...
  /* Get formatted nozzle schedule string (NPS + Schedule + OD inch & mm) or fallback to diameter */
  function getNozzleSpec(npsFieldId, diamFieldId, defaultD) {
    var npsField = $(npsFieldId);
    var schField = npsFieldId === 'tk-noz-nps' ? $('tk-noz-sch') : $('tk-ovf-sch');

    if (npsField && npsField.value && PIPE_NPS[npsField.value]) {
      var npsData = PIPE_NPS[npsField.value];
      var npsLabel = npsData.label;                      // already carries its own trailing "
      var od = npsData.od;
      var mm = npsData.mm || (od * 25.4);
      var sch = schField ? schField.value : '—';
      return npsLabel + ' NPS SCH ' + sch + ' (OD ' + od.toFixed(2) + '" / ' + mm.toFixed(1) + ' mm)';
    }
    return (num(diamFieldId, defaultD) || defaultD).toFixed(2) + '"';
  }

  /* Text sprite for a level ring or nozzle callout — the industrial 3D tank
     already labels every level and nozzle it draws; the analytical tank drew
     the same rings and stubs with no text at all, so nothing on it said
     which ring was HHLL vs LLL or what a nozzle's size was without leaving
     this view. sizeAttenuation is off so the label stays legible whether
     the tank is a 1 m test vessel or a 60 m API 650 tank. */
  function tankLabel(text, colour) {
    var cv = document.createElement('canvas'), cx = cv.getContext('2d');
    var font = '700 30px ui-monospace, "IBM Plex Mono", monospace';
    cx.font = font;
    var pad = 10;
    ...
    cx = cv.getContext('2d');
    cx.font = font;
    cx.fillStyle = 'rgba(8,13,18,0.86)';
    cx.fillRect(0, 0, cv.width, cv.height);
    cx.strokeStyle = 'rgba(148,163,184,0.5)'; cx.lineWidth = 2; cx.strokeRect(1, 1, cv.width - 2, cv.height - 2);
    cx.fillStyle = colour || '#e2e8f0';
    cx.textBaseline = 'middle';
    cx.fillText(text, pad, cv.height / 2 + 1);
    var tex = new THREE.CanvasTexture(cv); tex.needsUpdate = true;
    var sp = new THREE.Sprite(new THREE.SpriteMaterial({ map: tex, transparent: true, sizeAttenuation: false, depthTest: false }));
    var h = 0.045;
  ...
  function update3D(r) {
    ...
    // roof by selection — improved broach/cap rendering with cone transition
    if (/Cone/.test(r.roof)) {
      var coneRise = Math.max(0.25, r.roofRise);
      // cone transition zone (truncated cone from shell to apex)
      var transZone = Math.min(0.15, R * 0.3);
      var coneGeo = new THREE.ConeGeometry(R * 1.02, coneRise + transZone, 56, 2, true);
      var cone = new THREE.Mesh(coneGeo, shellMat);
      cone.position.y = H + (coneRise + transZone) / 2; g.add(cone);
    } else if (/Dome/.test(r.roof)) {
      // improved dome with broach/cap transition: cone junction + spherical cap
      var broachH = Math.min(0.2, R * 0.25);
      var broach = new THREE.Mesh(new THREE.ConeGeometry(R * 1.02, broachH, 56, 1, true), shellMat);
      broach.position.y = H + broachH / 2; g.add(broach);

      // spherical cap above broach — proper proportions for realistic appearance
      var capR = R * 0.95;
      var capMesh = new THREE.Mesh(new THREE.SphereGeometry(capR, 48, 24, 0, Math.PI * 2, 0, Math.PI / 2.2), shellMat);
      capMesh.position.y = H + broachH + capR * 0.35; g.add(capMesh);
    ...
    // liquid + level planes (cut-away) — tinted by the selected service fluid,
    // the same real-world-colour idea used in the pump-hydraulics 3D, so the
    // engineer can tell what's inside at a glance and see it change live.
    var liqH = Math.max(0.05, r.elHLL / 1000);
    var liqCol = fluidColor(r.fluid);
    var liq = new THREE.Mesh(new THREE.CylinderGeometry(R * 0.985, R * 0.985, liqH, 48),
      new THREE.MeshStandardMaterial({ color: liqCol, transparent: true, opacity: cut ? 0.6 : 0.32, roughness: 0.2, metalness: 0.05 }));
    liq.position.y = liqH / 2; g.add(liq);

    /* Level rings + callouts used to be cut-away only, so the shell had to be
       switched to X-ray mode before anything said which level was which —
       this is exactly the "user should know all the level code" the
       external view was missing. Drawn just outside the shell (R*1.02, not
       the old R*0.99 which sat under an opaque shell in external view and
       was invisible), they now show in both view modes, matching how the
       industrial 3D tank always shows its level callouts with no toggle. */
    var lv = [['LLLL', r.elLLLL, 0x8b5cf6], ['LLL', r.elLLL, 0x22c55e], ['HLL', r.elHLL, 0xf59e0b], ['HHLL', r.elHHLL, 0xef4444]];
    lv.forEach(function (L) {
      var name = L[0], m2 = L[1] / 1000, colourHex = L[2];
      if (!isFinite(m2) || m2 <= 0 || m2 > H) return;
      var ring = new THREE.Mesh(new THREE.TorusGeometry(R * 1.02, 0.035, 6, 56), new THREE.MeshBasicMaterial({ color: colourHex }));
      ring.rotation.x = Math.PI / 2; ring.position.y = m2; g.add(ring);
      var colourCss = '#' + colourHex.toString(16).padStart(6, '0');
      var lab = tankLabel(name + '  ' + m2.toFixed(3) + ' m', colourCss);
      lab.position.set(-(R * 1.35), m2, 0);
      g.add(lab);
    });

    /* Resolve nozzle diameters from NPS selections or legacy fields
       Priority: NPS selector value (from PIPE_NPS) > legacy diameter field */
    function getNozzleDiameter(npsFieldId, diamFieldId, defaultD) {
      var npsField = $(npsFieldId);
      if (npsField && npsField.value && PIPE_NPS[npsField.value]) {
        return PIPE_NPS[npsField.value].od * 0.0254 / 2;  // convert inch OD → m radius
      }
      return (num(diamFieldId, defaultD) || defaultD) * 0.0254 / 2;
    }

    // nozzles: outlet (bottom), inlet (top), overflow — rendered as real hollow
    // pipe stubs (outer wall + a dark bore + an open-end ring showing wall
    // thickness) instead of a solid rod, so they read as pipe, not bar stock.
    var nz = new THREE.MeshStandardMaterial({ color: 0xef4444, metalness: 0.7, roughness: 0.4, side: THREE.DoubleSide });
    var nzb = new THREE.MeshStandardMaterial({ color: 0x2563eb, metalness: 0.7, roughness: 0.4, side: THREE.DoubleSide });
    var boreMat = new THREE.MeshStandardMaterial({ color: 0x0a0a0a, metalness: 0.1, roughness: 0.95, side: THREE.DoubleSide });
    /* label optional — flange location and size, the same "which nozzle is
       this" gap the level rings above had: a bare coloured stub said
       nothing about what it was or what size it carried without leaving
       this view to check the input form. */
    function nozzle(y, mtl, radOD, label) {
      var len = R * 0.5, radID = radOD * 0.82, cx = R + R * 0.22;
      var outer = new THREE.Mesh(new THREE.CylinderGeometry(radOD, radOD, len, 20, 1, true), mtl);
      outer.rotation.z = Math.PI / 2; outer.position.set(cx, y, 0); g.add(outer);
      var bore = new THREE.Mesh(new THREE.CylinderGeometry(radID, radID, len * 1.01, 20, 1, true), boreMat);
      bore.rotation.z = Math.PI / 2; bore.position.set(cx, y, 0); g.add(bore);
      // open-end ring — the flange face, showing the pipe wall thickness at a glance
      var ring = new THREE.Mesh(new THREE.RingGeometry(radID, radOD, 20), mtl);
      ring.rotation.y = Math.PI / 2; ring.position.set(cx + len / 2, y, 0); g.add(ring);
      if (label) {
        var lab = tankLabel(label, '#e2e8f0');
        lab.position.set(cx + len / 2 + 0.18, y, 0);
        g.add(lab);
      }
    }

    var radOut = Math.max(0.06, getNozzleDiameter('tk-noz-nps', 'tk-noz-out', 4));
    var radOvf = Math.max(0.05, getNozzleDiameter('tk-ovf-nps', 'tk-noz-ovf', 3));
    var outSpec = getNozzleSpec('tk-noz-nps', 'tk-noz-out', 4);
    var ovfSpec = getNozzleSpec('tk-ovf-nps', 'tk-noz-ovf', 3);

    nozzle(Math.max(0.15, r.elLLLL / 1000 * 0.4), nzb, radOut, 'OUTLET  ' + outSpec);
    nozzle(H * 0.92, nz, radOut, 'INLET  ' + outSpec);
    nozzle(r.elOverflow / 1000, nz, radOvf, 'OVERFLOW  ' + ovfSpec);
    ...
    /* Ground grid scales to the tank instead of a fixed 140-unit plane — a
       1 m tank on a grid sized for a 60 m tank reads as a speck no matter how
       close the camera sits, so the cell size tracks R and the plane always
       shows roughly the same number of squares around the tank. */
    var cell = Math.max(0.3, Math.min(10, R * 0.9));
    var gridSize = cell * 24, gridDiv = 24;
    if (three.grid) { three.scene.remove(three.grid); three.grid.geometry.dispose(); three.grid.material.dispose(); }
    var newGrid = new THREE.GridHelper(gridSize, gridDiv, 0x223045, 0x1a2740);
    three.scene.add(newGrid); three.grid = newGrid;

    three.sph.tx = 0; three.sph.ty = H * 0.45;
    three.sph.r = Math.max(4, Math.max(R * 2.3, H * 2.0) + 2.2);
    /* Let the wheel zoom out far enough to frame the whole tank and back in
       close enough to inspect a small one, scaled off the same R/H used to
       fit the default view. minR is bounded to R·1.6 (not a small fixed
       constant) — the orbit target sits ON the tank's own axis, so any
       camera distance smaller than the shell radius R can end up INSIDE the
       cylindrical shell at a near-horizontal viewing angle, showing only
       whatever's close (a nozzle stub) with the shell wall behind the
       camera — exactly the "zoomed into a solid red bar" view this fixes. */
    three.sph.minR = Math.max(1.2, R * 1.6);
    three.sph.maxR = Math.max(60, Math.max(R, H) * 8);
    if (three.sph.r < three.sph.minR) three.sph.r = three.sph.minR;
    three.place();
  }
  ...
  function updateNozzleSuggestion(nozzleType) {
    if (nozzleType === 'outlet') {
      ...
      if (lllDiv && Q > 0) {
        ...
        var Dm1 = num('tk-D', 3900) / 1000;
        var area1 = Math.PI / 4 * Dm1 * Dm1;                 // m² — same cross-section calc() uses
        var lllCalc = ((Q * tres / 60) / area1) * 1000;      // mm, tied to the actual tank diameter
        lllDiv.style.display = 'block';
        lllDiv.innerHTML = '→ LLL (from ' + tres + ' min residence at Ø ' + (Dm1).toFixed(2) + ' m) ≈ ' + Math.round(lllCalc) + ' mm above LLLL';
      }
      ...
      if (hllDiv && Q2 > 0) {
        ...
        var Dm2 = num('tk-D', 3900) / 1000;
        var area2 = Math.PI / 4 * Dm2 * Dm2;                 // m² — same cross-section calc() uses
        var hllCalc = ((Q2 * tres2 / 60) / area2) * 1000;    // mm, tied to the actual tank diameter
        hllDiv.style.display = 'block';
        hllDiv.innerHTML = '→ HLL (from ' + tres2 + ' min residence at Ø ' + (Dm2).toFixed(2) + ' m) ≈ ' + Math.round(hllCalc) + ' mm below HHLL';
      }
    }
  }

  /* Validate required inputs for tank design */
  function validateTankInputs() {
    var missing = [];
    var inputs = [
      { id: 'tk-fluid', label: 'Service fluid' },
      { id: 'tk-tdes', label: 'Design temperature' },
      { id: 'tk-pdes', label: 'Design pressure' },
      { id: 'tk-D', label: 'Tank diameter (D)' },
      { id: 'tk-H', label: 'Tank height (H)' },
      { id: 'tk-reqcap', label: 'Required capacity' },
      { id: 'tk-mat', label: 'Tank material' },
      { id: 'tk-q-out', label: 'Outlet flow rate (for nozzle sizing)' },
      { id: 'tk-q-in', label: 'Inlet flow rate (for nozzle sizing)' }
    ];
    inputs.forEach(function(inp) {
      var el = $(inp.id);
      if (!el) return;
      /* 0 barg is the design pressure of every atmospheric tank this
         module exists to design, and 0 °C is a real design temperature.
         Both used to be reported as missing. */
      if (window.AROVALID ? window.AROVALID.missing(el)
                          : !(el.value && String(el.value).trim() && String(el.value).trim() !== '0')) missing.push(inp.label);
    });
    return missing;
  }

  /* Show required inputs dialog (like pump hydraulics module) */
  function showRequiredInputsDialog(missing) {
    if (window.__aroBackgroundRun) return;   // a re-run is not a request to design
    var m = document.createElement('div'); m.id = 'tk-reqinput-modal';
    ...
    var inner = '<div style="background:#0f172a;border:2px solid #ef4444;border-radius:8px;max-width:520px;padding:24px;box-shadow:0 20px 60px rgba(0,0,0,0.8);">'
      ...
      + '<div style="font-size:13px;color:#cbd5e1;margin-bottom:16px;line-height:1.6;">Enter values for the following before running tank calculation — without them the result would be built on defaulted zeros and would not be physically meaningful:</div>'
    ...
    inner += '</ul>'
      + '<button id="tk-reqinput-ok" style="width:100%;background:linear-gradient(135deg,#ea580c,#f97316);border:none;color:#fff;font-family:var(--font-mono);font-size:14px;font-weight:800;padding:14px;border-radius:5px;cursor:pointer;">OK, I\'LL FILL THEM IN</button>'
      + '</div>';
    m.innerHTML = inner;
    document.body.appendChild(m);
    var okBtn = $('tk-reqinput-ok');
    if (okBtn) okBtn.onclick = function() { m.remove(); };
    m.addEventListener('click', function(e) { if (e.target === m) m.remove(); });
  }

  /* Update material guidance dynamically when fluid or temperature changes */
  function updateMaterialGuidance() {
    var fluid = val('tk-fluid', 'Water');
    var tempC = num('tk-tdes', 20);
    var moc = recommendMOC(fluid, tempC);
    var matIntDiv = $('tk-mathint');
    if (matIntDiv) {
      matIntDiv.innerHTML = '★ Material guidance for ' + esc(fluid) + ' at ' + tempC + ' °C: ' + esc(moc) + ' — non-corrosive service — plain carbon steel is standard';
    }
  }

  /* ─────────── Apply Design Corrections ───────────
     Reads the live Design Correction Window checks (designAssistant) and
     writes a corrected value into the actual input field behind each
     failing item — not just an advisory note. Re-derives after every write
     because resizing D/H moves every other check (capacity, freeboard,
     gaps), the same way the DPHE "apply auto-upgrade" loop iterates to a
     converged design instead of a single blind pass. */
  function applyDesignCorrections() {
    var changed = [];
    var note = function (msg) { if (changed.indexOf(msg) === -1) changed.push(msg); };
    for (var iter = 0; iter < 6; iter++) {
      ...
      var isFailing = function (re) { return failing.some(function (c) { return re.test(c.label); }); };

      /* H/D out of band or capacity short → resize D & H to the size-class's
         mid-band ratio at the required capacity, with a build-up margin so
         the level stack (LLLL+LLL+HHLLcurb+HLL) still fits inside H. */
      if (isFailing(/^H\/D|^Capacity check|^Diameter — API 650|^Shell height — API 650/)) {
        var capForSizing = r.reqCap > 0 ? r.reqCap : r.geoCap;
        if (capForSizing > 0) {
          var cls = sizeClass(capForSizing);
          var midRatio = (cls.ld[0] + cls.ld[1]) / 2;
          /* Margin is the ACTUAL current level build-up (LLLL+LLL+HHLLcurb+HLL),
             not a flat guess — a bigger overflow nozzle or longer residence
             time needs more headroom than a token 1 m, and using the
             pre-resize (smaller-D, so larger) build-up as the basis is a safe
             overestimate since a bigger D only shrinks the residence-time
             terms. */
          var buildUpM = (r.hLLLL + r.hLLL + r.hHHLLcurb + r.hHLL) / 1000;
          var marginM = Math.max(1.0, buildUpM + 0.3);
          var HCAP = 25, DCAP = 60;   // API 650 atmospheric-tank envelope, metres
          /* Solve for D with H = midRatio·D held EXACTLY (not margin tacked on
             afterward) so the H/D check lands dead-centre in its band instead
             of drifting off it — for a small tank the margin is a large
             fraction of D, so "cbrt then +margin" used to converge to a
             stable point outside the ±0.15 tolerance and Apply looked like it
             only worked once. Bisect on WORKING capacity — area·(H−margin) —
             so capacity and the ratio both clear together in one pass. */
          var capFn = function (D) { return (Math.PI / 4) * D * D * (midRatio * D - marginM); };
          var Dlo = 0.2, Dhi = 100;
          while (capFn(Dhi) < capForSizing && Dhi < 1e5) Dhi *= 2;
          for (var bi = 0; bi < 60; bi++) {
            var Dmid = (Dlo + Dhi) / 2;
            if (capFn(Dmid) < capForSizing) Dlo = Dmid; else Dhi = Dmid;
          }
          var Dsugg = (Dlo + Dhi) / 2;
          var Hsugg = midRatio * Dsugg;
          /* The class ratio builds taller than a single atmospheric shell is
             allowed to go — API 650 caps shell height at 25 m regardless of
             what the L/D thumb-rule would otherwise pick. Go wide instead of
             tall: hold H at the envelope and solve D from working capacity
             directly, which is what actually determines whether this fits a
             single tank at all. */
          if (Hsugg > HCAP) {
            Hsugg = HCAP - 0.1;
            var usableH = Hsugg - marginM;
            Dsugg = usableH > 0 ? Math.sqrt(capForSizing / ((Math.PI / 4) * usableH)) : DCAP + 1;
          }
          /* Even at the full 60 m × 25 m envelope this capacity doesn't fit a
             single shell — cap both and leave the capacity check honestly
             failing (design assistant will say so) rather than silently
             drawing a design past the code limit. Splitting into multiple
             tanks is the real answer at that point, which is outside what a
             single D/H pair can express here. */
          if (Dsugg > DCAP) { Dsugg = DCAP; Hsugg = HCAP - 0.1; }
          var DsuggMm = Math.max(500, Math.round(Dsugg * 1000 / 10) * 10);
          var HsuggMm = Math.max(800, Math.round(Hsugg * 1000 / 10) * 10);
          if ($('tk-D') && Math.abs(num('tk-D', 0) - DsuggMm) > 5) { $('tk-D').value = DsuggMm; note('Tank diameter → ' + DsuggMm + ' mm'); progressed = true; }
          if ($('tk-H') && Math.abs(num('tk-H', 0) - HsuggMm) > 5) { $('tk-H').value = HsuggMm; note('Tank height → ' + HsuggMm + ' mm'); progressed = true; }
        }
      }

      r = compute();
      if (designAssistant(r).some(function (c) { return /^Shell — practical minimum/.test(c.label) && !c.ok; })) {
        var target = Math.max(r.tReq, shellMinThumb(r.Dm), 5);
        var tRounded = Math.ceil(target * 2) / 2;
        if ($('tk-t') && num('tk-t', 0) < tRounded - 1e-6) { $('tk-t').value = tRounded; note('Shell thickness → ' + tRounded + ' mm'); progressed = true; }
      }

      r = compute();
      if (designAssistant(r).some(function (c) { return /^Bottom plate/.test(c.label) && !c.ok; })) {
        var tbNew = Math.min(10, Math.max(6, r.tb || 6));
        if ($('tk-tb')) { $('tk-tb').value = tbNew; note('Bottom plate → ' + tbNew + ' mm'); progressed = true; }
      }

      r = compute();
      if (designAssistant(r).some(function (c) { return /^LLLL → LLL gap/.test(c.label) && !c.ok; })) {
        if ($('tk-lll-min')) { $('tk-lll-min').value = 400; note('LLLL→LLL minimum gap → 400 mm'); progressed = true; }
      }

      r = compute();
      if (designAssistant(r).some(function (c) { return /^HHLL → HLL gap/.test(c.label) && !c.ok; })) {
        if ($('tk-hll-min')) { $('tk-hll-min').value = 400; note('HHLL→HLL minimum gap → 400 mm'); progressed = true; }
      }

      r = compute();
      if (designAssistant(r).some(function (c) { return /^Freeboard above HHLL/.test(c.label) && !c.ok; })) {
        if ($('tk-hhll-spec')) { $('tk-hhll-spec').value = 400; note('HHLL curb clearance spec → 400 mm'); progressed = true; }
      }

      if (!progressed) break;   // nothing left this pass knows how to fix automatically
    }
    return changed;
  }

  /* Auto-Design: Recommends material, residence time, nozzle sizes based on fluid & temp,
     then applies the Design Correction Window's fixes so the checks actually clear. */
  function autoDesign() {
    var fluid = val('tk-fluid', 'Water');
    var tempC = num('tk-tdes', 20);
    var capReq = num('tk-reqcap', 10);

    /* 1. Material recommendation based on fluid + temperature */
    var mocRec = recommendMOC(fluid, tempC);
    var matFound = false;
    var matSelect = $('tk-mat');
    if (matSelect) {
      for (var i = 0; i < matSelect.options.length; i++) {
        if (matSelect.options[i].text.includes('CS (A36') || matSelect.options[i].text.includes('SS')) {
          if ((mocRec.includes('SS') && matSelect.options[i].text.includes('SS')) ||
              (mocRec.includes('CS') && matSelect.options[i].text.includes('CS'))) {
    ...
    /* 2. Suggest residence times (3-5 min for typical industrial duty) */
    var resTimeDefault = fluid.match(/Acid|Ammonia|Caustic/) ? 5 : 3;
    $('tk-res-out').value = resTimeDefault;
    $('tk-res-in').value = resTimeDefault;

    /* 3. Suggest outlet/inlet nozzle size based on flow rate (if not already set) —
       write BOTH the NPS selector and the legacy diameter field it drives, since
       compute() reads the legacy field directly and a plain .value= assignment
       does not fire the 'change' listener that normally keeps them in sync. */
    var qOut = num('tk-q-out', 0);
    if (qOut > 0 && !$('tk-noz-nps').value) {
      var sugNps = suggestNozzleNPS(qOut, 'discharge');
      if (sugNps && PIPE_NPS[sugNps]) {
        $('tk-noz-nps').value = sugNps;
        $('tk-noz-out').value = PIPE_NPS[sugNps].od.toFixed(2);
      }
    }

    var qIn = num('tk-q-in', 0);
    if (qIn > 0 && !$('tk-ovf-nps').value) {
      var sugOvf = suggestNozzleNPS(qIn * 0.7, 'discharge');
      if (sugOvf && PIPE_NPS[sugOvf]) {
        $('tk-ovf-nps').value = sugOvf;
        $('tk-noz-ovf').value = PIPE_NPS[sugOvf].od.toFixed(2);
      }
    }

    /* 4. Resolve geometry / thickness / gap / freeboard corrections so the
       Design Correction Window actually clears instead of just decorating
       the panel with a material recommendation. */
    var geomChanges = applyDesignCorrections();

    /* 5. Show a consolidated status covering everything Auto-Design touched */
    var statusDiv = $('tk-run-status');
    if (statusDiv) {
      statusDiv.style.display = 'block';
      statusDiv.style.background = 'linear-gradient(135deg,#ea580c,#f97316)';
      statusDiv.style.color = '#fff';
      var extra = geomChanges.length ? ('<br/>✓ ' + geomChanges.join('<br/>✓ ')) : '';
      statusDiv.innerHTML = '⚡ <b>AUTO-DESIGN APPLIED</b><br/>✓ Material: ' + mocRec.split(' —')[0]
        + '<br/>✓ Residence time: ' + resTimeDefault + ' min<br/>✓ Nozzle sizes auto-set' + extra;
    }

    /* Run calculation to apply all changes */
    calc();
  }
  ...
  function wire() {
    ...
    if (flSel) flSel.addEventListener('change', function () {
      ...
      var rho = p && p.rho != null ? resolveTankFluidRho(flSel.value, p.rho) : null;
      var rhoEl = $('tk-rho');
      if (rhoEl) rhoEl.value = (rho != null) ? Number(CV(rho, 'density').toFixed(6)).toString() : '';
      if (window.aroSetProvBadge) window.aroSetProvBadge('tk-rho', resolveTankFluidRho.lastStatus);
      updateMaterialGuidance();  /* Update material recommendation when fluid changes */
    ...
    INPUT_IDS.forEach(function (id) {
      ...
      e.addEventListener('input', function () {
        /* Tank diameter feeds the residence-time suggestion's cross-section —
           keep both suggestion boxes live as the engineer types D. */
        if (id === 'tk-D' || id === 'tk-q-out' || id === 'tk-res-out') updateNozzleSuggestion('outlet');
        if (id === 'tk-D' || id === 'tk-q-in' || id === 'tk-res-in') updateNozzleSuggestion('overflow');
        pushUndo(); calc(); updHistBtns();
      });
    ...
    var cb = $('tk-calc'); if (cb) cb.addEventListener('click', function () {
      var missing = validateTankInputs();
      if (missing.length > 0) {
        showRequiredInputsDialog(missing);
        return;
      }
      if (window.ARORESET) window.ARORESET.lift('tk');
      updateMaterialGuidance();
      calc();
      runStatus();
    });
    var ab = $('tk-autodesign'); if (ab) ab.addEventListener('click', function () { pushUndo(); autoDesign(); updHistBtns(); });
    /* "APPLY DESIGN CORRECTIONS" lives inside the results panel and is
       redrawn on every calc(), so it is wired by delegation rather than a
       direct listener that would be lost on the next render. */
    document.addEventListener('click', function (e) {
      if (!e.target || e.target.id !== 'tk-apply-fixes') return;
      pushUndo();
      var applied = applyDesignCorrections();
      calc();
      updHistBtns();
      var statusDiv = $('tk-run-status');
      if (statusDiv) {
        statusDiv.style.display = 'block';
        statusDiv.style.background = applied.length ? 'linear-gradient(135deg,#16a34a,#22c55e)' : 'linear-gradient(135deg,#64748b,#94a3b8)';
        statusDiv.style.color = '#fff';
        statusDiv.innerHTML = applied.length
          ? ('✓ <b>DESIGN CORRECTIONS APPLIED</b><br/>✓ ' + applied.join('<br/>✓ '))
          : 'No further automatic correction available — the remaining item(s) need engineering judgement (e.g. a service or roof-type change).';
      }
    });
    /* Opens the same merged report every other module's REPORT button
       does — lib/aro-engineering.js's reportHtml() carries everything
       report() below built (design data, geometry, API 650 shell-thickness
       trace, checks, BOM, design-assistant guidance) plus what that
       function never had: provenance-tagged inputs, the industrial-3D
       snapshot, the GA drawing (now wired via mfgDetailFor('tank')), and
       the REPORT CONTENTS / page-size controls. report() itself stays
       defined and working as the fallback if AROENG isn't present. */
    var rb = $('tk-report'); if (rb) rb.addEventListener('click', function () {
    ...
    var stb = $('tk-selftest'); if (stb) stb.addEventListener('click', function () {
      ...
      if (statusDiv) {
        statusDiv.style.display = 'block';
        statusDiv.style.background = allPass ? 'linear-gradient(135deg,#16a34a,#22c55e)' : 'linear-gradient(135deg,#dc2626,#ef4444)';
        statusDiv.style.color = '#fff';
        statusDiv.innerHTML = '🧪 <b>SELF-TEST ' + (allPass ? 'PASSED' : 'FAILED') + '</b> — ' + results.length + ' size classes checked, your sheet is untouched<br/>'
          + lines.join('<br/>');
      }
    });
    function setView(v) {
  ...
  function drawing() {
    ...
    s += txt(tx - 99, yOf(r.elOverflow) - 5, 'OVF ' + getNozzleSpec('tk-ovf-nps', 'tk-noz-ovf', 3), 7.5, '#dc2626', 'end', '700');
    ...
    var nozSched = [
      ['N1', 'INLET', getNozzleSpec('tk-noz-nps', 'tk-noz-out', 4), 'ASME B16.5', U(r.H * 0.92, 'length-mm', 0) + '  (near top)'],
      ['N2', 'OUTLET', getNozzleSpec('tk-noz-nps', 'tk-noz-out', 4), 'ASME B16.5', 'CL ' + U(nozCLmm, 'length-mm', 0) + ' from bottom'],
      ['N3', 'OVERFLOW', getNozzleSpec('tk-ovf-nps', 'tk-noz-ovf', 3), 'ASME B16.5', U(r.elOverflow, 'length-mm', 0)],
  ...
  /* Compact 2D elevation, self-contained (own line/txt/rect helpers rather
     than sharing drawing()'s closure) so it can be dropped into the report
     as a real image of THIS tank — actual D/H/roof/levels/fluid colour from
     the current inputs, not a generic placeholder icon. */
  function elevationSVG(r) {
    ...
    var tw2 = 170, th = 300, tx = W / 2 - tw2 / 2, ty = 80;
    var scale = th / r.H;
    var yOf = function (mm) { return ty + th - mm * scale; };
    var s = '<svg viewBox="0 0 ' + W + ' ' + Hh + '" style="width:100%;max-width:420px;background:#fff;font-family:Arial;">';
    s += rect(0, 0, W, Hh, '#fff');
    s += txt(W / 2, 18, r.fluid + ' · ' + U(r.Dm, 'length-m', 1) + ' Ø × ' + U(r.Hm, 'length-m', 1) + ' · ' + r.roof, 9.5, '#0f172a', 'middle', '700');

    // roof
    if (/Cone/.test(r.roof)) s += '<polygon points="' + tx + ',' + ty + ' ' + (tx + tw2 / 2) + ',' + (ty - 30) + ' ' + (tx + tw2) + ',' + ty + '" fill="#f1f5f9" stroke="#0f172a" stroke-width="1.2"/>';
    else if (/Dome/.test(r.roof)) s += '<path d="M ' + tx + ' ' + ty + ' A ' + (tw2 / 2) + ' 34 0 0 1 ' + (tx + tw2) + ' ' + ty + '" fill="#f1f5f9" stroke="#0f172a" stroke-width="1.2"/>';
    else s += rect(tx, ty - 8, tw2, 8, '#f1f5f9', '#0f172a', 1.2);
    // shell
    s += rect(tx, ty, tw2, th, '#f8fafc', '#0f172a', 1.4);
    // liquid fill to HLL, tinted by the selected fluid — the same colour the
    // 3D view uses, so the report and the live view read as one design.
    var liqTop = yOf(r.elHLL);
    s += '<rect x="' + (tx + 2) + '" y="' + liqTop + '" width="' + (tw2 - 4) + '" height="' + ((ty + th) - liqTop) + '" fill="' + hex(fluidColor(r.fluid)) + '" opacity="0.45"/>';
    // foundation
    s += rect(tx - 10, ty + th, tw2 + 20, 10, '#e2e8f0', '#0f172a', 1);

    // level lines
    var lvls = [['TTL', r.elTTL, '#0f172a'], ['HHLL', r.elHHLL, '#dc2626'], ['HLL', r.elHLL, '#ea580c'],
      ['LLL', r.elLLL, '#16a34a'], ['LLLL', r.elLLLL, '#7c3aed'], ['BTL', 0, '#0f172a']];
    var MINGAP = 12, prevY = -1e9;
    lvls.forEach(function (L) {
      var y = yOf(L[1]), ly = y;
      if (ly - prevY < MINGAP) ly = prevY + MINGAP;
      prevY = ly;
      s += line(tx, y, tx + tw2 + 8, y, 0.7, L[2], '4 2');
      s += txt(tx + tw2 + 12, ly + 3, L[0], 7.5, L[2], 'start', '700');
    });

    // nozzle stubs (outlet/inlet/overflow) as short call-out ticks
    s += line(tx - 14, yOf(Math.max(150, r.elLLLL * 0.4)), tx, yOf(Math.max(150, r.elLLLL * 0.4)), 2, '#2563eb');
    s += line(tx - 14, yOf(r.elOverflow), tx, yOf(r.elOverflow), 2, '#ef4444');

    // dimensions
    s += line(tx - 26, ty, tx - 26, ty + th, 0.7, '#dc2626')
      + txt(tx - 30, ty + th / 2, U(r.H, 'length-mm', 0), 8, '#dc2626', 'end', '700');
    s += line(tx, ty + th + 24, tx + tw2, ty + th + 24, 0.7, '#dc2626')
      + txt(tx + tw2 / 2, ty + th + 38, 'Ø ' + U(r.D, 'length-mm', 0), 8, '#dc2626', 'middle', '700');
    s += txt(W / 2, Hh - 8, esc(r.matName) + ' · shell t ' + U(r.t, 'length-mm', 1) + ' · ' + U(r.workCap, 'volume', 1) + ' working', 8, '#64748b', 'middle');
    s += '</svg>';
    return s;
  }
  ...
  function report() {
    ...
    b += sec('4 · 2D ELEVATION (SCHEMATIC — DRAWN TO THIS DESIGN)');
    b += '<div style="text-align:center;">' + elevationSVG(r) + '<div style="font-size:9px;color:#94a3b8;margin-top:2px;">Not for construction — see the GA Drawing for the dimensioned issue.</div></div>';
    b += sec('5 · LIQUID LEVELS');
    b += T([['LLLL — nozzle CL + radius + liquid', U(r.lllCalc, 'length-mm', 1) + '  → adopted ' + U(r.hLLLL, 'length-mm', 1)],
      ...
      ['Outlet/Inlet nozzle size', getNozzleSpec('tk-noz-nps', 'tk-noz-out', 4)],
      ['HHLL below curb (1.5 × ' + getNozzleSpec('tk-ovf-nps', 'tk-noz-ovf', 3) + ')', U(r.hCurbApi, 'length-mm', 1) + '  → adopted ' + U(r.hHHLLcurb, 'length-mm', 1)],
    ...
    b += sec('6 · SHELL DESIGN — API 650 (1-FOOT METHOD)');
  ...
  /* ─────────── Self-test ───────────
     A lightweight in-app regression check for the governing calculation and
     the auto-fix loop, run on demand (the TEST button) rather than on every
     load. Exercises applyDesignCorrections() across a spread of required
     capacities from 5 m³ to 50,000 m³ and confirms it converges to zero
     CRITICAL and zero REVIEW items on the Design Correction Window, plus a
     handful of independent physical sanity checks (level ordering, the
     freeboard/curb identity, shell ≥ its own required thickness, working
     capacity ≥ required) that would catch a regression even if every
     advisory band happened to still be satisfied. The engineer's actual
     sheet is snapshotted first and restored afterward, so running it never
     disturbs whatever design is on screen. */
  function selfTest() {
    var saved = snapshot();
    var baseline = {
      'tk-fluid': 'Water', 'tk-tdes': '45', 'tk-pdes': '0', 'tk-ca': '1.5',
      'tk-mat': 'CS (A36 / IS 2062)', 'tk-orient': 'Vertical', 'tk-roof': 'Cone Roof',
      'tk-service': 'Storage tank', 'tk-E': '1',
      'tk-noz-cl': '240', 'tk-noz-out': '4', 'tk-liq-above': '150', 'tk-lll-spec': '500',
      'tk-q-out': '20', 'tk-res-out': '3', 'tk-lll-min': '100',
      'tk-noz-ovf': '3', 'tk-hhll-spec': '100', 'tk-q-in': '20', 'tk-res-in': '3', 'tk-hll-min': '100'
    };
    var cases = [
      { name: 'Tiny (5 m³)', reqcap: 5, D: 800, H: 800 },
      { name: 'Small (50 m³)', reqcap: 50, D: 1000, H: 1000 },
      { name: 'Medium (500 m³)', reqcap: 500, D: 2000, H: 2000 },
      { name: 'Large (5,000 m³)', reqcap: 5000, D: 4000, H: 4000 },
      { name: 'Very large (50,000 m³)', reqcap: 50000, D: 10000, H: 8000 }
    ];
    var results = [];
    cases.forEach(function (c) {
      Object.keys(baseline).forEach(function (id) { var e = $(id); if (e) e.value = baseline[id]; });
      var reqEl = $('tk-reqcap'); if (reqEl) reqEl.value = c.reqcap;
      var dEl = $('tk-D'); if (dEl) dEl.value = c.D;
      var hEl = $('tk-H'); if (hEl) hEl.value = c.H;
      var tEl = $('tk-t'); if (tEl) tEl.value = '0';
      var tbEl = $('tk-tb'); if (tbEl) tbEl.value = '6';

      applyDesignCorrections();

      var r = compute();
      var checks = designAssistant(r);
      var critical = checks.filter(function (ck) { return !ck.ok && ck.priority === 'critical'; });
      var review = checks.filter(function (ck) { return !ck.ok && ck.priority !== 'critical'; });
      var sane = [];
      if (!(r.elLLLL < r.elLLL && r.elLLL < r.elHLL && r.elHLL < r.elHHLL && r.elHHLL <= r.elTTL)) sane.push('level order violated');
      if (Math.abs((r.H - r.elHHLL) - r.hHHLLcurb) > 0.01) sane.push('freeboard/curb identity broken');
      if (!(r.t >= r.tReq - 1e-6)) sane.push('shell thinner than its own required thickness');
      if (!(r.workCap >= r.reqCap - 1e-3)) sane.push('working capacity below required');
      if (!(r.dOk && r.hOk)) sane.push('outside the API 650 60 m / 25 m single-shell envelope');
      /* CRITICAL and the independent sanity checks must both be clear — those
         are correctness bugs if they fire. A lingering REVIEW item is not:
         a very large tank genuinely has to run outside the "typical L/D"
         field-practice band once the 25 m height cap forces it wide (e.g. a
         50,000 m³ tank is legitimately short and squat in the real world),
         and that is the tool correctly saying so, not a defect to chase. */
      var pass = critical.length === 0 && sane.length === 0;
      results.push({
        name: c.name, pass: pass,
        detail: pass ? (review.length ? (review.length + ' review (expected at this size) — ') : 'clean — ') + 'D ' + Math.round(r.D) + ' mm × H ' + Math.round(r.H) + ' mm, L/D ' + r.LD.toFixed(2)
          : [].concat(
  ...
  window.AROTANK = {
    /* read-only, for the engineering data registry */
    materials: MATERIALS,
    calc: calc, compute: compute, report: report, drawing: drawing, MATERIALS: MATERIALS,
    /* Exposed for the in-app self-test and for external verification —
       these are the functions whose correctness actually matters (the
       governing calc, the advisory checks, and the auto-fix loop). */
    designAssistant: designAssistant, applyDesignCorrections: applyDesignCorrections,
    autoDesign: autoDesign, selfTest: selfTest, elevationSVG: elevationSVG
  };
})();

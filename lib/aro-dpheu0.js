/* ═══════════════════════════════════════════════════════════════════════════
   ...
   WHAT IT DOES NOT DO. It does not size anything — U₀ here only feeds the
   verification chart and verdict below, never the pipe/hairpin sizing math.

   THE BOX'S OWN NUMBER. The box shows the band midpoint by default — the
   same figure a blank box already fed to the verification chart silently,
   now visible where the assumption is actually made. It stays that way
   only until an engineer types over it (tracked via AROPROV, the app's
   existing default/yours/calculated marker, so this follows the same rule
   every other auto-suggested field does); after that the typed figure is
   the assumption and this file never touches the box again. The
   output-side field stays, and the two are the same value — typing in
   either updates the other, because two boxes for one quantity that can
   disagree is worse than one box in an awkward place.
   ═══════════════════════════════════════════════════════════════════════════ */
(function () {
  'use strict';
  ...
  function chart(b, uAssumed, uCalc) {
    ...
    /* the number line itself, so the decade ticks read as marks on one
       continuous scale rather than floating hatch marks */
  ...
  function render() {
    ...
    if (!host) return;
    var b = band();
    var el = $('aro-dphe-u0-in');
    /* The box used to sit at "0" and rely on the engineer reading, elsewhere,
       that blank/0 silently takes the band midpoint — a real number was
       being assumed and used in the verification chart below without ever
       appearing where the assumption is actually entered. Writing the
       midpoint into the box itself does not change what gets assumed (it is
       the exact figure blank already produced, straight from dpheTypicalU —
       see calculatedU()/the chart below, both read the same field), it only
       stops asking the engineer to take that on faith. It only ever touches
       the box while AROPROV still calls it a default; the moment an
       engineer types a real entry here, this stops for good, same as every
       other auto-suggested field in the app. */
    ...
    if (el && !isUserSet && b) {
      ...
      if (display !== '—' && el.value !== display) {
        ...
        syncFrom('aro-dphe-u0-in', 'dphe-u-assumed');
      }
    }
  ...
  function build() {
    ...
    box.innerHTML =
      ...
      + 'title="Pre-filled with the typical band\'s midpoint for this service — type over it to use your own figure"></div>'

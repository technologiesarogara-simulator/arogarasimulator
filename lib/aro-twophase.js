...
(function () {
  ...
  function panelHTML() {
    ...
    h += two(fld('Liquid mass flow', 'tp2-wl', 'kg/hr', 300, '1'), fld('Gas mass flow', 'tp2-wg', 'kg/hr', 800, '1'));
    ...
    h += '<div id="tp2-svcuser" style="display:none;">' + fld('C factor (API 14E)', 'tp2-cfactor', '', '', '1') + '</div>';
  ...
  function compute() {
    ...
    var C = CFACTOR[svc] !== undefined ? CFACTOR[svc] : num('tp2-cfactor', NaN);
  ...
  var INPUT_IDS = ['tp2-company', 'tp2-loc', 'tp2-pid', 'tp2-lineno', 'tp2-from', 'tp2-to', 'tp2-pair', 'tp2-sigma',
    ...
    'tp2-nps', 'tp2-sch', 'tp2-mat', 'tp2-eps', 'tp2-cfactor', 'tp2-len', 'tp2-dz', 'tp2-dpeq', 'tp2-service', 'tp2-pcterosion', 'tp2-dpallow'];
  ...
  function wire() {
    ...
    function toggleUser() {
      var m = $('tp2-matuser'), sv = $('tp2-svcuser');
      if (m) m.style.display = (ROUGH[val('tp2-mat', 'CS')] === undefined) ? 'block' : 'none';
      if (sv) sv.style.display = (CFACTOR[val('tp2-service', 'Clean Liquid')] === undefined) ? 'block' : 'none';
    }

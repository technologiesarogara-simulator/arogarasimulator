...
(function () {
  ...
  function setMode(m) {
    var a = $('tp2-auto'), mn = $('tp2-manual');
    if (!a || !mn) return;
    a.style.display = m === 'manual' ? 'none' : 'block';
    mn.style.display = m === 'manual' ? 'block' : 'none';
    if (m === 'manual' && window.AROPID) window.AROPID.build('tp2-manual');
    if (m === 'auto') setTimeout(function () { resize3D(); calc(); }, 60);
  }
  ...
  function build() {
    ...
    host.innerHTML = modeBar()
      + '<div id="tp2-auto">' + panelHTML() + '</div>'
      + '<div id="tp2-manual" style="display:none;"></div>';
    built = true;
    var md = $('tp2-mode');

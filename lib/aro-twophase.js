...
(function () {
  ...
  function fld(label, id, unit, v, step) {
    ...
    return '<label style="display:block;margin:4px 0;font-family:var(--font-mono);font-size:10px;color:var(--text-muted);">' + label
      ...
      + (unit ? '<span class="unit"' + (type ? ' data-unit-type="' + type + '"' : '')
              ...
              + (type ? symbolNow(type) + suffix : unit) + '</span>' : '') + '</span></label>';
  }
  ...
  function manualHTML() {
    ...
    h += '<h4 class="aro-doc-h">AUTO vs MANUAL &mdash; the P&amp;ID workbench</h4>'
      ...
      + 'mixture velocity &mdash; so the line reads as fluid in service, not a static rod. A <b>COLOUR: VELOCITY / '
      + 'MATERIAL</b> toggle in the 3D toolbar switches every uncoloured leg between the velocity diagnostic and the '
      + 'pipe\'s actual finish for the material chosen in section 4.</p>';

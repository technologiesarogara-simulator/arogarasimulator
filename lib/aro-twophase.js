...
(function () {
  ...
  function manualHTML() {
    ...
    h += '<p class="aro-doc-lead">A two-phase line carries liquid and gas together, so it is sized on the flow '
      ...
      + 'velocity and pressure drop. Work down the sections in order; each input is validated as you enter it and the results follow RUN, so you see '
  ...
  function panelHTML() {
    ...
    h += '<div class="panel" style="max-height:calc(100vh - 190px);overflow-y:auto;">'
      ...
      + '<div id="tp2-3dwrap" style="position:relative;width:100%;height:300px;background:#eef1f4;border:1px solid var(--border-muted);border-radius:6px;overflow:hidden;">'
    ...
    if (window.AROENG) {
      try {
        window.AROENG.publish('line-twophase', {
          ...
          /* The result itself, the way the other four line modules publish
             theirs. Without it the header read CURRENT while the drawing, the
             3D model and the report figures had nothing to draw from. */
          values: r
        });
      } catch (e) {}
    }
  ...
  function init3D() {
    ...
    var sph = { r: 21, theta: 0.35, phi: 1.30, tx: 0, ty: 0, tz: 0, minR: 8, maxR: 80 };
    function place() {
    ...
    /* Hand this scene to the shared 3D toolbar, the same way the other four
       line-sizing services and PHE and Tank do. Without it the GRID and
       BACKGROUND buttons reach only the INDUSTRIAL picture on this stream,
       and the ANALYTICAL one ignores both — the one service out of ten that
       behaved differently. */
    try { if (window.ARO3DI && window.ARO3DI.registerLegacy)
      window.ARO3DI.registerLegacy('line-twophase', function () { return three; }); } catch (e) {}
    place();
    var down = null;
  ...
  function report() {
    ...
    b += '<div style="text-align:center;margin-bottom:14px;"><div style="font-size:18px;font-weight:800;color:#ea580c;">AROGARA — TWO-PHASE LINE SIZING REPORT</div><div style="font-size:10px;color:#64748b;">AROGARA · Digital Engineering Design Platform</div></div>';
  ...
  function wire() {
    ...
    /* Same merge as the other line-sizing services: open the shared
       top-toolbar report instead of this module's own modal. report()
       stays defined, just no longer reachable from this button. */
    var rb = $('tp2-report'); if (rb) rb.addEventListener('click', function () { if (window.AROENG && window.AROENG.report) window.AROENG.report(); else report(); });
  ...
  function applyPid(sum) {
    ...
    /* Drawing a leg is not a request to be shown the results — without this
       the first leg takes the module to CALCULATED and the suite scrolls the
       workbench off the top of the panel while the line is still being
       drawn. See the same guard in lib/aro-linesize.js. */
    var was = window.__aroBackgroundRun;
    window.__aroBackgroundRun = true;
    try { calc(); }
    finally { setTimeout(function () { window.__aroBackgroundRun = was || false; }, 140); }
  }

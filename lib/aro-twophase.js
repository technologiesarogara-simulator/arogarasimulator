...
(function () {
  ...
  function suggestions(r) {
    ...
    if (!r.sizeOk) {
      ...
      out.push({ title: need ? 'Increase bore to ' + need + '" — the smallest size that meets the requirement'
        ...
        why: 'The duty needs at least ' + f2n(r.DreqMm) + ' mm (' + f2n(r.DreqIn) + '") to hold the mixture at or below '
          + f2n(r.Vallow) + ' m/s, and the line is drawn at ' + r.nps + '" (' + f2n(r.Dmm) + ' mm). '
    ...
    if (!r.eroOk && r.pct < 60) out.push({ title: 'Review the erosional set point — currently ' + f2n(r.pct) + ' % of Ve',
      why: 'The allowable velocity is only ' + f2n(r.Vallow) + ' m/s because the design is held to ' + f2n(r.pct)
        + ' % of the ' + f2n(r.Ve) + ' m/s erosional velocity. API RP 14E is normally applied at 75–100 % for a clean, non-erosive service. Raising the set point to 75 % gives ' + f2n(r.Ve * 0.75) + ' m/s without changing the pipe.',
    ...
    if ((!r.eroOk || !r.dpOk) && r.sch !== '5') {
      ...
      if (lighter && PIPE[r.nps] && PIPE[r.nps].s[lighter] !== undefined) {
        out.push({ title: 'Open the bore by going to schedule ' + lighter + ' at the same NPS',
          why: 'At ' + r.nps + '" the internal diameter grows from ' + f2n(r.Dmm) + ' mm to '
            + f2n(PIPE[r.nps].s[lighter] * 25.4) + ' mm on schedule ' + lighter + '. Velocity falls with D² and friction with D⁵, so this often clears the check without moving to the next NPS — provided the pressure rating still covers the duty.',
  ...
  function report() {
    ...
    b += '<div style="text-align:center;margin-bottom:14px;"><div style="font-size:18px;font-weight:800;color:#ea580c;">AROGARA FLOWSIZE — TWO-PHASE LINE SIZING REPORT</div><div style="font-size:10px;color:#64748b;">AROGARA TECHNOLOGIES | DIGITAL INDIA INITIATIVE</div></div>';

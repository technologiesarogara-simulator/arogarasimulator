    ...
    Viewer.prototype.setGeometry = function (layout, opts) {
      ...
      if (!layout || !layout.valid) {
        while (this.group.children.length) {
          var c0 = this.group.children.pop();
          if (c0.geometry) c0.geometry.dispose();
          if (c0.material) c0.material.dispose();
        }
        this._hw = null;
        return;
      }
      ...
      /* Dragging the vane-count/exit-angle sliders only changes the blade
         shapes, not the backplate/hub/bolts/rings — those depend only on
         r1, r2 and hubHeight (i.e. only the D2 slider). Rebuilding all of
         that (and disposing the old copies) on every single vane-count or
         angle tick was most of the per-frame cost that made dragging feel
         sluggish; it is now only rebuilt when the geometry it actually
         depends on has changed. */
      var hw = this._hw;
      var hwKey = layout.r1 + '|' + layout.r2 + '|' + hubHeight;
      if (!hw || hw.key !== hwKey) {
        if (hw) { hw.meshes.forEach(function (m) { self.group.remove(m); if (m.geometry) m.geometry.dispose(); if (m.material) m.material.dispose(); }); }
        ...
        /* Real semi-open impellers (the common case this schematic models)
           ...
           saturated pink/blue/orange so the 3-point light rig above
           actually has something to put a real highlight and shadow on. */
        var metalMat0 = new THREE.MeshStandardMaterial({ color: 0x8b93a0, metalness: 0.82, roughness: 0.32 });
        ...
        this.group.add(backplate); meshes.push(backplate);
        ...
        this.group.add(hub); meshes.push(hub);
        ...
        for (var bi = 0; bi < boltCount; bi++) {
          ...
          this.group.add(bolt); meshes.push(bolt);
        }
        ...
        [layout.r1, layout.r2].forEach(function (r, idx) {
          ...
          self.group.add(ring); meshes.push(ring);
        });

        hw = this._hw = { key: hwKey, meshes: meshes, metalMat: metalMat0 };
        this._floor.position.y = backplateY - backplateH / 2 - Math.max(0.01, layout.r2 * 0.05);
        this._fitCamera(layout.r2);
      }

      // vanes — extrude a thin blade shape along the curve, standing on the
      // backplate. These DO depend on vaneCount/beta2Deg, so they rebuild
      // on every call regardless of whether the hardware above did.
      if (this._blades) { this._blades.forEach(function (m) { self.group.remove(m); if (m.geometry) m.geometry.dispose(); }); }
      ...
      layout.blades.forEach(function (blade) {
        ...
        var geo = new THREE.ExtrudeGeometry(shape, { depth: hubHeight, bevelEnabled: true, bevelThickness: thickness * 0.15, bevelSize: thickness * 0.12, bevelSegments: 1 });
        geo.rotateX(Math.PI / 2);
        geo.translate(0, backplateY + backplateH / 2, 0);
        var mesh = new THREE.Mesh(geo, hw.metalMat);
    ...
    Viewer.prototype.start = function () {
      ...
      function tick() {
        ...
        /* Idle while the canvas is off screen — see lib/aro-raf.js. This
           ...
           forever, even scrolled miles away — the exact "120 long frames
           out of 121" cost aro-raf.js was built to eliminate elsewhere. */
        if (window.AROVIS && !window.AROVIS.visible(self.renderer.domElement)) return;
        /* resize() only did real work on setGeometry() — nothing re-checked
           the canvas's own size on a browser zoom or window resize, so the
           WebGL backing buffer stayed at its old resolution while the CSS
           box (width:100%) reflowed underneath it, stretching/warping the
           rendered frame until the next slider drag happened to call
           setGeometry() again. Checking it every frame (same self-healing
           pattern aro-pumpflowviz3d.js already uses) costs one cheap
           clientWidth/clientHeight read and is a no-op whenever the size

...
(function () {
  ...
  if (typeof document !== 'undefined') {
    function themePalette() {
      return (window.AROVIZTHEME && window.AROVIZTHEME.palette) ? window.AROVIZTHEME.palette() : { bg: '#050810', text: '#e2e8f0', textMuted: '#94a3b8' };
    }
    ...
    function Viewer(canvas) {
      ...
      this._t = 0;
      this._stations = [];
      /* This canvas only ever draws lines/text via clearRect + strokes —
         there is no THREE.js scene to tint, so the whole background comes
         from CSS. clearRect makes it transparent, letting the canvas
         element's own inline background (dark) show through regardless of
         the page theme. Painted here instead, and repainted live on a
         theme switch, matching the pattern the 3D viewers already use. */
      if (window.AROVIZTHEME && window.AROVIZTHEME.onChange) {
        window.AROVIZTHEME.onChange(function () { self._draw(); });
      }
    }
    ...
    Viewer.prototype._draw = function () {
      ...
      /* Was: canvas.width/height set directly to clientWidth/clientHeight,
         a 1x backing store that any high-DPI/retina display then upscales
         to fill the CSS box — the actual source of this diagram reading as
         soft/blurry next to the other three pump charts, which already use
         this same hd() helper. Drawing coordinates below are now in CSS-
         pixel space (W/H), same as before this fix, just backed by a
         sharper bitmap. */
      var hdRes = window.AROPUMPCHART ? window.AROPUMPCHART.hd(canvas) : null;
      var ctx = hdRes ? hdRes.ctx : this.ctx;
      var W = hdRes ? hdRes.W : canvas.clientWidth, H = hdRes ? hdRes.H : canvas.clientHeight;
      this.ctx = ctx;
      ...
      stations.forEach(function (s, i) {
        ...
        ctx.beginPath();
        ctx.fillStyle = s.known ? col : pal.textMuted;
        ...
        ctx.fillStyle = s.known ? pal.text : pal.textMuted;
        ctx.fillText(s.known ? s.velocity_ms.toFixed(2) + ' m/s' : 'n/a', xs[i], midY - 12);
        tipPts.push({
          x: xs[i], y: midY, color: s.known ? col : pal.textMuted,
          html: '<b>' + s.label.replace(/</g, '&lt;') + '</b><br>' + (s.known ? s.velocity_ms.toFixed(2) + ' m/s' : 'not calculated')
        });
      });
      if (window.AROPUMPCHART) {
        window.AROPUMPCHART.attachTooltip(canvas);
        window.AROPUMPCHART.setTooltipPoints(canvas, tipPts);
      }
    };
    ...
    Viewer.prototype.start = function () {
      ...
      function tick() {
        ...
        /* Idle while the canvas is off screen — see lib/aro-raf.js. This
           ...
           redrawing every frame forever, even scrolled miles away. */

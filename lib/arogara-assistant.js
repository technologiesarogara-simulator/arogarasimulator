/* ═══════════════════════════════════════════════════════════════════════
   ...
   Global floating AI assistant for AROGARA FLOWSIZE.
   Two brains:
...
(function () {
  ...
  var css = ''
    /* ── ONE PALETTE, TWO THEMES ──────────────────────────────────────
       This panel was written in fixed dark colours, so an engineer working
       in the light theme opened the copilot and got a night-blue window in
       the middle of a white application. The surfaces are tokens now and
       daylight redeclares them; the orange and blue brand colours are
       deliberately NOT tokenised, because they are the identity and they
       hold on either ground. */
    + '#aro-ai-panel,#aro-ai-settings{--ai-bg1:rgba(14,22,46,.98);--ai-bg2:rgba(7,12,26,.99);'
    +   '--ai-line:rgba(126,162,255,.22);--ai-line2:rgba(126,162,255,.15);'
    +   '--ai-bar:rgba(13,22,47,.55);--ai-bub:rgba(13,22,47,.7);--ai-bubline:rgba(126,162,255,.18);'
    +   '--ai-ink:#c9d7f2;--ai-strong:#fff;--ai-sub:#7f96c2;--ai-foot:#5770a3;'
    +   '--ai-field:rgba(4,9,22,.85);--ai-fieldink:#e6eeff;--ai-chip:#9fb4de;'
    +   '--ai-code:#9fc1ff;--ai-shadow:rgba(0,0,0,.6);}'
    + 'body.theme-day #aro-ai-panel,body.theme-day #aro-ai-settings{'
    +   '--ai-bg1:rgba(255,255,255,.99);--ai-bg2:rgba(244,247,250,.99);'
    +   '--ai-line:rgba(37,64,120,.20);--ai-line2:rgba(37,64,120,.14);'
    +   '--ai-bar:rgba(233,238,245,.9);--ai-bub:#ffffff;--ai-bubline:rgba(37,64,120,.16);'
    +   '--ai-ink:#1e293b;--ai-strong:#0b1220;--ai-sub:#5a6b8c;--ai-foot:#7c8aa5;'
    +   '--ai-field:#ffffff;--ai-fieldink:#0f172a;--ai-chip:#33507f;'
    +   '--ai-code:#1d4ed8;--ai-shadow:rgba(15,23,42,.22);}'
    ...
    + '#aro-ai-panel{position:fixed;bottom:92px;right:22px;z-index:99991;width:min(410px,calc(100vw - 32px));height:min(600px,calc(100vh - 120px));display:none;flex-direction:column;background:linear-gradient(170deg,var(--ai-bg1),var(--ai-bg2));border:1px solid var(--ai-line);border-radius:18px;box-shadow:0 26px 70px var(--ai-shadow);overflow:hidden;font-family:"Outfit",sans-serif;}'
    ...
    + '.aro-ai-head{display:flex;align-items:center;gap:10px;padding:13px 14px;border-bottom:1px solid var(--ai-line2);background:var(--ai-bar);}'
    ...
    + '.aro-ai-head .ttl b{display:block;color:var(--ai-strong);font-size:13.5px;font-weight:600;letter-spacing:.02em;}'
    + '.aro-ai-head .ttl span{display:flex;align-items:center;gap:5px;font-size:10.5px;color:var(--ai-sub);}'
    + '.aro-ai-head .dot{width:6px;height:6px;border-radius:50%;background:#00b875;box-shadow:0 0 6px #00b875;}'
    + '.aro-ai-head button{background:none;border:none;color:var(--ai-sub);cursor:pointer;font-size:15px;padding:4px 6px;border-radius:6px;}'
    + '.aro-ai-head button:hover{color:var(--ai-strong);background:rgba(126,162,255,.12);}'
    ...
    + '.aro-ai-msg.bot{align-self:flex-start;background:var(--ai-bub);border:1px solid var(--ai-bubline);color:var(--ai-ink);border-bottom-left-radius:4px;}'
    + '.aro-ai-msg.bot b{color:var(--ai-strong);}'
    + '.aro-ai-msg.bot code{background:rgba(42,82,190,.25);padding:1px 5px;border-radius:4px;font-family:"IBM Plex Mono",monospace;font-size:11.5px;color:var(--ai-code);}'
    + '.aro-ai-msg.bot ul{margin:6px 0;padding-left:18px;}'
    + '.aro-ai-msg.bot li{margin:3px 0;}'
    + '.aro-ai-typing{align-self:flex-start;display:flex;gap:4px;padding:12px 16px;background:var(--ai-bub);border:1px solid var(--ai-bubline);border-radius:14px;border-bottom-left-radius:4px;}'
    ...
    + '.aro-ai-chip{padding:6px 11px;font-size:11px;color:var(--ai-chip);background:rgba(42,82,190,.14);border:1px solid rgba(126,162,255,.25);border-radius:999px;cursor:pointer;transition:all .13s;font-family:inherit;}'
    + '.aro-ai-chip:hover{background:rgba(255,117,56,.16);border-color:rgba(255,117,56,.4);color:#ffd9c4;}'
    + '.aro-ai-inbar{display:flex;gap:8px;padding:11px 12px;border-top:1px solid var(--ai-line2);background:var(--ai-bar);}'
    + '.aro-ai-inbar textarea{flex:1;resize:none;max-height:90px;padding:10px 12px;background:var(--ai-field);border:1px solid var(--ai-line);border-radius:11px;color:var(--ai-fieldink);font-size:12.8px;font-family:inherit;outline:none;line-height:1.45;}'
    ...
    /* Specific enough to beat '.aro-ai-inbar button', which is the send
       button's orange gradient and was claiming the paperclip as well. */
    + '.aro-ai-inbar button.aro-ai-attach{width:38px;height:42px;flex:none;align-self:flex-end;border:1px solid var(--ai-line);border-radius:11px;background:var(--ai-field);cursor:pointer;display:flex;align-items:center;justify-content:center;color:var(--ai-sub);font-size:17px;transition:all .12s;}'
    + '.aro-ai-inbar button.aro-ai-attach:hover{border-color:#ff7538;color:#ff7538;transform:none;}'
    + '.aro-ai-pend{display:flex;gap:7px;flex-wrap:wrap;padding:0 12px 8px;}'
    + '.aro-ai-thumb{position:relative;width:52px;height:52px;border-radius:9px;overflow:hidden;border:1px solid var(--ai-line);background:var(--ai-field);}'
    + '.aro-ai-thumb img{width:100%;height:100%;object-fit:cover;display:block;}'
    + '.aro-ai-thumb button{position:absolute;top:1px;right:1px;width:16px;height:16px;border:none;border-radius:50%;background:rgba(0,0,0,.66);color:#fff;font-size:10px;line-height:1;cursor:pointer;padding:0;}'
    + '.aro-ai-msg img.shot{max-width:100%;border-radius:9px;margin:5px 0 2px;display:block;border:1px solid var(--ai-bubline);}'
    + '.aro-ai-drop{position:absolute;inset:0;z-index:6;display:none;align-items:center;justify-content:center;background:rgba(255,117,56,.14);border:2px dashed #ff7538;border-radius:18px;color:#ff7538;font-size:13px;font-weight:600;pointer-events:none;}'
    + '#aro-ai-panel.dragover .aro-ai-drop{display:flex;}'
    + '.aro-ai-foot{padding:5px 14px 9px;font-size:9.5px;color:var(--ai-foot);text-align:center;letter-spacing:.04em;}'
    + '#aro-ai-settings{position:absolute;inset:0;display:none;flex-direction:column;background:var(--ai-bg2);z-index:5;padding:16px;overflow-y:auto;}'
    + '#aro-ai-settings.open{display:flex;}'
    + '#aro-ai-settings h3{margin:4px 0 14px;color:var(--ai-strong);font-size:15px;font-weight:600;display:flex;justify-content:space-between;align-items:center;}'
    + '#aro-ai-settings h3 button{background:none;border:none;color:var(--ai-sub);font-size:15px;cursor:pointer;}'
    ...
    + '.aro-ai-set-opt b{display:block;color:var(--ai-fieldink);font-size:13px;margin-bottom:3px;}'
    + '.aro-ai-set-opt span{font-size:11px;color:var(--ai-sub);line-height:1.5;display:block;}'
    ...
    + '.aro-ai-set-field input,.aro-ai-set-field select{width:100%;padding:9px 11px;background:var(--ai-field);border:1px solid var(--ai-line);border-radius:9px;color:var(--ai-fieldink);font-size:12.5px;font-family:inherit;outline:none;box-sizing:border-box;}'
    ...
    + '.aro-ai-set-note{font-size:10.5px;color:var(--ai-foot);line-height:1.55;margin-top:10px;}'
  ...
  /* ── Claude API brain ────────────────────────────────────────────── */
  function askClaude(userText, done, fail, imgs) {
    var sys = 'You are ARO AI, the expert process-engineering copilot inside AROGARA FLOWSIZE, '
    ...
    /* With images the content becomes a block list: every image first, then
       the question, which is the order the model reads best. */
    if (imgs && imgs.length) {
      var parts = [];
      imgs.forEach(function (im) {
        var m = /^data:([^;]+);base64,(.*)$/.exec(im.url || '');
        if (!m) return;
    ...
    fetch('https://api.anthropic.com/v1/messages', {
      ...
      body: JSON.stringify({ model: cfg.model, max_tokens: 1800, system: sys, messages: msgs })
    }).then(function (r) {
  ...
  function build() {
    ...
    panel.innerHTML = ''
      + '<div class="aro-ai-drop">Drop the image here</div>'
      ...
      + '<div class="aro-ai-pend" id="aro-ai-pend"></div>'
      + '<div class="aro-ai-inbar">'
      + '  <button class="aro-ai-attach" id="aro-ai-attach" title="Attach an image — a datasheet, a curve, a photo of a nameplate">\uD83D\uDCCE</button>'
      + '  <input type="file" id="aro-ai-file" accept="image/*" multiple style="display:none;">'
      + '  <textarea id="aro-ai-in" rows="1" placeholder="Ask about your design, or drop in an image…"></textarea>'
      + '  <button id="aro-ai-send" title="Send"><svg viewBox="0 0 24 24"><path d="M2 21l21-9L2 3v7l15 2-15 2v7z"/></svg></button>'
      + '</div>'
      + '<div class="aro-ai-foot" id="aro-ai-foot">ARO AI · AROGARA FLOWSIZE COPILOT</div>'
    ...
    /* ── attaching an image: three ways in ─────────────────────────── */
    var file = panel.querySelector('#aro-ai-file');
    panel.querySelector('#aro-ai-attach').addEventListener('click', function () { file.click(); });
    file.addEventListener('change', function () { addFiles(file.files); file.value = ''; });

    /* paste — the fastest route for a screenshot */
    ta.addEventListener('paste', function (e) {
      var items = (e.clipboardData || {}).items || [];
      var imgs = [].slice.call(items).filter(function (it) { return /^image\//.test(it.type); });
    ...
    ['dragleave', 'drop'].forEach(function (ev) {
      panel.addEventListener(ev, function (e) {
        if (ev === 'drop') { e.preventDefault(); addFiles(e.dataTransfer && e.dataTransfer.files); }
        if (ev === 'dragleave' && panel.contains(e.relatedTarget)) return;
        panel.classList.remove('dragover');
      });
    });

    /* removing one before it is sent */
    panel.querySelector('#aro-ai-pend').addEventListener('click', function (e) {
      var b2 = e.target.closest ? e.target.closest('[data-ai-rm]') : null;
      if (!b2) return;
      PENDING.splice(parseInt(b2.getAttribute('data-ai-rm'), 10), 1);
      renderPending();
    });
  ...
  /* ── IMAGES ────────────────────────────────────────────────────────
     A datasheet, a vendor curve, a photo of a nameplate: an engineer
     checking a value has the answer in front of them and no way to show it.
     Attached images are held as data URLs and sent alongside the question.

     They go nowhere until a question is sent, and they go only to the
     Anthropic endpoint the engineer configured with their own key. The
     built-in engine cannot read them, and says so rather than answering as
     if it had — a copilot that appears to have looked at a drawing it never
     received is worse than one that admits it cannot. */
  var PENDING = [];
  var MAX_IMG = 4, MAX_BYTES = 4.5 * 1024 * 1024;

  function renderPending() {
    var host = document.getElementById('aro-ai-pend');
    if (!host) return;
    host.innerHTML = PENDING.map(function (im, i) {
      return '<div class="aro-ai-thumb"><img src="' + im.url + '" alt="attachment">'
        + '<button data-ai-rm="' + i + '" title="Remove">\u2715</button></div>';
    }).join('');
  }

  /* Long edges beyond 1400 px are shrunk: a phone photo is several megabytes
     of pixels the model does not need, and the request would simply fail. */
  function addFiles(files) {
    [].slice.call(files || []).forEach(function (f) {
      if (!f || !/^image\//.test(f.type)) return;
      if (PENDING.length >= MAX_IMG) return;
      var fr = new FileReader();
      fr.onload = function () {
        shrink(String(fr.result), f.type, function (url) {
          if (url.length > MAX_BYTES * 1.37) {
            addBubble('bot', 'That image is too large to send even after resizing. '
              + 'A screenshot or a photo under about 4 MB will go through.', false);
            return;
          }
          PENDING.push({ url: url, name: f.name || 'image', type: /png/i.test(url.slice(0, 30)) ? 'image/png' : 'image/jpeg' });
          renderPending();
        });
      };
      fr.readAsDataURL(f);
  ...
  function shrink(dataUrl, type, cb) {
    var img = new Image();
    img.onload = function () {
      var max = 1400;
      var w = img.naturalWidth, h = img.naturalHeight;
      if (w <= max && h <= max && dataUrl.length < 1.2e6) { cb(dataUrl); return; }
  ...
  function sendCurrent() {
    ...
    if (!q) q = 'What can you tell me about this?';
    ta.value = ''; ta.style.height = 'auto';
    var imgs = PENDING.slice();
    PENDING = []; renderPending();
    send(q, imgs);
  }

  var busy = false;
  function send(q, imgs) {
    if (busy) return;
    busy = true;
    imgs = imgs || [];
    var b = addBubble('user', q, false);
    if (imgs.length && b) {
      imgs.forEach(function (im) {
        var i2 = document.createElement('img');
        i2.className = 'shot'; i2.src = im.url; i2.alt = im.name;
        b.appendChild(i2);
      });
      var m = document.getElementById('aro-ai-msgs');
      if (m) m.scrollTop = m.scrollHeight;
    }
    /* The conversation history keeps the words, not the pixels — forty turns
       of base64 would not fit in local storage and would evict everything
       else in it. */
    history.push({ role: 'user', text: q + (imgs.length ? '  [' + imgs.length + ' image'
      + (imgs.length === 1 ? '' : 's') + ' attached]' : '') });
    saveHist();
    typing(true);
    ...
    if (cfg.engine === 'claude' && cfg.apiKey) {
      askClaude(q, finish, finish, imgs);
    } else if (imgs.length) {
      /* Say what actually happened rather than answering around it. */
      setTimeout(function () {
        finish('I can see that you attached ' + imgs.length + ' image'
          + (imgs.length === 1 ? '' : 's') + ', but **the built-in engine cannot read images** — '
          + 'it answers from a fixed knowledge base on this machine, with no vision.\n\n'
          + 'Connect a Claude API key in **\u2699 Settings** and I will read the image itself: '
          + 'a datasheet, a pump curve, a P&ID extract or a photo of a nameplate.\n\n'
          + 'In the meantime, type the numbers you can see and I will work with those.');
      }, 400);
    } else {

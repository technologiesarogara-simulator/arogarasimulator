...
(function () {
  ...
  function renderProfile(card) {
    card.innerHTML = ''
      ...
      + '  <div class="aro-choice' + (state.method === 'email' ? ' sel' : '') + '" data-m="email"><span class="ci">📧</span><b>Email link</b><span>Secure sign-in link</span></div>'
  ...
  function friendlyAuthError(err) {
    ...
    if (state.firebaseStatus === 'unavailable') {
    ...
    var map = {
      ...
      'auth/quota-exceeded': 'The SMS quota is exhausted for today. Use email verification or Developer access.',
      'auth/billing-not-enabled': 'SMS verification requires the project billing plan to be enabled. Use email verification or Developer access.',
  ...
  function startSMS(fb) {
    ...
    return fb.auth().signInWithPhoneNumber(full, window.__aroRecaptcha).then(function (conf) {
      ...
      renderOTP(document.getElementById('aro-card'));
    });
  }
  ...
  function renderOTP(card) {
    ...
    card.innerHTML = ''
      ...
      + '<p class="hint">We sent an SMS to <b style="color:#dbe7ff;">' + esc(state.ccode + ' ' + state.phone) + '</b>. Enter the code below to verify your device.</p>'
      + '<div class="aro-err" id="aro-err"></div>'
    ...
    card.querySelector('#aro-verify').addEventListener('click', function () {
      ...
      state.confirmation.confirm(code).then(function () {
        finish({ demo: false, channel: 'sms' });
      }).catch(function (err) {

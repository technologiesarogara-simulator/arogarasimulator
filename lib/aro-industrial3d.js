/* ═══════════════════════════════════════════════════════════════════════════
   AROGARA — INDUSTRIAL 3D VISUALISATION LAYER
   ---------------------------------------------------------------------------
...
(function () {
  ...
  register('pump', {
    ...
    data: function () {
      ...
      /* Phase 2 of the Pump Hydraulics Advanced Upgrade (window.
         pumpAdvancedState, app.js) can rank a positive-displacement
         family top for this duty — this model still only ever draws an
         end-suction centrifugal shape (rebuilding a distinct assembly
         per PD family is a separate, much larger undertaking), so the
         caption below says so rather than leaving the mismatch silent. */
      var famSel = window.pumpAdvancedState && window.pumpAdvancedState.familySelection;
      var topFamilyCategory = (famSel && famSel.ready) ? famSel.top.category : null;
      var isPD = topFamilyCategory === 'pd-rotary' || topFamilyCategory === 'pd-reciprocating';
      return {
        ok: ok, isPD: isPD, topFamilyCategory: topFamilyCategory,
    ...
    note: function (d) {
      return 'Baseplate, foundation and coupling guard are arrangement steel shown for context — '
        ...
        + (d && d.isPD ? ' Phase 2\'s top-ranked family for this duty is a positive-displacement '
          + 'machine — this model still draws a generic centrifugal end-suction shape for '
          + 'illustration only; see the Positive-Displacement Mode section for the real safety '
          + 'screening.' : '');
    }
  });
  ...
  register('phe', {
    ...
    data: function () {
      ...
      if (!ok) {
        return { ok: false, N: 41, Lp: 1.2, Wp: 0.5, Dp: 0.15, pitch: 0.0035,
          matName: 'SS316', gasket: 'EPDM', ptype: 'Chevron (Herringbone)' };
      }
      ...
      return {
        ...
        /* The construction the engineer actually selected. Without these the
           pack was drawn in one fixed colour pair and the bill of material
           called every unit a gasketed SS316 chevron, whatever was chosen. */
        matName: r.matName, gasket: r.gasket, ptype: r.ptype, t: r.t, beta: r.beta
      };
    },
    assembly: function (d, lod) {
      var A = window.AROPARTS;
      ...
      /* The floor was frameT * 1.5, which left the two covers (frameT +
         0.8 frameT) taking more of the model's length than the pack itself,
         so the plates — the only part that carries the selected metal and
         gasket colour — read as a sliver between two dark slabs. Raising it
         gives the pack the larger share, which is also what a real frame
         looks like; a genuinely thick pack (N large / pitch large) is
         unaffected, since the true N*pitch still wins whenever it is bigger. */
      var packL = Math.max(frameT * 2.6, Math.min(d.N * d.pitch, Lp * 1.6));
      var y = Lp / 2 + 0.22;
      ...
      /* Dim levels for flow mode: the two covers are what hide the stream, so
         they go nearly clear; everything else keeps more of itself. */
      var dim = function (m, v) { m.traverse(function (o) { if (o.isMesh) o.userData.aroDim = v; }); return m; };
      g.add(dim(at(box(frameT, Lp * 1.14, Wp * 1.18, M.machineDk()), xFix, y, 0), 0.10));
      g.add(dim(at(box(pressT, Lp * 1.14, Wp * 1.18, M.machineDk()), xPress, y, 0), 0.10));
      ...
      /* Plate colour comes from the metal selected, read from aro-phe.js's own
         table so this view and the analytical one cannot disagree about what
         titanium looks like. Only the COLOUR is borrowed: that table's
         metalness suits a scene with an environment map, and per the note
         above this one has none, so the low-metalness finish stays. The
         alternating plate is the same metal shaded down — the pack reads as
         one material in two lights rather than as two different metals. */
      var AP = window.AROPHE;
      var metal = (AP && AP.matLook) ? AP.matLook(d.matName).c : 0xe2e8f0;
      function shade(hex, f2) {
        return (Math.round((hex >> 16 & 255) * f2) << 16)
             | (Math.round((hex >> 8 & 255) * f2) << 8)
             | Math.round((hex & 255) * f2);
      }
      /* Lower than the 0.25 the covers and nozzles use: by the reasoning in
         the note above, less metalness means the base colour survives more of
         the frame where no light reaches it directly, and the base colour is
         the whole point of these two. The alternating plate is shaded to 0.72
         rather than darker — enough to read as an alternation, not so much
         that half the pack goes black and takes the metal's identity with it. */
      var plateA = mat(metal, { m: 0.12, r: 0.52 });
      var plateB = mat(shade(metal, 0.72), { m: 0.12, r: 0.58 });
      /* A brazed or fully-welded pack has no gaskets to draw. Drawing them
         anyway was the clearest way to show a construction that does not
         exist. */
      var gasketed = (AP && AP.hasGasket) ? AP.hasGasket(d.ptype) : true;
      var gMat = (AP && AP.gasketLook) ? mat(AP.gasketLook(d.gasket), { m: 0.1, r: 0.85 }) : M.gasket();
      /* How the selected construction is named, on the view and in the bill
         of material below. */
      var ptypeName = String(d.ptype || 'Chevron (Herringbone)').replace(/\s*\(.*\)$/, '').toUpperCase();
      var plateSpec = String(d.matName || 'SS316').toUpperCase()
        + (isFinite(d.t) ? ' — ' + f(d.t * 1000, 1) + ' mm' : '');
      for (var i = 0; i < shown; i++) {
        var px = -packL / 2 + step * (i + 0.5);
        /* Plates fade enough to see the stream weaving between them; the
           gasket beads stay bright, so the channel each one seals is still
           readable while the flow runs. */
        g.add(dim(at(box(step * 0.55, Lp, Wp, i % 2 ? plateB : plateA), px, y, 0), 0.55));
        /* The bead is modelled as a full-face slab, so at high opacity it
           simply hides the plate behind it. Held below the plate so the pack
           still reads as metal banded with elastomer, not a block of gasket. */
        if (gasketed) g.add(dim(at(box(step * 0.22, Lp * 1.005, Wp * 1.005, gMat), px + step * 0.38, y, 0), 0.45));
      }

      /* carrying bar above, guide bar below, tie bolts around the frame */
      g.add(dim(at(box(packL + frameT * 2.6, Wp * 0.09, Wp * 0.09, M.steelDk()), 0, y + Lp * 0.58, 0), 0.85));
      g.add(dim(at(box(packL + frameT * 2.6, Wp * 0.07, Wp * 0.07, M.steelDk()), 0, y - Lp * 0.58, 0), 0.85));
      /* Tie bolts with a nut and washer on each end — the bolting is what
         holds the compression a gasketed pack needs, and a bare rod does not
         read as bolting. Thin enough to stay near-opaque in flow mode without
         hiding the stream behind them. */
      var boltR = Wp * 0.028, nutR = boltR * 1.95, bLen = packL + frameT * 2.4;
      [[1, 1], [1, -1], [-1, 1], [-1, -1]].forEach(function (c2) {
        var by = y + c2[0] * Lp * 0.42, bz = c2[1] * Wp * 0.64;
        g.add(dim(at(alongX(cyl(boltR, boltR, bLen, M.steelDk(), 10)), 0, by, bz), 0.9));
        [-1, 1].forEach(function (e3) {
          // hex nut, then a thin washer seating it against the cover
          g.add(dim(at(alongX(cyl(nutR, nutR, boltR * 1.7, M.steel(), 6)),
            e3 * (bLen / 2 - boltR * 0.85), by, bz), 0.92));
          g.add(dim(at(alongX(cyl(nutR * 1.12, nutR * 1.12, boltR * 0.5, M.steelDk(), 12)),
            e3 * (bLen / 2 - boltR * 2.2), by, bz), 0.92));
        });
      });
      ...
      /* Was Math.min(Dp, step * 0.6) * 0.7, where step is one DRAWN plate's
         slice of the pack — a fraction of the pitch, so on a 1.2 m plate the
         stream came out under 10 mm and was invisible at panel size. The
         stream runs through the ports, so the port bore is what sets it;
         capped against the plate width so a large port cannot swamp the
         model. */
      var portBore = Math.min(Dp, Wp * 0.3) * 0.7;
      asm.multiRoute = [
      ...
      /* The pack now changes colour and construction with the selection, so
         the view has to say which one it is drawing. */
      if (d.ok) {
        g.add(at(label(ptypeName + '  ·  ' + plateSpec
          + (gasketed ? '  ·  ' + String(d.gasket || 'EPDM').toUpperCase() + ' GASKET' : '  ·  GASKET-FREE'),
          { h: 0.026 }), 0, y + Lp * 0.71, 0));
      }
      if (d.ok && d.N > shown) {
        g.add(at(label('pack drawn to ' + shown + ' of ' + d.N + ' plates for legibility', { h: 0.024 }),
          0, y + Lp * 0.66, Wp * 0.9));
      }
      /* Name the construction that was selected. These lines used to read
         "GASKETED", "CHEVRON", "SS 316 / TITANIUM" and "NBR / EPDM" on every
         unit, so a titanium brazed pack was bought as a gasketed stainless
         one — the bill of material has to follow the selection. */
      asm.custom(g, {
        label: 'PLATE HEAT EXCHANGER, ' + (gasketed ? 'GASKETED' : ptypeName),
        size: d.ok ? d.N + ' PLATES' : null,
        qty: 1, unit: 'off', material: plateSpec, bom: true,
        detail: d.ok ? f(d.Lp, 3) + ' × ' + f(d.Wp, 3) + ' m plate, ' + (d.npass || 1) + ' pass' : ''
      }, { od: Math.max(d.Lp, d.Wp) });
      if (d.ok) {
        asm.item({ label: 'HEAT TRANSFER PLATE, ' + ptypeName, size: f(d.Lp, 3) + ' × ' + f(d.Wp, 3) + ' m',
          qty: d.N, unit: 'off', material: plateSpec, bom: true,
          detail: isFinite(d.beta) ? d.beta + '° chevron angle' : '' });
        /* A gasketed pack's gaskets are the parts that get ordered, fitted and
           replaced, so "PLATE GASKET SET, 1 set" told a fabricator nothing.
           One moulded gasket sits in each plate's peripheral groove: it seals
           the channel around the plate edge AND rings the two ports that must
           stay closed to that channel. Plates alternate orientation through
           the pack, so the set is two variants in roughly equal number. */
        if (gasketed) {
          var gTmax = (window.AROPHE && window.AROPHE.gasketTmax)
            ? window.AROPHE.gasketTmax[d.gasket] : null;
          asm.item({ label: 'PLATE GASKET — FIELD SEAL + PORT RINGS', qty: d.N, unit: 'off',
            material: String(d.gasket || 'EPDM').toUpperCase(), bom: true,
            detail: 'One per plate, in the peripheral groove — seals the channel and rings the two '
              + 'ports closed to it. Plates alternate A/B, so the set is two variants, '
              + Math.ceil(d.N / 2) + ' + ' + Math.floor(d.N / 2) + ' off'
              + (isFinite(gTmax) ? '. Rated to ' + gTmax + ' °C continuous' : '') });
          asm.item({ label: 'PLATE GASKET — COMMISSIONING SPARES', qty: Math.max(2, Math.ceil(d.N * 0.1)),
            unit: 'off', material: String(d.gasket || 'EPDM').toUpperCase(), bom: true,
            detail: '10 % of the pack, the usual allowance for gaskets damaged on first assembly' });
        } else {
          /* Not silent on a welded or brazed pack either — something still
             seals those channels, and it is a bought-in consumable. */
          asm.item({ label: 'SEALED WITHOUT GASKETS — ' + ptypeName, qty: d.N - 1, unit: 'channel',
            material: /Brazed/i.test(String(d.ptype)) ? 'BRAZE FILLER (COPPER / NICKEL)' : 'WELD SEAM',
            bom: true,
            detail: 'Channels are closed by the ' + (/Brazed/i.test(String(d.ptype)) ? 'braze joint' : 'welded seam')
              + ' — no elastomer, and the pack cannot be opened for mechanical cleaning' });
        }
    ...
    overlay: function (d) {
      return [
        ...
        ['CONSTRUCTION', d.ok
          ? String(d.ptype || '—').replace(/\s*\(.*\)$/, '') + '  ·  ' + (d.matName || '—')
            + (isFinite(d.t) ? ' ' + f(d.t * 1000, 1) + ' mm' : '')
            + (isFinite(d.beta) ? '  ·  β' + d.beta + '°' : '')
            + '  ·  ' + ((window.AROPHE && window.AROPHE.hasGasket && !window.AROPHE.hasGasket(d.ptype))
              ? 'gasket-free' : (d.gasket || '—') + ' gasket')
          : '—'],
  ...
  function syncButtons(id) {
    ...
    /* Both read their state out loud rather than only lighting up — GRID is
       a toggle whose OFF state is otherwise indistinguishable from a model
       that simply has no grid, and BG cycles so the name of the current one
       is the only way to know what the next press does. */
    var gb = p.wrap.querySelector('.aro3di-btn[data-a="grid"]');
    if (gb) { var gOn = p.gridOn !== false;
      gb.textContent = '▦ GRID: ' + (gOn ? 'ON' : 'OFF'); gb.classList.toggle('on', gOn); }
    var gbg = p.wrap.querySelector('.aro3di-btn[data-a="bg"]');
    if (gbg) { var ch = BGS[p.bgIdx || 0];
      gbg.textContent = '◑ BG: ' + ch.label; gbg.classList.toggle('on', (p.bgIdx || 0) !== 0); }
    ...
    /* syncButtons runs after every build, mode switch and reset, which is
       exactly when a fresh grid or a newly handed-over analytical scene would
       otherwise ignore the choice already made. Costs nothing while both are
       at their defaults. */
    reassertViewPrefs(id);
  }
  ...
  function mount(id) {
    ...
    wrap.innerHTML =
      ...
      + '<span class="aro3di-sp"></span>'
      + '<button type="button" class="aro3di-btn" data-a="zoomin" title="Zoom in — scrolling over the model does the same">ZOOM +</button>'
      + '<button type="button" class="aro3di-btn" data-a="zoomout" title="Zoom out — scrolling over the model does the same">ZOOM &minus;</button>'
      ...
      /* GRID and BG sit on the shared toolbar, so one pair covers every
         module — and because each module hands its own scene over through
         registerLegacy(), the same two buttons drive the ANALYTICAL picture
         as well as the INDUSTRIAL one. */
      + '<button type="button" class="aro3di-btn" data-a="grid" title="Show or hide the floor grid">▦ GRID</button>'
      + '<button type="button" class="aro3di-btn" data-a="bg" title="Cycle the background: theme, white, light grey, slate, black">◑ BG</button>'
    ...
    wrap.addEventListener('click', function (e) {
      ...
      if (a === 'zoomin') zoomBy(id, 1 / 1.2);
      if (a === 'zoomout') zoomBy(id, 1.2);
      if (a === 'reset') { p.detail = 'industrial'; p.flow = false; p.showBom = false;
                           p.gridOn = true; p.bgIdx = 0; applyGrid(id); applyBg(id);
      ...
      if (a === 'grid') { p.gridOn = !p.gridOn; applyGrid(id); syncButtons(id); }
      if (a === 'bg') { p.bgIdx = ((p.bgIdx || 0) + 1) % BGS.length; applyBg(id); syncButtons(id); }
  ...
  function wakeLegacy(id) {
    ...
    var existing = c.querySelector('canvas');
    if (existing) {
      /* A canvas already exists — normally nothing to do. But this app
         opens dozens of WebGL canvases across its modules, all created up
         front rather than only when their tab is visited, and browsers
         cap how many contexts can stay live at once (Chrome: 16) —
         "WARNING: Too many active WebGL contexts. Oldest context will be
         lost." fires in the console once that cap is crossed, and it is
         always one of the OLDEST contexts that pays for it. Pump's legacy
         scene is created at page load, before the engineer has touched
         anything, so it is consistently among the oldest and the most
         likely to be silently evicted long before ANALYTICAL is ever
         clicked. A lost context renders nothing — the canvas, the scene
         graph, the running animation loop all still look completely
         normal from JS, which is exactly why this reads as "ANALYTICAL
         shows a blank box" rather than an error anywhere. Checking here
         and rebuilding on a genuine loss is the standard recovery for
         exactly this browser behavior. */
  ...
  function paintOverlay(id, d) {
    ...
    p.foot.textContent = (typeof def.note === 'function' ? def.note(d) : def.note) || '';
  ...
  function frameLegacy(g, which) {
    ...
    root.traverse(function (o) {
      if (isGrid(o) || (o.userData && o.userData.aroIndicative)) return;
  ...
  /* Button-driven zoom, for a precise fixed step or a keyboard/touch-only
     device — CustomOrbitControls' own wheel handler already zooms on a
     plain scroll over the model, and this is the alternative alongside it,
  ...
  function zoomBy(id, factor) {
    ...
    if (p.mode === 'ana') {
      var lg = legacyOf(id);
      if (!lg) return;
      /* Two shapes of legacy scene reach this branch: pump/dphe/sthe's own
         CustomOrbitControls-driven rig (same shape as the industrial
         viewport, handled below), and tank/phe/line-sizing's simpler
         spherical rig with no controls object at all — same distinction
         frameLegacy() already draws for the VIEW buttons, just applied to
         zoom instead of camera angle. Without this branch the button
         silently did nothing for that second group, which is every module
         that isn't pump/dphe/sthe. */
      if (lg.controls) {
        var c = lg.controls;
        c.targetSpherical.radius = Math.max(c.minDistance, Math.min(c.maxDistance, c.targetSpherical.radius * factor));
        return;
      }
      if (lg.sph && typeof lg.place === 'function') {
        var minR = (lg.sph.minR != null) ? lg.sph.minR : 1;
        var maxR = (lg.sph.maxR != null) ? lg.sph.maxR : 400;
        lg.sph.r = Math.max(minR, Math.min(maxR, lg.sph.r * factor));
        lg.place();
      }
      return;
    }
    if (!p.vp || !p.vp.controls) return;
    var ctrl = p.vp.controls;
    ctrl.targetSpherical.radius = Math.max(ctrl.minDistance, Math.min(ctrl.maxDistance, ctrl.targetSpherical.radius * factor));
    p.vp.invalidate();
  }
  ...
  /* includeBody also dims EQUIPMENT parts. An exchanger's shell and plate
     pack are built with asm.custom(), which makes them EQUIPMENT, not
     PIPING — so without this the body stays opaque and the streamlines
     drawn through it are hidden inside solid geometry. Pumps and line
     spools take the route() path and never pass it, so they are unchanged. */
  function dimPiping(asm, on, includeBody) {
    for (var i = 0; i < asm.items.length; i++) {
      var it = asm.items[i];
      var kind = String(it.kind || '');
      if (!(PIPING.test(kind) || (includeBody && kind === 'EQUIPMENT')) || !it.group) continue;
      it.group.traverse(function (o) {
        ...
        if (on) {
          ...
          m2.transparent = true;
          /* One flat opacity washed out the parts worth looking at. A mesh can
             ask for its own: the covers go nearly clear so the stream shows
             through, while the gaskets and tie bolts stay legible — those are
             thin enough not to hide anything and are exactly what an engineer
             is checking when the flow is running. */
          m2.opacity = (o.userData && isFinite(o.userData.aroDim)) ? o.userData.aroDim : 0.34;
          m2.depthWrite = false;
          o.material = m2;
  ...
  function setFlow(id) {
    ...
    if (p.asm) dimPiping(p.asm, false, true);
    p.vp.flowTick = null;
    p.vp.dirty = true;
    ...
    p.flowObj = group;
    p.vp.root.add(group);
    /* An exchanger's streamlines run THROUGH its body, so that body has to
       go translucent or there is nothing to see — the same trade the PHE
       module's own analytical view already makes in its internal-flow mode. */
    dimPiping(p.asm, true, !!(p.asm.multiRoute && p.asm.multiRoute.length));
  ...
  function isometricSvg(id) {
    ...
    /* Per-leg length dimension — the numeric callouts ("15", "25", "10"…) a
       real piping isometric carries between every direction change: an
       extension line off the pipe, an arrowed dimension line offset from it,
       the leg's length sitting on the line. The length is read off the
       walked 3D route in real units, not the projected pixel distance —
       isometric projection foreshortens unevenly, so screen length is never
       the dimension a fabricator would cut to. */
    (function () {
      function dimArrow(px, py, dx, dy) {
        var bx = px - dx * 7, by = py - dy * 7;
        return K.path('M' + px + ' ' + py + ' L' + (bx - dy * 2.6) + ' ' + (by + dx * 2.6)
          + ' L' + (bx + dy * 2.6) + ' ' + (by - dx * 2.6) + ' Z', { fill: K.DIM, stroke: K.DIM });
      }
      for (var li = 1; li < sp.length; li++) {
        var a = sp[li - 1], b = sp[li];
        var dx = b.x - a.x, dy = b.y - a.y, segL = Math.hypot(dx, dy);
        if (segL < 18) continue;                    // too short on the sheet to dimension usefully
        var ux = dx / segL, uy = dy / segL, nx = -uy, ny = ux, off = 24;
        var a2 = { x: a.x + nx * off, y: a.y + ny * off }, b2 = { x: b.x + nx * off, y: b.y + ny * off };
        body += K.line(a.x, a.y, a2.x, a2.y, { stroke: K.DIM, w: 0.5 });
        body += K.line(b.x, b.y, b2.x, b2.y, { stroke: K.DIM, w: 0.5 });
        body += K.line(a2.x, a2.y, b2.x, b2.y, { stroke: K.DIM, w: 0.7 });
        body += dimArrow(a2.x, a2.y, -ux, -uy);
        body += dimArrow(b2.x, b2.y, ux, uy);
        var lenM = route[li].distanceTo(route[li - 1]);
        var label = f(lenM, 2) + ' m', mx = (a2.x + b2.x) / 2, my = (a2.y + b2.y) / 2;
        var lw = String(label).length * 5.2 + 10;
        body += '<rect x="' + (mx - lw / 2) + '" y="' + (my - 8) + '" width="' + lw + '" height="11" fill="#ffffff"/>';
        body += K.txt(mx, my + 1, label, { anchor: 'middle', size: 8, fill: K.DIM });
      }
    })();
  ...
  /* ── GRID AND BACKGROUND ────────────────────────────────────────────────
     Both act on the INDUSTRIAL viewport and on the module's own ANALYTICAL
     scene, because the two share this toolbar and it would be strange for a
     button to work on one picture and not the other. 'theme' is the default
     and hands the background back to whatever the light/dark setting says,
     so the control adds choices without taking the existing behaviour away. */
  var BGS = [
    { k: 'theme', label: 'THEME' },
    { k: 0xffffff, label: 'WHITE' },
    { k: 0xe9edf1, label: 'LIGHT' },
    { k: 0x2b323b, label: 'SLATE' },
    { k: 0x000000, label: 'BLACK' }
  ];
  /* THREE.GridHelper sets no isGridHelper flag — it identifies itself through
     type. Every check in this file used the flag, so none of them ever matched:
     the floor grid was silently never recoloured with the theme, and never
     excluded from the camera fit either. */
  function isGrid(o) { return !!o && (o.isGridHelper || o.type === 'GridHelper'); }
  /* GRID has to hide whatever a module actually stands on, and not every one
     uses a GridHelper: the pump and the line-sizing services draw a named
     plant-grade plane instead. Kept separate from isGrid() because the other
     two call sites must not treat that plane as a grid — the theme code gives
     it its own deck colour, and blending the two would repaint the floor with
     the grid's colour. */
  function isFloor(o) { return isGrid(o) || (!!o && o.name === 'aro-grade-plane'); }
  function eachScene(id, fn) {
    var p = PANELS[id];
    if (p && p.vp && p.vp.scene) fn(p.vp.scene);
    try { var g = legacyOf(id); if (g && g.scene) fn(g.scene); } catch (e) {}
  }
  function applyGrid(id) {
    var p = PANELS[id]; if (!p) return;
    var on = p.gridOn !== false;
    eachScene(id, function (sc) {
      sc.traverse(function (o) { if (isFloor(o)) o.visible = on; });
    });
    if (p.vp) p.vp.dirty = true;
  }
  function applyBg(id) {
    var p = PANELS[id]; if (!p) return;
    if (!T && typeof THREE !== 'undefined') T = THREE;
    if (!T) return;
    var choice = BGS[p.bgIdx || 0];
    if (choice.k === 'theme') { themeLegacyScenes(sceneBg(), id);
      if (p.vp && p.vp.scene) p.vp.scene.background = new T.Color(sceneBg()); }
    else eachScene(id, function (sc) { sc.background = new T.Color(choice.k); });
    if (p.vp) p.vp.dirty = true;
  }
  /* A rebuild makes a fresh grid, and a module can hand its analytical scene
     over at any time — so the chosen state has to be re-asserted rather than
     applied once at click time. */
  function reassertViewPrefs(id) {
    var p = PANELS[id]; if (!p) return;
    if (p.gridOn === false) applyGrid(id);
    if (p.bgIdx) applyBg(id);
  }

...
(function () {
  ...
  register('pump', {
    ...
    assembly: function (d, lod) {
      ...
      /* Held at least two label-heights above the DISCHARGE callout — the
         two that were found sitting on top of each other, merged into
         unreadable text, on a 1/2" pump. Two label-heights of clearance is
         more than either sprite's own height, so the two cannot touch
         regardless of how small the nozzle that used to set this gap is. */
      R.add(at(tagLabel(d.tag), px - casR * 1.1, disLabelY + Lh * 2.6, 0));
  ...
  function setFlow(id) {
    ...
    if (p.flowObj) {
      ...
      if (p.flowObj.geometry) p.flowObj.geometry.dispose();
      if (p.flowObj.material) p.flowObj.material.dispose();
      p.flowObj = null;
    }
    ...
    var trueSpan = p.asm.trueLength ? p.asm.trueLength : span;

    /* the column sits inside the bore, not around the outside of it */
    ...
    /* one band roughly every two diameters, so the streaks read as flow at
       any line size instead of stretching into stripes on a long run */
    tex.repeat.set(Math.max(4, Math.round(span / Math.max(0.05, bore * 2))), 1);

    var m = new T.MeshBasicMaterial({
      color: look.c, map: tex, transparent: true, opacity: look.o,
    ...
    /* Speed from the calculated velocity, so a faster line looks faster and
       the ratio between two designs is the ratio of their V. The texture is
       scrolled rather than the geometry moved — the column stays where the
       pipe is and only the material inside it travels. */
    var d0 = p.lastData || {};
    var vRef = isFinite(d0.V) && d0.V > 0 ? d0.V
             : (isFinite(d0.vs) && d0.vs > 0 ? d0.vs : 1);
    var rate = Math.max(0.05, Math.min(1.6, vRef / Math.max(0.5, trueSpan))) * tex.repeat.x;
    p.vp.flowTick = function (t) {
      tex.offset.x = -(t * rate) % 1;
    };
    p.flowLook = look;
  }

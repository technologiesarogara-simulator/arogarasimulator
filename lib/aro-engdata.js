/* ═══════════════════════════════════════════════════════════════════════════
   AROGARA — ENGINEERING DATA REGISTRY AND PROJECT DESIGN BASIS
   ---------------------------------------------------------------------------
...
(function () {
  ...
  var SI = {
    ...
    /* Roughness is not one property. The tank table holds absolute hydraulic
       roughness in millimetres — the Moody values a friction factor is read
       from. The plate table holds a manufactured plate finish, and its own
       header says micrometres. Comparing them reported a 333x disagreement
       between two numbers that are both correct and measure different
       things. They are separate properties now, and never compared. */
    epsHyd: { unit: 'mm', to: function (v) { return v; } },
    finishUm: { unit: 'µm', to: function (v) { return v; } },
  ...
  var LABEL = {
    ...
    k: 'Thermal conductivity', S: 'Allowable stress',
    epsHyd: 'Absolute hydraulic roughness \u03b5', finishUm: 'Plate surface finish',
  ...
  function normKey(name) {
    /* A parenthetical is sometimes a condition and sometimes a grade.
       "(80\u00b0C)" is a condition and folding it away is right; "(Light)" is a
       grade, and folding that away merged Crude Oil (Light) at 5 cP with
       Crude Oil (Heavy) at 50 cP and then reported the two as one substance
       disagreeing with itself tenfold. Keep the grade in the identity. */
    /* The common library owns the identity rules — it also folds "liq" onto
       "liquid", so both panels count the same substances. Fall back to the
       local rule when it has not loaded. */
    if (window.AROENGLIB && window.AROENGLIB.parse) {
      try { return window.AROENGLIB.parse(name).key; } catch (e) {}
    }
    return String(name).toLowerCase()
      .replace(/\(([^)]*)\)/g, function (all, inner) {
        return /[0-9]/.test(inner) ? ' ' : ' ' + inner + ' ';
      })
      .replace(/[^a-z0-9]+/g, ' ')
      .trim();
  }
  ...
  function html() {
    ...
    /* The three tables that used to sit here — TABLE CONSISTENCY, FLUID
       PROPERTIES and MATERIAL PROPERTIES — have been removed from this panel
       on request. Nothing behind them is gone: the registry still indexes
       every table, still normalises into SI and still cross-checks, and all
       of it is reachable in the Engineering Data tab, which presents the same
       data per substance with its condition, source and status rather than as
       three long lists. This panel is now the design basis alone. */
    h += '</div>';
    return h;
  }
  ...
  /* The Project Design Basis panel is withdrawn from the UI at the user's
     request. render() is left as a deliberate no-op rather than deleting
     the module: window.AROENGDATA below (fluids()/materials()/basis()) is
     a real dependency of other modules (aro-datalib.js, aro-datalib-ui.js,
     aro-projecttabs.js, aro-userlib.js, aro-impact.js, aro-registers.js) —
     only the DOM mount is disabled, the data API those read stays intact. */
  function render() {}

...
(function () {
  ...
  function css() {
    ...
    s.textContent = [
      ...
      /* fixed + appended to <body> — a "portal" — so no ancestor's
         overflow:hidden (e.g. .panel) can clip the popup off-screen;
         left/top/width are computed from the select's own bounding box
         each time it opens and kept in sync while it stays open. */
      '.aro-pick-pop{position:fixed;z-index:100040;margin-top:2px;',
      ...
      '.aro-pick-list{overflow:auto;flex:1 1 auto;min-height:0;}',
  ...
  function enhance(sel) {
    ...
    function reposition() {
      ...
      pop.style.left = Math.max(0, r.left) + 'px';
      pop.style.width = r.width + 'px';
      if (spaceBelow < 160 && spaceAbove > spaceBelow) {
        pop.style.top = '';
        pop.style.bottom = (window.innerHeight - r.top + 2) + 'px';
        pop.style.maxHeight = Math.max(120, Math.min(280, spaceAbove)) + 'px';
      } else {
        pop.style.bottom = '';
        pop.style.top = (r.bottom + 2) + 'px';
        pop.style.maxHeight = Math.max(120, Math.min(280, spaceBelow)) + 'px';
      }
    }
    function close() {
      if (!pop) return;
    ...
    function open() {
      ...
      /* portal: append to <body>, not the select's own (possibly
         overflow:hidden) ancestor, then position it over the select */

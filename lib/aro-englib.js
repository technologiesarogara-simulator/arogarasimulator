/* ═══════════════════════════════════════════════════════════════════════════
   AROGARA — COMMON ENGINEERING LIBRARY  (window.AROENGLIB)
   ---------------------------------------------------------------------------
...
(function () {
  ...
  /* The Common Engineering Library browser is withdrawn from the UI at the
     user's request. render() is left as a deliberate no-op rather than
     deleting the module: window.AROENGLIB below (records()/search()/
     toSI()/parse()/etc.) is a real dependency of other modules
     (aro-datalib.js's ARODATA registry build, aro-datalib-ui.js,
  ...
  /* ══ 12 · EXPORTS ═══════════════════════════════════════════════════════ */
  /* The panel is rebuilt when the project tab comes back into view: what it
     reports depends on module state that can move while it is hidden.
     Deferred, because the click that switches tabs arrives before the tab is
     visible; guarded by the signature, so a click that changes nothing costs
     a string comparison. */

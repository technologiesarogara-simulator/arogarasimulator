...
(function () {
  ...
  var STATUS = {
    ...
    /* calc:true as of the engineer's explicit decision: a legacy value with a
       recorded condition and source may feed a calculation, carrying this
       note as a visible caveat wherever it's used — "screening grade," not
       silently promoted to something it isn't. */
    'REFERENCE ONLY': { rank: 3, calc: true,
    ...
    /* Rank orders canonical selection, and it puts everything a calculation
       may legitimately use above everything it may not. An engineer's sourced
       entry outranking an unchecked legacy table is the point: the alternative
       is resolving to a value the library itself refuses to let anyone use. */
    'USER INPUT': { rank: 2, calc: true,
      note: 'Entered by an engineer on this project, with the source they cited. It may feed a '
          + 'calculation because an engineer took responsibility for it — it is never promoted '
          + 'to reference data, and it stays labelled wherever it appears.' },
    'USER SUPPLIED': { rank: 4, calc: false,
      note: 'Entered by an engineer on this project. Never treated as reference data.' },
    'PROJECT OVERRIDE': { rank: 1, calc: true,
      note: 'This project uses a different value from the library, with a stated reason. The '
          + 'library record is unchanged and still visible underneath.' },
    'MODULE OVERRIDE': { rank: 1, calc: true,
      note: 'One design object uses a different value from the project, with a stated reason. '
          + 'It applies to that object only.' },
    'IMPORTED — PENDING REVIEW': { rank: 5, calc: false,
      note: 'Arrived through the import path and has not been through engineering review. '
          + 'Visible, searchable, and not yet cleared to feed a calculation.' },
    /* Same decision as REFERENCE ONLY above, extended to this weaker case —
       the condition isn't even recorded, so the note carries a correspondingly
       stronger caveat. Still ranked below REFERENCE ONLY (6 vs 3): when both
       exist for the same property, the one with a recorded condition wins. */
    'CONDITION INCOMPLETE': { rank: 6, calc: true,
      note: 'The value exists but the condition it applies at is not recorded.' },
    'CONFLICT': { rank: 7, calc: false,
      note: 'Two sources disagree. Both are kept; neither is used until one is chosen.' },
    'UNVERIFIED': { rank: 8, calc: false, note: 'No source recorded.' },
    'SUPERSEDED': { rank: 9, calc: false,
      note: 'Replaced by a later record. Kept so the earlier design decision stays legible.' },
    'DEPRECATED': { rank: 10, calc: false, note: 'Superseded. Kept for traceability.' },
    'NOT AVAILABLE': { rank: 11, calc: false,
  ...
  /* Each form declares the fields its editor must collect. The editor is
     generated from this rather than showing one box for everything, because
     a text box that accepts "16.2 at about 100 C, maybe" is how a library
     stops being a database. */
  var FORM_FIELDS = {
    CONSTANT: { fields: ['value', 'unit'],
      hint: 'One number at one stated condition.' },
    RANGE: { fields: ['min', 'max', 'unit'],
      hint: 'A published minimum and maximum — a specification band, not a tolerance on one number.' },
    TABULAR: { fields: ['xProperty', 'xUnit', 'points', 'unit'],
      hint: 'Values against an independent variable, normally temperature. Interpolated, never extrapolated.' },
    CURVE: { fields: ['xProperty', 'xUnit', 'points', 'unit', 'curveLabel'],
      hint: 'A digitised curve. Same shape as a table, and labelled as read off a chart so its precision is not overstated.' },
    CORRELATION: { fields: ['expression', 'variables', 'validity', 'unit'],
      hint: 'A named, cited expression with its range of validity. Expressions are not invented here.' },
    CATEGORICAL: { fields: ['categorical', 'scale'],
      hint: 'A rating on a controlled scale, so it can be compared and sorted.' },
    TEXT: { fields: ['text'],
      hint: 'A short descriptive entry — a hardness designation, a heat treatment note. Not a place for a value.' }
  };
  ...
  /* Properties an engineer added to the dictionary join the same list. They
     declare a quantity like any other, so they are stored canonically and
     convert on display — a user property is not an excuse for a loose number
     in an unstated unit. */
  var PLIST = P.slice();
  function refreshProps() {
    PLIST = P.slice();
    try {
      var st = window.ARODATASTORE;
      if (!st) return;
      st.userProps().forEach(function (p) {
        if (PROPS[p.key] && !PROPS[p.key].userDefined) return;   /* never shadow a dictionary property */
        var row = [p.key, p.label, p.domain || 'USER', p.qty || 'dimensionless', p.applies || 'both'];
        PROPS[p.key] = { key: row[0], label: row[1], domain: row[2], qty: row[3],
          applies: row[4], userDefined: true, note: p.note || null };
        PLIST.push(row);
      });
    } catch (e) {}
  }
  ...
  function propsInDomain(domain, kind) {
    return PLIST.filter(function (r) {
  ...
  var SOURCE_TYPES = ['PRIMARY AUTHORITATIVE', 'GOVERNMENT REFERENCE', 'MANUFACTURER DATASHEET',
    ...
    'SECONDARY REFERENCE', 'PROJECT DATASHEET', 'VENDOR TEST CERTIFICATE',
    'MEASURED ON SITE', 'USER SUPPLIED',
    /* The honest name for a number that was already inside this application
       when the library was built. It is not a source in any engineering
       sense; it says only "this software has been using this figure". */
    'LEGACY MODULE VALUE', 'SOFTWARE TABLE'];
  ...
  function value(o) {
    ...
    var rec = {
      ...
      scale: o.scale || null,
      text: o.text || null,
      curveLabel: o.curveLabel || null,
      condition: condition(o.condition),
      source: source(o.source),
      status: o.status || 'UNVERIFIED',
      note: o.note || null,
      /* Which layer of the hierarchy this record belongs to. Master records
         say so; overrides carry the layer they were entered at. */
      layer: o.layer || 'MASTER LIBRARY',
  ...
  /* Source names that describe one of this application's own property tables
     rather than a publication. A value carrying one of these has exactly one
     provenance claim behind it — "this software has been using this figure" —
     and the library says so. */
  var LEGACY_SOURCES = [
    'Heat-exchanger design reference property tables',
  ...
  function build() {
    ...
    if (!L) { mergeStore(); return; }
    var recs = [];
    try { recs = L.records(); } catch (e) { mergeStore(); return; }
    ...
    recs.forEach(function (r) {
      ...
      var v = value({
        ...
        source: {
          ...
          sourceType: !r.engSource || r.engSource === NOT_STATED ? 'LEGACY MODULE VALUE'
            /* These two "sources" are the names of tables inside this
               application. They are not documents anyone can look up, and
               calling them SECONDARY REFERENCE dressed a legacy in-app number
               up as a cited one — 424 of the 516 values held, which is most
               of the library describing itself as better sourced than it is.
               Moody/Colebrook and ASME II-D stay as they are: those name real
               published work. */
            : LEGACY_SOURCES.indexOf(r.engSource) >= 0 ? 'LEGACY MODULE VALUE'
            : r.engType === 'CODE' ? 'LICENSED STANDARD'
  ...
  /* Subjects and values an engineer created through the library itself. They
     join the master layer as records of their own — they never overwrite a
     migrated record, and where both exist for one property the pair is a
     CONFLICT to be resolved, not an average to be taken. */
  function mergeStore() {
    var st = window.ARODATASTORE;
    if (!st) return;
    if (!SUBJECTS) { SUBJECTS = {}; VALUES = {}; }
    try {
      st.userSubjects().forEach(function (u) {
        if (u.compositionOnly) return;
        if (SUBJECTS[u.id]) return;
        SUBJECTS[u.id] = subject({
          id: u.id, kind: u.kind, name: u.name, family: u.family,
          identity: u.identity || { preferredName: u.name },
          origin: u.copiedFrom ? 'USER DEFINED — COPIED' : 'USER DEFINED'
        });
        VALUES[u.id] = VALUES[u.id] || {};
      });
      st.userValues().forEach(function (r) {
        if (!SUBJECTS[r.subjectId]) return;
        if (!PROPS[r.property]) return;
        var v = value({
          id: r.id, subjectId: r.subjectId, property: r.property,
          form: r.form, si: r.si, siMin: r.siMin, siMax: r.siMax,
          original: r.original, originalUnit: r.originalUnit,
          table: r.table, xProperty: r.xProperty, xUnit: r.xUnit,
          correlation: r.correlation, categorical: r.categorical, text: r.text,
          curveLabel: r.curveLabel,
          condition: r.condition, source: r.source,
          status: r.status || 'USER INPUT',
          note: r.note, layer: 'MASTER LIBRARY', origin: 'USER ENTERED',
          by: r.createdBy, at: r.createdAt
        });
        VALUES[r.subjectId] = VALUES[r.subjectId] || {};
        (VALUES[r.subjectId][r.property] = VALUES[r.subjectId][r.property] || []).push(v);
      });
    } catch (e) {}
  }
  ...
  function propertiesOf(id, domain) {
    ...
    var list = domain ? propsInDomain(domain, s.kind)
      : PLIST.filter(function (r) { return r[4] === 'both' || r[4] === s.kind; })
        .map(function (r) { return PROPS[r[0]]; });
  ...
  var MODULES = {
    ...
    hx: { label: 'Heat Exchanger',
      ...
      /* A fixed-tubesheet exchanger is a differential-expansion problem before
         it is a heat transfer one, so the elastic constants belong here beside
         the conductivity. Listing them changes what the library offers and
         nothing in the exchanger calculation. */
      material: ['k', 'density', 'cte', 'S', 'E', 'yield', 'corrosion'] },
  ...
  /* ══ 13b · MODULE INPUTS ════════════════════════════════════════════════
     What each module will actually accept, so a mapping is a named pair —
     AROGARA PROPERTY → MODULE INPUT — and not a hope that the two ends mean
     the same thing. Nothing here writes into a module: this is the contract
     an engineer confirms before any value is allowed to travel. */
  var MODULE_INPUTS = {
    pump: [
      ['fluid-density', 'Fluid density', 'density', 'kg/m³'],
      ['fluid-sg', 'Specific gravity', 'sg', '—'],
      ['fluid-visc', 'Dynamic viscosity', 'mu', 'cP'],
      ['fluid-vp', 'Vapour pressure', 'pvap', 'bar'],
      ['pipe-rough', 'Pipe roughness ε', 'epsHyd', 'mm']
    ],
    line: [
      ['ls-density', 'Fluid density', 'density', 'kg/m³'],
      ['ls-visc', 'Dynamic viscosity', 'mu', 'cP'],
      ['ls-kinvisc', 'Kinematic viscosity', 'nuKin', 'cSt'],
      ['ls-vp', 'Vapour pressure', 'pvap', 'bar'],
      ['ls-z', 'Compressibility factor', 'z', '—'],
      ['ls-rough', 'Pipe roughness ε', 'epsHyd', 'mm']
    ],
    hx: [
      ['hx-rho', 'Fluid density', 'density', 'kg/m³'],
      ['hx-cp', 'Specific heat', 'cp', 'kJ/kg·K'],
      ['hx-mu', 'Dynamic viscosity', 'mu', 'cP'],
      ['hx-k', 'Fluid thermal conductivity', 'k', 'W/m·K'],
      ['hx-tube-k', 'Tube wall conductivity', 'k', 'W/m·K'],
      ['hx-mat-S', 'Allowable stress', 'S', 'MPa'],
      ['hx-mat-cte', 'Thermal expansion', 'cte', 'µm/m·K'],
      ['hx-mat-E', "Young's modulus", 'E', 'GPa'],
      ['hx-mat-yield', 'Yield strength', 'yield', 'MPa']
    ],
    tank: [
      ['tk-rho', 'Stored fluid density', 'density', 'kg/m³'],
      ['tk-sg', 'Specific gravity', 'sg', '—'],
      ['tk-S', 'Allowable stress', 'S', 'MPa'],
      ['tk-yield', 'Yield strength', 'yield', 'MPa'],
      ['tk-E', "Young's modulus", 'E', 'GPa'],
      ['tk-ca', 'Corrosion allowance', 'corrAllow', 'mm']
    ],
    workbench: [
      ['wb-rho', 'Line fluid density', 'density', 'kg/m³'],
      ['wb-mu', 'Line fluid viscosity', 'mu', 'cP'],
      ['wb-rough', 'Pipe roughness ε', 'epsHyd', 'mm']
    ],
    general: []
  };
  function moduleInputs(moduleKey) {
    return (MODULE_INPUTS[moduleKey] || []).map(function (r) {
      return { key: r[0], label: r[1], property: r[2], unit: r[3] };
    });
  }
  /* The input this module would naturally take this property on. Offered as
     a default in the mapping dialog and always shown, never applied silently. */
  function suggestedInput(moduleKey, propKey) {
    var hit = null;
    moduleInputs(moduleKey).forEach(function (i) { if (!hit && i.property === propKey) hit = i; });
    return hit;
  }

  /* ══ 13c · PROPERTY TEMPLATES ═══════════════════════════════════════════
     What an engineer is normally expected to record for a kind of subject.
     A template creates EMPTY rows to fill — it never creates values. Its
     whole purpose is to make the gaps visible on a new material rather than
     leaving an engineer to remember which properties a pressure part needs. */
  var TEMPLATES = {
    'PRESSURE-RETAINING METAL': { kind: 'material',
      props: ['density', 'E', 'nu', 'yield', 'tensile', 'S', 'k', 'cp', 'cte',
        'maxService', 'corrAllow', 'weldability', 'corrosion'],
      note: 'What a pressure part is normally specified on.' },
    'PIPING MATERIAL': { kind: 'material',
      props: ['density', 'epsHyd', 'S', 'yield', 'maxService', 'corrosionRate', 'weldability'],
      note: 'What line sizing and pipe wall thickness ask for.' },
    'HEAT TRANSFER SURFACE': { kind: 'material',
      props: ['density', 'k', 'cp', 'cte', 'S', 'maxService', 'corrosion'],
      note: 'What an exchanger asks of its tube or plate material.' },
    'INSULATION': { kind: 'material',
      props: ['density', 'k', 'cp', 'maxService', 'moisture'],
      note: 'What a heat loss calculation asks for.' },
    'PROCESS LIQUID': { kind: 'fluid',
      props: ['density', 'sg', 'mu', 'nuKin', 'cp', 'k', 'pvap', 'sigma', 'boilingPoint', 'ph'],
      note: 'What hydraulics and thermal duty both need.' },
    'PROCESS GAS': { kind: 'fluid',
      props: ['density', 'mw', 'mu', 'cp', 'cv', 'gamma', 'k', 'z', 'tcrit', 'pcrit'],
      note: 'What compressible flow needs.' },
    'FLAMMABLE FLUID': { kind: 'fluid',
      props: ['density', 'mu', 'pvap', 'flashPoint', 'autoignition', 'lel', 'uel', 'hazard'],
      note: 'Adds the safety properties a flammable service is specified on.' }
  };
  function templatesFor(kind) {
    return Object.keys(TEMPLATES).filter(function (k) {
      return !kind || TEMPLATES[k].kind === kind;
    }).map(function (k) {
      return { name: k, kind: TEMPLATES[k].kind, props: TEMPLATES[k].props,
        note: TEMPLATES[k].note };
    });
  }

  /* ══ 13d · RESOLUTION THROUGH THE HIERARCHY ═════════════════════════════
     The single place that answers "what value would a calculation see?", and
     the single place that can answer "and which layer did it come from?".
     Nothing else in the library is permitted to decide this, because a
     resolution rule that lives in two places is a resolution rule that
     disagrees with itself the week after it is changed. */
  var LAYERS = ['MASTER LIBRARY', 'PROJECT SELECTION', 'PROJECT OVERRIDE',
    'MODULE MAPPING', 'MODULE OVERRIDE', 'CALCULATION'];

  function masterValues(subjectId, propKey) {
    ensure();
    return ((VALUES[subjectId] || {})[propKey] || []).slice();
  }

  /* Two records for one property are not automatically a disagreement. Two
     values at DIFFERENT conditions are a temperature series, and combining
     them is the useful thing to do. Two values at the SAME condition that do
     not agree is a conflict, and neither is used until an engineer chooses.
     Getting this distinction wrong in either direction is damaging: call a
     series a conflict and the library blocks good data; call a conflict a
     series and it silently interpolates between two sources. */
  function conflictAmong(vs) {
    if (!vs || vs.length < 2) return false;
    var byCond = {};
    vs.forEach(function (v) {
      var c = conditionSummary(v.condition);
      (byCond[c] = byCond[c] || []).push(v);
    });
    var clash = false;
    Object.keys(byCond).forEach(function (c) {
      var g = byCond[c];
      for (var i = 1; i < g.length; i++) {
        var a = g[0].si, b = g[i].si;
        if (a == null || b == null) continue;
        if (Math.abs(a - b) > Math.abs(a || b) * 1e-6) clash = true;
      }
    });
    return clash;
  }

  /* Several single values, each at its own stated temperature, read as a
     series — which is what they are. Built for display only: no record is
     merged, rewritten or deleted to produce it. */
  function combineSeries(vs) {
    if (!vs || vs.length < 2) return null;
    var pts = [];
    vs.forEach(function (v) {
      if (v.form !== FORM.CONSTANT || v.si == null) return;
      var t = v.condition && v.condition.temperature;
      var n = parseFloat(t);
      if (!isFinite(n)) return;
      pts.push([n, v.si, v.condition.temperatureUnit || '°C']);
    });
    if (pts.length < 2) return null;
    var unit = pts[0][2];
    if (!pts.every(function (p) { return p[2] === unit; })) return null;
    pts.sort(function (a, b) { return a[0] - b[0]; });
    /* Only a series if the temperatures actually differ. */
    if (pts[0][0] === pts[pts.length - 1][0]) return null;
    return { xUnit: unit, table: pts.map(function (p) { return [p[0], p[1]]; }) };
  }

  function resolve(subjectId, propKey, opts) {
    opts = opts || {};
    ensure();
    var st = window.ARODATASTORE;
    var p = PROPS[propKey] || { key: propKey, qty: 'dimensionless', label: propKey };
    var master = masterValues(subjectId, propKey);
    var masterPick = master.length ? pick(master) : null;

    var lineage = [];
    function step(layer, state, detail, val) {
      lineage.push({ layer: layer, state: state, detail: detail || null, value: val || null });
    }

    step('MASTER LIBRARY', masterPick ? 'HELD' : 'NOT AVAILABLE',
      masterPick ? master.length + ' record' + (master.length === 1 ? '' : 's')
        + ' · ' + masterPick.status : 'No traceable value held', masterPick);

    var selected = false, projOv = null, mapping = null, modOv = null;
    if (st) {
      try {
        selected = st.mappings().some(function (m) { return m.subjectId === subjectId; })
          || selection().some(function (x) { return x.subjectId === subjectId; });
        projOv = st.projectOverride(subjectId, propKey);
        if (opts.mappingId) {
          st.mappings().forEach(function (m) { if (m.id === opts.mappingId) mapping = m; });
          modOv = st.moduleOverride(opts.mappingId, propKey);
        }
      } catch (e) {}
    }

    step('PROJECT SELECTION', selected ? 'SELECTED' : 'NOT SELECTED',
      selected ? 'This project has taken this subject from the library'
        : 'Not yet chosen for this project', null);
    step('PROJECT OVERRIDE', projOv ? 'OVERRIDDEN' : 'NONE',
      projOv ? projOv.reason : 'The project uses the library value', projOv);

    if (opts.mappingId) {
      var paired = mapping && mapping.pairs.filter(function (x) { return x.property === propKey; })[0];
      step('MODULE MAPPING', mapping ? (paired ? mapping.status : 'NOT MAPPED') : 'NONE',
        mapping ? ((paired ? '→ ' + (paired.inputLabel || paired.input) : 'this property is not mapped')
          + (mapping.statusReason ? ' · ' + mapping.statusReason : '')) : null, null);
      step('MODULE OVERRIDE', modOv ? 'OVERRIDDEN' : 'NONE',
        modOv ? modOv.reason : 'This object uses the project value', modOv);
    }

    var effective = modOv || projOv || masterPick || null;
    var effLayer = modOv ? 'MODULE OVERRIDE' : (projOv ? 'PROJECT OVERRIDE'
      : (masterPick ? 'MASTER LIBRARY' : 'NONE'));
    var clash = conflictAmong(master);
    var status = effective ? (effective.status || 'UNVERIFIED') : 'NOT AVAILABLE';
    /* An unresolved disagreement in the master layer is reported as one, even
       though a record was selected to display — the selection is a display
       choice and must not read as a settled answer. An override settles it,
       because that is exactly what an override is for. */
    if (clash && !modOv && !projOv) status = 'CONFLICT';

    return {
      subjectId: subjectId, property: propKey, prop: p,
      master: master, masterPick: masterPick,
      projectOverride: projOv, mapping: mapping, moduleOverride: modOv,
      effective: effective, layer: effLayer, status: status,
      usableInCalc: canCalculate(status),
      conflict: clash,
      records: master.length,
      series: (!modOv && !projOv) ? combineSeries(master) : null,
      lineage: lineage
    };
  }

  /* ══ 13e · COMPLETENESS ═════════════════════════════════════════════════
     Not a score to be optimised. It is a statement of how much of what this
     subject could carry is actually recorded, so an engineer knows whether
     they are looking at a characterised material or a name with a density. */
  function completeness(subjectId) {
    ensure();
    var s = SUBJECTS[subjectId];
    if (!s) return null;
    var byDomain = domainCounts(subjectId);
    var total = 0, held = 0;
    byDomain.forEach(function (d) { total += d.total; held += d.held; });
    return {
      total: total, held: held, missing: total - held,
      pct: total ? Math.round(held / total * 100) : 0,
      byDomain: byDomain,
      /* Which of the properties the design modules actually consume are
         present — far more useful than the headline percentage, because a
         material with 8 of 8 piping properties is complete for piping
         however little else it carries. */
      forModules: Object.keys(MODULES).filter(function (m) {
        return relevantFor(m, s.kind);
      }).map(function (m) {
        var need = relevantFor(m, s.kind) || [];
        var have = need.filter(function (k) { return masterValues(subjectId, k).length > 0; });
        return { module: m, label: MODULES[m].label, need: need.length, have: have.length,
          missing: need.filter(function (k) { return masterValues(subjectId, k).length === 0; }) };
      })
    };
  }

  /* ══ 13f · COMPARE ══════════════════════════════════════════════════════
     Two to five subjects side by side on the same properties, in one unit,
     with each cell carrying its own status. Cells that differ are marked;
     they are never reconciled, because two grades legitimately differ and
     the library has no business deciding which one the design wants. */
  function compare(ids, propKeys) {
    ensure();
    var subs = (ids || []).map(function (i) { return SUBJECTS[i]; }).filter(Boolean);
    if (subs.length < 2) return null;
    var keys = propKeys && propKeys.length ? propKeys : null;
    if (!keys) {
      /* Default to every property at least one of them holds. */
      var seen = {};
      subs.forEach(function (s) {
        Object.keys(VALUES[s.id] || {}).forEach(function (k) { seen[k] = true; });
      });
      keys = PLIST.map(function (r) { return r[0]; }).filter(function (k) { return seen[k]; });
    }
    return {
      subjects: subs,
      rows: keys.map(function (k) {
        var p = PROPS[k] || { key: k, label: k, qty: 'dimensionless' };
        var cells = subs.map(function (s) {
          var r = resolve(s.id, k);
          return { subjectId: s.id, value: r.effective, status: r.status,
            layer: r.layer, si: r.effective ? r.effective.si : null };
        });
        var nums = cells.map(function (c) { return c.si; }).filter(function (v) {
          return v != null && isFinite(v);
        });
        var differs = nums.length > 1
          && (Math.max.apply(null, nums) - Math.min.apply(null, nums)) > 1e-12;
        var gap = nums.length > 1 && Math.min.apply(null, nums) !== 0
          ? (Math.max.apply(null, nums) - Math.min.apply(null, nums)) / Math.abs(Math.min.apply(null, nums)) : null;
        return { prop: p, cells: cells, differs: differs, spread: gap,
          heldBy: cells.filter(function (c) { return c.value; }).length };
      })
    };
  }

  /* ══ 13g · IMPACT ═══════════════════════════════════════════════════════
     Before a value changes: what is currently pointing at it. This does not
     recalculate anything and does not predict a result — it lists the design
     objects whose mapping would go OUTDATED, which is a fact rather than a
     forecast. */
  function impactOf(subjectId, propKey) {
    var st = window.ARODATASTORE;
    var maps = st ? st.mappingsFor(subjectId, propKey) : [];
    var sels = selection().filter(function (x) {
      return x.subjectId === subjectId && (!propKey || x.properties.indexOf(propKey) >= 0);
    });
    return {
      mappings: maps.map(function (m) {
        return { id: m.id, object: m.object, module: m.moduleLabel, status: m.status,
          pairs: m.pairs.filter(function (p) { return !propKey || p.property === propKey; }) };
      }),
      selections: sels,
      objects: maps.map(function (m) { return m.object || m.moduleLabel; }),
      total: maps.length + sels.length
    };
  }
  ...
  function searchProperties(q) {
    ...
    return PLIST.map(function (r) { return PROPS[r[0]]; }).filter(function (p) {
  ...
  function dictionaryCsv() {
    var rows = [['property_key', 'label', 'domain', 'quantity', 'canonical_si_unit',
      'display_units', 'applies_to', 'origin']];
    refreshProps();
    /* PLIST, so whoever prepares a dataset is told about the properties this
       project added as well as the ones shipped. A dictionary that omits half
       of what the library accepts is a dictionary that causes rejected rows. */
    PLIST.forEach(function (r) {
      rows.push([r[0], r[1], r[2], r[3], siUnit(r[3]), unitsFor(r[3]).join(' | '), r[4],
        (PROPS[r[0]] || {}).userDefined ? 'ADDED ON THIS PROJECT' : 'DICTIONARY']);
    });
    return rows.map(function (r) {
      return r.map(function (c) {
        var s = String(c == null ? '' : c);
  ...
  window.ARODATA = {
    ...
    STATUS: STATUS, FORM: FORM, FORM_FIELDS: FORM_FIELDS, SCALE: SCALE,
    DOMAINS: DOMAINS, PROPS: PROPS, LAYERS: LAYERS,
    ...
    moduleInputs: moduleInputs, suggestedInput: suggestedInput,
    templatesFor: templatesFor, TEMPLATES: TEMPLATES,
    masterValues: masterValues, resolve: resolve, completeness: completeness,
    compare: compare, impactOf: impactOf, refreshProps: refreshProps,
    ...
    stats: function () {
      ...
      /* PLIST, not P: a property an engineer added to the dictionary is a
         property the library carries, and the headline count that ignored it
         reported 105 while the subject in front of them listed 106. */
      return { subjects: subs, values: vals, byStatus: byStatus,
        properties: PLIST.length, dictionary: P.length,
        userProperties: PLIST.length - P.length, domains: DOMAINS.length };
    },

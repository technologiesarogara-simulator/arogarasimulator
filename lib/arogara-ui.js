...
(function () {
  ...
  var css = ''
    ...
    /* This shadow was tuned for the graphite/dark skin, where a strong dark
       drop-shadow reads as the console header lifting off the panel behind
       it. Applied unconditionally, the same dark halo under the light
       (theme-day) skin reads as a detached, "floating" card instead — the
       header still visually works, it just needed its own, much softer,
       shadow once the panel around it turned light. */
    + 'html:not(.theme-day) .pump-sticky-header{box-shadow:0 6px 16px rgba(0,0,0,0.45);}'
    + 'html.theme-day .pump-sticky-header{box-shadow:0 2px 8px rgba(15,23,42,0.10);}'
    /* Narrow width: the header is a single flex row (dot, label, 3 buttons)
       with no wrapper around the buttons, so a tight width squeezed the
       label onto 2-3 lines while the buttons stayed pinned mid-row — a tall,
       lopsided header that read as "floating" free of the layout around it.
       Forcing the label onto its own full-width line lets the three buttons
       wrap together onto a clean second row instead. */
    + '@media (max-width:480px){'
    + '.pump-sticky-header{flex-wrap:wrap;row-gap:8px;}'
    + '.pump-sticky-header>span{flex:1 1 100%;}'
    + '.pump-sticky-header #pump-undo-btn{margin-left:0 !important;}'
    + '}';
  ...
  /* Collapsible sections for <details class="pump-accordion"> forms.
     ...
     feed each other and the panel reports results after RUN CALCULATION — you could not

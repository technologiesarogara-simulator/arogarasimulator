...
(function () {
  ...
  function band() {
    ...
    /* The fluid pair alone does not fix the band — the plate metal, its
       thickness, the corrugation angle and the plate type all move what the
       unit can reach. Read that construction from the module itself so this
       preview and the results card can never disagree about it. */
    var ctor = (typeof AP.construction === 'function') ? AP.construction() : null;
    return AP.uBand(hc, cc, ctor);
  }

  /* The gasket does not change U — it decides how hot the unit may run and
     what it may run on. Reported as the service limit it actually is, rather
     than folded into the coefficient where it does not belong. */
  function gasketNote() {
    var AP = window.AROPHE;
    var ctor = (AP && typeof AP.construction === 'function') ? AP.construction() : null;
    if (!ctor || !AP.gasketTmax) return '';
    var tmax = AP.gasketTmax[ctor.gasket];
    if (!isFinite(tmax)) return '';
    var hottest = Math.max(n('phe-hf-tin', -Infinity), n('phe-cf-tin', -Infinity));
    var ok = !isFinite(hottest) || hottest <= tmax;
    return '<br><b>' + esc(ctor.gasket) + '</b> gasket — rated to ' + tmax + ' °C'
      + (isFinite(hottest)
        ? (ok ? ', above the ' + hottest.toFixed(0) + ' °C inlet. '
              : ' but the inlet is ' + hottest.toFixed(0) + ' °C — reselect the gasket. ')
        : '. ')
      + 'Gasket choice sets the temperature and compatibility limit, not U.';
  }
    ...
    if (ref) {
      ref.innerHTML = b
        ? 'Service reads as <b>' + esc(b.basis) + '</b>.<br>Typical U₀ <b>' + disp(b.lo, 0)
          ...
          + (b.adjusted
            ? ' — moved from the <b>' + disp(b.refLo, 0) + '–' + disp(b.refHi, 0)
              + '</b> quoted for a 0.5 mm SS316 60° chevron, because of the plate selected.'
            : '')
  ...
  function build() {
    ...
    /* The construction fields belong in this list for the same reason the
       fluid fields do: each one moves the band this box draws. */
    ['phe-hf-phase', 'phe-cf-phase', 'phe-hf-cp', 'phe-cf-cp', 'phe-hf-k', 'phe-cf-k',
      'phe-hf-name', 'phe-cf-name', 'phe-hf-tin', 'phe-cf-tin',
      'phe-pmat', 'phe-gasket', 'phe-ptype', 'phe-t', 'phe-beta'].forEach(function (id) {
      var e = $(id);
      if (!e) return;

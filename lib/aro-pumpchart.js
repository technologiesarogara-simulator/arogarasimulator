...
(function () {
  ...
  function grid(ctx, W, H, pad, pal, opt) {
    ...
    ctx.fillStyle = pal.textMuted || pal.axis; ctx.font = '10px monospace';
    ...
    if (opt.xLabel) {
      ctx.fillStyle = pal.axis; ctx.font = '600 10px monospace'; ctx.textAlign = 'center'; ctx.textBaseline = 'alphabetic';
    ...
    if (opt.title) {
      ctx.fillStyle = pal.text; ctx.font = '700 10.5px monospace'; ctx.textAlign = 'left'; ctx.textBaseline = 'top';
      ctx.fillText(opt.title, pad.l, 2);
    }
  }

  /* items: [{label, color, alpha, dash:[..]|null, swatch:'line'|'box'}]
     Drawn as a wrapped row of chips anchored at (x, y), background boxed
     for legibility over whatever the plot drew underneath. Returns the
     total height consumed so a caller can reserve space above it. */
  function legend(ctx, x, y, maxW, items, pal) {
    if (!items || !items.length) return 0;
    ctx.save();
    ctx.font = '9.5px monospace';
    var padX = 6, gap = 10, chipH = 14, rowGap = 3, swatchW = 14;
    ...
    rows.forEach(function (row, ri) {
      ...
      row.forEach(function (cell) {
        ...
        ctx.globalAlpha = 1;
        ctx.fillStyle = pal.text; ctx.textAlign = 'left'; ctx.textBaseline = 'middle';

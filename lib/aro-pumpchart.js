...
(function () {
  ...
  /* ── hover tooltip for hand-drawn charts ─────────────────────────────
     None of these plain-canvas charts had any hover feedback at all — a
     Chart.js-based chart shows the exact value under the cursor for free,
     this kit's charts showed nothing. Rather than have each of the three
     callers (and the flow-viz diagram) build its own hit-testing, this is
     ONE shared mousemove/nearest-point/floating-label implementation: a
     caller attaches it once per canvas, then on every redraw just hands
     over the current draw's own point list (screen-space x/y already
     known from the same loop that plotted them, plus whatever label text
     it wants shown) — attachTooltip() itself never needs to know what a
     "pump curve" or "MOC envelope" point even is. */
  function injectTooltipCss() {
    if (document.getElementById('aro-pc-tt-css')) return;
    var s = document.createElement('style');
    s.id = 'aro-pc-tt-css';
    s.textContent = '.aro-pc-tooltip{position:absolute;display:none;pointer-events:none;z-index:22;'
      + 'max-width:220px;padding:5px 8px;border-radius:5px;font-family:var(--font-mono,monospace);'
      + 'font-size:10.5px;line-height:1.5;background:rgba(11,17,23,0.94);color:#e2e8f0;'
      + 'border:1px solid rgba(148,163,184,0.4);box-shadow:0 6px 18px rgba(0,0,0,0.4);white-space:nowrap;}'
      + 'html.theme-day .aro-pc-tooltip{background:rgba(255,255,255,0.98);color:#0f172a;border-color:#c8d0d8;}'
      + '.aro-pc-tt-dot{position:absolute;width:8px;height:8px;margin:-4px 0 0 -4px;border-radius:50%;'
      + 'border:2px solid #fff;pointer-events:none;z-index:21;display:none;box-shadow:0 0 0 1px rgba(0,0,0,0.5);}';
    document.head.appendChild(s);
  }
  function attachTooltip(canvas) {
    if (!canvas || canvas._aroTtAttached) return;
    canvas._aroTtAttached = true;
    canvas._aroTtPoints = [];
    injectTooltipCss();
    var host = canvas.parentElement;
    if (host && getComputedStyle(host).position === 'static') host.style.position = 'relative';
    var tip = document.createElement('div'), dot = document.createElement('div');
    tip.className = 'aro-pc-tooltip'; dot.className = 'aro-pc-tt-dot';
    if (host) { host.appendChild(tip); host.appendChild(dot); }
    function hide() { tip.style.display = 'none'; dot.style.display = 'none'; canvas.style.cursor = ''; }
    canvas.addEventListener('mousemove', function (e) {
      var rect = canvas.getBoundingClientRect();
      var mx = e.clientX - rect.left, my = e.clientY - rect.top;
      var pts = canvas._aroTtPoints || [];
      var best = null, bestD = 24;
      for (var i = 0; i < pts.length; i++) {
        var d = Math.hypot(pts[i].x - mx, pts[i].y - my);
        if (d < bestD) { bestD = d; best = pts[i]; }
      }
      if (!best) { hide(); return; }
      tip.style.display = 'block';
      tip.innerHTML = best.html;
      var left = best.x + 12, top = best.y - 10;
      if (left + 220 > rect.width) left = best.x - 12 - Math.min(220, tip.offsetWidth || 120);
      if (top < 0) top = best.y + 12;
      tip.style.left = left + 'px'; tip.style.top = top + 'px';
      dot.style.display = 'block'; dot.style.left = best.x + 'px'; dot.style.top = best.y + 'px';
      dot.style.background = best.color || '#38bdf8';
      canvas.style.cursor = 'crosshair';
    });
    canvas.addEventListener('mouseleave', hide);
  }
  /* points: [{x, y, html, color?}] in CSS-pixel canvas coordinates (the
     same space hd()'s returned ctx already draws in) — replace on every
     redraw; attachTooltip() only needs calling once per canvas. */
  function setTooltipPoints(canvas, points) { if (canvas) canvas._aroTtPoints = points || []; }

  window.AROPUMPCHART = {
    hd: hd, grid: grid, legend: legend, niceStep: niceStep,
    attachTooltip: attachTooltip, setTooltipPoints: setTooltipPoints
  };
})();

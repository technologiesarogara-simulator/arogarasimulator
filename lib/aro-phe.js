    ...
    function paginate() {
      ...
      setTimeout(function () {
        ...
        [].slice.call(clone.querySelectorAll(ATOMIC)).forEach(function (el) {
          ...
          if (r.height > 2 && r.height <= pageHpx * 0.5) blocks.push({ t: r.top - padTop, b: r.bottom - padTop });
        });
            ...
            return html2pdf().set({ jsPDF: { unit: 'mm', format: 'a4', orientation: landscape ? 'landscape' : 'portrait' } })
              ...
              .then(function (pdf) {
                ...
                while (y < canvas.height - 1 && guard++ < 300) {
                  var cut = Math.min(y + sliceH, canvas.height);
                  if (cut < canvas.height) {
                    var cutCss = cut / k, best = cut;
                    for (var i = 0; i < blocks.length; i++) {
                      var bk = blocks[i];
                      if (cutCss > bk.t + 0.5 && cutCss < bk.b - 0.5) {
                        var alt = Math.floor(bk.t * k);
                        if (alt > y + sliceH * 0.25 && alt < best) best = alt;
                      }
                    }
                    cut = best;
                  }

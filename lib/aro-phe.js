...
(function () {
  ...
  function panelHTML() {
    ...
    h += '<details class="pump-accordion" id="phe-manual">'
      ...
      + '<p>Press <b>&#9654; RUN PHE DESIGN</b>. The 3D plate-pack, the results panel, the design optimiser and every '
  ...
  var pheInputsValidatedOnce = false;
  ...
  function showPHEInputsDialog(missing) {
    if (pheInputsValidatedOnce) return;  /* Show only once per session */
    pheInputsValidatedOnce = true;
  ...
  function gTempProfile(r) {
    ...
    s += '<text x="' + (W - pad) + '" y="' + (pad + 2) + '" fill="#22c55e" font-size="11" text-anchor="end">LMTD ' + U(r.lmtd, 'temp-diff', 1) + ' · ΔTm ' + U(r.dTm, 'temp-diff', 1) + '</text>';
    // clickable legend

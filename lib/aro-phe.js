...
(function () {
  ...
  function wire() {
    ...
    ids.forEach(function (id) { var e = $(id); if (e) { e.addEventListener('input', function () { pushUndo(); calc(); updateNotify(); }); e.addEventListener('change', function () { pushUndo(); calc(); updateNotify(); }); } });
    // fluid-library selectors auto-fill properties then recalc
    var hn = $('phe-hf-name'); if (hn) hn.addEventListener('change', function () { pushUndo(); applyFluid('hf'); calc(); updateNotify(); });
    var cn = $('phe-cf-name'); if (cn) cn.addEventListener('change', function () { pushUndo(); applyFluid('cf'); calc(); updateNotify(); });
    var pm = $('phe-pmat'); if (pm) pm.addEventListener('change', function () { pushUndo(); updateMatInfo(); calc(); updateNotify(); });

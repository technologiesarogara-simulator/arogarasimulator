...
(function () {
  ...
  function gCoeffs(r) {
    ...
    var items = [
      { l: 'U₀ assumed', v: (r.uSug.lo + r.uSug.hi) / 2, c: '#64748b' },
      { l: 'h hot film', v: r.H.h, c: '#ef4444' },
      { l: 'h cold film', v: r.C.h, c: '#3b82f6' },
      { l: 'Clean U', v: r.Uclean, c: '#38bdf8' },
      { l: 'Design U', v: r.Ud, c: r.uInBand ? '#22c55e' : '#f59e0b' }
    ];
    ...
    items.forEach(function (it, i) {
      ...
      s += '<rect x="' + cx + '" y="' + BY(it.v) + '" width="' + bw + '" height="' + (y0 - BY(it.v)) + '" fill="' + it.c + '" rx="2"/>';
      ...
      s += '<text x="' + (cx + bw / 2) + '" y="' + (y0 + 15) + '" fill="#94a3b8" font-size="9.5" text-anchor="middle">' + it.l + '</text>';
    });
  ...
  function gDpSweep(r) {
    ...
    s += poly(function (fr) { return DY(dpAt(dpH0, fr)); }, '#ef4444', 2.4);
    s += poly(function (fr) { return DY(dpAt(dpC0, fr)); }, '#3b82f6', 2.4);
    s += poly(function (fr) { return VY(vAt(r.nozzles.hotIn.v, fr)); }, '#f59e0b', 1.8, '5 3');
    s += poly(function (fr) { return VY(vAt(r.nozzles.coldIn.v, fr)); }, '#a855f7', 1.8, '5 3');
    // design point marker
    s += '<circle cx="' + PX(100) + '" cy="' + DY(dpH0) + '" r="4" fill="#ef4444"/><circle cx="' + PX(100) + '" cy="' + DY(dpC0) + '" r="4" fill="#3b82f6"/>';
    ...
    // legend
    var lg = [['ΔP hot', '#ef4444'], ['ΔP cold', '#3b82f6'], ['Vel hot', '#f59e0b'], ['Vel cold', '#a855f7']];
    lg.forEach(function (L, i) { var lx = x0 + i * 130; s += '<rect x="' + lx + '" y="30" width="14" height="4" fill="' + L[1] + '"/><text x="' + (lx + 18) + '" y="34" fill="#cbd5e1" font-size="9">' + L[0] + '</text>'; });
    return s + '</svg>';
  }
  ...
  function renderGraphs(r) {
    ...
    box.innerHTML = '<div style="font-family:var(--font-mono);font-size:10px;font-weight:800;color:var(--color-saffron);letter-spacing:0.05em;margin:4px 0 6px;border-top:1px solid var(--border-muted);padding-top:10px;">PERFORMANCE GRAPHS — LIVE (auto-updates with inputs)</div>' + buildGraphs(r);
  }

  function graph() {
    var r = LAST || compute();
    modal('PHE — PERFORMANCE, ΔP &amp; U₀ DESIGN CRITERIA', buildGraphs(r));
  }

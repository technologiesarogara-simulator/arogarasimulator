...
(function () {
  ...
  function panelHTML() {
    ...
    h += '<div class="panel panel-input" style="max-height:calc(100vh - 120px);overflow-y:auto;overflow-x:hidden;">'
      ...
      + '<button id="phe-undo" class="phe-hbtn" title="Undo">↶</button>'
      + '<button id="phe-redo" class="phe-hbtn" title="Redo">↷</button>'
      + '<button id="phe-reset" class="phe-hbtn" style="border-color:#ef4444;color:#ef4444;" title="Reset to defaults">↺</button></div>'
    ...
    h += twoCol(fld('Specific gravity', 'phe-hf-sg', '–', 0.972, '0.001'), fld('Fouling Rf', 'phe-hf-rf', 'm²K/W', 0.000018, '0.000001'));
    h += fld('Allow ΔP', 'phe-hf-dpa', 'kPa', 50);
    ...
    h += twoCol(fld('Specific gravity', 'phe-cf-sg', '–', 0.995, '0.001'), fld('Fouling Rf', 'phe-cf-rf', 'm²K/W', 0.000018, '0.000001'));
    h += fld('Allow ΔP', 'phe-cf-dpa', 'kPa', 50);
    ...
    h += '<style>.phe-act{flex:1;background:transparent;border:1px solid var(--color-saffron);color:var(--color-saffron);font-family:var(--font-mono);font-size:10px;font-weight:700;padding:7px;border-radius:4px;cursor:pointer;}.phe-act:hover{background:rgba(255,117,56,0.12);}'
      ...
      + '.phe-hbtn{width:26px;height:24px;background:transparent;border:1px solid #3b82f6;color:#3b82f6;border-radius:4px;font-size:13px;cursor:pointer;line-height:1;}.phe-hbtn:hover{background:rgba(59,130,246,0.15);}'
    ...
    h += '<div class="panel" style="max-height:calc(100vh - 120px);overflow-y:auto;">'
      ...
      + '<div id="phe-noz-hin" class="phe-noz" style="left:8px;top:60px;background:rgba(220,38,38,0.85);color:#fff;"></div>'
      + '<div id="phe-noz-hout" class="phe-noz" style="left:8px;bottom:34px;background:rgba(220,38,38,0.85);color:#fff;"></div>'
      + '<div id="phe-noz-cin" class="phe-noz" style="right:8px;top:60px;background:rgba(37,99,235,0.85);color:#fff;"></div>'
      + '<div id="phe-noz-cout" class="phe-noz" style="right:8px;bottom:34px;background:rgba(37,99,235,0.85);color:#fff;"></div></div>'
  ...
  // short "system updated" chime via WebAudio + a floating toast
  function updatedFeedback() {
    ...
    var wrap = $('phe-3dwrap') || document.body;
    var t = document.createElement('div');
    t.textContent = '✓ SYSTEM UPDATED — DESIGN RECALCULATED';
    t.style.cssText = 'position:fixed;left:50%;top:70px;transform:translateX(-50%);z-index:100050;background:linear-gradient(135deg,#16a34a,#22c55e);color:#fff;font-family:var(--font-mono);font-size:12px;font-weight:800;letter-spacing:0.05em;padding:10px 20px;border-radius:8px;box-shadow:0 8px 24px rgba(0,0,0,0.4);animation:pheToast 1.8s ease forwards;';
  ...
  var UNDO = [], REDO = [], INPUT_IDS = ['phe-project', 'phe-client', 'phe-tag', 'phe-service', 'phe-engineer', 'phe-rev',
    ...
    'phe-hf-m', 'phe-hf-tin', 'phe-hf-tout', 'phe-hf-rho', 'phe-hf-mu', 'phe-hf-cp', 'phe-hf-k', 'phe-hf-sg', 'phe-hf-rf', 'phe-hf-dpa', 'phe-hf-pdes',
    'phe-cf-m', 'phe-cf-tin', 'phe-cf-tout', 'phe-cf-rho', 'phe-cf-mu', 'phe-cf-cp', 'phe-cf-k', 'phe-cf-sg', 'phe-cf-rf', 'phe-cf-dpa', 'phe-cf-pdes',
  ...
  /* ─────────── wiring ─────────── */
  function wire() {
    var ids = ['phe-hf-m', 'phe-hf-tin', 'phe-hf-tout', 'phe-hf-rho', 'phe-hf-mu', 'phe-hf-cp', 'phe-hf-k', 'phe-hf-rf', 'phe-hf-sg',
      'phe-cf-m', 'phe-cf-tin', 'phe-cf-tout', 'phe-cf-rho', 'phe-cf-mu', 'phe-cf-cp', 'phe-cf-k', 'phe-cf-rf', 'phe-cf-sg',
    ...
    var hn = $('phe-hf-name'); if (hn) hn.addEventListener('change', function () { pushUndo(); applyFluid('hf'); calc(); });
    var cn = $('phe-cf-name'); if (cn) cn.addEventListener('change', function () { pushUndo(); applyFluid('cf'); calc(); });
    ...
    var cb = $('phe-calc'); if (cb) cb.addEventListener('click', function () {
      ...
      updatedFeedback();
      var res = $('phe-results'); if (res) { res.style.transition = 'none'; res.style.boxShadow = '0 0 0 2px #22c55e'; setTimeout(function () { res.style.boxShadow = 'none'; }, 600); res.scrollIntoView({ behavior: 'smooth', block: 'nearest' }); }
    });
  ...
  function compute() {
    ...
    ['phe-cf-m', 'phe-hf-m', 'phe-hf-tout', 'phe-cf-tout'].forEach(function (id) {
      ...
      if (auto && auto.id === id) { e.value = (Math.round(auto.v * 1000) / 1000); e.readOnly = true; e.classList.add('phe-auto'); }
      else { e.readOnly = false; e.classList.remove('phe-auto'); }
    });
  ...
  function calc() {
    ...
    var nl = function (id, t) { var e = $(id); if (e) e.textContent = t; };
    nl('phe-noz-hin', '▸ HOT IN · ' + r.nozzles.hotName.split(' (')[0]);
    nl('phe-noz-hout', '◂ HOT OUT · ' + r.nozzles.hotName.split(' (')[0]);
    nl('phe-noz-cin', 'COLD IN · ' + r.nozzles.coldName.split(' (')[0] + ' ◂');
    nl('phe-noz-cout', 'COLD OUT · ' + r.nozzles.coldName.split(' (')[0] + ' ▸');
    update3D(r);
  }

...
(function () {
  ...
  /* full graph pack (coupled to the current design) — used in output panel,
     modal & report. Charts were stacked full-width, one per row — each an
     SVG that scales to its container's width at a fixed aspect ratio, so
     full width also meant full, oversized height: three charts read as
     dominating the page against the compact result rows beside them. A
     responsive grid holds them at a legible but much smaller width instead
     (the SVG viewBox keeps every chart proportionate and readable, just
     smaller), which is the same fix in spirit as the results-grid change
     made to the STHE/DPHE and PHE result CARDS earlier this session —
     content unchanged, only how it's laid out. */
  function buildGraphs(r) {
    return '<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(340px,1fr));gap:12px;align-items:start;">'
      + '<div>' + gTempProfile(r) + '</div>'
      + '<div>' + gCoeffs(r) + '</div>'
      + '<div>' + gDpSweep(r) + '</div>'
      + '</div>';
  }

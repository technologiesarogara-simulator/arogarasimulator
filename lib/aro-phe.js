...
(function () {
  ...
  function applyFluid(side) {
    ...
    setSI('phe-' + side + '-rho', fv.rho); e('phe-' + side + '-mu') && (e('phe-' + side + '-mu').value = fv.mu);
    setSI('phe-' + side + '-cp', fv.cp / 1000); setSI('phe-' + side + '-k', fv.k);
    var sgEl = e('phe-' + side + '-sg'); if (sgEl) sgEl.value = f.sg;
    var muwEl = e('phe-' + side + '-muw'); if (muwEl) muwEl.value = fv.mu;   // wall viscosity defaults to bulk μ
  ...
  function wire() {
    ...
    var hn = $('phe-hf-name'); if (hn) hn.addEventListener('change', function () { pushUndo(); if (FLUIDS[val('phe-hf-name', '')]) applyFluid('hf'); else blankProps('hf'); calc(); updateFluidPhase('hf'); });
    var cn = $('phe-cf-name'); if (cn) cn.addEventListener('change', function () { pushUndo(); if (FLUIDS[val('phe-cf-name', '')]) applyFluid('cf'); else blankProps('cf'); calc(); updateFluidPhase('cf'); });

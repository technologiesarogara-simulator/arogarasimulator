...
(function () {
  ...
  window.AROPDF = function (element, filename, opts) {
    ...
    var ATOMIC = 'tr, table, img, canvas, .pdf-atomic, .aln-rr, .tp2-rr, .pid-rr, .res-value, .pump-res-card';
    ...
    function paginate() {
      ...
      setTimeout(function () {
        ...
        [].slice.call(clone.querySelectorAll(ATOMIC)).forEach(function (el) {
          ...
          if (r.height > 2 && r.height <= pageHpx) blocks.push({ t: r.top - padTop, b: r.bottom - padTop });
        });

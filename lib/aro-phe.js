...
(function () {
  ...
  /* The U0 bands quoted in uBand() below are the manufacturer rule-of-thumb
     figures, and every one of those tables is quoted for the SAME reference
     construction: a 0.5 mm SS316 plate with a 60° chevron. Selecting a
     different plate metal, a thicker plate, a shallower corrugation or a
     wide-gap/free-flow plate changes what the unit can actually reach, so
     the band has to move with the construction — otherwise the design is
     judged against a target belonging to a plate nobody selected. */
  var U_REF_K = 16.3, U_REF_T = 0.0005, U_REF_BETA = 60;
  /* Film-coefficient multiplier against a standard chevron plate of the same
     size. Wide-gap and free-flow plates deliberately trade heat transfer for
     an unobstructed channel (fibrous / high-viscosity duty); a double-wall
     plate puts a second metal interface between the two media. Welded,
     semi-welded and brazed units keep the chevron pattern, so they carry the
     chevron plate's own factor and differ only in how they are sealed. */
  var PTYPE_FILM = {
    'Chevron (Herringbone)': 1.00, 'Gasketed': 1.00, 'Semi-Welded': 1.00,
    'Fully-Welded': 1.00, 'Brazed': 1.00,
    'Double-Wall': 0.80, 'Wide-Gap': 0.65, 'Free-Flow': 0.55
  };
  function ptypeFilm(p) { var v = PTYPE_FILM[p]; return isFinite(v) ? v : 1.00; }

  /* How each plate metal actually looks, so the pack in the 3D view reads as
     the metal that was selected instead of one generic grey: stainless bright
     and cool, titanium duller and greyer, nickel and Monel warmer, cupronickel
     frankly pink, aluminium light and low-gloss. */
  var MAT_LOOK = {
    'SS304':             { c: 0xb8c0c8, m: 0.88, r: 0.30 },
    'SS304L':            { c: 0xb8c0c8, m: 0.88, r: 0.31 },
    'SS316':             { c: 0xaeb7c0, m: 0.90, r: 0.28 },
    'SS316L':            { c: 0xaeb7c0, m: 0.90, r: 0.26 },
    'SS317L':            { c: 0xa6afb8, m: 0.90, r: 0.28 },
    'Titanium Gr2':      { c: 0x8f9499, m: 0.72, r: 0.46 },
    'Ti-Pd Gr7':         { c: 0x94989c, m: 0.72, r: 0.44 },
    'SMO 254':           { c: 0xa8b2bb, m: 0.88, r: 0.30 },
    'Hastelloy C276':    { c: 0x9aa0a6, m: 0.82, r: 0.38 },
    'Hastelloy C22':     { c: 0x9ea4aa, m: 0.82, r: 0.38 },
    'Nickel 200':        { c: 0xc3c0b4, m: 0.86, r: 0.32 },
    'Monel 400':         { c: 0xb5b2a6, m: 0.84, r: 0.34 },
    'Inconel 625':       { c: 0xa7a59c, m: 0.84, r: 0.36 },
    'Duplex 2205':       { c: 0xb0b8bf, m: 0.88, r: 0.30 },
    'Super Duplex 2507': { c: 0xaab2b9, m: 0.88, r: 0.29 },
    'Cupronickel 90/10': { c: 0xc8a68c, m: 0.85, r: 0.33 },
    'Aluminium 3003':    { c: 0xd2d6da, m: 0.70, r: 0.42 }
  };
  function matLook(name) { return MAT_LOOK[name] || MAT_LOOK['SS316']; }
  /* Gasket elastomers are colour-coded on real units — the bead around each
     plate is how a fitter tells EPDM from Viton across the shop floor. */
  var GASKET_LOOK = { 'NBR': 0x24262a, 'EPDM': 0x191b1e, 'Viton': 0x3f6d3a,
    'HNBR': 0x2d3136, 'Silicone': 0xa8443a, 'PTFE': 0xe6e8ea };
  function gasketLook(name) { return GASKET_LOOK[name] != null ? GASKET_LOOK[name] : 0x191b1e; }
  /* Brazed and fully-welded packs have no gaskets at all — leaving the bead
     off them is the quickest way to see which construction is selected. */
  function hasGasket(p) { return !/Brazed|Fully-Welded/.test(String(p || '')); }
  // U0-assumption service classification — module scope (not calc()-local) so
  // the input-side preview (lib/aro-pheu0.js) can classify the service from
  // the same rule the results card uses, rather than a second table that
  // could drift from this one. s.cp is J/kg·K, s.k is W/m·K (SI).
  ...
  /* The fluid pair fixes the band; the construction then moves it. Called
     with two arguments this returns the bare rule-of-thumb figures exactly as
     before; pass the plate construction as a third argument to get the band
     for the plate actually selected. */
  function uBand(hc, cc, ctor) {
    var b;
    if (hc === 'phase' || cc === 'phase') b = { lo: 3500, hi: 7000, basis: 'Phase-change / steam service' };
    else if (hc === 'gas' || cc === 'gas') b = { lo: 100, hi: 500, basis: 'Gas–liquid service' };
    else if (hc === 'oil' || cc === 'oil') b = { lo: 300, hi: 1000, basis: 'Oil / hydrocarbon–liquid service' };
    else b = { lo: 2500, hi: 6000, basis: 'Water–water / aqueous service' };
    return ctor ? applyConstruction(b, ctor) : b;
  }

  /* Re-solve a fluid-pair band for the construction on the form. The band's
     own numbers stay the film-side target; only the metal wall resistance and
     the corrugation / plate-type film factor are swapped from the reference
     construction to the selected one. So a 0.8 mm Inconel wide-gap plate is
     judged against what THAT plate can reach, not against a 0.5 mm SS316
     chevron — which is the whole point of picking a material on the form.

     1/U = R_film + t/k, so the wall term is separable: strip the reference
     wall off the quoted band, scale the remaining film resistance by the
     corrugation and plate-type factors, then add the real wall back. */
  function applyConstruction(b, c) {
    var k = (isFinite(c.k) && c.k > 0) ? c.k : U_REF_K;
    var t = (isFinite(c.t) && c.t > 0) ? c.t : U_REF_T;
    var beta = isFinite(c.beta) ? c.beta : U_REF_BETA;
    var fPt = ptypeFilm(c.ptype);
    var fBeta = chevronConst(beta).c / chevronConst(U_REF_BETA).c;
    var fFilm = fPt * fBeta;
    var RwRef = U_REF_T / U_REF_K, RwAct = t / k;
    function shift(U) {
      var Rfilm = 1 / U - RwRef;
      if (!(Rfilm > 0)) return U;            // band already tighter than the reference wall
      return 1 / (Rfilm / fFilm + RwAct);
    }
    var lo = shift(b.lo), hi = shift(b.hi);
    /* Spell out every term that actually moved the band, so the number on
       screen can be traced back to the selection that caused it. */
    var notes = [];
    if (Math.abs(fBeta - 1) > 0.02) notes.push(beta + '° chevron');
    if (Math.abs(fPt - 1) > 0.02) notes.push(String(c.ptype || '').replace(/\s*\(.*\)$/, ''));
    if (c.matName && (Math.abs(k - U_REF_K) > 0.5 || Math.abs(t - U_REF_T) > 1e-6)) {
      notes.push(c.matName + ' ' + (t * 1000).toFixed(1) + ' mm (k ' + k + ' W/m·K)');
    }
    return {
      lo: Math.min(lo, hi), hi: Math.max(lo, hi),
      basis: b.basis + (notes.length ? ' · ' + notes.join(' · ') : ''),
      fluidBasis: b.basis, refLo: b.lo, refHi: b.hi,
      adjusted: notes.length > 0, notes: notes
    };
  }

  /* The plate construction currently on the form, read through the same
     unit-aware field reader compute() uses. Exposed on the namespace so the
     input-side U0 preview reads it from one place instead of re-deriving it
     and getting the mm/inch conversion wrong in imperial. */
  function readConstruction() {
    var matName = val('phe-pmat', 'SS316');
    var mat = MATERIALS[matName] || MATERIALS['SS316'];
    return {
      k: mat.k, t: num('phe-t', 0.5) / 1000, beta: num('phe-beta', 60),
      ptype: val('phe-ptype', 'Chevron (Herringbone)'),
      matName: matName, gasket: val('phe-gasket', 'EPDM')
    };
  }
  ...
  /* Port/nozzle diameter is auto-calculated from the duty (see compute())
     unless the engineer has actually typed their own value — tracked here
     rather than by comparing against a hardcoded default, since the
     auto-calculated value itself changes with the duty and isn't one fixed
     number to compare against. */
  var pheDpUserSet = false;

  /* ─────────── DUAL CONFIGURATION SUPPORT (Analytical + Industrial) ─────────── */
  var dualConfigMode = false;           // toggle simultaneous analytical/industrial tuning
  var LAST_ANALYTICAL = null, LAST_INDUSTRIAL = null;
  // Industrial standard thickness thresholds (mm) — used to auto-size industrial config
  var INDUSTRIAL_THICKNESS_STD = [0.5, 0.6, 0.7, 0.8, 1.0];
  function nearestIndustrialThickness(proposed) {
    var best = 0.5, bestDiff = Math.abs(proposed - 0.5);
    INDUSTRIAL_THICKNESS_STD.forEach(function (t) {
      var d = Math.abs(proposed - t);
  ...
  /* ═══════════════════════════════════════════════════════════════════════
     NOZZLE / CONNECTION SELECTION  —  all four connections, sized separately
     ───────────────────────────────────────────────────────────────────────
     Every connection was previously given the same bore as the plate port
     and reported only a velocity. Hot and cold rarely carry the same
     volumetric flow, so sizing them together is wrong on one of the two, and
     a velocity on its own does not select anything: a connection is chosen
     from a standard size table, checked against velocity AND momentum, and
     given a flange rating from the design pressure and temperature.

     Criteria, all liquid service:
       · design velocity target      ≤ 3.0 m/s   normal selection basis
       · plate-vibration threshold     4.5 m/s   above this, port-induced
                                                 plate vibration is possible
                                                 (Deppmann, Velocity Limits in
                                                 Gasketed PHEs)
       · absolute maximum              6.0 m/s
       · momentum ρv²                ≤ 8968 kg/m·s²  nozzle/entrance limit
                                                 for liquid
     Sizes are ASME B36.10M / B36.19M nominal bores with standard-wall (Sch 40
     / 40S) inside diameters. Flange class is screened against ASME B16.5
     pressure–temperature ratings, Group 1.1 for carbon steel and Group 2.2
     for the austenitic stainless grades.
     ═══════════════════════════════════════════════════════════════════════ */
  var DN_SIZES = [
    { dn: 15,  nps: '1/2"',   id: 15.8 }, { dn: 20,  nps: '3/4"',   id: 21.0 },
    { dn: 25,  nps: '1"',     id: 26.6 }, { dn: 32,  nps: '1-1/4"', id: 35.1 },
    { dn: 40,  nps: '1-1/2"', id: 40.9 }, { dn: 50,  nps: '2"',     id: 52.5 },
    { dn: 65,  nps: '2-1/2"', id: 62.7 }, { dn: 80,  nps: '3"',     id: 77.9 },
    { dn: 100, nps: '4"',     id: 102.3 }, { dn: 125, nps: '5"',    id: 128.2 },
    { dn: 150, nps: '6"',     id: 154.1 }, { dn: 200, nps: '8"',    id: 202.7 },
    { dn: 250, nps: '10"',    id: 254.5 }, { dn: 300, nps: '12"',   id: 303.3 },
    { dn: 350, nps: '14"',    id: 333.3 }, { dn: 400, nps: '16"',   id: 381.0 }
  ];

  /* ASME B16.5 pressure–temperature ratings, barg, at the listed °C.
     Interpolated linearly between breakpoints; below 38 °C the 38 °C figure
     is held, which is how the tables themselves are read. */
  var FLANGE_PT = {
    'CS':  { 150: [[38,19.6],[100,17.7],[200,13.8],[300,10.2],[400,6.5]],
             300: [[38,51.1],[100,46.6],[200,43.8],[300,39.8],[400,34.7]],
             600: [[38,102.1],[100,93.2],[200,87.6],[300,79.6],[400,69.4]] },
    'SS':  { 150: [[38,15.9],[100,13.3],[200,11.5],[300,10.0],[400,9.3]],
             300: [[38,41.4],[100,37.6],[200,33.3],[300,30.0],[400,27.6]],
             600: [[38,82.7],[100,75.2],[200,66.8],[300,60.7],[400,55.3]] }
  };
  function flangeAllowable(group, cls, tC) {
    var rows = (FLANGE_PT[group] || FLANGE_PT.SS)[cls];
    if (!rows) return 0;
    if (tC <= rows[0][0]) return rows[0][1];
    for (var i = 1; i < rows.length; i++) {
      if (tC <= rows[i][0]) {
        var a = rows[i - 1], b = rows[i];
        return a[1] + (b[1] - a[1]) * (tC - a[0]) / (b[0] - a[0]);
      }
    }
    return rows[rows.length - 1][1];
  }
  /* Lowest class that still covers the duty at temperature. */
  function flangeClassFor(group, Pdes, tC) {
    var classes = [150, 300, 600];
    for (var i = 0; i < classes.length; i++) {
      if (flangeAllowable(group, classes[i], tC) >= Pdes) {
        return { cls: classes[i], allow: flangeAllowable(group, classes[i], tC), ok: true };
      }
    }
    return { cls: 600, allow: flangeAllowable(group, 600, tC), ok: false };
  }

  /* One connection. mdot kg/s, rho kg/m³, mu Pa·s, tC °C, Pdes barg. */
  function selectNozzle(tag, mdot, rho, mu, tC, Pdes, matGroup) {
    var V_TARGET = 3.0, V_VIB = 4.5, V_MAX = 6.0, RHOV2_MAX = 8968;
    var out = { tag: tag, mdot: mdot, rho: rho };
    if (!(mdot > 0) || !(rho > 0)) { out.ok = false; out.note = 'no flow entered'; return out; }
    var Qv = mdot / rho;                                  // m³/s
    out.Qv = Qv;
    /* bore the target velocity asks for, then the first standard size that
       is at least that big — never round DOWN onto a faster connection */
    var dReq = Math.sqrt(4 * Qv / (Math.PI * V_TARGET));  // m
    out.dReq = dReq;
    var pick = null;
    for (var i = 0; i < DN_SIZES.length; i++) {
      if (DN_SIZES[i].id / 1000 >= dReq) { pick = DN_SIZES[i]; break; }
    }
    if (!pick) pick = DN_SIZES[DN_SIZES.length - 1];
    out.dn = pick.dn; out.nps = pick.nps; out.id = pick.id;
    var d = pick.id / 1000, A = Math.PI * d * d / 4;
    var v = Qv / A;
    out.v = v;
    out.rhov2 = rho * v * v;
    out.Re = mu > 0 ? rho * v * d / mu : NaN;
    /* one velocity head lost at entry plus one at exit, the usual allowance
       for a nozzle into a port manifold */
    out.dp = 1.5 * (rho * v * v / 2) / 1000;              // kPa
    var fl = flangeClassFor(matGroup, Pdes, tC);
    out.flangeClass = fl.cls; out.flangeAllow = fl.allow; out.flangeOk = fl.ok;
    out.rating = 'ASME B16.5 CL' + fl.cls + ' RF';
    /* verdict — the binding criterion is named, not just a pass/fail */
    if (v > V_MAX) { out.status = 'fail'; out.note = 'velocity ' + v.toFixed(2) + ' m/s exceeds the ' + V_MAX + ' m/s maximum'; }
    else if (out.rhov2 > RHOV2_MAX) { out.status = 'fail'; out.note = 'ρv² ' + Math.round(out.rhov2) + ' exceeds the ' + RHOV2_MAX + ' kg/m·s² erosion limit'; }
    else if (v > V_VIB) { out.status = 'warn'; out.note = 'above ' + V_VIB + ' m/s — port-induced plate vibration is possible'; }
    else if (v > V_TARGET) { out.status = 'warn'; out.note = 'above the ' + V_TARGET + ' m/s design target; acceptable, ΔP rises'; }
    else { out.status = 'pass'; out.note = 'within the ' + V_TARGET + ' m/s design target'; }
    if (!fl.ok) { out.status = 'fail'; out.note = 'design pressure exceeds CL600 at ' + Math.round(tC) + ' °C'; }
    out.ok = out.status !== 'fail';
    out.limits = { vTarget: V_TARGET, vVib: V_VIB, vMax: V_MAX, rhov2Max: RHOV2_MAX };
    return out;
  }


  /* ═══════════════════════════════════════════════════════════════════════
     The connection schedule and the pressure-drop build-up, rendered for the
     ISSUED report (print styling, not the panel's dark theme). Returned as
     AROENG report sections so they are numbered with the rest of the document.
     ═══════════════════════════════════════════════════════════════════════ */
  function reportSchedules(r) {
    if (!r) return [];
    var out = [];
    var th = 'padding:4px 6px;border:1px solid #d1d5db;background:#f3f4f6;text-align:left;color:#334155;font-weight:700;';
    var td = 'padding:4px 6px;border:1px solid #d1d5db;color:#111827;';
    var note = 'font-size:9.5px;color:#4b5563;margin:6px 0 0;line-height:1.55;';

    if (r.nozzleSchedule && r.nozzleSchedule.length) {
      var L0 = r.nozzleSchedule[0].limits || {};
      var t = '<table style="width:100%;border-collapse:collapse;font-size:10px;"><tr>'
        + ['Connection', 'Size', 'Bore', 'Flow', 'Velocity', 'ρv²', 'Re', 'ΔP', 'Rating', 'Status']
            .map(function (x) { return '<th style="' + th + '">' + x + '</th>'; }).join('') + '</tr>';
      r.nozzleSchedule.forEach(function (n) {
        var col = n.status === 'fail' ? '#b91c1c' : (n.status === 'warn' ? '#b45309' : '#15803d');
        t += '<tr>' + [
          n.tag, 'DN' + (n.dn || '—') + ' / ' + (n.nps || '—'),
          isFinite(n.id) ? n.id.toFixed(1) + ' mm' : '—',
          isFinite(n.Qv) ? (n.Qv * 3600).toFixed(2) + ' m³/h' : '—',
          isFinite(n.v) ? n.v.toFixed(2) + ' m/s' : '—',
          isFinite(n.rhov2) ? Math.round(n.rhov2) : '—',
          isFinite(n.Re) ? Math.round(n.Re).toLocaleString() : '—',
          isFinite(n.dp) ? n.dp.toFixed(2) + ' kPa' : '—',
          n.rating || '—',
          '<b style="color:' + col + ';">' + (n.status || '—').toUpperCase() + '</b>'
        ].map(function (x) { return '<td style="' + td + '">' + x + '</td>'; }).join('') + '</tr>';
      });
      t += '</table>';
      r.nozzleSchedule.forEach(function (n) {
        if (n.status && n.status !== 'pass') {
          t += '<div style="font-size:9.5px;margin:3px 0;color:'
            + (n.status === 'fail' ? '#b91c1c' : '#b45309') + ';">'
            + (n.status === 'fail' ? '✕ ' : '⚠ ') + n.tag + ' — ' + n.note + '</div>';
        }
      });
      t += '<div style="' + note + '">Each connection sized on its own flow and density — the first standard '
        + 'bore holding velocity at or below ' + (L0.vTarget || 3) + ' m/s, never rounded down onto a faster '
        + 'one. ASME B36.10M / B36.19M nominal bores at standard wall. Checked against ' + (L0.vVib || 4.5)
        + ' m/s, above which port-induced plate vibration is possible, a ' + (L0.vMax || 6) + ' m/s ceiling, '
        + 'and ρv² ≤ ' + (L0.rhov2Max || 8968) + ' kg/m·s² for liquid. Flange class per ASME B16.5 '
        + 'pressure–temperature ratings, group '
        + (r.matGroup === 'CS' ? '1.1 (carbon steel)' : '2.2 (austenitic stainless)')
        + ', at each connection\'s own design temperature.</div>';
      out.push({ title: 'CONNECTION SCHEDULE — ALL FOUR NOZZLES', html: t });
    }

    if (r.dpH && r.dpC && isFinite(r.dpH.perPlate)) {
      var p2 = '<table style="width:100%;border-collapse:collapse;font-size:10px;"><tr>'
        + ['Side', 'G (kg/m²s)', 'Re', 'f', 'ΔP / plate', '× passes', 'Channel', 'Port', 'Total', 'Allowable', 'Used']
            .map(function (x) { return '<th style="' + th + '">' + x + '</th>'; }).join('') + '</tr>';
      [['HOT', r.H, r.dpH, r.dpHa], ['COLD', r.C, r.dpC, r.dpCa]].forEach(function (x) {
        var used = x[3] > 0 ? (x[2].dp / x[3] * 100) : NaN;
        var col = !isFinite(used) ? '#111827' : (used > 100 ? '#b91c1c' : (used > 85 ? '#b45309' : '#15803d'));
        p2 += '<tr>' + [
          '<b>' + x[0] + '</b>', x[1].G.toFixed(1), Math.round(x[1].Re).toLocaleString(), x[1].f.toFixed(4),
          '<b>' + x[2].perPlate.toFixed(2) + ' kPa</b>', '× ' + x[2].npassSeries,
          x[2].ch.toFixed(1) + ' kPa', x[2].port.toFixed(1) + ' kPa',
          '<b style="color:' + col + ';">' + x[2].dp.toFixed(1) + ' kPa</b>', x[3].toFixed(1) + ' kPa',
          '<b style="color:' + col + ';">' + (isFinite(used) ? used.toFixed(0) + ' %' : '—') + '</b>'
        ].map(function (y) { return '<td style="' + td + '">' + y + '</td>'; }).join('') + '</tr>';
      });
      p2 += '</table>';
      p2 += '<div style="' + note + '">ΔP per plate is the channel friction over one plate length, '
        + '4·f·(Lp/Dh)·G²/2ρ. Channels within a pass are in parallel and each sees the full plate, so the pack '
        + 'total is that figure times the passes in series. The port term covers the inlet/outlet manifold and '
        + 'port entry/exit and is the flange-to-flange path — the nozzle loss in the connection schedule is the '
        + 'same physical loss and is not counted twice. A total over its allowance is cured differently '
        + 'depending on which column is large: a high ΔP/plate wants a shorter plate or a wider channel gap, '
        + 'a high pass count wants a different arrangement.</div>';
      out.push({ title: 'PRESSURE DROP BUILD-UP — PER PLATE AND PER TERM', html: p2 });
    }
    return out;
  }
  ...
  function unitSpan(unit, utype, extra) {
    ...
    return '<span class="unit"' + (utype ? ' data-unit-type="' + utype + '"' : '') + ' style="font-size:10px;color:#64748b;min-width:38px;text-transform:none;' + (extra || '') + '">' + sym + '</span>';
  }
  function fld(label, id, unit, v, step, utype) {
    return '<label style="display:block;margin:4px 0;font-family:var(--font-mono);font-size:11px;color:var(--text-muted);">' + label
      ...
      + 'style="flex:1;background:rgba(2,6,18,0.6);border:1px solid var(--border-muted);color:#e2e8f0;font-family:var(--font-mono);font-size:13px;padding:6px 9px;border-radius:3px;"/>'
  ...
  function dateFld(label, id) {
    return '<label style="display:block;margin:4px 0;font-family:var(--font-mono);font-size:11px;color:var(--text-muted);">' + label
      + '<input id="' + id + '" type="date" value="' + new Date().toISOString().slice(0, 10) + '" style="width:100%;margin-top:2px;background:rgba(2,6,18,0.6);border:1px solid var(--border-muted);color:#e2e8f0;font-family:var(--font-mono);font-size:13px;padding:6px 9px;border-radius:3px;box-sizing:border-box;color-scheme:dark;"/></label>';
  }
  function txt(label, id, v) {
    return '<label style="display:block;margin:4px 0;font-family:var(--font-mono);font-size:11px;color:var(--text-muted);">' + label
      + '<input id="' + id + '" type="text" value="' + (v || '') + '" style="width:100%;margin-top:2px;background:rgba(2,6,18,0.6);border:1px solid var(--border-muted);color:#e2e8f0;font-family:var(--font-mono);font-size:13px;padding:6px 9px;border-radius:3px;box-sizing:border-box;"/></label>';
  }
  function sel(label, id, opts, cur) {
    return '<label style="display:block;margin:4px 0;font-family:var(--font-mono);font-size:11px;color:var(--text-muted);">' + label
      + '<select id="' + id + '" style="width:100%;margin-top:2px;background:rgba(2,6,18,0.6);border:1px solid var(--border-muted);color:#e2e8f0;font-family:var(--font-mono);font-size:13px;padding:6px 9px;border-radius:3px;">'
  ...
  function selStd(label, id, opts, cur) {
    return '<label style="display:block;margin:4px 0;font-family:var(--font-mono);font-size:11px;color:var(--text-muted);">' + label
      ...
      + 'style="width:100%;margin-top:2px;background:rgba(2,6,18,0.6);border:1px solid var(--border-muted);color:#e2e8f0;font-family:var(--font-mono);font-size:13px;padding:6px 9px;border-radius:3px;box-sizing:border-box;"/></label>';
  }
  ...
  function panelHTML() {
    ...
    // ---- LEFT: inputs ----
    h += '<div class="panel panel-input" style="max-height:calc(100vh - 215px);overflow-y:auto;overflow-x:hidden;">'
      ...
      + '<button id="phe-reset" class="phe-hbtn phe-hbtn-red" title="Reset to defaults"><span style="font-size:13px;">↻</span><span>RESET</span></button>'
      + '<button id="phe-dual-config" class="phe-hbtn" title="Enable dual analytical/industrial tuning" style="background:rgba(59,130,246,0.08);border-color:#3b82f6;color:#3b82f6;"><span style="font-size:12px;">⚖</span><span>DUAL CONFIG</span></button></div>'
    ...
    h += twoCol('<div id="phe-hf-name-phase" style="font-family:var(--font-mono);font-size:9px;color:#94a3b8;margin:-2px 0 4px;line-height:1.4;"></div>',
                '<div id="phe-cf-name-phase" style="font-family:var(--font-mono);font-size:9px;color:#94a3b8;margin:-2px 0 4px;line-height:1.4;"></div>');
    ...
    /* Flow arrangement was decided for the engineer and could not be stated.
       The module picked whichever of counter/co-current gave the larger ΔTm,
       which is the right DEFAULT and the wrong RULE: a duty specified as a
       parallel-flow exchanger could not be entered at all, so a parallel-flow
       problem could never be reproduced or checked. AUTO keeps the old
       behaviour and still names what it chose. */
    h += selStd('FLOW ARRANGEMENT', 'phe-arr',
      ['Auto — use the better of the two', 'Counter-current', 'Co-current (parallel flow)'],
      'Auto — use the better of the two');
    h += '<div style="font-family:var(--font-mono);font-size:8.5px;color:#94a3b8;line-height:1.45;margin:1px 0 3px;">↳ A plate pack is close to true counter-current (F ≈ 0.99) and counter-current gives the larger ΔTm, so it is the default. Force <b>co-current</b> only when the exchanger really is piped parallel-flow — ΔTm falls and the area grows.</div>';
    /* Two legitimate ways to size, and only one of them existed. The module
       always derived U from the chevron correlations and sized the pack to
       the duty — correct when you are designing the exchanger. But a great
       many duties arrive with U already fixed by the specification, the
       client's data sheet or a textbook problem, and the question asked is
       simply A = Q / (U · ΔTm). That could not be answered here: the U₀ box
       says so itself — "comparison only". SPECIFIED sizes on the number in
       that box and still reports the correlation U beside it, so the two can
       be compared instead of one silently winning. */
    h += selStd('U BASIS FOR SIZING', 'phe-ubasis',
      ['Calculated — from the plate correlations', 'Specified — use my U₀ figure'],
      'Calculated — from the plate correlations');
    h += '<div style="font-family:var(--font-mono);font-size:8.5px;color:#94a3b8;line-height:1.45;margin:1px 0 3px;">↳ <b>Calculated</b> derives U from the chevron correlations and the fouling entered — use it when designing the exchanger. <b>Specified</b> sizes on the U₀ you typed in the box above the RUN button (A = Q / U₀·ΔTm) — use it when the duty hands you U. The correlation value is still reported for comparison.</div>';
    ...
    h += '<style>.phe-act{flex:1;background:transparent;border:1px solid var(--color-saffron);color:var(--color-saffron);font-family:var(--font-mono);font-size:10px;font-weight:700;padding:8px;border-radius:4px;cursor:pointer;}.phe-act:hover{background:rgba(255,117,56,0.12);}'
      /* Sized and weighted to match the DPHE/STHE result cards (.card-label
         11px / .card-value 20px bold) — PHE's own datasheet used an 11px
         label AND an 11px value with no visual weight between them, which
         read as a wall of same-size text next to those modules' large,
         bold headline numbers. */
      + '.phe-rr{display:flex;justify-content:space-between;align-items:baseline;gap:10px;padding:6px 0;border-bottom:1px dashed var(--border-muted);font-family:var(--font-mono);break-inside:avoid;}'
      + '.phe-rr span{font-size:11px;color:var(--text-muted);}'
      + '.phe-rr b{font-size:15px;font-weight:700;color:var(--text-header);white-space:nowrap;}'
      + '.phe-rr.warn b{color:#ef4444;}.phe-rr.ok b{color:#22c55e;}'
      + '.phe-cardh{font-family:var(--font-mono);font-size:12px;font-weight:800;color:var(--color-saffron);letter-spacing:0.05em;margin:14px 0 4px;break-inside:avoid;break-after:avoid;}'
      /* Two columns FILLED LEFT-TO-RIGHT, ROW BY ROW (CSS grid's default
         auto-placement) rather than CSS columns' top-of-column-1-then-
         jump-to-column-2 order — the natural reading direction DPHE/STHE's
         own card grid already uses, so scanning down the page reads pairs
         of related rows across, not a whole column of one section before
         the next. Each .phe-cardh spans both columns as a section break. */
      + '#phe-results{display:grid;grid-template-columns:1fr 1fr;column-gap:24px;}'
      + '#phe-results .phe-cardh{grid-column:1/-1;}'
      /* A schedule is a wide thing. Dropped into one half of a two-column
         grid it loses its right-hand columns behind a horizontal scrollbar,
         which is no way to read a nozzle list or a pressure-drop build-up —
         and it wastes the other column, which sits empty beside it. These
         span the full panel and lay their explanatory note underneath. */
      + '#phe-results .phe-wide{grid-column:1/-1;width:100%;}'
      + '#phe-results .phe-wide table{width:100%;table-layout:auto;}'
      + '#phe-results .phe-wide th,#phe-results .phe-wide td{white-space:nowrap;}'
      + '@media (max-width:760px){#phe-results{grid-template-columns:1fr;}}'
    ...
    // ---- RIGHT: 3D + results ----
    h += '<div class="panel" style="max-height:calc(100vh - 215px);overflow-y:auto;">'
      ...
      + '<button id="phe-view-flow" class="phe-viewbtn" data-view="flow">🌊 INTERNAL FLOW</button>'
      + '<button id="phe-zoom-in" class="phe-viewbtn" title="Zoom in">ZOOM +</button>'
      + '<button id="phe-zoom-out" class="phe-viewbtn" title="Zoom out">ZOOM &minus;</button></div>'
      + '<div style="font-family:var(--font-mono);font-size:9.5px;color:#94a3b8;text-align:center;margin-bottom:4px;">3D PLATE-PACK — LIVE VIEW &nbsp;·&nbsp; DRAG TO ROTATE · SCROLL OR THE ZOOM BUTTONS TO ZOOM</div>'
      ...
      // section order matches STHE/DPHE: 3D (above) -> calc summary -> graphs
      // -> flow comparison -> result summary -> final deliverables
      ...
      + '<div id="phe-flow-compare" style="display:none; margin-top:14px; padding:12px; background:rgba(236,72,153,0.08); border:1px solid rgba(236,72,153,0.2); border-radius:8px;"></div>'
      + '<div id="phe-key-summary" style="margin-top:14px; display:none; border:1px solid rgba(0,184,117,0.35); border-radius:var(--radius-md); padding:10px 12px; background:rgba(0,184,117,0.04);">'
  ...
  /* ── ARODATA adapter — stabilization plan §05 ───────────────────────────
     Same usableInCalc-gated pattern as every other adapter in this
     migration. PHE's own fluid names already carry a temperature qualifier
     ("Water (25°C)", "Hot Water (80°C)") that a plain lowercase key does
     not reproduce, so today this simply never matches — which is correct,
     not a gap: a value at one condition must never silently stand in for a
     different one. It starts matching, and adopting, the day ARODATA
     itself carries a condition-tagged, verified entry under the same name.
     Units: this table's own mu is Pa·s and cp is J/kg·K already — the same
     basis ARODATA reports in, so neither needs conversion here (density
     and thermal conductivity likewise share a basis with ARODATA). */
  window.AROFLUIDLOG = window.AROFLUIDLOG || [];
  function resolvePheFluid(name, legacy) {
    ...
    function tryProp(propKey, legacyVal, outKey) {
      if (legacyVal == null) return;
      try {
        var r = window.ARODATA.resolve(sid, propKey);
        if (r && r.usableInCalc && r.effective && isFinite(r.effective.si)) { out[outKey] = r.effective.si; out.status[outKey] = r.status; }
        else if (r && r.masterPick) {
          window.AROFLUIDLOG.push({ module: 'phe', fluid: name, property: outKey,
            legacy: legacyVal, arodata: r.masterPick.si, status: r.status, at: Date.now() });
          if (window.AROFLUIDLOG.length > 500) window.AROFLUIDLOG.shift();
        }
      } catch (e) {}
    }
    tryProp('density', legacy.rho, 'rho');
    tryProp('mu', legacy.mu, 'mu');
    tryProp('cp', legacy.cp, 'cp');
    tryProp('k', legacy.k, 'k');
    return out;
  }

  // blank a side's physical property fields (used when the user picks Custom/manual)
  /* The six physical properties the fluid library writes into a side. */
  var PROP_FIELDS = ['rho', 'mu', 'muw', 'cp', 'k', 'sg'];

  /* A property the engineer has typed over is an OVERRIDE and has to survive.
     It did not: RUN called applyFluid() before calc(), so the library wrote
     the selected fluid's numbers back over the sheet every single time. Typing
     a brine's cp of 2.80 and pressing RUN gave 4.18 — water's — and with it an
     energy balance of 0.671 instead of 1.000, a duty of 16 kW instead of
     12.56 kW, and an area, plate count, pressure drop, nozzle size and BOM all
     sized for a fluid that was never entered. It also meant no fluid outside
     the library could be designed for at all.

     A field is flagged the moment it is typed in, and the library then leaves
     it alone. Choosing a different fluid from the dropdown still loads that
     fluid's properties in full — asking for a fluid IS asking for its numbers,
     so that call clears the flags first. */
  function markUserProp(el) { if (el) el.setAttribute('data-aro-userset', '1'); }
  function isUserProp(el) { return !!(el && el.getAttribute('data-aro-userset') === '1'); }
  function clearUserProps(side) {
    PROP_FIELDS.forEach(function (p) {
      var e = $('phe-' + side + '-' + p); if (e) e.removeAttribute('data-aro-userset');
    });
  }
  function wireUserPropFlags() {
    ['hf', 'cf'].forEach(function (side) {
      PROP_FIELDS.forEach(function (p) {
        var e = $('phe-' + side + '-' + p);
        if (!e || e._aroPropBound) return;
        e._aroPropBound = true;
        e.addEventListener('input', function () { markUserProp(e); });
      });
    });
  }

  function blankProps(side) {
    PROP_FIELDS.forEach(function (p) { var e = $('phe-' + side + '-' + p); if (e) e.value = ''; });
    clearUserProps(side);
  }
  // auto-fill a side's physical properties from the fluid library.
  // force = the engineer picked a fluid, so load all of it and drop overrides.
  function applyFluid(side, force) {
    var f = FLUIDS[val('phe-' + side + '-name', '')];
    if (!f) return;                                  // "Custom (manual)" — leave user values
    if (force) clearUserProps(side);
    ...
    var fv = resolvePheFluid(val('phe-' + side + '-name', ''), f);
    /* every write below goes through this, so an overridden field is skipped */
    var keep = function (p) { return isUserProp($('phe-' + side + '-' + p)); };
    if (!keep('rho')) setSI('phe-' + side + '-rho', fv.rho);
    if (!keep('mu') && e('phe-' + side + '-mu')) e('phe-' + side + '-mu').value = fv.mu;
    if (!keep('cp')) setSI('phe-' + side + '-cp', fv.cp / 1000);
    if (!keep('k')) setSI('phe-' + side + '-k', fv.k);
    var sgEl = e('phe-' + side + '-sg'); if (sgEl && !keep('sg')) sgEl.value = f.sg;
    var muwEl = e('phe-' + side + '-muw'); if (muwEl && !keep('muw')) muwEl.value = fv.mu;   // wall viscosity defaults to bulk μ
    if (window.aroSetProvBadge) {
      window.aroSetProvBadge('phe-' + side + '-rho', fv.status.rho);
      window.aroSetProvBadge('phe-' + side + '-mu', fv.status.mu);
      window.aroSetProvBadge('phe-' + side + '-cp', fv.status.cp);
      window.aroSetProvBadge('phe-' + side + '-k', fv.status.k);
    }
  }
  ...
  var UNDO = [], REDO = [], INPUT_IDS = ['phe-project', 'phe-client', 'phe-tag', 'phe-service', 'phe-engineer', 'phe-rev',
    ...
    'phe-ptype', 'phe-pmat', 'phe-gasket', 'phe-L', 'phe-W', 'phe-t', 'phe-b', 'phe-beta', 'phe-dp', 'phe-phi', 'phe-pitch', 'phe-pass', 'phe-arr', 'phe-ubasis', 'phe-margin'];
  ...
  /* Boiling/freezing reference beside the fluid dropdown — "no idea about
     selected fluid at what temperature boil, freeze" for PHE's hot/cold
     fluids. Advisory only (see lib/aro-fluidphase.js); side is 'hf' or
     'cf', matching this module's own field-name convention. */
  function updateFluidPhase(side) {
    var out = $('phe-' + side + '-name-phase');
    if (!out || !window.AROFLUIDPHASE) return;
    var name = val('phe-' + side + '-name', '');
    var lineText = window.AROFLUIDPHASE.line(name);
    if (!lineText) { out.textContent = ''; return; }
    var tin = num('phe-' + side + '-tin', NaN);
    var warn = window.AROFLUIDPHASE.phaseNote(name, tin);
    out.innerHTML = escPhase(lineText) + (warn ? '<br><span style="color:#f59e0b;">' + escPhase(warn) + '</span>' : '');
  }
  ...
  function wire() {
    var ids = ['phe-hf-m', 'phe-hf-tin', 'phe-hf-tout', 'phe-hf-rho', 'phe-hf-mu', 'phe-hf-muw', 'phe-hf-cp', 'phe-hf-k', 'phe-hf-rf', 'phe-hf-sg', 'phe-hf-pdes',
      ...
      'phe-pass', 'phe-arr', 'phe-ubasis', 'phe-margin', 'phe-ptype', 'phe-hf-dpa', 'phe-cf-dpa', 'phe-smart', 'phe-hf-phase', 'phe-cf-phase'];
    ...
    ['phe-hf-tin', 'phe-cf-tin'].forEach(function (id) {
      var e = $(id);
      if (!e) return;
      e.addEventListener('input', function () { updateFluidPhase(id.indexOf('hf') >= 0 ? 'hf' : 'cf'); });
      e.addEventListener('change', function () { updateFluidPhase(id.indexOf('hf') >= 0 ? 'hf' : 'cf'); });
    });
    // fluid-library selectors auto-fill properties then recalc
    var hn = $('phe-hf-name'); if (hn) hn.addEventListener('change', function () { pushUndo(); if (FLUIDS[val('phe-hf-name', '')]) applyFluid('hf', true); else blankProps('hf'); calc(); updateFluidPhase('hf'); });
    var cn = $('phe-cf-name'); if (cn) cn.addEventListener('change', function () { pushUndo(); if (FLUIDS[val('phe-cf-name', '')]) applyFluid('cf', true); else blankProps('cf'); calc(); updateFluidPhase('cf'); });
    updateFluidPhase('hf'); updateFluidPhase('cf');
    ...
    /* Opens the same merged report the top-toolbar REPORT button does —
       lib/aro-engineering.js's reportHtml() now carries everything report()
       above built (design data, fluids/process, thermal, plate/pack,
       pressure drop, mechanical, BOM, graphs, assumptions) plus what that
       function never had (purpose/scope, provenance-tagged inputs, 3D
       snapshot, isometric, calculation trace, pass/fail validation).
       report() itself is left defined and working, just no longer the
       button's target — nothing about merging the two required deleting
       either one. */
    var rb = $('phe-report'); if (rb) rb.addEventListener('click', function () {
      if (window.AROENG && window.AROENG.report) window.AROENG.report(); else report();
    });
    ...
    function setView(v) {
      ...
      /* This canvas lives inside the ARO3DI wrap's ANALYTICAL slot, which is
         display:none whenever INDUSTRIAL is the active picture — so the very
         first time this runs after that slot was hidden, the renderer's own
         cached width/height (set the LAST time init3D()/a resize measured
         it, possibly 0 if that was before the slot was ever shown) is stale.
         ARO3DI's own INDUSTRIAL↔ANALYTICAL switch already re-measures and
         resizes for exactly this reason (aro-industrial3d.js setMode(),
         'ana' branch) — this toggle switches WITHIN the analytical picture
         and never did the same, so geometry could get added to a group
         whose renderer was still sized from before the canvas had any real
         width. Re-measuring here costs nothing when the size hasn't
         actually changed. */
      if (three && three.canvas && three.canvas.clientWidth) {
        three.cam.aspect = three.canvas.clientWidth / Math.max(1, three.canvas.clientHeight);
        three.cam.updateProjectionMatrix();
        three.rn.setSize(three.canvas.clientWidth, three.canvas.clientHeight, false);
      }
    ...
    function zoomPlatePack(factor) {
      if (!three || !three.sph) return;
      three.sph.r = Math.max(three.sph.minR, Math.min(three.sph.maxR, three.sph.r * factor));
      three.place();
    }
    var zi = $('phe-zoom-in'); if (zi) zi.addEventListener('click', function () { zoomPlatePack(1 / 1.2); });
    var zo = $('phe-zoom-out'); if (zo) zo.addEventListener('click', function () { zoomPlatePack(1.2); });
    var sg = $('phe-suggest-geo'); if (sg) sg.addEventListener('click', function () { pushUndo(); suggestGeometry(); });
    /* setSI()/set() (programmatic writes — suggestGeometry, restore(),
       Smart Input) set .value directly with no event, so this only fires
       on a real keystroke or a real selection — exactly "the engineer
       typed their own port size", not "the app suggested one".
       wire() (below) also puts phe-dp in its live-recalc-on-input list, and
       that listener calls compute() on the SAME keystroke — a capturing
       listener on a document ancestor always runs before a bubble-phase
       listener on the target itself, so this is what guarantees
       pheDpUserSet is already true by the time that recalc reads it,
       whichever order the two get attached in. */
    document.addEventListener('input', function (e) {
      if (e.target && e.target.id === 'phe-dp') pheDpUserSet = true;
    }, true);
    // undo / redo / reset
    ...
    // dual-config mode toggle
    var dcb = $('phe-dual-config');
    if (dcb) {
      dcb.addEventListener('click', function () {
        dualConfigMode = !dualConfigMode;
        dcb.classList.toggle('phe-auto', dualConfigMode);
        dcb.style.background = dualConfigMode ? 'rgba(59,130,246,0.2)' : 'rgba(59,130,246,0.08)';
        dcb.style.borderColor = dualConfigMode ? '#3b82f6' : '#3b82f6';
        dcb.title = dualConfigMode ? 'Dual config ON — analytical & industrial sync' : 'Enable dual analytical/industrial tuning';
        if (LAST_ANALYTICAL) calc();
      });
    }
    wireCombo();
  ...
  function compute() {
    ...
    /* F corrects the LMTD for an arrangement that is NOT pure counter-current.
       A single-pass plate pack is pure: one stream up, the other down, no
       cross and no pass turns — so the counter-current LMTD describes it
       exactly and F = 1. The same is true of a genuinely parallel-flow
       exchanger: the co-current LMTD is already its exact mean difference.
       Fixed factors of 0.99 and 0.90 were applied to both regardless, which
       made a hand-checkable single-pass duty impossible to reproduce — the
       0.90 inflated a parallel-flow area by 11 % against the textbook result.
       Multi-pass is where a correction genuinely belongs: the pass turns put
       part of the pack in co-current and part in cross, and the deviation
       grows with the number of passes. */
    var passRaw = val('phe-pass', '1 Pass / 1 Pass');
    var passNums = (passRaw.match(/\d+/g) || ['1', '1']).map(Number);
    var maxPass = Math.max(1, Math.max.apply(null, passNums));
    var singlePass = maxPass <= 1;
    var Fc = singlePass ? 1.00 : (maxPass >= 4 ? 0.93 : (maxPass === 3 ? 0.95 : 0.97));
    var Fp = singlePass ? 1.00 : Fc;
    ...
    /* …unless the engineer has stated the arrangement. A duty specified as
       parallel flow is a fact about the exchanger, not a candidate to be
       optimised away. A forced choice that is thermodynamically impossible
       (temperature cross) is refused rather than silently swapped, so the
       infeasibility is reported instead of hidden. */
    var arrSel = val('phe-arr', 'Auto');
    var arrForced = null;
    if (/counter/i.test(arrSel)) { if (lmCounter.ok) { counter = true; arrForced = 'counter'; } }
    else if (/co-current|parallel/i.test(arrSel)) { if (lmCo.ok) { counter = false; arrForced = 'co'; } }
    ...
    /* Co-current effectiveness for the flow-arrangement comparison, same
       method DPHE's own comparison uses (app.js, window.dpheFlowAnalysis):
       back out NTU from the actual (counter-current-basis) effectiveness
       above, then apply the co-current ε-NTU relation to that same NTU.
       Matched to DPHE's approach on purpose, not a separate derivation —
       the two modules' flow comparisons should agree in method. */
    var effNTU = (eff > 0.999) ? 999 : -Math.log(1 - eff) / (1 + Cr);
    var coEff = (Cr < 0.9999) ? (1 - Math.exp(-effNTU * (1 + Cr))) / (1 + Cr) : effNTU / (1 + effNTU);

    var flowCmp = {
      counterLMTD: lmCounter.lmtd, coLMTD: lmCo.lmtd,
      counterDTm: dTmCounter, coDTm: dTmCo,
      counterOk: lmCounter.ok, coOk: lmCo.ok,
      counterEff: eff, coEff: coEff,
      best: counter ? 'Counter-current' : 'Co-current',
      forced: arrForced,                 /* 'counter' | 'co' | null (auto) */
      selection: arrSel,
    ...
    /* Port/nozzle diameter — auto-calculated from the duty (same method
       suggestGeometry() below already uses for its own port step: 0.30×W as
       a start, then widened until port velocity is at/under 3 m/s) UNLESS
       the user has actually typed their own value into the field, tracked
       by pheDpUserSet. Recompute happens every run, so the suggested size
       tracks the current duty/geometry live rather than freezing at
       whatever it was the one time a suggest button was pressed — and once
       a user does override it, their number is never touched, only used. */
    if (!pheDpUserSet) {
      var dpSnap = function (v, s) { return Math.round(v / s) * s; };
      var dpq = Math.max(hot.m / hot.rho, cold.m / cold.rho);          // m3/s, larger side governs
      var dpMmAuto = dpSnap(Wp * 1000 * 0.30, 5);
      var vPortAuto = dpq / (Math.PI * Math.pow(dpMmAuto / 1000, 2) / 4);
      var dpGuard = 0;
      while (vPortAuto > 3 && dpGuard++ < 12) {
        dpMmAuto += 5;
        vPortAuto = dpq / (Math.PI * Math.pow(dpMmAuto / 1000, 2) / 4);
      }
      setSI('phe-dp', dpMmAuto);
    }
    var Dp = num('phe-dp', 150) / 1000, pitch = num('phe-pitch', 3.0) / 1000;
    /* All four ports share one bore, so one of the two streams has to set it.
       Recorded here rather than left implicit, because "why is the nozzle this
       size" is the first question asked of a port schedule. */
    var qHot = hot.m / hot.rho, qCold = cold.m / cold.rho;
    var portBasis = {
      gov: qHot >= qCold ? 'HOT' : 'COLD',
      q: Math.max(qHot, qCold),
      vGov: Math.max(qHot, qCold) / (Math.PI * Dp * Dp / 4),
      startMm: Wp * 1000 * 0.30,
      vLimit: 3,
      userSet: pheDpUserSet
    };
    ...
    /* PLATE TYPE has to reach the film coefficient, or it is a dead input:
       the channel correlation keys off the corrugation angle alone, so every
       plate type returned an identical U, area and plate count. A wide-gap or
       free-flow plate trades heat transfer for an unobstructed channel and a
       double-wall plate puts a second metal interface between the media —
       real differences that changed nothing on screen.

       This is the same factor the expected-U band already applies, which is
       what made the mismatch worse than doing nothing: the band moved for a
       wide-gap plate while the design stayed put, so the verdict compared the
       design against a target it could not reach. One factor, used in both
       places, keeps them honest. */
    var fPtype = ptypeFilm(ptype);
    ...
    function dP(s, side) {
      ...
      /* Per-plate figure. Every channel in a pass is in parallel and sees the
         full plate length, so one plate's friction is the channel drop over a
         single pass length; the passes are in series, which is where the
         factor npass in dpCh comes from. Reporting only the pack total hides
         which of the two — plate length or pass count — is spending the
         allowance, and that is the first thing you change when a design is
         over its ΔP. */
      var dpPerPlate = 4 * side.f * (Lp / Dh) * (side.G * side.G / (2 * s.rho));  // Pa, one pass length
      /* velocity head, the unit both loss terms are counted in */
      var velHead = side.G * side.G / (2 * s.rho);                                 // Pa
      return { dp: (dpCh + dpPort) / 1000, ch: dpCh / 1000, port: dpPort / 1000, vport: vport,
               perPlate: dpPerPlate / 1000, velHead: velHead / 1000,
               Gport: Gp, npassSeries: npass };   // kPa
    }
    /* U basis. SPECIFIED takes the figure from the U₀ box (SI via its
       data-unit-type), and falls back to the correlations if that box is
       empty or zero rather than dividing by nothing. */
    var uBasisSel = val('phe-ubasis', 'Calculated');
    var uSpecWanted = /specified/i.test(uBasisSel);
    var uSpec = num('aro-phe-u0-in', 0);
    var uSpecUsed = uSpecWanted && isFinite(uSpec) && uSpec > 0;
    ...
    function state(N) {
      ...
      /* The correlation value is always computed and always reported; it is
         only the SIZING that switches to the specified figure. */
      var Udesign = uSpecUsed ? uSpec : Ud;
      var Aprov = (N - 1) * Ap;
      var Areq = Q / (Udesign * dTm) * margin;
      return { N: N, Ncp: Ncp, H: H, C: C, Uclean: Uclean, Ud: Ud, Udesign: Udesign,
        Aprov: Aprov, Areq: Areq,
    ...
    // U0 assumption band — from the service-fluid pair AND the plate
    // construction selected, so changing metal / thickness / corrugation /
    // plate type moves the target the design is judged against.
    var hPhase = val('phe-hf-phase', 'Liquid'), cPhase = val('phe-cf-phase', 'Liquid');
    var hc = sideClass(hot, hPhase), cc = sideClass(cold, cPhase);
    var uSug = uBand(hc, cc, { k: kw, t: t, beta: beta, ptype: ptype, matName: matName });
    ...
    /* Full connection schedule — each of the four sized on its own flow and
       density, checked on velocity and momentum, and given a flange class
       from the design pressure at that connection's own temperature. Carbon
       and stainless take different B16.5 rating groups. */
    var matGroup = /carbon|cs\b/i.test(matName) ? 'CS' : 'SS';
    var PdesH = num('phe-hf-pdes', 10), PdesC = num('phe-cf-pdes', 10);
    var nozzleSchedule = [
      selectNozzle('H1 IN  (hot inlet)',   hot.m,  hot.rho,  hot.mu,  hot.tin,   PdesH, matGroup),
      selectNozzle('H2 OUT (hot outlet)',  hot.m,  hot.rho,  hot.mu,  hot.tout,  PdesH, matGroup),
      selectNozzle('C1 IN  (cold inlet)',  cold.m, cold.rho, cold.mu, cold.tin,  PdesC, matGroup),
      selectNozzle('C2 OUT (cold outlet)', cold.m, cold.rho, cold.mu, cold.tout, PdesC, matGroup)
    ];
    var nozzleWorst = nozzleSchedule.reduce(function (w, n) {
      var rank = { pass: 0, warn: 1, fail: 2 };
      return (rank[n.status] || 0) > (rank[w] || 0) ? n.status : w;
    }, 'pass');
    ...
    return {
      ...
      uBasis: uSpecUsed ? 'specified' : 'calculated', uSpec: uSpecUsed ? uSpec : null,
      Udesign: st.Udesign,
      ...
      counter: counter, flowCmp: flowCmp, matName: matName, mat: mat, ptype: ptype,
      uSug: uSug, uInBand: uInBand, nozzles: nozzles, portBasis: portBasis,
      nozzleSchedule: nozzleSchedule, nozzleWorst: nozzleWorst, matGroup: matGroup,
      smart: smart, auto: auto
    };
  }

  /* INDUSTRIAL CONFIGURATION — tuned version matching market-equivalent units */
  function computeIndustrial(analyticalResult) {
    if (!analyticalResult) return null;
    /* Industrial standard configuration: match analytical duty but with
       industrial-standard plate thickness, slightly conservative plate count,
       and geometric parameters aligned to manufacturer standard offerings. */
    var r = analyticalResult;
    var t_ind = nearestIndustrialThickness(r.t * 1000) / 1000;  // snap to standard
    var Rw_ind = t_ind / r.kw;
    // Slightly more conservative plate count (add 10–15% surface margin for industrial practice)
    var Ncp = r.Ncp;
    var indMargin = 1 + 0.15;  // 15% surface margin for industrial units
    var Areq_ind = r.Q / (r.Ud * r.dTm) * indMargin;
    var Aprov_ind = (r.N + 4) * r.Ap;  // +4 plates for conservative industrial design
    var overSurf_ind = (Aprov_ind / Areq_ind - 1) * 100;
    // Nozzle calculations for both cold and hot sides
    function nozCalc(m, rho, side_name) {
      var Aport = Math.PI * r.Dp * r.Dp / 4;
      var v = m / (rho * Aport);
      var dp = 1.4 * (rho * v * v / 2) / 1000;  // kPa
      return { v: v, dp: dp, name: side_name };
    }
    var nozzles_ind = {
      hotIn: nozCalc(r.hot.m, r.hot.rho, r.nozzles.hotName + ' HOT IN'),
      hotOut: nozCalc(r.hot.m, r.hot.rho, r.nozzles.hotName + ' HOT OUT'),
      coldIn: nozCalc(r.cold.m, r.cold.rho, r.nozzles.coldName + ' COLD IN'),
      coldOut: nozCalc(r.cold.m, r.cold.rho, r.nozzles.coldName + ' COLD OUT'),
      hotName: r.nozzles.hotName,
      coldName: r.nozzles.coldName
    };
    return {
      // Copy core parameters
      hot: r.hot, cold: r.cold, Q: r.Q, Qh: r.Qh, Qc: r.Qc, Qbal: r.Qbal,
      lmtd: r.lmtd, F: r.F, dTm: r.dTm, Cr: r.Cr, Cmin: r.Cmin, Cmax: r.Cmax,
      Qmax: r.Qmax, eff: r.eff, NTU: r.NTU, approach: r.approach,
      // Industrial-adjusted geometry
      Lp: r.Lp, Wp: r.Wp, t: t_ind, b: r.b, phi: r.phi, beta: r.beta,
      Dp: r.Dp, pitch: r.pitch, kw: r.kw, Rw: Rw_ind,
      Dh: r.Dh, Ap: r.Ap,
      // Industrial plate count (+4 from analytical, always conservative)
      N: r.N + 4, Ncp: Ncp, npass: r.npass,
      Areq: Areq_ind, Aprov: Aprov_ind,
      Ureq: r.Q / (Aprov_ind * r.dTm),
      Uclean: r.Uclean, Ud: r.Ud, overSurf: overSurf_ind,
      H: r.H, C: r.C,
      dpH: r.dpH, dpC: r.dpC, dpHa: r.dpHa, dpCa: r.dpCa,
      Pdes: r.Pdes, Phydro: r.Phydro,
      packLen: (r.N + 4) * r.pitch * 1000,
      frameLen: (r.N + 4) * r.pitch * 1000 + 250,
      nBolts: r.nBolts, boltDia: r.boltDia,
      wEmpty: r.Ap * t_ind * 7900 * (r.N + 4) + 0.35 * ((r.N + 4) * r.pitch * 1000 + 250),
      wOper: r.Ap * t_ind * 7900 * (r.N + 4) + 0.35 * ((r.N + 4) * r.pitch * 1000 + 250) + (r.N + 3) * r.Ap * r.Lp * ((r.hot.rho + r.cold.rho) / 2),
      gasket: r.gasket, gasketOk: r.gasketOk,
      counter: r.counter, flowCmp: r.flowCmp,
      matName: r.matName, mat: r.mat, ptype: r.ptype,
      uSug: r.uSug, uInBand: r.uInBand,
      // Industrial-spec nozzles (separate calculations for cold/hot cases)
      nozzles: nozzles_ind,
      smart: r.smart, auto: r.auto,
      // Mark this as industrial config
      isIndustrial: true
    };
  }
  ...
  function applyAutoLock(smart, auto) {
    ...
    /* Port/nozzle diameter — same badge idea, but never read-only: the
       engineer can always type over the auto-calculated size (see
       pheDpUserSet in compute()), unlike the four fields above which are
       genuinely solved-for and have nothing left for the user to enter. */
    var dpE = $('phe-dp');
    if (dpE) {
      var dpLbl = dpE.closest('label');
      if (dpLbl) {
        var dpBadge = dpLbl.querySelector('.phe-auto-badge');
        if (!pheDpUserSet) {
          if (!dpBadge) {
            dpBadge = document.createElement('span');
            dpBadge.className = 'phe-auto-badge';
            dpBadge.style.cssText = 'color:#22c55e;font-weight:700;margin-left:5px;font-size:8.5px;';
            dpBadge.textContent = '⚡ AUTO (editable)';
            dpLbl.querySelector('span') ? dpLbl.insertBefore(dpBadge, dpLbl.querySelector('span')) : dpLbl.appendChild(dpBadge);
          }
        } else if (dpBadge) { dpBadge.remove(); }
      }
    }
  }
  ...
  function calc() {
    ...
    var r = LAST_ANALYTICAL = compute();
    // Compute industrial configuration simultaneously (dual-config mode)
    var rInd = null;
    if (dualConfigMode) {
      LAST_INDUSTRIAL = rInd = computeIndustrial(r);
    }
    var r = LAST = r;  // r is the primary (analytical) for display
    ...
    if (window.AROENG) {
      try {
        window.AROENG.publish('phe', {
          ...
          values: r,
          /* The two schedules go into the issued report as well as the panel.
             A report that says a design is over its pressure-drop allowance
             without showing which term spends it, or lists a nozzle size with
             nothing behind it, has to be read alongside the tool to be
             useful — and the report is what gets printed, sent and filed. */
          sections: reportSchedules(r)
        });
      } catch (e) {}
    }
    ...
    // flow-arrangement comparison (auto-recommended, no user input) — its own
    // section, positioned like STHE/DPHE's FLOW COMPARISON panel rather than
    // buried as the first item in the results list.
    var fc = r.flowCmp;
    var fcHtml = '<div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:8px;">'
      + '<div style="font-weight:700; color:#f472b6; font-family:var(--font-mono); font-size:11px;">⚖ Counter-Current vs Co-Current — PHE Flow Analysis</div></div>'
      + '<div style="font-size:9px; color:#cbd5e1; background:rgba(0,0,0,0.3); padding:8px; border-radius:4px; line-height:1.6;">'
      + row('Counter-current ε', fc.counterEff.toFixed(4), r.counter ? 'ok' : '')
      + row('Co-current ε', fc.coEff.toFixed(4), !r.counter ? 'ok' : '')
      + row('Counter-current ΔTm', U(fc.counterDTm, 'temp-diff', 1) + (fc.counterOk ? '' : ' (infeasible)'), r.counter ? 'ok' : '')
      + row('Co-current ΔTm', U(fc.coDTm, 'temp-diff', 1) + (fc.coOk ? '' : ' (infeasible)'), !r.counter ? 'ok' : '')
      + row('★ Recommended', fc.best + (fc.gain > 0.5 ? ' (−' + f1(fc.gain) + '% area)' : ''), 'ok')
      + '</div>';
    var fcBox = $('phe-flow-compare');
    if (fcBox) { fcBox.innerHTML = fcHtml; fcBox.style.display = 'block'; }
    ...
    /* Which U actually sized the pack has to be stated. Reading an area
       without knowing whether it came from the correlations or from a
       figure the engineer supplied is reading half a result. */
    h += row('Sizing U basis', r.uBasis === 'specified'
      ? 'SPECIFIED ' + U(r.uSpec, 'htc', 0) + ' (correlation gives ' + U(r.Ud, 'htc', 0) + ')'
      : 'CALCULATED from correlations');

    h += '<div class="phe-cardh">16 · PLATE-PACK DESIGN</div>';
    /* A 0.4 m² exchanger and a 400 m² exchanger cannot share one decimal
       place. At 1 dp a small plate pack reads "0.4 m²" whatever it really
       is, which is too coarse to check a design against a hand calculation
       or against a vendor's figure. Decimals now follow magnitude. */
    var areaDp = function (v) { return !isFinite(v) ? 1 : (v < 1 ? 3 : (v < 10 ? 2 : 1)); };
    h += row('Area required', U(r.Areq, 'area', areaDp(r.Areq)));
    h += row('Area provided', U(r.Aprov, 'area', areaDp(r.Aprov)));
    ...
    /* ── PRESSURE DROP BUILD-UP, PER PLATE AND PER TERM ───────────────────
       "pressure drop calculation each plates". A single pack figure says a
       design is over its allowance without saying WHY, and the two cures are
       opposite: a high per-plate drop wants a shorter plate or a wider gap, a
       high pass count wants a different arrangement. Both are now visible,
       with the friction factor and mass flux that produced them. */
    (function () {
      var rows = [
        { k: 'HOT',  s: r.H, d: r.dpH, allow: r.dpHa, f: r.hot },
        { k: 'COLD', s: r.C, d: r.dpC, allow: r.dpCa, f: r.cold }
      ];
      h += '<div class="phe-wide">';
      h += '<div style="font-family:var(--font-mono);font-size:10px;font-weight:700;color:var(--color-saffron);'
        + 'letter-spacing:.06em;margin:10px 0 4px;">PRESSURE DROP BUILD-UP — PER PLATE AND PER TERM</div>';
      h += '<div style="overflow-x:auto;"><table style="width:100%;border-collapse:collapse;'
        ...
        + '<th style="padding:3px 5px;border-bottom:1px solid var(--border-muted);">SIDE</th>'
        + '<th style="padding:3px 5px;border-bottom:1px solid var(--border-muted);">G</th>'
        + '<th style="padding:3px 5px;border-bottom:1px solid var(--border-muted);">Re</th>'
        + '<th style="padding:3px 5px;border-bottom:1px solid var(--border-muted);">f</th>'
        + '<th style="padding:3px 5px;border-bottom:1px solid var(--border-muted);">ΔP / PLATE</th>'
        + '<th style="padding:3px 5px;border-bottom:1px solid var(--border-muted);">× PASSES</th>'
        + '<th style="padding:3px 5px;border-bottom:1px solid var(--border-muted);">CHANNEL</th>'
        + '<th style="padding:3px 5px;border-bottom:1px solid var(--border-muted);">PORT</th>'
        + '<th style="padding:3px 5px;border-bottom:1px solid var(--border-muted);">TOTAL</th>'
        + '<th style="padding:3px 5px;border-bottom:1px solid var(--border-muted);">ALLOW</th>'
        + '<th style="padding:3px 5px;border-bottom:1px solid var(--border-muted);">USED</th>'
        + '</tr></thead><tbody>';
      rows.forEach(function (x) {
        var used = x.allow > 0 ? x.d.dp / x.allow * 100 : NaN;
        var col = !isFinite(used) ? '#94a3b8' : (used > 100 ? '#ef4444' : (used > 85 ? '#f59e0b' : '#22c55e'));
        h += '<tr>'
          + '<td style="padding:3px 5px;color:var(--text-header);font-weight:700;">' + x.k + '</td>'
          + '<td style="padding:3px 5px;">' + x.s.G.toFixed(1) + ' kg/m²s</td>'
          + '<td style="padding:3px 5px;">' + Math.round(x.s.Re).toLocaleString() + '</td>'
          + '<td style="padding:3px 5px;">' + x.s.f.toFixed(4) + '</td>'
          + '<td style="padding:3px 5px;font-weight:700;">' + x.d.perPlate.toFixed(2) + ' kPa</td>'
          + '<td style="padding:3px 5px;">× ' + x.d.npassSeries + '</td>'
          + '<td style="padding:3px 5px;">' + x.d.ch.toFixed(1) + ' kPa</td>'
          + '<td style="padding:3px 5px;">' + x.d.port.toFixed(1) + ' kPa</td>'
          + '<td style="padding:3px 5px;font-weight:700;color:' + col + ';">' + x.d.dp.toFixed(1) + ' kPa</td>'
          + '<td style="padding:3px 5px;">' + x.allow.toFixed(1) + ' kPa</td>'
          + '<td style="padding:3px 5px;color:' + col + ';font-weight:700;">'
          + (isFinite(used) ? used.toFixed(0) + ' %' : '—') + '</td>'
          + '</tr>';
      });
      h += '</tbody></table></div>';
      h += '<div style="font-family:var(--font-mono);font-size:9px;color:#94a3b8;line-height:1.5;'
        + 'padding:6px 8px;margin:4px 0 2px;background:rgba(0,0,0,.14);border-radius:3px;">'
        + '↳ <b>ΔP / plate</b> is the channel friction over one plate length, 4·f·(Lp/Dh)·G²/2ρ. '
        + 'Channels within a pass are in parallel and each sees the full plate, so the pack total is that '
        + 'figure multiplied by the number of passes in series — the <b>× passes</b> column. '
        + '<b>Port</b> is the inlet/outlet manifold and port entry/exit, 1.4 velocity heads per pass at the '
        + 'port bore; it is the flange-to-flange path, so the nozzle loss in the connection schedule above is '
        + 'the same physical loss and is not added twice. '
        + 'A total over its allowance is cured differently depending on which column is large: a high '
        + 'ΔP/plate wants a shorter plate or a wider channel gap, a high pass count wants a different '
        + 'arrangement.</div>';
      h += '</div>';
    }());
    ...
    /* ── CONNECTION SCHEDULE ──────────────────────────────────────────────
       The four connections, each selected on its own flow, and each carrying
       the number that decided it. A velocity alone never selected anything;
       this is the schedule a fabricator and a piping engineer both work from. */
    if (r.nozzleSchedule && r.nozzleSchedule.length) {
      var L0 = r.nozzleSchedule[0].limits || {};
      h += '<div class="phe-wide">';
      h += '<div style="font-family:var(--font-mono);font-size:10px;font-weight:700;color:var(--color-saffron);'
        + 'letter-spacing:.06em;margin:10px 0 4px;">CONNECTION SCHEDULE — ALL FOUR NOZZLES</div>';
      h += '<div style="overflow-x:auto;"><table style="width:100%;border-collapse:collapse;'
        + 'font-family:var(--font-mono);font-size:9.5px;">'
        + '<thead><tr style="color:#94a3b8;text-align:left;">'
        + '<th style="padding:3px 5px;border-bottom:1px solid var(--border-muted);">CONNECTION</th>'
        + '<th style="padding:3px 5px;border-bottom:1px solid var(--border-muted);">SIZE</th>'
        + '<th style="padding:3px 5px;border-bottom:1px solid var(--border-muted);">BORE</th>'
        + '<th style="padding:3px 5px;border-bottom:1px solid var(--border-muted);">FLOW</th>'
        + '<th style="padding:3px 5px;border-bottom:1px solid var(--border-muted);">VEL</th>'
        + '<th style="padding:3px 5px;border-bottom:1px solid var(--border-muted);">ρv²</th>'
        + '<th style="padding:3px 5px;border-bottom:1px solid var(--border-muted);">Re</th>'
        + '<th style="padding:3px 5px;border-bottom:1px solid var(--border-muted);">ΔP</th>'
        + '<th style="padding:3px 5px;border-bottom:1px solid var(--border-muted);">RATING</th>'
        + '</tr></thead><tbody>';
      r.nozzleSchedule.forEach(function (n) {
        var col = n.status === 'fail' ? '#ef4444' : (n.status === 'warn' ? '#f59e0b' : '#22c55e');
        h += '<tr>'
          + '<td style="padding:3px 5px;color:var(--text-header);">' + n.tag + '</td>'
          + '<td style="padding:3px 5px;font-weight:700;color:' + col + ';">DN' + (n.dn || '—') + ' / ' + (n.nps || '—') + '</td>'
          + '<td style="padding:3px 5px;">' + (isFinite(n.id) ? n.id.toFixed(1) + ' mm' : '—') + '</td>'
          + '<td style="padding:3px 5px;">' + (isFinite(n.Qv) ? (n.Qv * 3600).toFixed(2) + ' m³/h' : '—') + '</td>'
          + '<td style="padding:3px 5px;color:' + col + ';">' + (isFinite(n.v) ? n.v.toFixed(2) + ' m/s' : '—') + '</td>'
          + '<td style="padding:3px 5px;">' + (isFinite(n.rhov2) ? Math.round(n.rhov2) : '—') + '</td>'
          + '<td style="padding:3px 5px;">' + (isFinite(n.Re) ? Math.round(n.Re).toLocaleString() : '—') + '</td>'
          + '<td style="padding:3px 5px;">' + (isFinite(n.dp) ? n.dp.toFixed(2) + ' kPa' : '—') + '</td>'
          + '<td style="padding:3px 5px;">' + (n.rating || '—') + '</td>'
          + '</tr>';
      });
      h += '</tbody></table></div>';
      /* The nozzle and the plate port are the SAME flow path in a plate
         exchanger — the flanged stub feeds the port directly. Reporting a
         DN200 connection on a 160 mm port would describe a machine that
         cannot be built: whichever is smaller governs, and a nozzle much
         larger than the port buys nothing because the port still throttles.
         Any material difference is stated rather than left for the reader to
         notice. */
      (function () {
        var portMm = r.Dp * 1000;
        var nozMm = r.nozzleSchedule.reduce(function (m, n) { return Math.max(m, n.id || 0); }, 0);
        if (portMm > 0 && nozMm > 0) {
          var diff = Math.abs(nozMm - portMm) / portMm * 100;
          if (diff > 15) {
            h += '<div style="font-family:var(--font-mono);font-size:9px;line-height:1.5;margin:3px 0;color:#fbbf24;">'
              + '⚠ Connection DN' + Math.round(nozMm) + ' mm bore against a '
              + portMm.toFixed(0) + ' mm plate port — ' + diff.toFixed(0) + ' % apart. '
              + 'They are one flow path: the smaller governs, so the port sets the real velocity. '
              + 'Either open the port to suit the connection or take the next size down.</div>';
          }
        }
      }());
      /* the binding criterion for any connection that is not simply fine */
      r.nozzleSchedule.forEach(function (n) {
        if (n.status && n.status !== 'pass') {
          h += '<div style="font-family:var(--font-mono);font-size:9px;line-height:1.5;margin:3px 0;'
            + 'color:' + (n.status === 'fail' ? '#f87171' : '#fbbf24') + ';">'
            + (n.status === 'fail' ? '✕ ' : '⚠ ') + n.tag + ' — ' + n.note + '</div>';
        }
      });
      h += '<div style="font-family:var(--font-mono);font-size:9px;color:#94a3b8;line-height:1.5;'
        + 'padding:6px 8px;margin:4px 0 2px;background:rgba(0,0,0,.14);border-radius:3px;">'
        + '↳ Each connection is sized on its own flow and density, not on a shared bore: '
        + 'the first standard size whose bore holds the velocity at or below the '
        + (L0.vTarget || 3) + ' m/s design target, never rounded down onto a faster one. '
        + 'Sizes are ASME B36.10M / B36.19M nominal bores at standard wall. '
        + 'Checked against ' + (L0.vVib || 4.5) + ' m/s, above which port-induced plate vibration is possible, '
        + 'an absolute ceiling of ' + (L0.vMax || 6) + ' m/s, and a momentum limit ρv² ≤ '
        + (L0.rhov2Max || 8968) + ' kg/m·s² for liquid. Flange class is screened against ASME B16.5 '
        + 'pressure–temperature ratings, group ' + (r.matGroup === 'CS' ? '1.1 (carbon steel)' : '2.2 (austenitic stainless)')
        + ', at each connection\'s own design temperature.</div>';
      h += '</div>';
    }
    /* Which stream set the bore, and by what rule — the first question asked
       of any port schedule, and it used to be answerable only by reading the
       source. */
    if (r.portBasis) {
      var pb = r.portBasis;
      /* Short label/value pairs only — row() lays its value out on one line and
         clips anything longer. The sentence that needs room goes in the
         wrapping note below instead of being truncated mid-word. */
      h += row('Governing stream', pb.gov + ' — ' + (pb.q * 3600).toFixed(1) + ' m³/h');
      h += row('Auto-size rule', pb.userSet ? 'not applied — bore entered by user'
        : '30 % of plate width, +5 mm steps');
      h += row('Governing velocity', U(pb.vGov, 'velocity', 2) + ' / ' + U(pb.vLimit, 'velocity', 0) + ' limit',
        pb.vGov > pb.vLimit * 1.05 ? 'warn' : 'ok');
      h += '<div style="font-family:var(--font-mono);font-size:9px;color:#94a3b8;line-height:1.5;'
        + 'padding:6px 8px;margin:4px 0 2px;background:rgba(0,0,0,.14);border-radius:3px;">'
        + '↳ All four ports share one bore, set by the larger volumetric stream ('
        + pb.gov.toLowerCase() + ' side). '
        + (pb.userSet
            ? 'The bore here was entered by hand, so the auto-size rule is reported for reference only.'
            : 'Sizing starts at 30 % of plate width (' + U(pb.startMm, 'length-mm', 0)
              + ') and widens in 5 mm steps until the governing port velocity is at or under '
              + U(pb.vLimit, 'velocity', 0) + '.')
        + ' Both services enter and leave on the fixed cover of a symmetric frame, so the four ports '
        + 'are punched to one size — the lighter stream simply runs slower through its own pair.'
        + '</div>';
    }
    ...
    /* Industrial configuration results — displayed alongside analytical when dual-config mode is active */
    if (dualConfigMode && rInd && rInd.isIndustrial) {
      var hInd = '';
      hInd += '<div style="margin-top:20px; padding-top:20px; border-top:1px solid rgba(0,184,117,0.2);"></div>';
      hInd += '<div class="phe-cardh" style="background:rgba(0,184,117,0.08);border-color:rgba(0,184,117,0.3);">🏭 INDUSTRIAL STANDARD DESIGN — MARKET-EQUIVALENT</div>';
      hInd += row('Plate thickness', U(rInd.t * 1000, 'length-mm', 2) + ' (Industrial std)', rInd.overSurf > 0 ? 'ok' : 'warn');
      hInd += row('Total plates', rInd.N + ' (' + (rInd.N - 1) + ' channels) [Analytical: ' + r.N + ']', 'ok');
      hInd += row('Area required', U(rInd.Areq, 'area', areaDp(rInd.Areq)));
      hInd += row('Area provided', U(rInd.Aprov, 'area', areaDp(rInd.Aprov)));
      hInd += row('Over-surface', f1(rInd.overSurf) + ' % [+15% margin applied]', rInd.overSurf < 0 ? 'warn' : 'ok');
      hInd += row('Ud (dirty)', U(rInd.Ud, 'htc', 0));
      hInd += row('Compressed pack', U(rInd.frameLen, 'length-mm', 0));
      hInd += row('Weight empty', U(rInd.wEmpty, 'mass', 0));
      hInd += row('Weight operating', U(rInd.wOper, 'mass', 0));
      hInd += '<div class="phe-cardh" style="background:rgba(56,189,248,0.08);border-color:rgba(56,189,248,0.3);margin-top:12px;">Nozzle Sizing — Industrial</div>';
      hInd += row('HOT IN velocity', U(rInd.nozzles.hotIn.v, 'velocity', 2) + ' · ΔP ' + U(rInd.nozzles.hotIn.dp, 'press-drop-kpa', 1), rInd.nozzles.hotIn.v > 6 ? 'warn' : 'ok');
      hInd += row('HOT OUT velocity', U(rInd.nozzles.hotOut.v, 'velocity', 2) + ' · ΔP ' + U(rInd.nozzles.hotOut.dp, 'press-drop-kpa', 1), rInd.nozzles.hotOut.v > 6 ? 'warn' : 'ok');
      hInd += row('COLD IN velocity', U(rInd.nozzles.coldIn.v, 'velocity', 2) + ' · ΔP ' + U(rInd.nozzles.coldIn.dp, 'press-drop-kpa', 1), rInd.nozzles.coldIn.v > 6 ? 'warn' : 'ok');
      hInd += row('COLD OUT velocity', U(rInd.nozzles.coldOut.v, 'velocity', 2) + ' · ΔP ' + U(rInd.nozzles.coldOut.dp, 'press-drop-kpa', 1), rInd.nozzles.coldOut.v > 6 ? 'warn' : 'ok');

      var resultsEl = $('phe-results');
      if (resultsEl) resultsEl.innerHTML += hInd;
    }
    ...
    try {
      ...
      if (ksP2 && ksC2) {
        ...
        ksC2.innerHTML =
          ...
          + kr2('Port / Nozzle Size (all 4)', (portNps2 ? 'NPS ' + portNps2 + '″ · ' : '') + U(r.Dp * 1000, 'length-mm', 0), pheDpUserSet ? USER3 : AUTO3);
    ...
    /* Display configuration (use industrial if dual-config mode is active) */
    var displayConfig = (dualConfigMode && rInd) ? rInd : r;
    var configLabel = (dualConfigMode && rInd) ? ' [INDUSTRIAL]' : '';

    /* Name the construction on the tag as well as the duty — the model now
       changes shape and colour with these selections, so the caption has to
       say which plate is being shown. */
    var tag = $('phe-3dtag');
    if (tag) tag.textContent = (pheView === 'flow' ? 'INTERNAL FLOW · ' : '')
      + displayConfig.N + ' plates · ' + (displayConfig.N - 1) + ' channels · '
      + U(displayConfig.Q / 1000, 'heat-duty', 0) + ' · ' + displayConfig.ptype
      + ' · ' + displayConfig.matName + ' ' + U(displayConfig.t * 1000, 'length-mm', 1)
      + ' · β' + displayConfig.beta + '° · '
      + U(displayConfig.Lp * 1000, 'length-mm', 0) + '×' + U(displayConfig.Wp * 1000, 'length-mm', 0)
      + (hasGasket(displayConfig.ptype) ? ' · ' + displayConfig.gasket + ' gasket' : ' · gasket-free')
      + configLabel;
    // 3D nozzle labels reflect the user's hot/cold fluid names live
    var nl = function (id, t) { var e = $(id); if (e) e.innerHTML = t; };
    var hn = displayConfig.nozzles.hotName.split(' (')[0], cn = displayConfig.nozzles.coldName.split(' (')[0];
    var dpmm = Math.round(displayConfig.Dp * 1000);
    var tC = function (siT) { return U(siT, 'temperature', 0); };
    nl('phe-noz-hin', '▸ HOT IN · ' + hn + '<br><span style="font-weight:400;">' + tC(displayConfig.hot.tin) + ' · DN' + dpmm + '</span>');
    nl('phe-noz-hout', '◂ HOT OUT · ' + hn + '<br><span style="font-weight:400;">' + tC(displayConfig.hot.tout) + ' · DN' + dpmm + '</span>');
    nl('phe-noz-cin', 'COLD IN · ' + cn + ' ◂<br><span style="font-weight:400;">' + tC(displayConfig.cold.tin) + ' · DN' + dpmm + '</span>');
    nl('phe-noz-cout', 'COLD OUT · ' + cn + ' ▸<br><span style="font-weight:400;">' + tC(displayConfig.cold.tout) + ' · DN' + dpmm + '</span>');
    update3D(displayConfig);
    renderGraphs(r);
  ...
  function init3D() {
    ...
    var sph = { r: 24, theta: 0.9, phi: 1.05, tx: 0, ty: 3, tz: 0, minR: 6, maxR: 80 };
    function place() {
    ...
    /* Exposed the same way the pump viewer exposes window.pumpLiveViewer3D, so
       the plate pack can be inspected from a test without a screenshot. */
    window.pheViewer3D = three;
    ...
    /* Plain scroll zooms this preview directly — no modifier needed, same
       as every other 3D canvas in the app. */
    canvas.addEventListener('wheel', function (e) {
      e.preventDefault();
      sph.r = Math.max(sph.minR, Math.min(sph.maxR, sph.r * (e.deltaY < 0 ? 0.9 : 1.1))); place();
    }, { passive: false });
  ...
  function update3D(r) {
    ...
    var ptype = r.ptype || val('phe-ptype', 'Chevron (Herringbone)');
    ...
    /* The plate's real proportions drive the model: a 2500×500 plate is a tall
       narrow pack and a 1000×1000 plate is a square one, and they must not
       look alike. Hold the on-screen plate AREA roughly constant so the camera
       framing stays steady while the shape tracks the actual Lp/Wp. */
    var aspect = Math.min(5, Math.max(0.8, (r.Lp || 1.2) / Math.max(1e-6, r.Wp || 0.5)));
    var PW = Math.sqrt(44 / aspect), PH = PW * aspect;
    // real plate gauge, floored so a 0.4 mm plate is still visible on screen
    var PT = Math.min(0.22, Math.max(0.05, (r.t || 0.0005) * 1000 * 0.14));
    var packW = N * pitch;
    var x0 = -packW / 2;
    var plOp = flow ? 0.14 : 0.92, frOp = flow ? 0.22 : 1.0;
    var look = matLook(r.matName);
    var steel = new THREE.MeshStandardMaterial({ color: look.c, metalness: look.m, roughness: look.r, transparent: flow, opacity: flow ? 0.35 : 1 });
    /* The pack was drawn in full-strength red and blue against a near-black
       navy frame, so at any zoom the plates were the only thing the eye could
       find — gasket beads, tie bolts and the flow path all disappeared into
       it. The channel tint only has to say WHICH side a channel carries, so
       it is now the plate metal pulled toward red or blue rather than pure
       red or blue, and the frame is light enough for dark gaskets and bolting
       to read against it. */
    function tint(hex, toward, amt) {
      var r1 = hex >> 16 & 255, g1 = hex >> 8 & 255, b1 = hex & 255;
      var r2 = toward >> 16 & 255, g2 = toward >> 8 & 255, b2 = toward & 255;
      return (Math.round(r1 + (r2 - r1) * amt) << 16)
           | (Math.round(g1 + (g2 - g1) * amt) << 8)
           | Math.round(b1 + (b2 - b1) * amt);
    }
    var frameMat = new THREE.MeshStandardMaterial({ color: brazed ? 0x8a97a6 : 0x4a6fa5, metalness: 0.62, roughness: 0.45, transparent: flow, opacity: frOp });
    var hotMat = new THREE.MeshStandardMaterial({ color: tint(look.c, 0xf0705a, 0.42), metalness: 0.35, roughness: 0.52, transparent: true, opacity: plOp });
    var coldMat = new THREE.MeshStandardMaterial({ color: tint(look.c, 0x67a8ea, 0.42), metalness: 0.35, roughness: 0.52, transparent: true, opacity: plOp });
    ...
    /* Gasket bead along each plate's top and bottom edge, in the elastomer's
       own colour — the stripe that makes a gasketed pack look like one. Welded
       and brazed packs get none, which is what tells them apart on sight. */
    if (hasGasket(ptype) && !flow) {
      var gMat = new THREE.MeshStandardMaterial({ color: gasketLook(r.gasket), metalness: 0.05, roughness: 0.85 });
      var gT = Math.min(0.26, pitch * 0.55), gH = Math.max(0.12, PH * 0.022);
      for (var gi = 0; gi < Math.min(N, 48); gi++) {
        var gx = x0 + gi * pitch;
        box(gT, gH, PW + 0.1, gMat, gx, PH + 0.6 - gH / 2, 0);
        box(gT, gH, PW + 0.1, gMat, gx, 0.6 + gH / 2, 0);
      }
    }
    /* Herringbone ridges on the leading plate, at the chevron angle actually
       entered — a 30° plate and a 65° plate are different products and the
       view should not draw them identically. */
    if (!flow) {
      var bAng = Math.max(20, Math.min(75, r.beta || 60));
      var chevMat = new THREE.MeshStandardMaterial({ color: look.c, metalness: look.m, roughness: Math.min(0.95, look.r + 0.18) });
      var rows = 7, ridgeLen = PW / 2 / Math.max(0.26, Math.cos((90 - bAng) * Math.PI / 180));
      for (var ri = 0; ri < rows; ri++) {
        var ry = 0.6 + (ri + 0.9) * (PH / (rows + 1));
        for (var half = 0; half < 2; half++) {
          var rg = new THREE.Mesh(new THREE.BoxGeometry(PT * 0.9, Math.max(0.07, PH * 0.012), ridgeLen), chevMat);
          // mirror the two halves so each row forms the V the corrugation makes
          rg.rotation.x = (half ? 1 : -1) * (90 - bAng) * Math.PI / 180;
          rg.position.set(x0 - PT * 0.8, ry, (half ? 1 : -1) * PW / 4);
          g.add(rg);
        }
      }
    }
    ...
    if (!brazed) for (var tb = 0; tb < Math.min(6, r.nBolts); tb++) {
      ...
      var rodL = packW + 2;
      var rod = new THREE.Mesh(new THREE.CylinderGeometry(0.12, 0.12, rodL, 10), steel);
      rod.rotation.z = Math.PI / 2; rod.position.set(x0 + packW / 2, PH + 1.4, bz); g.add(rod);
      /* Hex nut and washer on each end, same as the industrial view draws —
         a bare rod does not read as the bolting that holds the pack shut. */
      [-1, 1].forEach(function (e4) {
        var nut = new THREE.Mesh(new THREE.CylinderGeometry(0.23, 0.23, 0.2, 6), steel);
        nut.rotation.z = Math.PI / 2;
        nut.position.set(x0 + packW / 2 + e4 * (rodL / 2 - 0.1), PH + 1.4, bz); g.add(nut);
        var wsh = new THREE.Mesh(new THREE.CylinderGeometry(0.27, 0.27, 0.06, 12), frameMat);
        wsh.rotation.z = Math.PI / 2;
        wsh.position.set(x0 + packW / 2 + e4 * (rodL / 2 - 0.24), PH + 1.4, bz); g.add(wsh);
      });
    }
    ...
    /* Frame on whichever dimension is now the largest — a tall 2500 mm plate
       makes the pack taller than it is long, and fitting on pack length alone
       would crop it. */
    three.sph.tx = x0 + packW / 2;
    three.sph.r = Math.max(18, Math.max(packW * 1.7, PH * 1.9, PW * 1.9) + 12);
    three.place();
  }
  ...
  function legChip(x, y, key, label, color, dashed) {
    return '<g class="phe-legchip" data-ser="' + key + '" style="cursor:pointer;">'
      ...
      + '<text x="' + (x + 21) + '" y="' + (y - 2) + '" fill="#cbd5e1" font-size="12">' + label + '</text></g>';
  }
  ...
  // `prefs` (from AROCHARTCTRL's per-chart store) lets an engineer override
  // the axis top, either series color and the plot background live — same
  // gear-icon customization every Chart.js graph in the suite already has
  // (lib/aro-chart-controls.js), retrofitted here since these three graphs
  // are hand-drawn SVG, not Chart.js.
  function gTempProfile(r, prefs) {
    prefs = prefs || {};
    var W = 720, H = 380, pad = 58;
    var bg = prefs.bg || '#0b1220';
    var hotColor = (prefs.colors && prefs.colors['HOT stream']) || '#ef4444';
    var coldColor = (prefs.colors && prefs.colors['COLD stream']) || '#3b82f6';
    ...
    var s = '<svg id="phe-svg-temp" viewBox="0 0 ' + W + ' ' + H + '" style="width:100%;background:' + bg + ';border-radius:8px;font-family:monospace;">';
    s += '<text x="' + (W / 2) + '" y="26" fill="#38bdf8" font-size="16" font-weight="800" text-anchor="middle">TEMPERATURE PROFILE — ' + val('phe-tag', 'PHE-101') + '</text>';
    var tmin = Math.min(hTout, cTin) - 8;
    var tmax = prefs.yMax != null ? prefs.yMax : Math.max(hTin, cTout) + 8;
    function X(f) { return pad + f * (W - 2 * pad); }
    function Y(T) { return (H - pad) - (T - tmin) / (tmax - tmin) * (H - 2 * pad); }
    var axisFmt = (prefs.sci && window.AROCHARTCTRL_SCI) ? window.AROCHARTCTRL_SCI : function (v) { return v.toFixed(0); };
    s += '<line x1="' + pad + '" y1="' + (H - pad) + '" x2="' + (W - pad) + '" y2="' + (H - pad) + '" stroke="#334155"/>';
    s += '<line x1="' + pad + '" y1="' + pad + '" x2="' + pad + '" y2="' + (H - pad) + '" stroke="#334155"/>';
    for (var g5 = 0; g5 <= 5; g5++) { var T = tmin + g5 / 5 * (tmax - tmin); s += '<line x1="' + pad + '" y1="' + Y(T) + '" x2="' + (W - pad) + '" y2="' + Y(T) + '" stroke="#1e293b"/><text x="' + (pad - 6) + '" y="' + (Y(T) + 4) + '" fill="#94a3b8" font-size="12" text-anchor="end">' + axisFmt(T) + '</text>'; }
    s += '<g data-ser="t_hot"><polyline points="' + X(0) + ',' + Y(hTin) + ' ' + X(1) + ',' + Y(hTout) + '" fill="none" stroke="' + hotColor + '" stroke-width="3"/>';
    s += '<text x="' + (X(0.02)) + '" y="' + (Y(hTin) - 6) + '" fill="' + hotColor + '" font-size="13">HOT ' + hTin.toFixed(0) + '→' + hTout.toFixed(0) + tsym + '</text></g>';
    s += '<g data-ser="t_cold">';
    if (r.counter) s += '<polyline points="' + X(0) + ',' + Y(cTout) + ' ' + X(1) + ',' + Y(cTin) + '" fill="none" stroke="' + coldColor + '" stroke-width="3"/>';
    else s += '<polyline points="' + X(0) + ',' + Y(cTin) + ' ' + X(1) + ',' + Y(cTout) + '" fill="none" stroke="' + coldColor + '" stroke-width="3"/>';
    s += '<text x="' + (X(0.55)) + '" y="' + (Y(cTin) + 18) + '" fill="' + coldColor + '" font-size="13">COLD ' + cTin.toFixed(0) + '→' + cTout.toFixed(0) + tsym + '</text></g>';
    s += '<text x="' + X(0.5) + '" y="' + (H - 16) + '" fill="#cbd5e1" font-size="13" text-anchor="middle">Length fraction →</text>';
    /* Was y=pad+2 — the top gridline drawn just above (g5 loop, T=tmax)
       ...
       title already uses. */
    s += '<text x="' + (W - pad) + '" y="' + (pad - 10) + '" fill="#22c55e" font-size="13" text-anchor="end">LMTD ' + U(r.lmtd, 'temp-diff', 1) + ' · ΔTm ' + U(r.dTm, 'temp-diff', 1) + '</text>';
    // clickable legend
    s += legChip(pad, 44, 't_hot', 'HOT stream', hotColor) + legChip(pad + 150, 44, 't_cold', 'COLD stream', coldColor);
    s += '</svg>';
    return s;
  }

  // ── heat-transfer coefficient bar chart (h hot / h cold / clean U / design U vs assumed band) ──
  function gCoeffs(r, prefs) {
    prefs = prefs || {};
    var W = 720, H = 300, padL = 66, padB = 50, padT = 46;
    var bg = prefs.bg || '#0b1220';
    var uLo = fromSIval(r.uSug.lo, 'htc'), uHi = fromSIval(r.uSug.hi, 'htc');
    var defColors = { 'U₀ assumed': '#64748b', 'h hot film': '#ef4444', 'h cold film': '#3b82f6', 'Clean U': '#38bdf8', 'Design U': r.uInBand ? '#22c55e' : '#f59e0b' };
    var items = [
      { l: 'U₀ assumed', v: (uLo + uHi) / 2, k: 'c_uassm' },
      { l: 'h hot film', v: fromSIval(r.H.h, 'htc'), k: 'c_hhot' },
      { l: 'h cold film', v: fromSIval(r.C.h, 'htc'), k: 'c_hcold' },
      { l: 'Clean U', v: fromSIval(r.Uclean, 'htc'), k: 'c_uclean' },
      { l: 'Design U', v: fromSIval(r.Ud, 'htc'), k: 'c_udesign' }
    ].map(function (it) { return Object.assign(it, { c: (prefs.colors && prefs.colors[it.l]) || defColors[it.l] }); });
    var vmax = prefs.yMax != null ? prefs.yMax : Math.max.apply(null, items.map(function (i) { return i.v; }).concat([uHi])) * 1.15;
    var plotW = W - padL - 20, plotH = H - padT - padB, x0 = padL, y0 = H - padB;
    function BY(v) { return y0 - v / vmax * plotH; }
    var axisFmt = (prefs.sci && window.AROCHARTCTRL_SCI) ? window.AROCHARTCTRL_SCI : function (v) { return Math.round(v).toLocaleString(); };
    var s = '<svg id="phe-svg-coeffs" viewBox="0 0 ' + W + ' ' + H + '" style="width:100%;background:' + bg + ';border-radius:8px;font-family:monospace;margin-top:12px;">';
    s += '<text x="' + (W / 2) + '" y="24" fill="#f59e0b" font-size="16" font-weight="800" text-anchor="middle">HEAT-TRANSFER COEFFICIENTS (' + unitSym('htc', 'W/m²·°C') + ')</text>';
    for (var g = 0; g <= 4; g++) { var vv = g / 4 * vmax; var yy = BY(vv); s += '<line x1="' + x0 + '" y1="' + yy + '" x2="' + (W - 20) + '" y2="' + yy + '" stroke="#1e293b"/><text x="' + (x0 - 6) + '" y="' + (yy + 4) + '" fill="#94a3b8" font-size="11" text-anchor="end">' + axisFmt(vv) + '</text>'; }
    ...
    s += '<text x="' + (W - 24) + '" y="' + (BY(uHi) - 5) + '" fill="#22c55e" font-size="11" text-anchor="end">assumed band ' + Math.round(uLo) + '–' + Math.round(uHi) + '</text>';
    ...
    items.forEach(function (it, i) {
      ...
      s += '<text x="' + (cx + bw / 2) + '" y="' + (BY(it.v) - 6) + '" fill="#e2e8f0" font-size="12" text-anchor="middle" font-weight="700">' + Math.round(it.v).toLocaleString() + '</text>';
      s += '<text x="' + (cx + bw / 2) + '" y="' + (y0 + 17) + '" fill="#cbd5e1" font-size="11.5" text-anchor="middle">' + it.l + '</text></g>';
    });
    s += '<line x1="' + x0 + '" y1="' + y0 + '" x2="' + (W - 20) + '" y2="' + y0 + '" stroke="#334155"/>';
    // clickable legend (toggle any bar on/off)
    items.forEach(function (it, i) { s += legChip(x0 + i * 132, 40, it.k, it.l, it.c); });
    s += '<text x="' + (W / 2) + '" y="' + (H - 6) + '" fill="#94a3b8" font-size="11.5" text-anchor="middle">Design U ' + (r.uInBand ? 'within' : 'OUTSIDE') + ' the ' + r.uSug.basis.toLowerCase() + ' band · fouling allowance ' + Math.round((1 - r.Ud / r.Uclean) * 100) + '%</text>';
    return s + '</svg>';
  }

  // ── ΔP & velocity vs % of design flow (both streams sweep 50–150%) ──
  function gDpSweep(r, prefs) {
    prefs = prefs || {};
    var W = 720, H = 320, padL = 64, padR = 64, padB = 50, padT = 46;
    var bg = prefs.bg || '#0b1220';
    ...
    var dpMaxAuto = Math.max(dpH0, dpC0) * Math.pow(1.5, 1.75) * 1.15; dpMaxAuto = Math.max(dpMaxAuto, lim * 1.1);
    var dpMax = prefs.yMax != null ? prefs.yMax : dpMaxAuto;
    var vMax = Math.max(vHot0, vCold0) * 1.6 * 1.15 || 3;
    var defColors = { 'ΔP hot': '#ef4444', 'ΔP cold': '#3b82f6', 'Vel hot': '#f59e0b', 'Vel cold': '#a855f7' };
    function col(name) { return (prefs.colors && prefs.colors[name]) || defColors[name]; }
    ...
    var s = '<svg id="phe-svg-dp" viewBox="0 0 ' + W + ' ' + H + '" style="width:100%;background:' + bg + ';border-radius:8px;font-family:monospace;margin-top:12px;">';
    s += '<text x="' + (W / 2) + '" y="24" fill="#38bdf8" font-size="16" font-weight="800" text-anchor="middle">ΔP &amp; PORT VELOCITY vs % OF DESIGN FLOW</text>';
    for (var g = 0; g <= 4; g++) { var dv = g / 4 * dpMax; var yy = DY(dv); s += '<line x1="' + x0 + '" y1="' + yy + '" x2="' + (W - padR) + '" y2="' + yy + '" stroke="#1e293b"/><text x="' + (x0 - 6) + '" y="' + (yy + 4) + '" fill="#94a3b8" font-size="11" text-anchor="end">' + Math.round(dv) + '</text><text x="' + (W - padR + 6) + '" y="' + (yy + 4) + '" fill="#94a3b8" font-size="11">' + (g / 4 * vMax).toFixed(1) + '</text>'; }
    for (var gx = 50; gx <= 150; gx += 25) { s += '<text x="' + PX(gx) + '" y="' + (y0 + 18) + '" fill="#94a3b8" font-size="11" text-anchor="middle">' + gx + '%</text><line x1="' + PX(gx) + '" y1="' + padT + '" x2="' + PX(gx) + '" y2="' + y0 + '" stroke="#111c30"/>'; }
    // allowable ΔP limit line
    s += '<line x1="' + x0 + '" y1="' + DY(lim) + '" x2="' + (W - padR) + '" y2="' + DY(lim) + '" stroke="#f43f5e" stroke-width="1.4" stroke-dasharray="6 4"/><text x="' + (x0 + 6) + '" y="' + (DY(lim) - 5) + '" fill="#f43f5e" font-size="11">allowable ΔP ' + Math.round(lim) + ' ' + dpsym + '</text>';
    function poly(fn, c, w, dash) { var pts = fracs.map(function (fr) { return PX(fr) + ',' + fn(fr); }).join(' '); return '<polyline points="' + pts + '" fill="none" stroke="' + c + '" stroke-width="' + w + '"' + (dash ? ' stroke-dasharray="' + dash + '"' : '') + '/>'; }
    s += '<g data-ser="d_dphot">' + poly(function (fr) { return DY(dpAt(dpH0, fr)); }, col('ΔP hot'), 2.4) + '<circle cx="' + PX(100) + '" cy="' + DY(dpH0) + '" r="4" fill="' + col('ΔP hot') + '"/></g>';
    s += '<g data-ser="d_dpcold">' + poly(function (fr) { return DY(dpAt(dpC0, fr)); }, col('ΔP cold'), 2.4) + '<circle cx="' + PX(100) + '" cy="' + DY(dpC0) + '" r="4" fill="' + col('ΔP cold') + '"/></g>';
    s += '<g data-ser="d_velhot">' + poly(function (fr) { return VY(vAt(vHot0, fr)); }, col('Vel hot'), 1.8, '5 3') + '</g>';
    s += '<g data-ser="d_velcold">' + poly(function (fr) { return VY(vAt(vCold0, fr)); }, col('Vel cold'), 1.8, '5 3') + '</g>';
    s += '<text x="' + (W / 2) + '" y="' + (H - 6) + '" fill="#cbd5e1" font-size="11.5" text-anchor="middle">% of design flow (both streams) · left axis ΔP (' + dpsym + ') · right axis velocity (' + vsym + ')</text>';
    // clickable legend
    var lg = [['ΔP hot', col('ΔP hot'), 'd_dphot', 0], ['ΔP cold', col('ΔP cold'), 'd_dpcold', 0], ['Vel hot', col('Vel hot'), 'd_velhot', 1], ['Vel cold', col('Vel cold'), 'd_velcold', 1]];
    lg.forEach(function (L, i) { s += legChip(x0 + i * 128, 40, L[2], L[0], L[1], L[3]); });
    return s + '</svg>';
  }

  /* full graph pack (coupled to the current design) — used in output panel,
     modal & report. These are hand-drawn SVGs at a fixed viewBox (720px
     wide) that scale to fill whatever width their container gives them —
     including the text inside, since SVG font-size is a viewBox unit, not
     a CSS pixel. A 3-up grid (auto-fit, minmax 340px) packed all three into
     roughly half that viewBox width, so every label shrank to ~65% of its
     nominal size: axis numbers and titles that read as "not visible,
     very very small" — exactly the complaint that came in with screenshots
     of this panel. Stacking them one per row fixed that, but a max-width
     capped near the native viewBox size left visible side gaps once the
     panel was wider than that cap — its own complaint, from the same
     screenshots this was meant to answer. No cap: each chart now fills
     100% of whatever width its container actually has, which is also what
     keeps it correct across a browser resize or a page zoom — nothing here
     is measured once and fixed, the SVG's own viewBox scaling redoes the
     fit on every layout change for free. */
  function buildGraphs(r) {
    var chartPrefs = function (key) { return window.AROCHARTCTRL ? window.AROCHARTCTRL.customPrefs(key) : {}; };
    return '<div style="display:flex;flex-direction:column;gap:16px;">'
      + '<div style="width:100%;">' + gTempProfile(r, chartPrefs('phe-graph-temp')) + '</div>'
      + '<div style="width:100%;">' + gCoeffs(r, chartPrefs('phe-graph-coeffs')) + '</div>'
      + '<div style="width:100%;">' + gDpSweep(r, chartPrefs('phe-graph-dp')) + '</div>'
      + '</div>';
  }
  // wires the same live customization gear (axis scale, series color,
  // background) every Chart.js graph in the suite already has onto these
  // hand-drawn SVGs — call once after each (re)insertion into the DOM.
  // `root` scopes the lookup: the output panel and the GRAPH modal can both
  // hold a copy of these charts (same ids) at once, so a plain
  // document-wide getElementById would silently grab whichever copy happens
  // to sit first in the document instead of the one just inserted here.
  function wireGraphCustomization(r, root) {
    if (!window.AROCHARTCTRL) return;
    root = root || document;
    var specs = [
      { id: 'phe-svg-temp', key: 'phe-graph-temp', fn: gTempProfile,
        defaults: function () { return { yMax: Math.max(fromSIval(r.hot.tin, 'temperature'), fromSIval(r.cold.tout, 'temperature')) + 8, bg: '#0b1220', series: ['HOT stream', 'COLD stream'], colors: { 'HOT stream': '#ef4444', 'COLD stream': '#3b82f6' } }; } },
      { id: 'phe-svg-coeffs', key: 'phe-graph-coeffs', fn: gCoeffs,
        defaults: function () { return { bg: '#0b1220', series: ['U₀ assumed', 'h hot film', 'h cold film', 'Clean U', 'Design U'], colors: { 'U₀ assumed': '#64748b', 'h hot film': '#ef4444', 'h cold film': '#3b82f6', 'Clean U': '#38bdf8', 'Design U': r.uInBand ? '#22c55e' : '#f59e0b' } }; } },
      { id: 'phe-svg-dp', key: 'phe-graph-dp', fn: gDpSweep,
        defaults: function () { return { bg: '#0b1220', series: ['ΔP hot', 'ΔP cold', 'Vel hot', 'Vel cold'], colors: { 'ΔP hot': '#ef4444', 'ΔP cold': '#3b82f6', 'Vel hot': '#f59e0b', 'Vel cold': '#a855f7' } }; } }
    ];
    specs.forEach(function (sp) {
      var svgEl = root.querySelector('#' + sp.id);
      if (!svgEl) return;
      window.AROCHARTCTRL.enhanceCustom(svgEl, sp.key, {
        getDefaults: sp.defaults,
        apply: function (prefs) {
          var cur = root.querySelector('#' + sp.id);
          if (!cur) return;
          cur.outerHTML = sp.fn(LAST || r, prefs);
          applyGraphToggles(root);
        }
      });
    });
  }
  // exposed so the shared top-toolbar report (lib/aro-engineering.js) can embed
  // the same graph pack this module already draws in its own output panel and
  // modal, instead of a fourth reimplementation.
  window.buildGraphs = buildGraphs;
  window.pheLastResult = function () { return LAST; };
  ...
  function renderGraphs(r) {
    ...
    wireGraphToggles(); applyGraphToggles(box); wireGraphCustomization(r, box);
  }
  ...
  function graph() {
    ...
    var mbody = $('phe-mbody');
    wireGraphToggles(); applyGraphToggles(mbody || document); wireGraphCustomization(r, mbody || document);
  }
  ...
  function buildDrawing() {
    ...
    /* The front+side views were drawn at a fixed pixel size tuned years ago
       against a smaller panel, so most of this 1080×540 box sat empty
       around them. Every offset below (nozzle positions, feet, dimension
       lines, balloon leaders) is relative to fx/fy/fw/fh and sx/sy/sh, so
       none of that math needed touching — wrapping the whole front+side
       fragment in one scale transform enlarges everything together,
       preserving every proportion exactly, rather than re-deriving dozens
       of hand-placed offsets by hand. */
    s += '<g transform="translate(-30,-50) scale(1.12)">';
    ...
    ['1. Number of plates (N), heat-transfer area (A) and L / C dimensions shall be as per duty (see design data).',
     ...
     '5. Property of ' + (val('phe-project', 'AROGARA') || 'AROGARA') + ' · REV. ' + (val('phe-rev', '0') || '0') + ' · not to be copied without permission.'
  ...
  function report() {
    ...
    b += '<div style="text-align:center;margin-bottom:14px;"><div style="font-size:18px;font-weight:800;color:#ea580c;">AROGARA — PLATE HEAT EXCHANGER DESIGN REPORT</div><div style="font-size:10px;color:#64748b;">AROGARA · Digital Engineering Design Platform</div></div>';
    ...
    ['Flow arrangement', r.flowCmp.best + ' (auto — counter ε=' + r.flowCmp.counterEff.toFixed(3) + ' ΔTm ' + U(r.flowCmp.counterDTm, 'temp-diff', 1) + ' vs co-current ε=' + r.flowCmp.coEff.toFixed(3) + ' ΔTm ' + U(r.flowCmp.coDTm, 'temp-diff', 1) + ')'],
    ...
    /* The build-up belongs in the issued document too. A report that states a
       design is over its allowance without showing which term spends it makes
       the reader open the tool again to find out — and the report is what gets
       sent out, printed and filed. Same figures as the panel, one grid. */
    (function () {
      var G = function (cells, head) {
        return '<tr>' + cells.map(function (c) {
          return '<' + (head ? 'th' : 'td') + ' style="padding:3px 6px;border:1px solid #e2e8f0;'
            + (head ? 'background:#f1f5f9;text-align:left;color:#334155;' : 'color:#0f172a;') + '">' + c + '</' + (head ? 'th' : 'td') + '>';
        }).join('') + '</tr>';
      };
      b += '<div style="font-size:11px;font-weight:700;color:#334155;margin:8px 0 3px;">Build-up — per plate and per term</div>';
      b += '<table style="width:100%;border-collapse:collapse;font-size:10px;">';
      b += G(['Side', 'G (kg/m²s)', 'Re', 'f', 'ΔP / plate', '× passes', 'Channel', 'Port', 'Total', 'Allowable', 'Used'], true);
      [['HOT', r.H, r.dpH, r.dpHa], ['COLD', r.C, r.dpC, r.dpCa]].forEach(function (x) {
        var used = x[3] > 0 ? (x[2].dp / x[3] * 100) : NaN;
        b += G([x[0], x[1].G.toFixed(1), Math.round(x[1].Re).toLocaleString(), x[1].f.toFixed(4),
                x[2].perPlate.toFixed(2) + ' kPa', '× ' + x[2].npassSeries,
                x[2].ch.toFixed(1) + ' kPa', x[2].port.toFixed(1) + ' kPa',
                x[2].dp.toFixed(1) + ' kPa', x[3].toFixed(1) + ' kPa',
                isFinite(used) ? used.toFixed(0) + ' %' : '—']);
      });
      b += '</table>';
      b += '<div style="font-size:9.5px;color:#64748b;margin:4px 0 0;line-height:1.5;">'
        + 'ΔP per plate is the channel friction over one plate length, 4·f·(Lp/Dh)·G²/2ρ; channels within a '
        + 'pass are in parallel, so the pack total is that figure times the passes in series. The port term '
        + 'covers the inlet/outlet manifold and port entry/exit and is the flange-to-flange path — the nozzle '
        + 'loss in the connection schedule is the same physical loss and is not counted twice.</div>';
    }());

    /* ── CONNECTION SCHEDULE in the report ─────────────────────────────── */
    if (r.nozzleSchedule && r.nozzleSchedule.length) {
      b += sec('16 · CONNECTION SCHEDULE');
      b += '<table style="width:100%;border-collapse:collapse;font-size:10px;">';
      b += '<tr style="background:#f1f5f9;">'
        + ['Connection', 'Size', 'Bore', 'Flow', 'Velocity', 'ρv²', 'Re', 'ΔP', 'Rating', 'Status'].map(function (x) {
            return '<th style="padding:4px;border:1px solid #e2e8f0;text-align:left;">' + x + '</th>'; }).join('')
        + '</tr>';
      r.nozzleSchedule.forEach(function (n) {
        var col = n.status === 'fail' ? '#b91c1c' : (n.status === 'warn' ? '#b45309' : '#15803d');
        b += '<tr>' + [
          ...
          isFinite(n.dp) ? n.dp.toFixed(2) + ' kPa' : '—',
          n.rating || '—',
          '<span style="color:' + col + ';font-weight:700;">' + (n.status || '—').toUpperCase() + '</span>'
        ].map(function (x) { return '<td style="padding:4px;border:1px solid #e2e8f0;">' + x + '</td>'; }).join('') + '</tr>';
      });
      b += '</table>';
      r.nozzleSchedule.forEach(function (n) {
        if (n.status && n.status !== 'pass') {
          b += '<div style="font-size:9.5px;margin:3px 0;color:' + (n.status === 'fail' ? '#b91c1c' : '#b45309') + ';">'
            + (n.status === 'fail' ? '✕ ' : '⚠ ') + n.tag + ' — ' + n.note + '</div>';
        }
      });
      var L0 = r.nozzleSchedule[0].limits || {};
      b += '<div style="font-size:9.5px;color:#64748b;margin:4px 0 0;line-height:1.5;">'
        + 'Each connection sized on its own flow and density — the first standard bore holding velocity at or '
        + 'below ' + (L0.vTarget || 3) + ' m/s, never rounded down. ASME B36.10M / B36.19M nominal bores at '
        + 'standard wall. Checked against ' + (L0.vVib || 4.5) + ' m/s (port-induced plate vibration possible above), '
        + 'a ' + (L0.vMax || 6) + ' m/s ceiling and ρv² ≤ ' + (L0.rhov2Max || 8968) + ' kg/m·s². Flange class per '
        + 'ASME B16.5 pressure–temperature ratings, group '
        + (r.matGroup === 'CS' ? '1.1 (carbon steel)' : '2.2 (austenitic stainless)')
        + ', at each connection\'s own design temperature.</div>';
    }
    b += sec('12 · MECHANICAL');
  ...
  /* The MECHANICAL section (design/hydrotest pressure, frame length, tie-
     bolts, weight) and the summary BOM only existed inside report() above —
     exposed so the shared top-toolbar report can carry the same numbers
     instead of a design-pressure figure that only ever showed up in one of
     the two reports a PHE design can produce. */
  window.pheMechanicalHTML = function (r) {
    if (!r) return '';
    function T2(rows) { return '<table style="width:100%;border-collapse:collapse;font-size:11px;margin:6px 0;">' + rows.map(function (r2) { return '<tr><td style="padding:3px 8px;border:1px solid #e2e8f0;color:#475569;">' + r2[0] + '</td><td style="padding:3px 8px;border:1px solid #e2e8f0;font-weight:700;color:#0f172a;">' + r2[1] + '</td></tr>'; }).join('') + '</table>'; }
    var h = '<div style="font-size:10px;font-weight:800;color:#374151;margin:0 0 4px;">MECHANICAL DESIGN</div>'
      + T2([
        ['Design / hydrotest pressure', UG(r.Pdes, 1) + ' / ' + UG(r.Phydro, 1)],
        ['Frame / pack length', U(r.frameLen, 'length-mm', 0) + ' / ' + U(r.packLen, 'length-mm', 0)],
        ['Tie-bolts', r.nBolts + ' × M' + r.boltDia],
        ['Weight empty / operating', U(r.wEmpty, 'mass', 0) + ' / ' + U(r.wOper, 'mass', 0)]
      ]);
    return h;
  };
  ...
  /* One page-size choice, shared by every export path in the app (the
     generic AROENG report, STHE's manufacturing report, and every drawing
     modal) — so choosing A3 once in any report's header applies everywhere
     without asking again per module. */
  function pdfFormatPref() {
    try { return localStorage.getItem('aro_pdf_format_v1') === 'a3' ? 'a3' : 'a4'; }
    catch (e) { return 'a4'; }
  }
  function setPdfFormatPref(fmt) {
    try { localStorage.setItem('aro_pdf_format_v1', fmt === 'a3' ? 'a3' : 'a4'); } catch (e) {}
  }
  window.AROPDF_FORMAT = { get: pdfFormatPref, set: setPdfFormatPref };
  ...
  window.AROPDF = function (element, filename, opts) {
    ...
    var pdfFormat = (opts.format === 'a3' || opts.format === 'a4') ? opts.format : pdfFormatPref();
    var sheetDims = (pdfFormat === 'a3') ? [297, 420] : [210, 297];  // portrait [short, long], mm
    var PW = landscape ? sheetDims[1] : sheetDims[0], PH = landscape ? sheetDims[0] : sheetDims[1];
    ...
    var fallback = function (err) {
      ...
      return html2pdf().set({
        ...
        jsPDF: { unit: 'mm', format: pdfFormat, orientation: landscape ? 'landscape' : 'portrait' },
    ...
    function paginate() {
      ...
      setTimeout(function () {
        ...
        /* The page setup has to go to THIS call too, not only to the one that
           ...
           windowWidth was tried here to pin html2canvas's virtual viewport to
           hostW as well, on the theory that a narrow real browser width was
           letting a mobile media query resize the A3/A4 choice's effect away.
           It did the opposite: html2canvas positions the OFFSCREEN host (a
           position:fixed, width:0, overflow:hidden clipping box holding pad)
           using the REAL window's dimensions, so forcing a mismatched virtual
           width moved its capture region off of where pad actually sits,
           rendering blank pages for every module's report export, on every
           device — a much worse regression than the one it was meant to fix.
           Removed; pad's own explicit inline width already governs what
           gets captured, without needing html2canvas to also know about it. */
        html2pdf().set({
            margin: margin,
            jsPDF: { unit: 'mm', format: pdfFormat, orientation: landscape ? 'landscape' : 'portrait' },
          ...
          .then(function (canvas) {
            ...
            return html2pdf().set({ jsPDF: { unit: 'mm', format: pdfFormat, orientation: landscape ? 'landscape' : 'portrait' } })
  ...
  function exportDrawingPdf(body, filename, done) {
    ...
    svgToImg(svg, 2, function (png) {
      ...
      html2pdf().set({ jsPDF: { unit: 'mm', format: pdfFormatPref(), orientation: 'landscape' }, html2canvas: { scale: 1 } }).from(dummy).toPdf().get('pdf').then(function (pdf) {
  ...
  window.AROPHE = { calc: calc, compute: compute, report: report, drawing: drawing,
    ...
    /* exposed so the input-side U0 preview (lib/aro-pheu0.js) classifies the
       service and reads its band from this exact rule, not a copy of it.
       construction() hands it the plate selection through the same unit-aware
       reader compute() uses, so the preview and the results card cannot
       disagree about which plate is on the form. */
    sideClass: sideClass, uBand: uBand, construction: readConstruction,
    gasketTmax: GASKET_TMAX,
    /* exposed so the industrial 3D view (lib/aro-industrial3d.js) draws the
       selected metal and elastomer from this same table rather than a second
       copy of it, and agrees with the analytical view about which is which. */
    matLook: matLook, gasketLook: gasketLook, hasGasket: hasGasket };

...
(function () {
  ...
  function drawing() {
    ...
    /* ═════ 1 · GENERAL ARRANGEMENT (front + side + top views) ═════ */
    s += panel(16, 16, 1080, 700, '1 · GENERAL ARRANGEMENT');

    // ---- FRONT VIEW ----
    var fx = 70, fy = 90, fw = 150, fh = 420;
    s += txt(fx + fw / 2, fy - 12, 'FRONT VIEW', 9.5, '#0f172a', 'middle', '700');
    s += rect(fx, fy, fw, fh, '#f8fafc', '#0f172a', 1.4);
    s += rect(fx + 10, fy + 10, fw - 20, fh - 20, 'none', '#94a3b8', 0.5);
    // 4 nozzles: H1 IN (hot in) TL, H2 OUT (cold out) TR, H2 IN (cold in) BL, H1 OUT (hot out) BR
    var nfr = [[fx + 42, fy + 66, 'H1 IN', '#dc2626'], [fx + fw - 42, fy + 66, 'H2 OUT', '#2563eb'], [fx + 42, fy + fh - 66, 'H2 IN', '#2563eb'], [fx + fw - 42, fy + fh - 66, 'H1 OUT', '#dc2626']];
    nfr.forEach(function (n) { s += '<circle cx="' + n[0] + '" cy="' + n[1] + '" r="20" fill="#fff" stroke="' + n[3] + '" stroke-width="1.6"/><circle cx="' + n[0] + '" cy="' + n[1] + '" r="11" fill="none" stroke="' + n[3] + '" stroke-width="1"/>';
      // 4 flange bolt holes
      [[-13, 0], [13, 0], [0, -13], [0, 13]].forEach(function (d) { s += '<circle cx="' + (n[0] + d[0]) + '" cy="' + (n[1] + d[1]) + '" r="2" fill="none" stroke="' + n[3] + '"/>'; });
    });
    s += txt(nfr[0][0] - 24, nfr[0][1] + 34, 'H1 IN', 8, '#dc2626', 'start', '700');
    s += txt(nfr[1][0] + 24, nfr[1][1] + 34, 'H2 OUT', 8, '#2563eb', 'end', '700');
    s += txt(nfr[2][0] - 24, nfr[2][1] - 26, 'H2 IN', 8, '#2563eb', 'start', '700');
    s += txt(nfr[3][0] + 24, nfr[3][1] - 26, 'H1 OUT', 8, '#dc2626', 'end', '700');
    // feet
    s += rect(fx + 12, fy + fh, 34, 14, '#e2e8f0', '#0f172a', 1) + rect(fx + fw - 46, fy + fh, 34, 14, '#e2e8f0', '#0f172a', 1);
    // dims: width (top) & height (left)
    s += dim(fx, fy - 30, fx + fw, fy - 30, String(Wframe));
    s += dim(fx - 34, fy, fx - 34, fy + fh, String(Hframe));
    s += dim(fx + 12, fy + fh + 30, fx + fw - 12, fy + fh + 30, String(Wframe - 60));
    // balloons on front view
    s += bln(fx - 6, fy + 6, '1', fx + 6, fy + 14);
    s += bln(nfr[0][0] - 40, nfr[0][1], '5', nfr[0][0] - 20, nfr[0][1]);

    // ---- SIDE VIEW (plate pack between fixed frame & pressure plate) ----
    var sx = 300, sy = 90, sh = 420, packW = Math.min(300, Math.max(120, r.N * 3));
    s += txt(sx + (60 + packW) / 2, sy - 12, 'SIDE VIEW', 9.5, '#0f172a', 'middle', '700');
    // top carry bar & bottom guide bar
    s += rect(sx - 10, sy - 10, packW + 90, 8, '#e2e8f0', '#0f172a', 1);      // top carry bar (3)
    s += rect(sx - 10, sy + sh + 2, packW + 90, 8, '#e2e8f0', '#0f172a', 1);  // bottom carry bar (4)
    // fixed frame plate (left)
    s += rect(sx, sy, 18, sh, '#cbd5e1', '#0f172a', 1.4);
    // plate pack (hatched)
    var pkx = sx + 26;
    s += rect(pkx, sy + 8, packW, sh - 16, '#f1f5f9', '#0f172a', 1);
    var nlines = Math.min(60, Math.max(10, r.N));
    for (var i = 0; i < nlines; i++) { var px = pkx + 3 + i * ((packW - 6) / nlines); s += line(px, sy + 8, px, sy + sh - 8, 0.7, '#94a3b8'); }
    // pressure plate (movable, right)
    var ppx = pkx + packW + 6;
    s += rect(ppx, sy, 18, sh, '#cbd5e1', '#0f172a', 1.4);
    // tie bolts (horizontal through pack)
    var nTie = Math.min(4, Math.max(2, Math.ceil(r.nBolts / 4)));
    for (var tb = 0; tb < nTie; tb++) { var by = sy + 50 + tb * (sh - 100) / (nTie - 1 || 1); s += line(sx - 14, by, ppx + 30, by, 2.2, '#334155'); s += '<circle cx="' + (ppx + 30) + '" cy="' + by + '" r="4" fill="none" stroke="#0f172a"/>'; }
    // side nozzles (project from fixed frame, left)
    [[sy + 66, '#dc2626'], [sy + sh - 66, '#2563eb']].forEach(function (n) { s += rect(sx - 34, n[0] - 10, 18, 20, '#fff', n[1], 1.4) + '<circle cx="' + (sx - 40) + '" cy="' + n[0] + '" r="3" fill="none" stroke="' + n[1] + '"/>'; });
    // feet
    s += rect(sx + 4, sy + sh + 10, 30, 14, '#e2e8f0', '#0f172a', 1) + rect(ppx - 10, sy + sh + 10, 30, 14, '#e2e8f0', '#0f172a', 1);
    // dims: L (top overall) ±5, C (bottom tie centres) ±5, height right
    s += dim(sx, sy - 34, ppx + 18, sy - 34, 'L ±5 = ' + Ltot);
    s += dim(sx - 14, sy + sh + 34, ppx + 30, sy + sh + 34, 'C ±5 = ' + Ctie);
    s += dim(ppx + 44, sy, ppx + 44, sy + sh, String(Hframe - 140));
    // balloons on side view
    s += bln(sx + 9, sy - 22, '3', sx + 20, sy - 8);
    s += bln(sx + 9, sy + sh + 30, '4', sx + 20, sy + sh + 8);
    s += bln(pkx + packW / 2, sy - 22, '6', pkx + packW / 2, sy + 8);
    s += bln(ppx + 9, sy + 30, '2', ppx + 9, sy + 40);
    s += bln(ppx + 30, sy + 50, '10', ppx + 20, sy + 50);

    // ---- TOP VIEW ----
    var tx = 70, ty = 560, tw = 470, th = 120;
    s += txt(tx + tw / 2, ty - 10, 'TOP VIEW', 9.5, '#0f172a', 'middle', '700');
    s += rect(tx, ty, 16, th, '#cbd5e1', '#0f172a', 1.2);
    s += rect(tx + 22, ty + 8, tw - 60, th - 16, '#f1f5f9', '#0f172a', 1);
    for (var it = 0; it < 40; it++) { var ix = tx + 26 + it * ((tw - 68) / 40); s += line(ix, ty + 8, ix, ty + th - 8, 0.6, '#94a3b8'); }
    s += rect(tx + tw - 34, ty, 16, th, '#cbd5e1', '#0f172a', 1.2);
    // top nozzles pair on left
    s += rect(tx - 24, ty + 22, 24, 16, '#fff', '#dc2626', 1.3) + rect(tx - 24, ty + th - 38, 24, 16, '#fff', '#2563eb', 1.3);
    s += dim(tx + tw + 14, ty, tx + tw + 14, ty + th, String(Wframe - 60));

    /* ═════ 2 · PLATE DETAIL (TYP.) ═════ */
    s += panel(1112, 16, 412, 340, '2 · PLATE DETAIL (TYP.)');
    var plx = 1150, ply = 70, plw = 150, plh = 250;
    s += '<rect x="' + plx + '" y="' + ply + '" width="' + plw + '" height="' + plh + '" rx="18" fill="#eef2f7" stroke="#0f172a" stroke-width="1.3"/>';
    for (var c = 0; c < 18; c++) { var yy = ply + 22 + c * (plh - 44) / 18; s += '<polyline points="' + plx + ',' + yy + ' ' + (plx + plw / 2) + ',' + (yy + 9) + ' ' + (plx + plw) + ',' + yy + '" fill="none" stroke="#94a3b8" stroke-width="1"/>'; }
    ...
    s += dim(plx - 24, ply, plx - 24, ply + plh, String(Lp));
    s += dim(plx + 30, ply + plh + 20, plx + plw - 30, ply + plh + 20, String(Math.round(Wp * 0.78)));
    // thickness strip to the right
    var thx = plx + plw + 44;
    s += rect(thx, ply, 10, plh, '#cbd5e1', '#0f172a', 1);
    s += txt(thx + 5, ply - 8, tmm, 8, '#dc2626', 'middle', '700');
    s += txt(thx + 26, ply + plh + 16, (r.b * 1000).toFixed(1), 8, '#dc2626', 'middle', '700');
    s += txt(plx + plw / 2, ply + plh + 40, 'β ' + r.beta + '° CHEVRON · t ' + tmm + ' mm · φ ' + r.phi.toFixed(2), 8, '#64748b', 'middle');

    /* ═════ 3 · PORT / NOZZLE DETAIL (TYP.) ═════ */
    s += panel(1112, 372, 412, 200, '3 · PORT / NOZZLE DETAIL (TYP.)');
    var ndx = 1230, ndy = 430;
    // flange face (OD/ID) with bolt circle
    s += '<circle cx="' + ndx + '" cy="' + ndy + '" r="46" fill="#fff" stroke="#0f172a" stroke-width="1.4"/>';
    s += '<circle cx="' + ndx + '" cy="' + ndy + '" r="22" fill="#eef2f7" stroke="#0f172a" stroke-width="1.2"/>';
    for (var bh = 0; bh < 4; bh++) { var a = Math.PI / 4 + bh * Math.PI / 2; s += '<circle cx="' + (ndx + 34 * Math.cos(a)) + '" cy="' + (ndy + 34 * Math.sin(a)) + '" r="4" fill="none" stroke="#0f172a"/>'; }
    s += dim(ndx - 46, ndy + 66, ndx + 46, ndy + 66, 'Ø' + flOD + ' (OD)');
    s += dim(ndx - 22, ndy - 62, ndx + 22, ndy - 62, 'Ø' + flID + ' (ID)');
    s += txt(ndx, ndy + 92, 'DN ' + DN + ' · ANSI B16.5 CLASS 150 (SERRATED / RF)', 8, '#64748b', 'middle');

    /* ═════ BILL OF MATERIAL ═════ */
    var bx0 = 1112, by0 = 590;
    s += txt(bx0 + 200, by0 + 2, 'BILL OF MATERIAL (BOM)', 11, '#0f172a', 'middle', '800');
    var bom = bomRows(r, pmat, gsk, Dp);
    s += table(bx0, by0 + 10, [30, 176, 130, 34, 42], [['NO', 'DESCRIPTION', 'MATERIAL', 'QTY', 'REMARK']].concat(bom.map(function (b2, i2) { return [String(i2 + 1), b2[0], b2[1], String(b2[2]), (b2[5] || '').slice(0, 8)]; })), true);

    /* ═════ DESIGN DATA ═════ */
    var dx = 560, dy = 560;
    s += txt(dx + 175, dy + 2, 'DESIGN DATA', 11, '#0f172a', 'middle', '800');
    s += table(dx, dy + 10, [150, 100, 100], [
      ['SERVICE', hName, cName],
      ...
      ['ΔP', r.dpH.dp.toFixed(1) + ' kPa', r.dpC.dp.toFixed(1) + ' kPa'],
    ...
    /* ═════ TITLE BLOCK ═════ */
    var tbx = 1112, tby = svgH - 210, tbw = 412;
    s += rect(tbx, tby, tbw, 130, '#fff', '#0f172a', 1.4);
    ...
    s += line(tbx, tby + 50, tbx + tbw, tby + 50, 0.8);
    var trows = [
    ...
    trows.forEach(function (row, ri) {
      var yy = tby + 50 + ri * 20;
      s += line(tbx, yy + 20, tbx + tbw, yy + 20, 0.6);
      s += line(tbx + 90, yy, tbx + 90, yy + 20, 0.6) + line(tbx + 250, yy, tbx + 250, yy + 20, 0.6) + line(tbx + 330, yy, tbx + 330, yy + 20, 0.6);
      s += txt(tbx + 6, yy + 14, row[0], 7.5, '#64748b', 'start', '700') + txt(tbx + 96, yy + 14, row[1], 8.5, '#0f172a');
      s += txt(tbx + 256, yy + 14, row[2], 7.5, '#64748b', 'start', '700') + txt(tbx + 336, yy + 14, row[3], 8.5, '#0f172a');
    });
    s += txt(tbx + 6, tby + 124, 'REV. ' + (val('phe-rev', '0') || '0') + ' · ' + val('phe-project', 'ANOVIX TECHNOLOGIES'), 7.5, '#64748b', 'start');

    /* ═════ NOTES ═════ */
    var noy = 900;
    s += txt(70, noy, 'NOTES :', 10, '#0f172a', 'start', '800');
    ['1. Number of plates (N), heat-transfer area (A) and L / C dimensions shall be as per duty (see design data).',
     ...
     '3. All welds to be continuous and leak-tight; gaskets clip-on, field-replaceable.',
     '4. Hydrotest at ' + r.Phydro.toFixed(1) + ' barg (1.5 × design) as per design code (ASME Sec VIII / EN 13445).',
     '5. This drawing is the property of ' + (val('phe-project', 'ANOVIX TECHNOLOGIES') || 'ANOVIX TECHNOLOGIES') + ' and must not be copied without permission.'
    ].forEach(function (nt, ni) { s += txt(70, noy + 18 + ni * 15, nt, 8.5, '#334155', 'start'); });

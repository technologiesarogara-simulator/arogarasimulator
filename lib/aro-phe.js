...
(function () {
  ...
  // editable text field with a datalist of standard choices (custom entry allowed)
  function selStd(label, id, opts, cur) {
    var dlId = id + '-dl';
    return '<label style="display:block;margin:4px 0;font-family:var(--font-mono);font-size:10px;color:var(--text-muted);">' + label
      + ' <span style="color:#38bdf8;font-size:8px;">▾ standard / custom</span>'
      + '<input id="' + id + '" list="' + dlId + '" type="text" value="' + (cur || '') + '" '
      + 'style="width:100%;margin-top:2px;background:rgba(2,6,18,0.6);border:1px solid var(--border-muted);color:#e2e8f0;font-family:var(--font-mono);font-size:11px;padding:5px 7px;border-radius:3px;box-sizing:border-box;"/>'
      + '<datalist id="' + dlId + '">' + opts.map(function (o) { return '<option value="' + o + '">'; }).join('') + '</datalist></label>';
  }

...
(function () {
  ...
  function panelHTML() {
    ...
    h += '<style>.phe-act{flex:1;background:transparent;border:1px solid var(--color-saffron);color:var(--color-saffron);font-family:var(--font-mono);font-size:10px;font-weight:700;padding:8px;border-radius:4px;cursor:pointer;}.phe-act:hover{background:rgba(255,117,56,0.12);}'
      + '.phe-rr{display:flex;justify-content:space-between;padding:3px 0;border-bottom:1px dashed var(--border-muted);font-family:var(--font-mono);font-size:11px;break-inside:avoid;}'
      + '.phe-rr span{color:var(--text-muted);}.phe-rr b{color:var(--text-header);}.phe-rr.warn b{color:#ef4444;}.phe-rr.ok b{color:#22c55e;}'
      + '.phe-cardh{font-family:var(--font-mono);font-size:11px;font-weight:800;color:var(--color-saffron);letter-spacing:0.05em;margin:12px 0 4px;break-inside:avoid;break-after:avoid;}'
      /* Same idea as STHE/DPHE's .sthe-results-grid fix: a long single column
         of one-value-per-row output reads as a wall of text. CSS columns lay
         the same rows/headers out two-up without touching how the results
         string is built — every .phe-cardh/.phe-rr already exists, this only
         changes how they flow, and break-inside:avoid above keeps a header
         from separating from the rows under it. */
      + '#phe-results{column-count:2;column-gap:20px;}'
      + '@media (max-width:760px){#phe-results{column-count:1;}}'
    ...
    h += '<div class="panel" style="max-height:calc(100vh - 120px);overflow-y:auto;">'
      ...
      + '<div style="font-family:var(--font-mono);font-size:9.5px;color:#94a3b8;text-align:center;margin-bottom:4px;">3D PLATE-PACK — LIVE VIEW &nbsp;·&nbsp; DRAG TO ROTATE · CTRL/&#8984;+SCROLL OR THE ZOOM BUTTONS TO ZOOM</div>'
  ...
  function init3D() {
    ...
    /* Ctrl/Cmd+scroll to zoom, same opt-in gesture every other 3D canvas in
       ...
       instead of getting captured by whichever small preview canvas the

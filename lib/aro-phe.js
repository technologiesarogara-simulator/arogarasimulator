...
(function () {
  ...
  function calc() {
    ...
    if (r.portBasis) {
      var pb = r.portBasis;
      h += row('Sizing basis', 'One bore shared by all four ports — ' + pb.gov
        + ' side governs at ' + (pb.q * 3600).toFixed(1) + ' m³/h (the larger volumetric stream)');
      h += row('Rule applied', pb.userSet
        ? 'USER-ENTERED bore — the auto-size rule is not applied'
        : 'Start at 30 % of plate width (' + U(pb.startMm, 'length-mm', 0)
          + '), widen in 5 mm steps until the governing port velocity is at or under '
          + U(pb.vLimit, 'velocity', 0),
        pb.userSet ? 'ok' : 'ok');
      h += row('Governing port velocity', U(pb.vGov, 'velocity', 2)
        + ' vs ' + U(pb.vLimit, 'velocity', 0) + ' target',
        pb.vGov > pb.vLimit * 1.05 ? 'warn' : 'ok');
      h += row('Why one bore', 'Both services enter and leave on the fixed cover of a symmetric '
        + 'frame, so the four ports are punched to one size — the lighter stream simply runs slower');
    }

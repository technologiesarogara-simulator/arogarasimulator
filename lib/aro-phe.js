...
(function () {
  ...
  function init3D() {
    ...
    (function loop() { requestAnimationFrame(loop); if (three && three.animFlow) three.animFlow(); rn.render(scene, cam); updateNozzleLabels(); })();
  ...
  function drawing() {
    ...
    modal('PHE — FABRICATION / GA DRAWING + BOM', s + bomHtml, true, true);
  }
  ...
  function report() {
    ...
    b += '<div style="text-align:center;margin-bottom:14px;"><div style="font-size:18px;font-weight:800;color:#ea580c;">AROGARA FLOWSIZE — PLATE HEAT EXCHANGER DESIGN REPORT</div><div style="font-size:10px;color:#64748b;">ANOVIX TECHNOLOGIES | DIGITAL INDIA INITIATIVE</div></div>';
  ...
  window.AROPDF = function (element, filename, opts) {
    ...
    if (typeof html2pdf === 'undefined') { try { window.print(); } catch (e) {} return; }
    ...
    /* A heading is not content — it is a promise about what comes next. Left to
       the atomic rule alone a heading is only a few pixels tall, so a cut could
       fall straight after it and print "PERFORMANCE GRAPHS" at the foot of one
       page with the graph itself on the next. These selectors mark the headings
       that must travel with whatever follows them. */
    var HEADING = 'h1, h2, h3, h4, h5, .pdf-keep-next, [data-pdf-keep-next],'
      + '.rep-h, .rep-title, .panel-title, .aln-cardh, .tk-cardh, .tp-cardh,'
      + '.pid-cardh, .sthe-cardh, .phe-cardh, .rep-section-title, .aro-doc-h';

    /* The next element that actually occupies space — a heading is often
       followed by a wrapper or a stray text node before the real block. */
    function nextBlock(el) {
      var n = el.nextElementSibling;
      while (n) {
        var r = n.getBoundingClientRect();
        if (r.height > 2) return n;
        n = n.nextElementSibling;
      }
      /* Last child of its container: the container's next sibling carries on. */
      var p = el.parentElement;
      if (p && p !== clone) return nextBlock(p);
      return null;
    }
    ...
    function paginate() {
      ...
      setTimeout(function () {
        ...
        var blocks = [];
        [].slice.call(clone.querySelectorAll(ATOMIC)).forEach(function (el) {
          var r = el.getBoundingClientRect();
          if (r.height > 2 && r.height <= pageHpx * 0.5) blocks.push({ t: r.top - padTop, b: r.bottom - padTop, kind: 'atomic' });
        });

        /* Bind every heading to the block it introduces. The pair is protected
           as one span, so a cut landing inside it is pushed back to the top of
           the heading and both travel to the next page together.

           The span is capped at most of a page: a heading over a very long
           table can never be kept whole, and demanding it would push the cut
           so far back that a page came out nearly empty. In that case the
           heading is kept with the first part of the block instead, which is
           all that is needed to stop it being orphaned. */
        var KEEP_MAX = pageHpx * 0.86;
        [].slice.call(clone.querySelectorAll(HEADING)).forEach(function (hd) {
          var hr = hd.getBoundingClientRect();
          if (!(hr.height > 1)) return;
          var nb = nextBlock(hd);
          var top = hr.top - padTop;
          var bottom = hr.bottom - padTop;
          if (nb) {
            var nr = nb.getBoundingClientRect();
            /* Only if the block really does follow the heading on the page. */
            if (nr.top - padTop >= top - 1) bottom = nr.bottom - padTop;
          }
          /* Always demand at least a couple of lines of what follows, so the
             heading cannot sit alone at the foot of a page. */
          bottom = Math.max(bottom, top + hr.height + 46);
          if (bottom - top > KEEP_MAX) bottom = top + KEEP_MAX;
          blocks.push({ t: top, b: bottom, kind: 'keep' });
        });

        html2pdf().set({ html2canvas: { scale: opts.scale || 2, useCORS: true, backgroundColor: bg } })
  ...
  function exportDrawingPdf(body, filename, done) {
    ...
    if (!svg || typeof html2pdf === 'undefined') { var r = window.AROPDF(body, filename, { landscape: true }); if (r && r.then) r.then(done, done); else setTimeout(done, 1500); return; }

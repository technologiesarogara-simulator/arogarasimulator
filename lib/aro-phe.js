...
(function () {
  ...
  function panelHTML() {
    ...
    h += twoCol(dateFld('DATE', 'phe-date'), txt('REV', 'phe-rev', '0'));
  ...
  function wire() {
    ...
    var hn = $('phe-hf-name'); if (hn) hn.addEventListener('change', function () { applyFluid('hf'); calc(); });
    var cn = $('phe-cf-name'); if (cn) cn.addEventListener('change', function () { applyFluid('cf'); calc(); });
    var pm = $('phe-pmat'); if (pm) pm.addEventListener('change', function () { updateMatInfo(); calc(); });
    var cb = $('phe-calc'); if (cb) cb.addEventListener('click', function () {
      applyFluid('hf'); applyFluid('cf'); calc();
      var res = $('phe-results'); if (res) { res.style.transition = 'none'; res.style.boxShadow = '0 0 0 2px #22c55e'; setTimeout(function () { res.style.boxShadow = 'none'; }, 500); res.scrollIntoView({ behavior: 'smooth', block: 'nearest' }); }
    });
    ...
    // seed library properties + material info on first build

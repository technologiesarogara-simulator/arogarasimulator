...
(function () {
  ...
  window.AROPDF = function (element, filename, opts) {
    ...
    // an explicit landscape flag always wins; only auto-detect when unspecified
    var wide = (opts.landscape != null) ? !!opts.landscape : (element.scrollWidth > 980);
    var conf = {
      margin: opts.margin != null ? opts.margin : 6,
      filename: filename,
      image: { type: 'jpeg', quality: 0.96 },
      html2canvas: { scale: opts.scale || 2, useCORS: true, backgroundColor: opts.bg || '#ffffff', windowWidth: element.scrollWidth, scrollX: 0, scrollY: 0 },
      jsPDF: { unit: 'mm', format: 'a4', orientation: wide ? 'landscape' : 'portrait' },
      pagebreak: { mode: ['avoid-all', 'css', 'legacy'] }
    };
    return html2pdf().set(conf).from(element).output('blob').then(function (blob) {
      var url = URL.createObjectURL(blob);
      var a = document.createElement('a'); a.href = url; a.download = filename; document.body.appendChild(a); a.click();
      setTimeout(function () { a.remove(); URL.revokeObjectURL(url); }, 1500);
    }).catch(function (e) { try { console.error('PDF export failed', e); window.print(); } catch (_) {} });
  };
  ...
  // export a drawing modal (big SVG GA sheet, which already contains the BOM)
  // as a fitted landscape PDF — rasterise the SVG then addImage it directly to a
  // jsPDF page (html2canvas is unreliable on a huge SVG / injected image).
  function exportDrawingPdf(body, filename, done) {
    ...
    svgToImg(svg, 2, function (png) {
      ...
      html2pdf().set({ jsPDF: { unit: 'mm', format: 'a4', orientation: 'landscape' }, html2canvas: { scale: 1 } }).from(dummy).toPdf().get('pdf').then(function (pdf) {
        try {
          var pw = pdf.internal.pageSize.getWidth(), ph = pdf.internal.pageSize.getHeight(), ar = sw / sh;
          var iw = pw - 8, ih = iw / ar; if (ih > ph - 8) { ih = ph - 8; iw = ih * ar; }

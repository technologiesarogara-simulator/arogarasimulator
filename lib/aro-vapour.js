...
(function () {
  ...
  /* ── Saturation temperature: the inverse of pBarA ──────────────────────
     A saturated suction is defined by its PRESSURE, not its temperature —
     water at 5 bar(a) is at 152 °C, not at whatever the operating-temperature
     box happens to say — so the panel needs to go the other way round.

     Vapour pressure rises monotonically with temperature for every fluid in
     the table, whichever branch computes it (the water correlation, an
     Antoine fit, or an aqueous solution's activity factor), so a bisection
     inverts all three without needing a separate closed form for each and
     without inheriting a different error behaviour per branch. It is
     bracketed first, so a pressure outside the fluid's achievable range
     returns NaN instead of a confidently wrong root. */
  function tSat(fluid, pTargetBarA) {
    if (!has(fluid) || !isFinite(pTargetBarA) || pTargetBarA <= 0) return NaN;
    var lo = -120, hi = 600;
    var pLo = pBarA(fluid, lo), pHi = pBarA(fluid, hi);
    // walk the ends in until the correlation returns a number
    var guard = 0;
    while ((!isFinite(pLo) || pLo <= 0) && lo < hi && guard++ < 200) { lo += 5; pLo = pBarA(fluid, lo); }
    guard = 0;
    while (!isFinite(pHi) && hi > lo && guard++ < 200) { hi -= 5; pHi = pBarA(fluid, hi); }
    if (!isFinite(pLo) || !isFinite(pHi)) return NaN;
    if (pTargetBarA < pLo || pTargetBarA > pHi) return NaN;   // not reachable
    for (var i = 0; i < 200; i++) {
      var mid = (lo + hi) / 2;
      var pm = pBarA(fluid, mid);
      if (!isFinite(pm)) { hi = mid; continue; }
      if (pm < pTargetBarA) lo = mid; else hi = mid;
  ...
  window.AROVP = {
    pBarA: pBarA, tSat: tSat, has: has, basis: basis, label: label, note: note,

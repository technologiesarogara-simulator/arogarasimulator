...
(function () {
  ...
  function buildPipe(fromPort, toPort, color, nps) {
    ...
    var r = npsRadius(nps), y = (pa.y + pb.y) / 2;
    ...
    grp.userData.pipe = { from: fromPort, to: toPort, fromNid: fg.userData.nid, toNid: tg.userData.nid, mat: mat, emat: emat, color: col, nps: nps || 3 };
  ...
  A3.buildFromModel = function (nodes, pipes, nameOf) {
    ...
    (pipes || []).forEach(function (p) {
      ...
      if (op && ip) { var g = buildPipe(op, ip, p.color, p.nps); if (g) g.userData.pipe.pid = p.id; }
    });
    rebuilding = false;
    if (objects.length) {

...
(function () {
  ...
  function updateOperatorFrame(dt) {
    ...
    if (manual) {
      ...
      o.path = null;
      o.arriveEquip = null; o.showingEquipInfo = false;
      clearRouteLine();
      ...
      if (dist < (isFinalLeg ? 0.15 : 0.45)) {
        ...
        if (!o.path.length) {
          ...
          if (o.arriveEquip) {
            var af = o.arriveFace;
            o.yaw = Math.atan2(af.x - o.pos.x, af.z - o.pos.z);
            select(o.arriveEquip);
            if (embStatus) embStatus('Arrived at ' + (o.arriveEquip.userData.props ? o.arriveEquip.userData.props.Type : 'equipment') + '.');
            o.arriveEquip = null; o.showingEquipInfo = true;
          }
        } else {
  ...
  A3.setOperatorMode = function (on) {
    ...
    if (on) {
      if (!operatorObj) {
        ...
        scene.add(fig);
        operatorObj = { figure: fig, pos: new THREE.Vector3(), yaw: Math.PI, pitch: 0.15, path: null, keys: {}, arriveEquip: null, arriveFace: null, showingEquipInfo: false };
      }
      refreshOperatorNav();
  ...
  A3.resetOperator = function () { if (!operatorObj) return; operatorObj.pos.copy(operatorSpawn); operatorObj.path = null; operatorObj.arriveEquip = null; operatorObj.showingEquipInfo = false; clearRouteLine(); pushOperatorPanel(); };
      ...
      if (operatorMode) {
        ...
        if (ray.ray.intersectPlane(GROUND_PLANE, groundPt)) {
          ...
          if (!clickPath) { if (embStatus) embStatus('NO VALID OPERATOR ACCESS ROUTE — no walkable path to that point.'); }
            ...
            operatorObj.path = clickPath; operatorObj.arriveEquip = null; operatorObj.showingEquipInfo = false;
            updateRouteLine();
          }
        }
        return;

...
(function () {
  ...
  function updateOperatorFrame(dt) {
    ...
    if (manual) {
      ...
      var dx = o.target.x - o.pos.x, dz = o.target.z - o.pos.z, dist = Math.hypot(dx, dz);
      if (dist < 0.15) {
        o.target = null;
        if (o.arriveEquip) {
          var af = o.arriveFace;
        ...
        if (!moved || (o.pos.x === before && o.pos.z === beforeZ)) {
          o.target = null;
          if (embStatus) embStatus(o.arriveEquip ? ('NO VALID OPERATOR ACCESS ROUTE — can\'t reach ' + (o.arriveEquip.userData.props ? o.arriveEquip.userData.props.Type : 'that equipment') + ' from here.') : 'NO VALID OPERATOR ACCESS ROUTE — obstacle blocks the direct path.');
          o.arriveEquip = null;
        }
      }
    }
  ...
  A3.setOperatorMode = function (on) {
    ...
    if (on) {
      if (!operatorObj) {
        ...
        scene.add(fig);
        operatorObj = { figure: fig, pos: new THREE.Vector3(), yaw: Math.PI, pitch: 0.15, target: null, keys: {}, arriveEquip: null, arriveFace: null, showingEquipInfo: false };
      }
      buildOperatorColliders();
      operatorObj.pos.copy(findOperatorSpawn());
      operatorSpawn.copy(operatorObj.pos);
      operatorObj.target = null; operatorObj.keys = {}; operatorObj.arriveEquip = null; operatorObj.showingEquipInfo = false;
  ...
  A3.resetOperator = function () { if (!operatorObj) return; operatorObj.pos.copy(operatorSpawn); operatorObj.target = null; operatorObj.arriveEquip = null; operatorObj.showingEquipInfo = false; pushOperatorPanel(); };
  A3.debugOperator = function () { return operatorObj ? { x: operatorObj.pos.x, z: operatorObj.pos.z, yaw: operatorObj.yaw, target: operatorObj.target, circles: operatorColliders.circles.length, segs: operatorColliders.segs.length } : null; };
  /* ── PHASE 3: "walk to equipment" ─────────────────────────────────────
     ...
     computes ONE safe approach point per equipment on demand — just
     outside its own footprint circle (the same footprint data Phase 1's
     foundations and Phase 2's colliders already use), tried first on the
     side closest to the operator's current position, then stepped around
     the equipment until an unblocked point is found. A full per-nozzle
     InspectionPoint list (valve/instrument/manway/motor granularity) is
     a natural follow-on once this basic approach-and-face flow is
     confirmed to be what's wanted. */
  A3.walkToEquipment = function (nid) {
    ...
    var dirX = operatorObj.pos.x - center.x, dirZ = operatorObj.pos.z - center.z;
    var dlen = Math.hypot(dirX, dirZ);
    var baseAngle = dlen > 1e-3 ? Math.atan2(dirZ, dirX) : 0;
    var dest = null;
    for (var i = 0; i < 8; i++) {
      var ang = baseAngle + i * (Math.PI / 4);
      var cx = center.x + Math.cos(ang) * (footR + standoff), cz = center.z + Math.sin(ang) * (footR + standoff);
      if (!operatorBlocked(cx, cz)) { dest = { x: cx, z: cz }; break; }
    }
    ...
    operatorObj.target = dest;
    operatorObj.arriveFace = center.clone();
    ...
    canvas.addEventListener('click', function (e) {
      ...
      if (operatorMode) {
        ...
        if (ray.ray.intersectPlane(GROUND_PLANE, groundPt)) {
          if (operatorBlocked(groundPt.x, groundPt.z)) { if (embStatus) embStatus('NO VALID OPERATOR ACCESS ROUTE — that point is inside equipment.'); }
          else { operatorObj.target = { x: groundPt.x, z: groundPt.z }; operatorObj.arriveEquip = null; operatorObj.showingEquipInfo = false; }
        }
        return;
      }
  ...
  function close() { if (raf) cancelAnimationFrame(raf); clearSelBox(); if (host) host.remove(); host = null; renderer && renderer.dispose && renderer.dispose(); renderer = null; objects = []; contactShadows = []; jacketMeshes = []; gapMarkers = []; spinList = []; picked = null; flowTubes = []; vizHalos = []; flowVizMode = 'off'; foundations = []; }

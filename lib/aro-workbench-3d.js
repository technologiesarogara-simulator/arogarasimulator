...
(function () {
  ...
  var flowVizMode = 'off', flowTubes = [], FLOWTEX3D = null, dimmedEquip = [];
  ...
  /* FOG_DENSITY was a fixed constant tuned for close-up single-equipment
     framing (camera distance ~6-80 world units). A whole connected plant
     (dozens of units wide) auto-frames at a much larger camera distance —
     at this fixed density, FogExp2's falloff is total well before the
     camera pulls back far enough to see the whole thing, so the "plant
     view" the workbench should be able to show was, in practice, a blank
     fogged-out screen. Fog density is now recomputed every camera move
     (updateCamera(), below) as FOG_K / cameraDistance, clamped to a
     sensible range: this reproduces the exact same look as the old fixed
     0.012 at the ~40-unit distance a single piece of equipment typically
     frames at (FOG_K = 0.012*40), while scaling down proportionally for a
     wide plant so the far side of it stays visible instead of vanishing
     into the fog before the camera has even pulled back far enough to
     frame it. */
  var FOG_K = 0.48;
  var FOG_DENSITY_MIN = 0.00025, FOG_DENSITY_MAX = 0.012;
  ...
  function M() { return {
    ...
    // Bare cast iron — pump/valve/compressor BODIES cast in one piece
    // (as opposed to mats.dark/mats.navy, which stay for sheet-steel
    // covers, guards and painted motor frames). A cast body reads
    // completely differently from painted sheet steel: duller, greyer,
    // a rougher as-cast surface rather than a smooth paint film.
    castIron:new THREE.MeshStandardMaterial({ color: 0x4b4f52, metalness: 0.55, roughness: 0.55, envMapIntensity: ENV_MAP_INTENSITY }),
    // Safety-yellow paint — coupling guards, machine guarding, moving-part
    // warnings. Matte, non-metallic; this is paint, not powder-coated steel.
    safetyYellow:new THREE.MeshStandardMaterial({ color: 0xf4c430, metalness: 0.05, roughness: 0.55, envMapIntensity: ENV_MAP_INTENSITY })
  }; }
  var mats;
  ...
  var FAC = {
    'centrifugal-pump': function () {
      /* Rebuilt against a real ANSI end-suction pump skid photo (cast-iron
         volute, a motor at least as large as the casing with a real fan
         shroud and conduit box, a safety-yellow coupling guard, a chrome
         shaft stub) — the old version read as two same-tone blue barrels
         with a painted-on bolt for a "guard", nothing like the reference. */
      var g = new THREE.Group();
      /* Casing swapped from cast-iron (flat, dull grey) to polished
         chrome/stainless — the reference product-shot look this was tuned
         against (a bright, reflective pump casing against a saturated blue
         motor, not two shades of the same grey) reads as "real" precisely
         because the two masses contrast, and a mirror-bright casing shows
         off the volute's real shape (the cutwater lobe, the shaft line)
         instead of absorbing it into flat shading. */
      var vol = cyl(1.5, 1.5, 1.1, mats.silver); vol.rotation.x = Math.PI / 2; vol.position.set(-2.3, 1.4, 0); g.add(vol);
      var volFace = cyl(1.55, 1.55, 0.14, mats.gunmetal); volFace.rotation.x = Math.PI / 2; volFace.position.set(-2.3, 1.4, 0.6); g.add(volFace);
      // volute cutwater lobe — the off-centre bulge a real spiral casing has,
      // not a perfect drum
      var lobe = cyl(1.5, 1.5, 1.1, mats.silver); lobe.rotation.x = Math.PI / 2; lobe.scale.set(1.18, 1, 0.55); lobe.position.set(-2.3, 1.15, -0.6); g.add(lobe);
      // impeller (spins) — visible only through the suction bore
      var imp = new THREE.Group();
      for (var i = 0; i < 6; i++) { var bld = box(1.1, 0.14, 0.34, mats.orange); bld.rotation.y = i / 6 * Math.PI * 2; imp.add(bld); }
      imp.position.set(-2.3, 1.4, 0); imp.rotation.x = Math.PI / 2; imp.userData.spin = 'z'; g.add(imp); spinTag(imp);
      // suction nozzle (axial, +Z) and discharge (up) — same polished
      // stainless as the casing, raised-face gasket ring at the flange
      var suc = cyl(0.55, 0.55, 1.0, mats.silver); suc.rotation.x = Math.PI / 2; suc.position.set(-2.3, 1.4, 1.2); g.add(suc);
      g.add(flgZ(-2.3, 1.4, 1.7, 0.75));
      var dis = cyl(0.5, 0.5, 1.3, mats.silver); dis.position.set(-2.3, 2.6, 0); g.add(dis);
      g.add(nozzleY(3.2, 0.0, 0.5, mats.silver, -2.3));
      // coupling guard — the safety-yellow perforated shroud a real skid
      // always has over the exposed shaft/coupling between pump and motor
      var guard = box(0.95, 1.15, 1.35, mats.safetyYellow); guard.position.set(-0.85, 1.4, 0); g.add(guard);
      var guardStripe = box(0.97, 0.16, 1.37, mats.dark); guardStripe.position.set(-0.85, 1.75, 0); g.add(guardStripe);
      // shaft stub visible at each open end of the guard
      var shaft1 = cyl(0.09, 0.09, 0.3, mats.silver, 12); shaft1.rotation.z = Math.PI / 2; shaft1.position.set(-1.5, 1.4, 0); g.add(shaft1);
      var shaft2 = cyl(0.09, 0.09, 0.3, mats.silver, 12); shaft2.rotation.z = Math.PI / 2; shaft2.position.set(-0.2, 1.4, 0); g.add(shaft2);
      // motor — the dominant mass on a real skid, not a slim afterthought:
      // bigger diameter, a tapered fan shroud cap, ribbed fin rings and a
      // conduit box. Brightened navy → the same saturated motor blue as
      // mats.blue (0x2563eb) — against the now-chrome casing this is the
      // two-tone "blue motor, bright metal pump" read the reference calls
      // for; navy next to bare grey cast-iron just read as two greys.
      var mot = cyl(1.15, 1.15, 3.0, mats.blue); mot.rotation.z = Math.PI / 2; mot.position.set(1.55, 1.4, 0); g.add(mot);
      var fanShroud = cyl(1.2, 0.85, 0.55, mats.dark, 24); fanShroud.rotation.z = Math.PI / 2; fanShroud.position.set(3.3, 1.4, 0); g.add(fanShroud);
      /* Fins are a cast ridge on the motor's OWN frame, not a separate part
         — a real TEFC motor is one colour, fins included; painting them
         mats.dark (13 rings, densely spaced enough to cover ~85% of the
         motor's length) made the whole finned two-thirds of the motor read
         as a flat dark mass with the blue barely showing between rings.
         Matching mats.blue here means the ribbing still reads as a shape
         (each ring's own highlight/shadow), but the motor is unmistakably
         blue along its whole length, the way the reference actually shows it. */
      for (var f = 0; f < 13; f++) { var fin = torus(1.18, 0.035, mats.blue); fin.rotation.y = Math.PI / 2; fin.position.set(0.35 + f * 0.21, 1.4, 0); g.add(fin); }
      var conduit = box(0.5, 0.35, 0.4, mats.dark); conduit.position.set(1.2, 2.6, 0); g.add(conduit);
      var feet = box(0.15, 0.35, 2.5, mats.dark); feet.position.set(1.55, 0.3, 0); g.add(feet);
      // baseplate skid — wider and slightly proud of the floor
      var bp = box(6.0, 0.3, 2.6, mats.dark); bp.position.set(0.4, 0.15, 0); g.add(bp);
      ...
      g.userData.portDefs = [
        { id: 'suction', role: 'in', name: 'Suction', local: new THREE.Vector3(-2.3, 1.4, 1.7), dir: new THREE.Vector3(0, 0, 1) },
        { id: 'discharge', role: 'out', name: 'Discharge', local: new THREE.Vector3(-2.3, 3.2, 0), dir: new THREE.Vector3(0, 1, 0) }
      ];
      return g;
    },
    'sthe': function () {
    ...
    'gate-valve': function () {
      /* Rebuilt against a real bronze gate-valve photo: cast-bronze body
         (not a blue plastic-looking sphere), a real bonnet/yoke stack, a
         chrome-plated rising stem, and a solid 5-spoke iron handwheel with
         a rim disc, not three bare sticks. */
      var g = new THREE.Group();
      var body = mesh(new THREE.SphereGeometry(0.9, 20, 16), mats.brass); body.scale.y = 1.1; body.position.y = 1.4; g.add(body);
      g.add(flgZ(0, 1.4, 1.0, 0.55)); g.add(flgZ(0, 1.4, -1.0, 0.55));
      var e1 = cyl(0.4, 0.4, 0.8, mats.brass); e1.rotation.x = Math.PI / 2; e1.position.set(0, 1.4, 0.6); g.add(e1);
      var e2 = e1.clone(); e2.position.z = -0.6; g.add(e2);
      // bonnet / yoke — cast iron, tapered toward the stem the way a real
      // gate valve's bonnet narrows, not a plain box
      var bonnet = cyl(0.42, 0.55, 0.9, mats.dark, 24); bonnet.position.y = 2.3; g.add(bonnet);
      g.add(boltCircle(6, 0.5, 2.0, 'y', mats.bolt));
      var yoke = cyl(0.3, 0.42, 0.5, mats.dark, 20); yoke.position.y = 2.95; g.add(yoke);
      var stemNut = cyl(0.32, 0.32, 0.22, mats.dark, 16); stemNut.position.y = 3.28; g.add(stemNut);
      var stem = cyl(0.075, 0.075, 1.5, mats.silver, 16); stem.position.y = 3.95; g.add(stem);
      // handwheel — rim disc + 5 real spokes + a domed centre hub, cast iron
      var wheel = torus(0.65, 0.1, mats.dark); wheel.rotation.x = Math.PI / 2; wheel.position.y = 4.65; g.add(wheel);
      for (var s = 0; s < 5; s++) { var sp = box(1.2, 0.07, 0.07, mats.dark); sp.rotation.y = s / 5 * Math.PI; sp.position.y = 4.65; g.add(sp); }
      var hub = cyl(0.13, 0.13, 0.26, mats.dark, 16); hub.rotation.x = Math.PI / 2; hub.position.y = 4.65; g.add(hub);
      g.userData.props = { Type: 'Gate Valve', 'Size (NPS)': 3, 'Rating': 'CL150', 'End': 'RF Flanged', 'Body': 'Bronze/WCB', 'Trim': 'SS316' };
      g.userData.portDefs = [
    ...
    'kettle-reboiler': function () {                       // kettle reboiler — oversized shell bulge houses the disengagement space above the bundle
      var g = new THREE.Group();
      var R = 1.6, L = 5.5;
      // Shell/kettle bulge are the fabricated CS pressure boundary (props:
      // "Material: CS/SS") — mats.copper here made the whole vessel read as
      // solid copper, which no real reboiler shell is; copper stays for the
      // tube bundle below, where it belongs.
      var sh = cyl(R, R, L, mats.steel); sh.rotation.z = Math.PI / 2; sh.position.y = 2.4; g.add(sh);
      g.add(dishSidePos(R, L / 2, 2.4, mats.steel));
      var kettle = cyl(R * 1.4, R, 2, mats.steel); kettle.rotation.z = Math.PI / 2; kettle.position.set(-L / 2 + 1, 2.4, 0); g.add(kettle);
    ...
    'split-case-pump': function () {                        // horizontal double-suction pump — inline flanges, visible split line
      var g = new THREE.Group();
      var casing = cyl(0.9, 0.9, 1.6, mats.castIron, 24); casing.rotation.z = Math.PI / 2; casing.position.y = 1.3; g.add(casing);
      var splitLine = box(1.62, 0.03, 1.85, mats.dark); splitLine.position.y = 1.3; g.add(splitLine);
    ...
    'aodd-pump': function () {                              // air-operated double-diaphragm pump — twin chambers + centre air block + top muffler
      ...
      [-dx, dx].forEach(function (sx) {
        var can = cyl(0.4, 0.4, 0.85, mats.silver, 20); can.rotation.z = Math.PI / 2; can.position.set(sx, y, 0); g.add(can);
        var cap = mesh(new THREE.SphereGeometry(0.4, 16, 12, 0, Math.PI * 2, 0, Math.PI / 2), mats.silver);
    ...
    'peristaltic-pump': function () {                       // rotor-and-shoe hose pump — round housing, hose loop visible through a window
      ...
      g.userData.props = { Type: 'Peristaltic Pump', 'Flow (m³/h)': 2, 'Diff P (bar)': 4, Tube: 'NR/NBR', Material: 'CI housing' };
      g.userData.portDefs = [
    ...
    'diaphragm-valve': function () {                        // weir body + large diaphragm bonnet dome
      ...
      var wheel = torus(0.28, 0.045, mats.dark); wheel.rotation.x = Math.PI / 2; wheel.position.y = 1.05; g.add(wheel);
    ...
    'angle-valve': function () {                            // 90°-offset ports — inlet horizontal, outlet vertical
      ...
      var outNoz = cyl(0.2, 0.2, 0.3, mats.silver); outNoz.position.y = 0.35; g.add(outNoz);
  ...
  var GASKET_MAT = null;
  /* Stage 4B, revised: this used to be a hairline-thin (r*0.05) ring on
     the theory that the equipment's own flange face already reaches "at
     or near" pos, so anything thicker would be "a spacer invented to
     paper over a gap." In practice, a great many hand-authored factories'
     drawn flange face doesn't land EXACTLY on the declared port position
     — close, but off by enough that a hairline-thin gasket left a visible
     sliver of open air between the pipe and the equipment body at normal
     zoom, which is exactly the "not well connected" defect reported.
     Widened into a real coupling collar (r*0.45, still capped well short
     of the flange hub/face beyond it) that always physically bridges that
     small mismatch — prioritising "this always visibly touches" over
     "this is the geometrically minimal ring," which is what was actually
     asked for. Applies to EVERY connection (this is called unconditionally
     for both port ends, whether or not that end also gets a full
     flangeAt()), so it's the one change that closes the gap across the
     whole equipment library at once rather than needing every factory's
     own flange geometry re-measured by hand. */
  function gasketAt(pos, dir, r) {
    ...
    var d = dir.clone().normalize();
    var g = mesh(new THREE.CylinderGeometry(r * 1.15, r * 1.15, r * 0.45, 22), GASKET_MAT);
    g.position.copy(pos);
  ...
  /* Field operator figure — a real plant reads as inhabited, with people
     actually monitoring the big equipment, not just pipe and steel on an
     empty grid; it also gives every big vessel a genuine human-scale
     reference the way a real site photo always has one. Deliberately
     low-poly/schematic (cylinders, spheres and boxes, no face or individual
     features) — a scale-and-presence marker, not a rendered portrait of an
     actual person. The PPE it wears is real and standard for a plant
     operator, not decoration: full blue coverall (not bare limbs) with
     reflective bands at the chest and ankles, dark safety boots, and a
     white hard hat.
     The radio has TWO poses, not one fixed "on a call" arm: at rest it's
     clipped to the chest strap, both arms hanging naturally — a walkie-
     talkie is not held to the ear all day, only while actually talking on
     it. g.userData.radioRig exposes the two variants (restRadio / onCallArm)
     so A3.setOperatorOnCall can swap which is visible; see that function
     for when the swap actually happens. */
  var PERSON_MAT = null, PERSON_COVERALL_MAT = null, PERSON_STRIPE_MAT = null, PERSON_BOOT_MAT = null, PERSON_HAT_MAT = null, PERSON_RADIO_MAT = null, PERSON_EYE_MAT = null, PERSON_SKIN_MAT = null;
  /* scale: visual size multiplier on top of the true 1.73m figure. Left at
     1 the person is anatomically correct but reads as a barely-visible
     speck beside a 30m column — legible next to a small pump, lost next
     to a fractionator. personScaleFor() below derives this from the
     equipment's own height/diameter so bigger units get a proportionally
     bigger (still human-shaped, still standing on the ground) marker. */
  function personFigure(scale) {
    if (!PERSON_MAT) PERSON_MAT = new THREE.MeshStandardMaterial({ color: 0x2b2f38, metalness: 0.05, roughness: 0.85, envMapIntensity: ENV_MAP_INTENSITY });
    if (!PERSON_COVERALL_MAT) PERSON_COVERALL_MAT = new THREE.MeshStandardMaterial({ color: 0x2c5591, metalness: 0.05, roughness: 0.78, envMapIntensity: ENV_MAP_INTENSITY });
    if (!PERSON_STRIPE_MAT) PERSON_STRIPE_MAT = new THREE.MeshStandardMaterial({ color: 0xe8ecf1, metalness: 0.35, roughness: 0.3, envMapIntensity: ENV_MAP_INTENSITY });
    if (!PERSON_BOOT_MAT) PERSON_BOOT_MAT = new THREE.MeshStandardMaterial({ color: 0x1a1c22, metalness: 0.1, roughness: 0.75, envMapIntensity: ENV_MAP_INTENSITY });
    if (!PERSON_HAT_MAT) PERSON_HAT_MAT = new THREE.MeshStandardMaterial({ color: 0xf8fafc, metalness: 0.1, roughness: 0.5, envMapIntensity: ENV_MAP_INTENSITY });
    if (!PERSON_RADIO_MAT) PERSON_RADIO_MAT = new THREE.MeshStandardMaterial({ color: 0x14161b, metalness: 0.2, roughness: 0.55, envMapIntensity: ENV_MAP_INTENSITY });
    var g = new THREE.Group();
    // A cylinder spanning two arbitrary points — the only way to build the
    // bent "on a call" arm below without a real skeleton/IK system.
    function seg(a, b, r, mat) {
      var dx = b.x - a.x, dy = b.y - a.y, dz = b.z - a.z, len = Math.hypot(dx, dy, dz);
      var m = cyl(r, r, Math.max(0.02, len), mat, 8);
      m.position.set((a.x + b.x) / 2, (a.y + b.y) / 2, (a.z + b.z) / 2);
      m.quaternion.setFromUnitVectors(new THREE.Vector3(0, 1, 0), new THREE.Vector3(dx, dy, dz).normalize());
      return m;
    }
    // Legs — full coverall, not bare skin, each with a reflective ankle
    // band and a dark safety boot planted slightly forward of the shin.
    var legL = mesh(new THREE.CylinderGeometry(0.058, 0.062, 0.78, 8), PERSON_COVERALL_MAT); legL.position.set(-0.09, 0.46, 0); g.add(legL);
    var legR = legL.clone(); legR.position.x = 0.09; g.add(legR);
    [-0.09, 0.09].forEach(function (lx) {
      var band = mesh(new THREE.CylinderGeometry(0.066, 0.066, 0.05, 8), PERSON_STRIPE_MAT); band.position.set(lx, 0.56, 0); g.add(band);
      var boot = box(0.1, 0.1, 0.2, PERSON_BOOT_MAT); boot.position.set(lx, 0.05, 0.035); g.add(boot);
    });
    // Torso — coverall body with a chest-height reflective band
    var torso = mesh(new THREE.CylinderGeometry(0.16, 0.15, 0.58, 10), PERSON_COVERALL_MAT); torso.position.set(0, 1.15, 0); g.add(torso);
    var chestBand = mesh(new THREE.CylinderGeometry(0.168, 0.168, 0.06, 10), PERSON_STRIPE_MAT); chestBand.position.set(0, 1.28, 0); g.add(chestBand);
    // Both arms hang naturally at rest, same coverall sleeve
    var armL = mesh(new THREE.CylinderGeometry(0.045, 0.045, 0.5, 8), PERSON_COVERALL_MAT); armL.position.set(-0.19, 1.1, 0); g.add(armL);
    var armR = armL.clone(); armR.position.x = 0.19; g.add(armR);
    // Radio at rest: clipped to the chest strap, like a real epaulette mic
    // — visible whenever nobody's on a call.
    var restRadio = new THREE.Group();
    var restRadioBody = box(0.05, 0.09, 0.035, PERSON_RADIO_MAT); restRadioBody.position.set(0.1, 1.32, 0.145); restRadio.add(restRadioBody);
    var restAntenna = cyl(0.005, 0.005, 0.07, PERSON_RADIO_MAT, 6); restAntenna.position.set(0.1, 1.395, 0.14); restRadio.add(restAntenna);
    g.add(restRadio);
    // On a call: right arm bends at the elbow to bring the SAME radio up
    // to the head — built once, hidden until A3.setOperatorOnCall(true)
    // swaps the two groups' visibility (see that function).
    var onCallArm = new THREE.Group();
    var shoulder = { x: 0.19, y: 1.34, z: 0 }, elbow = { x: 0.24, y: 1.08, z: 0.03 }, hand = { x: 0.13, y: 1.48, z: 0.12 };
    onCallArm.add(seg(shoulder, elbow, 0.045, PERSON_COVERALL_MAT));
    onCallArm.add(seg(elbow, hand, 0.04, PERSON_COVERALL_MAT));
    var handRadio = box(0.055, 0.13, 0.04, PERSON_RADIO_MAT); handRadio.position.set(hand.x + 0.01, hand.y + 0.02, hand.z + 0.02); handRadio.rotation.x = -0.3; onCallArm.add(handRadio);
    var handAntenna = cyl(0.006, 0.006, 0.1, PERSON_RADIO_MAT, 6); handAntenna.position.set(hand.x + 0.01, hand.y + 0.115, hand.z - 0.01); onCallArm.add(handAntenna);
    onCallArm.visible = false;
    g.add(onCallArm);
    var head = mesh(new THREE.SphereGeometry(0.11, 12, 10), PERSON_MAT); head.position.set(0, 1.53, 0); g.add(head);
    // A face is what makes a low-poly figure read as "someone", not just a
    // pipe fitting — two eyes and a brow line, cheap geometry, big legibility
    // gain at the scale this figure is actually seen on screen.
    if (!PERSON_EYE_MAT) PERSON_EYE_MAT = new THREE.MeshStandardMaterial({ color: 0x0b1220, metalness: 0, roughness: 0.5 });
    if (!PERSON_SKIN_MAT) PERSON_SKIN_MAT = new THREE.MeshStandardMaterial({ color: 0xc99a72, metalness: 0, roughness: 0.9, envMapIntensity: ENV_MAP_INTENSITY });
    var face = mesh(new THREE.SphereGeometry(0.098, 12, 10, 0, Math.PI * 2, 0, Math.PI * 0.6), PERSON_SKIN_MAT);
    face.position.set(0, 1.525, 0.005); g.add(face);
    [-0.035, 0.035].forEach(function (ex) {
      var eye = mesh(new THREE.SphereGeometry(0.012, 8, 8), PERSON_EYE_MAT); eye.position.set(ex, 1.545, 0.098); g.add(eye);
    });
    var hat = mesh(new THREE.SphereGeometry(0.115, 12, 8, 0, Math.PI * 2, 0, Math.PI / 2), PERSON_HAT_MAT); hat.position.set(0, 1.59, 0); g.add(hat);
    var brim = mesh(new THREE.CylinderGeometry(0.13, 0.13, 0.02, 16), PERSON_HAT_MAT); brim.position.set(0, 1.595, 0); g.add(brim);
    if (scale && scale !== 1) g.scale.setScalar(scale);
    g.userData.noPorts = true;
    g.userData.radioRig = { restRadio: restRadio, restArm: armR, onCallArm: onCallArm, onCall: false };
    g.userData.props = { Type: 'Field Operator', Note: 'Scale reference — not a process equipment item', Scale: scale ? scale.toFixed(2) + 'x' : '1.00x (1.73m)' };
    return g;
  }
  function personScaleFor(sz) {
    var span = Math.max(sz.y, sz.x, sz.z);
    return Math.max(1, Math.min(2.6, 1 + (span - 6) / 10));
  }

  /* ============================================================
     ADDITIVE PLANT-OPERABILITY MODULE — PHASE 1: EQUIPMENT FOUNDATIONS
     Every big/medium equipment item gets a real concrete foundation
     shaped for its own install type — this does NOT touch the existing
     per-item skirt/saddle/pedestal geometry already built into each FAC
     factory; it adds the civil foundation *underneath* that. Dimensions
     are derived from each item's own footprint plus a configurable
     margin, never a single hard-coded size for every item. Margins here
     are engineering placeholders (labelled PROJECT-DEFINED in the
     item's info panel) — not a substitute for real geotechnical/civil
     design. Foundations are cosmetic/informational only in this phase:
     later phases (maintenance envelopes, walkways, operator mode) will
     read equipment footprints the same way but are not implemented yet.
     ============================================================ */
  var CONCRETE_MAT = null, CONCRETE_MAT_EDGE = null, ANCHOR_BOLT_MAT = null;
  var FOUNDATION_MARGIN = 0.35;   // metres of concrete beyond the equipment footprint, per side
  var FOUNDATION_THK = 0.4;       // pad/ring thickness, metres
  var foundations = [], foundationsVisible = true;
  function foundationMatsInit() {
    if (!CONCRETE_MAT) CONCRETE_MAT = new THREE.MeshStandardMaterial({ color: 0xc6c6bd, metalness: 0.02, roughness: 0.95, envMapIntensity: ENV_MAP_INTENSITY });
    if (!CONCRETE_MAT_EDGE) CONCRETE_MAT_EDGE = new THREE.MeshStandardMaterial({ color: 0xafafa4, metalness: 0.02, roughness: 0.95, envMapIntensity: ENV_MAP_INTENSITY });
    if (!ANCHOR_BOLT_MAT) ANCHOR_BOLT_MAT = new THREE.MeshStandardMaterial({ color: 0x556070, metalness: 0.8, roughness: 0.4, envMapIntensity: ENV_MAP_INTENSITY });
  }
  function anchorBoltRing(cx, cz, r, n) {
    var g = new THREE.Group();
    for (var i = 0; i < n; i++) {
      var a = (i / n) * Math.PI * 2;
      var b = cyl(0.02, 0.02, 0.22, ANCHOR_BOLT_MAT, 6);
      b.position.set(cx + Math.cos(a) * r, 0.11, cz + Math.sin(a) * r);
      g.add(b);
    }
    return g;
  }
  var FOUND_TANK_RE = /\btank\b|sphere/i;
  var FOUND_VERTICAL_RE = /column|tower|reactor|vessel|regenerator|fractionator|absorber|stripper|drum|separator|scrubber|silo/i;
  var FOUND_HORIZONTAL_RE = /exchanger|cooler|condenser|\bhx\b|kettle/i;
  var FOUND_ROTATING_RE = /pump|compressor|blower|\bfan\b|turbine/i;
  var FOUND_STRUCTURE_RE = /pipe\s*rack|structure|platform/i;
  function classifyFoundation(type) {
    if (!type) return null;
    if (FOUND_TANK_RE.test(type)) return 'RING_WALL';
    if (FOUND_VERTICAL_RE.test(type)) return 'CIRCULAR_PAD';
    if (FOUND_HORIZONTAL_RE.test(type)) return 'SADDLE_FOUNDATION';
    if (FOUND_ROTATING_RE.test(type)) return 'RECTANGULAR_PAD';
    if (FOUND_STRUCTURE_RE.test(type)) return 'STRUCTURAL_FOOTING';
    return null;
  }
  function buildFoundationFor(g) {
    var type = g.userData.props && g.userData.props.Type;
    var kind = classifyFoundation(type);
    if (!kind) return null;
    foundationMatsInit();
    var bb = new THREE.Box3().setFromObject(g);
    var sz = bb.getSize(new THREE.Vector3());
    if (Math.max(sz.x, sz.y, sz.z) < 1.2) return null;   // too small to warrant a modelled foundation (valves, small instruments)
    var cx = (bb.min.x + bb.max.x) / 2, cz = (bb.min.z + bb.max.z) / 2;
    var fg = new THREE.Group();
    var padTop = 0.05;   // pad sits slightly proud of grade, same convention as existing baseplate skids
    if (kind === 'CIRCULAR_PAD') {
      var r = Math.max(sz.x, sz.z) / 2 + FOUNDATION_MARGIN;
      var pad = cyl(r, r, FOUNDATION_THK, CONCRETE_MAT, 28);
      pad.position.set(cx, padTop - FOUNDATION_THK / 2, cz);
      fg.add(pad);
      fg.add(anchorBoltRing(cx, cz, r * 0.62, 8));
    } else if (kind === 'RING_WALL') {
      var rr = Math.max(sz.x, sz.z) / 2 + FOUNDATION_MARGIN * 0.6;
      var ring = mesh(new THREE.TorusGeometry(rr, 0.2, 8, 32), CONCRETE_MAT);
      ring.rotation.x = Math.PI / 2; ring.position.set(cx, padTop + 0.06, cz);
      fg.add(ring);
      var slab = cyl(rr, rr, 0.12, CONCRETE_MAT_EDGE, 28);
      slab.position.set(cx, padTop, cz);
      fg.add(slab);
    } else if (kind === 'SADDLE_FOUNDATION') {
      var w = Math.max(0.8, sz.x * 0.22), d = sz.z + FOUNDATION_MARGIN * 2;
      var padA = box(w, FOUNDATION_THK, d, CONCRETE_MAT); padA.position.set(bb.min.x + w * 0.6, padTop - FOUNDATION_THK / 2, cz);
      var padB = box(w, FOUNDATION_THK, d, CONCRETE_MAT); padB.position.set(bb.max.x - w * 0.6, padTop - FOUNDATION_THK / 2, cz);
      fg.add(padA); fg.add(padB);
    } else if (kind === 'RECTANGULAR_PAD') {
      var pw = sz.x + FOUNDATION_MARGIN * 2, pd = sz.z + FOUNDATION_MARGIN * 2;
      var padR = box(pw, FOUNDATION_THK * 0.6, pd, CONCRETE_MAT);
      padR.position.set(cx, padTop - FOUNDATION_THK * 0.3, cz);
      fg.add(padR);
      fg.add(anchorBoltRing(cx, cz, Math.min(pw, pd) * 0.32, 4));
    } else if (kind === 'STRUCTURAL_FOOTING') {
      [[bb.min.x, bb.min.z], [bb.min.x, bb.max.z], [bb.max.x, bb.min.z], [bb.max.x, bb.max.z]].forEach(function (c) {
        var f = box(0.6, 0.35, 0.6, CONCRETE_MAT);
        f.position.set(c[0], -0.16, c[1]);
        fg.add(f);
      });
    }
    fg.visible = foundationsVisible;
    fg.userData.noPorts = true;
    fg.userData.forEquip = g;
    fg.userData.props = { Type: 'Foundation — ' + kind.replace(/_/g, ' '), Equipment: type, Margin: 'PROJECT-DEFINED (' + FOUNDATION_MARGIN + ' m)', Note: 'Civil/geotechnical placeholder — verify against actual foundation design' };
    return fg;
  }
  ...
  /* Walk one pipe's own real centerline and drop a support under every
     horizontal leg long enough to plausibly need one — factored out of
     buildFromModel's own bulk pass so a single pipe's resize can rebuild
     just its own supports afterward instead of only the bulk rebuild ever
     creating them. Each support is tagged back to the pipe group it
     belongs to (userData.forPipe) so removeSupportsForPipe can find and
     clear exactly this pipe's own supports before rebuilding them — a
     support was never resized when its pipe's NPS changed (nothing ever
     revisited it after the one bulk pass that built it), leaving a
     support clamp sized for the OLD, now-wrong pipe radius sitting round
     a pipe that had since grown past it or shrunk inside it. */
  var SUPPORT_MIN_SPAN = 4, SUPPORT_SPACING = 3.5, SUPPORT_MIN_ELEV = 0.4;
  function buildSupportsForPipe(grp) {
    ...
    for (var si = 0; si < pd.pts.length - 1; si++) {
      ...
      var nSup = Math.max(1, Math.floor(len / SUPPORT_SPACING) - 1);
      for (var k = 1; k <= nSup; k++) {
        var sPos = pa2.clone().addScaledVector(full, k / (nSup + 1));
        var supGrp = pipeSupportAt(sPos, dir2, pd.pipeR);
        if (supGrp.children.length) {
          supGrp.userData.forPipe = grp;
          scene.add(supGrp); pipes3d.push(supGrp);
        }
      }
    }
  }
  function removeSupportsForPipe(grp) {
    for (var i = pipes3d.length - 1; i >= 0; i--) {
      var g = pipes3d[i];
      if (g.userData.forPipe !== grp) continue;
  ...
  function buildPipe(fromPort, toPort, color, nps, ltype, dz, result) {
    ...
    /* Default process-pipe colour: a coated dusty-rose/copper tone — the
       reference industrial 3D walkthroughs this renderer is matched against
       use exactly this colour for every process line, which is what reads
       as "real plant" at a glance (bare grey-blue pipe next to grey-blue
       equipment shells has no separation between piping and vessels at
       all). A line-type default (LTYPE3D above — electrical yellow,
       pneumatic cyan, etc.) or an explicit user-picked colour still wins;
       this is only the fallback for an ordinary, uncoloured process line. */
    var col = color || lt.color || '#c2917a';
    ...
    /* Metalness/roughness dropped from 0.75/0.4 (bare mirror-steel) to
       0.32/0.5 — piping in the reference walkthroughs reads as a coated
       pipe (soft, broad highlight) rather than polished bare metal, and a
       lower metalness also means the coloured coat still shows even where
       no specular highlight lands, instead of the colour only appearing at
       the reflection and going near-black everywhere else. */
    var mat = new THREE.MeshStandardMaterial({ color: new THREE.Color(col), metalness: 0.32, roughness: 0.5, envMapIntensity: ENV_MAP_INTENSITY });
    var emat = new THREE.MeshStandardMaterial({ color: new THREE.Color(col).multiplyScalar(0.85), metalness: 0.3, roughness: 0.52, envMapIntensity: ENV_MAP_INTENSITY });
  ...
  /* Was a smooth sine-wave brightness gradient — a continuous shimmer with
     no real high/low contrast, which read as "a pipe with a subtle glow
     animation" rather than fluid actually moving through it. Redrawn as
     distinct bright packets (soft-edged blobs, real dark gaps between
     them) scrolling along the strand — closer to how slug/droplet flow
     actually looks, and the much higher peak-to-gap contrast makes the
     motion itself unmistakable at a glance instead of needing to stare
     at it to notice the shimmer moving. */
  function flowTexture3D() {
    ...
    for (var i = 0; i < packets; i++) {
      var cx = (i + 0.5) / packets * 256;
      var rg = x.createRadialGradient(cx, 16, 0, cx, 16, 256 / packets * 0.62);
      rg.addColorStop(0, 'rgba(255,255,255,1)');
      rg.addColorStop(0.55, 'rgba(255,255,255,0.75)');
      rg.addColorStop(1, 'rgba(255,255,255,0)');
      x.fillStyle = rg;
      x.fillRect(cx - 256 / packets, 0, 256 / packets * 2, 32);
    }
  ...
  /* One tube on the pipe's own centerline — was 3 thin strands braided
     around it via the curve's Frenet frame, on the idea that CFD
     post-processors show a field as a bundle of streamlines rather than
     one flat-shaded tube. In practice the Frenet frame twists along a
     curved route (through every elbow), so the 3 offset strands visibly
     rotated around each other going through a bend — reading as separate
     layers of flow spiralling past one another rather than one body of
     fluid moving uniformly, which is what was actually asked for. A
     single thick core (0.62 of the pipe's own radius — most of the bore,
     not a thin wire down the middle) reads as one uniform flow. Colour
  ...
  function buildFlowTubeFor(grp) {
    ...
    var coreR = Math.max(0.03, pd.pipeR * 0.62);
    var tube = new THREE.TubeGeometry(curve0, segs, coreR, 10, false);
  ...
  /* Internal flow core — FLOW mode used to animate motion only in the
     connecting pipes, so every big vessel/column/reactor sat there as an
     inert box the fluid apparently skipped straight through between one
     pipe and the next. For equipment whose own Type reads as a major
     process vessel (column, reactor, vessel, tower, drum...) and that's
     actually big enough to read as a "unit operation" rather than a
     small fitting, run the same flowing-packet core through its interior
     between its own real inlet and outlet nozzles. This is an honest,
     general approximation — straight through the body via its own
     centerline, not the real tray-by-tray liquid path or catalyst-bed
     circulation a specific unit has — applied uniformly to any equipment
     of this shape rather than requiring bespoke internal mechanics
     modelled for each of the 139 factories individually. */
  var BIG_UNIT_RE = /column|reactor|vessel|tower|regenerator|fractionator|absorber|stripper|drum/i;
  /* The internal flow core above is built inside the equipment's own solid
     body — invisible, same as a pipe's core would be without setPipeDim()
     making the pipe wall see-through first. Equipment materials are
     shared singletons (mats.purple etc. — every column of that colour
     uses the SAME material object), so dimming one in place would dim
     every other piece of equipment of that colour too; this clones just
     the material on the one largest mesh in this group (almost always
     the main shell) and swaps the clone in, leaving the shared original
     untouched for every other instance. Tracked in dimmedEquip so
     clearFlowTubes can put the original back when FLOW mode ends. */
  function dimEquipmentShellFor(g) {
    var best = null, bestVol = 0;
    g.traverse(function (o) {
      if (!o.isMesh || !o.geometry) return;
      if (!o.geometry.boundingBox) o.geometry.computeBoundingBox();
      var bb = o.geometry.boundingBox, s = bb.getSize(new THREE.Vector3());
      var vol = Math.abs(s.x * s.y * s.z);
      if (vol > bestVol) { bestVol = vol; best = o; }
    });
    if (!best || !best.material) return;
    var clone = best.material.clone();
    clone.transparent = true; clone.opacity = 0.28; clone.depthWrite = false;
    dimmedEquip.push({ mesh: best, orig: best.material });
    best.material = clone;
  }
  function restoreEquipmentDim() {
    dimmedEquip.forEach(function (e) { e.mesh.material.dispose(); e.mesh.material = e.orig; });
    dimmedEquip = [];
  }
  function buildInternalFlowFor(g) {
    var type = g.userData.props && g.userData.props.Type;
    if (!type || !BIG_UNIT_RE.test(type)) return false;
    var bb = new THREE.Box3().setFromObject(g);
    var sz = bb.getSize(new THREE.Vector3());
    if (Math.max(sz.x, sz.y, sz.z) < 2) return false;   // too small to read as a "big unit"
    var ports = g.userData.ports || [];
    var inP = ports.filter(function (p) { return p.userData.port.role.indexOf('in') === 0; })[0];
    var outP = ports.filter(function (p) { return p !== inP; })[0];
    if (!inP || !outP) return false;
    var pa = new THREE.Vector3(), pb = new THREE.Vector3();
    inP.getWorldPosition(pa); outP.getWorldPosition(pb);
    if (pa.distanceTo(pb) < 0.3) return false;
    // Routed through the equipment's own centre, not a straight line
    // corner-to-corner, so the core visibly passes through the body
    // instead of clipping along its outer wall.
    var center = bb.getCenter(new THREE.Vector3());
    var mid = new THREE.Vector3(center.x, (pa.y + pb.y) / 2, center.z);
    var span = pa.distanceTo(mid) + mid.distanceTo(pb);
    if (span < 0.3) return false;
    var curve = new THREE.CatmullRomCurve3([pa, mid, pb], false, 'catmullrom', 0.15);
    var segs = Math.max(24, Math.round(span * 10));
    var r = Math.max(0.06, Math.min(sz.x, sz.z) * 0.09);
    var repeatX = Math.max(4, Math.round(span / Math.max(0.05, r * 2)));
    var rate = 0.35 * repeatX * 0.12;
    var tube = new THREE.TubeGeometry(curve, segs, r, 10, false);
    var tex = flowTexture3D().clone();
    tex.needsUpdate = true; tex.wrapS = THREE.RepeatWrapping;
    tex.repeat.set(repeatX, 1);
    var mat = new THREE.MeshBasicMaterial({
      color: 0x8fd3ff, map: tex, transparent: true, opacity: 0.85,
      depthWrite: false, side: THREE.DoubleSide
    });
    var mesh = new THREE.Mesh(tube, mat);
    mesh.renderOrder = 3;
  ...
  A3.setFlowVizMode = function (mode) {
    ...
    if (mode === 'velocity' || mode === 'pressure') {
      ...
      objects.forEach(function (g) { if (buildInternalFlowFor(g)) dimEquipmentShellFor(g); });
    }
    if (embStatus) {
  ...
  A3.buildFromModel = function (nodes, pipes, nameOf) {
    ...
    pipes3d.slice().forEach(function (grp) { buildSupportsForPipe(grp); });
    /* One field-operator figure standing at the base of every big unit
       operation (same BIG_UNIT_RE/size gate the internal flow core uses
       below) — placed just outside its footprint on whichever side has
       the most clear ground, facing the equipment. */
    objects.slice().forEach(function (g) {
      var type = g.userData.props && g.userData.props.Type;
      if (!type || !BIG_UNIT_RE.test(type)) return;
      var bb = new THREE.Box3().setFromObject(g);
      var sz = bb.getSize(new THREE.Vector3());
      if (Math.max(sz.x, sz.y, sz.z) < 2) return;
      var cx = (bb.min.x + bb.max.x) / 2, cz = (bb.min.z + bb.max.z) / 2;
      var pScale = personScaleFor(sz);
      var gap = 0.55 * pScale;
      var p = personFigure(pScale);
      p.position.set(bb.max.x + gap, 0, cz);
      p.rotation.y = -Math.PI / 2;   // face the equipment (local +X faces it)
      scene.add(p); objects.push(p);
    });
    /* PHASE 1 of the additive plant-operability module: a concrete
       foundation shaped for each item's own install type, sized from its
       own footprint. Kept out of the `objects` array (not pipeable/
       selectable equipment) so BOM/search/click-select — anything that
       treats `objects` as "the process equipment list" — stays untouched
       by this addition. */
    objects.slice().forEach(function (g) {
      var fnd = buildFoundationFor(g);
      if (fnd) { scene.add(fnd); foundations.push(fnd); }
    });
    /* PHASE 9 — see the module comment by platformRequiredFor: a ladder
       + grated platform deck on every vertical vessel/column/tank tall
       enough that its top isn't reachable from grade. Always-on
       scenery like the foundations above, not a toggleable overlay. */
    objects.slice().forEach(function (g) {
      if (!platformRequiredFor(g)) return;
      var access = buildAccessStructureFor(g);
      if (access) { scene.add(access); ladderMeshes.push(access); }
    });
    // Assembly point + site vehicle — see the module comment above
    // buildAssemblyPointMarker. Needs fresh colliders first (nothing
    // rebuilds them between A3.clearAll() at the top of this function and
    // here, and after a flowsheet SWITCH they'd otherwise still reference
    // the previous flowsheet's equipment) — just the cheap collider pass,
    // not the full weighted walk-grid refreshOperatorNav() also builds,
    // which is only actually needed once Operator Mode/WALKWAY/CHECK is
    // used and gets built there regardless.
    if (objects.length) {
      buildOperatorColliders();
      var apSpot = findAssemblyPointSpot();
      assemblyPointPos = apSpot;
      assemblyPointMesh = buildAssemblyPointMarker(apSpot.x, apSpot.z);
      scene.add(assemblyPointMesh);
      var opSpawn = findOperatorSpawn();
      var vSpot = findVehicleSpot(opSpawn);
      vehiclePos = vSpot;
      vehicleMesh = buildVehicle();
      vehicleMesh.position.set(vSpot.x, 0, vSpot.z);
      scene.add(vehicleMesh);
      objects.push(vehicleMesh);   // clickable, like everything else in the scene
      // Site office, parked off the spawn point the same way the vehicle
      // is, and a paved strip tying spawn / vehicle / office / assembly
      // point together — see the module comment above buildOfficeBuilding.
      if (!ROAD_MAT) ROAD_MAT = new THREE.MeshStandardMaterial({ color: 0x4b5563, metalness: 0.05, roughness: 0.95, envMapIntensity: ENV_MAP_INTENSITY });
      if (!ROAD_LINE_MAT) ROAD_LINE_MAT = new THREE.MeshStandardMaterial({ color: 0xfacc15, metalness: 0.05, roughness: 0.8, envMapIntensity: ENV_MAP_INTENSITY });
      var officeSpot = { x: opSpawn.x - 4.5, z: opSpawn.z + 3.5 };
      officeMesh = buildOfficeBuilding(officeSpot.x, officeSpot.z);
      scene.add(officeMesh);
      roadGroup = new THREE.Group();
      roadGroup.add(roadSegment(officeSpot, opSpawn, 2.4, ROAD_MAT, ROAD_LINE_MAT));
      roadGroup.add(roadSegment(opSpawn, vSpot, 2.0, ROAD_MAT, ROAD_LINE_MAT));
      roadGroup.add(roadSegment(opSpawn, apSpot, 2.0, ROAD_MAT, ROAD_LINE_MAT));
      scene.add(roadGroup);
    }
    rebuilding = false;
    if (objects.length) {
      ...
      /* 340 was the old cap — plenty for a handful of units, but a full
         connected plant (dozens of units laid out in a wide zig-zag) is
         routinely several times wider than that, so the camera could never
         actually pull back far enough to frame it and, combined with the
         old fixed fog density, rendered nothing at all. 2200 comfortably
         covers the widest plant this library can lay out today, and the
         fog/far-plane fixes above keep it visible at that distance. */
      sph.r = Math.min(2200, Math.max(16, Math.max(sz.x, sz.y, sz.z) * 1.7));
      updateCamera();
    }
  ...
  /* Rebuild a pipe group at a new NPS. This used to walk grp.children and
     resize EVERY CylinderGeometry mesh to (r, r, its-old-length) — but a
     pipe group isn't just the straight tube: the flange hub, flange FACE
     (deliberately 1.6x the pipe radius so it reads as a plate wider than
     the pipe), the 8 bolts and 8 nuts (their own small fixed radii, sitting
     at a bolt-circle offset computed from the OLD radius), and the gasket
     collar are all CylinderGeometry too. Force-fitting all of them to the
     bare new pipe radius shrank the flange face down to the pipe's own
     width and left the bolts — still positioned at their old, now much
     too wide, bolt-circle offset — floating outside it: exactly the
     mangled/"ghosted" geometry reported after resizing.
     buildPipe() is the one place that already knows how to proportion
     every one of those parts from a single radius, so rebuild through it
     instead of patching children in place — remove the old group
     (geometries disposed; NOT materials, since flange bolts/gaskets share
     module-level singletons other pipes still reference) and construct a
     fresh one at the new size between the exact same two ports. */
  function resizePipe(grp, nps) {
    var pd = grp.userData.pipe; if (!pd) return grp;
    removeSupportsForPipe(grp);   // old clamps were sized for the old radius — drop them, not just the pipe
    var idx = pipes3d.indexOf(grp);
    grp.traverse(function (o) { if (o.geometry) o.geometry.dispose(); });
    if (grp.parent) grp.parent.remove(grp);
    if (idx >= 0) pipes3d.splice(idx, 1);
    var wasRebuilding = rebuilding; rebuilding = true;   // suppress onConnect writeback — same endpoints, nothing to write back
    var ng = buildPipe(pd.from, pd.to, pd.color, nps, pd.ltype, pd.dz, pd.result);
    rebuilding = wasRebuilding;
    if (ng) {
      ng.userData.pipe.pid = pd.pid;
      ng.userData.pipe.sourcePortId = pd.sourcePortId;
      ng.userData.pipe.targetPortId = pd.targetPortId;
      if (pickedPipe === grp) pickedPipe = ng;
      buildSupportsForPipe(ng);   // fresh clamps sized to the new radius
    }
    return ng || grp;
  }
  ...
  A3.setAllPipeSize = function (nps) { pipes3d.slice().forEach(function (g) { resizePipe(g, nps); }); if (embStatus) embStatus('All pipes set to ' + nps + '″.'); };
  ...
  /* The reverse of selectedNid — a tag picked on the 2D sheet (or the
  ...
  A3.inspect = function () { return { scene: scene, objects: objects, picked: picked, selBox: selBox, camera: camera, renderer: renderer }; };
  A3.clearAll = function () { flowTubes.forEach(function (f) { if (f.mesh.parent) f.mesh.parent.remove(f.mesh); if (f.mesh.geometry) f.mesh.geometry.dispose(); if (f.mesh.material) f.mesh.material.dispose(); }); flowTubes = []; dimmedEquip = []; vizHalos.forEach(function (h) { if (h.parent) h.parent.remove(h); if (h.geometry) h.geometry.dispose(); if (h.material) h.material.dispose(); }); vizHalos = []; objects.forEach(function (o) { scene.remove(o); }); pipes3d.forEach(function (o) { scene.remove(o); }); foundations.forEach(function (f) { scene.remove(f); f.traverse(function (o) { if (o.geometry) o.geometry.dispose(); }); }); foundations = []; ladderMeshes.forEach(function (f) { scene.remove(f); f.traverse(function (o) { if (o.geometry) o.geometry.dispose(); }); }); ladderMeshes = []; clearWalkwayMeshes(); clearRouteLine(); clearMaintZoneMeshes(); clearSafetyZoneMeshes(); maintConflicts = []; if (reachEnvelopeMesh) { scene.remove(reachEnvelopeMesh); reachEnvelopeMesh = null; } if (assemblyPointMesh) { scene.remove(assemblyPointMesh); assemblyPointMesh.traverse(function (o) { if (o.geometry) o.geometry.dispose(); }); assemblyPointMesh = null; } assemblyPointPos = null; if (officeMesh) { scene.remove(officeMesh); officeMesh.traverse(function (o) { if (o.geometry) o.geometry.dispose(); }); officeMesh = null; } if (roadGroup) { scene.remove(roadGroup); roadGroup.traverse(function (o) { if (o.geometry) o.geometry.dispose(); }); roadGroup = null; } if (operatorObj && operatorObj.inVehicle) { operatorObj.inVehicle = false; operatorObj.figure.visible = operatorMode; } vehicleMesh = null; vehiclePos = null; walkGrid = null; clearContactShadows(); objects = []; pipes3d = []; jacketMeshes = []; portMeshes = []; spinList = []; picked = null; pickedPipe = null; pendingPort = null; portDebug = []; gapMarkers = []; connHighlighted = []; };
  A3.setFoundationsVisible = function (v) { foundationsVisible = !!v; foundations.forEach(function (f) { f.visible = foundationsVisible; }); };
  ...
  function buildScene(canvas) {
    ...
    /* The rest of this module (the 2D SVG) is always light — it never
       follows the app-wide day/night toggle, so a 3D view that DID follow
       it (dark whenever the app wasn't explicitly in day mode) was the one
       panel in this module inconsistent with its sibling. Fixed light
       background/floor/grid, matching the 2D canvas, for the same reason
       that one never goes dark either. */
    ...
    /* Matches scene.background exactly (not a darker "atmospheric" tint —
       ...
       not a mood change. */
    scene.fog = new THREE.FogExp2(0xe9edf1, FOG_DENSITY_MAX);
    /* far raised 500 → 4000: capped at 500 a camera pulled back far enough
       to frame a wide, many-unit plant (see sph.r caps below) had its own
       target equipment past the far clipping plane — invisible outright,
       on top of the fog issue above. near stays 0.1 for extreme close-up
       "LOCATE IN 3D" inspection of small fittings. */
  ...
  function updateCamera() {
    ...
    // Fog density scales with how far back the camera actually is — see the
    // FOG_K note by its declaration. Recomputed on every move (not just on
    // auto-frame) so manual wheel zoom-out on a big scene also stays clear.
    if (scene && scene.fog) {
      scene.fog.density = Math.max(FOG_DENSITY_MIN, Math.min(FOG_DENSITY_MAX, FOG_K / Math.max(sph.r, 1)));
    }
    notifyZoom();
  }
  ...
  /* Mirrors the 2D orange connection halo (render() in aro-workbench.js):
  ...
  function select(g) {
    picked = g;
    // Any equipment selection while walking around — via the FIND box, a
    // "walk to equipment" arrival, or selectByNid — should hold the
    // properties panel it just opened. Without this, the periodic operator
    // location push (updateOperatorFrame, every ~6 frames) stomped it back
    // to the OPERATOR MODE panel within a fraction of a second.
    if (operatorMode && operatorObj) operatorObj.showingEquipInfo = !!g;
    clearSelBox();
  ...
  function buildUI() {
    ...
    host.innerHTML =
      ...
      + '<span class="a3-sep"></span>'
      + '<button class="a3-btn" data-zoom="in" title="Zoom in — mouse wheel does the same">Zoom +</button><button class="a3-btn" data-zoom="out" title="Zoom out — mouse wheel does the same">Zoom &minus;</button>'
      + '<span class="a3-sep"></span>'
  ...
  function wire() {
    ...
    host.querySelectorAll('[data-zoom]').forEach(function (b) {
      b.addEventListener('click', function () {
        var factor = b.getAttribute('data-zoom') === 'in' ? 0.9 : 1.1;
        sph.r = Math.max(0.25, Math.min(2200, sph.r * factor));
        updateCamera();
      });
    });
  ...
  /* ============================================================
     ADDITIVE PLANT-OPERABILITY MODULE — PHASE 2: OPERATOR AVATAR
     A user-controlled, correctly-scaled (1.73m) walking figure with
     collision-aware movement — click-to-walk in a straight line to any
     open point, or WASD + mouse-drag look for manual control. This is
     deliberately NOT a navmesh/pathfinder (that's Phase 4's WalkwayGraph/
     PathFinder, listed after this one in the user's own phase order) —
     here "collision-aware" means the operator's requested move is
     rejected/slid along an obstacle, exactly what Phase 2 in the spec
     asks for ("Basic walking. Collision detection."), not that a route
     is planned around obstacles.
     Colliders are approximate on purpose: equipment footprints as
     circles (from each item's own XZ bounding box — same footprint data
     Phase 1's foundations already compute) and low pipe runs as
     capsules along each pipe's real centreline points, skipped once a
     run is above head height (overhead pipe racks don't block someone
     walking underneath them). Foundations are NOT separate colliders —
     they sit right under the equipment they belong to, so the
     equipment's own collider already covers that footprint.
     ============================================================ */
  var GROUND_PLANE = new THREE.Plane(new THREE.Vector3(0, 1, 0), 0);
  var operatorMode = false, operatorCamMode = 'third';
  var operatorObj = null, operatorColliders = { circles: [], segs: [] };
  var operatorSpawn = new THREE.Vector3();
  var opLookDrag = null, operatorFollowDist = 4.5, opPanelTick = 0;
  var OPERATOR_RADIUS = 0.35, OPERATOR_SPEED = 1.7, OPERATOR_SPEED_FAST = 3.4, OPERATOR_SPEED_VEHICLE = 7.5, OPERATOR_LOOK_SENS = 0.006;
  var VEHICLE_BOARD_RADIUS = 4.5;   // comfortably past computeApproachPoint's own standoff distance from the vehicle's footprint
  function buildOperatorColliders() {
    var circles = [], segs = [];
    objects.forEach(function (g) {
      if (operatorObj && g === operatorObj.figure) return;
      var t = g.userData.props && g.userData.props.Type;
      if (!t || t === 'Field Operator' || t === 'Operator (You)') return;
      var bb = new THREE.Box3().setFromObject(g);
      var sz = bb.getSize(new THREE.Vector3());
      if (Math.max(sz.x, sz.z) < 0.3) return;
      circles.push({ cx: (bb.min.x + bb.max.x) / 2, cz: (bb.min.z + bb.max.z) / 2, r: Math.max(sz.x, sz.z) / 2 });
    });
    pipes3d.forEach(function (grp) {
      var pd = grp.userData.pipe;
      if (!pd || !pd.pts || pd.pts.length < 2) return;
      for (var i = 0; i < pd.pts.length - 1; i++) {
        var a = pd.pts[i], b = pd.pts[i + 1];
        if (a.y > 1.3 || b.y > 1.3) continue;   // overhead piping doesn't block ground walking
        segs.push({ ax: a.x, az: a.z, bx: b.x, bz: b.z, r: Math.max(0.12, pd.pipeR || 0.1) });
      }
    });
    operatorColliders = { circles: circles, segs: segs };
  }
  function operatorBlocked(x, z) {
    var rr = OPERATOR_RADIUS, cs = operatorColliders.circles, sg = operatorColliders.segs, i;
    for (i = 0; i < cs.length; i++) {
      var c = cs[i], dx = x - c.cx, dz = z - c.cz, rad = c.r + rr;
      if (dx * dx + dz * dz < rad * rad) return true;
    }
    for (i = 0; i < sg.length; i++) {
      var s = sg[i], vx = s.bx - s.ax, vz = s.bz - s.az;
      var len2 = vx * vx + vz * vz;
      var t = len2 > 1e-6 ? Math.max(0, Math.min(1, ((x - s.ax) * vx + (z - s.az) * vz) / len2)) : 0;
      var px = s.ax + vx * t, pz = s.az + vz * t, dx2 = x - px, dz2 = z - pz, rad2 = s.r + rr;
      if (dx2 * dx2 + dz2 * dz2 < rad2 * rad2) return true;
    }
    return false;
  }
  function tryMoveOperator(dx, dz) {
    var p = operatorObj.pos;
    // Standing on a platform deck (Phase 9 access structure): the grade-level
    // collision grid means nothing up here, so walking is just clamped to
    // the deck's own footprint instead of checked against operatorBlocked.
    if (operatorObj.platform) {
      var pl = operatorObj.platform;
      var cnx = Math.max(pl.minX, Math.min(pl.maxX, p.x + dx));
      var cnz = Math.max(pl.minZ, Math.min(pl.maxZ, p.z + dz));
      var didMove = (cnx !== p.x || cnz !== p.z);
      p.x = cnx; p.z = cnz;
      return didMove;
    }
    var nx = p.x + dx, nz = p.z + dz;
    if (!operatorBlocked(nx, nz)) { p.x = nx; p.z = nz; return true; }
    if (!operatorBlocked(nx, p.z)) { p.x = nx; return true; }   // slide along a wall
    if (!operatorBlocked(p.x, nz)) { p.z = nz; return true; }
    return false;
  }
  function findOperatorSpawn() {
    // Spawn clear of the whole plant footprint, not just a fixed offset from
    // wherever the orbit camera happened to be aimed — that previously put
    // the operator's very first step within arm's reach of a pipe rack or
    // support, so the collision system (correctly) refused to let them walk
    // any further and it looked like movement was broken.
    var bb = new THREE.Box3(), any = false;
    objects.forEach(function (g) {
      var t = g.userData.props && g.userData.props.Type;
      if (!t || t === 'Field Operator' || t === 'Operator (You)') return;
      bb.expandByObject(g); any = true;
    });
    var baseX = any ? (bb.min.x + bb.max.x) / 2 : sph.tx;
    var baseZ = any ? bb.max.z + 8 : sph.tz + 6;
    if (!operatorBlocked(baseX, baseZ)) return new THREE.Vector3(baseX, 0, baseZ);
    for (var r = 3; r <= 100; r += 3) {
      for (var a = 0; a < Math.PI * 2; a += Math.PI / 8) {
        var tx = baseX + Math.cos(a) * r, tz = baseZ + Math.sin(a) * r;
        if (!operatorBlocked(tx, tz)) return new THREE.Vector3(tx, 0, tz);
      }
    }
    return new THREE.Vector3(baseX, 0, baseZ);
  }
  /* ============================================================
     PHASE 4: WALKWAY NETWORK + PATHFINDING
     Phase 2 gave the operator collision detection (a blocked move is
     rejected/slid, nothing more); this phase adds the actual route
     planning the spec's PlantNavMesh/WalkwayGraph/PathFinder call for.
     Rather than authoring a separate NavMesh data structure, this reuses
     the exact same collision test (operatorBlocked, over the exact same
     equipment-footprint + low-pipe colliders Phase 2 built) sampled onto
     a coarse walkability grid, then runs A* over open cells — so the
     path the operator walks and the "can I get there at all" check are
     always in sync with what's actually solid in the scene, not a
     hand-authored network that can drift out of sync with it.
     The visualised walkway network (setWalkwayVisible) is the same grid
     again: a painted path from a common hub point out to every
     equipment's approach point, so what's drawn is provably walkable —
     never a decorative line laid down separately from the nav data.
     ============================================================ */
  var WALK_GRID_SPACING = 2.0, WALK_GRID_MARGIN = 12;
  // 4x wasn't nearly strong enough — a modest diagonal shortcut across open
  // yard could still beat a longer, correctly-paved detour, which is
  // exactly the "operator walks off-road" complaint. At 30x, even a route
  // via the network several times longer than the direct line still costs
  // less than cutting the corner, so the network is followed unless it
  // genuinely doesn't reach anywhere near the destination.
  var WALKWAY_OFF_NETWORK_COST = 30;
  var WALKWAY_COST_RADIUS = 1.4;      // metres — how close a cell must be to a hub-to-equipment route to count as "on" it
  var walkGrid = null;
  // Rasterises one route segment's "on the walkway" cost=1 onto the grid,
  // by sampling points along it (cheap approximation of point-to-segment
  // distance — plenty accurate at this grid's 2m resolution).
  function markCostAlongSegment(cost, cols, rows, minX, minZ, spacing, ax, az, bx, bz, radius) {
    var steps = Math.max(1, Math.ceil(Math.hypot(bx - ax, bz - az) / (spacing * 0.5)));
    var cellR = Math.ceil(radius / spacing);
    for (var s = 0; s <= steps; s++) {
      var t = s / steps, px = ax + (bx - ax) * t, pz = az + (bz - az) * t;
      var cc = Math.round((px - minX) / spacing), cr = Math.round((pz - minZ) / spacing);
      for (var dr = -cellR; dr <= cellR; dr++) {
        for (var dc = -cellR; dc <= cellR; dc++) {
          var nc = cc + dc, nr = cr + dr;
          if (nc < 0 || nr < 0 || nc >= cols || nr >= rows) continue;
          var wx = minX + nc * spacing, wz = minZ + nr * spacing;
          if (Math.hypot(wx - px, wz - pz) <= radius) cost[nr * cols + nc] = 1;
        }
      }
    }
  }
  function buildWalkGrid() {
    ...
    if (!any) { walkGrid = null; return; }
    var minX = bb.min.x - WALK_GRID_MARGIN, minZ = bb.min.z - WALK_GRID_MARGIN;
    var maxX = bb.max.x + WALK_GRID_MARGIN, maxZ = bb.max.z + WALK_GRID_MARGIN;
    var cols = Math.max(2, Math.ceil((maxX - minX) / WALK_GRID_SPACING));
    var rows = Math.max(2, Math.ceil((maxZ - minZ) / WALK_GRID_SPACING));
    var blocked = new Uint8Array(cols * rows);
    for (var r = 0; r < rows; r++) {
      for (var c = 0; c < cols; c++) {
        blocked[r * cols + c] = operatorBlocked(minX + c * WALK_GRID_SPACING, minZ + r * WALK_GRID_SPACING) ? 1 : 0;
      }
    }
    walkGrid = { minX: minX, minZ: minZ, cols: cols, rows: rows, spacing: WALK_GRID_SPACING, blocked: blocked, cost: null };
    // Real operators walk the plant's paved aisles, not a straight line
    // across open yard — plain A* on the grid above has no reason to
    // prefer one over the other since nothing off-aisle is actually
    // "blocked". Trace the same hub-to-every-equipment routes the WALKWAY
    // visualisation draws (with cost still null here, i.e. plain-distance
    // A*, so this pass can't feed back into itself) and mark every grid
    // cell near one of those routes cheap to enter; everywhere else costs
    // WALKWAY_OFF_NETWORK_COST times as much. findPath below then hugs the
    // drawn aisles for any leg long enough for that to matter, and only
    // leaves them for the unavoidable last few metres up to the specific
    // piece of equipment being walked to.
    var cost = new Float32Array(cols * rows);
    cost.fill(WALKWAY_OFF_NETWORK_COST);
    var hub = findOperatorSpawn();
    objects.forEach(function (g) {
      var t = g.userData.props && g.userData.props.Type;
      if (!t || t === 'Field Operator' || t === 'Operator (You)' || !g.userData.nid) return;
      var gbb = new THREE.Box3().setFromObject(g);
      var gsz = gbb.getSize(new THREE.Vector3());
      if (Math.max(gsz.x, gsz.z) < 0.8) return;   // same "main aisle" size gate as the WALKWAY visualisation
      var dest = computeApproachPoint(g, hub.x, hub.z);
      if (!dest) return;
      var route = findPath(hub.x, hub.z, dest.x, dest.z);
      if (!route || route.length < 2) return;
      var prev = { x: hub.x, z: hub.z };
      route.forEach(function (wp) {
        markCostAlongSegment(cost, cols, rows, minX, minZ, WALK_GRID_SPACING, prev.x, prev.z, wp.x, wp.z, WALKWAY_COST_RADIUS);
        prev = wp;
      });
    });
    // The assembly point and site vehicle aren't process equipment, so the
    // loop above never draws a spoke out to either of them — without this
    // they'd be entirely off-network for their whole distance, at
    // WALKWAY_OFF_NETWORK_COST for the full trip, defeating the point of
    // having a network at all for two of its most-used destinations.
    [assemblyPointPos, vehiclePos].forEach(function (spot) {
      if (!spot) return;
      var route2 = findPath(hub.x, hub.z, spot.x, spot.z);
      if (!route2 || route2.length < 2) return;
      var prev2 = { x: hub.x, z: hub.z };
      route2.forEach(function (wp) {
        markCostAlongSegment(cost, cols, rows, minX, minZ, WALK_GRID_SPACING, prev2.x, prev2.z, wp.x, wp.z, WALKWAY_COST_RADIUS);
        prev2 = wp;
      });
    });
    walkGrid.cost = cost;
  }
  function refreshOperatorNav() { buildOperatorColliders(); buildWalkGrid(); }
  function cellBlocked(c, r) {
    if (!walkGrid || c < 0 || r < 0 || c >= walkGrid.cols || r >= walkGrid.rows) return true;
    return walkGrid.blocked[r * walkGrid.cols + c] === 1;
  }
  function worldToCell(x, z) { return { c: Math.round((x - walkGrid.minX) / walkGrid.spacing), r: Math.round((z - walkGrid.minZ) / walkGrid.spacing) }; }
  function cellToWorld(c, r) { return { x: walkGrid.minX + c * walkGrid.spacing, z: walkGrid.minZ + r * walkGrid.spacing }; }
  function nearestOpenCell(cell) {
    if (!cellBlocked(cell.c, cell.r)) return cell;
    for (var rad = 1; rad <= 6; rad++) {
      for (var dr = -rad; dr <= rad; dr++) {
        for (var dc = -rad; dc <= rad; dc++) {
          if (Math.max(Math.abs(dr), Math.abs(dc)) !== rad) continue;
          if (!cellBlocked(cell.c + dc, cell.r + dr)) return { c: cell.c + dc, r: cell.r + dr };
        }
      }
    }
    return null;
  }
  function pathHeuristic(a, b) { var dx = Math.abs(a.c - b.c), dz = Math.abs(a.r - b.r); return Math.max(dx, dz) + (Math.SQRT2 - 1) * Math.min(dx, dz); }
  // Binary min-heap open-set — an early version scanned a plain array for
  // the lowest fScore on every pop (O(n) per pop, O(n²) per search). On an
  // open plant with few obstacles A* has to expand a large fraction of the
  // grid before it reaches the goal, so at ~6000 cells and ~39 searches for
  // the walkway network that was measured at 5+ seconds of a frozen tab.
  // The heap makes each pop O(log n); duplicate pushes (no decrease-key)
  // are skipped lazily via the `closed` check on pop, same net result.
  ...
  function pathHeapPop(heap) {
    ...
    if (heap.length) {
      ...
      while (true) {
        ...
        if (smallest === i) break;
        var t = heap[smallest]; heap[smallest] = heap[i]; heap[i] = t; i = smallest;
      }
    }
    return top;
  }
  // Grid A* — not run per animation frame, only on a click-to-walk, a
  // "walk to equipment", or the (infrequent) walkway-network rebuild.
  function findPath(fromX, fromZ, toX, toZ) {
    if (!walkGrid) return [{ x: toX, z: toZ }];
    var g = walkGrid;
    var start = nearestOpenCell(worldToCell(fromX, fromZ));
    var goal = nearestOpenCell(worldToCell(toX, toZ));
    if (!start || !goal) return null;
    function idx(c, r) { return r * g.cols + c; }
    var startIdx = idx(start.c, start.r), goalIdx = idx(goal.c, goal.r);
    if (startIdx === goalIdx) return [{ x: toX, z: toZ }];
    var heap = [], cameFrom = {}, gScore = {}, closed = {};
    gScore[startIdx] = 0;
    pathHeapPush(heap, { idx: startIdx, f: pathHeuristic(start, goal) });
    var iter = 0, MAX_ITER = 30000;
    while (heap.length && iter++ < MAX_ITER) {
      var current = pathHeapPop(heap).idx;
      if (closed[current]) continue;
      if (current === goalIdx) {
        var pts = [], cur = current;
        while (cur !== undefined) { var cc2 = cur % g.cols, cr2 = Math.floor(cur / g.cols); pts.push(cellToWorld(cc2, cr2)); cur = cameFrom[cur]; }
        pts.reverse();
        pts[pts.length - 1] = { x: toX, z: toZ };
        return pts;
      }
      closed[current] = true;
      var cc = current % g.cols, cr = Math.floor(current / g.cols);
      for (var dr = -1; dr <= 1; dr++) {
        for (var dc = -1; dc <= 1; dc++) {
          if (!dc && !dr) continue;
          var nc = cc + dc, nr = cr + dr;
          if (cellBlocked(nc, nr)) continue;
          if (dc && dr && (cellBlocked(cc + dc, cr) || cellBlocked(cc, cr + dr))) continue;   // no cutting corners between two blocked cells
          var ni = idx(nc, nr);
          if (closed[ni]) continue;
          var cellCost = g.cost ? g.cost[ni] : 1;
          var tentativeG = gScore[current] + ((dc && dr) ? Math.SQRT2 : 1) * cellCost;
          if (tentativeG < (gScore[ni] !== undefined ? gScore[ni] : Infinity)) {
            cameFrom[ni] = current; gScore[ni] = tentativeG;
            pathHeapPush(heap, { idx: ni, f: tentativeG + pathHeuristic({ c: nc, r: nr }, goal) });
          }
        }
      }
    }
    return null;   // no route within the search budget — genuinely unreachable, or enclosed
  }
  // Same approach-point logic Phase 3 uses for "walk to equipment", pulled
  // out so the walkway-network visualisation below computes the identical
  // point rather than a second, potentially-diverging approximation.
  function approachPointFromCenter(center, footR, fromX, fromZ) {
    var standoff = Math.max(1.4, footR * 0.55 + 0.8);
    var dirX = fromX - center.x, dirZ = fromZ - center.z;
    var baseAngle = Math.hypot(dirX, dirZ) > 1e-3 ? Math.atan2(dirZ, dirX) : 0;
    for (var i = 0; i < 8; i++) {
      var ang = baseAngle + i * (Math.PI / 4);
      var cx = center.x + Math.cos(ang) * (footR + standoff), cz = center.z + Math.sin(ang) * (footR + standoff);
      if (!operatorBlocked(cx, cz)) return { x: cx, z: cz, center: center };
    }
    return null;
  }
  function computeApproachPoint(g, fromX, fromZ) {
    var bb = new THREE.Box3().setFromObject(g);
    var center = bb.getCenter(new THREE.Vector3());
    var sz = bb.getSize(new THREE.Vector3());
    var footR = Math.max(sz.x, sz.z) / 2;
    return approachPointFromCenter(center, footR, fromX, fromZ);
  }
  /* Same standoff logic, for a target that's just a world point (a field
     crew figure's last-known x/z) rather than a live mesh reference —
     used by the CALL OPERATOR panel, where the actual clicked figure gets
     deselected (select(null)) the moment answering the call turns
     Operator Mode on, so only the coordinates survive by the time DRIVE
     or WALK is actually pressed. 0.5m is a person-scale footprint, not a
     measured one. */
  function approachPointNear(x, z, fromX, fromZ, footR) {
    return approachPointFromCenter(new THREE.Vector3(x, 0, z), footR || 0.5, fromX, fromZ);
  }
  A3.approachPointNear = approachPointNear;
  var WALKWAY_MAT = null, WALKWAY_BOX_GEO = null, walkwayMeshes = [], walkwayVisible = false;
  function walkwayMatsInit() {
    if (!WALKWAY_MAT) WALKWAY_MAT = new THREE.MeshStandardMaterial({ color: 0xd9c48a, metalness: 0.04, roughness: 0.9, envMapIntensity: ENV_MAP_INTENSITY });
    if (!WALKWAY_BOX_GEO) WALKWAY_BOX_GEO = new THREE.BoxGeometry(1, 1, 1);
  }
  function clearWalkwayMeshes() { walkwayMeshes.forEach(function (m) { if (m.parent) m.parent.remove(m); }); walkwayMeshes = []; }
  function addWalkwaySegment(ax, az, bx, bz) {
    var dx = bx - ax, dz = bz - az, len = Math.hypot(dx, dz);
    if (len < 0.05) return;
    var m = new THREE.Mesh(WALKWAY_BOX_GEO, WALKWAY_MAT);
    m.scale.set(1.1, 0.04, len);
    m.position.set((ax + bx) / 2, 0.02, (az + bz) / 2);
    m.rotation.y = Math.atan2(dx, dz);
    m.renderOrder = 1;
    m.receiveShadow = true;
    scene.add(m);
    walkwayMeshes.push(m);
  }
  function buildWalkwayNetwork() {
    clearWalkwayMeshes();
    if (!walkGrid) return;
    walkwayMatsInit();
    var hub = (operatorMode && operatorObj) ? operatorObj.pos : findOperatorSpawn();
    objects.forEach(function (g) {
      var t = g.userData.props && g.userData.props.Type;
      if (!t || t === 'Field Operator' || t === 'Operator (You)' || !g.userData.nid) return;
      var bb = new THREE.Box3().setFromObject(g);
      var sz = bb.getSize(new THREE.Vector3());
      if (Math.max(sz.x, sz.z) < 0.8) return;   // skip small valves/instruments — too many to be a legible "main aisle" network
      var dest = computeApproachPoint(g, hub.x, hub.z);
      if (!dest) return;
      var path = findPath(hub.x, hub.z, dest.x, dest.z);
      if (!path || path.length < 2) return;
      var prev = { x: hub.x, z: hub.z };
      path.forEach(function (wp) { addWalkwaySegment(prev.x, prev.z, wp.x, wp.z); prev = wp; });
    });
  }
  A3.setWalkwayVisible = function (v) {
    walkwayVisible = !!v;
    if (walkwayVisible) { refreshOperatorNav(); buildWalkwayNetwork(); }
    else { clearWalkwayMeshes(); }
  };
  var routeLine = null, routeRoad = null;
  function clearRouteLine() {
    if (routeLine) { scene.remove(routeLine); if (routeLine.geometry) routeLine.geometry.dispose(); if (routeLine.material) routeLine.material.dispose(); routeLine = null; }
    if (routeRoad) { scene.remove(routeRoad); routeRoad.traverse(function (o) { if (o.geometry) o.geometry.dispose(); }); routeRoad = null; }
  }
  function updateRouteLine() {
    clearRouteLine();
    if (!operatorObj || !operatorObj.path || !operatorObj.path.length) return;
    var pts = [new THREE.Vector3(operatorObj.pos.x, 0.08, operatorObj.pos.z)];
    operatorObj.path.forEach(function (wp) { pts.push(new THREE.Vector3(wp.x, 0.08, wp.z)); });
    if (pts.length < 2) return;
    /* Driving gets a painted road under the route, not just a line — the
       vehicle is real collision geometry already routed clear of every
       obstacle (the same A* grid every foot trip uses), but a thin dashed
       line read as "wandering toward a point" rather than "following a
       route", which is what actually looked like the car cutting across
       open ground toward equipment. Walking keeps the plain line — a
       person doesn't need a paved lane drawn under their own feet. */
    if (operatorObj.inVehicle) {
      if (!ROAD_MAT) ROAD_MAT = new THREE.MeshStandardMaterial({ color: 0x4b5563, metalness: 0.05, roughness: 0.95, envMapIntensity: ENV_MAP_INTENSITY });
      if (!ROAD_LINE_MAT) ROAD_LINE_MAT = new THREE.MeshStandardMaterial({ color: 0xfacc15, metalness: 0.05, roughness: 0.8, envMapIntensity: ENV_MAP_INTENSITY });
      routeRoad = new THREE.Group();
      for (var i = 1; i < pts.length; i++) {
        routeRoad.add(roadSegment({ x: pts[i - 1].x, z: pts[i - 1].z }, { x: pts[i].x, z: pts[i].z }, 2.2, ROAD_MAT, ROAD_LINE_MAT));
      }
      scene.add(routeRoad);
    }
    var geo = new THREE.BufferGeometry().setFromPoints(pts);
    var mat = new THREE.LineDashedMaterial({ color: 0x2563eb, dashSize: 0.5, gapSize: 0.35, linewidth: 2, transparent: true, opacity: operatorObj.inVehicle ? 0.35 : 0.85 });
    routeLine = new THREE.Line(geo, mat);
    routeLine.computeLineDistances();
    routeLine.renderOrder = 4;
    scene.add(routeLine);
  }
  function distToBox2D(box, x, z) {
    var dx = Math.max(box.cx - box.hx - x, x - (box.cx + box.hx), 0);
    var dz = Math.max(box.cz - box.hz - z, z - (box.cz + box.hz), 0);
    return Math.sqrt(dx * dx + dz * dz);
  }
  /* ============================================================
     PHASE 5: MAINTENANCE CLEARANCE ENVELOPES + CONFLICT DETECTION
     A different envelope shape per equipment class (never one generic
     clearance for everything, per the spec): a motor/coupling pull zone
     beyond one end of rotating equipment, a tube-bundle removal run
     beyond one end of a shell & tube exchanger (sized to the unit's own
     length — the real rule of thumb for how much clear space a bundle
     pull needs), and a manway/ladder access strip along one side of a
     vertical vessel/column/tank. Zones are axis-aligned world-space
     boxes, not rotated to each equipment's own orientation — a
     deliberate simplification (like Phase 1's circular footprints)
     rather than a full oriented-box system.
     Conflict detection reuses the same footprint/pipe data Phase 2's
     colliders use, but rebuilt here with a source reference on each
     entry so a zone is never flagged as conflicting with its own
     equipment. ============================================================ */
  var MAINT_MAT_OK = null, MAINT_MAT_CONFLICT = null, maintZoneMeshes = [], maintVisible = false, maintConflicts = [];
  function maintMatsInit() {
    if (!MAINT_MAT_OK) MAINT_MAT_OK = new THREE.MeshBasicMaterial({ color: 0x22c55e, transparent: true, opacity: 0.22, depthWrite: false, side: THREE.DoubleSide });
    if (!MAINT_MAT_CONFLICT) MAINT_MAT_CONFLICT = new THREE.MeshBasicMaterial({ color: 0xdc2626, transparent: true, opacity: 0.4, depthWrite: false, side: THREE.DoubleSide });
  }
  function classifyMaintClass(type) {
    if (!type) return null;
    if (FOUND_ROTATING_RE.test(type)) return 'ROTATING';
    if (FOUND_HORIZONTAL_RE.test(type)) return 'HX';
    if (FOUND_TANK_RE.test(type) || FOUND_VERTICAL_RE.test(type)) return 'VESSEL';
    return null;
  }
  function maintenanceZonesFor(g) {
    var type = g.userData.props && g.userData.props.Type;
    var cls = classifyMaintClass(type);
    if (!cls) return [];
    var bb = new THREE.Box3().setFromObject(g);
    var sz = bb.getSize(new THREE.Vector3());
    if (Math.max(sz.x, sz.y, sz.z) < 1.2) return [];
    var c = bb.getCenter(new THREE.Vector3());
    var longAxisX = sz.x >= sz.z;
    var zones = [];
    if (cls === 'ROTATING') {
      // MOTOR_REMOVAL — full pull length beyond the coupling end
      var pullLen = Math.max(1.2, longAxisX ? sz.x : sz.z);
      var box = longAxisX
        ? { cx: bb.max.x + pullLen / 2, cz: c.z, hx: pullLen / 2, hz: sz.z / 2 + 0.3 }
        : { cx: c.x, cz: bb.max.z + pullLen / 2, hx: sz.x / 2 + 0.3, hz: pullLen / 2 };
      box.cy = c.y; box.hy = sz.y / 2;
      zones.push({ kind: 'MOTOR / COUPLING REMOVAL', type: 'MOTOR_REMOVAL', requiredClearance: pullLen, box: box });
      // VALVE_ACCESS — the opposite (suction/discharge) end, a shallower zone
      // for reaching the local block/isolation valves, not a full bundle pull
      var valveW = 0.9;
      var boxV = longAxisX
        ? { cx: bb.min.x - valveW / 2, cz: c.z, hx: valveW / 2, hz: sz.z / 2 + 0.2 }
        : { cx: c.x, cz: bb.min.z - valveW / 2, hx: sz.x / 2 + 0.2, hz: valveW / 2 };
      boxV.cy = c.y; boxV.hy = sz.y / 2;
      zones.push({ kind: 'SUCTION / DISCHARGE VALVE ACCESS', type: 'VALVE_ACCESS', requiredClearance: valveW, box: boxV });
    } else if (cls === 'HX') {
      // TUBE_PULL — the real rule of thumb: clear space ≥ the bundle's own length
      var tubeLen = Math.max(2, longAxisX ? sz.x : sz.z);
      var box2 = longAxisX
        ? { cx: bb.max.x + tubeLen / 2, cz: c.z, hx: tubeLen / 2, hz: sz.z / 2 }
        : { cx: c.x, cz: bb.max.z + tubeLen / 2, hx: sz.x / 2, hz: tubeLen / 2 };
      box2.cy = c.y; box2.hy = sz.y / 2;
      zones.push({ kind: 'TUBE BUNDLE PULL', type: 'TUBE_PULL', requiredClearance: tubeLen, box: box2 });
      // SERVICE — shell-side nozzle / channel-cover access along the other side
      var sideW = 1.0;
      var boxS = longAxisX
        ? { cx: c.x, cz: bb.max.z + sideW / 2, hx: sz.x / 2, hz: sideW / 2 }
        : { cx: bb.max.x + sideW / 2, cz: c.z, hx: sideW / 2, hz: sz.z / 2 };
      boxS.cy = c.y; boxS.hy = sz.y / 2;
      zones.push({ kind: 'SHELL NOZZLE / CHANNEL COVER ACCESS', type: 'SERVICE', requiredClearance: sideW, box: boxS });
    } else if (cls === 'VESSEL') {
      // MANWAY — ladder/platform access strip up one side
      var accessW = 1.2;
      var box3 = { cx: bb.max.x + accessW / 2, cz: c.z, hx: accessW / 2, hz: sz.z / 2 + 0.4, cy: c.y, hy: sz.y / 2 };
      zones.push({ kind: 'MANWAY / LADDER ACCESS', type: 'MANWAY', requiredClearance: accessW, box: box3 });
      // VALVE_ACCESS — bottom drain/instrument access at grade, opposite side
      var drainW = 0.8;
      var box4 = { cx: bb.min.x - drainW / 2, cz: c.z, hx: drainW / 2, hz: Math.min(sz.z / 2, 1.2), cy: 0.35, hy: 0.35 };
      zones.push({ kind: 'BOTTOM DRAIN / INSTRUMENT ACCESS', type: 'VALVE_ACCESS', requiredClearance: drainW, box: box4 });
    }
    return zones;
  }
  function clearMaintZoneMeshes() { maintZoneMeshes.forEach(function (m) { if (m.parent) m.parent.remove(m); }); maintZoneMeshes = []; }
  function buildMaintenanceZones() {
    clearMaintZoneMeshes();
    maintConflicts = [];
    maintMatsInit();
    var circles = [];
    objects.forEach(function (o) {
      var t = o.userData.props && o.userData.props.Type;
      if (!t || t === 'Field Operator' || t === 'Operator (You)') return;
      var bb2 = new THREE.Box3().setFromObject(o);
      var sz2 = bb2.getSize(new THREE.Vector3());
      if (Math.max(sz2.x, sz2.z) < 0.3) return;
      circles.push({ cx: (bb2.min.x + bb2.max.x) / 2, cz: (bb2.min.z + bb2.max.z) / 2, r: Math.max(sz2.x, sz2.z) / 2, src: o, type: t });
    });
    var segs = [];
    pipes3d.forEach(function (grp) {
      var pd = grp.userData.pipe;
      ...
      for (var i = 0; i < pd.pts.length - 1; i++) {
        var a = pd.pts[i], b = pd.pts[i + 1];
        segs.push({ ax: a.x, az: a.z, bx: b.x, bz: b.z, r: Math.max(0.12, pd.pipeR || 0.1), ymin: Math.min(a.y, b.y), ymax: Math.max(a.y, b.y), fromNid: pd.fromNid, toNid: pd.toNid });
      }
    });
    objects.forEach(function (g) {
      var zones = maintenanceZonesFor(g);
      if (!zones.length) return;
      var label = g.userData.props ? g.userData.props.Type : 'equipment';
      zones.forEach(function (z) {
        var conflict = null;
        for (var i = 0; i < circles.length && !conflict; i++) {
          var c2 = circles[i];
          if (c2.src === g) continue;
          if (distToBox2D(z.box, c2.cx, c2.cz) < c2.r) conflict = c2.type;
        }
        for (var j = 0; j < segs.length && !conflict; j++) {
          var s = segs[j];
          // this equipment's own process connection isn't an obstruction —
          // only a DIFFERENT line routed through the clearance counts
          if (g.userData.nid && (s.fromNid === g.userData.nid || s.toNid === g.userData.nid)) continue;
          if (z.box.cy - z.box.hy > s.ymax + 0.3 || z.box.cy + z.box.hy < s.ymin - 0.3) continue;   // no vertical overlap with this run
          var vx = s.bx - s.ax, vz = s.bz - s.az, len2 = vx * vx + vz * vz;
          var t2 = len2 > 1e-6 ? Math.max(0, Math.min(1, ((z.box.cx - s.ax) * vx + (z.box.cz - s.az) * vz) / len2)) : 0;
          var px = s.ax + vx * t2, pz = s.az + vz * t2;
          if (distToBox2D(z.box, px, pz) < s.r) conflict = 'a pipe run';
        }
        var mesh = new THREE.Mesh(new THREE.BoxGeometry(z.box.hx * 2, z.box.hy * 2, z.box.hz * 2), conflict ? MAINT_MAT_CONFLICT : MAINT_MAT_OK);
        mesh.position.set(z.box.cx, z.box.cy, z.box.cz);
        mesh.renderOrder = 2;
        scene.add(mesh);
        maintZoneMeshes.push(mesh);
        if (conflict) maintConflicts.push({ nid: g.userData.nid || null, equipment: label, zone: z.kind, blockedBy: conflict, requiredClearance: z.requiredClearance });
      });
    });
  }
  A3.setMaintenanceVisible = function (v) {
    maintVisible = !!v;
    if (maintVisible) {
      buildMaintenanceZones();
      if (embStatus) embStatus(maintConflicts.length ? (maintConflicts.length + ' MAINTENANCE ACCESS CONFLICT' + (maintConflicts.length > 1 ? 'S' : '') + ' found — see red zones.') : 'Maintenance clearance zones shown — no conflicts detected.');
    } else { clearMaintZoneMeshes(); }
  };
  A3.getMaintenanceConflicts = function () { return maintConflicts.slice(); };
  /* ============================================================
     PHASE 6: SAFETY ZONES
     Per the spec: "Do NOT invent regulatory clearance numbers." A small
     ClearanceRuleEngine-style table (SAFETY_RULES — equipmentClass,
     zoneType, minimumClearance, source, reference), not numbers
     scattered through the render code, matching the spec's own
     ClearanceRule interface. Every value here is source:'PROJECT' — a
     placeholder to be confirmed against a real hazard study, never
     presented as a code/standard number. Reuses the same ROTATING/HX/
     VESSEL buckets Phase 5 already classifies equipment into, plus an
     equipment-independent HIGH_PRESSURE rule keyed off each item's own
     Design P property (real spec data already on the equipment, not a
     guess) rather than its type.
     ============================================================ */
  var SAFETY_MAT = null, SAFETY_MAT_HP = null, safetyZoneMeshes = [], safetyZoneData = [], safetyVisible = false;
  var SAFETY_RULES = [
    { equipmentClass: 'ROTATING', zoneType: 'ROTATING_HAZARD', minimumClearance: 1.5, source: 'PROJECT', reference: 'Placeholder — confirm against site layout-spacing / HAZOP basis' },
    { equipmentClass: 'HX', zoneType: 'HOT_SURFACE', minimumClearance: 1.0, source: 'PROJECT', reference: 'Placeholder — confirm insulation / hot-surface exposure basis' },
    { equipmentClass: 'VESSEL', zoneType: 'PROCESS_HAZARD', minimumClearance: 2.0, source: 'PROJECT', reference: 'Placeholder — confirm against process hazard basis' }
  ];
  var HIGH_PRESSURE_RULE = { zoneType: 'HIGH_PRESSURE', minimumClearance: 3.0, source: 'PROJECT', reference: 'Placeholder — confirm against overpressure/PSV discharge basis', thresholdBarg: 10 };
  function safetyRuleFor(cls) { for (var i = 0; i < SAFETY_RULES.length; i++) if (SAFETY_RULES[i].equipmentClass === cls) return SAFETY_RULES[i]; return null; }
  function designPressureOf(g) {
    var props = g.userData.props || {};
    var key = Object.keys(props).filter(function (k) { return /design p \(barg\)/i.test(k); })[0];
    var v = key ? parseFloat(props[key]) : NaN;
    return isFinite(v) ? v : null;
  }
  function safetyMatsInit() {
    if (!SAFETY_MAT) SAFETY_MAT = new THREE.MeshBasicMaterial({ color: 0xdc2626, transparent: true, opacity: 0.16, depthWrite: false, side: THREE.DoubleSide });
    if (!SAFETY_MAT_HP) SAFETY_MAT_HP = new THREE.MeshBasicMaterial({ color: 0x7c2d12, transparent: true, opacity: 0.2, depthWrite: false, side: THREE.DoubleSide });
  }
  function clearSafetyZoneMeshes() { safetyZoneMeshes.forEach(function (m) { if (m.parent) m.parent.remove(m); }); safetyZoneMeshes = []; safetyZoneData = []; }
  function addSafetyZoneMesh(c, r, mat, props, ownerNid) {
    var mesh = new THREE.Mesh(new THREE.CylinderGeometry(r, r, 0.06, 28), mat);
    mesh.position.set(c.x, 0.03, c.z);
    mesh.renderOrder = 1;
    mesh.userData.noPorts = true;
    mesh.userData.props = props;
    scene.add(mesh);
    safetyZoneMeshes.push(mesh);
    safetyZoneData.push({ cx: c.x, cz: c.z, r: r, ownerNid: ownerNid || null, label: props.Type });
  }
  function buildSafetyZones() {
    clearSafetyZoneMeshes();
    safetyMatsInit();
    objects.forEach(function (g) {
      var type = g.userData.props && g.userData.props.Type;
      if (!type) return;
      var cls = classifyMaintClass(type);
      var rule = cls ? safetyRuleFor(cls) : null;
      var bb = new THREE.Box3().setFromObject(g);
      var sz = bb.getSize(new THREE.Vector3());
      if (Math.max(sz.x, sz.y, sz.z) < 1.2) return;
      var c = bb.getCenter(new THREE.Vector3());
      var footR = Math.max(sz.x, sz.z) / 2;
      if (rule) {
        addSafetyZoneMesh(c, footR + rule.minimumClearance, SAFETY_MAT, {
          Type: 'Safety Zone — ' + rule.zoneType.replace(/_/g, ' '), Equipment: type,
          Basis: rule.source + ' — ' + rule.reference, Radius: (footR + rule.minimumClearance).toFixed(2) + ' m'
        }, g.userData.nid);
      }
      var designP = designPressureOf(g);
      if (designP !== null && designP >= HIGH_PRESSURE_RULE.thresholdBarg) {
        addSafetyZoneMesh(c, footR + HIGH_PRESSURE_RULE.minimumClearance, SAFETY_MAT_HP, {
          Type: 'Safety Zone — HIGH PRESSURE', Equipment: type, 'Design P (barg)': designP,
          Basis: HIGH_PRESSURE_RULE.source + ' — ' + HIGH_PRESSURE_RULE.reference, Radius: (footR + HIGH_PRESSURE_RULE.minimumClearance).toFixed(2) + ' m'
        }, g.userData.nid);
      }
    });
  }
  A3.setSafetyZoneVisible = function (v) {
    safetyVisible = !!v;
    if (safetyVisible) buildSafetyZones(); else clearSafetyZoneMeshes();
  };
  /* ============================================================
     PHASE 9: PLATFORM ACCESS — detection + a real (if simplified) deck
     The spec's own text defers full platform/stair/ladder GENERATION to
     "later architecture" and asks this phase only to flag which
     equipment needs one — platformRequiredFor() does that. Beyond the
     flag, buildAccessStructureFor() now builds an actual always-on
     ladder (2 rails + rungs) AND a grated deck with a 3-sided handrail
     at the required height — a real deck an operator could stand on,
     just not yet code-sized (stair rise/run, full 4-sided guardrail
     with a gate at the ladder opening, structural bracing) the way a
     genuine PlatformGenerator would produce.
     ============================================================ */
  var PLATFORM_REACH_LIMIT = 2.0;   // PROJECT-DEFINED "maximum reach from grade", metres — configurable, not a validated ergonomic standard
  function platformRequiredFor(g) {
    var type = g.userData.props && g.userData.props.Type;
    if (classifyMaintClass(type) !== 'VESSEL') return false;
    var bb = new THREE.Box3().setFromObject(g);
    return (bb.max.y - bb.min.y) > (PLATFORM_REACH_LIMIT + 0.5);
  }
  var LADDER_MAT = null, PLATFORM_DECK_MAT = null, ladderMeshes = [];
  // Pure geometry for an equipment's ladder+deck — shared by the mesh
  // builder below and by A3.climbToPlatform, so the operator's climb
  // animation always lands on exactly the deck that's actually drawn.
  function accessGeometryFor(g) {
    var bb = new THREE.Box3().setFromObject(g);
    var h = bb.max.y - 0.3;
    if (h < 2) return null;
    var railGap = 0.4;
    var cx = bb.max.x + 0.12, cz = (bb.min.z + bb.max.z) / 2;
    var deckD = 1.0, deckW = Math.max(1.0, railGap + 0.6);
    var deckX = cx + deckD / 2 - 0.08;
    return { bb: bb, h: h, railGap: railGap, cx: cx, cz: cz, deckD: deckD, deckW: deckW, deckX: deckX };
  }
  function buildAccessStructureFor(g) {
    var ag = accessGeometryFor(g);
    if (!ag) return null;
    var h = ag.h, railGap = ag.railGap, cx = ag.cx, cz = ag.cz, deckD = ag.deckD, deckW = ag.deckW, deckX = ag.deckX;
    if (!LADDER_MAT) LADDER_MAT = new THREE.MeshStandardMaterial({ color: 0xf4c430, metalness: 0.5, roughness: 0.55, envMapIntensity: ENV_MAP_INTENSITY });
    if (!PLATFORM_DECK_MAT) PLATFORM_DECK_MAT = new THREE.MeshStandardMaterial({ color: 0x94a3b8, metalness: 0.5, roughness: 0.65, envMapIntensity: ENV_MAP_INTENSITY });
    var grp = new THREE.Group();
    // ladder
    var railL = cyl(0.02, 0.02, h, LADDER_MAT, 6); railL.position.set(cx, h / 2, cz - railGap / 2); grp.add(railL);
    var railR = railL.clone(); railR.position.z = cz + railGap / 2; grp.add(railR);
    var nRungs = Math.max(3, Math.floor(h / 0.3));
    for (var i = 1; i < nRungs; i++) {
      var rung = cyl(0.015, 0.015, railGap, LADDER_MAT, 6);
      rung.rotation.x = Math.PI / 2;
      rung.position.set(cx, (i / nRungs) * h, cz);
      grp.add(rung);
    }
    // grated deck at the ladder top, extending outward
    var deck = box(deckD, 0.06, deckW, PLATFORM_DECK_MAT);
    deck.position.set(deckX, h, cz);
    grp.add(deck);
    // 3-sided handrail (front + two sides — the 4th side is the ladder/equipment
    // side, where a real design would put a swing gate, not a fixed rail)
    var postH = 1.0, railY = h + postH;
    [[deckX - deckD / 2, cz - deckW / 2], [deckX + deckD / 2, cz - deckW / 2], [deckX + deckD / 2, cz + deckW / 2], [deckX - deckD / 2, cz + deckW / 2]].forEach(function (p) {
      var post = cyl(0.02, 0.02, postH, LADDER_MAT, 6);
      post.position.set(p[0], h + postH / 2, p[1]);
      grp.add(post);
    });
    var frontRail = box(0.02, 0.02, deckW, LADDER_MAT); frontRail.position.set(deckX + deckD / 2, railY, cz); grp.add(frontRail);
    var sideRail1 = box(deckD, 0.02, 0.02, LADDER_MAT); sideRail1.position.set(deckX, railY, cz - deckW / 2); grp.add(sideRail1);
    var sideRail2 = sideRail1.clone(); sideRail2.position.z = cz + deckW / 2; grp.add(sideRail2);
    grp.userData.noPorts = true;
    grp.userData.props = { Type: 'Access Platform & Ladder', Note: 'Auto-placed — deck height and ladder are real, but rise/run, full guardrail and bracing are not yet code-sized (pending a real PlatformGenerator)' };
    return grp;
  }
  /* ============================================================
     PLANT SAFETY: ASSEMBLY POINT + a SITE VEHICLE for fast travel
     Every real plant emergency plan names a muster/assembly point —
     this gives the operability model an actual one to walk to, not
     just a checklist item with no destination. The vehicle answers
     the matching complaint that a big flowsheet is a long walk from
     one end to the other: board it near where it's parked and it
     covers ground at OPERATOR_SPEED_VEHICLE instead of on-foot speed.
     Both are auto-placed once per flowsheet build, the same "always-on
     scenery" pattern as foundations and the access ladders above —
     not toggleable overlays, because they're either there or they
     aren't, the way a real site's muster point and site jeep are.
     ============================================================ */
  var ASSEMBLY_POLE_MAT = null, ASSEMBLY_FLAG_MAT = null, ASSEMBLY_RING_MAT = null, ASSEMBLY_EXT_MAT = null, ASSEMBLY_EXT_CAP_MAT = null, ASSEMBLY_EXT_STAND_MAT = null;
  var assemblyPointMesh = null, assemblyPointPos = null;
  function buildAssemblyPointMarker(x, z) {
    if (!ASSEMBLY_POLE_MAT) ASSEMBLY_POLE_MAT = new THREE.MeshStandardMaterial({ color: 0xe2e8f0, metalness: 0.3, roughness: 0.5, envMapIntensity: ENV_MAP_INTENSITY });
    if (!ASSEMBLY_FLAG_MAT) ASSEMBLY_FLAG_MAT = new THREE.MeshStandardMaterial({ color: 0x16a34a, metalness: 0.1, roughness: 0.7, envMapIntensity: ENV_MAP_INTENSITY, side: THREE.DoubleSide });
    if (!ASSEMBLY_RING_MAT) ASSEMBLY_RING_MAT = new THREE.MeshStandardMaterial({ color: 0x16a34a, metalness: 0.05, roughness: 0.85, envMapIntensity: ENV_MAP_INTENSITY });
    if (!ASSEMBLY_EXT_MAT) ASSEMBLY_EXT_MAT = new THREE.MeshStandardMaterial({ color: 0xd42020, metalness: 0.3, roughness: 0.45, envMapIntensity: ENV_MAP_INTENSITY });
    if (!ASSEMBLY_EXT_CAP_MAT) ASSEMBLY_EXT_CAP_MAT = new THREE.MeshStandardMaterial({ color: 0x1a1c22, metalness: 0.2, roughness: 0.6, envMapIntensity: ENV_MAP_INTENSITY });
    if (!ASSEMBLY_EXT_STAND_MAT) ASSEMBLY_EXT_STAND_MAT = new THREE.MeshStandardMaterial({ color: 0xfacc15, metalness: 0.2, roughness: 0.6, envMapIntensity: ENV_MAP_INTENSITY });
    var grp = new THREE.Group();
    var ring = mesh(new THREE.RingGeometry(1.5, 2.0, 32), ASSEMBLY_RING_MAT);
    ring.rotation.x = -Math.PI / 2; ring.position.set(x, 0.015, z); grp.add(ring);
    var pole = cyl(0.04, 0.04, 2.6, ASSEMBLY_POLE_MAT, 8); pole.position.set(x, 1.3, z); grp.add(pole);
    var flag = box(0.6, 0.4, 0.02, ASSEMBLY_FLAG_MAT); flag.position.set(x + 0.32, 2.35, z); grp.add(flag);
    // Muster points carry a mounted extinguisher in real plants — the
    // yellow stand makes it findable at a glance, same convention as the
    // hazard-yellow used elsewhere in the model.
    var extX = x - 2.3, extZ = z;
    var standBase = cyl(0.16, 0.18, 0.05, ASSEMBLY_EXT_STAND_MAT, 12); standBase.position.set(extX, 0.025, extZ); grp.add(standBase);
    var standPost = cyl(0.03, 0.03, 0.55, ASSEMBLY_EXT_STAND_MAT, 8); standPost.position.set(extX, 0.3, extZ); grp.add(standPost);
    var extBody = cyl(0.09, 0.075, 0.4, ASSEMBLY_EXT_MAT, 14); extBody.position.set(extX, 0.62, extZ); grp.add(extBody);
    var extCap = cyl(0.055, 0.075, 0.06, ASSEMBLY_EXT_CAP_MAT, 14); extCap.position.set(extX, 0.85, extZ); grp.add(extCap);
    var extHandle = box(0.025, 0.13, 0.025, ASSEMBLY_EXT_CAP_MAT); extHandle.position.set(extX, 0.92, extZ); grp.add(extHandle);
    grp.userData.noPorts = true;
    grp.userData.props = { Type: 'Assembly Point', Note: 'Designated muster point with a mounted fire extinguisher — the operator can be sent here from the CALL OPERATOR panel or the ASSEMBLY toolbar button.' };
    return grp;
  }
  /* ============================================================
     SITE OFFICE + PLANT ROAD — the ground-level "arrival" scenery
     around the operator's spawn point: a small office block with its
     own concrete apron, and a paved strip linking spawn, the parked
     vehicle and the assembly point. Purely visual dressing (foot/vehicle
     safety is already enforced by the real A* walkway pathfinder these
     meshes sit beside, not by this geometry) — it exists so the plant
     reads as a real site with an entrance, not a bare equipment field.
     ============================================================ */
  var OFFICE_WALL_MAT = null, OFFICE_ROOF_MAT = null, OFFICE_DOOR_MAT = null, OFFICE_GLASS_MAT = null,
      OFFICE_PLINTH_MAT = null, ROAD_MAT = null, ROAD_LINE_MAT = null, OFFICE_SIGN_TEX = null;
  var officeMesh = null, roadGroup = null;
  function officeSignTexture() {
    if (OFFICE_SIGN_TEX) return OFFICE_SIGN_TEX;
    var cv = document.createElement('canvas'); cv.width = 384; cv.height = 96;
    var cx = cv.getContext('2d');
    cx.fillStyle = '#0f172a'; cx.fillRect(0, 0, cv.width, cv.height);
    cx.strokeStyle = '#ff7538'; cx.lineWidth = 5; cx.strokeRect(3, 3, cv.width - 6, cv.height - 6);
    cx.textAlign = 'center'; cx.textBaseline = 'middle';
    cx.font = '800 34px ui-monospace, "IBM Plex Mono", monospace';
    cx.fillStyle = '#f8fafc';
    cx.fillText('AROGARA SITE OFFICE', cv.width / 2, cv.height / 2);
    OFFICE_SIGN_TEX = new THREE.CanvasTexture(cv);
    return OFFICE_SIGN_TEX;
  }
  function buildOfficeBuilding(x, z) {
    if (!OFFICE_WALL_MAT) OFFICE_WALL_MAT = new THREE.MeshStandardMaterial({ color: 0xe7e2d6, metalness: 0.05, roughness: 0.85, envMapIntensity: ENV_MAP_INTENSITY });
    if (!OFFICE_ROOF_MAT) OFFICE_ROOF_MAT = new THREE.MeshStandardMaterial({ color: 0x3f5163, metalness: 0.2, roughness: 0.6, envMapIntensity: ENV_MAP_INTENSITY });
    if (!OFFICE_DOOR_MAT) OFFICE_DOOR_MAT = new THREE.MeshStandardMaterial({ color: 0x4a3423, metalness: 0.1, roughness: 0.7, envMapIntensity: ENV_MAP_INTENSITY });
    if (!OFFICE_GLASS_MAT) OFFICE_GLASS_MAT = new THREE.MeshStandardMaterial({ color: 0x93c5e8, metalness: 0.6, roughness: 0.15, transparent: true, opacity: 0.6, envMapIntensity: ENV_MAP_INTENSITY });
    if (!OFFICE_PLINTH_MAT) OFFICE_PLINTH_MAT = new THREE.MeshStandardMaterial({ color: 0x9ca3af, metalness: 0.05, roughness: 0.9, envMapIntensity: ENV_MAP_INTENSITY });
    var grp = new THREE.Group();
    var W = 5.2, D = 3.6, H = 2.7;
    var plinth = box(W + 1.4, 0.2, D + 1.4, OFFICE_PLINTH_MAT); plinth.position.set(x, 0.1, z); grp.add(plinth);
    var walls = box(W, H, D, OFFICE_WALL_MAT); walls.position.set(x, 0.2 + H / 2, z); grp.add(walls);
    var roof = box(W + 0.4, 0.14, D + 0.4, OFFICE_ROOF_MAT); roof.position.set(x, 0.2 + H + 0.07, z); grp.add(roof);
    var door = box(0.9, 1.9, 0.05, OFFICE_DOOR_MAT); door.position.set(x - 1.4, 0.2 + 0.95, z + D / 2 + 0.02); grp.add(door);
    [x + 0.1, x + 1.6].forEach(function (wx) {
      var win = box(0.9, 1.0, 0.05, OFFICE_GLASS_MAT); win.position.set(wx, 0.2 + 1.55, z + D / 2 + 0.02); grp.add(win);
      var sill = box(1.0, 0.06, 0.1, OFFICE_ROOF_MAT); sill.position.set(wx, 0.2 + 1.0, z + D / 2 + 0.05); grp.add(sill);
    });
    var signGeo = new THREE.PlaneGeometry(1.9, 0.48);
    var signMat = new THREE.MeshBasicMaterial({ map: officeSignTexture(), transparent: true });
    var sign = new THREE.Mesh(signGeo, signMat);
    sign.position.set(x, 0.2 + H + 0.36, z + D / 2 + 0.03);
    grp.add(sign);
    grp.userData.noPorts = true;
    grp.userData.props = { Type: 'Site Office', Note: 'Maintenance dispatch base — where a CALL OPERATOR response is coordinated from before the crew walks or drives out.' };
    return grp;
  }
  function roadSegment(a, b, width, mat, lineMat) {
    var g = new THREE.Group();
    var dx = b.x - a.x, dz = b.z - a.z, len = Math.hypot(dx, dz);
    if (len < 0.05) return g;
    var slab = box(width, 0.02, len, mat);
    slab.position.set((a.x + b.x) / 2, 0.011, (a.z + b.z) / 2);
    slab.rotation.y = Math.atan2(dx, dz);
    g.add(slab);
    var line = box(0.08, 0.001, len * 0.9, lineMat);
    line.position.set((a.x + b.x) / 2, 0.022, (a.z + b.z) / 2);
    line.rotation.y = slab.rotation.y;
    g.add(line);
    g.userData.noPorts = true;
    return g;
  }
  // Clear of the whole plant footprint, anchored at the opposite corner
  // from findOperatorSpawn() so the two markers don't collide.
  function findAssemblyPointSpot() {
    var bb = new THREE.Box3(), any = false;
    objects.forEach(function (g) {
      var t = g.userData.props && g.userData.props.Type;
      if (!t || t === 'Field Operator' || t === 'Operator (You)' || !g.userData.nid) return;
    ...
    var baseX = any ? bb.min.x - 10 : -10, baseZ = any ? bb.min.z - 10 : -10;
    if (!operatorBlocked(baseX, baseZ)) return { x: baseX, z: baseZ };
    for (var r = 3; r <= 100; r += 3) {
      for (var a = 0; a < Math.PI * 2; a += Math.PI / 8) {
        var tx = baseX + Math.cos(a) * r, tz = baseZ + Math.sin(a) * r;
        if (!operatorBlocked(tx, tz)) return { x: tx, z: tz };
      }
    }
    return { x: baseX, z: baseZ };
  }
  var VEHICLE_BODY_MAT = null, VEHICLE_DARK_MAT = null, VEHICLE_GLASS_MAT = null,
      VEHICLE_RIM_MAT = null, VEHICLE_STEEL_MAT = null, VEHICLE_EXT_MAT = null,
      VEHICLE_EXT_HANDLE_MAT = null, VEHICLE_TOOL_MAT = null, VEHICLE_BEACON_MAT = null,
      VEHICLE_LAMP_MAT = null, VEHICLE_TAIL_MAT = null, VEHICLE_LOGO_TEX = null;
  var vehicleMesh = null, vehiclePos = null;
  /* The vehicle's side decal — one canvas drawn once and shared by both
     doors (the left one gets scale.x = -1, not a second draw), same
     brand mark and saffron accent as the app chrome itself so the model
     reads as "ours" rather than a generic prop. */
  function vehicleLogoTexture() {
    if (VEHICLE_LOGO_TEX) return VEHICLE_LOGO_TEX;
    var cv = document.createElement('canvas'); cv.width = 512; cv.height = 160;
    var cx = cv.getContext('2d');
    cx.fillStyle = '#f8fafc'; cx.beginPath();
    (cx.roundRect ? cx.roundRect(0, 0, cv.width, cv.height, 14) : cx.rect(0, 0, cv.width, cv.height)); cx.fill();
    cx.strokeStyle = '#ff7538'; cx.lineWidth = 6; cx.strokeRect(3, 3, cv.width - 6, cv.height - 6);
    cx.textAlign = 'center'; cx.textBaseline = 'middle';
    cx.font = '800 62px ui-monospace, "IBM Plex Mono", monospace';
    cx.fillStyle = '#ff7538';
    cx.fillText('AROGARA', cv.width / 2, 72);
    cx.font = '700 20px ui-monospace, "IBM Plex Mono", monospace';
    cx.fillStyle = '#334155';
    cx.fillText('DIGITAL ENGINEERING DESIGN PLATFORM', cv.width / 2, 122);
    VEHICLE_LOGO_TEX = new THREE.CanvasTexture(cv);
    VEHICLE_LOGO_TEX.anisotropy = 4;
    return VEHICLE_LOGO_TEX;
  }
  function buildVehicle() {
    if (!VEHICLE_BODY_MAT) VEHICLE_BODY_MAT = new THREE.MeshStandardMaterial({ color: 0x2f6b3a, metalness: 0.35, roughness: 0.5, envMapIntensity: ENV_MAP_INTENSITY });
    if (!VEHICLE_DARK_MAT) VEHICLE_DARK_MAT = new THREE.MeshStandardMaterial({ color: 0x1a1c22, metalness: 0.2, roughness: 0.7, envMapIntensity: ENV_MAP_INTENSITY });
    if (!VEHICLE_GLASS_MAT) VEHICLE_GLASS_MAT = new THREE.MeshStandardMaterial({ color: 0x93c5e8, metalness: 0.6, roughness: 0.15, transparent: true, opacity: 0.55, envMapIntensity: ENV_MAP_INTENSITY });
    if (!VEHICLE_RIM_MAT) VEHICLE_RIM_MAT = new THREE.MeshStandardMaterial({ color: 0xcbd5e1, metalness: 0.75, roughness: 0.35, envMapIntensity: ENV_MAP_INTENSITY });
    if (!VEHICLE_STEEL_MAT) VEHICLE_STEEL_MAT = new THREE.MeshStandardMaterial({ color: 0x94a3b8, metalness: 0.7, roughness: 0.4, envMapIntensity: ENV_MAP_INTENSITY });
    if (!VEHICLE_EXT_MAT) VEHICLE_EXT_MAT = new THREE.MeshStandardMaterial({ color: 0xd42020, metalness: 0.3, roughness: 0.45, envMapIntensity: ENV_MAP_INTENSITY });
    if (!VEHICLE_EXT_HANDLE_MAT) VEHICLE_EXT_HANDLE_MAT = new THREE.MeshStandardMaterial({ color: 0x1a1c22, metalness: 0.2, roughness: 0.6, envMapIntensity: ENV_MAP_INTENSITY });
    if (!VEHICLE_TOOL_MAT) VEHICLE_TOOL_MAT = new THREE.MeshStandardMaterial({ color: 0xf59e0b, metalness: 0.55, roughness: 0.35, envMapIntensity: ENV_MAP_INTENSITY });
    if (!VEHICLE_BEACON_MAT) VEHICLE_BEACON_MAT = new THREE.MeshStandardMaterial({ color: 0xffb300, emissive: 0xff8f00, emissiveIntensity: 0.6, metalness: 0.1, roughness: 0.3, envMapIntensity: ENV_MAP_INTENSITY });
    if (!VEHICLE_LAMP_MAT) VEHICLE_LAMP_MAT = new THREE.MeshStandardMaterial({ color: 0xfff7e0, emissive: 0xffe58a, emissiveIntensity: 0.4, metalness: 0.1, roughness: 0.3, envMapIntensity: ENV_MAP_INTENSITY });
    if (!VEHICLE_TAIL_MAT) VEHICLE_TAIL_MAT = new THREE.MeshStandardMaterial({ color: 0xb91c1c, emissive: 0x7f1d1d, emissiveIntensity: 0.3, metalness: 0.2, roughness: 0.4, envMapIntensity: ENV_MAP_INTENSITY });
    var grp = new THREE.Group();

    // ── Body: cab up front (+Z), open service bed at the rear (-Z) ──
    var body = box(1.7, 0.6, 3.4, VEHICLE_BODY_MAT); body.position.set(0, 0.5, 0); grp.add(body);
    var cabin = box(1.55, 0.55, 1.6, VEHICLE_BODY_MAT); cabin.position.set(0, 1.1, -0.35); grp.add(cabin);
    var windshield = box(1.5, 0.5, 0.05, VEHICLE_GLASS_MAT); windshield.position.set(0, 1.1, 0.42); grp.add(windshield);
    var rearGlass = box(1.5, 0.42, 0.05, VEHICLE_GLASS_MAT); rearGlass.position.set(0, 1.08, -1.15); grp.add(rearGlass);
    // Door seam lines and mirrors — cheap detail, reads as "a real vehicle" from a few metres away
    [-0.86, 0.86].forEach(function (dx) {
      var seam = box(0.015, 0.5, 1.5, VEHICLE_DARK_MAT); seam.position.set(dx, 0.55, 0.1); grp.add(seam);
      var mirrorArm = box(0.06, 0.03, 0.18, VEHICLE_DARK_MAT); mirrorArm.position.set(dx * 0.98, 1.18, 0.55); grp.add(mirrorArm);
      var mirror = box(0.05, 0.14, 0.2, VEHICLE_DARK_MAT); mirror.position.set(dx * 1.12, 1.16, 0.55); grp.add(mirror);
    });
    // Headlamps / tail-lamps
    [-0.55, 0.55].forEach(function (dx) {
      var head = box(0.28, 0.14, 0.04, VEHICLE_LAMP_MAT); head.position.set(dx, 0.62, 1.72); grp.add(head);
      var tail = box(0.22, 0.16, 0.04, VEHICLE_TAIL_MAT); tail.position.set(dx, 0.62, -1.72); grp.add(tail);
    });
    // Wheels with a lighter alloy-style rim inset
    [[-0.75, -1.1], [0.75, -1.1], [-0.75, 1.1], [0.75, 1.1]].forEach(function (p) {
      var wheel = cyl(0.32, 0.32, 0.22, VEHICLE_DARK_MAT, 18); wheel.rotation.z = Math.PI / 2; wheel.position.set(p[0], 0.32, p[1]); grp.add(wheel);
      var rim = cyl(0.16, 0.16, 0.23, VEHICLE_RIM_MAT, 18); rim.rotation.z = Math.PI / 2; rim.position.set(p[0], 0.32, p[1]); grp.add(rim);
    });
    var bar = box(1.6, 0.06, 0.06, VEHICLE_DARK_MAT); bar.position.set(0, 0.85, 1.55); grp.add(bar);   // front bumper/roll-bar hint
    // Amber roof beacon — the plant-standard "service vehicle on site" cue
    var beaconPost = cyl(0.02, 0.02, 0.1, VEHICLE_DARK_MAT, 8); beaconPost.position.set(0, 1.43, -0.35); grp.add(beaconPost);
    var beacon = mesh(new THREE.SphereGeometry(0.08, 12, 10), VEHICLE_BEACON_MAT); beacon.position.set(0, 1.53, -0.35); grp.add(beacon);

    // ── Rear service bed: low rails + the tools/pipe the crew actually carries ──
    var bedRailMat = VEHICLE_DARK_MAT;
    var bedFloorZ = -1.7, bedFrontZ = -1.15, bedY = 0.82;
    [[-0.83, (bedFloorZ + bedFrontZ) / 2], [0.83, (bedFloorZ + bedFrontZ) / 2]].forEach(function (p) {
      var rail = box(0.04, 0.26, Math.abs(bedFrontZ - bedFloorZ), bedRailMat); rail.position.set(p[0], bedY, p[1]); grp.add(rail);
    });
    var tailgate = box(1.66, 0.24, 0.04, bedRailMat); tailgate.position.set(0, bedY, bedFloorZ); grp.add(tailgate);
    // Toolbox against the cab-side bulkhead
    var toolbox = box(0.55, 0.28, 0.4, VEHICLE_STEEL_MAT); toolbox.position.set(-0.45, 0.94, bedFrontZ + 0.25); grp.add(toolbox);
    var toolboxLid = box(0.57, 0.03, 0.42, VEHICLE_DARK_MAT); toolboxLid.position.set(-0.45, 1.09, bedFrontZ + 0.25); grp.add(toolboxLid);
    // A pair of spanners laid across the toolbox — shaft + a wider head at each end
    [0.05, 0.13].forEach(function (dz, i) {
      var spGrp = new THREE.Group();
      var shaft = box(0.26, 0.018, 0.03, VEHICLE_TOOL_MAT); spGrp.add(shaft);
      [-0.12, 0.12].forEach(function (ex) {
        var headJaw = box(0.05, 0.045, 0.03, VEHICLE_TOOL_MAT); headJaw.position.set(ex, 0, 0); spGrp.add(headJaw);
      });
      spGrp.rotation.y = 0.25 + i * 0.3;
      spGrp.position.set(-0.42 + i * 0.06, 1.115, bedFrontZ + 0.22 - i * 0.05);
      grp.add(spGrp);
    });
    // A length of pipe carried flat along the bed floor
    var pipe = cyl(0.045, 0.045, 1.5, VEHICLE_STEEL_MAT, 14);
    pipe.rotation.x = Math.PI / 2; pipe.position.set(0.45, 0.9, -1.35); grp.add(pipe);
    var pipeCapA = cyl(0.05, 0.05, 0.02, VEHICLE_DARK_MAT, 14); pipeCapA.rotation.x = Math.PI / 2; pipeCapA.position.set(0.45, 0.9, -1.35 - 0.75); grp.add(pipeCapA);
    var pipeCapB = pipeCapA.clone(); pipeCapB.position.z = -1.35 + 0.75; grp.add(pipeCapB);

    // ── Fire extinguisher — mounted on the cab exterior, in plain view ──
    var extBody = cyl(0.075, 0.06, 0.32, VEHICLE_EXT_MAT, 14); extBody.position.set(0.85, 0.98, -0.6); grp.add(extBody);
    var extCap = cyl(0.045, 0.06, 0.05, VEHICLE_EXT_HANDLE_MAT, 14); extCap.position.set(0.85, 1.17, -0.6); grp.add(extCap);
    var extHandle = box(0.02, 0.11, 0.02, VEHICLE_EXT_HANDLE_MAT); extHandle.position.set(0.85, 1.22, -0.6); grp.add(extHandle);
    var extHose = cyl(0.008, 0.008, 0.16, VEHICLE_EXT_HANDLE_MAT, 8); extHose.rotation.z = Math.PI / 2.4; extHose.position.set(0.87, 1.05, -0.52); grp.add(extHose);
    var extBracket = box(0.1, 0.28, 0.03, VEHICLE_DARK_MAT); extBracket.position.set(0.795, 0.98, -0.6); grp.add(extBracket);

    // ── AROGARA side decals — one texture, mirrored for the far door ──
    var logoTex = vehicleLogoTexture();
    var logoMat = new THREE.MeshBasicMaterial({ map: logoTex, transparent: true });
    var logoGeo = new THREE.PlaneGeometry(1.9, 0.6);
    var logoR = new THREE.Mesh(logoGeo, logoMat);
    logoR.rotation.y = Math.PI / 2; logoR.position.set(0.856, 0.62, 0.05);
    grp.add(logoR);
    var logoL = new THREE.Mesh(logoGeo, logoMat);
    logoL.rotation.y = -Math.PI / 2; logoL.scale.x = -1; logoL.position.set(-0.856, 0.62, 0.05);
    grp.add(logoL);

    grp.userData.noPorts = true;
    grp.userData.props = { Type: 'Site Vehicle', Note: 'Board it (within ' + VEHICLE_BOARD_RADIUS + 'm) for fast long-distance travel — ' + OPERATOR_SPEED_VEHICLE + ' m/s vs. ' + OPERATOR_SPEED + ' m/s on foot. Carries a toolbox, spare pipe and a mounted fire extinguisher, same as a real site response vehicle.' };
    return grp;
  }
  // Parked near the operator's own spawn point (the plant "entrance"/motor
  // pool), offset so it doesn't sit exactly on top of the spawn location.
  function findVehicleSpot(spawn) {
    var baseX = spawn.x + 4, baseZ = spawn.z + 2;
    if (!operatorBlocked(baseX, baseZ)) return { x: baseX, z: baseZ };
    for (var r = 2; r <= 60; r += 2) {
      for (var a = 0; a < Math.PI * 2; a += Math.PI / 8) {
        var tx = baseX + Math.cos(a) * r, tz = baseZ + Math.sin(a) * r;
        if (!operatorBlocked(tx, tz)) return { x: tx, z: tz };
      }
  ...
  /* ============================================================
     PHASE 8: FIRST-PERSON REACH CHECK
     Section 20 of the spec: a configurable ergonomic reach envelope,
     explicitly NOT a hard-coded "official" ergonomics number —
     REACH_COMFORTABLE/REACH_MAX are project-tunable metres, same
     PROJECT-DEFINED spirit as Phases 5/6's clearances. Visualises as a
     translucent cylinder that follows the operator while walking
     (toggle from the operator panel — meaningful only in Operator
     Mode, first-person or third), and reachIssuesFor() feeds the
     operability checker below so an out-of-reach valve or instrument
     shows up without physically having to walk up to it and look.
     ============================================================ */
  var REACH_COMFORTABLE = 1.6, REACH_MAX = 2.0;   // PROJECT-DEFINED, metres — configurable, not a validated ergonomic standard
  var REACH_MAT = null, reachEnvelopeMesh = null, reachCheckVisible = false;
  function reachMatInit() { if (!REACH_MAT) REACH_MAT = new THREE.MeshBasicMaterial({ color: 0x2563eb, transparent: true, opacity: 0.12, depthWrite: false, side: THREE.DoubleSide }); }
  function updateReachEnvelope() {
    if (!reachCheckVisible || !operatorMode || !operatorObj) { if (reachEnvelopeMesh) reachEnvelopeMesh.visible = false; return; }
    if (!reachEnvelopeMesh) {
      reachMatInit();
      reachEnvelopeMesh = new THREE.Mesh(new THREE.CylinderGeometry(0.7, 0.7, REACH_MAX, 20, 1, true), REACH_MAT);
      reachEnvelopeMesh.renderOrder = 3;
      scene.add(reachEnvelopeMesh);
    }
    reachEnvelopeMesh.visible = true;
    reachEnvelopeMesh.position.set(operatorObj.pos.x, REACH_MAX / 2, operatorObj.pos.z);
  }
  A3.setReachCheckVisible = function (v) {
    reachCheckVisible = !!v;
    updateReachEnvelope();
  };
  A3.getReachCheckVisible = function () { return reachCheckVisible; };
  function reachIssuesFor(g) {
    var issues = [];
    (g.userData.ports || []).forEach(function (p) {
      var wp = new THREE.Vector3(); p.getWorldPosition(wp);
      if (wp.y > REACH_MAX) issues.push({ port: (p.userData.port && (p.userData.port.name || p.userData.port.role)) || 'port', elevation: wp.y });
    });
    return issues;
  }
  function pathNearPoint2D(path, hub, cx, cz, r) {
    var prev = { x: hub.x, z: hub.z };
    for (var i = 0; i < path.length; i++) {
      var wp = path[i];
      var vx = wp.x - prev.x, vz = wp.z - prev.z, len2 = vx * vx + vz * vz;
      var t = len2 > 1e-6 ? Math.max(0, Math.min(1, ((cx - prev.x) * vx + (cz - prev.z) * vz) / len2)) : 0;
      var px = prev.x + vx * t, pz = prev.z + vz * t;
  ...
  /* ============================================================
     PHASE 7: OPERABILITY CHECKER
     Ties Phases 2-9 together into the questions the spec's 12-point
     checklist (section 19) actually asks. Covered here, mapped to that
     list: (1) valid walkway — Phase 4's real pathfinder ; (6)(7)(8)
     maintenance clearance blocked / motor / bundle removable — Phase 5 ;
     (9) stairs/platform required — Phase 9 ; (2)(3)(4)(5) reach/access
     to controls, manways, valves, instruments — Phase 8's reach check ;
     (12) safety zone intersecting the normal walkway — cross-checked
     against Phase 6's zones ; (11) foundation interfering with access —
     cross-checked against Phase 1's foundation footprints (which are
     NOT part of the Phase 2/4 walk collider, so a path can legally
     route across one without pathfinding ever knowing) ; (10) overhead
     pipes obstructing access — pipe runs between ground-collision height
     and head height, sampled along the route. Not yet modelled: item
     from the checklist requiring real per-nozzle valve/instrument
     identification beyond port elevation (see reachIssuesFor's own
     scope note).
     ============================================================ */
  A3.runOperabilityCheck = function () {
    refreshOperatorNav();
    buildMaintenanceZones();
    buildSafetyZones();   // refresh safetyZoneData for the cross-check below, whether or not the SAFETY layer is currently shown
    var hub = (operatorMode && operatorObj) ? operatorObj.pos : findOperatorSpawn();
    // pipe runs between ground-collision height (Phase 2 ignores these —
    // they don't block walking) and head height — these DO obstruct someone
    // walking underneath, so item 10 needs its own, wider band
    var overheadSegs = [];
    pipes3d.forEach(function (grp) {
      var pd = grp.userData.pipe;
      if (!pd || !pd.pts || pd.pts.length < 2) return;
      for (var i = 0; i < pd.pts.length - 1; i++) {
        ...
        if (ymax < 1.3 || ymin > 2.0) continue;
        overheadSegs.push({ ax: a.x, az: a.z, bx: b.x, bz: b.z, fromNid: pd.fromNid, toNid: pd.toNid });
      }
    });
    var foundationCircles = [];
    foundations.forEach(function (fnd) {
      var owner = fnd.userData.forEquip;
      var bb = new THREE.Box3().setFromObject(fnd);
      var sz = bb.getSize(new THREE.Vector3());
      var c = bb.getCenter(new THREE.Vector3());
      foundationCircles.push({ cx: c.x, cz: c.z, r: Math.max(sz.x, sz.z) / 2, ownerNid: owner ? owner.userData.nid : null });
    });
    var report = [];
    objects.forEach(function (g) {
      var type = g.userData.props && g.userData.props.Type;
      if (!type || type === 'Field Operator' || type === 'Operator (You)' || !g.userData.nid) return;
      var dest = computeApproachPoint(g, hub.x, hub.z);
      var path = dest ? findPath(hub.x, hub.z, dest.x, dest.z) : null;
      var conflicts = maintConflicts.filter(function (c) { return c.nid && c.nid === g.userData.nid; });
      var maintClass = classifyMaintClass(type);
      var reach = reachIssuesFor(g);
      var routeNotes = [];
      if (path) {
        safetyZoneData.forEach(function (z) {
          if (z.ownerNid === g.userData.nid) return;
          if (pathNearPoint2D(path, hub, z.cx, z.cz, z.r)) routeNotes.push('crosses ' + z.label);
        });
        foundationCircles.forEach(function (f) {
          if (f.ownerNid === g.userData.nid) return;
          if (pathNearPoint2D(path, hub, f.cx, f.cz, f.r)) routeNotes.push('crosses another item\'s foundation');
        });
        overheadSegs.forEach(function (s) {
          if (s.fromNid === g.userData.nid || s.toNid === g.userData.nid) return;
          if (pathNearPoint2D(path, hub, (s.ax + s.bx) / 2, (s.az + s.bz) / 2, 0.6)) routeNotes.push('passes under a low pipe run');
        });
      }
      report.push({
        nid: g.userData.nid,
        type: type,
        operatorAccess: path ? 'PASS' : 'FAIL',
        maintenanceAccess: maintClass ? (conflicts.length ? 'WARNING' : 'PASS') : 'N/A',
        maintenanceConflict: conflicts.length ? conflicts.map(function (c) { return c.zone + ' — blocked by ' + c.blockedBy; }).join('; ') : null,
        platformRequired: platformRequiredFor(g),
        reachWarning: reach.length ? reach.map(function (r) { return r.port + ' at ' + r.elevation.toFixed(2) + 'm (max ' + REACH_MAX.toFixed(2) + 'm)'; }).join('; ') : null,
        routeNote: routeNotes.length ? routeNotes.join('; ') : null
      });
    });
    // buildMaintenanceZones()/buildSafetyZones() above were only needed to
    // refresh the conflict/zone DATA this report just used — if the
    // corresponding toolbar layer isn't actually toggled on, tear the
    // meshes back down rather than silently making them appear in the
    // scene as a side effect of running the check.
    if (!maintVisible) clearMaintZoneMeshes();
    if (!safetyVisible) clearSafetyZoneMeshes();
    return report;
  };
  function nearestEquipmentLabel(pos) {
    ...
    objects.forEach(function (g) {
      ...
      var bb = new THREE.Box3().setFromObject(g);
      var c = bb.getCenter(new THREE.Vector3());
      var d = Math.hypot(c.x - pos.x, c.z - pos.z);
      if (d < bestD) { bestD = d; best = t; }
    });
    return (best && bestD < 10) ? best : 'Open Area';
  }
  // A human-readable "where is the operator actually headed right now" —
  // the CALL OPERATOR panel and the toolbar buttons all end up setting one
  // of these fields, so this is the one place that turns whichever is set
  // into a single destination string for the live OPERATOR MODE panel.
  function operatorDestinationLabel(o) {
    if (!o.path || !o.path.length) return null;
    if (o.boardOnArrive) return 'the site vehicle';
    if (o.climbTarget) return 'the ladder at ' + (o.climbTarget.g.userData.props ? o.climbTarget.g.userData.props.Type : 'equipment');
    if (o.arriveEquip) return (o.arriveEquip.userData.props ? o.arriveEquip.userData.props.Type : 'equipment');
    if (o.arriveLabel) return o.arriveLabel;
    return 'the clicked point';
  }
  function operatorPanelInfo() {
    if (!operatorMode || !operatorObj) return { active: false };
    return {
      active: true, x: operatorObj.pos.x, z: operatorObj.pos.z, camMode: operatorCamMode, location: nearestEquipmentLabel(operatorObj.pos),
      elevation: operatorObj.pos.y, onPlatformNid: operatorObj.platform ? operatorObj.platform.nid : null, climbing: !!operatorObj.climb,
      inVehicle: !!operatorObj.inVehicle, destination: operatorDestinationLabel(operatorObj)
    };
  }
  /* The suspension flag used to only be checked at the periodic-tick call
     site below, not here — so a panel deliberately held off screen (the
     OPERABILITY CHECK report, the CALL OPERATOR "choose an action" panel)
     still got stomped by the ONE unconditional push at the end of
     setOperatorMode(true), the very first time Operator Mode actually
     turns on from a cold state (exactly what answering a call does).
     Checking it once here, centrally, covers every call site instead of
     requiring each one to remember to guard itself. */
  function pushOperatorPanel() { if (!operatorPanelSuspended && typeof A3.onOperatorPanel === 'function') A3.onOperatorPanel(operatorPanelInfo()); }
  function updateOperatorCamera() {
    // o.pos.y doubles as the operator's elevation above grade (0 at grade,
    // deck height while standing on a Phase 9 platform, or mid-climb) — the
    // ground-collision system never touches it, so it's safe to fold
    // straight into eye height here rather than track a second field.
    var o = operatorObj, eyeY = 1.62 + o.pos.y;
    if (operatorCamMode === 'first') {
      camera.position.set(o.pos.x, eyeY, o.pos.z);
      var cp = Math.cos(o.pitch);
      camera.lookAt(o.pos.x + Math.sin(o.yaw) * cp, eyeY + Math.sin(o.pitch), o.pos.z + Math.cos(o.yaw) * cp);
    } else {
      var d = operatorFollowDist;
      camera.position.set(o.pos.x - Math.sin(o.yaw) * d, 2.6 + o.pos.y + Math.max(0, o.pitch) * 3, o.pos.z - Math.cos(o.yaw) * d);
      camera.lookAt(o.pos.x, 1.4 + o.pos.y, o.pos.z);
    }
    if (scene && scene.fog) scene.fog.density = FOG_DENSITY_MIN;
  }
  /* Builds the leg queue for one climb (goingUp) or descent (!goingUp) and
     hands it to advanceClimb. Always three legs: step to the ladder foot
     (or deck edge), rise/fall along the rails, then step onto the deck (or
     away from the ladder foot back onto open grade) — same three-stage
     shape either direction, just reversed. */
  function startClimb(g, nid, ag, goingUp) {
    var o = operatorObj;
    var eqCenter = ag.bb.getCenter(new THREE.Vector3());
    // The ladder always sits just past the equipment's +X edge (see
    // accessGeometryFor), so facing "toward the equipment" from the ladder
    // is reliably close to -X — computed properly here instead of assumed,
    // in case a future ladder placement isn't purely +X anymore.
    var faceEquipYaw = Math.atan2(eqCenter.x - ag.cx, eqCenter.z - ag.cz);
    var climbDur = Math.max(1.8, ag.h * 1.3);
    var legs;
    if (goingUp) {
      legs = [
        { to: { x: ag.cx, y: 0, z: ag.cz }, dur: 0.5, faceYaw: faceEquipYaw },
        { to: { x: ag.cx, y: ag.h, z: ag.cz }, dur: climbDur, faceYaw: faceEquipYaw },
        { to: { x: ag.deckX, y: ag.h, z: ag.cz }, dur: 0.6, faceYaw: faceEquipYaw }
      ];
    } else {
      var awayYaw = faceEquipYaw + Math.PI;
      legs = [
        { to: { x: ag.cx, y: ag.h, z: ag.cz }, dur: 0.5, faceYaw: faceEquipYaw },
        { to: { x: ag.cx, y: 0, z: ag.cz }, dur: climbDur, faceYaw: faceEquipYaw },
        { to: { x: ag.cx + Math.sin(awayYaw) * 0.6, y: 0, z: ag.cz + Math.cos(awayYaw) * 0.6 }, dur: 0.5, faceYaw: awayYaw }
      ];
    }
    o.climb = {
      nid: nid, ag: ag, goingUp: goingUp,
      label: g.userData.props ? g.userData.props.Type : 'this equipment',
      legs: legs, li: 0, t: 0, from: { x: o.pos.x, y: o.pos.y, z: o.pos.z }
    };
    o.path = null; o.arriveEquip = null; o.climbTarget = null;
    clearRouteLine();
  }
  /* Advances a scripted climb/descend sequence — a short queue of straight
     legs (walk to the ladder base, rise/fall along the rails, step on/off
     the deck) driven purely by elapsed time, the same "engine computes, UI
     just renders" split as the rest of operator mode. Runs instead of the
     normal manual/path movement in updateOperatorFrame while o.climb is set. */
  function advanceClimb(o, dt) {
    var c = o.climb, leg = c.legs[c.li];
    c.t += dt;
    var tt = Math.min(1, leg.dur > 0 ? c.t / leg.dur : 1);
    o.pos.x = c.from.x + (leg.to.x - c.from.x) * tt;
    o.pos.y = c.from.y + (leg.to.y - c.from.y) * tt;
    o.pos.z = c.from.z + (leg.to.z - c.from.z) * tt;
    if (leg.faceYaw !== undefined) o.yaw = leg.faceYaw;
    if (tt >= 1) {
      c.from = { x: leg.to.x, y: leg.to.y, z: leg.to.z };
      c.li++; c.t = 0;
      if (c.li >= c.legs.length) {
        var label = c.label;
        if (c.goingUp) {
          var ag = c.ag;
          operatorObj.platform = {
            nid: c.nid,
            minX: ag.deckX - ag.deckD / 2 + 0.18, maxX: ag.deckX + ag.deckD / 2 - 0.18,
            minZ: ag.cz - ag.deckW / 2 + 0.18, maxZ: ag.cz + ag.deckW / 2 - 0.18
          };
          if (embStatus) embStatus('On the platform, ' + ag.h.toFixed(1) + 'm up — you can now see the top of ' + label + '. Click CLIMB DOWN to return to grade.');
        } else {
          operatorObj.platform = null;
          if (embStatus) embStatus('Back at grade, clear of the ladder.');
        }
        o.climb = null;
        // A queued action (e.g. WALK TO a different item, requested while
        // still up on a platform) resumes now that the descent it forced
        // is actually finished, instead of silently being clamped to the
        // deck it never left.
        if (o.afterClimb) { var fn = o.afterClimb; o.afterClimb = null; fn(); }
      }
    }
    o.figure.position.set(o.pos.x, o.pos.y, o.pos.z);
    o.figure.rotation.y = o.yaw;
    updateOperatorCamera();
    opPanelTick++;
    if (opPanelTick % 6 === 0 && !operatorPanelSuspended && !picked) pushOperatorPanel();
  }
  function updateOperatorFrame(dt) {
    var o = operatorObj;
    if (o.climb) { advanceClimb(o, dt); return; }
    var k = o.keys;
    var footSpeed = k.shift ? OPERATOR_SPEED_FAST : OPERATOR_SPEED;
    var speed = (o.inVehicle ? OPERATOR_SPEED_VEHICLE : footSpeed) * dt;
    var fwdX = Math.sin(o.yaw), fwdZ = Math.cos(o.yaw), rightX = Math.sin(o.yaw + Math.PI / 2), rightZ = Math.cos(o.yaw + Math.PI / 2);
    var mx = 0, mz = 0, manual = false;
    if (k.w) { mx += fwdX; mz += fwdZ; manual = true; }
    if (k.s) { mx -= fwdX; mz -= fwdZ; manual = true; }
    if (k.d) { mx += rightX; mz += rightZ; manual = true; }
    if (k.a) { mx -= rightX; mz -= rightZ; manual = true; }
    if (manual) {
      var len = Math.hypot(mx, mz);
      if (len > 1e-4) { tryMoveOperator(mx / len * speed, mz / len * speed); o.yaw = Math.atan2(mx, mz); }
      o.path = null;
      o.arriveEquip = null; o.showingEquipInfo = false; o.viewTopOnArrive = false; o.climbTarget = null; o.arriveLabel = null; o.boardOnArrive = false;
      clearRouteLine();
    } else if (o.path && o.path.length) {
      var wp = o.path[0];
      var dx = wp.x - o.pos.x, dz = wp.z - o.pos.z, dist = Math.hypot(dx, dz);
      var isFinalLeg = o.path.length === 1;
      if (dist < (isFinalLeg ? 0.15 : 0.45)) {
        o.path.shift();
        if (!o.path.length) {
          o.path = null;
          clearRouteLine();
          if (o.arriveEquip) {
            var af = o.arriveFace;
            var eqLabel = o.arriveEquip.userData.props ? o.arriveEquip.userData.props.Type : 'equipment';
            var adx = af.x - o.pos.x, adz = af.z - o.pos.z;
            o.yaw = Math.atan2(adx, adz);
            if (o.viewTopOnArrive) {
              var hDist = Math.max(0.5, Math.hypot(adx, adz));
              o.pitch = Math.max(-0.6, Math.min(0.9, Math.atan2(af.y - 1.62, hDist)));
              if (embStatus) embStatus('Looking up at the top of ' + eqLabel + ' (' + af.y.toFixed(1) + 'm) — switch to 1ST person for the clearest view.');
            } else if (embStatus) embStatus('Arrived at ' + eqLabel + '.');
            select(o.arriveEquip);
            o.arriveEquip = null; o.showingEquipInfo = true; o.viewTopOnArrive = false;
          } else if (o.climbTarget) {
            startClimb(o.climbTarget.g, o.climbTarget.nid, o.climbTarget.ag, true);
            o.climbTarget = null;
          } else if (o.boardOnArrive) {
            o.boardOnArrive = false;
            A3.boardVehicle();
            if (o.driveOnBoard) {
              var dob = o.driveOnBoard; o.driveOnBoard = null;
              A3.driveToPoint(dob.x, dob.z, dob.label);
            }
          } else if (o.arriveLabel) {
            if (embStatus) embStatus((o.inVehicle ? 'Arrived (by vehicle) at ' : 'Arrived at ') + o.arriveLabel + '.');
            o.arriveLabel = null;
          }
        } else {
          updateRouteLine();   // shorten the drawn route as each waypoint is reached
        }
      } else {
        var step = Math.min(dist, (o.inVehicle ? OPERATOR_SPEED_VEHICLE : OPERATOR_SPEED) * dt);
        var before = o.pos.x, beforeZ = o.pos.z;
        var moved = tryMoveOperator(dx / dist * step, dz / dist * step);
        o.yaw = Math.atan2(dx, dz);
        // The nav grid verifies the route is walkable at 2m resolution, but
        // a waypoint can still sit close enough to real geometry (a pipe
        // support, an equipment edge) that fine-grained continuous movement
        // clips it even though the coarse grid called that cell open —
        // this got a lot more likely once routes started hugging the
        // walkway network tightly (favouring cells right along real
        // structure, not just anywhere open). Previously any single stall
        // aborted the whole trip; now it skips just that one waypoint and
        // keeps heading for the next, only giving up if several in a row
        // fail — a real dead-end still gets caught, a single tight corner
        // no longer strands the operator standing still forever.
        if (!moved || (o.pos.x === before && o.pos.z === beforeZ)) {
          o.stallSkips = (o.stallSkips || 0) + 1;
          if (o.stallSkips <= 4 && o.path.length > 1) {
            o.path.shift();
            updateRouteLine();
          } else {
            o.path = null;
            clearRouteLine();
            if (embStatus) embStatus(o.arriveEquip ? ('NO VALID OPERATOR ACCESS ROUTE — can\'t reach ' + (o.arriveEquip.userData.props ? o.arriveEquip.userData.props.Type : 'that equipment') + ' from here.') : 'NO VALID OPERATOR ACCESS ROUTE — obstacle blocks the route.');
            o.arriveEquip = null; o.climbTarget = null; o.arriveLabel = null; o.boardOnArrive = false;
            o.stallSkips = 0;
          }
        } else {
          o.stallSkips = 0;
    ...
    if (o.inVehicle && vehicleMesh) {
      vehicleMesh.position.set(o.pos.x, o.pos.y, o.pos.z);
      vehicleMesh.rotation.y = o.yaw;
    } else {
      o.figure.position.set(o.pos.x, o.pos.y, o.pos.z);
      o.figure.rotation.y = o.yaw;
    }
    updateOperatorCamera();
    if (reachCheckVisible) updateReachEnvelope();
    opPanelTick++;
    // Suppressed for a few ticks right after arriving somewhere so the
    // equipment info panel select() just opened (below) stays on screen
    // instead of being clobbered by the next periodic location push —
    // cleared the moment the operator moves again (see `manual` above and
    // the click-to-walk handler in wireControls).
    if (opPanelTick % 6 === 0 && !o.showingEquipInfo && !operatorPanelSuspended && !picked) pushOperatorPanel();
  }
  // Lets the UI hold the OPERATOR MODE panel off screen while something
  // else (currently the OPERABILITY CHECK report) is deliberately showing
  // in its place — without this, the periodic push above stomps whatever
  // the UI just rendered there within a few frames of Operator Mode being
  // active, undoing a click almost as soon as it happens.
  var operatorPanelSuspended = false;
  A3.setOperatorPanelSuspended = function (v) { operatorPanelSuspended = !!v; };
  /* A short synthesized ring tone for CALL OPERATOR — no external audio
     asset needed (works offline, no licensing concerns to track). A
     two-tone "beep-beep, beep-beep" pattern via the Web Audio API.
     Every call is itself triggered by a click, so the autoplay-blocked-
     before-a-gesture case never actually applies here; wrapped in
     try/catch anyway since Web Audio isn't guaranteed everywhere. */
  var callAudioCtx = null;
  function playCallTone() {
    try {
      if (!callAudioCtx) callAudioCtx = new (window.AudioContext || window.webkitAudioContext)();
      var ctx = callAudioCtx;
      if (ctx.state === 'suspended') ctx.resume();
      var now = ctx.currentTime;
      [0, 0.34].forEach(function (offset) {
        [880, 1108].forEach(function (freq, i) {
          var osc = ctx.createOscillator(), gain = ctx.createGain();
          osc.type = 'sine'; osc.frequency.value = freq;
          var t0 = now + offset + i * 0.15;
    ...
    } catch (e) { /* audio unavailable — the call still works, just silently */ }
  }
  // Swaps which radio pose is visible on the CONTROLLABLE operator's own
  // figure — real radio discipline: the handset is clipped to the chest
  // at rest and only comes up to the ear while actually on a call.
  A3.setOperatorOnCall = function (on) {
    if (!operatorObj || !operatorObj.figure || !operatorObj.figure.userData.radioRig) return;
    var rig = operatorObj.figure.userData.radioRig;
    rig.onCall = !!on;
    rig.restRadio.visible = !on;
    rig.restArm.visible = !on;
    rig.onCallArm.visible = !!on;
  };
  var operatorCallTimer = null;
  /* CALL OPERATOR — radioing a field crew figure (the ambient "Field
     Operator" scenery meshes, not this avatar) raises the controllable
     operator's own handset to their ear for durationSec with a ring
     tone, then relaxes it back down automatically — the UI side (the
     CALL OPERATOR panel) follows this with its own choice of action once
     the "call" is answered. */
  A3.operatorTakeCall = function (durationSec) {
    if (!operatorMode) A3.setOperatorMode(true);
    if (!operatorObj) return false;
    playCallTone();
    A3.setOperatorOnCall(true);
    if (operatorCallTimer) clearTimeout(operatorCallTimer);
    operatorCallTimer = setTimeout(function () { A3.setOperatorOnCall(false); operatorCallTimer = null; }, (durationSec || 4) * 1000);
    return true;
  };
  // Real process equipment nearest a given world point — used by the CALL
  // OPERATOR panel to decide which item "climb to platform" means when
  // triggered from a field crew figure standing next to a specific unit,
  // rather than the equipment currently selected in the properties panel.
  function nearestEquipmentTo(x, z, requirePlatform) {
    var best = null, bestD = Infinity;
    objects.forEach(function (g) {
      var t = g.userData.props && g.userData.props.Type;
      if (!t || !g.userData.nid) return;
      if (requirePlatform && !platformRequiredFor(g)) return;
      var bb = new THREE.Box3().setFromObject(g);
      var c = bb.getCenter(new THREE.Vector3());
      var d = Math.hypot(c.x - x, c.z - z);
      if (d < bestD) { bestD = d; best = g; }
    });
    return best;
  }
  A3.nearestEquipmentNid = function (x, z, requirePlatform) { var g = nearestEquipmentTo(x, z, requirePlatform); return g ? g.userData.nid : null; };
  // The equipment's own Type name, e.g. "Storage Tank" or "Centrifugal
  // Pump" — what a field crew member would actually say over the radio
  // ("calling from the storage tank"), not its internal nid.
  A3.equipmentLabel = function (nid) {
    var g = nid ? groupByNid(nid) : null;
    return (g && g.userData.props && g.userData.props.Type) || 'the field';
  };
  var MONITOR_REPORT_PHRASES = [
    'level nominal, no visible leaks, area clear',
    'temperature and pressure within normal operating range',
    'no abnormal vibration or noise observed',
    'insulation and cladding intact, no hot spots noted',
    'valve line-up confirmed correct',
    'no unusual odour — area safe to continue routine rounds'
  ];
  /* SEND MONITOR REPORT — a lightweight "the field crew radioed in a
     status" flourish for the CALL OPERATOR panel: not a real instrument
     reading (there's no live sensor model here), just a plausible routine
     round-check phrase logged to the status bar against whichever
     equipment the call was actually about. */
  A3.sendMonitorReport = function (nid) {
    var g = nid ? groupByNid(nid) : null;
    var label = (g && g.userData.props && g.userData.props.Type) || 'the area';
    var phrase = MONITOR_REPORT_PHRASES[Math.floor(Math.random() * MONITOR_REPORT_PHRASES.length)];
    var line = 'MONITOR REPORT — ' + label + ': ' + phrase + '.';
    if (embStatus) embStatus(line);
    return line;
  };
  A3.setOperatorMode = function (on) {
    on = !!on;
    if (on === operatorMode) return;
    operatorMode = on;
    operatorPanelSuspended = false;   // never carry a stale hold across an Operator Mode toggle
    if (on) {
      if (!operatorObj) {
        var fig = personFigure(1);
        fig.userData.props = { Type: 'Operator (You)' };
        scene.add(fig);
        operatorObj = { figure: fig, pos: new THREE.Vector3(), yaw: Math.PI, pitch: 0.15, path: null, keys: {}, arriveEquip: null, arriveFace: null, arriveLabel: null, showingEquipInfo: false, viewTopOnArrive: false, platform: null, climb: null, climbTarget: null, afterClimb: null, inVehicle: false, boardOnArrive: false, driveOnBoard: null, onCall: false };
      }
      refreshOperatorNav();
      operatorObj.pos.copy(findOperatorSpawn());
      operatorSpawn.copy(operatorObj.pos);
      operatorObj.path = null; operatorObj.keys = {}; operatorObj.arriveEquip = null; operatorObj.arriveLabel = null; operatorObj.showingEquipInfo = false;
      operatorObj.platform = null; operatorObj.climb = null; operatorObj.climbTarget = null; operatorObj.afterClimb = null; operatorObj.boardOnArrive = false;
      if (operatorObj.inVehicle) A3.exitVehicle();
      operatorObj.inVehicle = false;
      A3.setOperatorOnCall(false);
      operatorObj.figure.visible = true;
      operatorObj.figure.position.set(operatorObj.pos.x, 0, operatorObj.pos.z);
      ORBIT_DRAGGING = false; opLookDrag = null;
      clearRouteLine();
      if (walkwayVisible) buildWalkwayNetwork();
      select(null);
    } else {
      if (operatorObj) operatorObj.figure.visible = false;
      if (reachEnvelopeMesh) reachEnvelopeMesh.visible = false;
      clearRouteLine();
      updateCamera();
    }
    pushOperatorPanel();
    if (typeof A3.onOperatorModeChange === 'function') A3.onOperatorModeChange(on);
  };
  A3.getOperatorMode = function () { return operatorMode; };
  A3.platformRequiredForNid = function (nid) { var g = groupByNid(nid); return !!(g && platformRequiredFor(g)); };
  A3.operatorOnPlatformNid = function () { return (operatorObj && operatorObj.platform) ? operatorObj.platform.nid : null; };
  A3.setOperatorCamMode = function (m) { operatorCamMode = (m === 'first') ? 'first' : 'third'; if (operatorMode) updateOperatorCamera(); };
  A3.resetOperator = function () {
    if (!operatorObj) return;
    if (operatorObj.inVehicle) A3.exitVehicle();
    A3.setOperatorOnCall(false);
    operatorObj.pos.copy(operatorSpawn); operatorObj.path = null; operatorObj.arriveEquip = null; operatorObj.showingEquipInfo = false; operatorObj.viewTopOnArrive = false; operatorObj.platform = null; operatorObj.climb = null; operatorObj.climbTarget = null; operatorObj.afterClimb = null; operatorObj.boardOnArrive = false; operatorObj.arriveLabel = null;
    clearRouteLine(); pushOperatorPanel();
  };
  /* If the operator is currently standing on a Phase 9 platform deck, any
     new grade-level destination (WALK TO / VIEW TOP / a different CLIMB)
     has to climb them back down first — tryMoveOperator clamps ALL
     movement to the deck footprint while operatorObj.platform is set, so
     queuing the new action straight into o.path would just pin them to
     the deck rather than actually going anywhere. Returns true if a
     descent was queued (caller should stop and wait for it), else false. */
  function descendIfOnPlatformThen(afterFn) {
    var o = operatorObj;
    if (!o || !o.platform) return false;
    var pg = groupByNid(o.platform.nid);
    var ag = pg ? accessGeometryFor(pg) : null;
    if (!pg || !ag) { o.platform = null; return false; }
    startClimb(pg, o.platform.nid, ag, false);
    o.afterClimb = afterFn || null;
    if (embStatus) embStatus('Climbing down first…');
    return true;
  }
  A3.debugOperator = function () { return operatorObj ? { x: operatorObj.pos.x, y: operatorObj.pos.y, z: operatorObj.pos.z, yaw: operatorObj.yaw, path: operatorObj.path, climbing: !!operatorObj.climb, platform: operatorObj.platform ? operatorObj.platform.nid : null, circles: operatorColliders.circles.length, segs: operatorColliders.segs.length, gridCells: walkGrid ? walkGrid.cols * walkGrid.rows : 0 } : null; };
  /* ── PHASE 3: "walk to equipment", now routed via Phase 4's pathfinder ──
     Generalised inspection-point logic: rather than the spec's fully
     enumerated per-nozzle table (OP-1 suction, OP-2 discharge, …), this
     computes ONE safe approach point per equipment on demand (see
     computeApproachPoint above) — just outside its own footprint circle,
     tried first on the side closest to the operator's current position,
     then stepped around the equipment until an unblocked point is found.
     A full per-nozzle InspectionPoint list (valve/instrument/manway/motor
     granularity) is a natural follow-on once this basic approach-and-face
     flow is confirmed to be what's wanted. The route there is now a real
     A* path (findPath), not a straight line — Phase 2's straight-line
     "walk until blocked, then give up" is only what's left for manual
     WASD movement, which has no destination to route towards. */
  A3.walkToEquipment = function (nid) {
    var g = nid ? groupByNid(nid) : picked;
    if (!g) return false;
    if (!operatorMode) A3.setOperatorMode(true);
    if (!operatorObj) return false;
    if (operatorObj.climb) { if (embStatus) embStatus('Still on the ladder — wait for it to finish.'); return false; }
    if (operatorObj.inVehicle) A3.exitVehicle();
    if (descendIfOnPlatformThen(function () { A3.walkToEquipment(nid); })) return true;
    var label = g.userData.props ? g.userData.props.Type : 'this equipment';
    var dest = computeApproachPoint(g, operatorObj.pos.x, operatorObj.pos.z);
    if (!dest) { if (embStatus) embStatus('NO VALID OPERATOR ACCESS ROUTE — ' + label + ' has no clear approach.'); return false; }
    var path = findPath(operatorObj.pos.x, operatorObj.pos.z, dest.x, dest.z);
    if (!path) { if (embStatus) embStatus('NO VALID OPERATOR ACCESS ROUTE — no walkable path to ' + label + '.'); return false; }
    operatorObj.path = path;
    operatorObj.arriveFace = dest.center.clone();
    operatorObj.arriveEquip = g;
    operatorObj.climbTarget = null; operatorObj.arriveLabel = null; operatorObj.boardOnArrive = false;
    operatorObj.viewTopOnArrive = false;
    operatorObj.showingEquipInfo = false;
    updateRouteLine();
    if (embStatus) embStatus('Walking to ' + label + '…');
    return true;
  };
  /* "VIEW TOP" — see the top of a piece of equipment (manway, instruments,
     davit) without a full climb-the-ladder simulation, which would need
     the operator to stand at platform height — a further step beyond what
     Phase 2's grade-only collision model supports today. Outside Operator
     Mode this just reframes the orbit camera close on the top, same
     pattern as LOCATE IN 3D. In Operator Mode it walks the operator to the
     equipment (reusing the exact same approach point + pathfinder as
     WALK TO EQUIPMENT) and then tilts the view up at the top from grade —
     genuinely useful for sighting a manway or an overhead nozzle, honestly
     short of physically standing on the platform next to it. */
  A3.viewEquipmentTop = function (nid) {
    var g = nid ? groupByNid(nid) : picked;
    if (!g) return false;
    var bb = new THREE.Box3().setFromObject(g);
    var sz = bb.getSize(new THREE.Vector3());
    var top = new THREE.Vector3((bb.min.x + bb.max.x) / 2, bb.max.y, (bb.min.z + bb.max.z) / 2);
    var label = g.userData.props ? g.userData.props.Type : 'this equipment';
    if (!operatorMode) {
      select(g);
      sph.tx = top.x; sph.ty = top.y - Math.min(1.5, sz.y * 0.15); sph.tz = top.z;
      sph.r = Math.max(4, Math.min(40, Math.max(sz.x, sz.z) * 2.2));
      sph.phi = 1.15;
      updateCamera();
      if (embStatus) embStatus('Framed the top of ' + label + '.');
      return true;
    }
    if (!operatorObj) return false;
    if (operatorObj.climb) { if (embStatus) embStatus('Still on the ladder — wait for it to finish.'); return false; }
    if (operatorObj.inVehicle) A3.exitVehicle();
    if (descendIfOnPlatformThen(function () { A3.viewEquipmentTop(nid); })) return true;
    var lookUp = function () {
      var dx = top.x - operatorObj.pos.x, dz = top.z - operatorObj.pos.z, hDist = Math.max(0.5, Math.hypot(dx, dz));
      operatorObj.yaw = Math.atan2(dx, dz);
      operatorObj.pitch = Math.max(-0.6, Math.min(0.9, Math.atan2(top.y - 1.62, hDist)));
      select(g);
      operatorObj.showingEquipInfo = true;
      if (embStatus) embStatus('Looking up at the top of ' + label + ' (' + top.y.toFixed(1) + 'm) — switch to 1ST person for the clearest view.');
    };
    var dest = computeApproachPoint(g, operatorObj.pos.x, operatorObj.pos.z);
    if (!dest) { if (embStatus) embStatus('NO VALID OPERATOR ACCESS ROUTE — ' + label + ' has no clear approach.'); return false; }
    if (Math.hypot(operatorObj.pos.x - dest.x, operatorObj.pos.z - dest.z) < 0.3) { lookUp(); return true; }
    var path = findPath(operatorObj.pos.x, operatorObj.pos.z, dest.x, dest.z);
    if (!path) { if (embStatus) embStatus('NO VALID OPERATOR ACCESS ROUTE — no walkable path to ' + label + '.'); return false; }
    operatorObj.path = path;
    operatorObj.arriveEquip = g;
    operatorObj.arriveFace = top.clone();
    operatorObj.climbTarget = null; operatorObj.arriveLabel = null; operatorObj.boardOnArrive = false;
    operatorObj.viewTopOnArrive = true;
    operatorObj.showingEquipInfo = false;
    updateRouteLine();
    if (embStatus) embStatus('Walking to ' + label + ' to look at the top…');
    return true;
  };
  /* CLIMB TO PLATFORM — the actual physical climb VIEW TOP explicitly falls
     short of: walks the operator to the ladder foot (reusing the same
     approach+pathfinder as WALK TO EQUIPMENT, biased toward the ladder's
     own side of the equipment rather than wherever the operator happens to
     be standing), then a scripted animation carries them up the rails onto
     the real deck built by buildAccessStructureFor — o.pos.y becomes the
     platform height, so ordinary first/third-person camera and the figure
     mesh both follow it with no separate "elevated" code path. Only valid
     Outside Operator Mode is meaningless (there's no operator to climb),
     so this always turns Operator Mode on first, same as WALK TO EQUIPMENT.
     Calling it again while already on that equipment's platform climbs
     back down instead of doing nothing — the single button doubles as
     CLIMB UP / CLIMB DOWN depending on where the operator already is. */
  A3.climbToPlatform = function (nid) {
    var g = nid ? groupByNid(nid) : picked;
    if (!g) return false;
    if (!operatorMode) A3.setOperatorMode(true);
    if (!operatorObj) return false;
    ...
    if (operatorObj.inVehicle) A3.exitVehicle();
    var label = g.userData.props ? g.userData.props.Type : 'this equipment';
    var targetNid = g.userData.nid;
    if (operatorObj.platform && operatorObj.platform.nid === targetNid) {
      var agDown = accessGeometryFor(g);
      if (!agDown) { operatorObj.platform = null; return false; }
      startClimb(g, targetNid, agDown, false);
      if (embStatus) embStatus('Climbing down from ' + label + '…');
      return true;
    }
    if (descendIfOnPlatformThen(function () { A3.climbToPlatform(nid); })) return true;
    var ag = accessGeometryFor(g);
    if (!ag) { if (embStatus) embStatus(label + ' has no platform to climb — only tall vertical vessels get one (see the operability check\'s "platform req." flag).'); return false; }
    // Bias the approach toward the ladder's own side of the equipment
    // (accessGeometryFor always places it just past +X) rather than
    // whichever side the operator happens to be standing on right now.
    var dest = computeApproachPoint(g, ag.cx, ag.cz);
    if (!dest) { if (embStatus) embStatus('NO VALID OPERATOR ACCESS ROUTE — ' + label + ' has no clear approach to its ladder.'); return false; }
    var path = findPath(operatorObj.pos.x, operatorObj.pos.z, dest.x, dest.z);
    if (!path) { if (embStatus) embStatus('NO VALID OPERATOR ACCESS ROUTE — no walkable path to the ladder at ' + label + '.'); return false; }
    operatorObj.path = path;
    operatorObj.arriveFace = dest.center.clone();
    operatorObj.arriveEquip = null;
    operatorObj.arriveLabel = null; operatorObj.boardOnArrive = false;
    operatorObj.viewTopOnArrive = false;
    operatorObj.showingEquipInfo = false;
    operatorObj.climbTarget = { g: g, nid: targetNid, ag: ag };
    updateRouteLine();
    if (embStatus) embStatus('Walking to the ladder at ' + label + '…');
    return true;
  };
  /* SITE VEHICLE — board it near where it's parked (VEHICLE_BOARD_RADIUS)
     for OPERATOR_SPEED_VEHICLE travel instead of walking speed. Only one
     vehicle mesh exists; "boarding" hides the walking figure and starts
     moving the vehicle mesh itself with the operator's position each
     frame (see the o.inVehicle branch in updateOperatorFrame), and
     "exiting" parks it back at its original spot and shows the figure
     again wherever the operator got out. */
  A3.boardVehicle = function () {
    if (!operatorMode) A3.setOperatorMode(true);
    if (!operatorObj) return false;
    if (operatorObj.inVehicle) return true;
    if (operatorObj.climb) { if (embStatus) embStatus('Still on the ladder — wait for it to finish.'); return false; }
    if (!vehicleMesh || !vehiclePos) { if (embStatus) embStatus('No vehicle on this flowsheet.'); return false; }
    if (descendIfOnPlatformThen(function () { A3.boardVehicle(); })) return true;
    var d = Math.hypot(operatorObj.pos.x - vehiclePos.x, operatorObj.pos.z - vehiclePos.z);
    if (d <= VEHICLE_BOARD_RADIUS) {
      operatorObj.inVehicle = true;
      operatorObj.boardOnArrive = false;
      operatorObj.figure.visible = false;
      vehicleMesh.position.set(operatorObj.pos.x, operatorObj.pos.y, operatorObj.pos.z);
      vehicleMesh.rotation.y = operatorObj.yaw;
      if (embStatus) embStatus('Boarded the vehicle — ' + OPERATOR_SPEED_VEHICLE + ' m/s. Click the ground or WALK TO / CLIMB an item to exit and continue on foot.');
      return true;
    }
    // Too far to board directly — walk there first (staying on the
    // walkway network like any other trip) and board automatically the
    // moment the operator actually arrives, instead of just printing
    // "walk there first" and leaving the button looking like it did
    // nothing. The vehicle is real collision geometry (see
    // buildOperatorColliders — that's deliberate, so you can't walk
    // through a parked jeep), so vehiclePos itself sits inside solid
    // footprint and is never actually reachable; route to a standoff
    // point just outside it instead, the same way equipment is approached.
    var vDest = computeApproachPoint(vehicleMesh, operatorObj.pos.x, operatorObj.pos.z);
    if (!vDest) { if (embStatus) embStatus('NO VALID OPERATOR ACCESS ROUTE — the vehicle has no clear approach.'); return false; }
    var path = findPath(operatorObj.pos.x, operatorObj.pos.z, vDest.x, vDest.z);
    if (!path) { if (embStatus) embStatus('NO VALID OPERATOR ACCESS ROUTE — no walkable path to the vehicle.'); return false; }
    operatorObj.path = path;
    operatorObj.arriveEquip = null; operatorObj.climbTarget = null; operatorObj.arriveLabel = null; operatorObj.viewTopOnArrive = false; operatorObj.showingEquipInfo = false;
    operatorObj.boardOnArrive = true;
    updateRouteLine();
    if (embStatus) embStatus('Walking to the vehicle (' + d.toFixed(0) + 'm away) — boarding on arrival…');
    return true;
  };
  A3.exitVehicle = function () {
    if (!operatorObj || !operatorObj.inVehicle) return false;
    operatorObj.inVehicle = false;
    operatorObj.figure.visible = operatorMode;
    operatorObj.figure.position.set(operatorObj.pos.x, operatorObj.pos.y, operatorObj.pos.z);
    if (vehicleMesh && vehiclePos) { vehicleMesh.position.set(vehiclePos.x, 0, vehiclePos.z); vehicleMesh.rotation.y = 0; }
    if (embStatus) embStatus('Exited the vehicle on foot.');
    return true;
  };
  A3.isOperatorInVehicle = function () { return !!(operatorObj && operatorObj.inVehicle); };
  /* DRIVE TO POINT — the CALL OPERATOR panel's "respond by vehicle" option:
     board the site vehicle (walking to it first if not already beside it,
     same as BOARD VEHICLE) and drive straight to a given world point along
     the real A* route, exactly like every other dispatch here — never a
     straight line through solid equipment. Kept separate from WALK TO
     EQUIPMENT/CLIMB TO PLATFORM (which always force a foot dispatch, by
     design) rather than adding a mode flag to those, since driving all the
     way to a nozzle or a ladder foot isn't a real thing — the crew parks
     and walks the last stretch, which those two already do correctly. */
  A3.driveToPoint = function (x, z, label) {
    label = label || 'the location';
    if (!operatorMode) A3.setOperatorMode(true);
    if (!operatorObj) return false;
    if (operatorObj.climb) { if (embStatus) embStatus('Still on the ladder — wait for it to finish.'); return false; }
    if (descendIfOnPlatformThen(function () { A3.driveToPoint(x, z, label); })) return true;
    if (!vehicleMesh || !vehiclePos) { if (embStatus) embStatus('No vehicle on this flowsheet.'); return false; }
    var dv = Math.hypot(operatorObj.pos.x - vehiclePos.x, operatorObj.pos.z - vehiclePos.z);
    if (operatorObj.inVehicle || dv <= VEHICLE_BOARD_RADIUS) {
      operatorObj.inVehicle = true;
      operatorObj.figure.visible = false;
      var path = findPath(operatorObj.pos.x, operatorObj.pos.z, x, z);
      if (!path) { if (embStatus) embStatus('NO VALID VEHICLE ROUTE — no drivable path to ' + label + '.'); return false; }
      operatorObj.path = path;
      operatorObj.arriveEquip = null; operatorObj.climbTarget = null; operatorObj.viewTopOnArrive = false;
      operatorObj.showingEquipInfo = false; operatorObj.boardOnArrive = false; operatorObj.driveOnBoard = null;
      operatorObj.arriveLabel = label;
      updateRouteLine();
      if (embStatus) embStatus('Driving to ' + label + '…');
      return true;
    }
    // Not next to the vehicle yet — walk there, board, then continue
    // driving on to the real destination (see the o.driveOnBoard pickup
    // in updateOperatorFrame's arrival handling, right after A3.boardVehicle()).
    var vDest = computeApproachPoint(vehicleMesh, operatorObj.pos.x, operatorObj.pos.z);
    if (!vDest) { if (embStatus) embStatus('NO VALID OPERATOR ACCESS ROUTE — the vehicle has no clear approach.'); return false; }
    var vPath = findPath(operatorObj.pos.x, operatorObj.pos.z, vDest.x, vDest.z);
    if (!vPath) { if (embStatus) embStatus('NO VALID OPERATOR ACCESS ROUTE — no walkable path to the vehicle.'); return false; }
    operatorObj.path = vPath;
    operatorObj.arriveEquip = null; operatorObj.climbTarget = null; operatorObj.arriveLabel = null;
    operatorObj.viewTopOnArrive = false; operatorObj.showingEquipInfo = false;
    operatorObj.boardOnArrive = true;
    operatorObj.driveOnBoard = { x: x, z: z, label: label };
    updateRouteLine();
    if (embStatus) embStatus('Walking to the vehicle (' + dv.toFixed(0) + 'm away) — will drive on to ' + label + ' once aboard…');
    return true;
  };
  /* ASSEMBLY POINT — a fixed muster location, walkable (or drivable) to
     from the ASSEMBLY toolbar button or the CALL OPERATOR panel. Doesn't
     select/inspect anything on arrival, unlike WALK TO EQUIPMENT — it's a
     place, not a piece of process equipment. */
  A3.assemblyPointPos = function () { return assemblyPointPos ? { x: assemblyPointPos.x, z: assemblyPointPos.z } : null; };
  A3.goToAssemblyPoint = function () {
    if (!assemblyPointPos) { if (embStatus) embStatus('No assembly point on this flowsheet.'); return false; }
    if (!operatorMode) A3.setOperatorMode(true);
    if (!operatorObj) return false;
    if (operatorObj.climb) { if (embStatus) embStatus('Still on the ladder — wait for it to finish.'); return false; }
    if (descendIfOnPlatformThen(function () { A3.goToAssemblyPoint(); })) return true;
    var path = findPath(operatorObj.pos.x, operatorObj.pos.z, assemblyPointPos.x, assemblyPointPos.z);
    if (!path) { if (embStatus) embStatus('NO VALID OPERATOR ACCESS ROUTE — no walkable path to the assembly point.'); return false; }
    operatorObj.path = path;
    operatorObj.arriveEquip = null; operatorObj.climbTarget = null; operatorObj.viewTopOnArrive = false; operatorObj.showingEquipInfo = false; operatorObj.boardOnArrive = false;
    operatorObj.arriveLabel = 'the assembly point';
    updateRouteLine();
    if (embStatus) embStatus((operatorObj.inVehicle ? 'Driving' : 'Walking') + ' to the assembly point…');
    return true;
  };
  ...
  function wireControls(canvas) {
    ...
    canvas.addEventListener('mousedown', function (e) {
      if (operatorMode) { if (e.button === 0) opLookDrag = { x: e.clientX, y: e.clientY, yaw: operatorObj.yaw, pitch: operatorObj.pitch }; return; }
    ...
    window.addEventListener('mousemove', function (e) {
      if (operatorMode) {
        if (!opLookDrag) return;
        var odx = e.clientX - opLookDrag.x, ody = e.clientY - opLookDrag.y;
        operatorObj.yaw = opLookDrag.yaw - odx * OPERATOR_LOOK_SENS;
        operatorObj.pitch = Math.max(-0.6, Math.min(0.6, opLookDrag.pitch - ody * OPERATOR_LOOK_SENS));
        return;
      }
    ...
    window.addEventListener('mouseup', function (e) {
      if (operatorMode) { opLookDrag = null; return; }
      if (dragConn) {
    ...
    canvas.addEventListener('wheel', function (e) {
      e.preventDefault();
      if (operatorMode) { operatorFollowDist = Math.max(1.8, Math.min(14, operatorFollowDist * (e.deltaY < 0 ? 0.9 : 1.1))); return; }
      // Floor was 1.2 world units (camera near-plane is 0.1, so plenty of
      // room below that) — that capped the scale-bar zoom readout at
      // 1333% (16/1.2), too coarse to actually inspect small parts (bolts,
      // gauges, instrument tags) up close. Lowered to let the readout go
      // well past 6000% before the near plane becomes a real concern.
      sph.r = Math.max(0.25, Math.min(2200, sph.r * (e.deltaY < 0 ? 0.9 : 1.1))); updateCamera();
    }, { passive: false });
    canvas.addEventListener('click', function (e) {
      var r = canvas.getBoundingClientRect(); m2.x = ((e.clientX - r.left) / r.width) * 2 - 1; m2.y = -((e.clientY - r.top) / r.height) * 2 + 1;
      ray.setFromCamera(m2, camera);
      if (operatorMode) {
        if (operatorObj.climb) return;   // scripted climb/descend in progress — ignore clicks until it finishes
        // A click that actually lands on something (a field crew figure to
        // radio, the site vehicle to board, or a piece of equipment to
        // inspect) selects it, same as ordinary orbit mode — previously
        // EVERY click in Operator Mode was read as "walk to this ground
        // point" before objects were even considered, so nothing was ever
        // clickable while walking around. Only an empty-ground click still
        // means "walk here".
        var opHit = ray.intersectObjects(objects, true);
        if (opHit.length) {
          var oo = opHit[0].object;
          while (oo.parent && objects.indexOf(oo) < 0) oo = oo.parent;
          if (oo !== operatorObj.figure) { select(oo); return; }
        }
        var groundPt = new THREE.Vector3();
        if (ray.ray.intersectPlane(GROUND_PLANE, groundPt)) {
          var gx = groundPt.x, gz = groundPt.z;
          if (descendIfOnPlatformThen(function () {
            var p2 = findPath(operatorObj.pos.x, operatorObj.pos.z, gx, gz);
            if (p2) { operatorObj.path = p2; operatorObj.arriveEquip = null; operatorObj.climbTarget = null; operatorObj.arriveLabel = null; operatorObj.boardOnArrive = false; operatorObj.showingEquipInfo = false; operatorObj.viewTopOnArrive = false; updateRouteLine(); }
          })) return;
          var clickPath = findPath(operatorObj.pos.x, operatorObj.pos.z, gx, gz);
          if (!clickPath) { if (embStatus) embStatus('NO VALID OPERATOR ACCESS ROUTE — no walkable path to that point.'); }
          else {
            operatorObj.path = clickPath; operatorObj.arriveEquip = null; operatorObj.climbTarget = null; operatorObj.arriveLabel = null; operatorObj.boardOnArrive = false; operatorObj.showingEquipInfo = false; operatorObj.viewTopOnArrive = false;
            updateRouteLine();
          }
        }
        return;
    ...
    window.addEventListener('keydown', function (e) {
      if (!operatorMode || !operatorObj) return;
      var tag = e.target && e.target.tagName;
      if (tag === 'INPUT' || tag === 'TEXTAREA' || tag === 'SELECT') return;
      var k = e.key.toLowerCase();
      if (k === 'w' || k === 'a' || k === 's' || k === 'd') { operatorObj.keys[k] = true; e.preventDefault(); }
      else if (k === 'shift') { operatorObj.keys.shift = true; }
      else if (k === 'r') { A3.resetOperator(); }
      else if (k === 'escape') { A3.setOperatorMode(false); }
    });
    window.addEventListener('keyup', function (e) {
      if (!operatorObj) return;
      var k = e.key.toLowerCase();
      if (k === 'w' || k === 'a' || k === 's' || k === 'd') operatorObj.keys[k] = false;
      else if (k === 'shift') operatorObj.keys.shift = false;
    });
  }
  ...
  function animate() {
    ...
    // Real elapsed time, not an assumed 60fps — the operator's walking
    // speed is specified in metres/second, so on a slower device (or this
    // sandbox's software-rendered WebGL) a fixed per-frame step would make
    // walking crawl at whatever framerate happens to render, instead of
    // covering the right distance per real second. Capped so a
    // backgrounded/stalled tab doesn't teleport the operator on resume.
    var nowMs = (typeof performance !== 'undefined' ? performance.now() : Date.now());
    var dtReal = lastFrameMs != null ? Math.min(0.1, Math.max(0, (nowMs - lastFrameMs) / 1000)) : 0.016;
    lastFrameMs = nowMs;
    if (operatorMode && operatorObj) { updateOperatorFrame(dtReal); }
    else if (AUTO_ROTATE && !ORBIT_DRAGGING) { sph.theta += 0.006; updateCamera(); }
  ...
  function close() { if (raf) cancelAnimationFrame(raf); clearSelBox(); if (host) host.remove(); host = null; renderer && renderer.dispose && renderer.dispose(); renderer = null; objects = []; contactShadows = []; jacketMeshes = []; gapMarkers = []; spinList = []; picked = null; flowTubes = []; vizHalos = []; flowVizMode = 'off'; foundations = []; walkwayMeshes = []; walkGrid = null; routeLine = null; ladderMeshes = []; maintZoneMeshes = []; safetyZoneMeshes = []; maintConflicts = []; reachEnvelopeMesh = null; }
  ...
  /* ══════════════════════════════════════════════════════════════════════
     ...
     buildFromModel(), or touch any 2D/toolbar/calculation code. It

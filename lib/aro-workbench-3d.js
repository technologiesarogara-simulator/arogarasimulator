...
(function () {
  ...
  function buildPipe(fromPort, toPort, color, nps, ltype) {
    ...
    grp.userData.pipe = { from: fromPort, to: toPort, fromNid: fgEarly.userData.nid, toNid: tgEarly.userData.nid, mat: mat, emat: emat, color: col, nps: nps || 3, ltype: ltype,
      ...
      pts: pts.map(function (p) { return p.clone(); }), rEqA: rEqA, rEqB: rEqB, pipeR: r };
  ...
  A3.pipeJointReport = function (pid) {
    ...
    var radius = {
      pipeR: pd.pipeR,
      sourceFlangeR: pd.rEqA, sourceMismatch: Math.abs(pd.rEqA - pd.pipeR),
      targetFlangeR: pd.rEqB, targetMismatch: Math.abs(pd.rEqB - pd.pipeR)
    };
    console.log('JOINT REPORT: ' + pid
      ...
      + '\n  source flange: ' + radius.sourceFlangeR.toFixed(4) + ' (mismatch ' + radius.sourceMismatch.toFixed(4) + ')'
      + '\n  target flange: ' + radius.targetFlangeR.toFixed(4) + ' (mismatch ' + radius.targetMismatch.toFixed(4) + ')');

...
(function () {
  ...
  A3.setFlowVizMode = function (mode) {
    ...
    if (mode === 'velocity' || mode === 'pressure') {
      ...
      var bands = mode === 'velocity' ? VEL_BANDS : DP_BANDS;
      pipes3d.forEach(function (grp) {
        var pd = grp.userData.pipe;
        if (!pd) return;
        var val = pd.result && pd.result[field];
        if (pd.result) withResult++;
        var col = bandColor(bands, val);
      ...
      objects.forEach(function (g) {
        ...
        if (avg == null) return;
        equipmentHaloFor(g, bandColor(bands, avg));
      });
    } else if (mode === 'flow') {
      pipes3d.forEach(function (grp) {
  ...
  // Legend content for the 2D toolbar — a single source of truth so the
  // colour bands never drift out of sync between the two files.
  A3.getVizBands = function () {
    return {
      velocity: VEL_BANDS.map(function (b) { return { c: '#' + b.c.toString(16).padStart(6, '0'), label: b.label }; }),
      pressure: DP_BANDS.map(function (b) { return { c: '#' + b.c.toString(16).padStart(6, '0'), label: b.label }; }),
      notCalc: '#' + NOT_CALC_COLOR.toString(16).padStart(6, '0')
    };
  };

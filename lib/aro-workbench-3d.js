...
(function () {
  ...
  A3.buildFromModel = function (nodes, pipes, nameOf) {
    ...
    /* PHASE 9 (detection half — see the module comment by
       platformRequiredFor): an access ladder on every vertical
       vessel/column/tank tall enough that its top isn't reachable from
       grade. Always-on scenery like the foundations above, not a
       toggleable overlay. */
    objects.slice().forEach(function (g) {
      if (!platformRequiredFor(g)) return;
      var ladder = buildLadderFor(g);
      if (ladder) { scene.add(ladder); ladderMeshes.push(ladder); }
    });
    rebuilding = false;
    if (objects.length) {
  ...
  A3.clearAll = function () { flowTubes.forEach(function (f) { if (f.mesh.parent) f.mesh.parent.remove(f.mesh); if (f.mesh.geometry) f.mesh.geometry.dispose(); if (f.mesh.material) f.mesh.material.dispose(); }); flowTubes = []; dimmedEquip = []; vizHalos.forEach(function (h) { if (h.parent) h.parent.remove(h); if (h.geometry) h.geometry.dispose(); if (h.material) h.material.dispose(); }); vizHalos = []; objects.forEach(function (o) { scene.remove(o); }); pipes3d.forEach(function (o) { scene.remove(o); }); foundations.forEach(function (f) { scene.remove(f); f.traverse(function (o) { if (o.geometry) o.geometry.dispose(); }); }); foundations = []; ladderMeshes.forEach(function (f) { scene.remove(f); f.traverse(function (o) { if (o.geometry) o.geometry.dispose(); }); }); ladderMeshes = []; clearWalkwayMeshes(); clearRouteLine(); clearMaintZoneMeshes(); clearSafetyZoneMeshes(); maintConflicts = []; walkGrid = null; clearContactShadows(); objects = []; pipes3d = []; jacketMeshes = []; portMeshes = []; spinList = []; picked = null; pickedPipe = null; pendingPort = null; portDebug = []; gapMarkers = []; connHighlighted = []; };
  ...
  function maintenanceZonesFor(g) {
    ...
    if (cls === 'ROTATING') {
      ...
      zones.push({ kind: 'MOTOR / COUPLING REMOVAL', requiredClearance: pullLen, box: box });
    } else if (cls === 'HX') {
      ...
      zones.push({ kind: 'TUBE BUNDLE PULL', requiredClearance: tubeLen, box: box2 });
    } else if (cls === 'VESSEL') {
      var accessW = 1.2;
      var box3 = { cx: bb.max.x + accessW / 2, cz: c.z, hx: accessW / 2, hz: sz.z / 2 + 0.4, cy: c.y, hy: sz.y / 2 };
      zones.push({ kind: 'MANWAY / LADDER ACCESS', requiredClearance: accessW, box: box3 });
    }
    return zones;
  }
  ...
  /* ============================================================
     ...
     Per the spec: "Do NOT invent regulatory clearance numbers." These
     radii are placeholder, configurable, project-defined values (see
     SAFETY_RADIUS below) reusing the same ROTATING/HX/VESSEL buckets
     Phase 5 already classifies equipment into — NOT a stand-in for a
     real hazard study, and every zone's own info panel says so.
     ============================================================ */
  var SAFETY_MAT = null, safetyZoneMeshes = [], safetyVisible = false;
  var SAFETY_RADIUS = { ROTATING: 1.5, HX: 1.0, VESSEL: 2.0 };
  function safetyMatsInit() { if (!SAFETY_MAT) SAFETY_MAT = new THREE.MeshBasicMaterial({ color: 0xdc2626, transparent: true, opacity: 0.16, depthWrite: false, side: THREE.DoubleSide }); }
  function clearSafetyZoneMeshes() { safetyZoneMeshes.forEach(function (m) { if (m.parent) m.parent.remove(m); }); safetyZoneMeshes = []; }
  function buildSafetyZones() {
    ...
    objects.forEach(function (g) {
      ...
      var c = bb.getCenter(new THREE.Vector3());
      var footR = Math.max(sz.x, sz.z) / 2;
      var r = footR + SAFETY_RADIUS[cls];
      var mesh = new THREE.Mesh(new THREE.CylinderGeometry(r, r, 0.06, 28), SAFETY_MAT);
      mesh.position.set(c.x, 0.03, c.z);
      mesh.renderOrder = 1;
      mesh.userData.noPorts = true;
      mesh.userData.props = { Type: 'Safety Zone', Equipment: type, Basis: 'PROJECT-DEFINED — placeholder radius, not a validated code/standard clearance', Radius: r.toFixed(2) + ' m' };
      scene.add(mesh);
  ...
  /* ============================================================
     PHASE 9 (detection half): PLATFORM ACCESS REQUIREMENT
     The spec itself defers actual platform/stair/ladder GENERATION to
     "later architecture" (its own words) and asks this phase only to
     flag which equipment needs one. This does that, plus a simple
     always-on access ladder mesh (2 rails + rungs) as a visual bonus —
     not a substitute for a real PlatformGenerator, which would also
     need to reason about platform decks, stair rise/run, and handrails.
     ============================================================ */
  ...
  var LADDER_MAT = null, ladderMeshes = [];
  function buildLadderFor(g) {
    var bb = new THREE.Box3().setFromObject(g);
    var h = bb.max.y - 0.3;
    if (h < 2) return null;
    ...
    var railL = cyl(0.02, 0.02, h, LADDER_MAT, 6); railL.position.set(cx, h / 2, cz - railGap / 2); lg.add(railL);
    var railR = railL.clone(); railR.position.z = cz + railGap / 2; lg.add(railR);
    ...
    lg.userData.noPorts = true;
    lg.userData.props = { Type: 'Access Ladder', Note: 'Auto-placed for platform-required equipment — placeholder pending a real PlatformGenerator (deck, stairs, handrail)' };
    return lg;
  }
  /* ============================================================
     PHASE 7: OPERABILITY CHECKER
     Ties Phases 2-6 together into the actual questions the spec wants
     answerable: can an operator reach this equipment (Phase 4's real
     pathfinder, not a straight-line guess), is its maintenance envelope
     clear (Phase 5), and does it need a platform (Phase 9's detection).
     Safety-zone/walkway overlap and the rest of the spec's 12-point
     checklist are natural extensions once this shape is confirmed.
     ============================================================ */
  A3.runOperabilityCheck = function () {
    ...
    objects.forEach(function (g) {
      ...
      var conflict = maintConflicts.filter(function (c) { return c.nid && c.nid === g.userData.nid; })[0];
      var maintClass = classifyMaintClass(type);
      report.push({
        nid: g.userData.nid,
        type: type,
        operatorAccess: path ? 'PASS' : 'FAIL',
        maintenanceAccess: maintClass ? (conflict ? 'WARNING' : 'PASS') : 'N/A',
        maintenanceConflict: conflict ? (conflict.zone + ' — blocked by ' + conflict.blockedBy) : null,

...
(function () {
  ...
  var WALKWAY_OFF_NETWORK_COST = 4;   // how much more it "costs" A* to cut across open yard vs. hug a drawn walkway
  ...
  function updateOperatorFrame(dt) {
    ...
    if (manual) {
      ...
      o.path = null;
      o.arriveEquip = null; o.showingEquipInfo = false; o.viewTopOnArrive = false; o.climbTarget = null; o.arriveLabel = null;
      clearRouteLine();
      ...
      if (dist < (isFinalLeg ? 0.15 : 0.45)) {
        ...
        // The nav grid already verified the whole route is walkable at 2m
        // resolution, so a stall here means fine-grained continuous-space
        // collision caught something the coarse grid missed (rare) —
        // abort rather than get stuck oscillating against it forever.
        if (!moved || (o.pos.x === before && o.pos.z === beforeZ)) {
          ...
          o.arriveEquip = null; o.climbTarget = null; o.arriveLabel = null;
        }
      }
    }
  ...
  A3.setOperatorMode = function (on) {
    ...
    if (on) {
      if (!operatorObj) {
        ...
        scene.add(fig);
        operatorObj = { figure: fig, pos: new THREE.Vector3(), yaw: Math.PI, pitch: 0.15, path: null, keys: {}, arriveEquip: null, arriveFace: null, arriveLabel: null, showingEquipInfo: false, viewTopOnArrive: false, platform: null, climb: null, climbTarget: null, afterClimb: null, inVehicle: false, onCall: false };
      }
      refreshOperatorNav();
      ...
      operatorObj.platform = null; operatorObj.climb = null; operatorObj.climbTarget = null; operatorObj.afterClimb = null;
  ...
  A3.resetOperator = function () {
    ...
    if (operatorObj.inVehicle) A3.exitVehicle();
    A3.setOperatorOnCall(false);
    operatorObj.pos.copy(operatorSpawn); operatorObj.path = null; operatorObj.arriveEquip = null; operatorObj.showingEquipInfo = false; operatorObj.viewTopOnArrive = false; operatorObj.platform = null; operatorObj.climb = null; operatorObj.climbTarget = null; operatorObj.afterClimb = null;
  ...
  A3.boardVehicle = function () {
    if (!operatorMode || !operatorObj) { if (embStatus) embStatus('Enter OPERATOR mode first.'); return false; }
    if (operatorObj.inVehicle) return true;
    if (operatorObj.climb || operatorObj.platform) { if (embStatus) embStatus('Climb down first — the vehicle can\'t reach a platform.'); return false; }
    ...
    if (d > VEHICLE_BOARD_RADIUS) { if (embStatus) embStatus('Walk up to the vehicle first — it\'s ' + d.toFixed(0) + 'm away.'); return false; }
  ...
  A3.goToAssemblyPoint = function () {
    ...
    operatorObj.path = path;
    operatorObj.arriveEquip = null; operatorObj.climbTarget = null; operatorObj.viewTopOnArrive = false; operatorObj.showingEquipInfo = false;
      ...
      if (operatorMode) {
        ...
        if (ray.ray.intersectPlane(GROUND_PLANE, groundPt)) {
          ...
          if (descendIfOnPlatformThen(function () {
            ...
            if (p2) { operatorObj.path = p2; operatorObj.arriveEquip = null; operatorObj.climbTarget = null; operatorObj.arriveLabel = null; operatorObj.showingEquipInfo = false; operatorObj.viewTopOnArrive = false; updateRouteLine(); }
          })) return;
          ...
          if (!clickPath) { if (embStatus) embStatus('NO VALID OPERATOR ACCESS ROUTE — no walkable path to that point.'); }
            ...
            operatorObj.path = clickPath; operatorObj.arriveEquip = null; operatorObj.climbTarget = null; operatorObj.arriveLabel = null; operatorObj.showingEquipInfo = false; operatorObj.viewTopOnArrive = false;
            updateRouteLine();
          }
        }
        return;

...
(function () {
  ...
  function generic(type, label) {
    ...
    else if (/^ko-drum$|^demister$/.test(t)) g = FAC['ko-drum']();
  ...
  function buildPipe(fromPort, toPort, color, nps, ltype, dz) {
    ...
    if (lt.jacket) {
      var jmat = new THREE.MeshStandardMaterial({ color: lt.jacketColor != null ? lt.jacketColor : 0x94a3b8, metalness: 0.6, roughness: 0.5, transparent: true, opacity: lt.jacketOpacity != null ? lt.jacketOpacity : 0.35 });
  ...
  A3.clearAll = function () { objects.forEach(function (o) { scene.remove(o); }); pipes3d.forEach(function (o) { scene.remove(o); }); clearContactShadows(); objects = []; pipes3d = []; jacketMeshes = []; portMeshes = []; spinList = []; picked = null; pickedPipe = null; pendingPort = null; portDebug = []; gapMarkers = []; connHighlighted = []; };
  ...
  function close() { if (raf) cancelAnimationFrame(raf); clearSelBox(); if (host) host.remove(); host = null; renderer && renderer.dispose && renderer.dispose(); renderer = null; objects = []; contactShadows = []; jacketMeshes = []; gapMarkers = []; spinList = []; picked = null; }

...
(function () {
  ...
  var FAC = {
    ...
    'multistage-pump': function () {                        // ring-section barrel pump — stacked stage rings + finned motor
      ...
      for (var f = 0; f < 8; f++) { var fin = torus(0.87, 0.035, mats.dark); fin.rotation.y = Math.PI / 2; fin.position.set(total / 2 + 0.9 + f * 0.22, 1.3, 0); mot.add(fin); }
    ...
    'gear-pump': function () {                              // compact PD / gear pump — visible gear lobes on the face
      ...
      [[-0.22, 0.18], [0.22, -0.18]].forEach(function (o) {
        ...
        for (var t = 0; t < 10; t++) { var a = t / 10 * Math.PI * 2; var tooth = box(0.08, 0.1, 0.05, mats.gold); tooth.position.set(0, Math.sin(a) * 0.42, Math.cos(a) * 0.42); gear.add(tooth); }
      });
  ...
  function generic(type, label) {
    ...
    else if (/sthe|shell|hx|exchanger|cooler|condenser|economizer|chiller|heater|boiler/.test(t)) g = FAC['sthe']();

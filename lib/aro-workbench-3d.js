...
(function () {
  ...
  var FAC = {
    'centrifugal-pump': function () {
      ...
      var g = new THREE.Group();
      // volute casing — cast iron, not painted blue
      var vol = cyl(1.5, 1.5, 1.1, mats.castIron); vol.rotation.x = Math.PI / 2; vol.position.set(-2.3, 1.4, 0); g.add(vol);
      var volFace = cyl(1.55, 1.55, 0.14, mats.dark); volFace.rotation.x = Math.PI / 2; volFace.position.set(-2.3, 1.4, 0.6); g.add(volFace);
      // volute cutwater lobe — the off-centre bulge a real spiral casing has,
      // not a perfect drum
      var lobe = cyl(1.5, 1.5, 1.1, mats.castIron); lobe.rotation.x = Math.PI / 2; lobe.scale.set(1.18, 1, 0.55); lobe.position.set(-2.3, 1.15, -0.6); g.add(lobe);
      ...
      // suction nozzle (axial, +Z) and discharge (up) — real cast-iron
      // nozzle necks, chrome-ish raised face gasket ring
      var suc = cyl(0.55, 0.55, 1.0, mats.castIron); suc.rotation.x = Math.PI / 2; suc.position.set(-2.3, 1.4, 1.2); g.add(suc);
      g.add(flgZ(-2.3, 1.4, 1.7, 0.75));
      var dis = cyl(0.5, 0.5, 1.3, mats.castIron); dis.position.set(-2.3, 2.6, 0); g.add(dis);
      g.add(nozzleY(3.2, 0.0, 0.5, mats.castIron, -2.3));
      ...
      // conduit box, all in a real motor-frame navy (not pump-casing blue)
      var mot = cyl(1.15, 1.15, 3.0, mats.navy); mot.rotation.z = Math.PI / 2; mot.position.set(1.55, 1.4, 0); g.add(mot);
      ...
      for (var f = 0; f < 13; f++) { var fin = torus(1.18, 0.035, mats.dark); fin.rotation.y = Math.PI / 2; fin.position.set(0.35 + f * 0.21, 1.4, 0); g.add(fin); }

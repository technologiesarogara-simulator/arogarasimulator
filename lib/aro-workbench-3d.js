...
(function () {
  ...
  A3.addByType = function (type, label, nid) {
    ...
    // Connection ports from the equipment's own bounding box.
    ...
    if (sy > sx * 1.3) {
      addPort(g, 'in', new THREE.Vector3(midX, bb.min.y - 0.35, midZ));
      addPort(g, 'out', new THREE.Vector3(midX, bb.max.y + 0.35, midZ));
    } else {
      addPort(g, 'in', new THREE.Vector3(bb.min.x - 0.35, midY, midZ));
      addPort(g, 'out', new THREE.Vector3(bb.max.x + 0.35, midY, midZ));
    }
  ...
  function buildPipe(fromPort, toPort, color, nps, ltype) {
    ...
    grp.add(flangeAt(pa, w1.clone().sub(pa), r));
    grp.add(flangeAt(pb, pb.clone().sub(w3), r));
    if (lt.jacket) {
  ...
  A3.buildFromModel = function (nodes, pipes, nameOf) {
    ...
    /* ── WHERE EACH ITEM GOES ──────────────────────────────────────────
       ...
       given a position. */
    var K = 0.125;                                  /* world units per sheet px */

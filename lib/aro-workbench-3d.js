...
(function () {
  ...
  var FAC = {
    ...
    'elbow45': function () {                                // 45° pipe bend on stub ends, with cast weld-bead rings
      var g = new THREE.Group();
    ...
    'tee-fitting': function () {                            // run + branch pipe tee, gunmetal with a gold branch collar
      var g = new THREE.Group();
    ...
    'cross-fitting': function () {                          // four-way pipe cross, gunmetal with a gold junction collar
      var g = new THREE.Group();
      var run = cyl(0.45, 0.45, 3.4, mats.gunmetal); run.rotation.z = Math.PI / 2; g.add(run);
    ...
    'reducer-fitting': function () {                        // concentric size-change cone, gunmetal with an orange taper band
      var g = new THREE.Group();
    ...
    'temp-indicator': function () {                         // thermowell probe into the process + remote dial head
      var g = new THREE.Group();
      var well = cyl(0.09, 0.06, 1.1, mats.steel); well.rotation.z = Math.PI / 2; well.position.set(0.55, 0.9, 0); g.add(well);
      var neck = cyl(0.1, 0.1, 0.5, mats.steel); neck.position.y = 1.35; g.add(neck);
      var head = cyl(0.55, 0.55, 0.32, mats.gunmetal, 28); head.rotation.x = Math.PI / 2; head.position.y = 1.75; g.add(head);
      var face = cyl(0.47, 0.47, 0.06, mats.white, 28); face.rotation.x = Math.PI / 2; face.position.y = 1.92; g.add(face);
      var needle = box(0.36, 0.04, 0.04, mats.maroon); needle.rotation.z = 1.1; needle.position.set(0.02, 1.93, 0.02); g.add(needle);
      var base = box(0.4, 0.16, 0.4, mats.dark); base.position.set(0.55, 0.08, 0); g.add(base);
      g.userData.props = { Type: 'Temperature Indicator (TI)', Range: '0-200 °C', Element: 'RTD Pt100 + thermowell', Dial: '100mm bimetallic' };
      return g;
    },
    'level-indicator': function () {                        // side-mounted reflex sight glass — a graduated clear column
      var g = new THREE.Group();
      var chamberA = cyl(0.14, 0.14, 2.0, mats.gunmetal, 16); chamberA.position.set(-0.35, 1.3, 0); g.add(chamberA);
      var glass = mesh(new THREE.CylinderGeometry(0.1, 0.1, 1.8, 16), mats.glass); glass.position.set(0.05, 1.3, 0); g.add(glass);
      var chamberB = cyl(0.14, 0.14, 2.0, mats.gunmetal, 16); chamberB.position.set(0.45, 1.3, 0); g.add(chamberB);
      var fluid = cyl(0.09, 0.09, 0.8, mats.blue, 16); fluid.position.set(0.05, 0.95, 0); g.add(fluid);
      [-0.35, 0.05, 0.45].forEach(function (cx) {
        var topN = cyl(0.05, 0.05, 0.2, mats.steel); topN.rotation.z = Math.PI / 2; topN.position.set(cx, 2.35, 0); g.add(topN);
        var botN = cyl(0.05, 0.05, 0.2, mats.steel); botN.rotation.z = Math.PI / 2; botN.position.set(cx, 0.25, 0); g.add(botN);
      });
      var bracket = box(1.0, 0.1, 0.2, mats.dark); bracket.position.set(0.05, 2.4, -0.15); g.add(bracket);
      g.userData.props = { Type: 'Level Indicator (LI)', Type2: 'Reflex sight glass', Range: '0-2 m', Connection: '3/4" NPT top/bottom' };
      return g;
    },
    'flow-meter': function () {                              // flanged inline flow tube + electronics transmitter head
      var g = new THREE.Group();
      var stub = cyl(0.4, 0.4, 1.8, mats.steel); stub.rotation.z = Math.PI / 2; g.add(stub);
      var body = cyl(0.55, 0.55, 0.6, mats.forest); body.rotation.z = Math.PI / 2; g.add(body);
      g.add(flangeX(1.15, 0.7, mats.steel)); g.add(flangeX(-1.15, 0.7, mats.steel));
      var neck = cyl(0.14, 0.14, 0.5, mats.dark); neck.position.y = 0.75; g.add(neck);
      var head = box(0.55, 0.5, 0.4, mats.gunmetal); head.position.y = 1.15; g.add(head);
      var screen = box(0.4, 0.28, 0.05, mats.white); screen.position.set(0, 1.15, 0.23); g.add(screen);
      g.position.y = 1.3;
      g.userData.props = { Type: 'Flow Meter (FT)', Technology: 'Electromagnetic', Signal: '4-20 mA / HART', 'Size (NPS)': 3 };
      return g;
    },
    'orifice-plate': function () {                          // thin restriction plate sandwiched between flanges + DP taps
      var g = new THREE.Group();
      var stub1 = cyl(0.4, 0.4, 0.9, mats.steel); stub1.rotation.z = Math.PI / 2; stub1.position.x = -0.65; g.add(stub1);
      var stub2 = cyl(0.4, 0.4, 0.9, mats.steel); stub2.rotation.z = Math.PI / 2; stub2.position.x = 0.65; g.add(stub2);
      g.add(flangeX(-0.2, 0.6, mats.gunmetal)); g.add(flangeX(0.2, 0.6, mats.gunmetal));
      var plate = cyl(0.5, 0.5, 0.04, mats.gold, 28); plate.rotation.z = Math.PI / 2; g.add(plate);
      var bore = cyl(0.2, 0.2, 0.05, mats.dark, 20); bore.rotation.z = Math.PI / 2; g.add(bore);
      var tapHi = cyl(0.05, 0.05, 0.5, mats.steel); tapHi.position.set(-0.5, 0.35, 0); g.add(tapHi);
      var tapLo = cyl(0.05, 0.05, 0.5, mats.steel); tapLo.position.set(0.5, 0.35, 0); g.add(tapLo);
      var dpCell = box(0.4, 0.3, 0.3, mats.white); dpCell.position.set(0, 0.75, 0); g.add(dpCell);
      g.position.y = 1.3;
    ...
    'fired-boiler': function () {                           // horizontal drum + red firebox + short exhaust stack
      var g = new THREE.Group();
      var drum = cyl(1.1, 1.1, 3.6, mats.silver, 26); drum.rotation.z = Math.PI / 2; drum.position.y = 2.6; g.add(drum);
      g.add(dishSideNeg(1.1, -1.8, 2.6, mats.silver)); g.add(dishSidePos(1.1, 1.8, 2.6, mats.silver));
      var firebox = box(3.0, 1.6, 2.0, mats.red); firebox.position.y = 0.9; g.add(firebox);
      var glow = cyl(0.35, 0.35, 0.1, mats.orange, 16); glow.rotation.z = Math.PI / 2; glow.position.set(-1.4, 0.9, 0); g.add(glow);
      var stack = cyl(0.35, 0.35, 1.8, mats.dark, 20); stack.position.set(0.8, 4.4, 0); g.add(stack);
      var cap = cyl(0.45, 0.45, 0.15, mats.dark, 20); cap.position.set(0.8, 5.35, 0); g.add(cap);
      g.add(nozzleY(2.6 + 1.1 + 0.5, 0, 0.28, mats.steel, -1.4));
      var bfwIn = cyl(0.24, 0.24, 0.9, mats.steel); bfwIn.rotation.z = Math.PI / 2; bfwIn.position.set(-1.9, 1.6, 0); g.add(bfwIn);
      g.userData.props = { Type: 'Fired Boiler (Fire-tube)', 'Capacity (TPH)': 15, 'Design P (barg)': 17.5, Fuel: 'NG/FO', Material: 'SA-516-70' };
      return g;
    },
    'steam-ejector': function () {                          // small converging-diverging venturi nozzle, motive steam on top
      var g = new THREE.Group();
      var conv = cyl(0.55, 0.22, 0.9, mats.gunmetal, 22); conv.rotation.z = Math.PI / 2; conv.position.x = -0.55; g.add(conv);
      var throat = cyl(0.16, 0.16, 0.3, mats.gold, 16); throat.rotation.z = Math.PI / 2; g.add(throat);
      var div = cyl(0.22, 0.6, 1.3, mats.gunmetal, 22); div.rotation.z = Math.PI / 2; div.position.x = 0.85; g.add(div);
      g.add(flangeX(-1.0, 0.6, mats.steel)); g.add(flangeX(1.5, 0.68, mats.steel));
      var motive = cyl(0.18, 0.18, 0.9, mats.steel); motive.position.set(-0.55, 0.55, 0); g.add(motive);
      g.add(flangeX(-0.55, 0.24, mats.gold));
      g.position.y = 1.3;
      g.userData.props = { Type: 'Steam Jet Ejector', Stage: '1st stage', Motive: 'MP steam', Capacity: '450 kg/h air', Material: 'CS/SS nozzle' };
      return g;
    },
  ...
  function generic(type, label) {
    ...
    else if (/^pd-pump$|^int-gear$|^twin-screw$|peristaltic|pneu-diaphragm|mag-drive|^pcp$|lr-vacuum|rv-vacuum/.test(t)) g = FAC['gear-pump']();
    ...
    else if (/vturbine|split-case|self-prime/.test(t)) g = FAC['centrifugal-pump']();
    ...
    // Fired Boiler gets its own drum + firebox shape — checked before the
    // "boiler" keyword in the sthe fallback below, which would otherwise
    // render it as a full Shell & Tube HX tube bundle.
    else if (/^boiler$/.test(t)) g = FAC['fired-boiler']();
    ...
    else if (/^cstr$|^batch$/.test(t)) g = FAC['reactor-cstr']();
    else if (/^pfr$|^loop$/.test(t)) g = FAC['reactor-pfr']();
    ...
    else if (/separat|drum|ko|flash|demister|vessel|vacuum-vessel|h-vessel|horizontal|steam-header|water-header/.test(t)) g = FAC['h-vessel']();
    ...
    else if (/^check$/.test(t)) g = FAC['check-valve']();

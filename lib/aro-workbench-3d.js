...
(function () {
  ...
  A3.clearAll = function () { flowTubes.forEach(function (f) { if (f.mesh.parent) f.mesh.parent.remove(f.mesh); if (f.mesh.geometry) f.mesh.geometry.dispose(); if (f.mesh.material) f.mesh.material.dispose(); }); flowTubes = []; dimmedEquip = []; vizHalos.forEach(function (h) { if (h.parent) h.parent.remove(h); if (h.geometry) h.geometry.dispose(); if (h.material) h.material.dispose(); }); vizHalos = []; objects.forEach(function (o) { scene.remove(o); }); pipes3d.forEach(function (o) { scene.remove(o); }); foundations.forEach(function (f) { scene.remove(f); f.traverse(function (o) { if (o.geometry) o.geometry.dispose(); }); }); foundations = []; clearWalkwayMeshes(); clearRouteLine(); walkGrid = null; clearContactShadows(); objects = []; pipes3d = []; jacketMeshes = []; portMeshes = []; spinList = []; picked = null; pickedPipe = null; pendingPort = null; portDebug = []; gapMarkers = []; connHighlighted = []; };
  ...
  function close() { if (raf) cancelAnimationFrame(raf); clearSelBox(); if (host) host.remove(); host = null; renderer && renderer.dispose && renderer.dispose(); renderer = null; objects = []; contactShadows = []; jacketMeshes = []; gapMarkers = []; spinList = []; picked = null; flowTubes = []; vizHalos = []; flowVizMode = 'off'; foundations = []; walkwayMeshes = []; walkGrid = null; routeLine = null; }

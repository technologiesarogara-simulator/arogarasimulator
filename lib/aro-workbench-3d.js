...
(function () {
  ...
  var FAC = {
    'centrifugal-pump': function () {
      ...
      g.userData.portDefs = [
        { role: 'in', name: 'Suction', local: new THREE.Vector3(-1.6, 1.4, 1.7), dir: new THREE.Vector3(0, 0, 1) },
        { role: 'out', name: 'Discharge', local: new THREE.Vector3(-1.6, 3.2, 0), dir: new THREE.Vector3(0, 1, 0) }
      ];
      return g;
    },
    'sthe': function () {
      ...
      g.userData.portDefs = [
        { role: 'in', name: 'Tube Inlet', local: new THREE.Vector3(-L / 2 - 0.7, 2.4, 0), dir: new THREE.Vector3(-1, 0, 0) },
        { role: 'out', name: 'Tube Outlet', local: new THREE.Vector3(L / 2 + 0.7, 2.4, 0), dir: new THREE.Vector3(1, 0, 0) },
        { role: 'in2', name: 'Shell Inlet', local: new THREE.Vector3(-L / 2 + 1.2, 2.4 + R + 0.8, 0), dir: new THREE.Vector3(0, 1, 0) },
        { role: 'out2', name: 'Shell Outlet', local: new THREE.Vector3(L / 2 - 1.2, 2.4 - R - 0.8, 0), dir: new THREE.Vector3(0, -1, 0) }
      ];
      return g;
    },
    'v-vessel': function () {
      ...
      g.userData.portDefs = [
        { role: 'in', name: 'Drain', local: new THREE.Vector3(0, 2 - 1.2, 0), dir: new THREE.Vector3(0, -1, 0) },
        { role: 'out', name: 'Outlet', local: new THREE.Vector3(0, 2 + H + R + 0.6, 0), dir: new THREE.Vector3(0, 1, 0) }
      ];
      return g;
    },
    'h-vessel': function () {
      ...
      g.userData.portDefs = [
        { role: 'in', name: 'Inlet', local: new THREE.Vector3(-L / 2 - 0.5, 2.4, 0), dir: new THREE.Vector3(-1, 0, 0) },
        { role: 'out', name: 'Outlet', local: new THREE.Vector3(-1.5, 2.4 + R + 0.6, 0), dir: new THREE.Vector3(0, 1, 0) }
      ];
      return g;
    },
    'column': function () {
      ...
      g.userData.portDefs = [
        { role: 'in', name: 'Feed', local: new THREE.Vector3(-R - 1.6, 2 + H * 0.45, 0), dir: new THREE.Vector3(-1, 0, 0) },
        { role: 'out', name: 'Overhead', local: new THREE.Vector3(0, 2 + H + R + 0.6, 0), dir: new THREE.Vector3(0, 1, 0) }
      ];
    ...
    'cone-tank': function () {
      ...
      g.userData.portDefs = [
        { role: 'in', name: 'Inlet', local: new THREE.Vector3(-R - 1.6, 0.9, 0), dir: new THREE.Vector3(-1, 0, 0) },
        { role: 'out', name: 'Outlet', local: new THREE.Vector3(0, 0.1, 0), dir: new THREE.Vector3(0, -1, 0) }
      ];
      return g;
    },
    'bullet': function () {
      ...
      g.userData.portDefs = [
        { role: 'in', name: 'Inlet', local: new THREE.Vector3(-L / 2 - R * 0.6, 2.2, 0), dir: new THREE.Vector3(-1, 0, 0) },
        { role: 'out', name: 'Outlet', local: new THREE.Vector3(L / 2 + R * 0.6, 2.2, 0), dir: new THREE.Vector3(1, 0, 0) }
      ];
      return g;
    },
    'gate-valve': function () {
      ...
      g.userData.portDefs = [
        { role: 'in', name: 'Inlet', local: new THREE.Vector3(0, 1.4, 1.0), dir: new THREE.Vector3(0, 0, 1) },
        { role: 'out', name: 'Outlet', local: new THREE.Vector3(0, 1.4, -1.0), dir: new THREE.Vector3(0, 0, -1) }
      ];
      return g;
    },
    'ball-valve': function () {
      ...
      g.userData.portDefs = [
        { role: 'in', name: 'Inlet', local: new THREE.Vector3(0, 1.4, 1.1), dir: new THREE.Vector3(0, 0, 1) },
        { role: 'out', name: 'Outlet', local: new THREE.Vector3(0, 1.4, -1.1), dir: new THREE.Vector3(0, 0, -1) }
      ];
      return g;
    },
    'compressor': function () {
      ...
      g.userData.portDefs = [
        { role: 'in', name: 'Suction', local: new THREE.Vector3(2 + 1.5 + 0.6, -1, 0), dir: new THREE.Vector3(0, 1, 0) },
        { role: 'out', name: 'Discharge', local: new THREE.Vector3(1.7, 3, 0), dir: new THREE.Vector3(0, 1, 0) }
      ];
      return g;
    },
    'reboiler': function () {
      ...
      g.userData.portDefs = [
        { role: 'in', name: 'Liquid Feed', local: new THREE.Vector3(L / 2 - 0.3, 2.4, 0), dir: new THREE.Vector3(1, 0, 0) },
        { role: 'out', name: 'Vapor Outlet', local: new THREE.Vector3(-L / 2 + 1, 2.4 + R * 1.4 + 0.6, 0), dir: new THREE.Vector3(0, 1, 0) }
      ];
      return g;
    },
    'plate-hx': function () {
    ...
    'aircooler': function () {
      ...
      g.userData.portDefs = [
        { role: 'in', name: 'Process Inlet', local: new THREE.Vector3(-4.4, 3.4, 0), dir: new THREE.Vector3(-1, 0, 0) },
        { role: 'out', name: 'Process Outlet', local: new THREE.Vector3(4.4, 3.4, 0), dir: new THREE.Vector3(1, 0, 0) }
      ];
      return g;
    },
    'reactor': function () {
    ...
    'filter': function () {
      ...
      g.userData.portDefs = [
        { role: 'in', name: 'Inlet', local: new THREE.Vector3(-R - 0.6, 2 + H * 0.8, 0), dir: new THREE.Vector3(-1, 0, 0) },
        { role: 'out', name: 'Outlet', local: new THREE.Vector3(R + 0.6, 2 + H * 0.8, 0), dir: new THREE.Vector3(1, 0, 0) }
      ];
      return g;
    },
    'fan': function () {
    ...
    'multistage-pump': function () {                        // ring-section barrel pump — stacked stage rings + finned motor
      ...
      g.userData.portDefs = [
        { role: 'in', name: 'Suction', local: new THREE.Vector3(x0 - 0.5, 1.3, 0), dir: new THREE.Vector3(-1, 0, 0) },
        { role: 'out', name: 'Discharge', local: new THREE.Vector3(x0 + segLen * 0.6, 1.3 + R + 1.1, 0), dir: new THREE.Vector3(0, 1, 0) }
      ];
      return g;
    },
    ...
    'double-pipe-hx': function () {                         // hairpin double-pipe — concentric tubes + U-bend
      ...
      g.userData.portDefs = [
        { role: 'in', name: 'Inner Pipe Inlet', local: new THREE.Vector3(-L / 2 - 0.3, 1.3, 0.55), dir: new THREE.Vector3(-1, 0, 0) },
        { role: 'out', name: 'Annulus Outlet', local: new THREE.Vector3(0, 1.3 + Ro + 0.35, -0.55), dir: new THREE.Vector3(0, 1, 0) }
      ];
      return g;
    },
    ...
    'silo-hopper': function () {                            // cylindrical silo with a conical BOTTOM discharge
      ...
      g.userData.portDefs = [
        { role: 'in', name: 'Inlet', local: new THREE.Vector3(0, waist + H + R + 0.3, 0), dir: new THREE.Vector3(0, 1, 0) },
        { role: 'out', name: 'Discharge', local: new THREE.Vector3(0, waist - coneH - 0.35, 0), dir: new THREE.Vector3(0, -1, 0) }
      ];
      return g;
    },
    'atm-tank': function () {                               // atmospheric tank — shallow dome roof, aluminium/white, wind girders
      ...
      g.userData.portDefs = [
        { role: 'in', name: 'Inlet', local: new THREE.Vector3(-R - 1.6, 0.9, 0), dir: new THREE.Vector3(-1, 0, 0) },
        { role: 'out', name: 'Vent', local: new THREE.Vector3(0, 0.25 + H + 0.3, 0), dir: new THREE.Vector3(0, 1, 0) }
      ];
      return g;
    },
    ...
    'globe-valve': function () {                            // conical bonnet + yoke posts distinguish it from the gate valve
      ...
      g.userData.portDefs = [
        { role: 'in', name: 'Inlet', local: new THREE.Vector3(0, 1.4, 0.9), dir: new THREE.Vector3(0, 0, 1) },
        { role: 'out', name: 'Outlet', local: new THREE.Vector3(0, 1.4, -0.9), dir: new THREE.Vector3(0, 0, -1) }
      ];
      return g;
    },
    'butterfly-valve': function () {                        // thin wafer body + lever/gearbox operator — no long flanged stubs
      ...
      g.userData.portDefs = [
        { role: 'in', name: 'Inlet', local: new THREE.Vector3(0, 1.3, 0.275), dir: new THREE.Vector3(0, 0, 1) },
        { role: 'out', name: 'Outlet', local: new THREE.Vector3(0, 1.3, -0.275), dir: new THREE.Vector3(0, 0, -1) }
      ];
      return g;
    },
    ...
    'control-valve': function () {                          // globe body under a large pneumatic diaphragm actuator dome
      ...
      g.userData.portDefs = [
        { role: 'in', name: 'Inlet', local: new THREE.Vector3(0, 0, 0.85), dir: new THREE.Vector3(0, 0, 1) },
        { role: 'out', name: 'Outlet', local: new THREE.Vector3(0, 0, -0.85), dir: new THREE.Vector3(0, 0, -1) }
      ];
      return g;
    },
    'psv-valve': function () {                              // angle relief valve — safety-red, pop-action cap, side lever
      ...
      g.userData.portDefs = [
        { role: 'in', name: 'Inlet', local: new THREE.Vector3(-0.75, 0, 0), dir: new THREE.Vector3(-1, 0, 0) },
        { role: 'out', name: 'Outlet (relief)', local: new THREE.Vector3(0, 2.1, 0), dir: new THREE.Vector3(0, 1, 0) }
      ];
      return g;
    },
    ...
    'needle-valve': function () {                           // small compact instrument-isolation valve — T-handle, no big wheel
      ...
      g.userData.portDefs = [
        { role: 'in', name: 'Inlet', local: new THREE.Vector3(-0.55, 0, 0), dir: new THREE.Vector3(-1, 0, 0) },
        { role: 'out', name: 'Outlet', local: new THREE.Vector3(0.55, 0, 0), dir: new THREE.Vector3(1, 0, 0) }
      ];
      return g;
    },
    ...
    'flow-meter': function () {                              // electromagnetic flow meter — blue tube + rounded white transmitter head
      ...
      g.userData.portDefs = [
        { role: 'in', name: 'Inlet', local: new THREE.Vector3(-1.15, 0, 0), dir: new THREE.Vector3(-1, 0, 0) },
        { role: 'out', name: 'Outlet', local: new THREE.Vector3(1.15, 0, 0), dir: new THREE.Vector3(1, 0, 0) }
      ];
      return g;
    },
    ...
    'y-strainer': function () {                             // inline pipe with a 45° screen pocket + bolted cap, matches 2D icon
      ...
      g.userData.portDefs = [
        { role: 'in', name: 'Inlet', local: new THREE.Vector3(-1.1, 0, 0), dir: new THREE.Vector3(-1, 0, 0) },
        { role: 'out', name: 'Outlet', local: new THREE.Vector3(1.1, 0, 0), dir: new THREE.Vector3(1, 0, 0) }
      ];
      return g;
    },
    ...
    'basket-filter': function () {                          // vertical housing with a large bolted top cover (basket access)
      ...
      g.userData.portDefs = [
        { role: 'in', name: 'Inlet', local: new THREE.Vector3(-R - 0.5, base + H * 0.75, 0), dir: new THREE.Vector3(-1, 0, 0) },
        { role: 'out', name: 'Outlet', local: new THREE.Vector3(R + 0.5, base + H * 0.75, 0), dir: new THREE.Vector3(1, 0, 0) }
      ];
      return g;
    },
    'bag-filter': function () {                             // dome top, straight barrel, tapered CONE bottom holding the bag
      ...
      g.userData.portDefs = [
        { role: 'in', name: 'Inlet', local: new THREE.Vector3(-R - 0.5, base + coneH + H * 0.7, 0), dir: new THREE.Vector3(-1, 0, 0) },
        { role: 'out', name: 'Outlet', local: new THREE.Vector3(R + 0.5, base + coneH * 1.4, 0), dir: new THREE.Vector3(1, 0, 0) }
      ];
      return g;
    },
    'duplex-filter': function () {                          // two parallel housings + a top transfer/switching valve
      ...
      g.userData.portDefs = [
        { role: 'in', name: 'Inlet', local: new THREE.Vector3(-R - 1.4, base + H * 0.7, 0), dir: new THREE.Vector3(-1, 0, 0) },
        { role: 'out', name: 'Outlet', local: new THREE.Vector3(R + 1.4, base + H * 0.7, 0), dir: new THREE.Vector3(1, 0, 0) }
      ];
      return g;
    },
    ...
    'venturi-meter': function () {
      ...
      g.userData.portDefs = [
        { role: 'in', name: 'Inlet', local: new THREE.Vector3(-0.95, 0, 0), dir: new THREE.Vector3(-1, 0, 0) },
    ...
    'vortex-meter': function () {
      ...
      g.userData.portDefs = [
        { role: 'in', name: 'Inlet', local: new THREE.Vector3(-0.95, 0, 0), dir: new THREE.Vector3(-1, 0, 0) },
        { role: 'out', name: 'Outlet', local: new THREE.Vector3(0.95, 0, 0), dir: new THREE.Vector3(1, 0, 0) }
      ];
      return g;
    },
    ...
    'sight-glass': function () {
      ...
      g.userData.portDefs = [
        { role: 'in', name: 'Inlet', local: new THREE.Vector3(-0.65, 0, 0), dir: new THREE.Vector3(-1, 0, 0) },
        { role: 'out', name: 'Outlet', local: new THREE.Vector3(0.65, 0, 0), dir: new THREE.Vector3(1, 0, 0) }
      ];
      return g;
    },
    ...
    'diaphragm-valve': function () {                        // weir body + large diaphragm bonnet dome
      ...
      g.userData.portDefs = [
        { role: 'in', name: 'Inlet', local: new THREE.Vector3(-0.75, 0, 0), dir: new THREE.Vector3(-1, 0, 0) },
        { role: 'out', name: 'Outlet', local: new THREE.Vector3(0.75, 0, 0), dir: new THREE.Vector3(1, 0, 0) }
      ];
      return g;
    },
    'pinch-valve': function () {                            // rubber sleeve body pinched by external bars
      ...
      g.userData.portDefs = [
        { role: 'in', name: 'Inlet', local: new THREE.Vector3(-0.68, 0, 0), dir: new THREE.Vector3(-1, 0, 0) },
        { role: 'out', name: 'Outlet', local: new THREE.Vector3(0.68, 0, 0), dir: new THREE.Vector3(1, 0, 0) }
      ];
      return g;
    },
    ...
    'cryo-valve': function () {                             // extended bonnet / tall stem for cryogenic service
      ...
      g.userData.portDefs = [
        { role: 'in', name: 'Inlet', local: new THREE.Vector3(0, 0, 0.5), dir: new THREE.Vector3(0, 0, 1) },
        { role: 'out', name: 'Outlet', local: new THREE.Vector3(0, 0, -0.5), dir: new THREE.Vector3(0, 0, -1) }
      ];
      return g;
    },
    'angle-valve': function () {                            // 90°-offset ports — inlet horizontal, outlet vertical
      ...
      g.userData.portDefs = [
        { role: 'in', name: 'Inlet', local: new THREE.Vector3(0, 0, 0.5), dir: new THREE.Vector3(0, 0, 1) },
        { role: 'out', name: 'Outlet', local: new THREE.Vector3(0, 0.55, 0), dir: new THREE.Vector3(0, 1, 0) }
      ];
      return g;
    },
  ...
  function addPort(g, role, localPos, name, dir) {
    ...
    s.userData.port = { role: role, name: name || (isIn ? 'Inlet' : 'Outlet'), dir: dir ? dir.clone() : new THREE.Vector3(isIn ? -1 : 1, 0, 0) };
  ...
  A3.addByType = function (type, label, nid, nps) {
    ...
    if (g.userData.noPorts) {
      ...
      g.userData.portDefs.forEach(function (pd) {
        addPort(g, pd.role, pd.local.clone(), pd.name, pd.dir);
      });
    } else {
      ...
      if (sy > sx * 1.3) {
        addPort(g, 'in', new THREE.Vector3(midX, bb.min.y - 0.35, midZ), null, new THREE.Vector3(0, -1, 0));
        addPort(g, 'out', new THREE.Vector3(midX, bb.max.y + 0.35, midZ), null, new THREE.Vector3(0, 1, 0));
      } else {
        addPort(g, 'in', new THREE.Vector3(bb.min.x - 0.35, midY, midZ), null, new THREE.Vector3(-1, 0, 0));
        addPort(g, 'out', new THREE.Vector3(bb.max.x + 0.35, midY, midZ), null, new THREE.Vector3(1, 0, 0));
      }
    }
  ...
  function elbowAt(pPrev, p, pNext, r, mat) {
    ...
    if (R < r * 0.3) return { mesh: null, t1: p, t2: p };   // leg too short for a real bend to fit
  ...
  function flangeAt(pos, dir, r) {
    if (!FLANGE_BOLT_MAT) FLANGE_BOLT_MAT = new THREE.MeshStandardMaterial({ color: 0x2b3341, metalness: 0.75, roughness: 0.35 });
    var steel = new THREE.MeshStandardMaterial({ color: 0xb8c2cf, metalness: 0.88, roughness: 0.24 });
  ...
  function buildPipe(fromPort, toPort, color, nps, ltype) {
    ...
    var mat = new THREE.MeshStandardMaterial({ color: new THREE.Color(col), metalness: 0.75, roughness: 0.4 });
    var emat = new THREE.MeshStandardMaterial({ color: new THREE.Color(col).multiplyScalar(0.85), metalness: 0.72, roughness: 0.42 });
    ...
    var la = pa.clone().addScaledVector(dirA, leadLen);
    var lb = pb.clone().addScaledVector(dirB, leadLen);
    ...
    // Sized to the real equipment nozzle NPS when the 2D model recorded
    // one (g.userData.nps, passed through from the node's own nps field —
    // never guessed here); falls back to the pipe's own radius otherwise.
    var fgEarly = fromPort.userData.port.group, tgEarly = toPort.userData.port.group;
    var rEqA = fgEarly.userData.nps ? npsRadius(fgEarly.userData.nps) : r;
    var rEqB = tgEarly.userData.nps ? npsRadius(tgEarly.userData.nps) : r;
    grp.add(flangeAt(pa, dirA, rEqA));
    grp.add(flangeAt(pb, dirB, rEqB));
    ...
    function reducerAt(pos, dir, rBig, rSmall, mat2) {
      var d = dir.clone().normalize();
  ...
  A3.buildFromModel = function (nodes, pipes, nameOf) {
    ...
    (pipes || []).forEach(function (p) {
      ...
      var op = portOfRoles(a, true) || portOfRoles(a, false), ip = portOfRoles(b, false) || portOfRoles(b, true);
  ...
  A3.clearAll = function () { objects.forEach(function (o) { scene.remove(o); }); pipes3d.forEach(function (o) { scene.remove(o); }); objects = []; pipes3d = []; portMeshes = []; spinList = []; picked = null; pickedPipe = null; pendingPort = null; portDebug = []; connHighlighted = []; };
  ...
  function buildScene(canvas) {
    ...
    /* High-metalness pipe/fitting materials (metalness ~0.9) have almost no
       diffuse response — any face not catching a direct specular highlight
       from the two directional lights fell close to black, which is why
       pipes read as "too dark to see" even though their base colour is a
       light steel grey. Hemisphere ground colour and ambient are the only
       light those faces get, so both are raised here to keep the shadow
       side of a pipe legibly grey instead of near-black; the key/fill
       directional lights are unchanged. */
    scene.add(new THREE.HemisphereLight(0xcbdcff, 0x4a5568, 0.8));
    ...
    scene.add(dir);
    var fill = new THREE.DirectionalLight(0x99bbff, 0.45); fill.position.set(-10, 8, -8); scene.add(fill);
  ...
  function close() { if (raf) cancelAnimationFrame(raf); clearSelBox(); if (host) host.remove(); host = null; renderer && renderer.dispose && renderer.dispose(); renderer = null; objects = []; spinList = []; picked = null; }
})();

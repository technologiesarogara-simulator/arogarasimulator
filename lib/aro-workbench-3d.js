...
(function () {
  ...
  function M() { return {
    ...
    // Cast concrete — the plinth a steel skid/baseplate actually sits on.
    // Every baseplate before this used mats.dark (painted steel) all the
    // way down to the floor, which reads as a steel slab floating on the
    // ground rather than a skid bolted to a real foundation. Non-metallic
    // and rough on purpose — concrete has no specular response like this.
    concrete:new THREE.MeshStandardMaterial({ color: 0x9a9d9c, metalness: 0, roughness: 0.92, envMapIntensity: ENV_MAP_INTENSITY * 0.4 })
  }; }
  var mats;
  ...
  // Same shell, minus a real geometric wedge (thetaStart/thetaLength) — a
  // genuine gap in the mesh, not a transparency hack, so whatever internals
  // a factory already models (trays, tube bundle) become visible through it
  // exactly like a vendor cutaway render, without hollowing out the vessel
  // everywhere else the way a see-through material would.
  function cylArc(rt, rb, h, mat, seg, thetaStart, thetaLength) { return mesh(new THREE.CylinderGeometry(rt, rb, h, seg || 32, 1, false, thetaStart, thetaLength), mat); }
  ...
  /* ── Column/tall-vessel external structure kit ──────────────────────────
     Caged access ladder, an access platform ring with handrail, a bolted
     manway, and lifting lugs — the walkable/inspectable exterior detail a
     real column has that a bare shell doesn't. Mounted on -Z, clear of the
     +X-side process nozzles every column factory already draws. */
  function colLadder(R, yBot, yTop, mat) {
    var g = new THREE.Group(), z = -(R + 0.12);
    var rail1 = cyl(0.025, 0.025, yTop - yBot, mat, 6); rail1.position.set(-0.18, (yTop + yBot) / 2, z); g.add(rail1);
    var rail2 = cyl(0.025, 0.025, yTop - yBot, mat, 6); rail2.position.set(0.18, (yTop + yBot) / 2, z); g.add(rail2);
    var n = Math.max(4, Math.round((yTop - yBot) / 0.35));
    for (var i = 0; i <= n; i++) { var y = yBot + i * (yTop - yBot) / n; var rung = cyl(0.02, 0.02, 0.4, mat, 6); rung.rotation.z = Math.PI / 2; rung.position.set(0, y, z); g.add(rung); }
    return g;
  }
  function colPlatform(R, y, mat) {
    var g = new THREE.Group(), pr = R * 1.4;
    var deck = torus(pr, 0.07, mat); deck.rotation.x = Math.PI / 2; deck.position.y = y; g.add(deck);
    var rail = torus(pr, 0.02, mat); rail.rotation.x = Math.PI / 2; rail.position.y = y + 0.55; g.add(rail);
    for (var i = 0; i < 10; i++) {
      var a = i / 10 * Math.PI * 2;
      if (a > Math.PI * 1.05 && a < Math.PI * 1.55) continue;   // gap for the ladder to pass through
      var post = cyl(0.02, 0.02, 0.55, mat, 6); post.position.set(Math.cos(a) * pr, y + 0.275, Math.sin(a) * pr); g.add(post);
    }
    return g;
  }
  function colManway(R, y, matBody, matBolt) {
    var g = new THREE.Group(), mr = R * 0.32;
    var cover = cyl(mr, mr, 0.08, matBody, 20); cover.rotation.z = Math.PI / 2; cover.position.set(R + 0.04, y, 0); g.add(cover);
    g.add(boltCircle(10, mr * 0.82, R + 0.08, 'x', matBolt));
    return g;
  }
  function liftLugs(y, mat) {
    var g = new THREE.Group();
    [1, -1].forEach(function (s) { var lug = box(0.06, 0.32, 0.05, mat); lug.position.set(s * 0.5, y, 0); g.add(lug); });
    return g;
  }
  // Concrete plinth a steel baseplate actually bolts to. Sized a bit wider
  // than the steel above it and sunk mostly below grade (y=0 is the floor
  // every other baseplate already sits flush on) so only a thin real lip
  // shows — visible detail without moving the steel, and so without
  // touching any equipment's already-tuned nozzle/port Y positions.
  function concretePad(cx, cz, w, d, lip) {
    lip = lip || 0.02;
    var h = 0.1;
    var pad = box(w, h, d, mats.concrete); pad.position.set(cx, lip - h / 2, cz); return pad;
  }
  // Circumferential weld-seam rings on a vertical shell — cheap, real detail
  // (courses of rolled plate are welded in rings, not cast as one piece)
  // that a bare cylinder never shows.
  function weldSeams(R, yBottom, yTop, n, mat) {
    var g = new THREE.Group();
    for (var i = 1; i < n; i++) {
      var y = yBottom + (yTop - yBottom) * i / n;
      var ring = torus(R + 0.006, 0.01, mat); ring.rotation.x = Math.PI / 2; ring.position.y = y; g.add(ring);
    }
    return g;
  }
  ...
  var FAC = {
    'centrifugal-pump': function () {
      ...
      // casing vent — small utility nozzle, top of volute, +Y
      var ventStem = cyl(0.15, 0.15, 0.5, mats.blue); ventStem.position.set(-1.6, 3.05, 0); g.add(ventStem);
      g.add(nozzleY(3.3, 0.0, 0.18, mats.blue, -1.6));
      // casing drain — small utility nozzle, bottom of volute, -Y
      var drainStem = cyl(0.15, 0.15, 0.35, mats.blue); drainStem.position.set(-1.6, -0.275, 0); g.add(drainStem);
      var drainR = 0.18 * 1.7;
      var drainF = cyl(drainR, drainR, 0.14, mats.steel); drainF.position.set(-1.6, -0.45, 0); g.add(drainF);
      var drainBolts = boltCircle(6, drainR * 0.78, -0.45, 'default', mats.bolt); drainBolts.position.x = -1.6; g.add(drainBolts);
      ...
      // baseplate on its own concrete foundation
      g.add(concretePad(0, 0, 5.6, 2.8));
      ...
      // Real nozzle centers (exact flange-face coords used above), not a bbox
      // guess — flangeR is likewise the ACTUAL radius each flange above was
      // drawn at, so buildPipe() never has to fall back to estimating it,
      // and a pipe of a different NPS gets a real reducer (rule 6) instead
      // of an invisibly stretched flange.
      g.userData.portDefs = [
        { id: 'suction', role: 'in', name: 'Suction', local: new THREE.Vector3(-1.6, 1.4, 1.7), dir: new THREE.Vector3(0, 0, 1), flangeR: 0.75 },
        { id: 'discharge', role: 'out', name: 'Discharge', local: new THREE.Vector3(-1.6, 3.2, 0), dir: new THREE.Vector3(0, 1, 0), flangeR: 0.85 },
        { id: 'vent', role: 'vent', name: 'Casing Vent', local: new THREE.Vector3(-1.6, 3.3, 0), dir: new THREE.Vector3(0, 1, 0), flangeR: 0.18 * 1.7 },
        { id: 'drain', role: 'drain', name: 'Casing Drain', local: new THREE.Vector3(-1.6, -0.45, 0), dir: new THREE.Vector3(0, -1, 0), flangeR: 0.18 * 1.7 }
      ];
      return g;
    },
    'sthe': function () {
      var g = new THREE.Group();
      var R = 1.6, L = 7;
      // Cutaway shell, same wedge convention as the column: a real gap
      // (facing +/-Z, unaffected by this shell's own Z-axis rotation) so
      // the tube bundle already modelled below is actually visible.
      var shell = cylArc(R, R, L, mats.steel, 32, Math.PI + 0.5, Math.PI * 2 - 1.0); shell.rotation.z = Math.PI / 2; shell.position.y = 2.4; g.add(shell);
    ...
    'v-vessel': function () {
      ...
      g.add(weldSeams(R, 2, 2 + H, 3, mats.dark));
    ...
    'h-vessel': function () {
      var g = new THREE.Group();
      var R = 1.5, L = 6;
      /* Cutaway shell, same convention as column/sthe/condenser/boiler —
         but unlike those, this one had nothing modelled inside to reveal
         (an empty tube is not a more industrial-looking empty tube with a
         hole in it). A demister pad and an inlet baffle — the same real
         internals 'ko-drum' already draws for the same equipment family —
         give the cutaway something worth showing. */
      var holeWidth = 1.15, gapStart = 0.6, gapLen = Math.PI * 2 - holeWidth;
      var sh = cylArc(R, R, L, mats.steel, 32, gapStart, gapLen); sh.rotation.z = Math.PI / 2; sh.position.y = 2.4; g.add(sh);
      g.add(dishSideNeg(R, -L / 2, 2.4, mats.steel)); g.add(dishSidePos(R, L / 2, 2.4, mats.steel));
      var inBaffle = box(0.85, 0.85, 0.06, mats.dark); inBaffle.position.set(-L / 2 + 0.7, 2.4, 0.12); g.add(inBaffle);
      var hvPad = demisterPad(R * 0.8, mats.gunmetal, 0); hvPad.rotation.z = Math.PI / 2; hvPad.position.set(L / 2 - 1.1, 2.4, 0); g.add(hvPad);
      g.add(nozzleY(2.4 + R + 0.6, 0, 0.35, mats.steel, -1.5));
      var drainNoz = cyl(0.2, 0.2, 0.6, mats.steel); drainNoz.position.set(1, 2.4 - R - 0.3, 0); g.add(drainNoz);
      g.add(saddleAt(-L / 3, R, 2.4, mats.dark)); g.add(saddleAt(L / 3, R, 2.4, mats.dark));
      g.userData.props = { Type: 'Horizontal Vessel', 'ID (mm)': 300, 'T/T (mm)': 6000, 'Design P (barg)': 8, Internals: 'Inlet baffle + wire-mesh demister pad', 'Material': 'SA-516-70' };
      g.userData.portDefs = [
        { id: 'inlet', role: 'in', name: 'Inlet', local: new THREE.Vector3(-L / 2 - 0.5, 2.4, 0), dir: new THREE.Vector3(-1, 0, 0) },
        { id: 'outlet', role: 'out', name: 'Outlet', local: new THREE.Vector3(L / 2 + 0.5, 2.4, 0), dir: new THREE.Vector3(1, 0, 0) },
        { id: 'drain', role: 'out2', name: 'Drain', local: new THREE.Vector3(1, 2.4 - R - 0.6, 0), dir: new THREE.Vector3(0, -1, 0) }
      ];
      return g;
    },
    'column': function () {
      var g = new THREE.Group();
      var R = 1.2, H = 9;
      // Cutaway shell: cylArc's thetaStart/thetaLength describe where the
      // MATERIAL is drawn, so a ~65° open wedge means the material covers
      // the other ~295° (2π - holeWidth) — confirmed against a live render
      // (an inverted holeWidth/materialLength here silently draws the
      // opposite of what's intended, an easy mistake). Visible from a real
      // orbit, not from every fixed angle, the way any cutaway window is.
      var holeWidth = 1.15, gapStart = 0.6, gapLen = Math.PI * 2 - holeWidth;
      var sh = cylArc(R, R, H, mats.purple, 32, gapStart, gapLen); sh.position.y = 2 + H / 2; g.add(sh);
      g.add(dishTop(R, 2 + H, mats.purple)); g.add(dishBot(R, 2, mats.purple));
      g.add(weldSeams(R, 2, 2 + H, 4, mats.dark));   // rolled-plate course seams
      /* Trays are near-full-diameter discs — cutting the shell alone still
         left a wall of opaque steel filling almost the whole opening one
         tray-spacing behind it, reading as solid from outside despite the
         real gap. Same wedge removed from every tray too, so the opening
         actually looks INTO the column at each level instead of just
         uncovering the next disc in line. */
      for (var tr = 0; tr < 8; tr++) { var tray = cylArc(R * 0.92, R * 0.92, 0.06, mats.steel, 32, gapStart, gapLen); tray.position.y = 2.6 + tr * (H - 1) / 8; g.add(tray); }
      g.add(nozzleY(2 + H + R + 0.6, 0, 0.3, mats.steel, 0));   // overhead
      g.add(nozzleX2(-R - 0.6, 2 + H * 0.45, 0.3, mats.steel, -1)); // feed
      g.add(nozzleX2(R + 0.6, 2 + H * 0.8, 0.25, mats.steel, 1));  // reflux return
      var bt = cyl(0.3, 0.3, 1, mats.steel); bt.position.y = 2 - 0.5; g.add(bt);
      g.add(nozzleX2(R + 0.6, 2 + 0.5, 0.28, mats.steel, 1));      // bottoms draw
      // external structure: caged ladder + access platform on the cutaway
      // side, a bolted manway lower down, lifting lugs on the top head
      g.add(colLadder(R, 2, 2 + H * 0.8, mats.dark));
      g.add(colPlatform(R, 2 + H * 0.45, mats.dark));
      g.add(colManway(R, 2 + H * 0.15, mats.steel, mats.bolt));
      g.add(liftLugs(2 + H + R * 0.4, mats.dark));
      g.userData.portDefs = [
        ...
        { id: 'reflux', role: 'in2', name: 'Reflux Return', local: new THREE.Vector3(R + 1.6, 2 + H * 0.8, 0), dir: new THREE.Vector3(1, 0, 0) },
        { id: 'bottoms', role: 'out2', name: 'Bottoms Draw', local: new THREE.Vector3(R + 1.6, 2 + 0.5, 0), dir: new THREE.Vector3(1, 0, 0) }
      ];
      // skirt
    ...
    'cone-tank': function () {
      var g = new THREE.Group();
      var R = 3, H = 4;
      // Cutaway shell + a translucent liquid-level slab inside, plus a
      // caged roof-access ladder — the same cutaway/structure treatment as
      // the column, sized up for this tank's much bigger radius.
      var holeWidth = 1.4, gapStart = 0.6, gapLen = Math.PI * 2 - holeWidth;
      var sh = cylArc(R, R, H, mats.blue, 40, gapStart, gapLen); sh.position.y = 0.2 + H / 2; g.add(sh);
      var levelMat = new THREE.MeshStandardMaterial({ color: 0xd08a3a, metalness: 0.1, roughness: 0.35, transparent: true, opacity: 0.75 });
      var level = cyl(R * 0.94, R * 0.94, 0.05, levelMat, 40); level.position.y = 0.2 + H * 0.42; g.add(level);
      ...
      g.add(colLadder(R, 0.3, 0.2 + H + 0.6, mats.dark));
    ...
    'fired-boiler': function () {                           // vertical package boiler — internal flame + coil cutaway, external ladder
      ...
      var concFloor = cyl(R * 1.15, R * 1.15, 0.1, mats.concrete, 28); concFloor.position.y = -0.03; g.add(concFloor);
      var floor = cyl(R, R, 0.2, mats.dark, 28); floor.position.y = 0.1; g.add(floor);
      // The comment above has always promised a "flame + coil cutaway", but
      // the shell was a plain opaque cylinder — the flame cone and coil
      // rings below were modelled and then hidden behind solid steel with
      // nothing ever cut away to see them. Same real-gap wedge as the
      // column/HX shells actually delivers it.
      var shell = cylArc(R, R, H, mats.blue, 28, Math.PI + 0.5, Math.PI * 2 - 1.0); shell.position.y = base + H / 2; g.add(shell);
      ...
      g.userData.portDefs = [
        { id: 'steamOut', role: 'out', name: 'Steam Outlet', local: new THREE.Vector3(-0.4, base + H + R + 0.4, 0), dir: new THREE.Vector3(0, 1, 0) },
        { id: 'bfwIn', role: 'in', name: 'Feedwater Inlet', local: new THREE.Vector3(-R - 0.8, base + 0.6, 0), dir: new THREE.Vector3(-1, 0, 0) }
      ];
      return g;
    },
    ...
    'condenser-hx': function () {                           // shell & tube condenser — teal, vapor-in-top / condensate-out-bottom
      var g = new THREE.Group();
      var R = 1.5, L = 5;
      var shell = cylArc(R, R, L, mats.teal, 32, Math.PI + 0.5, Math.PI * 2 - 1.0); shell.rotation.z = Math.PI / 2; shell.position.y = 2.2; g.add(shell);
      ...
      g.userData.portDefs = [
        { id: 'vaporIn', role: 'in', name: 'Vapor In', local: new THREE.Vector3(-L / 2 + 1, 2.2 + R + 0.6, 0), dir: new THREE.Vector3(0, 1, 0) },
        { id: 'condensate', role: 'out', name: 'Condensate Out', local: new THREE.Vector3(0, 2.2 - R - 0.9, 0), dir: new THREE.Vector3(0, -1, 0) },
        { id: 'cwIn', role: 'in2', name: 'CW In', local: new THREE.Vector3(-L / 2 - 1.6, 2.2, 0), dir: new THREE.Vector3(-1, 0, 0) },
        { id: 'cwOut', role: 'out2', name: 'CW Out', local: new THREE.Vector3(L / 2 + 1.6, 2.2, 0), dir: new THREE.Vector3(1, 0, 0) }
      ];
      return g;
    },
  ...
  A3.buildFromModel = function (nodes, pipes, nameOf) {
    ...
    /* K was raised back and forth trying to solve two conflicting problems
       with one number: 2D/P&ID need sheet positions wide enough apart that
       long process tags ("REFLUX RETURN TO C-101") don't overlap, while 3D
       wants those same equipment pieces sitting close together like a real
       compact plant. There is no single K that does both — so the 2D sheet
       positions (FLOWSHEETS' node x/y) stay at their original, spacious,
       label-friendly spread, and K alone is tuned down far enough to turn
       that same spread into a tight 3D layout independent of it. */
    var K = 0.023;                                  /* world units per sheet px */

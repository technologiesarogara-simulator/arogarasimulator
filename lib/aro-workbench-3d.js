...
(function () {
  ...
  function M() { return {
    steel:   new THREE.MeshStandardMaterial({ color: 0xb8c0cc, metalness: 0.95, roughness: 0.32, envMapIntensity: ENV_MAP_INTENSITY }),
    cs:      new THREE.MeshStandardMaterial({ color: 0x8a8f98, metalness: 0.85, roughness: 0.5, envMapIntensity: ENV_MAP_INTENSITY }),
    blue:    new THREE.MeshStandardMaterial({ color: 0x2563eb, metalness: 0.55, roughness: 0.45, envMapIntensity: ENV_MAP_INTENSITY }),
    navy:    new THREE.MeshStandardMaterial({ color: 0x1e3a8a, metalness: 0.6, roughness: 0.4, envMapIntensity: ENV_MAP_INTENSITY }),
    orange:  new THREE.MeshStandardMaterial({ color: 0xf97316, metalness: 0.5, roughness: 0.45, envMapIntensity: ENV_MAP_INTENSITY }),
    ...
    dark:    new THREE.MeshStandardMaterial({ color: 0x334155, metalness: 0.8, roughness: 0.5, envMapIntensity: ENV_MAP_INTENSITY }),
    bolt:    new THREE.MeshStandardMaterial({ color: 0x64748b, metalness: 0.9, roughness: 0.4, envMapIntensity: ENV_MAP_INTENSITY }),
    red:     new THREE.MeshStandardMaterial({ color: 0xdc2626, metalness: 0.4, roughness: 0.5, envMapIntensity: ENV_MAP_INTENSITY }),
    green:   new THREE.MeshStandardMaterial({ color: 0x16a34a, metalness: 0.5, roughness: 0.45, envMapIntensity: ENV_MAP_INTENSITY }),
    ...
    teal:    new THREE.MeshStandardMaterial({ color: 0x0d9488, metalness: 0.6, roughness: 0.4, envMapIntensity: ENV_MAP_INTENSITY }),
    purple:  new THREE.MeshStandardMaterial({ color: 0x7c3aed, metalness: 0.5, roughness: 0.4, envMapIntensity: ENV_MAP_INTENSITY }),
    gold:    new THREE.MeshStandardMaterial({ color: 0xeab308, metalness: 0.9, roughness: 0.3, envMapIntensity: ENV_MAP_INTENSITY }),
    maroon:  new THREE.MeshStandardMaterial({ color: 0x9f1239, metalness: 0.55, roughness: 0.45, envMapIntensity: ENV_MAP_INTENSITY }),
    silver:  new THREE.MeshStandardMaterial({ color: 0xd1d5db, metalness: 0.85, roughness: 0.28, envMapIntensity: ENV_MAP_INTENSITY }),
    gunmetal:new THREE.MeshStandardMaterial({ color: 0x4b5563, metalness: 0.85, roughness: 0.38, envMapIntensity: ENV_MAP_INTENSITY }),
    white:   new THREE.MeshStandardMaterial({ color: 0xf1f5f9, metalness: 0.2, roughness: 0.55, envMapIntensity: ENV_MAP_INTENSITY }),
    forest:  new THREE.MeshStandardMaterial({ color: 0x15803d, metalness: 0.5, roughness: 0.4, envMapIntensity: ENV_MAP_INTENSITY }),
    sky:     new THREE.MeshStandardMaterial({ color: 0x38bdf8, metalness: 0.4, roughness: 0.4, envMapIntensity: ENV_MAP_INTENSITY })
  }; }
  var mats;
  ...
  A3.addByType = function (type, label, nid, nps) {
    ...
    if (g.userData.noPorts) {
      ...
      g.userData.portDefs.forEach(function (pd) {
        addPort(g, pd.role, pd.local.clone(), pd.name, pd.dir, pd.id, pd.flangeR);
      });
    } else {
  ...
  function gasketAt(pos, dir, r) {
    if (!GASKET_MAT) GASKET_MAT = new THREE.MeshStandardMaterial({ color: 0x8a6a4a, metalness: 0.05, roughness: 0.85 });
    var d = dir.clone().normalize();
  ...
  function pipeSupportAt(pos, dirXZ, pipeR) {
    if (!SUPPORT_STEEL_MAT) SUPPORT_STEEL_MAT = new THREE.MeshStandardMaterial({ color: 0x475569, metalness: 0.55, roughness: 0.55 });
    var grp = new THREE.Group();
    var h = pos.y;
  ...
  function buildPipe(fromPort, toPort, color, nps, ltype, dz) {
    ...
    grp.add(flangeAt(pa, dirA, Math.max(rEqA, r)));
    grp.add(flangeAt(pb, dirB, Math.max(rEqB, r)));
  ...
  A3.buildFromModel = function (nodes, pipes, nameOf) {
    ...
    (pipes || []).forEach(function (p) {
      ...
      if (op && ip) {
        var g = buildPipe(op, ip, p.color, p.nps, p.ltype, p.dz);
  ...
  function createPipeSegment(start, end, diameter, mat) {
    ...
    var m = new THREE.Mesh(geo, mat || new THREE.MeshStandardMaterial({ color: 0xb8c0cc, metalness: 0.75, roughness: 0.4 }));
    m.position.copy(mid);

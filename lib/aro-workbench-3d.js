...
(function () {
  ...
  /* Field operator figure — a real plant reads as inhabited, with people
     ...
     low-poly/schematic (cylinders and spheres, no face or individual
     features) — a scale-and-presence marker, not a rendered portrait of
     an actual person. Hi-vis vest + white hard hat is real, standard
     plant PPE, not decoration. */
  var PERSON_MAT = null, PERSON_VEST_MAT = null, PERSON_HAT_MAT = null;
  ...
  function personFigure(scale) {
    ...
    if (!PERSON_VEST_MAT) PERSON_VEST_MAT = new THREE.MeshStandardMaterial({ color: 0xf4a825, metalness: 0.05, roughness: 0.75, envMapIntensity: ENV_MAP_INTENSITY });
    ...
    var g = new THREE.Group();
    var legL = mesh(new THREE.CylinderGeometry(0.055, 0.055, 0.85, 8), PERSON_MAT); legL.position.set(-0.09, 0.425, 0); g.add(legL);
    var legR = legL.clone(); legR.position.x = 0.09; g.add(legR);
    var torso = mesh(new THREE.CylinderGeometry(0.16, 0.14, 0.55, 10), PERSON_VEST_MAT); torso.position.set(0, 1.13, 0); g.add(torso);
    var armL = mesh(new THREE.CylinderGeometry(0.045, 0.045, 0.5, 8), PERSON_MAT); armL.position.set(-0.19, 1.1, 0); g.add(armL);
    var armR = armL.clone(); armR.position.x = 0.19; g.add(armR);
    var head = mesh(new THREE.SphereGeometry(0.11, 12, 10), PERSON_MAT); head.position.set(0, 1.52, 0); g.add(head);
    var hat = mesh(new THREE.SphereGeometry(0.115, 12, 8, 0, Math.PI * 2, 0, Math.PI / 2), PERSON_HAT_MAT); hat.position.set(0, 1.58, 0); g.add(hat);
    var brim = mesh(new THREE.CylinderGeometry(0.13, 0.13, 0.02, 16), PERSON_HAT_MAT); brim.position.set(0, 1.585, 0); g.add(brim);
  ...
  function tryMoveOperator(dx, dz) {
    var p = operatorObj.pos, nx = p.x + dx, nz = p.z + dz;
  ...
  function operatorPanelInfo() {
    ...
    return { active: true, x: operatorObj.pos.x, z: operatorObj.pos.z, camMode: operatorCamMode, location: nearestEquipmentLabel(operatorObj.pos) };
  }
  ...
  function updateOperatorCamera() {
    ...
    if (operatorCamMode === 'first') {
      ...
      camera.position.set(o.pos.x - Math.sin(o.yaw) * d, 2.6 + Math.max(0, o.pitch) * 3, o.pos.z - Math.cos(o.yaw) * d);
      camera.lookAt(o.pos.x, 1.4, o.pos.z);
    }
    if (scene && scene.fog) scene.fog.density = FOG_DENSITY_MIN;
  }
  function updateOperatorFrame(dt) {
    var o = operatorObj, k = o.keys;
    ...
    if (manual) {
      ...
      o.path = null;
      o.arriveEquip = null; o.showingEquipInfo = false; o.viewTopOnArrive = false;
      clearRouteLine();
    ...
    o.figure.position.set(o.pos.x, 0, o.pos.z);
    ...
    if (opPanelTick % 6 === 0 && !o.showingEquipInfo) pushOperatorPanel();
  }
  A3.setOperatorMode = function (on) {
    ...
    if (on) {
      if (!operatorObj) {
        ...
        scene.add(fig);
        operatorObj = { figure: fig, pos: new THREE.Vector3(), yaw: Math.PI, pitch: 0.15, path: null, keys: {}, arriveEquip: null, arriveFace: null, showingEquipInfo: false, viewTopOnArrive: false };
      }
      refreshOperatorNav();
  ...
  A3.resetOperator = function () { if (!operatorObj) return; operatorObj.pos.copy(operatorSpawn); operatorObj.path = null; operatorObj.arriveEquip = null; operatorObj.showingEquipInfo = false; operatorObj.viewTopOnArrive = false; clearRouteLine(); pushOperatorPanel(); };
  A3.debugOperator = function () { return operatorObj ? { x: operatorObj.pos.x, z: operatorObj.pos.z, yaw: operatorObj.yaw, path: operatorObj.path, circles: operatorColliders.circles.length, segs: operatorColliders.segs.length, gridCells: walkGrid ? walkGrid.cols * walkGrid.rows : 0 } : null; };
    ...
    canvas.addEventListener('click', function (e) {
      ...
      if (operatorMode) {
        ...
        if (ray.ray.intersectPlane(GROUND_PLANE, groundPt)) {
          var clickPath = findPath(operatorObj.pos.x, operatorObj.pos.z, groundPt.x, groundPt.z);
          if (!clickPath) { if (embStatus) embStatus('NO VALID OPERATOR ACCESS ROUTE — no walkable path to that point.'); }
          else {
            operatorObj.path = clickPath; operatorObj.arriveEquip = null; operatorObj.showingEquipInfo = false; operatorObj.viewTopOnArrive = false;
            updateRouteLine();
          }
        }
        return;

...
(function () {
  ...
  var PERSON_MAT = null, PERSON_COVERALL_MAT = null, PERSON_STRIPE_MAT = null, PERSON_BOOT_MAT = null, PERSON_HAT_MAT = null, PERSON_RADIO_MAT = null;
  ...
  A3.buildFromModel = function (nodes, pipes, nameOf) {
    ...
    if (objects.length) {
      ...
      var vSpot = findVehicleSpot(findOperatorSpawn());
      vehiclePos = vSpot;
  ...
  /* ============================================================
     ...
     ============================================================ */
  var ASSEMBLY_POLE_MAT = null, ASSEMBLY_FLAG_MAT = null, ASSEMBLY_RING_MAT = null;
  ...
  function buildAssemblyPointMarker(x, z) {
    ...
    grp.userData.noPorts = true;
    grp.userData.props = { Type: 'Assembly Point', Note: 'Designated muster point — the operator can be sent here from the CALL OPERATOR panel or the ASSEMBLY toolbar button.' };
    return grp;
  }
  ...
  function buildVehicle() {
    ...
    [[-0.75, -1.1], [0.75, -1.1], [-0.75, 1.1], [0.75, 1.1]].forEach(function (p) {
      var wheel = cyl(0.32, 0.32, 0.22, VEHICLE_DARK_MAT, 14); wheel.rotation.z = Math.PI / 2; wheel.position.set(p[0], 0.32, p[1]); grp.add(wheel);
    });
    var bar = box(1.6, 0.06, 0.06, VEHICLE_DARK_MAT); bar.position.set(0, 0.85, 1.55); grp.add(bar);   // front bumper/roll-bar hint
    grp.userData.noPorts = true;
    grp.userData.props = { Type: 'Site Vehicle', Note: 'Board it (within ' + VEHICLE_BOARD_RADIUS + 'm) for fast long-distance travel — ' + OPERATOR_SPEED_VEHICLE + ' m/s vs. ' + OPERATOR_SPEED + ' m/s on foot.' };
    return grp;
  }
  ...
  function pushOperatorPanel() { if (typeof A3.onOperatorPanel === 'function') A3.onOperatorPanel(operatorPanelInfo()); }
  ...
  A3.setOperatorMode = function (on) {
    ...
    if (on) {
      if (!operatorObj) {
        ...
        scene.add(fig);
        operatorObj = { figure: fig, pos: new THREE.Vector3(), yaw: Math.PI, pitch: 0.15, path: null, keys: {}, arriveEquip: null, arriveFace: null, arriveLabel: null, showingEquipInfo: false, viewTopOnArrive: false, platform: null, climb: null, climbTarget: null, afterClimb: null, inVehicle: false, boardOnArrive: false, onCall: false };
      }
      refreshOperatorNav();

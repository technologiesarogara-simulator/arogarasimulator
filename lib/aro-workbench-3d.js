...
(function () {
  ...
  var FAC = {
    ...
    'sthe': function () {
      ...
      // Real nozzle centers matching the 4 flanges built above (tube-side used
      // as the generic in/out until 3D supports multi-port equipment — P1).
      g.userData.portDefs = [
      ...
      g.userData.allPorts = [
        { name: 'Shell Inlet', local: new THREE.Vector3(-L / 2 + 1.2, 2.4 + R + 0.8, 0) },
        { name: 'Shell Outlet', local: new THREE.Vector3(L / 2 - 1.2, 2.4 - R - 0.8, 0) },
        { name: 'Tube Inlet', local: new THREE.Vector3(-L / 2 - 0.7, 2.4, 0) },
        { name: 'Tube Outlet', local: new THREE.Vector3(L / 2 + 0.7, 2.4, 0) }
      ];
      return g;
    },
    'v-vessel': function () {
  ...
  function addPort(g, role, localPos, name, dir) {
    ...
    s.userData.port = { role: role, name: name || (role === 'in' ? 'Inlet' : 'Outlet'), dir: dir ? dir.clone() : new THREE.Vector3(role === 'in' ? -1 : 1, 0, 0) };

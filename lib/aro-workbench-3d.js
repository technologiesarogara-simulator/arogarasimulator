...
(function () {
  ...
  var VEL_BANDS = [
    { max: 1.0, c: 0x2563eb, label: '<1 m/s' },
    { max: 2.5, c: 0x16a34a, label: '1–2.5 m/s' },
    { max: 4.0, c: 0xf59e0b, label: '2.5–4 m/s' },
    { max: Infinity, c: 0xdc2626, label: '>4 m/s' }
  ];
  var DP_BANDS = [
    { max: 0.05, c: 0x16a34a, label: '<0.05 bar' },
    { max: 0.2, c: 0xf59e0b, label: '0.05–0.2 bar' },
    { max: 0.5, c: 0xea580c, label: '0.2–0.5 bar' },
    { max: Infinity, c: 0xdc2626, label: '>0.5 bar' }
  ];
  var NOT_CALC_COLOR = 0x94a3b8;
  function bandColor(bands, val) {
    if (typeof val !== 'number' || !isFinite(val)) return NOT_CALC_COLOR;
    for (var i = 0; i < bands.length; i++) if (val <= bands[i].max) return bands[i].c;
  ...
  function buildFlowTubeFor(grp) {
    ...
    var curve = new THREE.CatmullRomCurve3(pd.pts, false, 'catmullrom', 0.02);
    var span = 0;
    for (var i = 1; i < pd.pts.length; i++) span += pd.pts[i].distanceTo(pd.pts[i - 1]);
    if (span < 0.05) return;
    var bore = Math.max(0.05, pd.pipeR * 0.55);
    var tube = new THREE.TubeGeometry(curve, Math.max(48, Math.round(span * 18)), bore, 12, false);
    var tex = flowTexture3D().clone();
    tex.needsUpdate = true;
    tex.wrapS = THREE.RepeatWrapping;
    var repeatX = Math.max(4, Math.round(span / Math.max(0.05, bore * 2)));
    tex.repeat.set(repeatX, 1);
    var v = pd.result && typeof pd.result.v === 'number' && isFinite(pd.result.v) ? pd.result.v : null;
    var mat = new THREE.MeshBasicMaterial({
      color: new THREE.Color(pd.color), map: tex, transparent: true, opacity: 0.95,
      depthWrite: false, side: THREE.DoubleSide
    });
    var mesh = new THREE.Mesh(tube, mat);
    mesh.renderOrder = 3;
    // rate scales with the pipe's own real velocity when RUN ANALYSIS has
    // been done; a fixed, clearly-slower default otherwise so "no data yet"
    // still animates (reads as flowing) without implying a real speed.
  ...
  A3.setFlowVizMode = function (mode) {
    ...
    if (mode === 'velocity' || mode === 'pressure') {
      pipes3d.forEach(function (grp) {
        var pd = grp.userData.pipe;
        if (!pd) return;
        var val = mode === 'velocity' ? (pd.result && pd.result.v) : (pd.result && pd.result.dp);
        if (pd.result) withResult++;
        var col = bandColor(mode === 'velocity' ? VEL_BANDS : DP_BANDS, val);
  ...
  A3.clearAll = function () { flowTubes.forEach(function (f) { if (f.mesh.parent) f.mesh.parent.remove(f.mesh); if (f.mesh.geometry) f.mesh.geometry.dispose(); if (f.mesh.material) f.mesh.material.dispose(); }); flowTubes = []; objects.forEach(function (o) { scene.remove(o); }); pipes3d.forEach(function (o) { scene.remove(o); }); clearContactShadows(); objects = []; pipes3d = []; jacketMeshes = []; portMeshes = []; spinList = []; picked = null; pickedPipe = null; pendingPort = null; portDebug = []; gapMarkers = []; connHighlighted = []; };
  ...
  function animate() {
    ...
    if (flowTubes.length) flowTubes.forEach(function (f) { f.tex.offset.x -= f.rate * 0.016; });
  ...
  function close() { if (raf) cancelAnimationFrame(raf); clearSelBox(); if (host) host.remove(); host = null; renderer && renderer.dispose && renderer.dispose(); renderer = null; objects = []; contactShadows = []; jacketMeshes = []; gapMarkers = []; spinList = []; picked = null; flowTubes = []; flowVizMode = 'off'; }

...
(function () {
  ...
  /* Field operator figure — a real plant reads as inhabited, with people
     ...
     reflective bands at the chest and ankles, dark safety boots, a white
     hard hat, and a radio held up to the head as if keying it to talk —
     the one deliberately bent, non-symmetric limb on an otherwise
     straight-limbed figure, because a hand-held walkie-talkie is
     specifically what a "real" operator figure was asked to carry, not a
     prop sitting unused on a belt clip. */
  ...
  function personFigure(scale) {
    ...
    // bent radio arm below without a real skeleton/IK system.
    ...
    // Left arm hangs naturally at the side, same coverall sleeve
    var armL = mesh(new THREE.CylinderGeometry(0.045, 0.045, 0.5, 8), PERSON_COVERALL_MAT); armL.position.set(-0.19, 1.1, 0); g.add(armL);
    // Right arm bends at the elbow to bring a hand-held radio up by the
    // head, keyed to talk — see the block comment above personFigure().
    var shoulder = { x: 0.19, y: 1.34, z: 0 }, elbow = { x: 0.24, y: 1.08, z: 0.03 }, hand = { x: 0.13, y: 1.48, z: 0.12 };
    g.add(seg(shoulder, elbow, 0.045, PERSON_COVERALL_MAT));
    g.add(seg(elbow, hand, 0.04, PERSON_COVERALL_MAT));
    var radio = box(0.055, 0.13, 0.04, PERSON_RADIO_MAT); radio.position.set(hand.x + 0.01, hand.y + 0.02, hand.z + 0.02); radio.rotation.x = -0.3; g.add(radio);
    var antenna = cyl(0.006, 0.006, 0.1, PERSON_RADIO_MAT, 6); antenna.position.set(hand.x + 0.01, hand.y + 0.115, hand.z - 0.01); g.add(antenna);
  ...
  var OPERATOR_RADIUS = 0.35, OPERATOR_SPEED = 1.7, OPERATOR_SPEED_FAST = 3.4, OPERATOR_LOOK_SENS = 0.006;
  ...
  function buildWalkGrid() {
    ...
    walkGrid = { minX: minX, minZ: minZ, cols: cols, rows: rows, spacing: WALK_GRID_SPACING, blocked: blocked };
  }
    ...
    while (heap.length && iter++ < MAX_ITER) {
      ...
      for (var dr = -1; dr <= 1; dr++) {
        for (var dc = -1; dc <= 1; dc++) {
          ...
          var tentativeG = gScore[current] + ((dc && dr) ? Math.SQRT2 : 1);
  ...
  function advanceClimb(o, dt) {
    ...
    opPanelTick++;
    if (opPanelTick % 6 === 0 && !operatorPanelSuspended) pushOperatorPanel();
  }
  function updateOperatorFrame(dt) {
    var o = operatorObj;
    if (o.climb) { advanceClimb(o, dt); return; }
    var k = o.keys;
    var speed = (k.shift ? OPERATOR_SPEED_FAST : OPERATOR_SPEED) * dt;
    ...
    if (manual) {
      ...
      o.path = null;
      o.arriveEquip = null; o.showingEquipInfo = false; o.viewTopOnArrive = false; o.climbTarget = null;
      clearRouteLine();
      ...
      if (dist < (isFinalLeg ? 0.15 : 0.45)) {
        ...
        var step = Math.min(dist, OPERATOR_SPEED * dt);
        ...
        if (!moved || (o.pos.x === before && o.pos.z === beforeZ)) {
          ...
          o.arriveEquip = null; o.climbTarget = null;
        }
      }
    }
    o.figure.position.set(o.pos.x, o.pos.y, o.pos.z);
    ...
    if (opPanelTick % 6 === 0 && !o.showingEquipInfo && !operatorPanelSuspended) pushOperatorPanel();
  }
  ...
  A3.setOperatorMode = function (on) {
    ...
    if (on) {
      if (!operatorObj) {
        ...
        scene.add(fig);
        operatorObj = { figure: fig, pos: new THREE.Vector3(), yaw: Math.PI, pitch: 0.15, path: null, keys: {}, arriveEquip: null, arriveFace: null, showingEquipInfo: false, viewTopOnArrive: false, platform: null, climb: null, climbTarget: null, afterClimb: null };
      }
      refreshOperatorNav();
      operatorObj.pos.copy(findOperatorSpawn());
      operatorSpawn.copy(operatorObj.pos);
      operatorObj.path = null; operatorObj.keys = {}; operatorObj.arriveEquip = null; operatorObj.showingEquipInfo = false;
      ...
      if (operatorMode) {
        ...
        if (ray.ray.intersectPlane(GROUND_PLANE, groundPt)) {
          ...
          if (descendIfOnPlatformThen(function () {
            ...
            if (p2) { operatorObj.path = p2; operatorObj.arriveEquip = null; operatorObj.climbTarget = null; operatorObj.showingEquipInfo = false; operatorObj.viewTopOnArrive = false; updateRouteLine(); }
          })) return;
          ...
          if (!clickPath) { if (embStatus) embStatus('NO VALID OPERATOR ACCESS ROUTE — no walkable path to that point.'); }
            ...
            operatorObj.path = clickPath; operatorObj.arriveEquip = null; operatorObj.climbTarget = null; operatorObj.showingEquipInfo = false; operatorObj.viewTopOnArrive = false;
            updateRouteLine();
          }
        }
        return;

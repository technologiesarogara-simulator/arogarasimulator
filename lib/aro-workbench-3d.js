...
(function () {
  ...
  function addPort(g, role, localPos, name, dir, id) {
    ...
    s.userData.port = { role: role, name: name || (isIn ? 'Inlet' : 'Outlet'), id: id, dir: dir ? dir.clone() : new THREE.Vector3(isIn ? -1 : 1, 0, 0) };
  ...
  A3.addByType = function (type, label, nid, nps) {
    ...
    if (g.userData.noPorts) {
      ...
      g.userData.portDefs.forEach(function (pd) {
        addPort(g, pd.role, pd.local.clone(), pd.name, pd.dir, pd.id);
      });
    } else {
  ...
  A3.buildFromModel = function (nodes, pipes, nameOf) {
    ...
    (pipes || []).forEach(function (p) {
      ...
      /* Real port identity first — what the 2D sheet actually recorded for
         this connection (p.from.portId / p.to.portId, from the shared
         registry) — falling back to the old role guess only when the id is
         absent (a type outside the first 5, or a pre-registry saved pipe
         that migratePipePortIds() couldn't resolve either). This is what
         stops a saved connection from silently landing on the WRONG real
         nozzle just because it shares a role with the one that was meant. */
      var op = (p.from && portOfId(a, p.from.portId)) || portOfRoles(a, true) || portOfRoles(a, false);
      var ip = (p.to && portOfId(b, p.to.portId)) || portOfRoles(b, false) || portOfRoles(b, true);

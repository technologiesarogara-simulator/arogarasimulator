...
(function () {
  ...
  var FAC = {
    ...
    'electrical-machine': function () {                     // TEFC motor / generator — finned barrel, terminal box, shaft
      ...
      for (var f = 0; f < 11; f++) { var fin = torus(1.14, 0.05, mats.dark); fin.rotation.y = Math.PI / 2; fin.position.set(-1.1 + f * 0.22, 1.9, 0); body.add(fin); }
    ...
    'elbow90': function () {                                // 90° pipe bend on stub ends
      var g = new THREE.Group();
      var el = mesh(new THREE.TorusGeometry(1.5, 0.45, 12, 24, Math.PI / 2), mats.steel);
      el.rotation.set(Math.PI / 2, 0, Math.PI); el.position.set(-1.5, 0, 1.5); g.add(el);
      var a1 = cyl(0.45, 0.45, 1.0, mats.steel); a1.position.set(-1.5, 0, 2.3); g.add(a1);
      var a2 = cyl(0.45, 0.45, 1.0, mats.steel); a2.rotation.x = Math.PI / 2; a2.position.set(-2.3, 0, 1.5); g.add(a2);
      g.userData.props = { Type: '90° Elbow', 'Size (NPS)': 3, Radius: 'Long radius', Material: 'A234-WPB' };
      return g;
    },
    'elbow45': function () {                                // 45° pipe bend on stub ends
      var g = new THREE.Group();
      var el = mesh(new THREE.TorusGeometry(1.8, 0.4, 12, 20, Math.PI / 4), mats.steel);
      el.rotation.set(Math.PI / 2, 0, Math.PI * 0.875); el.position.set(-1.3, 0, 0.7); g.add(el);
      var a1 = cyl(0.4, 0.4, 1.0, mats.steel); a1.position.set(-1.3 - 0.7 * 0.383, 0, 0.7 + 0.7 * 0.924); a1.rotation.x = Math.PI * 0.375; g.add(a1);
      var a2 = cyl(0.4, 0.4, 1.0, mats.steel); a2.rotation.x = Math.PI / 2; a2.position.set(-2.1, 0, 0); g.add(a2);
      g.userData.props = { Type: '45° Elbow', 'Size (NPS)': 3, Material: 'A234-WPB' };
      return g;
    },
    'tee-fitting': function () {                            // run + branch pipe tee
      var g = new THREE.Group();
      var run = cyl(0.45, 0.45, 3.4, mats.steel); run.rotation.z = Math.PI / 2; g.add(run);
      var br = cyl(0.42, 0.42, 1.7, mats.steel); br.position.y = -0.85; g.add(br);
      g.userData.props = { Type: 'Tee', 'Size (NPS)': 3, Pattern: 'Equal tee', Material: 'A234-WPB' };
      return g;
    },
    'cross-fitting': function () {                          // four-way pipe cross
      var g = new THREE.Group();
      var run = cyl(0.45, 0.45, 3.4, mats.steel); run.rotation.z = Math.PI / 2; g.add(run);
      var run2 = cyl(0.42, 0.42, 3.4, mats.steel); g.add(run2);
      g.userData.props = { Type: 'Cross', 'Size (NPS)': 3, Material: 'A234-WPB' };
      return g;
    },
    'reducer-fitting': function () {                        // concentric size-change cone
      var g = new THREE.Group();
      var cone = cyl(0.32, 0.62, 1.8, mats.steel, 22); cone.rotation.z = Math.PI / 2; g.add(cone);
      var s1 = cyl(0.32, 0.32, 0.8, mats.steel); s1.rotation.z = Math.PI / 2; s1.position.x = 1.3; g.add(s1);
      var s2 = cyl(0.62, 0.62, 0.8, mats.steel); s2.rotation.z = Math.PI / 2; s2.position.x = -1.3; g.add(s2);
  ...
  function generic(type, label) {
    ...
    else if (/^flange$/.test(t)) g = FAC['flange-fitting']();
    else if (/^pg$|^ti$|^li$/.test(t)) g = FAC['gauge']();
    else if (/^ft$|^orifice$|^rotameter$/.test(t)) g = FAC['inline-instrument']();
    else if (/pump/.test(t)) g = FAC['centrifugal-pump']();
    else if (/^pcp$|vturbine|lr-vacuum|rv-vacuum|split-case|self-prime|int-gear|twin-screw|peristaltic|pneu-diaphragm|mag-drive/.test(t)) g = FAC['centrifugal-pump']();
    ...
    else if (/sthe|shell|hx|exchanger|cooler|condenser|dphe|hairpin|economizer|chiller|heater|boiler/.test(t)) g = FAC['sthe']();

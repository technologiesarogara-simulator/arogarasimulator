...
(function () {
  ...
  A3.addByType = function (type, label, nid) {
    ...
    g.position.x = (objects.length % 5) * 8 - 16;
    g.position.z = Math.floor(objects.length / 5) * 8 - 8;
    g.userData.key = type;
  ...
  function buildPipe(fromPort, toPort) {
    ...
    for (var j = 1; j < pts.length - 1; j++) grp.add(elbow(pts[j], r, mats.dark));
    var fg = fromPort.userData.port.group, tg = toPort.userData.port.group;
    grp.userData.pipe = { from: fromPort, to: toPort, fromNid: fg.userData.nid, toNid: tg.userData.nid };
  ...
  A3.buildFromModel = function (nodes, pipes, nameOf) {
    ...
    (nodes || []).forEach(function (n) {
      ...
      // map 2D pixel layout → 3D ground plane (X right, Z depth); keep relative arrangement
      if (typeof n.x === 'number') g.position.x = (n.x + 0) / 22 - 8;
      if (typeof n.y === 'number') g.position.z = (n.y + 0) / 22 - 6;
  ...
  A3.clearAll = function () { objects.forEach(function (o) { scene.remove(o); }); pipes3d.forEach(function (o) { scene.remove(o); }); objects = []; pipes3d = []; portMeshes = []; spinList = []; picked = null; pendingPort = null; };
  ...
  function wireControls(canvas) {
    ...
    canvas.addEventListener('click', function (e) {
      ...
      // Pipe tool: connect equipment ports
      if (pipeMode) {
        var ph = ray.intersectObjects(portMeshes, false);
        if (ph.length) {
          var pm = ph[0].object;
          if (!pendingPort) { pendingPort = pm; pm.material.emissive.setHex(0xffffff); pm.scale.setScalar(1.8); if (embStatus) embStatus('Start port: ' + portLabel(pm) + ' — now click the equipment to connect it to.'); }

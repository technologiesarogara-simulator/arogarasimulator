...
(function () {
  ...
  function updateOperatorFrame(dt) {
    ...
    if (manual) {
      ...
      if (dist < 0.15) { o.target = null; }
        ...
        if (!moved || (o.pos.x === before && o.pos.z === beforeZ)) { o.target = null; if (embStatus) embStatus('NO VALID OPERATOR ACCESS ROUTE — obstacle blocks the direct path.'); }
      }
    }
    ...
    opPanelTick++;
    if (opPanelTick % 6 === 0) pushOperatorPanel();
  }
  A3.setOperatorMode = function (on) {
    ...
    if (on) {
      if (!operatorObj) {
        ...
        scene.add(fig);
        operatorObj = { figure: fig, pos: new THREE.Vector3(), yaw: Math.PI, pitch: 0.15, target: null, keys: {} };
      }
      buildOperatorColliders();
      operatorObj.pos.copy(findOperatorSpawn());
      operatorSpawn.copy(operatorObj.pos);
      operatorObj.target = null; operatorObj.keys = {};
  ...
  A3.resetOperator = function () { if (!operatorObj) return; operatorObj.pos.copy(operatorSpawn); operatorObj.target = null; pushOperatorPanel(); };
    ...
    canvas.addEventListener('click', function (e) {
      ...
      if (operatorMode) {
        ...
        if (ray.ray.intersectPlane(GROUND_PLANE, groundPt)) {
          ...
          else { operatorObj.target = { x: groundPt.x, z: groundPt.z }; }
        }
        return;
      }

...
(function () {
  ...
  /* ── A DATE FIELD THAT IS NOT A DATE FIELD ──────────────────────────────
     Everything above only works on a real <input type="date">. The project
     card's DATE box was a plain text input holding an ISO string, so it read
     "2026-08-18" and had no calendar to offer — no styling or click handler
     could give it one, because there was nothing there to open.

     Rather than fix that one box and wait for the next, any text input that
     is named as a date and either is empty or already holds an ISO date is
     promoted to a real date control. Both tests have to pass: a box called
     "dateOfIssue" holding free text is left alone, because turning it into a
     date control would throw its contents away. */
  function upgrade() {
    var n = 0;
    var list = document.querySelectorAll('input[type="text"]');
    for (var i = 0; i < list.length; i++) {
      var el = list[i];
      if (el.__aroDateUp) continue;
      var name = (el.id || '') + ' ' + (el.name || '');
      if (!/date/i.test(name)) continue;
      var v = String(el.value || '').trim();
      if (v !== '' && !/^\d{4}-\d{2}-\d{2}$/.test(v)) continue;   /* not ours to convert */
      el.__aroDateUp = true;
      try { el.type = 'date'; if (v) el.value = v; n++; } catch (e) {}
    }
    return n;
  }

  window.ARODATE = { open: open, upgrade: upgrade, fields: function () {
  ...
  function boot() {
    css(); upgrade();
    /* cards and panels are built as they are opened, so keep looking for a
       while and then whenever something is clicked */
    var n = 0;
    var iv = setInterval(function () { css(); upgrade(); if (++n > 40) clearInterval(iv); }, 700);
    document.addEventListener('click', function () { setTimeout(upgrade, 250); }, true);
  }

  ...
  if (typeof document !== 'undefined' && typeof window !== 'undefined' && window.THREE) {
    ...
    Viewer.prototype.start = function () {
      ...
      function tick() {
        ...
        /* resize() only ran on buildAssembly() — a browser zoom or window
           resize left the WebGL backing buffer at its old resolution while
           the CSS box (width:100%) reflowed underneath it, stretching/
           warping the twin until the next rebuild happened to call
           resize() again. Checking it every frame (same self-healing
           pattern aro-pumpflowviz3d.js already uses) costs one cheap
           clientWidth/clientHeight read and is a no-op when unchanged. */
        self.resize();

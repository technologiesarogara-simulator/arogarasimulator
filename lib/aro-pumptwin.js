...
(function () {
  ...
  function buildComponentManifest(p) {
    ...
    // ── Baseplate — structural only, nothing computed for it yet ──
  ...
  if (typeof document !== 'undefined' && typeof window !== 'undefined' && window.THREE) {
    ...
    function Viewer(canvas) {
      ...
      this.scene.add(key);
      var rim = new THREE.PointLight(0x38bdf8, 0.5);
    ...
    Viewer.prototype.buildAssembly = function (manifest) {
      ...
      var casingMesh = add('casing', new THREE.Mesh(new THREE.CylinderGeometry(0.30, 0.30, 0.28, 32), mat(0x334155)));
      ...
      var impellerMesh = add('impeller', new THREE.Mesh(new THREE.CylinderGeometry(0.16, 0.16, 0.05, 24), mat(0xf472b6)));
      ...
      var shaftMesh = add('shaft', new THREE.Mesh(new THREE.CylinderGeometry(0.035, 0.035, 1.7, 20), mat(0x94a3b8)));
      shaftMesh.rotation.z = Math.PI / 2;
      shaftMesh.position.set(0.05, 0, 0);

      // bearings — two rings on the shaft
      [-0.55, 0.35].forEach(function (bx) {
        var ring = add('bearings', new THREE.Mesh(new THREE.CylinderGeometry(0.09, 0.09, 0.08, 20), mat(0x64748b)));
      ...
      // coupling — short wide cylinder mid-shaft
      var couplingMesh = add('coupling', new THREE.Mesh(new THREE.CylinderGeometry(0.14, 0.14, 0.18, 24), mat(0x38bdf8)));
      ...
      // tint every mesh by its manifest verdict color, if known
      this._pickables.forEach(function (mesh) {
        var m = byId[mesh.userData.componentId];
        if (m && m.color) mesh.material.color.set(m.color);
      });
    ...
    Viewer.prototype.start = function () {
      ...
      function tick() {
        self._spin += 0.004;
        self.group.rotation.y = Math.sin(self._spin) * 0.25;

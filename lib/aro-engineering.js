...
(function () {
  ...
  var BASIS = {
    pump: {
      ...
      standards: [
        ...
        ['API 610 / ISO 13709 §5', 'Pump family and construction-class shortlisting (OH/BB/VS)'],
        ['Hydraulic Institute ANSI/HI 1.3', 'Impeller shape, vane count and eye-ratio banding by specific speed Ns'],
        ['API 610 cl. 6.10.1.4', 'Bearing L10 life minimum — 25,000 h at rated load, 16,000 h at maximum load'],
        ['API 682 / ISO 21049', 'Mechanical seal plan and face/secondary-material selection by fluid class'],
        ['NEMA MG-1 / IEC 60034', 'Motor enclosure rating, service factor and starting-method screening'],
        ['Affinity laws (Q∝N, H∝N², P∝N³)', 'VFD speed and impeller-trim exploration against the unchanged system curve']
      ],
      assumptions: [
        ...
        'Family and construction-class shortlisting ranks the built-in family/configuration database '
          + 'against this duty\'s flow, head and viscosity envelopes — it is not a vendor selection, '
          + 'and every candidate\'s reasons are shown, not only the top-ranked one.',
        'Impeller shape, vane count and eye ratio are read off Hydraulic Institute typical-range '
          + 'charts banded by specific speed Ns, then combined with the Euler exit-velocity triangle '
          + 'for D2, tip speed and the exit angle — a first-pass classification against published '
          + 'coefficients, not a vendor\'s hydraulic design. The 3D viewer\'s vane-count/angle/D2 '
          + 'sliders let you explore that geometry only; they never write back into the calculation.',
        'Material-of-construction screening reuses the fluid, temperature and pressure already '
          + 'entered for the hydraulic duty (never asked twice) against a built-in corrosivity/'
          + 'pressure-temperature-envelope table — it is a compatibility screen, not a corrosion-'
          + 'rate calculation.',
        'Shaft sizing takes lateral (radial) force from the calculated hydraulic thrust and torque '
          + 'from the calculated power and speed, then applies standard beam theory for deflection '
          + 'and first critical speed on the assumed shaft geometry.',
        'Bearing L10 life uses the ISO/ABMA basic dynamic load rating against the radial load the '
          + 'shaft calculation produced and the axial thrust the impeller-eye/differential-pressure '
          + 'estimate produced — not a measured or vendor-rated load.',
        'Mechanical seal plan, face materials and secondary-seal elastomers are ranked from a '
          + 'compatibility table keyed to the fluid class, temperature and pressure, per API 682 '
          + 'selection guidance — not a specific vendor\'s qualified seal list.',
        'Multiple-pump operation combines identical calculated units (parallel, series or duty-'
          + 'standby) against the same unchanged system curve; no transient, water-hammer or start-up '
          + 'sequencing is modelled.',
        'Positive-displacement safety screening (only when the top-ranked family is PD) checks '
          + 'relief protection and pulsation dampening against the calculated duty — it is a '
          + 'mandatory safety screen, not a full PD hydraulic sizing.'
      ],
      limits: [
        'Single-phase liquid only — no flashing, no entrained gas.',
        'The core hydraulic sizing (NPSH, head, power, nozzles) is a centrifugal-pump calculation. A '
          + 'positive-displacement duty gets safety screening only (relief protection, pulsation '
          + 'dampening, volumetric efficiency) — not a full PD sizing.',
        'Multiple identical units against a fixed system curve (parallel, series, duty-standby) are '
          + 'covered; true transient, water-hammer, and start-up sequencing are not.',
        'Slurry settling velocity is screened against a critical-velocity estimate, not a full solids-'
          + 'transport model.',
        'Nozzle sizing is by velocity band, not by nozzle-load or piping-stress analysis.',
        'Family/configuration/impeller/casing/shaft/bearing/seal/driver screenings are all first-pass '
          + 'classifications against published typical-range data or a named standard\'s clause — '
          + 'none of them replaces a vendor\'s detailed mechanical design or certified test data.'
      ],
      /* A reference shelf, not a calculation input - background on what
         Phase 2's family shortlist and Phase 4's impeller classification
         are actually choosing between, for an engineer who wants the
         reasoning behind a recommendation explained in plain terms rather
         than just a ranked list. Nothing here feeds a formula; it only
         explains one. */
      pumpTypes: [
        { name: 'End-suction centrifugal (API 610 OH2 / ASME B73.1)',
          bestFor: 'Single-stage, low-to-medium head, general process and utility transfer — the default first choice for most clean-liquid duties.',
          advantages: 'Simple, lowest cost, back pull-out design makes seal/bearing maintenance fast without disturbing piping.',
          limitations: 'Head is limited by what one impeller can produce; NPSH margin gets tighter as flow rises.',
          applications: 'Cooling water, general transfer, utility and firewater services.' },
        { name: 'Between-bearings, single or multistage (API 610 BB1/BB2/BB3)',
          bestFor: 'Higher head or higher pressure than a single overhung impeller can deliver, or where shaft stiffness matters.',
          advantages: 'Impeller supported on both sides gives a stiffer shaft and better tolerance of off-BEP operation; multistage adds head without a second machine.',
          limitations: 'Heavier, costlier, and a multistage seal/bearing arrangement is more involved to maintain.',
          applications: 'Boiler feedwater, pipeline booster, high-pressure injection.' },
        { name: 'Vertical (API 610 VS, sump / turbine style)',
          bestFor: 'Pumping from a tank, pit, sump or well where a small footprint or a submerged suction matters more than easy access.',
          advantages: 'Small footprint, self-venting on startup, no separate priming arrangement for a submerged suction.',
          limitations: 'Long shafts are more vibration-sensitive; servicing usually means pulling the whole column, not just the wet end.',
          applications: 'Cooling tower basins, lift/sump stations, deep-well and condensate service.' },
        { name: 'Positive-displacement — reciprocating (piston/plunger/diaphragm)',
          bestFor: 'A duty that needs flow essentially independent of discharge pressure, very high pressure, or accurate metering — the case this suite\'s PD safety screening (mandatory relief protection, pulsation dampening) applies to.',
          advantages: 'Flow tracks stroke rate and displacement almost regardless of pressure; can reach pressures a centrifugal machine cannot.',
          limitations: 'Pulsating flow needs dampening; relief protection is not optional — a blocked discharge has nowhere for the energy to go.',
          applications: 'Chemical injection/dosing, high-pressure cleaning, metering duties.' },
        { name: 'Positive-displacement — rotary (gear/screw/lobe/vane)',
          bestFor: 'Viscous or shear-sensitive fluids a centrifugal machine would handle poorly, at moderate pressure.',
          advantages: 'Smooth (non-pulsating) continuous flow, usually self-priming, efficiency holds up well as viscosity climbs.',
          limitations: 'Close internal clearances wear faster in an abrasive service; pressure capability is generally lower than reciprocating PD.',
          applications: 'Lube and fuel oil transfer, viscous chemicals, asphalt and polymer service.' }
      ],
      impellerTypes: [
        { name: 'Radial-flow (low specific speed Ns)',
          bestFor: 'High head, comparatively low flow — the narrow, high-Ns-avoiding band this suite\'s Phase 4 classification places at the low end of the Ns scale.',
          advantages: 'Efficient at its one design point; well suited to high-head single-stage or multistage duties.',
          limitations: 'Efficiency falls off faster away from BEP than a mixed-flow shape.',
          applications: 'Boiler feed, high-head injection, multistage pump first/interstage impellers.' },
        { name: 'Francis / mixed-flow (mid specific speed)',
          bestFor: 'The broad middle band most general process duties actually fall into — this suite\'s classification places most centrifugal duties here.',
          advantages: 'Good efficiency across a wider flow range than a pure radial shape; the general-purpose choice.',
          limitations: 'Neither the highest-head nor the highest-flow specialist — a compromise shape by design.',
          applications: 'General process transfer, most single-stage end-suction pumps.' },
        { name: 'Axial-flow / propeller (high specific speed)',
          bestFor: 'Very high flow at low head — the high-Ns end of the band.',
          advantages: 'Moves large volumes efficiently at low head where a radial impeller would be inefficient and oversized.',
          limitations: 'Cannot develop significant head; unsuitable once the duty needs real differential pressure.',
          applications: 'Flood control, circulating water, large low-head transfer.' },
        { name: 'Closed impeller (shrouded both sides)',
          bestFor: 'Clean liquids with no solids — the default assumption behind this suite\'s hydraulic model.',
          advantages: 'Best efficiency of the three constructions; the shrouds control the flow path precisely.',
          limitations: 'Wear-ring clearance is fixed at build; not tolerant of solids or fibrous material.',
          applications: 'Clean process liquids, water services, most of the duties this suite is built around.' },
        { name: 'Semi-open impeller (vanes on a backplate, no front shroud)',
          bestFor: 'A duty with occasional solids or light erosive service where some efficiency can be traded for tolerance — the construction Phase 5b\'s 3D viewer models.',
          advantages: 'Handles some solids/erosive content; front clearance can be reset by shimming as it wears, extending life without a new impeller.',
          limitations: 'Lower efficiency than closed at the same duty; that front clearance needs periodic adjustment.',
          applications: 'Light slurries, pulp/paper stock, moderately erosive services.' },
        { name: 'Open impeller (vanes only, no shroud)',
          bestFor: 'Slurries, solids-laden or fibrous fluids where clogging a shrouded impeller is the real risk.',
          advantages: 'Least prone to clogging or jamming on solids/rags; easiest of the three to clean and inspect.',
          limitations: 'Lowest efficiency of the three constructions; needs both front and back clearance maintained.',
          applications: 'Raw sewage, slurries, solids-laden industrial waste streams.' }
      ]
    },
    line: {
  ...
  var PROJECT = {
    ...
    created: null, modified: null, dirty: false, detailsDirty: false
  };
  ...
  var BAR_CSS = [
    ...
    '.aro-eb-k{letter-spacing:.10em;color:var(--text-muted);font-size:11px;font-weight:700;}',
    ...
    '.aro-ck-b{flex:0 0 74px;font-size:11px;font-weight:800;letter-spacing:.08em;text-align:center;',
    ...
    '.aro-ck-c{font-size:11px;color:var(--text-muted);letter-spacing:.06em;}',
    ...
    '.aro-sum span{font-size:11px;letter-spacing:.10em;color:var(--text-muted);}',
    ...
    '  font-size:11px;letter-spacing:.08em;color:var(--text-muted);font-weight:700;}',
    ...
    '.aro-step .s1{font-size:11px;letter-spacing:.09em;color:var(--text-muted);font-weight:700;}',
    ...
    /* ── provenance ── */
    '.aro-prov-chip{display:inline-block;font-family:var(--font-mono);font-size:10px;font-weight:800;',
    ...
    '.aro-tree-m{font-size:11px;color:var(--text-muted);}',
    '.aro-kv{display:grid;grid-template-columns:150px 1fr;gap:5px 12px;font-size:11px;}',
    '.aro-kv>i{font-style:normal;color:var(--text-muted);font-size:11px;letter-spacing:.08em;}',
  ...
  function renderBar() {
    ...
    /* Pump Sizing only — the module-selection/mechanical-design engineering
       reference library. Every other module's bar is unchanged; this button
       simply does not exist on their DOM, same pattern already used for
       whatever a given module doesn't have. */
    if (id === 'pump') {
      h += '<button class="aro-eb-btn" id="aro-eb-pumpref" title="Pump selection and mechanical-design engineering reference">PUMP REFERENCE</button>';
    }
    ...
    bind('aro-eb-pumpref', function () { if (window.AROPUMPREF) window.AROPUMPREF.open(); });
  ...
  function basisHtml(id) {
    ...
    /* A reference shelf on pump/impeller construction types - background
       for the family/impeller classifications above, not a calculation
       input. Nothing here is module-specific to any other id, so it only
       renders where the basis object actually carries it. */
    function refCards(title, items) {
      if (!items || !items.length) return '';
      return '<div class="aro-sec"><h4>' + esc(title) + '</h4>'
        + items.map(function (t) {
            return '<div style="border:1px solid var(--border-muted, #334155);border-radius:6px;padding:10px 12px;margin-bottom:8px;">'
              + '<div style="font-weight:800;margin-bottom:4px;">' + esc(t.name) + '</div>'
              + '<div style="margin-bottom:4px;">' + esc(t.bestFor) + '</div>'
              + '<div style="font-size:0.92em;"><b style="color:#16a34a;">Advantages —</b> ' + esc(t.advantages) + '</div>'
              + '<div style="font-size:0.92em;"><b style="color:#b45309;">Limitations —</b> ' + esc(t.limitations) + '</div>'
              + '<div style="font-size:0.92em;"><b style="color:#2563eb;">Typical applications —</b> ' + esc(t.applications) + '</div>'
              + '</div>';
          }).join('') + '</div>';
    }
    if (b.pumpTypes || b.impellerTypes) {
      h += '<div class="aro-sec"><h4>REFERENCE — HOW THESE CONSTRUCTIONS ARE CHOSEN</h4>'
        + '<div class="aro-note">Background for the family and impeller classifications above — what each '
        + 'construction is, why it suits (or does not suit) a given duty, and where it is normally used. '
        + 'None of this is a calculation input; it exists to make the reasoning behind a recommendation '
        + 'learnable, not just a ranked list to accept.</div></div>';
      h += refCards('PUMP FAMILY / CONSTRUCTION TYPES', b.pumpTypes);
      h += refCards('IMPELLER TYPES', b.impellerTypes);
    }
    return h;
  }
  ...
  /* ── PERFORMANCE GRAPHS AND MANUFACTURING DETAIL ────────────────────────
     These two sections existed only in each module's OWN separate "VIEW
     REPORT" button before — this report and that one were two different
     documents describing the same design. Rather than a second copy of the
     chart-drawing or TEMA-detail logic living here too (which would drift
     the moment one side changed), both pull from the exact functions the
     module's own report already used, so there is exactly one place each
     kind of content is built, however many places display it. */
  function canvasImgs(specs) {
    var out = '';
    specs.forEach(function (s) {
      try {
        var cnv = document.getElementById(s[0]);
        if (cnv && cnv.width) {
          out += '<div style="margin:0 0 10px;"><div style="font-size:11px;font-weight:700;color:#4b5563;margin-bottom:3px;">'
            + esc(s[1]) + '</div><img src="' + cnv.toDataURL('image/png')
            + '" style="width:100%;border:1px solid #d1d5db;border-radius:3px;background:#fff;" alt=""></div>';
        }
      } catch (e) {}
    });
    return out;
  }
  function graphsFor(id) {
    if (id === 'sthe') {
      return canvasImgs([
        ['sthe-temp-chart', 'Temperature profile along exchanger'],
        ['sthe-u-chart', 'Overall HTC — U comparison'],
        ['sthe-u0-chart', 'Estimated overall coefficient U₀'],
        ['sthe-clearance-chart', 'Shell−bundle clearance vs bundle diameter'],
        ['sthe-ft-chart', 'LMTD correction factor Fᵗ feasibility'],
        ['sthe-phase-chart', 'Phase-change behaviour']
      ]);
    }
    if (id === 'dphe') {
      return canvasImgs([
        ['dphe-u-chart', 'U₀ assumed vs calculated'],
        ['dphe-envelope-chart', 'Operating envelope — safety margin vs flow'],
        ['dphe-u0-band-chart', 'Estimated U₀ band by service fluid'],
        ['dphe-phase-chart', 'Phase-change behaviour']
      ]);
    }
    if (id === 'phe') {
      try {
        if (window.buildGraphs && window.pheLastResult) {
          var r = window.pheLastResult();
          if (r) return '<div style="background:#fff;">' + window.buildGraphs(r) + '</div>';
        }
      } catch (e) {}
    }
    if (id === 'pump') {
      /* This used to be only the four Chart.js canvases - the family
         selection map, the MOC pressure/temperature envelope, the affinity
         curve, the flow-visualization diagram and both 3D viewers are
         genuinely calculated/drawn results too, and were visible on the
         live panel while never once appearing in the exported report. The
         3D viewers run their own requestAnimationFrame loop, so an
         explicit render right before capture (if app.js exposed one)
         makes the snapshot deterministic instead of depending on whatever
         frame the loop last happened to leave in the buffer. */
      try { if (window.pumpPrepare3DForReportCapture) window.pumpPrepare3DForReportCapture(); } catch (e) {}
      return canvasImgs([
        ['chart-flow-head', 'Pump & system curve — flow vs head'],
        ['chart-suc-nozzle', 'Suction nozzle — velocity by size'],
        ['chart-dis-nozzle', 'Discharge nozzle — velocity by size'],
        ['chart-pump-curve', 'Predicted pump curve — head, efficiency, NPSHr'],
        ['pump-family-map-canvas', 'Family selection — flow/head envelope map'],
        ['pump-moc-chart-canvas', 'Material of construction — pressure/temperature envelope'],
        ['pump-affinity-canvas', 'Affinity laws — pump & system curve (base vs scaled)'],
        ['pump-flowviz-canvas', 'Internal flow visualization — velocity by station (schematic, not CFD)'],
        ['pump-impeller3d-canvas', 'Parametric impeller 3D viewer (schematic, not a manufacturing drawing)'],
        ['pump-twin-canvas', 'Pump digital twin — assembly with verdict color coding']
      ]);
    }
    return '';
  }
  /* Pump Hydraulics Advanced Upgrade (Phases 2-25): family/configuration/
     impeller, bill of materials, P&ID line list, operator inspection
     points, maintenance/foundation envelopes, life-cycle cost, pump
     comparison and reliability analysis, whichever of those the
     calculation actually produced for this duty — built in app.js
     (window.pumpAdvancedPhasesHTML) from results already computed
     there, not recomputed here. Empty string, and no section, for every
     module other than 'pump', or if that calculation hasn't run. */
  function pumpAdvancedDetailFor(id) {
    if (id !== 'pump' || typeof window.pumpAdvancedPhasesHTML !== 'function') return '';
    try { return window.pumpAdvancedPhasesHTML() || ''; } catch (e) { return ''; }
  }
  function pumpDecisionFlowsheetFor(id) {
    if (id !== 'pump' || typeof window.pumpDecisionFlowsheetReportHTML !== 'function') return '';
    try { return window.pumpDecisionFlowsheetReportHTML() || ''; } catch (e) { return ''; }
  }
  function mfgDetailFor(id) {
    try {
      if (id === 'sthe') {
        var g = function (elId) { var e = document.getElementById(elId); return e ? e.value : ''; };
        var front = g('sthe-front-head') || 'B', shellT = g('sthe-shell-type') || 'E', rearKey = g('sthe-rear-head') || 'fixed';
        var out = '';
        if (window.buildStheTemaComponentsHTML) out += window.buildStheTemaComponentsHTML(front, shellT, rearKey);
        if (window.buildStheTemaComparisonHTML) out += window.buildStheTemaComparisonHTML(front, shellT, rearKey);
        return out;
      }
      if (id === 'dphe') {
        var d = window.dpheReportData;
        if (d && window.buildDPHESVGDiagram && isFinite(d.Di)) {
          var svg = window.buildDPHESVGDiagram(d.Di, d.Do, d.D2, d.L, d.nHp, d.mc, d.mh, d.Tci, d.Tco, d.Thi, d.Tho, d.Q, d.Ud, d.LMTD,
            { excess: d.excessArea, hotInTube: d.hotInTube, fluidHot: d.fluidHot, fluidCold: d.fluidCold });
          return '<div style="border:1px solid #d1d5db;padding:6px;background:#fff;">' + svg + '</div>'
            + '<div style="font-size:11px;color:#4b5563;margin-top:5px;text-align:center;">System schematic — hairpin stack, flow direction and duty, drawn from this calculation.</div>';
        }
      }
      if (id === 'phe' && window.pheMechanicalHTML && window.pheLastResult) {
        var pr = window.pheLastResult();
        if (pr) return window.pheMechanicalHTML(pr);
      }
      if (id === 'pump' && window.buildPumpSVGDiagram && window.state && window.state.pump
          && window.state.pump.inputs && window.state.pump.results) {
        var svgP = window.buildPumpSVGDiagram(window.state.pump.inputs, window.state.pump.results);
        var out = '<div style="text-align:center;border:1px solid #d1d5db;padding:6px;background:#fff;">' + svgP + '</div>'
          + '<div style="font-size:11px;color:#4b5563;margin-top:5px;text-align:center;">Elevation schematic — suction/discharge arrangement, drawn from this calculation.</div>';
        try {
          var svgImp = window.ARODWG ? window.ARODWG.svgFor('pump-impeller') : null;
          if (svgImp) out += '<div style="text-align:center;border:1px solid #d1d5db;padding:6px;background:#fff;margin-top:10px;">' + svgImp + '</div>'
            + '<div style="font-size:11px;color:#4b5563;margin-top:5px;text-align:center;">Impeller detail — front view and side section, dimensioned from the Phase 4/5 screening.</div>';
        } catch (e) {}
        try {
          var svgCas = window.ARODWG ? window.ARODWG.svgFor('pump-casing') : null;
          if (svgCas) out += '<div style="text-align:center;border:1px solid #d1d5db;padding:6px;background:#fff;margin-top:10px;">' + svgCas + '</div>'
            + '<div style="font-size:11px;color:#4b5563;margin-top:5px;text-align:center;">Casing (volute) detail — plan section, dimensioned from the Phase 5 casing screening.</div>';
        } catch (e) {}
        return out;
      }
      /* Line sizing (all 5 services): the auto-generated dimensioned spool
         drawing (window.ARODWG) is always available once the calculation
         has run — it doesn't depend on the engineer having sketched
         anything. On top of it, a manual P&ID sketch (MANUAL mode in the
         workbench) is additive when present, same as before. */
      if (/^line-/.test(id)) {
        var outLine = '';
        try {
          var svgLine = window.ARODWG ? window.ARODWG.svgFor(id) : null;
          if (svgLine) outLine += '<div style="text-align:center;border:1px solid #d1d5db;padding:6px;background:#fff;">' + svgLine + '</div>'
            + '<div style="font-size:11px;color:#4b5563;margin-top:5px;text-align:center;">Piping spool — dimensioned run, fittings and supports, drawn from this calculation.</div>';
        } catch (eDwg) {}
        if (window.AROPID) {
          var i2p = null, i3p = null;
          try { i2p = window.AROPID.image2D(); } catch (ePid) {}
          try { i3p = window.AROPID.image3D(); } catch (ePid2) {}
          if (i2p) outLine += '<div style="text-align:center;margin:8px 0;"><img src="' + i2p + '" style="max-width:100%;border:1px solid #cbd5e1;border-radius:4px;"/>'
            + '<div style="font-size:11px;color:#4b5563;margin-top:3px;">2D P&amp;ID as drawn — leg lengths, sizes, tags and components</div></div>';
          if (i3p) outLine += '<div style="text-align:center;margin:8px 0;"><img src="' + i3p + '" style="max-width:100%;border:1px solid #cbd5e1;border-radius:4px;"/>'
            + '<div style="font-size:11px;color:#4b5563;margin-top:3px;">3D model of the same route — bore, run length and components follow the sketch</div></div>';
        }
        if (outLine) return outLine;
      }
      if (id === 'tank') {
        var svgTank = window.ARODWG ? window.ARODWG.svgFor('tank') : null;
        if (svgTank) return '<div style="text-align:center;border:1px solid #d1d5db;padding:6px;background:#fff;">' + svgTank + '</div>'
          + '<div style="font-size:11px;color:#4b5563;margin-top:5px;text-align:center;">General arrangement — shell, heads, nozzles and levels, drawn from this calculation.</div>';
      }
    } catch (e) {}
    return '';
  }
  ...
  function reportSections(id) {
    ...
    return {
      ...
      meta: [
        ...
        ['Software', 'AROGARA — engine v' + ENGINE_VERSION],
  ...
  function reportHtml(id) {
    ...
    h += '<div style="border-bottom:3px solid #d96b16;padding-bottom:9px;margin-bottom:14px;text-align:center;">'
      + '<img src="icon-512.png" alt="AROGARA" style="width:64px;height:64px;border-radius:10px;margin-bottom:6px;">'
      + '<div style="font-size:19px;font-weight:800;letter-spacing:.04em;">AROGARA</div>'
    ...
    function sec(title, inner, wrapId) {
      return '<div' + (wrapId ? ' id="' + wrapId + '"' : '') + ' style="margin-bottom:16px;">'
    ...
    if (dwg) {
      h += sec('6 · ENGINEERING DRAWING',
        ...
        + '<div style="font-size:11px;color:#4b5563;margin-top:5px;text-align:center;">'
        + 'Figure 1. ' + esc((MODULES[id] || {}).name || '') + ' — engineering design drawing, generated from this '
        + 'calculation. Not for fabrication.</div>', 'aro-rep-sec-dwg');
    }
    ...
    if (shot) {
      h += sec((dwg ? '7' : '6') + ' · INDUSTRIAL 3D MODEL',
        ...
        + 'Arrangement steel and access items are indicative.</div>', 'aro-rep-sec-3d');
    }
    ...
    if (iso) {
      ...
      h += sec((5 + nSec) + ' · PIPING ISOMETRIC',
        ...
        + ' — piping isometric, projected from the 3D route on standard isometric axes.</div>', 'aro-rep-sec-iso');
    }
    ...
    if (bom && bom.length) {
      ...
      h += sec((5 + nSec) + ' · BILL OF MATERIAL',
        '<div style="font-size:11px;color:#4b5563;margin-bottom:6px;">Taken from the assembled 3D '
    ...
    var graphs = graphsFor(id);
    if (graphs) {
      nSec += 1;
      h += sec((5 + nSec) + ' · PERFORMANCE GRAPHS',
        '<div style="font-size:11px;color:#4b5563;margin-bottom:6px;">The same charts the module\'s own '
        + 'results panel plots from this calculation.</div>' + graphs, 'aro-rep-sec-graphs');
    }

    var mfgDetail = mfgDetailFor(id);
    if (mfgDetail) {
      nSec += 1;
      h += sec((5 + nSec) + ' · MANUFACTURING / FABRICATION DETAIL', mfgDetail, 'aro-rep-sec-mfg');
    }

    var pumpAdvDetail = pumpAdvancedDetailFor(id);
    if (pumpAdvDetail) {
      nSec += 1;
      h += sec((5 + nSec) + ' · PUMP HYDRAULICS ADVANCED UPGRADE', pumpAdvDetail, 'aro-rep-sec-pumpadv');
    }

    var flowsheetDetail = pumpDecisionFlowsheetFor(id);
    if (flowsheetDetail) {
      nSec += 1;
      h += sec((5 + nSec) + ' · DECISION FLOWSHEET — HOW EACH KEY COMPONENT WAS SELECTED',
        '<div style="font-size:10.5px;color:#4b5563;margin-bottom:8px;">Input, what it was evaluated against, the ranked '
        + 'candidates, and which one this document records as chosen - the engineer\'s own pick where one was made, the '
        + 'system\'s recommendation otherwise.</div>' + flowsheetDetail, 'aro-rep-sec-flowsheet');
    }
    ...
    h += '<div style="margin-top:20px;border-top:1px solid #d1d5db;padding-top:8px;font-size:11px;color:#6b7280;">'
      + 'This report is a calculation record produced by AROGARA engine v' + ENGINE_VERSION
  ...
  function openReport() {
    ...
    modal('DESIGN REPORT — ' + (MODULES[id] ? MODULES[id].name : ''),
      reportToggleRow(body) + body,
  ...
  /* ── WHICH FIGURES GO IN THE REPORT ────────────────────────────────────
     "if user want they can add the 3d simulation image in report side
      other wise remove it. same case for all the graphs, 3d view,
      images report side."

     The three figure sections a report can carry — the engineering
     drawing, the industrial 3D model, the piping isometric — are each
     wrapped with a stable id by sec() above. This row shows or hides
     those wrappers directly: nothing is rebuilt, no second copy of the
     report is generated, so DOWNLOAD PDF and PRINT — which both read the
     same #aro-rep-body the checkboxes act on — see exactly what the
     screen shows. DOWNLOAD EXCEL is untouched: it was never carrying
     these figures, only the numeric tables, so there is nothing in it
     for this control to remove.

     The choice is remembered per browser, not per report, since asking
     an engineer to hide the same section every time they open a report
     is the failure this exists to fix. On first use every figure is
     shown — the report is not quietly thinner than it used to be until
     someone decides to make it that way. */
  var REPORT_TOGGLES = [
    { id: 'aro-rep-sec-3d', key: '3d', label: '3D model' },
    { id: 'aro-rep-sec-iso', key: 'iso', label: 'Piping isometric' },
    { id: 'aro-rep-sec-dwg', key: 'dwg', label: 'Engineering drawing' },
    { id: 'aro-rep-sec-graphs', key: 'graphs', label: 'Performance graphs' },
    { id: 'aro-rep-sec-mfg', key: 'mfg', label: 'Manufacturing detail' },
    /* Pump Hydraulics only — family/train-components/multi-pump/PD-safety/
       flow-viz/BOM/P&ID/inspection/maintenance/foundation/LCC/comparison,
       all folded into one block by pumpAdvancedPhasesHTML(). A no-op
       checkbox on every other module's report, exactly like the five
       above already are for a module that has no such section. */
    { id: 'aro-rep-sec-pumpadv', key: 'pumpadv', label: 'Pump advanced upgrade sections' }
  ];
  function reportPrefs() {
    try { return JSON.parse(localStorage.getItem('aro_report_figs_v1') || '{}'); }
    catch (e) { return {}; }
  }
  function setReportPref(key, on) {
    var p = reportPrefs(); p[key] = on;
    try { localStorage.setItem('aro_report_figs_v1', JSON.stringify(p)); } catch (e) {}
  }
  function reportToggleRow(bodyHtml) {
    var prefs = reportPrefs();
    /* Only offer a checkbox for a section this module's report actually
       contains — reportHtml() already builds each of these sections
       conditionally (a piping isometric with no route, pump-advanced
       detail on anything but 'pump', etc. are never written into the
       body at all), so a checkbox with no matching section id in the
       body controls nothing and only implies content that isn't there.
       Checking for the wrapper id inside the already-built body is exact
       by construction: it can never disagree with what PRINT/PDF see,
       because it is reading the same markup they read. */
    var present = REPORT_TOGGLES.filter(function (t) {
      return !bodyHtml || bodyHtml.indexOf('id="' + t.id + '"') !== -1;
    });
    var boxes = present.map(function (t) {
      var on = prefs[t.key] !== false;   /* shown unless explicitly turned off */
      return '<label style="display:inline-flex;align-items:center;gap:5px;margin-right:16px;'
        + 'font-size:11px;color:#374151;cursor:pointer;">'
        + '<input type="checkbox" class="aro-rep-tog" data-id="' + t.id + '" data-key="' + t.key + '"'
        + (on ? ' checked' : '') + '> ' + esc(t.label) + '</label>';
    }).join('');
    var fmt = (window.AROPDF_FORMAT ? window.AROPDF_FORMAT.get() : 'a4');
    var sizeBtns = ['a4', 'a3'].map(function (f) {
      var on = fmt === f;
      return '<button type="button" class="aro-rep-size" data-fmt="' + f + '" style="font-family:inherit;'
        + 'font-size:10.5px;font-weight:800;letter-spacing:.03em;padding:3px 12px;border-radius:3px;cursor:pointer;'
        + 'border:1px solid ' + (on ? '#a8500c' : '#d1d5db') + ';background:' + (on ? '#a8500c' : '#fff')
        + ';color:' + (on ? '#fff' : '#374151') + ';">' + f.toUpperCase() + '</button>';
    }).join('');
    return '<div class="aro-no-print" style="background:#f8fafc;border:1px solid #d1d5db;border-radius:4px;'
      + 'padding:8px 12px;margin-bottom:12px;font-family:Arial,Helvetica,sans-serif;'
      + 'display:flex;flex-wrap:wrap;align-items:center;gap:10px;">'
      + '<span style="font-size:10px;font-weight:800;letter-spacing:.08em;color:#a8500c;">REPORT CONTENTS</span>' + boxes
      + '<span style="flex:1 1 auto;"></span>'
      + '<span style="font-size:10px;font-weight:800;letter-spacing:.08em;color:#a8500c;">PAGE SIZE</span>'
      + '<span id="aro-rep-size-row" style="display:inline-flex;gap:4px;">' + sizeBtns + '</span>'
      + '</div>'
      + '<style>@media print{.aro-no-print{display:none !important;}}</style>';
  }
  function applyReportToggles() {
    var prefs = reportPrefs();
    REPORT_TOGGLES.forEach(function (t) {
      var sec2 = document.getElementById(t.id);
      if (sec2) sec2.style.display = (prefs[t.key] === false) ? 'none' : '';
    });
  }
  function wireReportToggles() {
    applyReportToggles();
    var boxes = document.querySelectorAll('.aro-rep-tog');
    for (var i = 0; i < boxes.length; i++) {
      boxes[i].addEventListener('change', function (e) {
        setReportPref(e.target.getAttribute('data-key'), e.target.checked);
        applyReportToggles();
      });
    }
    var sizeRow = document.getElementById('aro-rep-size-row');
    if (sizeRow && window.AROPDF_FORMAT) {
      sizeRow.querySelectorAll('.aro-rep-size').forEach(function (b) {
        b.addEventListener('click', function () {
          window.AROPDF_FORMAT.set(b.getAttribute('data-fmt'));
          sizeRow.querySelectorAll('.aro-rep-size').forEach(function (b2) {
            var on = b2 === b;
            b2.style.border = '1px solid ' + (on ? '#a8500c' : '#d1d5db');
            b2.style.background = on ? '#a8500c' : '#fff';
            b2.style.color = on ? '#fff' : '#374151';
          });
        });
      });
    }
  ...
  /* ── Generic reusable "REPORT CONTENTS" toggle row ──────────────────────
     reportToggleRow()/wireReportToggles() above are wired specifically to
     this file's own reportHtml() output and its REPORT_TOGGLES vocabulary.
     A module's own bespoke report — built entirely in app.js, outside this
     pipeline (e.g. STHE's technical/manufacturing datasheet) — still wants
     the identical show/hide-a-section pattern rather than a second, ad hoc
     implementation. These take the section list and a storage key as
     arguments instead of assuming REPORT_TOGGLES/aro_report_figs_v1, so any
     bespoke report can reuse the exact same mechanism the top-toolbar
     report uses, without touching that report's own saved preferences. */
  function genericTogglePrefs(storeKey) {
    try { return JSON.parse(localStorage.getItem(storeKey) || '{}') || {}; } catch (e) { return {}; }
  }
  function setGenericTogglePref(storeKey, key, on) {
    var p = genericTogglePrefs(storeKey); p[key] = on;
    try { localStorage.setItem(storeKey, JSON.stringify(p)); } catch (e) {}
  }
  function genericToggleRow(bodyHtml, sections, storeKey) {
    var prefs = genericTogglePrefs(storeKey);
    var present = sections.filter(function (t) {
      return !bodyHtml || bodyHtml.indexOf('id="' + t.id + '"') !== -1;
    });
    if (!present.length) return '';
    var boxes = present.map(function (t) {
      var on = prefs[t.id] !== false;   /* shown unless explicitly turned off */
      return '<label style="display:inline-flex;align-items:center;gap:5px;margin-right:16px;'
        + 'font-size:11px;color:#374151;cursor:pointer;">'
        + '<input type="checkbox" class="aro-gtog" data-store="' + esc(storeKey) + '" data-id="' + esc(t.id) + '"'
        + (on ? ' checked' : '') + '> ' + esc(t.label) + '</label>';
    }).join('');
    return '<div class="aro-no-print" style="background:#f8fafc;border:1px solid #d1d5db;border-radius:4px;'
      + 'padding:8px 12px;margin-bottom:12px;font-family:Arial,Helvetica,sans-serif;'
  ...
  function applyGenericToggles(storeKey, sections, root) {
    var prefs = genericTogglePrefs(storeKey);
    var scope = root || document;
    sections.forEach(function (t) {
      var el = scope.querySelector('#' + t.id);
      if (el) el.style.display = (prefs[t.id] === false) ? 'none' : '';
    });
  }
  function wireGenericToggles(storeKey, sections, root) {
    var scope = root || document;
    applyGenericToggles(storeKey, sections, scope);
    scope.querySelectorAll('.aro-gtog[data-store="' + storeKey + '"]').forEach(function (b) {
      b.addEventListener('change', function (e) {
        setGenericTogglePref(storeKey, e.target.getAttribute('data-id'), e.target.checked);
        applyGenericToggles(storeKey, sections, scope);
      });
    });
  }
  ...
  /* ── Storage ───────────────────────────────────────────────────────────
     This used to keep its own store (aro_projects_v1 / aro_project_current_v1),
     a whole-DOM field snapshot under a completely different schema from the
     PROJECT tab's tagged equipment/line records — an engineer could save
     from this bar and the PROJECT tab's SAVED PROJECTS list would never
     show it, or the reverse. There is now exactly one project record,
     owned by window.AROPROJECT; this bar reads and writes through it and
     keeps its own field-snapshot habit as one property on that same
     record, not a parallel document. Where AROPROJECT is not present (an
     older build, or this file loaded standalone) every function below
     degrades to a no-op rather than throwing, so the rest of the bar keeps
     working without Save/Open. */
  function haveProjectStore() { return !!(window.AROPROJECT && window.AROPROJECT.save); }

  function loadStore() {
    if (!haveProjectStore()) return {};
    try { return window.AROPROJECT.list() || {}; } catch (e) { return {}; }
  }
  ...
  /* Pulls PROJECT (this bar's small view model) back out of whatever
     AROPROJECT is currently holding open, after that record changes under
     us — a save, an open, or a switch made from the PROJECT tab itself. */
  function syncProjectFromStore(force) {
    if (!haveProjectStore()) return;
    var p = window.AROPROJECT.project();
    if (!p) return;
    /* aro-project.js boots after this file (script order in index.html),
       and the PROJECT tab can also switch or rename the open project on
       its own — so this bar cannot assume its view of PROJECT is current
       just because nothing happened through its own Save/Open UI. It is
       always safe to pull a *different* project's details in. Pulling the
       SAME project's details in is skipped only while PROJECT.detailsDirty
       is set — an edit made in this bar's own PROJECT SETTINGS panel via
       APPLY, not yet written through by SAVE. This is deliberately its own
       flag rather than the workspace-wide PROJECT.dirty, which goes true
       the instant any field on any tab changes and would otherwise block
       every sync from the moment the app is touched at all. */
    if (!force && p.id === PROJECT.id && PROJECT.detailsDirty) return;
    PROJECT.id = p.id; PROJECT.name = p.projectName || 'UNTITLED'; PROJECT.rev = p.revision || 'A';
    PROJECT.client = p.client || ''; PROJECT.engineer = p.engineer || '';
    PROJECT.note = p.description || ''; PROJECT.created = p.createdAt; PROJECT.modified = p.modifiedAt;
  }

  function doSave(asNew, name) {
    if (!haveProjectStore()) return false;
    if (asNew) {
      var newId = window.AROPROJECT.saveAs(name);
      if (!newId) return false;
    } else {
      window.AROPROJECT.setDetails({ name: PROJECT.name, client: PROJECT.client,
        engineer: PROJECT.engineer, rev: PROJECT.rev, note: PROJECT.note });
    }
    window.AROPROJECT.setFields(allFields());
    var mods = {};
    for (var k in PUB) { if (PUB[k]) mods[k] = { at: PUB[k].at, version: CALCV[k] || 0, checks: PUB[k].checks || [] }; }
    window.AROPROJECT.setModulesSnapshot(mods);
    window.AROPROJECT.setUnits((($('global-unit-system') || {}).value) || 'SI');
    window.AROPROJECT.save();
    syncProjectFromStore(true);
    PROJECT.dirty = false;
    PROJECT.detailsDirty = false;
    renderBar();
  ...
  function doOpen(pid) {
    if (!haveProjectStore()) return;
    window.AROPROJECT.loadById(pid);
    var p = window.AROPROJECT.project();
    if (!p) return;
    syncProjectFromStore(true);
    PROJECT.dirty = false;
    ...
    for (var k in (p.modulesSnapshot || {})) CALCV[k] = p.modulesSnapshot[k].version || 0;
    closeModal();
    renderBar();
    toast('Opened ' + p.projectName + ' rev ' + PROJECT.rev + ' — ' + n + ' fields restored. Re-run each module to refresh its results.');
  }
  ...
  function doNew() {
    ...
    if (haveProjectStore()) window.AROPROJECT.newProject();
    closeModal();
    renderBar();
  ...
  function openProject() {
    ...
    var keys = Object.keys(store).sort(function (a, b) {
      return String(store[b].modifiedAt || '').localeCompare(String(store[a].modifiedAt || ''));
    });
    ...
    if (!keys.length) {
      ...
      h += '<div class="aro-tree">' + keys.map(function (k) {
        ...
        return '<div class="aro-tree-r' + (k === PROJECT.id ? ' sel' : '') + '" data-open="' + k + '">'
          + '<span class="aro-tree-n">' + esc(p.projectName) + ' <span class="aro-tree-m">rev ' + esc(p.revision || 'A') + '</span></span>'
          + '<span class="aro-tree-m">' + esc(String(p.modifiedAt || '').slice(0, 16).replace('T', ' ')) + '</span>'
    ...
    bind('aro-pj-export', function () {
      var rec = haveProjectStore() ? window.AROPROJECT.project() : null;
      download(new Blob([JSON.stringify(rec || snapshot(), null, 2)], { type: 'application/json' }),
    ...
    bind('aro-pj-import', function () {
      ...
      inp.onchange = function () {
        ...
        rd.onload = function () {
          if (!haveProjectStore()) { alert('The project store is not available.'); return; }
          try {
            var p = JSON.parse(rd.result);
            var id = window.AROPROJECT.importJSON(p);
            if (!id) throw new Error('unrecognised shape');
            doOpen(id);
    ...
    if (box) box.addEventListener('click', function (e) {
      ...
      if (del) {
        e.stopPropagation();
        if (confirm('Delete this saved project? This cannot be undone.') && haveProjectStore()) {
          window.AROPROJECT.deleteById(del); openProject();
        }
        return;
      }
  ...
  document.addEventListener('keydown', function (e) {
    ...
    /* Ctrl+Z and Ctrl+Y used to be skipped whenever the focus was inside an
       input — the idea being to leave the browser's own text undo alone. But
       a data sheet is nothing BUT inputs, so the engineer was in a field
       every time they reached for the shortcut, and it never once fired.
       That is the whole of "undo and redo are not working by ctrl+z /
       ctrl+y".

       The module's undo restores every field on the sheet, which subsumes
       anything the browser's per-box text undo would have done, so it now
       runs wherever the focus is. Ctrl+Shift+Z is accepted for redo too,
       which is what a lot of people reach for. */
    else if (k === 'z' && e.shiftKey) {
      if (clickIn(activeTab(), '[id$="-redo-btn"],[id$="-redo"]')) e.preventDefault();
    }
    else if (k === 'z' || k === 'y') {
      var tab = activeTab();
  ...
  window.AROENG = {
    ...
    /* reusable "REPORT CONTENTS" toggle row for a module's own bespoke
       report (outside reportHtml()) — see the comment above their
       definitions for why this is a separate mechanism from the internal
       REPORT_TOGGLES used by openReport() itself. */
    toggleRow: genericToggleRow,
    wireToggles: wireGenericToggles
  };
  ...
  function boot() {
    injectCss();
    buildBar();
    /* Whatever project aro-project.js already has open (it owns the one
       store now) is picked up the moment renderBar first runs — see
       syncProjectFromStore(). aro-project.js boots after this file, so
       nothing is available to reopen here yet; there is nothing to lose by
       not trying, since the bar will show it as soon as it exists. */
    applyUnitLock();
    renderBar();

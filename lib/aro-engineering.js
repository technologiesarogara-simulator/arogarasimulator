...
(function () {
  ...
  function mfgDetailFor(id) {
    try {
      ...
      /* Line sizing (all 5 services): the drawing itself is optional, only
         present when the engineer sketched the route in the P&ID workbench
         (MANUAL mode) rather than typing a straight-run length — so this is
         additive when present, quietly nothing when it isn't, same as their
         own report already treats it. */
      if (/^line-/.test(id) && window.AROPID) {
        var i2p = null, i3p = null;
        try { i2p = window.AROPID.image2D(); } catch (ePid) {}
        try { i3p = window.AROPID.image3D(); } catch (ePid2) {}
        if (i2p || i3p) {
          var outPid = '';
          if (i2p) outPid += '<div style="text-align:center;margin:8px 0;"><img src="' + i2p + '" style="max-width:100%;border:1px solid #cbd5e1;border-radius:4px;"/>'
            + '<div style="font-size:11px;color:#4b5563;margin-top:3px;">2D P&amp;ID as drawn — leg lengths, sizes, tags and components</div></div>';
          if (i3p) outPid += '<div style="text-align:center;margin:8px 0;"><img src="' + i3p + '" style="max-width:100%;border:1px solid #cbd5e1;border-radius:4px;"/>'
  ...
  function reportToggleRow() {
    ...
    return '<div class="aro-no-print" style="background:#f8fafc;border:1px solid #d1d5db;border-radius:4px;'
      ...
      + '<span style="font-size:10px;font-weight:800;letter-spacing:.08em;color:#a8500c;'
      + 'margin-right:14px;">REPORT CONTENTS</span>' + boxes + '</div>'

...
(function () {
  ...
  var BASIS = {
    ...
    /* A plate exchanger shares almost none of its basis with a shell-and-tube
       unit, so it cannot share BASIS.hx: TEMA nomenclature, Kern's shell-side
       method, Sieder-Tate and Hausen tube-side correlations and the
       Bowman/Mueller/Nagle F chart all describe a bundle inside a shell and
       govern nothing in a plate pack. Everything below is what aro-phe.js
       actually evaluates. */
    phe: {
      purpose: 'Thermal and hydraulic design of a gasketed, welded or brazed plate heat '
             + 'exchanger: duty, LMTD and its correction, channel film coefficients, overall '
             + 'coefficient clean and dirty, plate count against the surface required, and '
             + 'channel plus port pressure drop on both sides.',
      standards: [
        ['Chevron-plate Nu correlation', 'Channel film coefficient as Nu = c·Re^n·Pr^(1/3), with c and n '
          + 'taken from the corrugation angle. The Kumar / Martin form for chevron plates; the '
          + 'constants here are calibrated against published gasketed-plate rating data '
          + '(water-water film coefficients in the 6–9 kW/m²·K band), not lifted from one paper'],
        ['Chevron friction factor', 'Fanning f scaled by the same corrugation angle, giving the '
          + 'channel pressure drop over the developed flow length'],
        ['ε-NTU', 'Effectiveness and NTU from the capacity-rate ratio'],
        ['Plate wall resistance', 't/k at the selected plate metal and thickness — the term that '
          + 'makes titanium, stainless and nickel differ at the same geometry'],
        ['ASME B36.10M', 'Port and nozzle bores, quoted as the nearest NPS'],
        ['Gasket temperature limits', 'Maximum continuous service temperature per elastomer, used '
          + 'to check the gasket against the hotter inlet (gasketed construction only)']
      ],
      assumptions: [
        'Steady state, single phase on both sides, no phase change.',
        'Physical properties constant, evaluated at the mean bulk temperature of each stream.',
        'No heat loss to ambient — the two duties are taken as equal.',
        'The LMTD correction is a fixed allowance, not a multipass chart: F = 0.99 counter-current '
          + 'and F = 0.90 co-current, because a plate pack approaches true counter-current flow. '
          + 'Both arrangements are evaluated every run and the better one is recommended.',
        'The channel correlation carries no viscosity-ratio correction, so a duty with a large '
          + 'bulk-to-wall viscosity difference (viscous oils, glycols cold) will read optimistic.',
        'Port pressure drop is 1.4 velocity heads per pass on each side, added to the channel drop.',
        'Fouling resistances are as entered; the design coefficient is the clean coefficient '
          + 'degraded by them, and the excess surface follows from that. Plate-exchanger fouling '
          + 'allowances are far smaller than shell-and-tube ones — a TEMA fouling factor entered '
          + 'here will oversize the unit badly.',
        'Wide-gap, free-flow and double-wall plates carry a lower film coefficient than a standard '
          + 'chevron plate of the same size; that is applied to the expected-U band, and the '
          + 'channel correlation itself is still the chevron one.',
        'All four ports share one bore, sized on the larger of the two volumetric flows.'
      ],
      limits: [
        'Thermal and hydraulic design only. No plate stress analysis, no frame or tie-bolt '
          + 'mechanical design, no fatigue assessment.',
        'No condensing or boiling correlations — a two-phase plate duty needs a specialist method.',
        'Not a substitute for a manufacturer selection. Real plate ports, corrugation patterns and '
          + 'pass arrangements come from a vendor catalogue; this sizes against a generic plate.',
        'The plate count is the surface the duty needs, rounded to the pack — it does not check '
          + 'that a frame exists to carry that many plates at the design pressure.'
      ]
    },
    tank: {
  ...
  function basisFor(id) {
    ...
    /* phe is not listed here: BASIS.phe is keyed by the module id, so the
       lookup above already returns it. */
    if (id === 'sthe' || id === 'dphe') return BASIS.hx;
    return null;
  }
  ...
  /* Pump results page (Sections 10, 11, 24, 26 only — everything else was
     removed from this page and now lives in the PUMP REFERENCE library):
     family comparison, configuration and internal flow visualization,
     whichever of those the calculation actually produced for this duty —
     built in app.js (window.pumpAdvancedPhasesHTML) from results already
     computed there, not recomputed here. Empty string, and no section,
     for every module other than 'pump', or if that calculation hasn't
     run. */
  ...
  function mfgDetailFor(id) {
    try {
      ...
      if (id === 'pump' && window.buildPumpSVGDiagram && window.state && window.state.pump
        ...
        /* Selected-pump fabrication schematic (Live Panel Step 2) — the
           same drawing the results page's own live panel shows, read from
           its cached last-render state so the report never redraws it
           from stale or missing inputs. The panel's own 3D orbit view is
           not captured here, the same way no other 3D viewport in this
           app is screenshotted into a report — a static document does not
           carry the drag/zoom interaction that makes an orbit view worth
           having. */
        try {
          var lpState = window.renderPumpLivePanel ? window.renderPumpLivePanel._last : null;
          if (lpState && window.AROPUMPLIVEPANEL && window.buildLivePumpArchetypeSVG) {
            var lpFamId = (window.pumpDecisionState && window.pumpDecisionState.override.family) || (lpState.result && lpState.result.top && lpState.result.top.id);
            var lpPanelData = window.AROPUMPLIVEPANEL.buildLivePumpPanelData({ familyId: lpFamId, duty: lpState.duty, nozzles: lpState.nozzles, moc: lpState.moc });
            if (lpPanelData.applicable) {
              var svgLp = window.buildLivePumpArchetypeSVG(lpPanelData);
              out += '<div style="text-align:center;border:1px solid #d1d5db;padding:6px;background:#fff;margin-top:10px;">' + svgLp + '</div>'
                + '<div style="font-size:11px;color:#4b5563;margin-top:5px;text-align:center;">Selected pump — fabrication schematic (' + esc(lpPanelData.archetype.label) + '), not a manufacturing drawing.</div>';
            }
          }
        } catch (e) {}
        return out;
  ...
  function reportHtml(id) {
    ...
    /* module-contributed sections, in the order the module listed them */
    var modSections = (PUB[id] && PUB[id].sections) || null;
    if (modSections) {
      modSections.forEach(function (ms, mi) {
        if (!ms || !ms.html) return;
        nSec += 1;
        h += sec((5 + nSec) + ' · ' + (ms.title || 'DETAIL'), ms.html, 'aro-rep-sec-mod' + mi);
      });
    }
  ...
  var REPORT_TOGGLES = [
    ...
    /* Pump Hydraulics only — family comparison, configuration and internal
       flow visualization, folded into one block by pumpAdvancedPhasesHTML().
       A no-op checkbox on every other module's report, exactly like the
       five above already are for a module that has no such section. */
  ...
  function publish(id, payload) {
    ...
    PUB[id] = {
      ...
      /* A module can hand the report whole sections of its own — schedules and
         tables the generic field/check/trace shape has no room for. Each entry
         is {title, html}; the report renders them in order, numbered with the
         rest. Without this a nozzle schedule or a pressure-drop build-up lives
         only in the on-screen panel, and the report — the thing that is issued,
         printed and filed — has to be read alongside the tool to be useful. */
      sections: (payload.sections && payload.sections.length) ? payload.sections.slice() : null,
      at: Date.now()
    };

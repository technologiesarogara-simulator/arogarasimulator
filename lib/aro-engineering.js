...
(function () {
  ...
  function activeModule() {
    var tab = activeTab();
    if (LAST_TAB_MODULE[tab] && MODULES[LAST_TAB_MODULE[tab]]) return LAST_TAB_MODULE[tab];
  ...
  var BAR_CSS = [
    '.aro-ebar{display:flex;align-items:center;gap:10px;padding:0 14px;height:34px;flex:0 0 34px;',
    '  background:var(--bg-panel);border-bottom:1px solid var(--border-muted);',
    '  font-family:var(--font-mono);font-size:10px;color:var(--text-muted);',
    '  overflow-x:auto;overflow-y:hidden;white-space:nowrap;user-select:none;z-index:9;}',
    '.aro-ebar::-webkit-scrollbar{height:0;}',
  ...
  function renderBar() {
    ...
    var p = PUB[id];
    var ran = p && p.at;
    var engine = !ran ? { dot: '#9ca3af', txt: 'NOT CALCULATED' }
               : (STALE[id] ? { dot: '#b7791f', txt: 'INPUTS CHANGED — RECALCULATE' }
               : { dot: '#16835b', txt: 'READY' });

    var h = '';
    h += '<div class="aro-eb-grp"><span class="aro-eb-k">PROJECT</span>'
       + '<span class="aro-eb-v" id="aro-eb-proj">' + esc(PROJECT.name) + (PROJECT.dirty ? ' *' : '') + '</span></div>';
    h += '<div class="aro-eb-sep"></div>';
    h += '<div class="aro-eb-grp"><span class="aro-eb-k">REV</span><span class="aro-eb-v">' + esc(PROJECT.rev) + '</span>'
       + '<span class="aro-eb-k" style="margin-left:6px;">CALC</span><span class="aro-eb-v">v'
       + (CALCV[id] || 0) + '</span></div>';
    h += '<div class="aro-eb-sep"></div>';
    h += '<div class="aro-eb-grp"><span class="aro-eb-k">MODULE</span><span class="aro-eb-v">'
       + esc(m.name) + '</span></div>';
    h += '<div class="aro-eb-sep"></div>';
    ...
    if (ck.length) {
      ...
      h += '<div class="aro-eb-st" id="aro-eb-status" title="Nothing has been calculated on this module yet">'
         + '<span class="aro-eb-k">NOT CALCULATED</span></div>';
    }
    ...
    bind('aro-eb-proj-btn', openProject);

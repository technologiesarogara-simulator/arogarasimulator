...
(function () {
  ...
  function Viewer(canvas) {
    ...
    /* Same fix as the live pump 3D viewer: flat single-key-light PBR
       shading with nothing to reflect is what makes a metal casing read
       as a matte plastic toy. A small procedural "studio" environment
       (canvas-drawn gradient + a few bright softbox bands, no external
       HDRI) baked through PMREMGenerator gives every MeshStandardMaterial
       part here real specular reflections. Self-contained rather than
       reaching into app.js, since this module loads before it. */
    try {
      var envCnv = document.createElement('canvas');
      envCnv.width = 512; envCnv.height = 256;
      var envCtx = envCnv.getContext('2d');
      var envGrad = envCtx.createLinearGradient(0, 0, 0, 256);
      envGrad.addColorStop(0, '#e4edf7');
      envGrad.addColorStop(0.40, '#b7c3d1');
      envGrad.addColorStop(0.50, '#828d9c');
      envGrad.addColorStop(0.60, '#4d5661');
      envGrad.addColorStop(1, '#1c1f24');
      envCtx.fillStyle = envGrad;
      envCtx.fillRect(0, 0, 512, 256);
      envCtx.fillStyle = 'rgba(255,255,255,0.6)';
      envCtx.fillRect(512 * 0.08, 256 * 0.10, 512 * 0.24, 256 * 0.11);
      envCtx.fillStyle = 'rgba(255,255,255,0.42)';
      envCtx.fillRect(512 * 0.42, 256 * 0.05, 512 * 0.30, 256 * 0.08);
      envCtx.fillStyle = 'rgba(255,255,255,0.32)';
      envCtx.fillRect(512 * 0.78, 256 * 0.22, 512 * 0.16, 256 * 0.16);
      var envTex = new THREE.CanvasTexture(envCnv);
      envTex.mapping = THREE.EquirectangularReflectionMapping;
      if (THREE.SRGBColorSpace) envTex.colorSpace = THREE.SRGBColorSpace;
      else if (THREE.sRGBEncoding) envTex.encoding = THREE.sRGBEncoding;
      var envPmrem = new THREE.PMREMGenerator(this.renderer);
      envPmrem.compileEquirectangularShader();
      var envRT = envPmrem.fromEquirectangular(envTex);
      envTex.dispose();
      envPmrem.dispose();
      this.scene.environment = envRT.texture;
    } catch (e) { /* environment reflections are cosmetic only */ }
  ...
  Viewer.prototype.start = function () {
    ...
    function tick() {
      ...
      /* Idle while the canvas is off screen — see lib/aro-raf.js. This
         ...
         animating particles and rendering every frame forever, even
         scrolled miles away. */

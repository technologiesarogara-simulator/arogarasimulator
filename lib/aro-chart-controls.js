...
(function () {
  ...
  /* Below this size a tick/title/legend label reads as "unclear" on an
     ...
     The axis TITLE ("Temperature (°C)", not the tick numbers beside it)
     gets its own, higher floor — it is the single label a reader looks
     for first to orient a chart, and it kept reading as too small even
     after the tick floor below made the numbers legible. */
  var MIN_FONT_PX = 12;
  var MIN_TITLE_FONT_PX = 14;
  function bumpFont(f, floor) {
    floor = floor || MIN_FONT_PX;
    if (!f) return { size: floor };
    var out = (typeof f === 'object') ? Object.assign({}, f) : {};
    out.size = Math.max(floor, out.size || 10);
    return out;
  }
  function bumpChartFonts(chart) {
    ...
    Object.keys(scales).forEach(function (sid) {
      ...
      if (sc.title) sc.title.font = bumpFont(sc.title.font, MIN_TITLE_FONT_PX);
    });
